...
StunDictionaryReader::StunDictionaryReader() {}

const StunAddressAttribute* StunDictionaryReader::GetAddress(int key) const {
  const StunAttribute* attr = GetOrNull(key);
  if (attr == nullptr || attr->value_type() != STUN_VALUE_ADDRESS) {
    return nullptr;
  }
  return reinterpret_cast<const StunAddressAttribute*>(attr);
}

const StunUInt32Attribute* StunDictionaryReader::GetUInt32(int key) const {
  const StunAttribute* attr = GetOrNull(key);
  if (attr == nullptr || attr->value_type() != STUN_VALUE_UINT32) {
    return nullptr;
  }
  return reinterpret_cast<const StunUInt32Attribute*>(attr);
}

const StunUInt64Attribute* StunDictionaryReader::GetUInt64(int key) const {
  const StunAttribute* attr = GetOrNull(key);
  if (attr == nullptr || attr->value_type() != STUN_VALUE_UINT64) {
    return nullptr;
  }
  return reinterpret_cast<const StunUInt64Attribute*>(attr);
}

const StunByteStringAttribute* StunDictionaryReader::GetByteString(
    int key) const {
  const StunAttribute* attr = GetOrNull(key);
  if (attr == nullptr || attr->value_type() != STUN_VALUE_BYTE_STRING) {
    return nullptr;
  }
  return reinterpret_cast<const StunByteStringAttribute*>(attr);
}

const StunUInt16ListAttribute* StunDictionaryReader::GetUInt16List(
    int key) const {
  const StunAttribute* attr = GetOrNull(key);
  if (attr == nullptr || attr->value_type() != STUN_VALUE_UINT16_LIST) {
    return nullptr;
  }
  return reinterpret_cast<const StunUInt16ListAttribute*>(attr);
}

const StunAttribute* StunDictionaryReader::GetOrNull(int key) const {
...
StunDictionaryReader::ParseDelta(const StunByteStringAttribute& update) {
  rtc::ByteBufferReader buf(update.bytes(), update.length());
  uint16_t magic;
...
StunDictionaryReader::ApplyDelta(const StunByteStringAttribute& update) {
  auto parsed_delta = ParseDelta(update);
  if (!parsed_delta.ok()) {
...
size_t StunDictionaryReader::GetLength(int key) const {
...
StunAddressAttribute* StunDictionaryWriter::SetAddress(int key) {
  auto attr = StunAttribute::CreateAddress(key);
  auto ptr = attr.get();
  Set(std::move(attr));
  return ptr;
}

StunUInt32Attribute* StunDictionaryWriter::SetUInt32(int key) {
  auto attr = StunAttribute::CreateUInt32(key);
  auto ptr = attr.get();
  Set(std::move(attr));
  return ptr;
}

StunUInt64Attribute* StunDictionaryWriter::SetUInt64(int key) {
  auto attr = StunAttribute::CreateUInt64(key);
  auto ptr = attr.get();
  Set(std::move(attr));
  return ptr;
}

StunByteStringAttribute* StunDictionaryWriter::SetByteString(int key) {
  auto attr = StunAttribute::CreateByteString(key);
  auto ptr = attr.get();
  Set(std::move(attr));
  return ptr;
}

StunUInt16ListAttribute* StunDictionaryWriter::SetUInt16List(int key) {
  auto attr = StunAttribute::CreateUInt16ListAttribute(key);
  auto ptr = attr.get();
  Set(std::move(attr));
  return ptr;
}

bool StunDictionaryWriter::Delete(int key) {
  ...
  // Create tombstone.
  auto tombstone = std::make_unique<StunByteStringAttribute>(key, nullptr, 0);
...
std::unique_ptr<StunByteStringAttribute> StunDictionaryWriter::CreateDelta() {
  ...
  buf.WriteUInt16(kDeltaMagic);    // 0,1
  buf.WriteUInt16(kDeltaVersion);  // 2,3

  // max version in Delta.
  buf.WriteUInt16(kVersionKey);            // 4,5
...
// Apply an delta ack, i.e prune list of pending changes.
