...
// modifications to B is transfered to B using the STUN_ATTR_GOOG_DELTA
 ...
 public:
  ...
  class Modification {
   ...
   private:
    ...
    Modification(const Modification<T>&);       // not copyable (but movable).
    Modification& operator=(Modification<T>&);  // not copyable (but movable).
  };
