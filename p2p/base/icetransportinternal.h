...
struct IceConfig {
  ...
  // The min time period for which a candidate pair must wait for response to
  // connectivity checks it becomes TRULY unwritable. This parameter
  // overrides the default value given by |CONNECTION_WRITE_TIMEOUT|
  // in port.h if set, when determining the writability of a candidate pair.
  absl::optional<int> ice_timeout;
  ...
  int ice_timeout_or_default() const;
