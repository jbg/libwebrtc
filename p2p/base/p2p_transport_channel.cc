...
void P2PTransportChannel::SortConnectionsIfNeeded() const {
  RTC_DCHECK_RUN_ON(network_thread_);
  /**
   * This method is needed as we might need to choose connection
   * to ping while a sort request is outstanding.
   *
   * a) Connection state change => RequestSortAndStateUpdate
   * b) The Ping selection code run and assumes a sorted list
   * c) The posted sort request comes in
   */
  if (!sort_dirty_)
    return;
