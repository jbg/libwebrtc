...
// TODO(bugs.webrtc.org/14367) remove once refactor lands.
// This is needed temporarily for ICE controller adapter to get a thread-safe
// view of the connections.
std::vector<Connection*> P2PTransportChannel::ConnectionsInternal() const {
  RTC_DCHECK_RUN_ON(network_thread_);
  return connections_;
}
...
P2PTransportChannel::IceControllerAdapter::LegacyConnections() const {
  if (active_ice_controller_) {
    return rtc::ArrayView<Connection*>(
        transport_->ConnectionsInternal().data(),
        transport_->ConnectionsInternal().size());
  }
