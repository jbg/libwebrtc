 ...
 public:
  static std::unique_ptr<UDPPort> Create(
    ...
    auto port = absl::WrapUnique(
        new UDPPort(args, LOCAL_PORT_TYPE, socket, emit_local_for_anyaddress));
  ...
  [[deprecated(
      "Pass arguments using CreatePortArgs")]] static std::unique_ptr<UDPPort>
  Create(webrtc::TaskQueueBase* thread,
    ...
    return Create({thread, factory, network, username, password, field_trials},
  ...
  static std::unique_ptr<UDPPort> Create(
    ...
    auto port = absl::WrapUnique(new UDPPort(
        args, LOCAL_PORT_TYPE, min_port, max_port, emit_local_for_anyaddress));
    port->set_stun_keepalive_delay(stun_keepalive_interval);
    if (!port->Init()) {
      return nullptr;
    }
...
class StunPort : public UDPPort {
 public:
  static std::unique_ptr<StunPort> Create(
      ...
      absl::optional<int> stun_keepalive_interval);
  [[deprecated(
      "Pass arguments using CreatePortArgs")]] static std::unique_ptr<StunPort>
  Create(webrtc::TaskQueueBase* thread,
         rtc::PacketSocketFactory* factory,
         const rtc::Network* network,
         uint16_t min_port,
 ...
 protected:
  StunPort(const CreatePortArgs& args,
           uint16_t min_port,
           uint16_t max_port,
           const ServerAddresses& servers);
};
