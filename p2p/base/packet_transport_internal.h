...
class RTC_EXPORT PacketTransportInternal : public sigslot::has_slots<> {
 public:
  ...
  void RegisterReceivedPacketCallback(void* id, F&& callback) {
    ...
    ++received_packet_callbacklist_count_;
    received_packet_callbacklist_.AddReceiver(id, std::forward<F>(callback));
  }
  void DeregisterReceivedPacketCallback(void* id) {
    RTC_DCHECK_RUN_ON(&network_checker_);
    --received_packet_callbacklist_count_;
    received_packet_callbacklist_.RemoveReceivers(id);
  }
 ...
 private:
  webrtc::CallbackList<PacketTransportInternal*, const rtc::ReceivedPacket&>
      received_packet_callbacklist_ RTC_GUARDED_BY(&network_checker_);
  int received_packet_callbacklist_count_ RTC_GUARDED_BY(&network_checker_) = 0;
};
