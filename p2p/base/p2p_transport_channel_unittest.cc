...
TEST_F(P2PTransportChannelTest,
  ...
  auto mock_async_resolver =
      std::make_unique<NiceMock<rtc::MockAsyncResolver>>();
  webrtc::MockAsyncResolverFactory mock_async_resolver_factory;
  EXPECT_CALL(mock_async_resolver_factory, Create())
  ...
  mock_async_resolver->SignalDone(mock_async_resolver.get());
  EXPECT_EQ_WAIT(LOCAL_PORT_TYPE,
                 ep2_ch1()->selected_connection()->remote_candidate().type(),
                 kMediumTimeout);
  ...
  DestroyChannels();
  rtc::AsyncResolverInterface* resolver = mock_async_resolver.get();
  ON_CALL(*mock_async_resolver, Destroy)
      .WillByDefault(testing::Invoke([resolver] { delete resolver; }));
  mock_async_resolver.release();
}
