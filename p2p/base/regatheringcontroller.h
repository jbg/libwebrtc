...
class BasicRegatheringController : public sigslot::has_slots<> {
 public:
  ...
  // A different config of the regathering interval range on all networks
  // cancels and reschedules the recurring schedules, if any, of regathering on
  // all networks. The same applies to the change of the regathering interval on
  // the failed networks. This rescheduling behavior is seperately defined for
  // the two config parameters.
 ...
 private:
  ...
  FRIEND_TEST_ALL_PREFIXES(RegatheringControllerTest,
                           ChangingConfigDoesNotCancelExistingSchedules);
  FRIEND_TEST_ALL_PREFIXES(
      RegatheringControllerTest,
      ChangingConfigCancelsExistingRecurringSchedulesAndReschedules);
