...
void UDPPort::SendStunBindingRequest(const rtc::SocketAddress& stun_addr) {
  if (stun_addr.IsUnresolvedIP()) {
    ...
    if (IsCompatibleAddress(stun_addr)) {
      ...
      stats_.sent_stun_keepalive_request_total++;
    } else {
...
void UDPPort::OnStunBindingRequestSucceeded(
  ...
  RTC_DCHECK(stats_.recv_stun_keepalive_response_total <
             stats_.sent_stun_keepalive_request_total);
  stats_.recv_stun_keepalive_response_total++;
  stats_.stun_keepalive_rtt_ms =
      (RTT_RATIO * stats_.stun_keepalive_rtt_ms + rtt_ms) / (RTT_RATIO + 1);
  stats_.stun_keepalive_rtt_ms_squared =
      (RTT_RATIO * stats_.stun_keepalive_rtt_ms_squared + rtt_ms * rtt_ms) /
      (RTT_RATIO + 1);
