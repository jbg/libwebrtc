...
class TurnEntry : public sigslot::has_slots<> {
 public:
  ...
  // Removes a connection from the list of tracked connections. If this was the
  // last connection removed, the function returns a safety flag that's used
  // to schedule the deletion of the entry after a timeout expires. If during
  // this timeout `TrackConnection` is called, the flag will be reset and
  // pending tasks associated with it, cancelled.
