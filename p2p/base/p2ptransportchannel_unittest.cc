...
TEST_F(P2PTransportChannelPingTest, TestGetState) {
  ...
  FakePortAllocator pa(rtc::Thread::Current(), nullptr);
  P2PTransportChannel ch("test channel", 1, &pa);
  EXPECT_EQ(StandardsCompliantIceTransportState::kIceTransportNew,
            ch.GetNewState());
  PrepareChannel(&ch);
  ch.MaybeStartGathering();
  EXPECT_EQ(IceTransportState::STATE_INIT, ch.GetState());
  // gathering complete with no candidates
  EXPECT_EQ(StandardsCompliantIceTransportState::kIceTransportFailed,
            ch.GetNewState());
  ch.AddRemoteCandidate(CreateUdpCandidate(LOCAL_PORT_TYPE, "1.1.1.1", 1, 100));
  ch.AddRemoteCandidate(CreateUdpCandidate(LOCAL_PORT_TYPE, "2.2.2.2", 2, 1));
  // gathering complete with candidates
  EXPECT_EQ(StandardsCompliantIceTransportState::kIceTransportCompleted,
            ch.GetNewState());
  Connection* conn1 = WaitForConnectionTo(&ch, "1.1.1.1", 1, &clock);
  Connection* conn2 = WaitForConnectionTo(&ch, "2.2.2.2", 2, &clock);
  ASSERT_TRUE(conn1 != nullptr);
