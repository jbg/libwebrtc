...
class P2PTransportChannelTestBase : public testing::Test,
 ...
 protected:
  ...
  void OnNetworkRouteChanged(IceTransportInternal* ice_transport,
    ...
    // If |rtp_overhead_per_packet| == 0, it switches to a nullptr and it
    // doesn't count. This may happen if all connections timed out.
    if (network_route.rtp_overhead_per_packet) {
...
class P2PTransportChannelPingTest : public testing::Test,
 ...
 protected:
  ...
  void OnNetworkRouteChanged(IceTransportInternal* ice_transport,
    ...
    last_sent_packet_id_ = network_route.last_sent_packet_id;
    ++selected_candidate_pair_switches_;
  }
  ...
  bool CandidatePairMatchesNetworkRoute(CandidatePairInterface* conn) {
    if (!conn) {
      return last_network_route_.rtp_overhead_per_packet == 0;
    } else {
      return conn->local_candidate().network_id() ==
                 last_network_route_.local_network_id &&
             conn->remote_candidate().network_id() ==
