...
class P2PTransportChannelTestBase : public testing::Test,
 ...
 protected:
  ...
  void OnNetworkRouteChanged(rtc::PacketTransportInternal* transport) {
    ASSERT_TRUE(transport);
    auto network_route = transport->GetNetworkRoute();
    // An overhead of 0 means that no candidate pair is selected.
...
class P2PTransportChannelPingTest : public testing::Test,
 ...
 protected:
  ...
  void OnNetworkRouteChanged(rtc::PacketTransportInternal* transport) {
    ASSERT_TRUE(transport);
    last_network_route_ = transport->GetNetworkRoute();
  ...
  bool CandidatePairMatchesNetworkRoute(CandidatePairInterface* pair) {
    if (!pair) {
      return last_network_route_.transport_overhead_per_packet == 0;
    } else {
