...
bool DatagramDtlsAdaptor::writable() const {
  // NOTE that due to order of state updates, writable_ may disagree with
  // ice_transport_->writable() if OnWritableState() was not yet called.
  // Returning ice_transport_->writable() to return most up to date state.
  // Similar implementaton as in p2p/base/dtls_transport.cc.
