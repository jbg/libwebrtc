...
// enum.
webrtc::IceCandidateType GetCandidateTypeByString(const Candidate& c) {
  if (c.is_local()) {
...
const webrtc::IceCandidatePairDescription& Connection::ToLogDescription() {
  ...
  log_description_->local_candidate_type = GetCandidateTypeByString(local);
  ...
  log_description_->remote_candidate_type = GetCandidateTypeByString(remote);
