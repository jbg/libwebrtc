...
bool LocalCandidateUsesPreferredWirelessNetwork(
  ...
  return (network_preference ==
          cricket::WirelessNetworkPreference::PREFER_EITHER) ||
         (network_type == rtc::ADAPTER_TYPE_WIFI &&
          network_preference ==
              cricket::WirelessNetworkPreference::PREFER_WIFI) ||
         (network_type == rtc::ADAPTER_TYPE_CELLULAR &&
          network_preference ==
              cricket::WirelessNetworkPreference::PREFER_CELLULAR);
}
...
void P2PTransportChannel::SetIceConfig(const IceConfig& config) {
  ...
  if (config_.network_preference != config.network_preference) {
    ...
    RTC_LOG(LS_INFO) << "Set network preference to "
                     << static_cast<int>(config_.network_preference);
  }
}
