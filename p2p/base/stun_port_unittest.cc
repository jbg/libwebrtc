...
class MockMdnsMdnsResponderProvider : public rtc::MdnsResponderProvider {
 public:
  MockMdnsMdnsResponderProvider() : mdns_responder_(new MockMdnsResponder()) {}
 ...
 public:
  ...
  StunPortTestBase(rtc::Network network, const rtc::IPAddress address)
      : ss_(new rtc::VirtualSocketServer()),
        ...
        mdns_responder_provider_(new MockMdnsMdnsResponderProvider()),
        done_(false),
        error_(false),
...
TEST_F(StunPortTest, TestStunCandidateGeneratedWithMdnsObfuscationEnabled) {
  ...
  // One of the generated condidates is a local candidate and the other is a
  // stun candidate.
  EXPECT_TRUE((port()->Candidates()[0].type() == cricket::LOCAL_PORT_TYPE &&
               port()->Candidates()[1].type() == cricket::STUN_PORT_TYPE) ||
              (port()->Candidates()[0].type() == cricket::STUN_PORT_TYPE &&
               port()->Candidates()[1].type() == cricket::LOCAL_PORT_TYPE));
}
