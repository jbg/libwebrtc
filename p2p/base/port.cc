...
bool Port::GetStunMessage(const char* data,
  ...
  if (stun_msg->type() == STUN_BINDING_REQUEST) {
    ...
    if (!ParseStunUsername(stun_msg.get(), &local_ufrag, &remote_ufrag) ||
      ...
      LOG_J(LS_ERROR, this)
          << "Received STUN request with bad local username " << local_ufrag
          << " from " << addr.ToSensitiveString();
    ...
    if (!stun_msg->ValidateMessageIntegrity(data, size, password_)) {
      LOG_J(LS_ERROR, this)
          << "Received STUN request with bad M-I "
          << "from " << addr.ToSensitiveString() << ", password_=" << password_;
      SendBindingErrorResponse(stun_msg.get(), addr, STUN_ERROR_UNAUTHORIZED,
                               STUN_ERROR_REASON_UNAUTHORIZED);
      return true;
    }
...
void Connection::set_write_state(WriteState value) {
  ...
  if (value != old_value) {
    LOG_J(LS_VERBOSE, this)
        << "set_write_state from: " << old_value << " to " << value;
...
void Connection::set_connected(bool value) {
  ...
  if (value != old_value) {
    LOG_J(LS_VERBOSE, this)
        << "set_connected from: " << old_value << " to " << value;
    SignalStateChange(this);
  }
}
...
void Connection::OnSendStunPacket(const void* data,
  ...
  if (err < 0) {
    LOG_J(LS_WARNING, this)
        ...
        << " err=" << err << " id=" << rtc::hex_encode(req->id());
  }
}
...
void Connection::UpdateState(int64_t now) {
  ...
  if (LOG_CHECK_LEVEL(LS_VERBOSE)) {
    ...
    LOG_J(LS_VERBOSE, this)
        ...
        << ", ms since last received data=" << now - last_data_received_
        << ", rtt=" << rtt << ", pings_since_last_response=" << pings;
  }
...
void Connection::ReceivedPingResponse(int rtt, const std::string& request_id) {
  ...
  current_round_trip_time_ms_ =
      rtc::Optional<uint32_t>(static_cast<uint32_t>(rtt));
...
void Connection::OnConnectionRequestErrorResponse(ConnectionRequest* request,
  ...
  if (error_code == STUN_ERROR_UNKNOWN_ATTRIBUTE ||
    ...
    LOG_J(LS_ERROR, this) << "Received STUN error response, code=" << error_code
                          << "; killing connection";
    FailAndDestroy();
  }
}
...
void Connection::OnConnectionRequestTimeout(ConnectionRequest* request) {
  ...
  rtc::LoggingSeverity sev = writable() ? rtc::LS_INFO : rtc::LS_VERBOSE;
  LOG_JV(sev, this) << "Timing-out STUN ping " << rtc::hex_encode(request->id())
