...
RtpTransportAdapter::CreateSrtpProxied(
  ...
  std::unique_ptr<RtpTransportAdapter> transport_adapter;
  rtp_transport_controller->network_thread()->Invoke<void>(
      RTC_FROM_HERE, [&]() mutable {
        transport_adapter.reset(new RtpTransportAdapter(
            parameters.rtcp, rtp, rtcp, rtp_transport_controller,
...
RTCError RtpTransportAdapter::SetParameters(
  ...
  // If the CNAME is empty, use the exist one.
