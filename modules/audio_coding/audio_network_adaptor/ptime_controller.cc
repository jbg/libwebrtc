...
PtimeController::PtimeController(
    ...
    : encoder_frame_lengths_ms_(encoder_frame_lengths_ms.begin(),
      ...
      use_stable_target_bitrate_(use_stable_target_bitrate) {}

void PtimeController::UpdateNetworkMetrics(
  ...
  if (network_metrics.uplink_bandwidth_bps) {
    uplink_bandiwdth_bps_ = network_metrics.uplink_bandwidth_bps;
  }
}

void PtimeController::MakeDecision(AudioEncoderRuntimeConfig* config) {
  if (!target_bitrate_bps_ || !overhead_bytes_per_packet_ ||
      !uplink_bandiwdth_bps_) {
    return;
  }

  auto it =
      absl::c_find_if(encoder_frame_lengths_ms_, [&](int frame_length_ms) {
        int target = use_stable_target_bitrate_ ? uplink_bandiwdth_bps_.value()
                                                : target_bitrate_bps_.value();
        return target - OverheadBps(frame_length_ms) > min_payload_bitrate_bps_;
      });

  // Longest frame length is chosen if none match our criteria.
  int frame_length_ms = it != encoder_frame_lengths_ms_.end()
                            ? *it
                            : encoder_frame_lengths_ms_.back();
  config->frame_length_ms = frame_length_ms;
  config->bitrate_bps =
      target_bitrate_bps_.value() - OverheadBps(frame_length_ms);
}

int PtimeController::OverheadBps(int frame_length_ms) const {
  return overhead_bytes_per_packet_.value() * 8 * 1000 / frame_length_ms;
}
