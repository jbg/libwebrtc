...
void PtimeController::MakeDecision(AudioEncoderRuntimeConfig* config) {
  ...
  auto it =
      absl::c_find_if(encoder_frame_lengths_ms_, [&](int frame_length_ms) {
        int target = use_stable_target_bitrate_ ? *uplink_bandiwdth_bps_
                                                : *target_bitrate_bps_;
  ...
  config->bitrate_bps =
      std::max(0, *target_bitrate_bps_ - OverheadBps(frame_length_ms));
}

int PtimeController::OverheadBps(int frame_length_ms) const {
  return *overhead_bytes_per_packet_ * 8 * 1000 / frame_length_ms;
}
