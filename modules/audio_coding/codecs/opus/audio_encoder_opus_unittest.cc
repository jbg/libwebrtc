...
TEST(AudioEncoderOpusTest, OpusDtxFilteringHighEnergyRefreshPackets) {
  ...
  while (rtp_timestamp < kMaxLoopLengthSaples) {
    encoded.Clear();
    // Every second call to the encoder will generate an Opus packet.
    for (int j = 0; j < 2; j++) {
      auto next_frame = audio_loop.GetNextBlock();
      info = encoder->Encode(rtp_timestamp, next_frame, &encoded);
      if (rtp_timestamp >= kMaxLoopLengthSaples - kSilenceDurationSamples) {
        size_t silence_frame_start =
            rtp_timestamp - (kMaxLoopLengthSaples - kSilenceDurationSamples);
  ...
  // be in a DTX state after encoding it. That is signalled by setting the flag
  // speech equal to 0.
