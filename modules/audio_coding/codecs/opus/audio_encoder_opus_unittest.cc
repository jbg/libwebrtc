...
TEST(AudioEncoderOpusTest, OpusDtxFilteringHighEnergyRefreshPackets) {
  ...
  constexpr size_t kInputBlockSizeSamples = 0.01f * kSampleRateHz;
  EXPECT_TRUE(audio_loop.Init(kInputFileName, kMaxLoopLengthSaples,
                              kInputBlockSizeSamples));
  AudioEncoder::EncodedInfo info;
  rtc::Buffer encoded(500);
  // Encode the audio file except the last part that corresponds to silence.
  constexpr size_t kSilenceDurationSamples = kSampleRateHz * 0.2f;
  uint32_t rtp_timestamp = 0;
  while (rtp_timestamp < kMaxLoopLengthSaples - kSilenceDurationSamples) {
    encoded.Clear();
    // Every second call to the encoder will generate an Opus packet.
    for (int j = 0; j < 2; j++) {
      info =
  ...
  // Giving the test file `audio_coding/testfile16kHz` we expect the encoder to
  // be in a DTX state after those encoded 11.4 seconds which is signalled by
  // setting the flag speech equal to 0.
  EXPECT_TRUE(info.speech == 0);

  // The remaining silence in the test file is copied to an array.
  constexpr size_t kNumberSilenceFrames =
      kSilenceDurationSamples / kInputBlockSizeSamples;
  std::array<int16_t, kSilenceDurationSamples> silence;
  for (size_t i = 0; i < kNumberSilenceFrames; i++) {
    auto next_frame = audio_loop.GetNextBlock();
    std::copy(next_frame.begin(), next_frame.end(),
              silence.begin() + i * kInputBlockSizeSamples);
  }

  // 6 silence bursts are fed to the encoder, the first three ones with a larger
  // energy and the last three with a lower energy. This test verifies that the
  // encoder just sends refresh DTX packets during the last bursts.
  ...
  for (size_t burst = 0; burst < 6; ++burst) {
    for (size_t i = 0; i < kNumberSilenceFrames; i++) {
      ...
      for (int j = 0; j < 2; j++) {
        ...
        std::transform(silence.begin() + i * kInputBlockSizeSamples,
                       silence.begin() + (i + 1) * kInputBlockSizeSamples,
                       silence_frame.begin(),
                       [gain](float s) { return gain * s; });
