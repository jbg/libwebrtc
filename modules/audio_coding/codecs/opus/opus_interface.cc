...
constexpr char kMinimizeNumberRefreshDtxPacketsFieldTrial[] =
    "WebRTC-Audio-OpusMinimizeNumberRefreshDtxPackets";
...
// whose energy is larger than the expected for no activity packets.
static bool WebRtcOpus_IsHighEnergyRefreshDtxPacket(
  ...
  if (number_frames > 0 &&
    ...
    if (WebRtcOpus_GetInDtx(inst) == 1 &&
      ...
      // than the smooth energy of non activity frames (with a 3 dB margin) and,
      // therefore, it is flagged as a high energy refresh DTX packet.
      return true;
    }
...
int16_t WebRtcOpus_EncoderCreate(OpusEncInst** inst,
  ...
  state->minimize_number_refresh_dtx_packets = webrtc::field_trial::IsEnabled(
      kMinimizeNumberRefreshDtxPacketsFieldTrial);
...
int16_t WebRtcOpus_MultistreamEncoderCreate(
  ...
  state->sample_rate_hz = sample_rate_hz;
  state->smooth_energy_non_active_frames = 0.0f;
  state->minimize_number_refresh_dtx_packets = false;
...
int WebRtcOpus_Encode(OpusEncInst* inst,
  ...
  if (inst->minimize_number_refresh_dtx_packets &&
      WebRtcOpus_GetUseDtx(inst) == 1 &&
