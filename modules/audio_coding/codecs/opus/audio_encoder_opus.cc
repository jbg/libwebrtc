...
AudioEncoder::EncodedInfo AudioEncoderOpusImpl::EncodeImpl(
  ...
  int has_voice = WebRtcOpus_PacketHasVoiceActivity(&encoded->data()[primary_offset], info.encoded_bytes);
  if (has_voice == -1) {
    // CELT mode packet or there was an error. This had set the speech flag to true historically.
    info.speech = true;
  } else {
