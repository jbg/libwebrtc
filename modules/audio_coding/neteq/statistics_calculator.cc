...
void StatisticsCalculator::ConcealedSamplesCorrection(int num_samples,
  ...
  if (is_voice) {
    const size_t canceled_out = std::min(static_cast<size_t>(num_samples),
                                         voice_concealed_samples_correction_);
    voice_concealed_samples_correction_ -= canceled_out;
    lifetime_stats_.voice_concealed_samples += num_samples - canceled_out;
  }
}
