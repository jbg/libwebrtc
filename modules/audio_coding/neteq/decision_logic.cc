...
NetEq::Operation DecisionLogic::GetDecision(const NetEqStatus& status,
  ...
  const int target_level_samples =
      delay_manager_->TargetDelayMs() * sample_rate_khz_;
...
absl::optional<int> DecisionLogic::PacketArrived(
  ...
  packet_arrival_history_.Insert(info.main_timestamp);
...
NetEq::Operation DecisionLogic::CngOperation(
  ...
  int optimal_level_samp = delay_manager_->TargetDelayMs() * sample_rate_khz_;
...
int DecisionLogic::GetNextPacketDelayMs(
    NetEqController::NetEqStatus status) const {
  if (config_.enable_stable_playout_delay) {
    return packet_arrival_history_.GetDelayMs(status.next_packet->timestamp);
  }
...
int DecisionLogic::GetPlayoutDelayMs(
  ...
  return buffer_level_filter_->filtered_current_level() / sample_rate_khz_;
}
...
int DecisionLogic::HighThreshold() const {
  if (config_.enable_stable_playout_delay) {
    return std::max(delay_manager_->TargetDelayMs(),
...
int DecisionLogic::LowThresholdCng() const {
  ...
  return std::max(0,
                  delay_manager_->TargetDelayMs() - kTargetLevelWindowMs / 2);
}
...
int DecisionLogic::HighThresholdCng() const {
  ...
  return delay_manager_->TargetDelayMs() + kTargetLevelWindowMs / 2;
}
