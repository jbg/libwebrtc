...
NetEq::Operation DecisionLogic::ExpectedPacketAvailable(
  ...
  if (!disallow_time_stretching_ && status.last_mode != NetEq::Mode::kExpand &&
    ...
    if (precise_jitter_buffer_target_mode_) {
      ...
      if (TimescaleAllowed()) {
        if (playout_delay_ms >
            target_delay_ms + kDelayAdjustmentGranularityMs / 2) {
...
NetEq::Operation DecisionLogic::FuturePacketAvailable(
    NetEqController::NetEqStatus status) {
  int targetLevelWindowMs = kTargetLevelWindowMs;

  if (precise_jitter_buffer_target_mode_) {
    targetLevelWindowMs = kDelayAdjustmentGranularityMs;
  ...
  if (config_.combine_concealment_decision || IsCng(status.last_mode)) {
    ...
    const int high_limit = TargetLevelMs() + targetLevelWindowMs / 2;
    const int low_limit =
        std::max(0, TargetLevelMs() - targetLevelWindowMs / 2);
