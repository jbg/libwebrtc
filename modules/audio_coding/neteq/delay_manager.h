...
class DelayManager {
 ...
 private:
  // Exposes CalculateEffectiveMinimumDelay for testing.
  FRIEND_TEST_ALL_PREFIXES(DelayManagerTest,
                           BaseMinimumDelayLowerThanMinimumDelay);
  FRIEND_TEST_ALL_PREFIXES(DelayManagerTest,
                           BaseMinimumDelayGreaterThanMinimumDelay);
  FRIEND_TEST_ALL_PREFIXES(DelayManagerTest,
                           BaseMinimumDelayGreaterThanBufferSizeIsClamped);
  FRIEND_TEST_ALL_PREFIXES(DelayManagerTest,
                           BaseMinimumDelayLowerThanMaxSizeIsUsed);
  FRIEND_TEST_ALL_PREFIXES(DelayManagerTest,
                           BaseMinimumDelayGreaterThanMaximumDelayIsClamped);

  // Provides delay which is used by LimitTargetLevel as lower bound on target
  // delay based on current |minimum_delay_ms_|, |base_minimum_dealy_ms_| and
  // |maximum_delay_ms_| and buffer size.
  int CalculateEffectiveMinimumDelay() const;
  ...
  int CalculateMinimumDelayUpperBound() const;

  // Provides 75% of currently possible maximum buffer size in milliseconds.
  int CalculateMaxBufferTimeQ75() const;
