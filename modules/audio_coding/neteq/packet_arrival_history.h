...
class PacketArrivalHistory {
 public:
  ...
  // Get delay for `rtp_timestamp` normalized against the earliest received
  // timestamp in history.
  int GetDelayMs(uint32_t rtp_timestamp) const;

  // Get the maximum packet delay observed in history.
 ...
 private:
  struct PacketArrival {
    ...
    // Unwrapped rtp timestamp scaled to miliseconds.
