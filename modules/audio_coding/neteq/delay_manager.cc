...
// The lower limit is equivalent of |CalculateEffectiveMinimumDelay| in packets.
...
void DelayManager::LimitTargetLevel() {
  if (packet_len_ms_ > 0 && CalculateEffectiveMinimumDelay() > 0) {
    int minimum_delay_packet_q8 =
        (CalculateEffectiveMinimumDelay() << 8) / packet_len_ms_;
...
bool DelayManager::IsValidMinimumDelay(int delay_ms) const {
  return 0 <= delay_ms && delay_ms <= CalculateMinimumDelayUpperBound();
}
...
int DelayManager::CalculateEffectiveMinimumDelay() const {
  // Clamp |base_minimum_delay_ms_| into the range which can be effectively
  // used.
  const int base_minimum_delay_ms = rtc::SafeClamp(
      base_minimum_delay_ms_, 0, CalculateMinimumDelayUpperBound());
  return std::max(minimum_delay_ms_, base_minimum_delay_ms);
}

int DelayManager::CalculateMinimumDelayUpperBound() const {
  // Choose the lowest possible bound discarding 0 cases which mean the value
  // is not set and unconstrained.
  const int q75 = CalculateMaxBufferTimeQ75();

  if (maximum_delay_ms_ > 0 && q75 > 0) {
    return std::min(maximum_delay_ms_, q75);
  }
  ...
  return kMaxBaseMinimumDelayMs;
}

int DelayManager::CalculateMaxBufferTimeQ75() const {
