...
DelayManager::DelayManager(size_t max_packets_in_buffer,
    ...
    : first_packet_received_(false),
      ...
      base_target_delay_ms_(base_target_delay_ms),
...
// |base_target_delay_ms_| in packets. We update |least_required_level_| while
// the above limits are applied.
// TODO(hlundin): Move this check to the buffer logistics class.
void DelayManager::LimitTargetLevel() {
  if (packet_len_ms_ > 0) {
    int min_delay_ms = std::max(minimum_delay_ms_, base_target_delay_ms_);
