...
DelayManager::DelayManager(size_t max_packets_in_buffer,
    ...
    : first_packet_received_(false),
      ...
      base_minimum_delay_ms(base_min_delay_ms),
      ...
      last_seq_no_(0),
      last_timestamp_(0),
      minimum_delay_ms_(base_minimum_delay_ms),
  ...
  RTC_DCHECK_GE(base_minimum_delay_ms, 0);
...
bool DelayManager::SetBaseMinimumDelay(int delay_ms) {
  ...
  base_minimum_delay_ms = delay_ms;
  return true;
}

int DelayManager::GetBaseMinimumDelay() const {
  return base_minimum_delay_ms;
}
...
int DelayManager::CalculateEffectiveMinimumDelay() const {
  // Ignore |base_minimum_delay_ms| if it is not compatible with current
  // settings.
  if (!IsValidMinimumDelay(base_minimum_delay_ms)) {
