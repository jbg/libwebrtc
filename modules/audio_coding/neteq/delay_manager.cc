...
DelayManager::IATVector DelayManager::ScaleHistogram(const IATVector& histogram,
  ...
  if (old_packet_length > new_packet_length) {
    ...
    for (size_t i = 0; i < histogram.size(); i++) {
      ...
      // Only subtract the part that was succesfully written to the new
      // histogram.
    ...
    // The histogram should be compressed.
    ...
    for (size_t i = 0; i < histogram.size(); i++) {
      acc += histogram[i];
      time_counter += old_packet_length;
      if (time_counter >= new_packet_length) {
        const int scaled_acc = acc * new_packet_length / time_counter;
        new_histogram[new_histogram_idx] += scaled_acc;
        new_histogram_idx =
            std::min(new_histogram_idx + 1, new_histogram.size() - 1);
    ...
    // During histogram compression, it is possible to reach the end of the
    // input histogram before having read enough entries to start writing to the
    // output, in that case we should still write the leftover part that was
    // read.
    new_histogram[new_histogram_idx] += acc;
  }
