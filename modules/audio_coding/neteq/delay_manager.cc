...
int DelayManager::CalculateEffectiveMinimumDelay() const {
  // Ignore |base_minimum_delay_ms_| if it is not compatible with current
  // settings.
  if (!IsValidMinimumDelay(base_minimum_delay_ms_)) {
    return minimum_delay_ms_;
  }
  return std::max(minimum_delay_ms_, base_minimum_delay_ms_);
}
}  // namespace webrtc
