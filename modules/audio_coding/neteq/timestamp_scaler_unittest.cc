...
TEST(TimestampScaler, TestG722LargeStep) {
  ...
  // TimestampUnwrapper doesn't support unwrapping to a point before the
  // initialization value So using an earlier timestamp will fail
  //  external_timestamp -= kStep * 20;
  //  internal_timestamp -= 2 * kStep * 20;

  external_timestamp -= kStep * 7;
  internal_timestamp -= 2 * kStep * 7;
