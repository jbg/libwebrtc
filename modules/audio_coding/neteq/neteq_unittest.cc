...
TEST_F(NetEqDecodingTestFaxMode, TestFrameWaitingTimeStatistics) {
  ...
  for (size_t i = 0; i < num_frames; ++i) {
    ...
    RTPHeader rtp_info;
    rtp_info.sequenceNumber = rtc::dchecked_cast<uint16_t>(i);
    rtp_info.timestamp = rtc::dchecked_cast<uint32_t>(i * kSamples);
    rtp_info.ssrc = 0x1234;     // Just an arbitrary SSRC.
    rtp_info.payloadType = 94;  // PCM16b WB codec.
    rtp_info.markerBit = 0;
 ...
 protected:
  ...
  void CheckBgn(int sampling_rate_hz) {
    ...
    for (int n = 0; n < 10; ++n) {  // Insert few packets and get audio.
      ...
      // Next packet.
      rtp_info.timestamp += rtc::dchecked_cast<uint32_t>(
          expected_samples_per_channel);
      rtp_info.sequenceNumber++;
      receive_timestamp += rtc::dchecked_cast<uint32_t>(
          expected_samples_per_channel);
    }
