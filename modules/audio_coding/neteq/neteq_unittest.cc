...
TEST_F(NetEqDecodingTestFaxMode, TestFrameWaitingTimeStatistics) {
  ...
  for (size_t i = 0; i < num_frames; ++i) {
    ...
    rtp_info.payloadType = 94;  // PCM16b WB codec.
    rtp_info.markerBit = 0;
    ASSERT_EQ(0, neteq_->InsertPacket(rtp_info, payload, 0, 0));
  }
  // Pull out all data.
  for (size_t i = 0; i < num_frames; ++i) {
    bool muted;
...
TEST_F(NetEqDecodingTest, UnknownPayloadType) {
  ...
  EXPECT_EQ(NetEq::kFail, neteq_->InsertPacket(rtp_info, payload, 0, 0));
}
...
TEST_F(NetEqDecodingTest, MAYBE_DecoderError) {
  ...
  EXPECT_EQ(0, neteq_->InsertPacket(rtp_info, payload, 0, 0));
...
class NetEqBgnTest : public NetEqDecodingTest {
 protected:
  void CheckBgn(int sampling_rate_hz) {
    ...
    int64_t receive_time_ms = 200000000000LL;
    bool muted;
    for (int n = 0; n < 10; ++n) {  // Insert few packets and get audio.
      ...
      ASSERT_EQ(0, neteq_->InsertPacket(
                       ...
                       receive_timestamp, receive_time_ms));
      output.Reset();
...
void NetEqDecodingTest::DuplicateCng() {
  ...
  ASSERT_EQ(0, neteq_->InsertPacket(
                   rtp_info,
                   rtc::ArrayView<const uint8_t>(payload, payload_len), 0, 0));
...
TEST_F(NetEqDecodingTest, CngFirst) {
  ...
  ASSERT_EQ(
      NetEq::kOK,
      neteq_->InsertPacket(
          rtp_info, rtc::ArrayView<const uint8_t>(payload, payload_len), 0, 0));
  ++seq_no;
  timestamp += kCngPeriodSamples;
...
TEST_F(NetEqDecodingTestTwoInstances, CompareMutedStateOnOff) {
  ...
  PopulateRtpInfo(0, 0, &rtp_info);
  EXPECT_EQ(0, neteq_->InsertPacket(rtp_info, payload, 0, 0));
  EXPECT_EQ(0, neteq2_->InsertPacket(rtp_info, payload, 0, 0));
...
TEST_F(NetEqDecodingTest, TestConcealmentEvents) {
  ...
  for (int i = 0; i < kNumConcealmentEvents; i++) {
    ...
    for (int j = 0; j < 10; j++) {
      ...
      neteq_->InsertPacket(rtp_info, payload, 0, 0);
