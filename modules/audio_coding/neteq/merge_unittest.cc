...
TEST_P(MergeTest, Process) {
  ...
  // constexpr size_t kInputLen = 2;
  // std::unique_ptr<int16_t[]> input(new int16_t[input_len]);
  int16_t input[input_len];
  std::fill(input, input + input_len, 17);
  merge_.Process(input, input_len, &output);
  EXPECT_GT(output.Size(), 0u);
}
