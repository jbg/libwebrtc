...
class NetEqImplTest : public ::testing::Test {
 protected:
  ...
  void TestDtmfPacket(int sample_rate_hz) {
    ...
    EXPECT_THAT(output.packet_infos_.size(), 0);
...
TEST_F(NetEqImplTest, VerifyTimestampPropagation) {
  ...
  ASSERT_THAT(output.packet_infos_.size(), 1);
  {
    const auto& packet_info = output.packet_infos_[0];
    EXPECT_THAT(packet_info->ssrc(), rtp_header.ssrc);
    EXPECT_THAT(packet_info->csrcs(), ElementsAre(43, 65, 17));
    EXPECT_THAT(packet_info->sequence_number(), rtp_header.sequenceNumber);
    EXPECT_THAT(packet_info->rtp_timestamp(), rtp_header.timestamp);
    EXPECT_THAT(packet_info->audio_level(), IsFalse());
    EXPECT_THAT(packet_info->receive_time_ms(), kReceiveTimeMs);
  }
...
TEST_F(NetEqImplTest, ReorderedPacket) {
  ...
  rtp_header.extension.audioLevel = 0;
  ...
  ASSERT_THAT(output.packet_infos_.size(), 1);
  {
    ...
    EXPECT_THAT(packet_info->sequence_number(), rtp_header.sequenceNumber);
    EXPECT_THAT(packet_info->rtp_timestamp(), rtp_header.timestamp);
    EXPECT_THAT(packet_info->audio_level(), 0);
    EXPECT_THAT(packet_info->receive_time_ms(), kReceiveTimeMs);
  }
  ...
  ASSERT_THAT(output.packet_infos_.size(), 1);
  {
    ...
    EXPECT_THAT(packet_info->sequence_number(), rtp_header.sequenceNumber);
    EXPECT_THAT(packet_info->rtp_timestamp(), rtp_header.timestamp);
    EXPECT_THAT(packet_info->audio_level(), 2);
    EXPECT_THAT(packet_info->receive_time_ms(), kReceiveTimeMs);
  }
...
TEST_F(NetEqImplTest, FirstPacketUnknown) {
  ...
  for (size_t i = 0; i < 3; ++i) {
    ...
    EXPECT_THAT(output.packet_infos_.size(), 1);
  }
}
...
TEST_F(NetEqImplTest, DecodingError) {
  ...
  EXPECT_EQ(1u, output.num_channels_);
  EXPECT_EQ(AudioFrame::kNormalSpeech, output.speech_type_);
  EXPECT_THAT(output.packet_infos_.size(), 2);  // 5 ms packets vs 10 ms output
