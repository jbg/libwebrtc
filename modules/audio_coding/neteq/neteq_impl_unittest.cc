...
TEST_F(NetEqImplTest, ReorderedPacket) {
  ...
  rtp_header.timestamp = 0x12345678;
  rtp_header.ssrc = 0x87654321;
  rtp_header.extension.audio_level = AudioLevel{};
  rtp_header.extension.audio_level->audio_level = 42;
  ...
  ASSERT_THAT(output.packet_infos_, SizeIs(1));
  {
    ...
    EXPECT_EQ(packet_info.audio_level(),
              rtp_header.extension.audio_level->audio_level);
    EXPECT_EQ(packet_info.receive_time(), expected_receive_time);
  }
  ...
  rtp_header.sequenceNumber -= 1;
  rtp_header.timestamp -= kPayloadLengthSamples;
  rtp_header.extension.audio_level->audio_level = 1;
  payload[0] = 1;
  clock_.AdvanceTimeMilliseconds(1000);
  EXPECT_EQ(NetEq::kOK, neteq_->InsertPacket(rtp_header, payload));
  rtp_header.sequenceNumber += 2;
  rtp_header.timestamp += 2 * kPayloadLengthSamples;
  rtp_header.extension.audio_level->audio_level = 2;
  payload[0] = 2;
...
TEST_F(NetEqImplTest, InsertRedPayload) {
  ...
  header.extension.audio_level = AudioLevel();
  header.extension.audio_level->audio_level = 12;
  header.numCSRCs = 1;
  ...
  EXPECT_EQ(frame.packet_infos_.front().audio_level(),
            header.extension.audio_level->audio_level);
