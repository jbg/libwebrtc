...
bool NoLoss::Lost(int decoded_time_ms) {
  return false;
}
...
bool UniformLoss::Lost(int decoded_time_ms) {
  int drop_this = rand();
...
bool GilbertElliotLoss::Lost(int decoded_time_ms) {
  ...
  if (lost_last_) {
    ...
    return lost_last_ = uniform_loss_model_->Lost(decoded_time_ms);
  } else {
...
bool FixedLossModel::Lost(int decoded_time_ms) {
  if (loss_events_it_ != loss_events_.end() &&
      decoded_time_ms > loss_events_it_->start_ms) {
    if (decoded_time_ms <=
        loss_events_it_->start_ms + loss_events_it_->duration_ms) {
      return true;
    } else {
      ++loss_events_it_;
      return false;
  ...
  switch (FLAG_random_loss_mode) {
    ...
    case kFixedLoss: {
      ...
      while (std::getline(loss_events_stream, loss_event_string, ',')) {
        ...
        auto result = loss_events.insert(FixedLossEvent(loss_event_params));
