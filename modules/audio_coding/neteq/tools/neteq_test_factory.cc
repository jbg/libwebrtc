...
std::unique_ptr<NetEqTest> NetEqTestFactory::InitializeTest(int argc,
  ...
  std::unique_ptr<NetEqTest::Callbacks> callbacks(new NetEqTest::Callbacks);
  ...
  ssrc_switch_detector_.reset(new SsrcSwitchDetector(delay_analyzer.get()));
  callbacks->post_insert_packet = ssrc_switch_detector_.get();
  stats_getter_.reset(new NetEqStatsGetter(
      std::move(delay_analyzer), FLAG_matlabplot, FLAG_pythonplot,
      FLAG_concealment_events, output_file_name));
  callbacks->get_audio_callback = stats_getter_.get();
  callbacks->simulation_ended_callback = stats_getter_.get();
  NetEq::Config config;
