...
int64_t NetEqTest::Run() {
  ...
  } while (step_result.simulation_position !=
           SimulationStepResult::SimulationPosition::kFinished);
...
NetEqTest::SimulationStepResult NetEqTest::RunToNextGetAudio() {
  ...
  while (!input_->ended()) {
    ...
    if (input_->NextOutputEventTime() &&
      ...
      result.simulation_position =
          input_->ended() ? SimulationStepResult::SimulationPosition::kFinished
                          : SimulationStepResult::SimulationPosition::kRunning;
      return result;
    }
  }
  result.simulation_step_ms = time_now_ms - start_time_ms;
...
NetEqTest::SimulationState NetEqTest::GetSimulationState() {
  SimulationState state;
  const auto network_stats = SimulationStats();
  state.current_delay_ms_ = network_stats.current_buffer_size_ms;
  state.mean_iat_ms_ = network_stats.mean_waiting_time_ms;
  return state;
}
