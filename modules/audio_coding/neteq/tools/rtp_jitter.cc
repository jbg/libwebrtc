...
rtc::Buffer ReadNextPacket(FILE* file) {
  ...
  rtc::Buffer buffer;
  buffer.SetData(kRtpDumpHeaderLength, [file](rtc::ArrayView<uint8_t> x) {
    return fread(x.data(), 1, kRtpDumpHeaderLength, file);
  });
  ...
  buffer.AppendData(
      len - kRtpDumpHeaderLength, [file, len](rtc::ArrayView<uint8_t> x) {
        return fread(x.data(), 1, len - kRtpDumpHeaderLength, file);
      });
  if (buffer.size() != len) {
    buffer.Clear();
  }
