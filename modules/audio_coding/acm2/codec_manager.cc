...
int IsValidSendCodec(const CodecInst& send_codec) {
  if ((send_codec.channels != 1) && (send_codec.channels != 2)) {
    RTC_DLOG(LS_ERROR) << "Wrong number of channels (" << send_codec.channels
  ...
  if (!maybe_codec_id) {
    RTC_DLOG(LS_ERROR) << "Invalid codec setting for the send codec.";
    return -1;
  }

  // Telephone-event cannot be a send codec.
  if (!STR_CASE_CMP(send_codec.plname, "telephone-event")) {
    RTC_DLOG(LS_ERROR) << "telephone-event cannot be a send codec";
    return -1;
  }

  if (!RentACodec::IsSupportedNumChannels(*maybe_codec_id, send_codec.channels)
           .value_or(false)) {
    RTC_DLOG(LS_ERROR) << send_codec.channels
...
bool CodecManager::SetCopyRed(bool enable) {
  if (enable && codec_stack_params_.use_codec_fec) {
    RTC_DLOG(LS_WARNING) << "Codec internal FEC and RED cannot be co-enabled.";
    return false;
  }
  if (enable && send_codec_inst_ &&
      codec_stack_params_.red_payload_types.count(send_codec_inst_->plfreq) <
          1) {
    RTC_DLOG(LS_WARNING) << "Cannot enable RED at " << send_codec_inst_->plfreq
                         << " Hz.";
    return false;
  }
...
bool CodecManager::SetVAD(bool enable, ACMVADMode mode) {
  ...
  if (enable && stereo_send) {
    RTC_DLOG(LS_ERROR) << "VAD/DTX not supported for stereo sending";
    return false;
  }
