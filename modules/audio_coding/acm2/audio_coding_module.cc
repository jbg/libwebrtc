...
int AudioCodingModuleImpl::SendFrequency() const {
  ...
  if (!encoder_stack_) {
    RTC_DLOG(LS_ERROR) << "SendFrequency Failed, no codec is registered";
    return -1;
  }
...
int AudioCodingModuleImpl::PreprocessToAddData(const AudioFrame& in_frame,
  ...
  if (!first_10ms_data_) {
    ...
    RTC_DLOG(LS_WARNING)
        << "Unexpected input timestamp: " << in_frame.timestamp_
...
int AudioCodingModuleImpl::SetREDStatus(bool enable_red) {
  ...
  return 0;
#else
  RTC_DLOG(LS_WARNING) << "  WEBRTC_CODEC_RED is undefined";
  return -1;
#endif
}
...
bool AudioCodingModuleImpl::RegisterReceiveCodec(
  ...
  if (!acm2::RentACodec::IsPayloadTypeValid(rtp_payload_type)) {
    RTC_DLOG_F(LS_ERROR) << "Invalid payload-type " << rtp_payload_type
                         << " for decoder.";
    return false;
  }
...
int AudioCodingModuleImpl::RegisterReceiveCodecUnlocked(
  ...
  if (codec.channels > 2) {
    RTC_DLOG_F(LS_ERROR)
        << "Unsupported number of channels: " << codec.channels;
    return -1;
  }
  ...
  if (!acm2::RentACodec::IsPayloadTypeValid(codec.pltype)) {
    RTC_DLOG_F(LS_ERROR) << "Invalid payload type " << codec.pltype << " for "
                         << codec.plname;
    return -1;
  }
...
int AudioCodingModuleImpl::RegisterExternalReceiveCodec(
  ...
  if (num_channels > 2 || num_channels < 0) {
    RTC_DLOG_F(LS_ERROR) << "Unsupported number of channels: " << num_channels;
    return -1;
  }
...
int AudioCodingModuleImpl::SetMinimumPlayoutDelay(int time_ms) {
  if ((time_ms < 0) || (time_ms > 10000)) {
    RTC_DLOG(LS_ERROR) << "Delay must be in the range of 0-10000 milliseconds.";
    return -1;
  }
...
bool AudioCodingModuleImpl::HaveValidEncoder(const char* caller_name) const {
  if (!encoder_stack_) {
    RTC_DLOG(LS_ERROR)
        << caller_name << " failed: No send codec is registered.";
    return false;
  }
  return true;
}
...
bool AudioCodingModule::IsCodecValid(const CodecInst& codec) {
  ...
  if (!valid)
    RTC_DLOG(LS_ERROR) << "Invalid codec setting";
  return valid;
}
