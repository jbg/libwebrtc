...
int AcmReceiver::InsertPacket(const WebRtcRTPHeader& rtp_header,
...

  {
    ...
    if (!ci) {
      RTC_DLOG_F(LS_ERROR) << "Payload-type "
...
int32_t AcmReceiver::AddCodec(int acm_codec_id,
  ...
  if (neteq_->RemovePayloadType(payload_type) != NetEq::kOK) {
    RTC_DLOG(LERROR) << "Cannot remove payload "
  ...
  if (ret_val != NetEq::kOK) {
    RTC_DLOG(LERROR) << "AcmReceiver::AddCodec " << acm_codec_id
                     << static_cast<int>(payload_type)
                     << " channels: " << channels;
    return -1;
  }
...
bool AcmReceiver::AddCodec(int rtp_payload_type,
  ...
  if (!success) {
    RTC_DLOG(LERROR) << "AcmReceiver::AddCodec failed for payload type "
...
int AcmReceiver::RemoveCodec(uint8_t payload_type) {
  rtc::CritScope lock(&crit_sect_);
  if (neteq_->RemovePayloadType(payload_type) != NetEq::kOK) {
    RTC_DLOG(LERROR) << "AcmReceiver::RemoveCodec "
                     << static_cast<int>(payload_type);
    return -1;
  }
...
int AcmReceiver::DecoderByPayloadType(uint8_t payload_type,
  ...
  if (ci) {
    *codec = *ci;
    ...
    RTC_DLOG(LERROR) << "AcmReceiver::DecoderByPayloadType "
                     << static_cast<int>(payload_type);
    return -1;
  }
}
