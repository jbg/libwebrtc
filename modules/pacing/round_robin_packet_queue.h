...
class RoundRobinPacketQueue {
 public:
  ...
  class PacketInfo {
   public:
    PacketInfo(RtpPacketPacer::PacketType type,
               uint32_t ssrc,
               uint16_t seq_number,
    ...
    PacketInfo(const PacketInfo& rhs);
    ~PacketInfo();

    bool operator<(const PacketInfo& other) const;

    int Priority() const { return priority_; }
    RtpPacketPacer::PacketType Type() const { return type_; }
    uint32_t Ssrc() const { return ssrc_; }
    uint16_t SequenceNumber() const { return sequence_number_; }
    int64_t CaptureTimeMs() const { return capture_time_ms_; }
    int64_t EnqueueTimeMs() const { return enqueue_time_ms_; }
    size_t SizeInBytes() const { return bytes_; }
    bool IsRetransmission() const { return retransmission_; }
    uint64_t EnqueueOrder() const { return enqueue_order_; }
    std::unique_ptr<RtpPacketToSend> ReleasePacket();

    // For internal use.
    absl::optional<RtpPacketIterator> PacketIterator() const {
      return packet_it_;
    }
   ...
   private:
    RtpPacketPacer::PacketType type_;
    int priority_;
    uint32_t ssrc_;
    ...
    // if applicable.
    absl::optional<RtpPacketIterator> packet_it_;
  };

  void Push(RtpPacketPacer::PacketType type,
            uint32_t ssrc,
            uint16_t seq_number,
            int64_t capture_time_ms,
            int64_t enqueue_time_ms,
 ...
 private:
  ...
  struct Stream {
    ...
    size_t bytes;
    uint32_t ssrc;
    std::priority_queue<PacketInfo> packet_queue;
  ...
  absl::optional<PacketInfo> pop_packet_;
