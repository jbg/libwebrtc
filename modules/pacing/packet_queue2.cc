...
void PacketQueue2::Push(const Packet& packet_to_insert) {
  ...
  auto stream_info_it = stream_infos_.find(packet.ssrc);
  if (stream_info_it == stream_infos_.end()) {
    stream_info_it = stream_infos_.emplace(packet.ssrc, Stream()).first;
    stream_info_it->second.priority_it = stream_priorities_.end();
    stream_info_it->second.ssrc = packet.ssrc;
  }

  Stream* stream_info = &stream_info_it->second;
  StreamPrioKey key(packet.ssrc, packet.priority, stream_info->bytes);
  if (stream_info->priority_it == stream_priorities_.end()) {
    // If the SSRC is not currently scheduled, add it to |stream_priorities_|.
    auto emplace_result = stream_priorities_.emplace(key);
    RTC_CHECK(emplace_result.second);
    stream_info->priority_it = emplace_result.first;
  } else if (packet.priority < stream_info->priority_it->priority) {
  ...
  stream_info->packet_queue.push(packet);
...
void PacketQueue2::Pop() {
  RTC_CHECK(!paused_);
  if (!Empty()) {
    Stream* stream_info = GetHighestPriorityStream();
    stream_priorities_.erase(stream_info->priority_it);
    ...
    if (stream_info->packet_queue.empty()) {
      ...
      StreamPrioKey key(stream_info->ssrc, priority, stream_info->bytes);
      auto emplace_result = stream_priorities_.emplace(key);
      RTC_CHECK(emplace_result.second);
      stream_info->priority_it = emplace_result.first;
    }
...
PacketQueue2::Stream* PacketQueue2::GetHighestPriorityStream() {
  ...
  uint32_t ssrc = stream_priorities_.begin()->ssrc;
