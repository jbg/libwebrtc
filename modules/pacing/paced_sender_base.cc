...
const int64_t PacedSenderBase::kMaxQueueLengthMs = 2000;
const float PacedSenderBase::kDefaultPaceMultiplier = 2.5f;

PacedSenderBase::PacedSenderBase(Clock* clock,
...
PacedSenderBase::~PacedSenderBase() {}

void PacedSenderBase::CreateProbeCluster(int bitrate_bps, int cluster_id) {
  prober_.CreateProbeCluster(bitrate_bps, TimeMilliseconds(), cluster_id);
}

void PacedSenderBase::Pause() {
  if (!paused_)
...
void PacedSenderBase::SetCongestionWindow(int64_t congestion_window_bytes) {
  congestion_window_bytes_ = congestion_window_bytes;
}

void PacedSenderBase::UpdateOutstandingData(int64_t outstanding_bytes) {
  outstanding_bytes_ = outstanding_bytes;
}

bool PacedSenderBase::Congested() const {
  if (congestion_window_bytes_ == kNoCongestionWindow)
    return false;
  return outstanding_bytes_ >= congestion_window_bytes_;
}

int64_t PacedSenderBase::TimeMilliseconds() const {
...
void PacedSenderBase::SetProbingEnabled(bool enabled) {
  RTC_CHECK_EQ(0, packet_counter_);
  prober_.SetEnabled(enabled);
}

void PacedSenderBase::SetPacingRates(uint32_t pacing_rate_bps,
...
void PacedSenderBase::InsertPacket(RtpPacketSender::Priority priority,
                                   uint32_t ssrc,
...
void PacedSenderBase::EnqueuePacket(std::unique_ptr<RtpPacketToSend> packet) {
  RTC_DCHECK(pacing_bitrate_kbps_ > 0)
      << "SetPacingRate must be called before InsertPacket.";
...
void PacedSenderBase::SetAccountForAudioPackets(bool account_for_audio) {
  account_for_audio_ = account_for_audio;
}

int64_t PacedSenderBase::ExpectedQueueTimeMs() const {
  RTC_DCHECK_GT(pacing_bitrate_kbps_, 0);
  return static_cast<int64_t>(packets_.SizeInBytes() * 8 /
                              pacing_bitrate_kbps_);
}

size_t PacedSenderBase::QueueSizePackets() const {
  return packets_.SizeInPackets();
}

int64_t PacedSenderBase::QueueSizeBytes() const {
  return packets_.SizeInBytes();
}

int64_t PacedSenderBase::FirstSentPacketTimeMs() const {
  return first_sent_packet_ms_;
}

int64_t PacedSenderBase::QueueInMs() const {
...
absl::optional<int64_t> PacedSenderBase::TimeUntilNextProbe() {
...
int64_t PacedSenderBase::TimeUntilNextProcess() {
...
int64_t PacedSenderBase::TimeUntilAvailableBudget() {
  // When paused or when there's no packets to send, wake up every 500 ms to
  // send a padding packet in order to ensure we won't get stuck in the paused
  // state due to no feedback being received.
  int64_t time_until_available_budget = kPausedProcessIntervalMs;
  if (paused_) {
    return time_until_available_budget;
  }

  if (!packets_.Empty() && media_budget_.target_rate_kbps() > 0) {
    time_until_available_budget =
        std::min(time_until_available_budget,
                 media_budget_.TimeMsUntilNonNegativeBudget());
    return Congested()
               ? std::max(time_until_available_budget, kDefaultMinPacketLimitMs)
               : time_until_available_budget;
  }

  if (!padding_failure_state_ && padding_budget_.target_rate_kbps() > 0 &&
      packet_counter_ > 0 && !Congested()) {
    return std::min(time_until_available_budget,
                    padding_budget_.TimeMsUntilNonNegativeBudget());
  }

  return time_until_available_budget;
}

bool PacedSenderBase::ShouldSendPacket(
    const RoundRobinPacketQueue::QueuedPacket& packet) {
  // Override conditions for whether to send the next packet in the queue or
  // not, from high priority to low.

  if (packet.type() == RtpPacketToSend::Type::kAudio && !pace_audio_) {
    // Not pacing audio, packet should be heedlessly sent immediately.
    return true;
  }

  if (Congested()) {
    // If network is congested, don't send any packet.
    return false;
  }

  if (prober_.IsProbing()) {
    // If we're probing, send packet even if we don't have budget.
    return true;
  }

  // No special overrides, send only if we have budget for it.
  return media_budget_.bytes_remaining() > 0;
}

int64_t PacedSenderBase::UpdateTimeAndGetElapsedMs(int64_t now_us) {
...
bool PacedSenderBase::ShouldSendKeepalive(int64_t now_us) const {
...
void PacedSenderBase::ProcessPackets() {
  ...
  if (ShouldSendKeepalive(now_us)) {
    if (legacy_packet_referencing_) {
      OnPaddingSent(TimeToSendPadding(1, PacedPacketInfo()));
    } else {
      ...
      for (auto& packet : keepalive_packets) {
        ...
        SendRtpPacket(std::move(packet), PacedPacketInfo());
      }
  ...
  while (!paused_) {
      ...
      if (!legacy_packet_referencing_) {
        ...
        if (padding_bytes_to_add > 0) {
          std::vector<std::unique_ptr<RtpPacketToSend>> padding_packets =
              GeneratePadding(padding_bytes_to_add);
    ...
    if (rtp_packet != nullptr) {
      SendRtpPacket(std::move(rtp_packet), pacing_info);
      success = RtpPacketSendResult::kSuccess;
    } else {
      success = TimeToSendPacket(packet->ssrc(), packet->sequence_number(),
  ...
  if (legacy_packet_referencing_ && packets_.Empty() && !Congested()) {
    ...
    if (packet_counter_ > 0) {
      ...
      if (padding_needed > 0) {
        size_t padding_sent = TimeToSendPadding(padding_needed, pacing_info);
...
size_t PacedSenderBase::PaddingBytesToAdd(
    absl::optional<size_t> recommended_probe_size,
    size_t bytes_sent) {
  if (!packets_.Empty()) {
...
RoundRobinPacketQueue::QueuedPacket* PacedSenderBase::GetPendingPacket(
...
void PacedSenderBase::OnPacketSent(
...
void PacedSenderBase::OnPaddingSent(size_t bytes_sent) {
  padding_failure_state_ = bytes_sent == 0;
  if (bytes_sent > 0) {
...
void PacedSenderBase::UpdateBudgetWithElapsedTime(int64_t delta_time_ms) {
  delta_time_ms = std::min(kMaxIntervalTimeMs, delta_time_ms);
  media_budget_.IncreaseBudget(delta_time_ms);
  padding_budget_.IncreaseBudget(delta_time_ms);
}

void PacedSenderBase::UpdateBudgetWithBytesSent(size_t bytes_sent) {
  outstanding_bytes_ += bytes_sent;
  media_budget_.UseBudget(bytes_sent);
  padding_budget_.UseBudget(bytes_sent);
}

void PacedSenderBase::SetQueueTimeLimit(int limit_ms) {
