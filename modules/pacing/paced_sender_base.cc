...
int64_t PacedSenderBase::TimeUntilAvailableBudget() {
  ...
  int64_t elapsed_time_us =
      clock_->TimeInMicroseconds() - time_last_process_us_;
  int64_t elapsed_time_ms = (elapsed_time_us + 500) / 1000;
  int64_t time_until_available_budget =
      std::max<int64_t>(kPausedProcessIntervalMs - elapsed_time_ms, 0);
  if (paused_) {
    return time_until_available_budget;
  }

  if (media_budget_.target_rate_kbps() > 0 && !packets_.Empty()) {
    time_until_available_budget =
        std::min(time_until_available_budget,
                 media_budget_.TimeMsUntilNonNegativeBudget());
  }
  if (padding_budget_.target_rate_kbps() > 0) {
    time_until_available_budget =
        std::min(time_until_available_budget,
