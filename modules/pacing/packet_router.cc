...
#if RTC_DCHECK_IS_ON
// Checks if a process thread is set, which should generally be the case.
// Some tests don't use an actual process thread and in those cases,
// Current() will return nullptr and we'll ignore that case.
#define EXPECT_PROCESS_THREAD(process_thread_name)                    \
  if (webrtc::ProcessThreadImpl::Current()) {                         \
...
void PacketRouter::OnReceiveBitrateChanged(const std::vector<uint32_t>& ssrcs,
  ...
  // TODO(tommi): This method is normally (and supposed to be) called on the
  // PacerThread. However, some tests call this method in the wrong context
  // and that needs to be fixed before we can enable this check.
  // RTC_DCHECK_RUN_ON(&process_thread_checker_);
