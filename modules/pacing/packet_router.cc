...
void PacketRouter::RemoveSendRtpModuleFromMap(uint32_t ssrc) {
  ...
  RTC_DCHECK(modules_used_for_send_packet_.empty());
  modules_used_for_send_packet_.erase(it->second);
...
void PacketRouter::SendPacket(std::unique_ptr<RtpPacketToSend> packet,
  ...
  modules_used_for_send_packet_.insert(rtp_module);
...
void PacketRouter::OnBatchComplete() {
  ...
  for (auto& module : modules_used_for_send_packet_) {
    module->OnBatchComplete();
  }
  modules_used_for_send_packet_.clear();
}
