...
TEST_F(PacketRouterTest, ForwardsAbortedRetransmissions) {
  ...
  const uint16_t kSsrc1 = 1234;
  const uint16_t kSsrc2 = 2345;
  const uint16_t kInvalidSsrc = 3456;
...
TEST_F(PacketRouterTest, ReportsRtxSsrc) {
  ...
  EXPECT_FALSE(packet_router_.GetRtxSsrcForMedia(kRtxSsrc1).has_value());
  EXPECT_FALSE(packet_router_.GetRtxSsrcForMedia(kSsrc2).has_value());
  EXPECT_FALSE(packet_router_.GetRtxSsrcForMedia(kInvalidSsrc).has_value());
