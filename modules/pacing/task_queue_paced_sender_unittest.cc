...
TEST(TaskQueuePacedSenderTest, Stats) {
  static constexpr int64_t kStartTime = 1234;
  GlobalSimulatedTimeController time_controller(Timestamp::Millis(kStartTime));
  MockPacketRouter packet_router;
  TaskQueuePacedSender pacer(
      time_controller.GetClock(), &packet_router,
      /*event_log=*/nullptr,
  ...
  // Insert a number of packets, covering one second.
  static constexpr size_t kPacketsToSend = 100;
  static constexpr size_t kPacingRate = kDefaultPacketSize * 8 * kPacketsToSend;
  pacer.SetPacingRates(DataRate::BitsPerSec(kPacingRate), DataRate::Zero());
  pacer.EnsureStarted();

  // Allowed `QueueSizeData` and `ExpectedQueueTime` deviation.
  static constexpr size_t kAllowedPacketsDeviation = 2;
  static constexpr int64_t kAllowedQueueSizeDeviation =
      kDefaultPacketSize * 8 * kAllowedPacketsDeviation;
  static constexpr int64_t kAllowedQueueTimeDeviation =
      kAllowedQueueSizeDeviation * 1000 / kPacingRate;

  int64_t expected_queue_size = 0;
  int64_t expected_queue_time = 0;

  EXPECT_CALL(packet_router, SendPacket).Times(kPacketsToSend);

  // Stats before insert any packets.
  EXPECT_EQ(pacer.OldestPacketWaitTime().ms(), 0);
  EXPECT_FALSE(pacer.FirstSentPacketTime().has_value());
  EXPECT_EQ(pacer.QueueSizeData().bytes(), 0);
  EXPECT_EQ(pacer.ExpectedQueueTime().ms(), 0);
  ...
  EXPECT_EQ(pacer.OldestPacketWaitTime().ms(), 200);
  EXPECT_TRUE(pacer.FirstSentPacketTime().has_value());
  EXPECT_EQ(pacer.FirstSentPacketTime()->ms(), kStartTime);

  expected_queue_size = kPacingRate * 800 / 1000;
  expected_queue_time = expected_queue_size * 1000 / kPacingRate;
  EXPECT_NEAR(pacer.QueueSizeData().bytes() * 8, expected_queue_size,
              kAllowedQueueSizeDeviation);
  EXPECT_NEAR(pacer.ExpectedQueueTime().ms(), expected_queue_time,
              kAllowedQueueTimeDeviation);

  // Advance to 500ms.
  time_controller.AdvanceTime(TimeDelta::Millis(300));
  EXPECT_EQ(pacer.OldestPacketWaitTime().ms(), 500);
  EXPECT_EQ(pacer.FirstSentPacketTime()->ms(), kStartTime);

  expected_queue_size = kPacingRate * 500 / 1000;
  expected_queue_time = expected_queue_size * 1000 / kPacingRate;
  EXPECT_NEAR(pacer.QueueSizeData().bytes() * 8, expected_queue_size,
              kAllowedQueueSizeDeviation);
  EXPECT_NEAR(pacer.ExpectedQueueTime().ms(), expected_queue_time,
  ...
  time_controller.AdvanceTime(
      TimeDelta::Millis(500 + kAllowedQueueTimeDeviation));
  EXPECT_EQ(pacer.OldestPacketWaitTime().ms(), 0);
  EXPECT_TRUE(pacer.FirstSentPacketTime().has_value());
  EXPECT_EQ(pacer.FirstSentPacketTime()->ms(), kStartTime);
  EXPECT_EQ(pacer.QueueSizeData().bytes(), 0);
