...
void PacingController::EnqueuePacket(std::unique_ptr<RtpPacketToSend> packet) {
  ...
  if (mode_ == ProcessMode::kDynamic && packet_queue_->SizeInPackets() == 0) {
...
Timestamp PacingController::NextSendTime() const {
  ...
  if (media_rate_ > DataRate::Zero() && packet_queue_->SizeInPackets() > 0) {
...
DataSize PacingController::PaddingToAdd(DataSize recommended_probe_size,
  ...
  if (packet_queue_->SizeInPackets() > 0) {
...
std::unique_ptr<RtpPacketToSend> PacingController::GetPendingPacket(
  ...
  bool unpaced_audio_packet =
      !pace_audio_ &&
      packet_queue_->LeadingAudioPacketEnqueueTime().has_value();
