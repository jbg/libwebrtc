...
int GetPriorityForType(RtpPacketSendInfo::Type type) {
...
void PacingController::ProcessPackets() {
  ...
  while (!paused_) {
    ...
    const RtpPacketSendInfo::Type packet_type = *rtp_packet->packet_type();
...
void PacingController::OnPacketSent(RtpPacketSendInfo::Type packet_type,
                                    DataSize packet_size,
                                    Timestamp send_time) {
  if (!first_sent_packet_time_) {
    first_sent_packet_time_ = send_time;
  }
  bool audio_packet = packet_type == RtpPacketSendInfo::Type::kAudio;
