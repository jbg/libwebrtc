...
class RtpPacketPacer : public RtpPacketSender {
 public:
  ...
  virtual void CreateProbeCluster(int bitrate_bps, int cluster_id) = 0;
  ...
  virtual void SetCongestionWindow(
      absl::optional<int64_t> congestion_window_bytes) = 0;
  virtual void UpdateOutstandingData(int64_t outstanding_bytes) = 0;
  ...
  virtual int64_t QueueInMs() const = 0;
  ...
  virtual int64_t QueueSizeBytes() const = 0;

  // Returns the time when the first packet was sent.
  virtual absl::optional<int64_t> FirstSentPacketTimeMs() const = 0;
  ...
  // priority.
  virtual int64_t ExpectedQueueTimeMs() const = 0;
  ...
  // Permanently shut down any scheduled processing.
  virtual void Stop() = 0;
};
