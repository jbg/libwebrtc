...
void RoundRobinPacketQueue::Push(int priority,
                                 ...
                                 bool retransmission,
                                 uint64_t enqueue_order) {
  Push(PacketInfo(priority, ssrc, seq_number, capture_time_ms, enqueue_time_ms,
...
void RoundRobinPacketQueue::Push(int priority,
  ...
  Push(PacketInfo(priority, packet->Ssrc(), packet->SequenceNumber(),
                  packet->capture_time_ms(), enqueue_time_ms,
                  packet->payload_size(), retransmission, enqueue_order,
                  enqueue_times_.insert(enqueue_time_ms),
                  rtp_packets_.begin()));
}
