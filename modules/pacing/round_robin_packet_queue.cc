...
bool RoundRobinPacketQueue::QueuedPacket::operator<(
  ...
  if (IsRetransmission() != other.IsRetransmission())
    return other.IsRetransmission();
...
RtpPacketToSend::Type RoundRobinPacketQueue::QueuedPacket::Type() const {
  auto type = owned_packet_->packet_type();
...
std::unique_ptr<RtpPacketToSend> RoundRobinPacketQueue::Pop() {
  ...
  QueuedPacket queued_packet = stream->packet_queue.top();
  ...
  return std::unique_ptr<RtpPacketToSend>(queued_packet.RtpPacket());
}
