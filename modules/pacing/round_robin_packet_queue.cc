...
void RoundRobinPacketQueue::Push(int priority,
                                 ...
                                 bool retransmission,
                                 uint64_t enqueue_order) {
  Push(priority, ssrc, seq_number, capture_time_ms, enqueue_time_ms,
       length_in_bytes, retransmission, enqueue_order, absl::nullopt);
}
...
void RoundRobinPacketQueue::Push(int priority,
  ...
  Push(priority, packet->Ssrc(), packet->SequenceNumber(),
       packet->capture_time_ms(), enqueue_time_ms, packet->payload_size(),
       retransmission, enqueue_order, rtp_packets_.begin());
}
...
void RoundRobinPacketQueue::Push(int priority,
                                 ...
                                 uint64_t enqueue_order,
                                 absl::optional<RtpPacketIterator> packet_it) {
  PacketInfo packet(priority, ssrc, seq_number, capture_time_ms,
