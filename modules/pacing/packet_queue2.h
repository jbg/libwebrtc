...
class PacketQueue2 {
 ...
 private:
  ...
  StreamInfo* GetHighestPriorityStream() const;
  ...
  // then reinsert a StremPrioKey if the priority has increased.
  std::multimap<StreamPrioKey, uint32_t> stream_priorities_;

  // A map of SSRCs to StreamInfos, mutable for GetHighestPriorityStream().
  mutable std::map<uint32_t, StreamInfo> stream_infos_;
