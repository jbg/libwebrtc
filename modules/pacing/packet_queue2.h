...
class PacketQueue2 {
 public:
  ...
  struct StreamPrioKey {
    ...
    StreamPrioKey(uint32_t ssrc,
                  RtpPacketSender::Priority priority,
                  int64_t bytes)
        : ssrc(ssrc), priority(priority), bytes(bytes) {}

    bool operator<(const StreamPrioKey& other) const {
      // There should only ever be at most one entry for a given SSRC.
      if (ssrc == other.ssrc)
        return false;
      ...
      // Arbitrary way to break the tie between two streams if they have the
      // same actual priority.
  ...
  struct Stream {
    ...
    std::set<StreamPrioKey>::iterator priority_it;
  };
 ...
 private:
  ...
  std::set<StreamPrioKey> stream_priorities_;

  // A map of SSRCs to Streams.
  std::map<uint32_t, Stream> stream_infos_;
