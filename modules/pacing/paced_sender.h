...
class PacedSender : public Module, public RtpPacketPacer {
 public:
  ...
  // TODO(sprang): Remove once downstream projets are updated.
  ...
  void SetCongestionWindow(
      absl::optional<int64_t> congestion_window_bytes) override;
  ...
  absl::optional<int64_t> FirstSentPacketTimeMs() const override;
 ...
 private:
  ...
  absl::optional<int64_t> first_sent_packet_ms_ RTC_GUARDED_BY(critsect_);

  RoundRobinPacketQueue packets_ RTC_GUARDED_BY(critsect_);
  uint64_t packet_counter_ RTC_GUARDED_BY(critsect_);

  absl::optional<int64_t> congestion_window_bytes_ RTC_GUARDED_BY(critsect_);
