...
std::unique_ptr<RtpPacketToSend> BuildPacket(RtpPacketSendInfo::Type type,
                                             uint32_t ssrc,
...
class MockPacingControllerCallback : public PacingController::PacketSender {
 public:
  void SendRtpPacket(std::unique_ptr<RtpPacketToSend> packet,
    ...
    SendPacket(
        packet->Ssrc(), packet->SequenceNumber(), packet->capture_time_ms(),
        packet->packet_type() == RtpPacketSendInfo::Type::kRetransmission,
        packet->packet_type() == RtpPacketSendInfo::Type::kPadding);
  }
  ...
  std::vector<std::unique_ptr<RtpPacketToSend>> GeneratePadding(
    ...
    if (padding_size > 0) {
      ...
      packet->set_packet_type(RtpPacketSendInfo::Type::kPadding);
 ...
 public:
  ...
  std::vector<std::unique_ptr<RtpPacketToSend>> GeneratePadding(
    ...
    for (size_t i = 0; i < num_packets; ++i) {
      ...
      packets.back()->set_packet_type(RtpPacketSendInfo::Type::kPadding);
...
class PacingControllerProbing : public PacingController::PacketSender {
 public:
  ...
  void SendRtpPacket(std::unique_ptr<RtpPacketToSend> packet,
                     const PacedPacketInfo& pacing_info) override {
    if (packet->packet_type() != RtpPacketSendInfo::Type::kPadding) {
      ++packets_sent_;
    }
  }
...
class PacingControllerTest
 ...
 protected:
  ...
  void Send(RtpPacketSendInfo::Type type,
            uint32_t ssrc,
            uint16_t sequence_number,
            int64_t capture_time_ms,
            size_t size) {
  ...
  void SendAndExpectPacket(RtpPacketSendInfo::Type type,
                           uint32_t ssrc,
                           uint16_t sequence_number,
                           int64_t capture_time_ms,
                           size_t size) {
    ...
    EXPECT_CALL(
        ...
        SendPacket(ssrc, sequence_number, capture_time_ms,
                   type == RtpPacketSendInfo::Type::kRetransmission, false))
        .Times(1);
  }

  std::unique_ptr<RtpPacketToSend> BuildRtpPacket(
      RtpPacketSendInfo::Type type) {
    auto packet = std::make_unique<RtpPacketToSend>(nullptr);
    packet->set_packet_type(type);
    switch (type) {
      case RtpPacketSendInfo::Type::kAudio:
        packet->SetSsrc(kAudioSsrc);
        break;
      case RtpPacketSendInfo::Type::kVideo:
        packet->SetSsrc(kVideoSsrc);
        break;
      case RtpPacketSendInfo::Type::kRetransmission:
      case RtpPacketSendInfo::Type::kPadding:
  ...
  void ConsumeInitialBudget() {
    ...
    for (size_t i = 0; i < packets_to_send_per_interval; ++i) {
      SendAndExpectPacket(RtpPacketSendInfo::Type::kVideo, kSsrc,
...
class PacingControllerFieldTrialTest
 ...
 protected:
  struct MediaStream {
    const RtpPacketSendInfo::Type type;
    const uint32_t ssrc;
  ...
  MediaStream audio{/*type*/ RtpPacketSendInfo::Type::kAudio,
                    /*ssrc*/ 3333, /*packet_size*/ 100, /*seq_num*/ 1000};
  MediaStream video{/*type*/ RtpPacketSendInfo::Type::kVideo,
...
TEST_P(PacingControllerTest, QueuePacket) {
  ...
  for (size_t i = 0; i < kPacketsToSend; ++i) {
    SendAndExpectPacket(RtpPacketSendInfo::Type::kVideo, ssrc,
  ...
  Send(RtpPacketSendInfo::Type::kVideo, ssrc, sequence_number,
...
TEST_P(PacingControllerTest, QueueAndPacePackets) {
  ...
  // Enqueue one extra packet.
  int64_t queued_packet_timestamp = clock_.TimeInMilliseconds();
  Send(RtpPacketSendInfo::Type::kVideo, kSsrc, sequence_number,
...
TEST_P(PacingControllerTest, RepeatedRetransmissionsAllowed) {
  ...
  for (size_t i = 0; i < 3; i++) {
    ...
    SendAndExpectPacket(
        is_retransmission ? RtpPacketSendInfo::Type::kRetransmission
                          : RtpPacketSendInfo::Type::kVideo,
...
TEST_P(PacingControllerTest,
  ...
  SendAndExpectPacket(RtpPacketSendInfo::Type::kVideo, ssrc, sequence_number,
                      clock_.TimeInMilliseconds(), 250);

  // Expect packet on second ssrc to be queued and sent as well.
  SendAndExpectPacket(RtpPacketSendInfo::Type::kVideo, ssrc + 1,
                      sequence_number, clock_.TimeInMilliseconds(), 250);
...
TEST_P(PacingControllerTest, Priority) {
  ...
  Send(RtpPacketSendInfo::Type::kVideo, ssrc_low_priority, sequence_number++,
       capture_time_ms_low_priority, 250);

  const size_t packets_to_send_per_interval =
      kTargetRate.bps() * kPaceMultiplier / (8 * 250 * 200);
  for (size_t i = 0; i < packets_to_send_per_interval; ++i) {
    Send(RtpPacketSendInfo::Type::kRetransmission, ssrc, sequence_number++,
         capture_time_ms, 250);
  }
  Send(RtpPacketSendInfo::Type::kAudio, ssrc, sequence_number++,
       capture_time_ms, 250);
...
TEST_P(PacingControllerTest, HighPrioDoesntAffectBudget) {
  ...
  for (size_t i = 0; i < kNumAudioPackets; ++i) {
    SendAndExpectPacket(RtpPacketSendInfo::Type::kAudio, ssrc,
                        sequence_number++, capture_time_ms, kPacketSize);
  }
  pacer_->ProcessPackets();
...
TEST_P(PacingControllerTest, DoesNotAllowOveruseAfterCongestion) {
  ...
  Send(RtpPacketSendInfo::Type::kVideo, ssrc, seq_num++, now_ms(), size);
  EXPECT_CALL(callback_, SendPacket).Times(1);
  clock_.AdvanceTimeMilliseconds(5);
  pacer_->ProcessPackets();
...
TEST_P(PacingControllerTest, Pause) {
  ...
  for (size_t i = 0; i < packets_to_send_per_interval; ++i) {
    ...
    Send(RtpPacketSendInfo::Type::kRetransmission, ssrc, sequence_number++,
         capture_time_ms, 250);
    Send(RtpPacketSendInfo::Type::kAudio, ssrc_high_priority, sequence_number++,
         capture_time_ms, 250);
  }
...
TEST_P(PacingControllerTest, ProbeClusterId) {
  ...
  EXPECT_CALL(callback, GeneratePadding).WillOnce([&](DataSize padding_size) {
    ...
    padding_packets.emplace_back(
        BuildPacket(RtpPacketSendInfo::Type::kPadding, ssrc, sequence_number++,
...
TEST_P(PacingControllerTest, OwnedPacketPrioritizedOnType) {
  ...
  for (RtpPacketSendInfo::Type type :
       {RtpPacketSendInfo::Type::kPadding,
        RtpPacketSendInfo::Type::kForwardErrorCorrection,
        RtpPacketSendInfo::Type::kVideo,
        RtpPacketSendInfo::Type::kRetransmission,
        RtpPacketSendInfo::Type::kAudio}) {
...
TEST_P(PacingControllerTest, SmallFirstProbePacket) {
  ...
  // Add high prio media.
  pacer_->EnqueuePacket(BuildRtpPacket(RtpPacketSendInfo::Type::kAudio));
  ...
  EXPECT_CALL(callback, GeneratePadding(DataSize::bytes(1)))
      .WillOnce([&](DataSize padding_size) {
        std::vector<std::unique_ptr<RtpPacketToSend>> padding_packets;
        padding_packets.emplace_back(
            BuildPacket(RtpPacketSendInfo::Type::kPadding, kAudioSsrc, 1,
  ...
  EXPECT_CALL(callback, SendRtpPacket)
      ...
      .WillRepeatedly([&](std::unique_ptr<RtpPacketToSend> packet,
                          const PacedPacketInfo& cluster_info) {
        if (packets_sent == 0) {
          EXPECT_EQ(packet->packet_type(), RtpPacketSendInfo::Type::kPadding);
        } else {
          if (packet->packet_type() == RtpPacketSendInfo::Type::kAudio) {
            media_seen = true;
          }
        }
        packets_sent++;
...
TEST_P(PacingControllerTest, TaskLate) {
  ...
  pacer_->EnqueuePacket(BuildRtpPacket(RtpPacketSendInfo::Type::kVideo));
  pacer_->EnqueuePacket(BuildRtpPacket(RtpPacketSendInfo::Type::kVideo));
  pacer_->EnqueuePacket(BuildRtpPacket(RtpPacketSendInfo::Type::kVideo));
  pacer_->EnqueuePacket(BuildRtpPacket(RtpPacketSendInfo::Type::kVideo));
