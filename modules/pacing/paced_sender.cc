...
void PacedSender::InsertPacket(std::unique_ptr<RtpPacketToSend> packet,
                               Priority priority,
                               bool retransmission) {
  rtc::CritScope cs(&critsect_);
  RTC_DCHECK(pacing_bitrate_kbps_ > 0)
  ...
  packets_.Push(priority, now_ms, retransmission, packet_counter_++,
                std::move(packet));
}
  ...
  while (!packets_.Empty() && !paused_) {
    ...
    if (rtp_packet != nullptr) {
      success = packet_sender_->TimeToSendPacket(
          std::move(rtp_packet), packet->IsRetransmission(), pacing_info);
    } else {
      success = packet_sender_->TimeToSendPacket(
