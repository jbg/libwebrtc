...
void PacedSender::InsertPacket(RtpPacketPacer::Priority priority,
  ...
  switch (priority) {
    case RtpPacketPacer::kHighPriority:
      type = PacketType::kAudio;
      break;
    case RtpPacketPacer::kNormalPriority:
      type = PacketType::kRetransmission;
      break;
    default:
      type = PacketType::kVideo;
  }
...
void PacedSender::EnqueuePacket(std::unique_ptr<RtpPacketToSend> packet,
  ...
  packets_.Push(type, now_ms, packet_counter_++, std::move(packet));
}
...
RoundRobinPacketQueue::PacketInfo* PacedSender::GetPendingPacket(
  ...
  bool audio_packet = packet->Type() == PacketType::kAudio;
