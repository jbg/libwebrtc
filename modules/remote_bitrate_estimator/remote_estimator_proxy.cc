...
bool RemoteEstimatorProxy::SendPeriodicFeedback() {
  ...
  auto begin_iterator = packet_arrival_times_.lower_bound(window_start_seq_);
  auto end_iterator = packet_arrival_times_.cend();
  ...
  window_start_seq_ = BuildFeedbackPacket(begin_iterator, end_iterator,
                                          window_start_seq_, &feedback_packet);
...
void RemoteEstimatorProxy::SendFeedbackOnRequest(
  ...
  window_start_seq_ = BuildFeedbackPacket(
      begin_iterator, end_iterator, first_sequence_number, &feedback_packet);
...
int64_t RemoteEstimatorProxy::BuildFeedbackPacket(
  ...
  // Base sequence is the expected first sequence number. This is known, but
