...
bool RemoteEstimatorProxy::BuildFeedbackPacket(
    std::map<int64_t, int64_t>::const_iterator first_iterator,
    std::map<int64_t, int64_t>::const_iterator last_iterator,
    int64_t base_sequence_number,
    bool include_timestamps,
    rtcp::TransportFeedback* feedback_packet) {
  if (first_iterator == packet_arrival_times_.cend()) {
...
void RemoteEstimatorProxy::SendFeedbackOnRequest(
  ...
  auto last_iterator = packet_arrival_times_.upper_bound(sequence_number);

  BuildFeedbackPacket(first_iterator, last_iterator, first_sequence_number,
                      feedback_request.include_timestamps, &feedback_packet);

  // Clear up to the first packet that is included in this feedback packet.
  packet_arrival_times_.erase(packet_arrival_times_.begin(), first_iterator);
