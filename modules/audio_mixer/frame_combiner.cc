...
void MixToFloatFrame(rtc::ArrayView<const AudioFrame* const> mix_list,
  ...
  auto raw_data = mixing_buffer.data();
...
void FrameCombiner::Combine(rtc::ArrayView<AudioFrame* const> mix_list,
  ...
  DeinterleavedView<float> deinterleaved(
      &mixing_buffer_[0], output_samples_per_channel, number_of_channels);
  ...
  AudioFrameView<float> mixing_buffer_view(
      &channel_pointers[0], number_of_channels, output_samples_per_channel);
