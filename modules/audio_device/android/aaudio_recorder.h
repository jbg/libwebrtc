...
class AAudioRecorder : public AAudioObserverInterface {
 public:
  ...
  // data, in the streams current data format, from the |audio_data| buffer.
 ...
 private:
  ...
  // Counts number of detected owerflow events reported by AAudio.
  ...
  // True ony for the first data callback in each audio session.
