...
FineAudioBuffer::FineAudioBuffer(AudioDeviceBuffer* audio_device_buffer)
    : audio_device_buffer_(audio_device_buffer),
      playout_samples_per_channel_10ms_(static_cast<size_t>(
          audio_device_buffer->PlayoutSampleRate() * 10 / 1000)),
      record_samples_per_channel_10ms_(static_cast<size_t>(
...
void FineAudioBuffer::DeliverRecordedData(
  ...
  while (record_buffer_.size() >= record_samples_per_channel_10ms_) {
    ...
    memmove(record_buffer_.data(),
            record_buffer_.data() + record_samples_per_channel_10ms_,
            (record_buffer_.size() - record_samples_per_channel_10ms_) *
                sizeof(int16_t));
    record_buffer_.SetSize(record_buffer_.size() -
                           record_samples_per_channel_10ms_);
  }
}
