...
int32_t AudioDeviceModuleImpl::CreatePlatformSpecificObjects() {
  ...
  AudioDeviceGeneric* audio_device = nullptr;

#if defined(WEBRTC_DUMMY_AUDIO_BUILD)
  audio_device = new AudioDeviceDummy(Id());
  LOG(INFO) << "Dummy Audio APIs will be utilized";
#elif defined(WEBRTC_DUMMY_FILE_DEVICES)
  audio_device = FileAudioDeviceFactory::CreateFileAudioDevice(Id());
  if (audio_device) {
...
//
  ...
  if ((audioLayer == kWindowsCoreAudio) ||
    ...
    if (AudioDeviceWindowsCore::CoreAudioIsSupported()) {
      ...
      audio_device = new AudioDeviceWindowsCore();
...
//
  ...
  if ((audioLayer == kLinuxPulseAudio) ||
    ...
    if (pulseDevice->Init() == AudioDeviceGeneric::InitStatus::OK) {
      ...
      audio_device = new AudioDeviceLinuxALSA();
      if (audio_device != NULL) {
        // Pulse Audio was not supported => revert to ALSA instead
        audio_layer_ = kLinuxAlsaAudio;  // modify the state set at construction
...
//
  ...
  if (audioLayer == kPlatformDefaultAudio) {
    ...
    audio_device = new AudioDeviceIOS();
...
//
  ...
  if (audioLayer == kPlatformDefaultAudio) {
    ...
    audio_device = new AudioDeviceMac();
