...
// TODOD(henrika): figure out why usage of this API always reports
...
TEST_F(CoreAudioUtilityWinTest, SharedModeInitialize) {
  ...
  HRESULT hr = core_audio_utility::SharedModeInitialize(
      client.Get(), &format, nullptr, 0, &endpoint_buffer_size);
  ...
  hr = core_audio_utility::SharedModeInitialize(
      client.Get(), &format, event_handle, 0, &endpoint_buffer_size);
  EXPECT_TRUE(SUCCEEDED(hr));
  EXPECT_GT(endpoint_buffer_size, 0u);
}
