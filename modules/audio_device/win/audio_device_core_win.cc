...
int32_t AudioDeviceWindowsCore::InitSpeaker()
{
    ...
    if (_usingOutputDeviceIndex)
    {
        ...
        {
          RTC_LOG(LS_ERROR)
              << "current device selection is invalid => unable to"
              << " initialize";
          return -1;
        }
    }
...
int32_t AudioDeviceWindowsCore::InitPlayout()
{
    ...
    if (hr == S_OK)
    {
        ...
        RTC_LOG(LS_VERBOSE)
            << "wFormatTag         : 0x" << std::hex << Wfx.wFormatTag
        ...
        RTC_LOG(LS_VERBOSE)
            << "_playBlockSizeInFrames     : " << _playBlockSizeInFrames;
...
int32_t AudioDeviceWindowsCore::InitRecording()
{
    ...
    if (hr == S_OK)
    {
        ...
        RTC_LOG(LS_VERBOSE)
            << "wFormatTag        : 0x" << std::hex << Wfx.Format.wFormatTag
            << std::dec << " (" << Wfx.Format.wFormatTag << ")";
        RTC_LOG(LS_VERBOSE) << "nChannels         : " << Wfx.Format.nChannels;
        RTC_LOG(LS_VERBOSE)
            << "nSamplesPerSec    : " << Wfx.Format.nSamplesPerSec;
        RTC_LOG(LS_VERBOSE)
            << "nAvgBytesPerSec   : " << Wfx.Format.nAvgBytesPerSec;
...
DWORD AudioDeviceWindowsCore::InitCaptureThreadPriority()
{
    ...
    if (_winSupportAvrt)
    {
        ...
        {
          RTC_LOG(LS_WARNING)
              << "failed to enable MMCSS on capture thread (err="
              << GetLastError() << ")";
          _TraceCOMError(GetLastError());
        }
    }
