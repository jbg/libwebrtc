...
bool CoreAudioBase::Init() {
  ...
  // Try to create the latest version (3) first and then fall back if to 2 and
  // finally 1 if needed. Only one of the pointers below will be non nullptr.
  ...
  if (audio_client3) {
    // equivalent to audio_client3.CopyTo(&audio_client3_);
...
bool CoreAudioBase::Start() {
  RTC_DLOG(INFO) << __FUNCTION__ << "[" << DirectionToString(direction())
                 << "]";
  if (IsRestarting()) {
    // Audio thread should be alive during internal restart.
    RTC_DCHECK(audio_thread_);
  }

  // Start an audio thread but not during restart.
  if (!audio_thread_) {
