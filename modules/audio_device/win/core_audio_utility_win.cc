...
HRESULT GetPreferredAudioParametersInternal(IAudioClient* client,
  ...
  double device_period_in_seconds =
      static_cast<double>(
          CoreAudioUtility::ReferenceTimeToTimeDelta(default_period).ms()) /
      1000.0L;
...
HRESULT CoreAudioUtility::SharedModeInitialize(
  ...
  *endpoint_buffer_size = buffer_size_in_frames;
  ...
  RTC_DLOG(INFO) << "stream latency: "
                 << CoreAudioUtility::ReferenceTimeToTimeDelta(latency).ms()
                 << " [ms]";
  return error.Error();
}
...
webrtc::TimeDelta CoreAudioUtility::ReferenceTimeToTimeDelta(
    REFERENCE_TIME time) {
