...
HRESULT GetPreferredAudioParametersInternal(IAudioClient* client,
  ...
  // TODO(henrika): perhaps don't log warning if fixed_sample_rate equals the
  // preferred rate.
  if (fixed_sample_rate > 0) {
...
HRESULT GetSharedModeEnginePeriod(IAudioClient3* client3,
  ...
  RTC_DCHECK(client3);
  if (GetAudioClientVersion() < 3) {
    RTC_LOG(LS_WARNING) << "Requires IAudioClient3 or higher";
    return AUDCLNT_E_UNSUPPORTED_FORMAT;
  }
