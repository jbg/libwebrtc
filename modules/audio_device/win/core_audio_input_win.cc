...
int CoreAudioInput::InitRecording() {
  ...
  // |audio_client_| or |audio_client2_| depending on platform support. The
  // base class will use optimal input parameters and do an event driven shared
  // mode initialization. The utilized format will be stored in |format_| and
  // can be used for configuration and allocation of audio buffers.
  ...
  ComPtr<IAudioCaptureClient> audio_capture_client =
      core_audio_utility::CreateCaptureClient(GetAudioClient());
