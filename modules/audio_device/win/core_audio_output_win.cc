...
int CoreAudioOutput::InitPlayout() {
  ...
  // |audio_client2_| or |audio_client_| depending on what the platform
  // supports. The base class will use optimal output parameters and do an
  // event driven shared mode initialization. The utilized format will be
  // stored in |format_| and can be used for configuration and allocation
  // of audio buffers.
  ...
  ComPtr<IAudioRenderClient> audio_render_client =
      core_audio_utility::CreateRenderClient(GetAudioClient());
  if (!audio_render_client.Get()) {
    return -1;
  }

  ComPtr<IAudioClock> audio_clock =
      core_audio_utility::CreateAudioClock(GetAudioClient());
  if (!audio_clock.Get()) {
    return -1;
  }
...
int CoreAudioOutput::StartPlayout() {
  ...
  if (!core_audio_utility::FillRenderEndpointBufferWithSilence(
          GetAudioClient(), audio_render_client_.Get())) {
...
bool CoreAudioOutput::OnDataCallback(uint64_t device_frequency) {
  ...
  _com_error error = GetAudioClient()->GetCurrentPadding(&num_unread_frames);
