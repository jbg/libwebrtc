...
bool CoreAudioOutput::HandleStreamDisconnected() {
  ...
  // TODO(henrika): improve detection of if device has changed...
  ...
  if (core_audio_utility::NumberOfActiveDevices(eRender) < 1) {
    RTC_DLOG(LS_ERROR) << "All output devices are disabled or removed";
    return false;
  }
  ...
  if (device_id != device_id_) {
    ...
    if (CoreAudioBase::SetDevice(0) == -1) {
      RTC_LOG(LS_WARNING) << "Failed to set new output device";
      return false;
    }
  } else {
    RTC_LOG(INFO)
