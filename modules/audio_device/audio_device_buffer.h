...
#if RTC_DCHECK_IS_ON
// TODO(b/156729026): Since rtc::ThreadChecker has been replaced with
// SequenceChecker, but the AudioDeviceBuffer implementation is still being
// used in a way that is thread specific and more than one TQ might be active on
// that thread in some tests, a temporary workaround is to have a custom thread
// checker that only checks the thread id.
