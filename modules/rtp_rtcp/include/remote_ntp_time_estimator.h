...
class RemoteNtpTimeEstimator {
 public:
  ...
  ABSL_DEPRECATED("Use UpdateRtcpTimestamp below with strict types")
  ...
  int64_t Estimate(uint32_t rtp_timestamp) {
    if (NtpTime ntp_time = EstimateNtp(rtp_timestamp); ntp_time.Valid()) {
  ...
  ABSL_DEPRECATED("Use EstimateRemoteToLocalClockOffset below")
