...
bool AudioLevelExtension::Parse(rtc::ArrayView<const uint8_t> data,
  ...
  if (data.size() != 1)
    return false;
  extension->voice_activity = (data[0] & 0x80) != 0;
  extension->audio_level = data[0] & 0x7F;
  return true;
}
...
bool AudioLevelExtension::Write(rtc::ArrayView<uint8_t> data,
  ...
  RTC_CHECK_LE(extension.audio_level, 0x7f);
  data[0] = (extension.voice_activity ? 0x80 : 0x00) | extension.audio_level;
  return true;
}
