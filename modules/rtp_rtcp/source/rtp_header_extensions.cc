...
bool ColorSpaceExtension::Parse(rtc::ArrayView<const uint8_t> data,
  ...
  if (data.size() == kValueSizeBytesWithoutHdrMetadata) {
    ...
    offset += ParseHdrMetadata(data.data() + offset, &hdr_metadata);
...
bool ColorSpaceExtension::Write(rtc::ArrayView<uint8_t> data,
  ...
  if (color_space.hdr_metadata()) {
    offset +=
        WriteHdrMetadata(data.data() + offset, *color_space.hdr_metadata());
  }
  RTC_DCHECK_EQ(ValueSize(color_space), offset);
  return true;
}
...
size_t ColorSpaceExtension::ParseHdrMetadata(const uint8_t* data,
                                             HdrMetadata* hdr_metadata) {
  size_t offset = 0;
  offset += ParseLuminance(data + offset,
  ...
  offset += ParseChromaticity(data + offset,
  ...
  hdr_metadata->max_content_light_level =
      ByteReader<uint16_t>::ReadBigEndian(data + offset);
  offset += 2;
...
size_t ColorSpaceExtension::WriteHdrMetadata(uint8_t* data,
                                             const HdrMetadata& hdr_metadata) {
  RTC_DCHECK(hdr_metadata.Validate());
  size_t offset = 0;
  offset += WriteLuminance(data + offset,
                           hdr_metadata.mastering_metadata.luminance_max,
                           kLuminanceMaxDenominator);
  offset += WriteLuminance(data + offset,
                           hdr_metadata.mastering_metadata.luminance_min,
                           kLuminanceMinDenominator);
  offset += WriteChromaticity(data + offset,
                              hdr_metadata.mastering_metadata.primary_r);
  offset += WriteChromaticity(data + offset,
                              hdr_metadata.mastering_metadata.primary_g);
  offset += WriteChromaticity(data + offset,
  ...
  ByteWriter<uint16_t>::WriteBigEndian(data + offset,
                                       hdr_metadata.max_content_light_level);
  offset += 2;
  ByteWriter<uint16_t>::WriteBigEndian(
      data + offset, hdr_metadata.max_frame_average_light_level);
  offset += 2;
  return offset;
}
