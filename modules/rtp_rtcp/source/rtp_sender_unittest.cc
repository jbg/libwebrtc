...
class MockRtpPacketSender : public RtpPacketPacer {
 public:
  MockRtpPacketSender() {}
  ...
  MOCK_METHOD6(InsertPacket,
               void(Priority priority,
                    ...
                    size_t bytes,
                    bool retransmission));
  MOCK_METHOD2(EnqueuePacket,
               void(std::unique_ptr<RtpPacketToSend>, PacketType));
};
  ...
  std::unique_ptr<RtpPacketToSend> SendPacket(int64_t capture_time_ms,
    ...
    if (GetParam().pacer_references_packets) {
      EXPECT_TRUE(rtp_sender_->SendToNetwork(
          ...
          RtpPacketPacer::kNormalPriority));
    } else {
      EXPECT_TRUE(rtp_sender_->SendToNetwork(
          absl::make_unique<RtpPacketToSend>(*packet), kAllowRetransmission,
          RtpPacketPacer::PacketType::kAudio));
    }
    return packet;
  }
...
TEST_P(RtpSenderTest, SendsPacketsWithTransportSequenceNumber) {
  ...
  if (GetParam().pacer_references_packets) {
    ...
    rtp_sender_->SendPacedPacket(std::move(packet), PacedPacketInfo());
  }
...
TEST_P(RtpSenderTest, WritesPacerExitToTimingExtension) {
  ...
  if (GetParam().pacer_references_packets) {
    EXPECT_CALL(
        mock_paced_sender_,
        InsertPacket(RtpPacketPacer::kNormalPriority, kSsrc, _, _, _, _));
...
TEST_P(RtpSenderTest, TrafficSmoothingWithExtensions) {
  ...
  if (GetParam().pacer_references_packets) {
    EXPECT_CALL(
        mock_paced_sender_,
        InsertPacket(RtpPacketPacer::kNormalPriority, kSsrc, kSeqNum, _, _, _));
    // Packet should be stored in a send bucket.
    EXPECT_TRUE(rtp_sender_->SendToNetwork(std::move(packet),
                                           kAllowRetransmission,
                                           RtpPacketPacer::kNormalPriority));
...
TEST_P(RtpSenderTest, TrafficSmoothingRetransmits) {
  ...
  if (GetParam().pacer_references_packets) {
    ...
    EXPECT_TRUE(rtp_sender_->SendToNetwork(
        ...
        RtpPacketPacer::PacketType::kVideo));
    // Immediately process send bucket and send packet.
    rtp_sender_->SendPacedPacket(absl::make_unique<RtpPacketToSend>(*packet),
                                 PacedPacketInfo());
  }
...
TEST_P(RtpSenderTest, SendPadding) {
  ...
  if (GetParam().pacer_references_packets) {
    EXPECT_CALL(
        mock_paced_sender_,
        InsertPacket(RtpPacketPacer::kNormalPriority, kSsrc, seq_num, _, _, _));
    // Packet should be stored in a send bucket.
    EXPECT_TRUE(rtp_sender_->SendToNetwork(std::move(packet),
                                           kAllowRetransmission,
                                           RtpPacketPacer::kNormalPriority));
...
TEST_P(RtpSenderTest, OnSendPacketUpdated) {
  ...
  if (GetParam().pacer_references_packets) {
    ...
    pacing_info.is_retransmission = kIsRetransmit;
    rtp_sender_->SendPacedPacket(std::move(packet), pacing_info);
  }
...
TEST_P(RtpSenderTest, SendRedundantPayloads) {
  ...
  for (size_t i = 0; i < kNumPayloadSizes; ++i) {
    ...
    if (GetParam().pacer_references_packets) {
      ...
      EXPECT_CALL(
          mock_paced_sender_,
          EnqueuePacket(AllOf(Pointee(Property(&RtpPacketToSend::Ssrc, kSsrc)),
...
TEST_P(RtpSenderTest, SendFlexfecPackets) {
  ...
  if (GetParam().pacer_references_packets) {
    EXPECT_CALL(mock_paced_sender_,
                InsertPacket(RtpPacketPacer::kLowPriority, kMediaSsrc, kSeqNum,
                             _, _, false));
    EXPECT_CALL(mock_paced_sender_, InsertPacket(RtpPacketPacer::kLowPriority,
    ...
    EXPECT_CALL(mock_paced_sender_,
                EnqueuePacket(_, RtpPacketPacer::PacketType::kVideo))
    ...
    EXPECT_CALL(mock_paced_sender_,
                EnqueuePacket(_, RtpPacketPacer::PacketType::kRedundancy))
        .WillOnce([&](std::unique_ptr<RtpPacketToSend> packet,
                      RtpPacketPacer::PacketType type) {
    ...
    rtp_sender_->SendPacedPacket(std::move(media_packet), PacedPacketInfo());
    rtp_sender_->SendPacedPacket(std::move(fec_packet), PacedPacketInfo());
  }
...
TEST_P(RtpSenderTest, NoFlexfecForTimingFrames) {
  ...
  if (GetParam().pacer_references_packets) {
    ...
    rtp_sender_->SendPacedPacket(std::move(rtp_packet), PacedPacketInfo());
  }
  ...
  if (GetParam().pacer_references_packets) {
    EXPECT_CALL(mock_paced_sender_, InsertPacket(RtpPacketPacer::kLowPriority,
        ...
        .WillOnce(::testing::SaveArg<2>(&flexfec_seq_num));
    EXPECT_CALL(mock_paced_sender_,
                InsertPacket(RtpPacketPacer::kLowPriority, kMediaSsrc,
                             kSeqNum + 1, _, _, false));
...
TEST_P(RtpSenderTest, FecOverheadRate) {
  ...
  if (GetParam().pacer_references_packets) {
    ...
    EXPECT_CALL(mock_paced_sender_,
                EnqueuePacket(_, RtpPacketPacer::PacketType::kVideo))
        .Times(kNumMediaPackets);
    EXPECT_CALL(mock_paced_sender_,
                EnqueuePacket(_, RtpPacketPacer::PacketType::kRedundancy))
        .Times(kNumFecPackets);
  }
