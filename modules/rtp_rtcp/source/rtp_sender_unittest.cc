...
TEST_P(RtpSenderTest, TrafficSmoothingRetransmits) {
  ...
  if (GetParam().pacer_references_packets) {
    ...
    EXPECT_TRUE(rtp_sender_->SendToNetwork(
        ...
        RtpPacketPacer::PacketType::kVideo));
    // Immediately process send bucket and send packet.
    std::unique_ptr<RtpPacketToSend> packet_copy =
        absl::make_unique<RtpPacketToSend>(*packet);
    EXPECT_TRUE(rtp_sender_->TrySendPacket(&packet_copy, PacedPacketInfo()));
  }
...
TEST_P(RtpSenderTest, OnSendPacketNotUpdatedForRetransmits) {
  ...
  if (GetParam().pacer_references_packets) {
    ...
    PacedPacketInfo pacing_info;
    pacing_info.is_retransmission = kIsRetransmit;
    EXPECT_TRUE(rtp_sender_->TrySendPacket(&packet, pacing_info));
  }
...
TEST_P(RtpSenderTest, SendFlexfecPackets) {
  ...
  if (GetParam().pacer_references_packets) {
    ...
    EXPECT_CALL(
        mock_paced_sender_,
        EnqueuePacket(_, RtpPacketPacer::PacketType::kForwardErrorCorrection))
        .WillOnce([&](std::unique_ptr<RtpPacketToSend> packet,
                      RtpPacketPacer::PacketType type) {
    ...
    EXPECT_TRUE(rtp_sender_->TrySendPacket(&media_packet, PacedPacketInfo()));
...
TEST_P(RtpSenderTest, NoFlexfecForTimingFrames) {
  ...
  if (GetParam().pacer_references_packets) {
    ...
    EXPECT_TRUE(rtp_sender_->TrySendPacket(&rtp_packet, PacedPacketInfo()));
  }
