...
TEST_P(RtpSenderTest, TrafficSmoothingRetransmits) {
  ...
  EXPECT_CALL(mock_rtc_event_log_,
              LogProxy(SameRtcEventTypeAs(RtcEvent::Type::RtpPacketOutgoing)));
  packet->set_packet_type(RtpPacketSendInfo::Type::kRetransmission);
    ...
    EXPECT_CALL(mock_paced_sender_, EnqueuePackets)
        .WillOnce([&](std::vector<std::unique_ptr<RtpPacketToSend>> packets) {
          for (auto& packet : packets) {
            if (packet->packet_type() == RtpPacketSendInfo::Type::kVideo) {
...
TEST_P(RtpSenderTest, SendPacketMatchesAudio) {
  std::unique_ptr<RtpPacketToSend> packet =
      BuildRtpPacket(kPayload, true, 0, fake_clock_.TimeInMilliseconds());
  packet->set_packet_type(RtpPacketSendInfo::Type::kAudio);
...
TEST_P(RtpSenderTest, SendPacketMatchesFlexfec) {
  std::unique_ptr<RtpPacketToSend> packet =
      BuildRtpPacket(kPayload, true, 0, fake_clock_.TimeInMilliseconds());
  packet->set_packet_type(RtpPacketSendInfo::Type::kForwardErrorCorrection);
...
TEST_P(RtpSenderTest, SendPacketUpdatesStats) {
  ...
  std::unique_ptr<RtpPacketToSend> video_packet =
      BuildRtpPacket(kPayload, true, 0, fake_clock_.TimeInMilliseconds());
  video_packet->set_packet_type(RtpPacketSendInfo::Type::kVideo);
  ...
  rtx_packet->set_packet_type(RtpPacketSendInfo::Type::kRetransmission);
  ...
  fec_packet->set_packet_type(RtpPacketSendInfo::Type::kForwardErrorCorrection);
...
TEST_P(RtpSenderTest, GeneratePaddingResendsOldPacketsWithRtx) {
  ...
  EXPECT_EQ(padding_packet->packet_type(), RtpPacketSendInfo::Type::kPadding);
