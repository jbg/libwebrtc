 ...
 public:
  ...
  MOCK_METHOD6(InsertPacket,
               void(Priority priority,
                    ...
                    size_t bytes,
                    bool retransmission));
  MOCK_METHOD3(PacePacket, void(std::unique_ptr<RtpPacketToSend>, int, bool));
};
...
TEST_P(RtpSenderTest, WritesPacerExitToTimingExtension) {
  ...
  if (GetParam().pacer_references_packets) {
    ...
    EXPECT_CALL(mock_paced_sender_,
                PacePacket(Pointee(Property(&RtpPacketToSend::Ssrc, kSsrc)),
...
TEST_P(RtpSenderTest, SendRedundantPayloads) {
  ...
  for (size_t i = 0; i < kNumPayloadSizes; ++i) {
    ...
    if (GetParam().pacer_references_packets) {
      ...
      EXPECT_CALL(
          mock_paced_sender_,
          PacePacket(AllOf(Pointee(Property(&RtpPacketToSend::Ssrc, kSsrc)),
...
TEST_P(RtpSenderTest, SendFlexfecPackets) {
  ...
  if (GetParam().pacer_references_packets) {
    ...
    EXPECT_CALL(mock_paced_sender_,
                PacePacket(_, RtpPacketSender::kLowPriority, false))
        .Times(2)
        .WillRepeatedly([&](std::unique_ptr<RtpPacketToSend> packet,
                            int priority, bool retransmission) {
...
TEST_P(RtpSenderTest, NoFlexfecForTimingFrames) {
  ...
  if (GetParam().pacer_references_packets) {
    ...
    EXPECT_CALL(
        mock_paced_sender_,
        PacePacket(Pointee(Property(&RtpPacketToSend::Ssrc, kFlexfecSsrc)),
                   RtpPacketSender::kLowPriority, false))
        .Times(0);  // Not called because packet should not be protected.
...
TEST_P(RtpSenderTest, FecOverheadRate) {
  ...
  if (GetParam().pacer_references_packets) {
    ...
    EXPECT_CALL(mock_paced_sender_, PacePacket(_, _, false))
        .Times(kNumMediaPackets + kNumFecPackets);
  }
