...
TEST_P(RtpSenderTest, SendsPacketsWithTransportSequenceNumber) {
  ...
  if (GetParam().pacer_references_packets) {
    ...
    rtp_sender_->SendPacket(std::move(packet), PacedPacketInfo());
  }
...
TEST_P(RtpSenderTest, TrafficSmoothingRetransmits) {
  ...
  if (GetParam().pacer_references_packets) {
    ...
    EXPECT_TRUE(rtp_sender_->SendToNetwork(
        ...
        RtpPacketPacer::PacketType::kVideo));
    // Immediately process send bucket and send packet.
    rtp_sender_->SendPacket(absl::make_unique<RtpPacketToSend>(*packet),
                            PacedPacketInfo());
  }
...
TEST_P(RtpSenderTest, OnSendPacketUpdated) {
  ...
  if (GetParam().pacer_references_packets) {
    ...
    rtp_sender_->SendPacket(std::move(packet), pacing_info);
  }
...
TEST_P(RtpSenderTest, SendFlexfecPackets) {
  ...
  if (GetParam().pacer_references_packets) {
    ...
    rtp_sender_->SendPacket(std::move(media_packet), PacedPacketInfo());
    rtp_sender_->SendPacket(std::move(fec_packet), PacedPacketInfo());
  }
...
TEST_P(RtpSenderTest, NoFlexfecForTimingFrames) {
  ...
  if (GetParam().pacer_references_packets) {
    ...
    rtp_sender_->SendPacket(std::move(rtp_packet), PacedPacketInfo());
  }
