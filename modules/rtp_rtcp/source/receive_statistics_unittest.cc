...
class TestRtcpCallback : public RtcpStatisticsCallback {
 public:
  TestRtcpCallback()
      : RtcpStatisticsCallback(), num_calls_(0), ssrc_(0), stats_() {}
  ~TestRtcpCallback() override {}
...
TEST_F(ReceiveStatisticsTest,
  ...
  receive_statistics_->IncomingPacket(header1_, kPacketSize1, false);
  receive_statistics_->IncomingPacket(header1_, kPacketSize1, false);
  EXPECT_EQ(3u, callback.num_calls_);
}
...
TEST_F(ReceiveStatisticsTest,
  ...
  receive_statistics_->IncomingPacket(header1_, kPacketSize1, false);
  receive_statistics_->IncomingPacket(header1_, kPacketSize1, false);
  ASSERT_EQ(2u, callback.num_calls_);
...
TEST_F(ReceiveStatisticsTest,
  ...
  RtcpStatistics stats_from_callback = callback.stats_;
  ...
  // Now update fraction lost, and check that we got a new callback with
  // matching values.
  receive_statistics_->GetStatistician(kSsrc1)->GetStatistics(
      &stats_from_getstatistics, /*update_fraction_lost=*/true);
  stats_from_callback = callback.stats_;
  EXPECT_EQ(stats_from_getstatistics.packets_lost,
            stats_from_callback.packets_lost);
  EXPECT_EQ(stats_from_getstatistics.extended_highest_sequence_number,
            stats_from_callback.extended_highest_sequence_number);
...
TEST_F(ReceiveStatisticsTest, FractionLostOnlyUpdatedWhenReportGenerated) {
  ...
  EXPECT_EQ(0u, callback.stats_.fraction_lost);
  ...
  receive_statistics_->GetStatistician(kSsrc1)->GetStatistics(
      &statistics, /*update_fraction_lost=*/true);
  // 25% = 63/255.
  EXPECT_EQ(63u, callback.stats_.fraction_lost);
  ...
  receive_statistics_->GetStatistician(kSsrc1)->GetStatistics(
      &statistics, /*update_fraction_lost=*/true);
  // 50% = 127/255.
  EXPECT_EQ(127, callback.stats_.fraction_lost);
}
...
// actually end up with negative loss, which should be snapped to zero
// according to RFC3550.
TEST_F(ReceiveStatisticsTest, NegativeLossSnappedToZero) {
...
TEST_F(ReceiveStatisticsTest, ExtendedHighestSequenceNumberComputation) {
  ...
  receive_statistics_->IncomingPacket(header1_, kPacketSize1, false);
  EXPECT_EQ(65535u, callback.stats_.extended_highest_sequence_number);

  // Wrap around.
  header1_.sequenceNumber = 1;
  receive_statistics_->IncomingPacket(header1_, kPacketSize1, false);
  EXPECT_EQ(65536u + 1u, callback.stats_.extended_highest_sequence_number);
  ...
  // TODO(deadbeef): With large jumps like this, RFC3550 suggests for the
  // receiver to assume the other side restarted, and reset all its sequence
  // number counters. Why aren't we doing this?
  header1_.sequenceNumber = 30000;
  receive_statistics_->IncomingPacket(header1_, kPacketSize1, false);
  EXPECT_EQ(65536u + 30000u, callback.stats_.extended_highest_sequence_number);

  header1_.sequenceNumber = 50000;
  receive_statistics_->IncomingPacket(header1_, kPacketSize1, false);
  EXPECT_EQ(65536u + 50000u, callback.stats_.extended_highest_sequence_number);
  ...
  // TODO(deadbeef): This seems very, very broken. Why do we do this? If
  // anything, should be treated as a restart as mentioned above.
...
TEST_F(ReceiveStatisticsTest, SimpleJitterComputation) {
  ...
  header1_.timestamp += 3;
  receive_statistics_->IncomingPacket(header1_, kPacketSize1, false);
  EXPECT_EQ(0u, callback.stats_.jitter);
  ...
  receive_statistics_->IncomingPacket(header1_, kPacketSize1, true);
  EXPECT_EQ(1u, callback.stats_.jitter);
  ...
  receive_statistics_->IncomingPacket(header1_, kPacketSize1, true);
  EXPECT_EQ(6u, callback.stats_.jitter);
  ...
  receive_statistics_->IncomingPacket(header1_, kPacketSize1, true);
  EXPECT_EQ(8u, callback.stats_.jitter);
}
...
TEST_F(ReceiveStatisticsTest, JitterComputationIgnoresReorderedPackets) {
  ...
  receive_statistics_->IncomingPacket(header1_, kPacketSize1, false);
  EXPECT_EQ(10u, callback.stats_.jitter);
}
