...
TEST(ReviseJitterTest, AllPacketsHaveDifferentPayloadTypeFrequency) {
  ...
  //          8[frequency KHz] - 160[timestamp diff]) * 16 - 0[jitter] + 8)
  //          / 16 = 240
  // packet3: revised jitter: 240 * 48[frequency KHz] / 8[frequency KHz] = 1440
  //          jitter = 1440[jitter] + (abs(50[receive time ms] *
  //          48[frequency KHz] - 960[timestamp diff]) * 16 - 1440[jitter] + 8)
  //          / 16 = 2790
  // final jitter: 2790 / 16 = 174
...
TEST(ReviseJitterTest,
  ...
  RtpPacketReceived packet4 =
      MakeNextRtpPacket(packet3, /*payload_type_frequency=*/8000,
  ...
  //          / 16 = 1590
  // packet4: revised jitter: 1590 * 8[frequency KHz] / 48[frequency KHz] = 265
...
TEST(ReviseJitterTest,
  ...
  //          / 16 = 1440
  // packet3: jitter = 1440[jitter] + (abs(50[receive time ms] *
  //          0[frequency KHz] - 55[timestamp diff]) * 16 - 1440[jitter] + 8)
  //          / 16 = 1405
  // packet4: revised jitter: 1405 * 8[frequency KHz] / 48[frequency KHz] = 234
...
TEST(ReviseJitterTest,
  ...
  RtpPacketReceived packet3 = MakeNextRtpPacket(
      packet2, /*payload_type_frequency=*/0, /*timestamp=*/1 + 960 + 55);
  RtpPacketReceived packet4 =
      MakeNextRtpPacket(packet3, /*payload_type_frequency=*/48000,
  ...
  //          0[frequency KHz] - 55[timestamp diff]) * 16 - 1440[jitter] + 8)
  //          / 16 = 1405
  // packet4: jitter = 1405[jitter] + (abs(50[receive time ms] *
  //          48[frequency KHz] - 960[timestamp diff]) * 16 - 1405[jitter] + 8)
  //          / 16 = 2757
  // final jitter: 2757 / 16 = 172
