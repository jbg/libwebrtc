...
constexpr const uint8_t kPacketMaskRandomTbl[] = {
  48,
...
rtc::ArrayView<const uint8_t> LookUpInRandomTable(int media_packet_index,
  ...
  RTC_DCHECK_LT(media_packet_index, kPacketMaskRandomTbl[0]);

  // Skip over the table size.
  const uint8_t* entry = &kPacketMaskRandomTbl[1];

  uint8_t entry_size_increment = 2;  // Changes to 6.
...
const size_t kRandomTableSize = kPacketMaskRandomTbl[0];
