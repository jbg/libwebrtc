...
TEST(RtpPacketHistoryRecentLargePacketMode,
  ...
  EXPECT_THAT(
      ...
      AllOf(Pointee(Property(&RtpPacketToSend::SequenceNumber, kStartSeqNum)),
            Pointee(Property(&RtpPacketToSend::payload_size, 1000))));
}
...
TEST(RtpPacketHistoryRecentLargePacketMode,
  ...
  EXPECT_THAT(history.GetPayloadPaddingPacket(),
              AllOf(Pointee(Property(&RtpPacketToSend::SequenceNumber,
                                     kStartSeqNum + 1)),
                    Pointee(Property(&RtpPacketToSend::payload_size, 950))));
}
