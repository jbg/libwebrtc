...
TEST(RtpPacketizerH264Test, DoesNotPacketizeWithEmptyNalUnit) {
  ...
  uint8_t bad[] = {0x00, 0x00, 0x01, /* empty NAL unit data */
                   0x00, 0x00, 0x01, 0x01};
  rtc::Buffer frame(bad, sizeof bad);
  RtpPacketizerH264 packetizer(frame, limits,
                               H264PacketizationMode::NonInterleaved);
  EXPECT_EQ(packetizer.NumPackets(), 0u);
}

TEST(RtpPacketizerH264Test, FuzzerStuff) {
  RtpPacketizer::PayloadSizeLimits limits;
  limits.max_payload_len = 1200;
  limits.first_packet_reduction_len = 0;  // 195;
  limits.last_packet_reduction_len = 0;
  limits.single_packet_reduction_len = 0;
  // A practical attack might have to work with this:
  // CONFIG: first_red=0 last_red=16 single_red=16 mode=NonInterleaved len=1896
  ...
  rtc::Buffer frame(bad, sizeof bad /* = 1896 */);
  RtpPacketizerH264 packetizer(frame, limits,
                               H264PacketizationMode::NonInterleaved);
  std::vector<RtpPacketToSend> packets = FetchAllPackets(&packetizer);
}
