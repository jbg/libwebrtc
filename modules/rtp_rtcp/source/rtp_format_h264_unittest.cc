...
struct ParsedPayloadH264Test : public RtpDepacketizer::ParsedPayload {
  ParsedPayloadH264Test() { video.video_type_header = RTPVideoHeaderH264(); }
...
RtpPacketizer* CreateH264Packetizer(H264PacketizationMode mode,
  ...
  header.video_type_header = RTPVideoHeaderH264();
  auto* h264 = &absl::get<RTPVideoHeaderH264>(header.video_type_header);
  h264->packetization_mode = mode;
...
class RtpDepacketizerH264Test : public ::testing::Test {
 protected:
  ...
  void ExpectPacket(ParsedPayloadH264Test* parsed_payload,
                    const uint8_t* data,
                    size_t length) {
