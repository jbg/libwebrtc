...
TEST(RtpPacketizerAv1TestEven, EvenDistributionDiffers) {
  ...
  auto payloads = Packetize(kFrame, {}, false);
  size_t expected_sizes[] = {1200, 1200, 1200, 523};

  auto payloads_even = Packetize(kFrame, {}, true);
  size_t expected_sizes_even[] = {1032, 1032, 1032, 1028};

  ASSERT_EQ(payloads.size(),
            payloads_even.size());  // Number of packets should not change.
  ASSERT_EQ(payloads.size(), arraysize(expected_sizes));
  ASSERT_EQ(payloads_even.size(), arraysize(expected_sizes_even));

  for (size_t i = 0; i < payloads.size(); i++) {
    EXPECT_EQ(payloads[i].rtp_packet.payload_size(), expected_sizes[i]);
    EXPECT_EQ(payloads_even[i].rtp_packet.payload_size(),
              expected_sizes_even[i]);
  }
}
