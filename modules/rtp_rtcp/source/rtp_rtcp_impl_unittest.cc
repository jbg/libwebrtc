...
TEST_F(RtpRtcpImplTest, ConfigurableRtcpReportInterval) {
  ...
  clock_.AdvanceTimeMilliseconds(1);
  sender_.impl_->Process();
  EXPECT_LT(-1, sender_.RtcpSent().first_packet_time_ms);
  EXPECT_EQ(1u, sender_.transport_.NumRTCPSent());
  ...
  clock_.AdvanceTimeMilliseconds(1);
  sender_.impl_->Process();
  EXPECT_LE(1u, sender_.transport_.NumRTCPSent());

  clock_.AdvanceTimeMilliseconds(kVideoReportInterval / 2);
  sender_.impl_->Process();
  EXPECT_LE(1u, sender_.transport_.NumRTCPSent());

  // Move out the range of second rtcp, the second rtcp must be sent
  clock_.AdvanceTimeMilliseconds(kVideoReportInterval / 2);
  sender_.impl_->Process();
  EXPECT_EQ(2u, sender_.transport_.NumRTCPSent());
}
