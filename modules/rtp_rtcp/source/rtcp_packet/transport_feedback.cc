...
constexpr TimeDelta kBaseScaleFactor = TransportFeedback::kDeltaTick * (1 << 8);
constexpr TimeDelta kTimeWrapPeriod = (int64_t{1} << 24) * kBaseScaleFactor;
...
void TransportFeedback::SetBase(uint16_t base_sequence,
  ...
  base_time_ticks_ =
      (ref_timestamp.us() % kTimeWrapPeriod.us()) / kBaseScaleFactor.us();
...
bool TransportFeedback::AddReceivedPacket(uint16_t sequence_number,
  ...
  if (include_timestamps_) {
    ...
    delta_full += (delta_full < 0 ? -(kDeltaTick / 2) : kDeltaTick / 2).us();
...
Timestamp TransportFeedback::BaseTime() const {
  return Timestamp::Zero() +
         static_cast<int64_t>(base_time_ticks_) * kBaseScaleFactor;
}
...
int64_t TransportFeedback::GetBaseTimeUs() const {
  ...
  // users return base time as negative when it used to be negative.
  Timestamp base_time = BaseTime();
  RTC_DCHECK_LT(base_time.us(), kTimeWrapPeriod.us());
  if (base_time - Timestamp::Zero() >= kTimeWrapPeriod / 2) {
    return base_time.us() - kTimeWrapPeriod.us();
  } else {
    return base_time.us();
  }
}

TimeDelta TransportFeedback::GetBaseDelta(Timestamp prev) const {
  TimeDelta delta = BaseTime() - prev;
...
bool TransportFeedback::IsConsistent() const {
  ...
  if (timestamp != last_timestamp_) {
    RTC_LOG(LS_ERROR) << "Last timestamp mismatch. Calculated: "
                      << timestamp.us() << ". Saved: " << last_timestamp_.us();
    return false;
  }
