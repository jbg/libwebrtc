 ...
 public:
  class ReceivedPacket {
   public:
    ...
    ABSL_DEPRECATED("Use version that returns TimeDelta")
  ...
  ABSL_DEPRECATED("Use version that returns Timestamp")
  int64_t GetBaseTimeUs() const { return BaseTime().us(); }
  ABSL_DEPRECATED("Use version that returns Timestamp")
  ...
  int64_t GetBaseDeltaUs(int64_t prev_timestamp_us) const {
    return GetBaseDelta(Timestamp::Micros(prev_timestamp_us)).us();
  }
  ABSL_DEPRECATED("Use version that takes Timestamp")
  TimeDelta GetBaseDelta(TimeDelta prev_timestamp) const {
    return GetBaseDelta(Timestamp::Zero() + prev_timestamp);
  }
  TimeDelta GetBaseDelta(Timestamp prev) const;
