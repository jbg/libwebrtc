...
TimeDelta ato_to_timedelta(uint16_t receive_info) {
  // receive_info
  // +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
  // |R|ECN|  Arrival time offset    | ...                           .
  // +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
...
rtc::EcnMarking to_ecn_marking(uint16_t receive_info) {
  const uint16_t ecn = (receive_info & 0x6000) >> 13;
  if (ecn == kEcnEct1) {
...
TransportLayerFeedback ::TransportLayerFeedback(std::vector<PacketInfo> packets,
                                                uint32_t compact_ntp_timestamp)
    : packets_(std::move(packets)),
      compact_ntp_timestamp_(compact_ntp_timestamp) {}

bool TransportLayerFeedback::Create(uint8_t* buffer,
                                    size_t* position,
                                    size_t max_length,
  ...
  *position += 4;
  ...
  //   +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
  //   .                                                               .
  auto write_report_for_ssrc = [&buffer, &position](
                                   rtc::ArrayView<const PacketInfo> packets) {
    // SSRC of nth RTP stream.
  ...
  for (size_t packet_index = 0; packet_index < packets_.size();
       ++packet_index) {
    if (packets_[packet_index].ssrc != ssrc) {
      write_report_for_ssrc(rtc::ArrayView<const PacketInfo>(
          &*packets_.begin() + packet_index - 1, number_of_packets_for_ssrc));
      ssrc = packets_[packet_index].ssrc;
      number_of_packets_for_ssrc = 1;
    } else {
      ++number_of_packets_for_ssrc;
    }
  }
  write_report_for_ssrc(rtc::ArrayView<const PacketInfo>(
      &*packets_.begin() + packets_.size() - number_of_packets_for_ssrc,
      number_of_packets_for_ssrc));
...
size_t TransportLayerFeedback::BlockLength() const {
  ...
  auto increase_size_per_ssrc = [](int number_of_packets_for_ssrc) {
    size_t size = kHeaderPerMediaSssrcLength;
    size_t packet_block_size = number_of_packets_for_ssrc * kPerPacketLength;
    size += packet_block_size + packet_block_size % 4;
    return size;
  };
      ...
      if (received) {
        packets_.push_back(
            {.ssrc = ssrc,
             ...
             .ecn = to_ecn_marking(packet_info)});
      }
    }
    if (num_reports % 2) {
      // 2 bytes padding
