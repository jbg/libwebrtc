...
bool RTPSenderVideo::SendVideo(
  ...
  const bool is_base_layer = temporal_id == 0 || temporal_id == kNoTemporalIdx;
  const bool is_discardable = video_header.generic.has_value()
                                  ? video_header.generic->discardable
                                  : false;
  if (video_header.frame_type == VideoFrameType::kVideoFrameKey ||
      (is_base_layer && !is_discardable)) {
