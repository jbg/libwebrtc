...
void RTPSenderVideo::RegisterPayloadType(int8_t payload_type,
  ...
  RtpPacketizationFormat format = PayloadNameToRtpPacketization(payload_name);
  {
    rtc::CritScope cs(&payload_type_crit_);
    payload_type_map_[payload_type] = format;
  }
...
void RTPSenderVideo::RegisterRawPayloadType(int8_t payload_type) {
  rtc::CritScope cs(&payload_type_crit_);
...
bool RTPSenderVideo::SendVideo(VideoFrameType frame_type,
  ...
  std::unique_ptr<RtpPacketizer> packetizer = RtpPacketizer::Create(
      format, rtc::MakeArrayView(payload_data, payload_size), limits,
