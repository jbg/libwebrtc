...
void RtcpTransceiverImpl::CreateCompoundPacket(Timestamp now,
  ...
  if (config_.non_sender_rtt_measurement) {
    // It look like bytes for ExtendedReport header is reserved twice, but in
    // practice same RtcpTransceiver won't both produce RRTR (i.e. it is a
    // receiver-only) and reply to RRTR (i.e. remote participant is a receiver
    // only). If that happen, then reserved_bytes would be slightly larger than
    // should, which is not an issue.
