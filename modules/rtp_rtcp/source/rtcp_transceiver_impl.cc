...
void RtcpTransceiverImpl::ReceivePacket(rtc::ArrayView<const uint8_t> packet,
  ...
  if (config_.network_link_observer != nullptr && !report_blocks.empty()) {
    ReportRttFromReportBlocks(now, report_blocks);
...
void RtcpTransceiverImpl::ReportRttFromReportBlocks(
    Timestamp now,
    rtc::ArrayView<const rtcp::ReportBlock> report_blocks) {
  RTC_DCHECK(config_.network_link_observer);
