  ...
  for (const uint8_t* next_block = packet_begin; next_block != packet_end;
    ...
    switch (rtcp_block.type()) {
      case rtcp::SenderReport::kPacketType: {
        ...
        last_report.local_received_time_us = rtc::TimeMicros();
...
std::vector<rtcp::ReportBlock> RtcpTransceiverImpl::CreateReportBlocks() {
  ...
  for (rtcp::ReportBlock& report_block : report_blocks) {
    ...
    constexpr uint32_t kMaxDelay = std::numeric_limits<uint32_t>::max();
    constexpr int kMaxDelayS = kMaxDelay / kCompactNtpPerSec;
    constexpr int64_t kMaxDelayUs = kMaxDelayS * rtc::kNumMicrosecsPerSec;
    if (delay_us > kMaxDelayUs)  // max delay ~= 18.2 hours.
      continue;
