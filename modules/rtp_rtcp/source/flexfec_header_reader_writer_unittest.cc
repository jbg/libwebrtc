...
void VerifyFinalizedHeaders(
  ...
  for (size_t i = 0; i < expected.size(); ++i) {
    SCOPED_TRACE(i);
    // Verify value of 2 seq_num_base bytes.
    EXPECT_EQ(packet[expected[i].byte_index], expected[i].seq_num_base[0]);
    EXPECT_EQ(packet[expected[i].byte_index + 1], expected[i].seq_num_base[1]);
    // Verify mask.
...
void VerifyWrittenAndReadHeaders(
  ...
  for (size_t i = 0; i < write_protected_streams.size(); ++i) {
    read_packet.protected_streams.push_back(
        {.ssrc = write_protected_streams[i].ssrc});
  }
  ...
  for (size_t i = 0; i < write_protected_streams.size(); ++i) {
    ...
    size_t mask_write_size = write_protected_streams[i].packet_mask_size;
    ...
    size_t expected_mask_read_size = writer.MinPacketMaskSize(
        write_protected_streams[i].packet_mask, mask_write_size);
...
TEST(FlexfecHeaderWriterTest, FinalizesHeaderWithKBit0SetSingleStream) {
  ...
  Packet written_packet =
      WritePacket({{.ssrc = 0x01,
                    .seq_num_base = kMediaStartSeqNum,
                    .packet_mask = kUlpfecPacketMask,
                    .packet_mask_size = sizeof(kUlpfecPacketMask)}});

  std::vector<FecPacketStreamWriteProperties> expected = {
      {.byte_index = 8,
       .seq_num_base = {kMediaStartSeqNum >> 8, kMediaStartSeqNum & 0xFF},
...
TEST(FlexfecHeaderWriterTest, FinalizesHeaderMultipleStreamsMultipleMasks) {
  ...
  Packet written_packet =
      WritePacket({{.ssrc = 0x01,
                    ...
                    .packet_mask_size = sizeof(kUlpfecPacketMask1)},
                   {.ssrc = 0x02,
                    .seq_num_base = kMediaStartSeqNum2,
                    .packet_mask = kUlpfecPacketMask2,
                    .packet_mask_size = sizeof(kUlpfecPacketMask2)},
                   {.ssrc = 0x03,
                    .seq_num_base = kMediaStartSeqNum3,
                    .packet_mask = kUlpfecPacketMask3,
                    .packet_mask_size = sizeof(kUlpfecPacketMask3)},
                   {.ssrc = 0x04,
                    .seq_num_base = kMediaStartSeqNum4,
                    .packet_mask = kUlpfecPacketMask4,
                    .packet_mask_size = sizeof(kUlpfecPacketMask4)}});

  std::vector<FecPacketStreamWriteProperties> expected = {
      {.byte_index = 8,
       .seq_num_base = {kMediaStartSeqNum1 >> 8, kMediaStartSeqNum1 & 0xFF},
       .mask = kFlexfecPacketMask1},
      {.byte_index = 24,
       .seq_num_base = {kMediaStartSeqNum2 >> 8, kMediaStartSeqNum2 & 0xFF},
       .mask = kFlexfecPacketMask2},
      {.byte_index = 28,
       .seq_num_base = {kMediaStartSeqNum3 >> 8, kMediaStartSeqNum3 & 0xFF},
       .mask = kFlexfecPacketMask3},
      {.byte_index = 36,
       .seq_num_base = {kMediaStartSeqNum4 >> 8, kMediaStartSeqNum4 & 0xFF},
