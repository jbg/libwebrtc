...
<<<<<<< PATCH SET (b436e2 Change flexfec header writer to finalize header according to)
=======
...
>>>>>>> BASE      (8de909 Change FinalizeFecHeader to receive list of streams)
  ...
  // (See FEC header schematic in flexfec_header_reader_writer.h.)
  size_t byte_index = 8;
  for (FecHeaderWriter::ProtectedStream protected_stream : protected_streams) {
    ByteWriter<uint16_t>::WriteBigEndian(&data[byte_index],
    ...
    // We treat the mask parts as unsigned integers with host order endianness
    // in order to simplify the bit shifting between bytes.
    if (protected_stream.packet_mask_size == kUlpfecPacketMaskSizeLBitSet) {
      ...
      ByteWriter<uint16_t>::WriteBigEndian(&data[byte_index], tmp_mask_part0);
      byte_index += kFlexfecPacketMaskSizes[0];
      tmp_mask_part1 >>= 2;  // Shift, thus clearing K-bit 1 and bit 15.
      ByteWriter<uint32_t>::WriteBigEndian(&data[byte_index], tmp_mask_part1);

      bool bit15 = (protected_stream.packet_mask[1] & 0x01) != 0;
      if (bit15)
        data[byte_index] |= 0x40;  // Set bit 15.

      bool bit46 = (protected_stream.packet_mask[5] & 0x02) != 0;
      bool bit47 = (protected_stream.packet_mask[5] & 0x01) != 0;
      if (!bit46 && !bit47) {
        data[byte_index] |= 0x80;  // Set K-bit 1.
        byte_index += kFlexfecPacketMaskSizes[1] - kFlexfecPacketMaskSizes[0];
      } else {
        byte_index += kFlexfecPacketMaskSizes[1] - kFlexfecPacketMaskSizes[0];
        memset(&data[byte_index], 0,
               kFlexfecPacketMaskSizes[2] -
                   kFlexfecPacketMaskSizes[1]);  // Clear all trailing bits.
        if (bit46)
          data[byte_index] |= 0x80;  // Set bit 46.
        if (bit47)
          data[byte_index] |= 0x40;  // Set bit 47.
        byte_index += kFlexfecPacketMaskSizes[2] - kFlexfecPacketMaskSizes[1];
      }
    } else if (protected_stream.packet_mask_size ==
      ...
      bool bit15 = (protected_stream.packet_mask[1] & 0x01) != 0;
      if (!bit15) {
        data[byte_index] |= 0x80;  // Set K-bit 0.
        byte_index += kFlexfecPacketMaskSizes[0];
      } else {
        byte_index += kFlexfecPacketMaskSizes[0];
        memset(&data[byte_index], 0U,
               kFlexfecPacketMaskSizes[1] -
                   kFlexfecPacketMaskSizes[0]);  // Clear all trailing bits.
        data[byte_index] |= 0x80;                // Set K-bit 1.
        data[byte_index] |= 0x40;                // Set bit 15.
        byte_index += kFlexfecPacketMaskSizes[1] - kFlexfecPacketMaskSizes[0];
      }
      ...
      RTC_DCHECK_NOTREACHED()
          << "Incorrect packet mask size: " << protected_stream.packet_mask_size
          << ".";
    }
  }
}
