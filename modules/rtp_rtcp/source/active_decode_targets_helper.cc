...
void ActiveDecodeTargetsHelper::OnFrame(
  ...
  if (num_chains == 0) {
    ...
    // when already printed for the same active decode targets, or
    // when active_decode_targets are not changed from it's default value of
    // all, including non-existent, decode targets are active.
    ...
    // remember active_decode_targets to avoid printing out same warning log.
