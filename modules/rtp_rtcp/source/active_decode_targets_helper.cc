...
uint32_t ActiveChainsBitmask(
  ...
  for (size_t dt = 0; dt < decode_target_protected_by_chain.size(); ++dt) {
    ...
    if (chain_idx < num_chains) {
      active_chains |= (uint32_t{1} << chain_idx);
    }
  }
...
void ActiveDecodeTargetsHelper::OnFrame(
  ...
  unsent_on_chain_bitmask_ = ActiveChainsBitmask(
      decode_target_protected_by_chain,
      /*num_chains=*/frame_is_part_of_chain.size(), active_decode_targets);
