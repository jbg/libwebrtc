...
TEST(RtcpTransceiverTest, CanBeDestoryedOnTaskQueueAfterCallToStop) {
  rtc::TaskQueue queue("rtcp");
...
TEST(RtcpTransceiverTest, CanBeDestoryAsynchroniouslyAfterStop) {
  rtc::TaskQueue queue("rtcp");
  NiceMock<MockTransport> outgoing_transport;
  RtcpTransceiverConfig config;
  config.outgoing_transport = &outgoing_transport;
  ...
  queue.PostTask(rtc::NewClosure([&] { heavy_task.Wait(kTimeoutMs); }));
  rtc::Event done(false, false);
