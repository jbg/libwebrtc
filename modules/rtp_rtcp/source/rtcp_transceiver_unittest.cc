...
TEST(RtcpTransceiverTest, DoesntPostToObserverAfterCallToRemove) {
  ...
  rtcp_transceiver.AddMediaReceiverObserver(kRemoteSsrc, observer.get());
...
TEST(RtcpTransceiverTest, RemoveMediaReceiverObserverIsNonBlocking) {
  const uint32_t kRemoteSsrc = 1234;
  MockTransport null_transport;
  rtc::TaskQueue queue("rtcp");
  RtcpTransceiverConfig config;
  ...
  queue.PostTask([&] { EXPECT_TRUE(queue_blocker.Wait(1000)); });
  rtcp_transceiver.RemoveMediaReceiverObserver(
      kRemoteSsrc, observer.get(),
      /*on_removed=*/rtc::NewClosure([&observer] { observer.reset(); }));

  EXPECT_THAT(observer, Not(IsNull()));
  queue_blocker.Set();
  WaitPostedTasks(&queue);
  EXPECT_THAT(observer, IsNull());
}
