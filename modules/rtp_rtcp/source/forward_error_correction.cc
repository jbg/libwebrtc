...
void ForwardErrorCorrection::InsertFecPacket(
  ...
  RTC_CHECK(fec_packet->protected_streams[0].seq_num_base.has_value());
  RTC_CHECK(fec_packet->protected_streams[0].packet_mask_offset.has_value());
  RTC_CHECK(fec_packet->protected_streams[0].packet_mask_size.has_value());
  ...
  if (fec_packet->protected_streams[0].packet_mask_offset.value() +
          fec_packet->protected_streams[0].packet_mask_size.value() >
  ...
  for (uint16_t byte_idx = 0;
       byte_idx < fec_packet->protected_streams[0].packet_mask_size.value();
       ++byte_idx) {
    uint8_t packet_mask =
        fec_packet->pkt
            ->data[fec_packet->protected_streams[0].packet_mask_offset.value() +
                   byte_idx];
    for (uint16_t bit_idx = 0; bit_idx < 8; ++bit_idx) {
      if (packet_mask & (1 << (7 - bit_idx))) {
        ...
        protected_packet->seq_num = static_cast<uint16_t>(
            fec_packet->protected_streams[0].seq_num_base.value() +
            (byte_idx << 3) + bit_idx);
