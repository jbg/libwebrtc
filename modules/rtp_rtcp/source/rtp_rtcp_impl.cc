...
ModuleRtpRtcpImpl::ModuleRtpRtcpImpl(const Configuration& configuration)
  ...
  if (!configuration.receiver_only) {
    ...
    rtcp_sender_.SetTimestampOffset(
        rtp_sender_->packet_creator_.TimestampOffset());
  }
...
void ModuleRtpRtcpImpl::SetRtxSendStatus(int mode) {
  rtp_sender_->packet_creator_.SetRtxStatus(mode);
}

int ModuleRtpRtcpImpl::RtxSendStatus() const {
  return rtp_sender_ ? rtp_sender_->packet_creator_.RtxStatus() : kRtxOff;
}

void ModuleRtpRtcpImpl::SetRtxSendPayloadType(int payload_type,
                                              int associated_payload_type) {
  rtp_sender_->packet_creator_.SetRtxPayloadType(payload_type,
                                                 associated_payload_type);
}

absl::optional<uint32_t> ModuleRtpRtcpImpl::RtxSsrc() const {
  return rtp_sender_ ? rtp_sender_->packet_creator_.RtxSsrc() : absl::nullopt;
}

absl::optional<uint32_t> ModuleRtpRtcpImpl::FlexfecSsrc() const {
  if (rtp_sender_) {
    return rtp_sender_->packet_creator_.FlexfecSsrc();
  }
  return absl::nullopt;
}
...
void ModuleRtpRtcpImpl::SetCsrcs(const std::vector<uint32_t>& csrcs) {
  ...
  rtp_sender_->packet_creator_.SetCsrcs(csrcs);
}
...
void ModuleRtpRtcpImpl::SetSendingMediaStatus(const bool sending) {
  if (rtp_sender_) {
    rtp_sender_->packet_creator_.SetSendingMediaStatus(sending);
  } else {
    RTC_DCHECK(!sending);
  }
}
...
bool ModuleRtpRtcpImpl::SendingMedia() const {
  return rtp_sender_ ? rtp_sender_->packet_creator_.SendingMedia() : false;
}
...
bool ModuleRtpRtcpImpl::TrySendPacket(RtpPacketToSend* packet,
  ...
  if (!rtp_sender_->packet_creator_.SendingMedia()) {
    return false;
  }
...
void ModuleRtpRtcpImpl::SetMaxRtpPacketSize(size_t rtp_packet_size) {
  ...
  if (rtp_sender_) {
    rtp_sender_->packet_creator_.SetMaxRtpPacketSize(rtp_packet_size);
  }
}
...
void ModuleRtpRtcpImpl::SetExtmapAllowMixed(bool extmap_allow_mixed) {
  rtp_sender_->packet_creator_.SetExtmapAllowMixed(extmap_allow_mixed);
}

int32_t ModuleRtpRtcpImpl::RegisterSendRtpHeaderExtension(
    const RTPExtensionType type,
    const uint8_t id) {
  return rtp_sender_->packet_creator_.RegisterRtpHeaderExtension(type, id);
}

void ModuleRtpRtcpImpl::RegisterRtpHeaderExtension(absl::string_view uri,
                                                   int id) {
  bool registered =
      rtp_sender_->packet_creator_.RegisterRtpHeaderExtension(uri, id);
  RTC_CHECK(registered);
}

int32_t ModuleRtpRtcpImpl::DeregisterSendRtpHeaderExtension(
    const RTPExtensionType type) {
  return rtp_sender_->packet_creator_.DeregisterRtpHeaderExtension(type);
}
void ModuleRtpRtcpImpl::DeregisterSendRtpHeaderExtension(
    absl::string_view uri) {
  rtp_sender_->packet_creator_.DeregisterRtpHeaderExtension(uri);
}
...
void ModuleRtpRtcpImpl::OnReceivedNack(
  ...
  rtp_sender_->packet_creator_.OnReceivedNack(nack_sequence_numbers, rtt);
}
...
void ModuleRtpRtcpImpl::OnReceivedRtcpReportBlocks(
  ...
  if (ack_observer_) {
    ...
    if (rtp_sender_->packet_creator_.RtxStatus() != kRtxOff) {
      rtx_ssrc = rtp_sender_->packet_creator_.RtxSsrc();
    }

    for (const RTCPReportBlock& report_block : report_blocks) {
      if (ssrc == report_block.source_ssrc) {
        rtp_sender_->packet_creator_.OnReceivedAckOnSsrc(
            report_block.extended_highest_sequence_number);
        ack_observer_->OnReceivedAck(
            report_block.extended_highest_sequence_number);
      } else if (rtx_ssrc && *rtx_ssrc == report_block.source_ssrc) {
        rtp_sender_->packet_creator_.OnReceivedAckOnRtxSsrc(
            report_block.extended_highest_sequence_number);
      }
    }
  }
...
RTPSender* ModuleRtpRtcpImpl::RtpSender() {
  return rtp_sender_ ? &rtp_sender_->packet_creator_ : nullptr;
}
