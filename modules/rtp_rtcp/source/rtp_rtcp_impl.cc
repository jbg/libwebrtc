...
void ModuleRtpRtcpImpl::Process() {
  ...
  // TODO(tommi): Figure out why we need to call Process() 200 times a second.
  ...
  if (rtp_sender_) {
    if (now >= last_bitrate_process_time_ + kRtpRtcpBitrateProcessTimeMs) {
      ...
      // TODO(tommi): Is this a bug? At the top of the function,
  ...
  // TODO(tommi): We update the RTT once a second, whereas other things that
  // run in this method are updated much more frequently. Move the RTT checking
  // over to the worker thread, which matches better with where the stats are
  // maintained.
  ...
  if (rtcp_sender_.Sending()) {
    ...
    // is increasing.
    // TODO(tommi): The timeout value needs to be checked every few seconds
    // (see internals of RtcpRrTimeout). Here, we may be polling it a couple
    // of hundred times a second, which isn't great since it grabs a lock.
    // Note also that LastReceivedReportBlockMs() (called above) and
