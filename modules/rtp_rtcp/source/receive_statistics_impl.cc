...
void StreamStatisticianImpl::IncomingPacket(const RTPHeader& header,
  ...
  RtcpStatistics rtcp_stats;
  {
    ...
    // RR.
    CalculateRtcpStatistics(&rtcp_stats, /*update_fraction_lost=*/false);
  }
...
bool StreamStatisticianImpl::GetStatistics(RtcpStatistics* statistics,
                                           bool update_fraction_lost) {
  {
    ...
    CalculateRtcpStatistics(statistics, update_fraction_lost);
  }
...
bool StreamStatisticianImpl::GetActiveStatisticsAndReset(
    RtcpStatistics* statistics) {
  {
    ...
    CalculateRtcpStatistics(statistics, /*update_fraction_lost=*/true);
  }
...
void StreamStatisticianImpl::CalculateRtcpStatistics(
    RtcpStatistics* statistics,
    bool update_fraction_lost) {
  RTC_DCHECK(statistics);
  ...
  // Note that if we've received *more* packets than we expected (due to
  // duplicates), loss is technically negative and should be snapped to zero
  // according to RFC3550.
  if (total_expected_packets >= receive_counters_.transmitted.packets) {
    // TODO(danilchap): Ensure cumulative loss is below maximum value of 2^24.
    statistics->packets_lost = extended_seq_max - received_seq_first_ + 1 -
                               receive_counters_.transmitted.packets;
  }
