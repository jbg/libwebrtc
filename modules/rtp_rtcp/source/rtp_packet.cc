...
void RtpPacket::PromoteToTwoByteHeaderExtension() {
  ...
  rtc::Buffer temp_buffer(data() + extensions_offset, extensions_size_);
  // Rewrite data.
  ...
  while (read_index < extensions_size_) {
    ...
    const uint8_t id = (temp_buffer[read_index] & 0xF0) >> 4;
    const uint8_t extension_info_length = (temp_buffer[read_index] & 0x0F) + 1;
    ++read_index;
    if (id > 0) {
      WriteAt(extensions_offset + write_index++, id);
      WriteAt(extensions_offset + write_index++, extension_info_length);
      const uint16_t extension_info_offset =
      ...
      memcpy(WriteAt(extensions_offset + write_index), &temp_buffer[read_index],
             extension_info_length);
      write_index += extension_info_length;
      read_index += extension_info_length;
    }
  }
