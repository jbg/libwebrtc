...
void RtpPacket::PromoteToTwoByteHeaderExtension() {
  ...
  for (auto extension_entry : extension_entries_) {
    size_t read_index = extension_entry.offset - extensions_offset;
    WriteAt(extensions_offset + write_index++, extension_entry.id);
    WriteAt(extensions_offset + write_index++, extension_entry.length);
    // Update offset.
    ...
    write_index += extension_entry.length;
  }
