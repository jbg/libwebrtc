...
RTCPSender::Configuration ToRtcpSenderConfiguration(
  ...
  result.non_sender_rtt_measurement = config.non_sender_rtt_measurement;
  result.event_log = config.event_log;
  if (config.rtcp_report_interval_ms) {
...
RTCPSender::RTCPSender(const RtpRtcpInterface::Configuration& config)
    : RTCPSender(ToRtcpSenderConfiguration(config)) {}
