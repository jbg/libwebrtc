...
// Arbitrary chosen. Large enough to produce good results in video_loopback.
const int kAudioTimestampsSmoothingWindow = 121;
}  // namespace
...
RTCPSender::RTCPSender(
    ...
    : audio_(audio),
      ...
      capture_timestamps_jump_estimator_(kAudioTimestampsSmoothingWindow),
      ssrc_(0),
      remote_ssrc_(0),
...
void RTCPSender::SetLastRtpTime(uint32_t rtp_timestamp,
  ...
  if (capture_time_ms < 0) {
    ...
    if (abs(capture_time_ms - time_now_ms) > 5)
      capture_time_ms = time_now_ms;
  }
