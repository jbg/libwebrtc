...
RTPSender::RTPSender(const RtpRtcp::Configuration& config,
    ...
    : clock_(config.clock),
      ...
      flexfec_ssrc_(config.flexfec_sender
                        ...
                        : absl::nullopt),
      owned_history_(packet_history == nullptr ? new RtpPacketHistory(clock_)
                                               : nullptr),
      packet_history_(packet_history == nullptr ? owned_history_.get()
                                                : packet_history),
      egress_(packet_history_ == nullptr
                  ? nullptr
                  : new RtpSenderEgress(config, packet_history_)),
      non_paced_packet_sender_(
          config.paced_sender || !egress_
              ? nullptr
              : new RtpSenderEgress::NonPacedPacketSender(egress_.get())),
      paced_sender_(packetSender ? packetSender
