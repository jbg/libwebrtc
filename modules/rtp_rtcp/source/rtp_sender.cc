...
// At the current time, the priority can be directly mapped to a packet type,
// and vice versa, so do that as fallback during transition.
...
RtpPacketPacer::Priority PacketToPriority(RtpPacketPacer::PacketType type) {
  switch (type) {
...
bool RTPSender::SendToNetwork(std::unique_ptr<RtpPacketToSend> packet,
  ...
  if (paced_sender_) {
    ...
    if (pacer_reference_packets_) {
      paced_sender_->InsertPacket(PacketToPriority(type), ssrc, seq_no,
