...
void RtpSenderEgress::SendPacket(RtpPacketToSend* packet,
  ...
  if (is_audio_) {
    ...
    BWE_TEST_LOGGING_PLOT_WITH_SSRC(1, "AudioTotBitrate_kbps", now_ms,
                                    GetSendRates().GetTotalRate().kbps(),
                                    packet_ssrc);
    BWE_TEST_LOGGING_PLOT_WITH_SSRC(
        1, "AudioNackBitrate_kbps", now_ms,
        GetSendRates().GetRate(RtpPacketMediaType::kRetransmission).kbps(),
        packet_ssrc);
#endif
  } else {
#if BWE_TEST_LOGGING_COMPILE_TIME_ENABLE
...
void RtpSenderEgress::ProcessBitrateAndNotifyObservers() {
  ...
  bitrate_callback_->Notify(
      send_rates.GetTotalRate().bps(),
      send_rates.GetRate(RtpPacketMediaType::kRetransmission).bps(), ssrc_);
}
...
RtpSendRates RtpSenderEgress::GetSendRates() const {
  ...
  for (size_t i = 0; i < kNumMediaTypes; ++i) {
    ...
    current_rates.SetRate(
        type, DataRate::BitsPerSec(send_rates_[i].Rate(now_ms).value_or(0)));
  }
...
void RtpSenderEgress::UpdateRtpStats(const RtpPacketToSend& packet) {
  ...
  RTC_DCHECK(packet.packet_type().has_value());
  auto& send_rate = send_rates_[static_cast<size_t>(*packet.packet_type())];
  send_rate.Update(packet.size(), now_ms);
