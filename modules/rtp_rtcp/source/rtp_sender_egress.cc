...
void RtpSenderEgress::ResetPacketOverhead() {
  rtc::CritScope cs(&lock_);
  sum_header_sizes_ = 0;
  header_sizes_.clear();
}

absl::optional<size_t> RtpSenderEgress::GetMediaPerPacketOverhead() const {
  rtc::CritScope cs(&lock_);
  if (header_sizes_.empty()) {
    return absl::nullopt;
  }
  return sum_header_sizes_ / header_sizes_.size();
}
...
bool RtpSenderEgress::SendPacketToNetwork(const RtpPacketToSend& packet,
  ...
  int bytes_sent = -1;
  if (transport_) {
    const bool is_media_packet =
        packet.packet_type() ==
        (is_audio_ ? RtpPacketMediaType::kAudio : RtpPacketMediaType::kVideo);
