...
size_t RtpPacketizerH264::PacketizeStapA(size_t fragment_index) {
  ...
  int payload_size_left = limits_.max_payload_len;
  if (fragment_index == 0)
    payload_size_left -= limits_.first_packet_reduction_len;
  int aggregated_fragments = 0;
  int fragment_headers_length = 0;
  for (size_t i = fragment_index; i < input_fragments_.size(); ++i) {
    const Fragment& fragment = input_fragments_[i];
    if (i == input_fragments_.size() - 1)
      payload_size_left -= limits_.last_packet_reduction_len;
    int size_needed = fragment_headers_length + fragment.length;
    if (payload_size_left < size_needed)
      break;
  ...
  ++num_packets_left_;  // multiple fragments consumed, single packet generated.
  return fragment_index + aggregated_fragments;
}
