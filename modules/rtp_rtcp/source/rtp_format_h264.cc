...
#include "rtc_base/numberics/safe_compare.h"
...
RtpPacketizerH264::RtpPacketizerH264(
  ...
  for (size_t i = 0; i < fragmentation.fragmentationVectorSize; ++i) {
    input_fragments_.push_back(
        payload.subview(fragmentation.fragmentationOffset[i],
  ...
  for (size_t i = 0; i < input_fragments_.size();) {
    switch (packetization_mode) {
      ...
      case H264PacketizationMode::NonInterleaved:
        ...
        if (rtc::SafeGt(input_fragments_[i].size(), single_packet_capacity)) {
          if (!PacketizeFuA(i))
            return false;
          ++i;
        } else {
...
bool RtpPacketizerH264::PacketizeFuA(size_t fragment_index) {
  ...
  for (size_t i = 0; i < payload_sizes.size(); ++i) {
    ...
    packets_.emplace(fragment.subview(offset, packet_length),
                     /*first_fragment=*/i == 0,
...
size_t RtpPacketizerH264::PacketizeStapA(size_t fragment_index) {
  ...
  while (payload_size_left >= payload_size_needed()) {
    ...
    packets_.emplace(fragment, aggregated_fragments == 0, false, true,
                     fragment[0]);
...
bool RtpPacketizerH264::PacketizeSingleNalu(size_t fragment_index) {
  ...
  RTC_CHECK(!fragment.empty());
  packets_.emplace(fragment, true /* first */, true /* last */,
...
bool RtpPacketizerH264::NextPacket(RtpPacketToSend* rtp_packet) {
  ...
  if (packet.first_fragment && packet.last_fragment) {
    ...
    uint8_t* buffer =
        rtp_packet->AllocatePayload(packet.source_fragment.size());
    memcpy(buffer, packet.source_fragment.data(),
           packet.source_fragment.size());
    packets_.pop();
