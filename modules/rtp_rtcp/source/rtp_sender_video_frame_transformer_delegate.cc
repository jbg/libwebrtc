...
class TransformableVideoSenderFrame : public TransformableVideoFrameInterface {
 public:
  ...
  void SetMetadata(const VideoFrameMetadata& metadata) override {
    bool old_frame_empty = header_.frame_type == VideoFrameType::kEmptyFrame;

    header_.SetFromMetadata(metadata);
    csrcs_ = metadata.GetCsrcs();

    bool new_frame_empty = header_.frame_type == VideoFrameType::kEmptyFrame;
    LOG(ERROR)
        << "HERRE TransformableVideoSenderFrame::SetMetadata with csrc count "
        << csrcs_.size() << " old frame empty: " << old_frame_empty
        << ", new frame empty: " << new_frame_empty;
...
void RTPSenderVideoFrameTransformerDelegate::SendVideo(
  ...
  LOG(ERROR) << "HERRE RTPSenderVideoFrameTransformerDelegate::SendVideo with "
                "csrc count "
             << transformed_video_frame->GetMetadata().GetCsrcs().size();
