...
int ParseVP8Extension(RTPVideoHeaderVP8* vp8,
  ...
  bool has_picture_id = (data[0] & 0x80) ? true : false;   // I bit
  bool has_tl0_pic_idx = (data[0] & 0x40) ? true : false;  // L bit
  bool has_tid = (data[0] & 0x20) ? true : false;          // T bit
  bool has_key_idx = (data[0] & 0x10) ? true : false;      // K bit
  ...
  if (has_tid || has_key_idx) {
    ...
    if (has_tid) {
      ...
      vp8->layerSync = (data[parsed_bytes] & 0x20) ? true : false;  // Y bit
    }
    if (has_key_idx) {
...
bool ParseVP8VideoResolution(RTPVideoHeader* video_header,
                             rtc::ArrayView<const uint8_t> data) {
...
absl::optional<RtpVideoDepacketizer::Parsed> RtpVideoDepacketizerVp8::Parse(
    rtc::CopyOnWriteBuffer rtp_payload) {
  rtc::ArrayView<const uint8_t> payload(rtp_payload.cdata(),
                                        rtp_payload.size());
  absl::optional<Parsed> parsed(absl::in_place);
...
size_t RtpVideoDepacketizerVp8::ParseRtpPayload(
  ...
  vp8_header.nonReference = (payload[0] & 0x20) ? true : false;  // N bit
  ...
  if (beginning_of_partition && partition_id == 0 &&
    ...
    if (!ParseVP8VideoResolution(video_header, payload.subview(offset))) {
      return kFailedToParse;
    }
  } else {
