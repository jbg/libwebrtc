...
bool CalculateObuSizes(ObuInfo* obu_info) {
  ...
  obu_info->prefix[0] = obu_header | kObuHasSizeBit;
  ...
  // All insertions that invalide iterators pointing into obu_info->data are
  // done in ParseObus function. Thus iterator would stay valid until used in
  // StoreObu function.
  obu_info->payload_offset = it;
  obu_info->prefix_size += WriteLeb128(
      obu_info->payload_size, obu_info->prefix.data() + obu_info->prefix_size);
  return true;
}

// Writes obu into raw buffer.
void StoreObu(const ObuInfo& obu_info, uint8_t* write_at) {
  // Copy the obu_header and obu_size fields.
  memcpy(write_at, obu_info.prefix.data(), obu_info.prefix_size);
  // Copy the payload.
  obu_info.data.CopyTo(write_at + obu_info.prefix_size,
                       obu_info.payload_offset);
}
...
rtc::scoped_refptr<EncodedImageBuffer> RtpDepacketizerAv1::AssembleFrame(
  ...
  for (const ObuInfo& obu_info : obu_infos) {
    StoreObu(obu_info, write_at);
    write_at += (obu_info.prefix_size + obu_info.payload_size);
  }
