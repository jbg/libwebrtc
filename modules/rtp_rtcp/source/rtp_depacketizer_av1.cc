...
bool CalculateObuSizes(ObuInfo* obu) {
  ...
  if (!ObuHasSize(obu_header)) {
    ...
    uint8_t byte;
    do {
      if (it == obu->data.end() || size_size >= 8) {
...
void StoreObu(const ObuInfo& obu, uint8_t* write_at) {
  auto header_iterator = obu.data.begin();
  uint8_t obu_header = *header_iterator;
  *write_at = obu_header | kObuHasSizeBit;
  ++write_at;
  int size_size;
  if (ObuHasExtension(obu_header)) {
    *write_at = *++header_iterator;  // extension_header
    ++write_at;
    size_size = obu.prefix_size - 2;
  } else {
    size_size = obu.prefix_size - 1;
  }
  // Copy the obu_size field.
  memcpy(write_at, obu.encoded_payload_size, size_size);
  write_at += size_size;
  // Copy the payload.
  obu.data.CopyTo(write_at, obu.payload_offset);
}
