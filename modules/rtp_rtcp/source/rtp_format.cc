...
std::vector<int> RtpPacketizer::SplitAboutEqually(
  ...
  if (limits.max_payload_len <= 0 ||
      limits.max_payload_len - limits.first_packet_reduction_len < 1 ||
      limits.max_payload_len - limits.last_packet_reduction_len < 1) {
    // Capacity not enough to put a single byte into one of the packets.
    return result;
  }
  ...
  if (payload_len < num_packets_left) {
    ...
    // bytes. E.g. there might be single byte of payload that can't be put
    // into single packet if
