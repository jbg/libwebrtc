...
constexpr int64_t kInvalidLastReceiveTimeMs =
    std::numeric_limits<int64_t>::min();
}  // namespace

constexpr int64_t AbsoluteCaptureTimeReceiver::kInterpolationMaxIntervalMs;

AbsoluteCaptureTimeReceiver::AbsoluteCaptureTimeReceiver(Clock* clock)
    : clock_(clock),
      remote_to_local_clock_offset_(absl::nullopt),
      last_receive_time_ms_(kInvalidLastReceiveTimeMs) {}
...
void AbsoluteCaptureTimeReceiver::SetRemoteToLocalClockOffset(
  ...
  remote_to_local_clock_offset_ = value;
}
...
AbsoluteCaptureTimeReceiver::OnReceivePacket(
  ...
  const int64_t receive_time_ms = clock_->TimeInMilliseconds();

  rtc::CritScope cs(&crit_);

  AbsoluteCaptureTime extension;
  if (received_extension == absl::nullopt) {
    if (!ShouldInterpolateExtension(receive_time_ms, source, rtp_timestamp,
                                    rtp_clock_frequency)) {
      last_receive_time_ms_ = kInvalidLastReceiveTimeMs;
      return absl::nullopt;
    }
...
bool AbsoluteCaptureTimeReceiver::ShouldInterpolateExtension(
  ...
  if (last_receive_time_ms_ == kInvalidLastReceiveTimeMs) {
    return false;
  }

  // Shouldn't if the last received extension is too old.
  if ((receive_time_ms - last_receive_time_ms_) > kInterpolationMaxIntervalMs) {
    return false;
  }
