...
TEST_F(RtpPacketizerVp8Test, TestEqualSizeModeFallback) {
  ...
  options.max_payload_len = 12;  // Small enough to produce 4 packets.
  RtpPacketizerVp8 packetizer(hdr_info_, helper_->payload(), options);
...
TEST_F(RtpPacketizerVp8Test, TestEqualSizeWithLastPacketReduction) {
  ...
  RtpPacketizer::Options options;
  options.max_payload_len = 15;  // Small enough to produce 5 packets.
  options.last_packet_reduction_len = 5;
  RtpPacketizerVp8 packetizer(hdr_info_, helper_->payload(), options);
  size_t num_packets = packetizer.NumPackets();
...
TEST_F(RtpPacketizerVp8Test, TestNonReferenceBit) {
  ...
  RtpPacketizer::Options options;
  options.max_payload_len = 25;  // Small enough to produce two packets.
  RtpPacketizerVp8 packetizer(hdr_info_, helper_->payload(), options);
  size_t num_packets = packetizer.NumPackets();
...
TEST_F(RtpPacketizerVp8Test, TestTl0PicIdxAndTID) {
  ...
  RtpPacketizer::Options options;
  options.max_payload_len = helper_->buffer_size();
  RtpPacketizerVp8 packetizer(hdr_info_, helper_->payload(), options);
  size_t num_packets = packetizer.NumPackets();
...
TEST_F(RtpDepacketizerVp8Test, TestWithPacketizer) {
  ...
  RtpPacketizer::Options options;
  options.max_payload_len = 20;
  RtpPacketizerVp8 packetizer(input_header, data, options);
