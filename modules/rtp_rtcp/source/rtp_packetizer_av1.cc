...
std::vector<RtpPacketizerAv1::Packet> RtpPacketizerAv1::PacketizeAboutEqually(
  ...
  for (const auto& packet : packets) {
    int available_bytes =
        ...
        kAggregationHeaderSize;  // Every packet has to have
                                 // an aggregation header of size
  ...
  if (packet_size_left_unused >
      packet_index * kMinBytesSavedPerPacketWithEvenDistribution) {
    size_t even_max_packet_limit = limits.max_payload_len;
    size_t size_reduction = packet_size_left_unused / packet_index;

    if (even_max_packet_limit > size_reduction &&
        size_reduction > kBytesOverheadEvenDistribution) {
      even_max_packet_limit -=
          (size_reduction - kBytesOverheadEvenDistribution);
    }
    PayloadSizeLimits limits_even = limits;
    limits_even.max_payload_len = static_cast<int>(even_max_packet_limit);
    if (limits_even.max_payload_len - limits_even.last_packet_reduction_len <
            3 ||
        limits_even.max_payload_len - limits_even.first_packet_reduction_len <
            3) {
      return packets;
    }
    // Second pass, the number of packets should not change.
    std::vector<Packet> packets_even = Packetize(obus, limits_even);
