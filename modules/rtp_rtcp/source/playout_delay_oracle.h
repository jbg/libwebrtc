...
class PlayoutDelayOracle {
 ...
 private:
  ...
  // been sent.
  int64_t sequence_number_ RTC_GUARDED_BY(crit_sect_);
  // Indicates whether the latest playout delay has been acked.
  bool playout_delay_acked_ RTC_GUARDED_BY(crit_sect_);
  ...
  PlayoutDelay latest_delay_ RTC_GUARDED_BY(crit_sect_);
