...
void RTCPReceiver::SetRemoteSSRC(uint32_t ssrc) {
  ...
  sr_stats_.last_arrival_timestamp.Reset();
  remote_ssrc_ = ssrc;
}
...
RTCPReceiver::GetSenderReportStats() const {
  MutexLock lock(&rtcp_receiver_lock_);
  if (sr_stats_.last_arrival_timestamp.Valid()) {
    return sr_stats_;
  }
  return absl::nullopt;
}
...
void RTCPReceiver::HandleSenderReport(const CommonHeader& rtcp_block,
  ...
  if (remote_ssrc_ == remote_ssrc) {
    ...
    sr_stats_.last_remote_timestamp = sender_report.ntp();
    sr_stats_.last_remote_rtp_timestamp = sender_report.rtp_timestamp();
    sr_stats_.last_arrival_timestamp = clock_->CurrentNtpTime();
    sr_stats_.packets_sent = sender_report.sender_packet_count();
    sr_stats_.bytes_sent = sender_report.sender_octet_count();
    sr_stats_.reports_count++;
  } else {
