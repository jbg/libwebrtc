...
struct RTCPReceiver::PacketInformation {
  ...
  // Choose rtt with minimum delay as it will represent the latest rtt.
  uint32_t min_rtt_delay = std::numeric_limits<uint32_t>::max();
  int64_t rtt_ms = 0;
...
void RTCPReceiver::HandleReportBlock(const ReportBlock& report_block,
  ...
  if (!receiver_only_ && send_time_ntp != 0) {
    ...
    if (packet_information->min_rtt_delay > delay_ntp) {
      packet_information->min_rtt_delay = delay_ntp;
