...
RTCPReceiver::RTCPReceiver(const RtpRtcp::Configuration& config,
    ...
    : clock_(config.clock),
      ...
      handle_remote_estimates_(
          (config.field_trials ? config.field_trials : &default_field_trials_)
              ->Lookup("WebRTC-RTCP-RemoteEstimate")
              .find("Disabled") != 0),
...
void RTCPReceiver::HandleApp(const rtcp::CommonHeader& rtcp_block,
                             PacketInformation* packet_information) {
  if (handle_remote_estimates_ &&
      rtcp::RemoteEstimate::IsNetworkEstimate(rtcp_block)) {
