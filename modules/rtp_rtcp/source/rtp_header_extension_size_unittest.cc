...
TEST(RtpHeaderExtensionSizeTest, ReturnsZeroIfNoExtensionsAreRegistered) {
  ...
  EXPECT_EQ(RtpHeaderExtensionSize(kExtensionSizes, registered), 0u);
}
...
TEST(RtpHeaderExtensionSizeTest, IncludesSizeOfExtensionHeaders) {
  ...
  // 4 bytes for extenion block header + 1 byte for indiviaul extension header
  // + 3 bytes for the value.
  EXPECT_EQ(RtpHeaderExtensionSize(kExtensionSizes, registered), 8u);
}

TEST(RtpHeaderExtensionSizeTest, RoundsUpTo32bitAlignmant) {
  constexpr RtpExtensionSize kExtensionSizes[] = {{RtpMid::kId, 4}};
  RtpHeaderExtensionMap registered;
  registered.Register<RtpMid>(kId);

  // 9 bytes of data including headers + 3 bytes of padding.
  EXPECT_EQ(RtpHeaderExtensionSize(kExtensionSizes, registered), 12u);
}
...
TEST(RtpHeaderExtensionSizeTest, SumsSeveralExtensions) {
  ...
  // 4 bytes for extenion block header + 18 bytes of value +
  // 2 bytes for two headers
  EXPECT_EQ(RtpHeaderExtensionSize(kExtensionSizes, registered), 24u);
}
...
TEST(RtpHeaderExtensionSizeTest, LargeIdForce2BytesHeader) {
  ...
  // 4 bytes for extenion block header + 5 bytes of value +
  // 2*2 bytes for two headers + 3 bytes of padding.
  EXPECT_EQ(RtpHeaderExtensionSize(kExtensionSizes, registered), 16u);
}
...
TEST(RtpHeaderExtensionSizeTest, LargeValueForce2BytesHeader) {
  ...
  // 4 bytes for extenion block header + 21 bytes of value +
  // 2*2 bytes for two headers + 3 byte of padding.
  EXPECT_EQ(RtpHeaderExtensionSize(kExtensionSizes, registered), 32u);
}
