...
bool RtpPacket::SetExtension(Values... values) {
  ...
  // TODO(webrtc:7990): Max size should depend on the current mode determining
  // if mixed one- and two-byte header extensions is supported. This will be
  // negotiated over SDP.
  // TODO(webrtc:7990): Add support for empty extensions, i.e., value_size==0.
  if (value_size == 0 || value_size > RtpExtension::kMaxValueSize)
    return false;
