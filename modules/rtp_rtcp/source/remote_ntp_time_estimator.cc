...
RemoteNtpTimeEstimator::~RemoteNtpTimeEstimator() = default;
...
bool RemoteNtpTimeEstimator::UpdateRtcpTimestamp(TimeDelta rtt,
  ...
  // Convert rtt to the ntp time units: take rtt in seconds and multiple by 2^32
  // For better precsision rtt is taken with best TimeDelta precision (us), and
  // then multiplaction and conversion to seconds are swapped to avoid float
  // arithmetic.
  int64_t rtt_ntp = rtt.us() * (int64_t{1} << 32) / 1'000'000;
  // Assume connection is symmetric and thus time to deliver the packet is half
  // the round trip time.
  int64_t deliver_time_ntp = rtt_ntp / 2;
