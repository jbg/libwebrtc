...
BbrNetworkController::BbrControllerConfig::GBBRConfig() {
  ...
  config.encoder_rate_gain_in_probe_rtt = 0.1;
  // Document prescribes 2*min_rtt to enter recovery, approximated with 2*200 ms
...
BbrNetworkController::BbrControllerConfig::QUICConfig() {
  ...
  config.exit_startup_rtt_threshold_ms = 0;  // Disabled
...
void BbrNetworkController::OnTransportPacketsFeedback(
  ...
  Optional<SentPacket> last_sent_packet =
  ...
  vector<PacketResult> acked_packets = msg.ReceivedWithSendInfo();
  vector<PacketResult> lost_packets = msg.LostWithSendInfo();
...
void BbrNetworkController::UpdateBandwidth(
  ...
  for (const PacketResult& packet : packets) {
    ...
    DataRate send_rate = result.acked_data / result.send_timespan;
