...
TEST_F(RttStatsTest, SmoothedRttStability) {
  for (int64_t time = 3; time < 20000; time++) {
    ...
    for (int64_t i = 0; i < 100; i++) {
      stats.UpdateRtt(TimeDelta::us(time), TimeDelta::ms(0), Timestamp::kEpoch);
...
TEST_F(RttStatsTest, PreviousSmoothedRtt) {
  ...
  rtt_stats_.UpdateRtt(TimeDelta::ms(100), TimeDelta::Zero(),
                       Timestamp::kEpoch);
...
TEST_F(RttStatsTest, ExpireSmoothedMetrics) {
  ...
  rtt_stats_.UpdateRtt(initial_rtt, TimeDelta::Zero(), Timestamp::kEpoch);
  ...
  rtt_stats_.UpdateRtt(doubled_rtt, TimeDelta::Zero(), Timestamp::kEpoch);
  ...
  rtt_stats_.UpdateRtt(half_rtt, TimeDelta::Zero(), Timestamp::kEpoch);
...
TEST_F(RttStatsTest, UpdateRttWithBadSendDeltas) {
  ...
  for (TimeDelta bad_send_delta : bad_send_deltas) {
    rtt_stats_.UpdateRtt(bad_send_delta, TimeDelta::Zero(), Timestamp::kEpoch);
    EXPECT_EQ(initial_rtt, rtt_stats_.min_rtt());
    EXPECT_EQ(initial_rtt, rtt_stats_.smoothed_rtt());
  }
}
