...
void SendSideCongestionController::MaybeRecreateControllers() {
  ...
  bool create_controller = controller_ == nullptr;
  bool use_feedback_only_controller =
      packet_feedback_available_ && controller_factory_with_feedback_;
  bool recreate_controller =
      use_feedback_only_controller != feedback_only_controller_;
  if (create_controller || recreate_controller) {
    if (use_feedback_only_controller) {
...
void SendSideCongestionController::StartProcessPeriodicTasks() {
  ...
  if (controller_task_) {
    ...
    // Only create UpdatePacerQueue task on the first call.
    send_side_cc_internal::StartPeriodicTask(
