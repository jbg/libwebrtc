...
void SendSideCongestionController::StartProcessPeriodicTasks() {
  ...
  if (!pacer_queue_update_task_) {
    pacer_queue_update_task_ =
        RepeatedTask::Start(kPacerQueueUpdateInterval, [this]() {
          RTC_DCHECK_RUN_ON(task_queue_);
          UpdatePacerQueue();
  ...
  if (controller_task_) {
    ...
    RepeatedTask::PostStop(std::move(controller_task_));
  }
  if (process_interval_.IsFinite()) {
    ...
    // first.
    controller_task_ = RepeatedTask::Start(process_interval_, [this]() {
      RTC_DCHECK_RUN_ON(task_queue_);
      UpdateControllerWithTimeInterval();
