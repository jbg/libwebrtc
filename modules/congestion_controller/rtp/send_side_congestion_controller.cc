...
void SendSideCongestionController::SetPerPacketFeedbackAvailable(
  ...
  task_queue_->PostTask([this, packet_feedback_available]() {
    RTC_DCHECK_RUN_ON(task_queue_.get());
    bool has_feedback_controller_ = feedback_controller_;
    if (feedback_controller_factory_ &&
        (has_feedback_controller_ != packet_feedback_available))
      RecreateNetworkControllers();
  });
}
