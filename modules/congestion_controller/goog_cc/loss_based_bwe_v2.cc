...
double GetLossProbability(double inherent_loss,
  ...
  if (inherent_loss < 0 || inherent_loss > 1) {
  ...
  double loss_probability = std::min(std::max(inherent_loss, 0.0), 1.0);
...
absl::optional<LossBasedBweV2::Config> CreateConfig(
...
bool IsValid(const LossBasedBweV2::Config& config) {
  ...
  if (config.bandwidth_rampup_upper_bound_factor <= 1.0) {
    RTC_LOG(LS_WARNING)
        << "The bandwidth rampup upper bound factor must be greater than 1: "
        << config.bandwidth_rampup_upper_bound_factor;
    valid = false;
  }
  if (config.higher_bandwidth_bias_factor < 0.0) {
    RTC_LOG(LS_WARNING)
  ...
  if (config.inherent_loss_upper_bound_bandwidth_balance <= DataRate::Zero()) {
    RTC_LOG(LS_WARNING)
        << "The inherent loss upper bound bandwidth balance "
           "must be positive: "
        << ToString(config.inherent_loss_upper_bound_bandwidth_balance);
    valid = false;
  }
  ...
  if (config.tcp_fairness_upper_bound_bandwidth_balance <= DataRate::Zero()) {
    RTC_LOG(LS_WARNING)
        << "The TCP fairness upper bound bandwidth balance must be positive: "
        << ToString(config.tcp_fairness_upper_bound_bandwidth_balance);
    valid = false;
  }
...
LossBasedBweV2::Derivatives LossBasedBweV2::GetDerivatives(
  ...
  for (const Observation& observation : observations_) {
    ...
    // TODO(linderborg): Workaround for when `loss_probability` is 0 or 1.
    derivatives.first +=
        temporal_weight *
        ((observation.num_lost_packets / loss_probability) -
         (observation.num_received_packets / (1 - loss_probability)));
    derivatives.second -=
        temporal_weight *
...
double LossBasedBweV2::GetInherentLossUpperBound(DataRate bandwidth) const {
  ...
  auto inherent_loss_upper_bound =
  ...
  for (const Observation& observation : observations_) {
    ...
    objective +=
        ...
        ((observation.num_lost_packets * std::log(loss_probability)) +
         (observation.num_received_packets * std::log(1 - loss_probability)));
    objective +=
...
void LossBasedBweV2::NewtonsMethodUpdate(
  ...
  for (int i = 0; i < config_->newton_iterations; ++i) {
    ...
    // TODO(linderborg): Workaround for when the second derivative is 0.
    channel_parameters.inherent_loss -=
