...
TEST_F(ProbeControllerTest, TestAllocatedBitrateCap) {
  ...
  // Set an estimated bitrate above the current max probe level, this will be
  // the new cap.
  estimated_bitrate_bps = 2 * max_allocated_bps + 1 * kMbpsMultiplier;
  probes =
      probe_controller_->SetEstimatedBitrate(estimated_bitrate_bps, NowMs());
  clock_.AdvanceTimeMilliseconds(5000);
  probes = probe_controller_->Process(NowMs());
  EXPECT_EQ(probes[0].target_data_rate.bps(), estimated_bitrate_bps);
