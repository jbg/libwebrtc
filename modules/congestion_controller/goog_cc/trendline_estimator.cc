...
absl::optional<double> ComputeSlopeCap(
  ...
  RTC_DCHECK(settings.beginning_packets + settings.end_packets <=
             packets.size());
  if (settings.beginning_packets + settings.end_packets > packets.size())
    return absl::nullopt;
...
TrendlineEstimatorSettings::TrendlineEstimatorSettings(
  ...
  if (window_size < 10 || 100 < window_size) {
    RTC_LOG(LS_WARNING) << "Window size must be between 10 and 100 packets";
    window_size = kDefaultTrendlineWindowSize;
  }
  if (enable_cap) {
    if (beginning_packets + end_packets > window_size) {
      RTC_LOG(LS_WARNING)
          << "Size of beginning plus end must be at most the window size";
      enable_cap = false;
    ...
    if (beginning_packets < 1 || end_packets < 1) {
      RTC_LOG(LS_WARNING)
          << "Need at least one packet in beginning and end window slice";
      enable_cap = false;
      beginning_packets = end_packets = 0;
      cap_uncertainty = 0.0;
    }
