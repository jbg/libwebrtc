...
class GoogCcNetworkControllerTest : public ::testing::Test {
 protected:
  GoogCcNetworkControllerTest() : current_time_(Timestamp::ms(123456)) {}
  ~GoogCcNetworkControllerTest() override {}

  void SetUp() override {
    controller_.reset(
        new GoogCcNetworkController(&event_log_, InitialConfig()));
  ...
  NetworkRouteChange CreateRouteChange(DataRate start_rate = DataRate(),
                                       DataRate min_rate = DataRate(),
                                       DataRate max_rate = DataRate()) {
