...
NetworkControlUpdate GoogCcNetworkController::OnNetworkRouteChange(
  ...
  if (safe_reset_on_route_change_) {
    ...
    if (estimated_bitrate_bps &&
        (!msg.constraints.starting_rate ||
         estimated_bitrate_bps < msg.constraints.starting_rate->bps())) {
      msg.constraints.starting_rate = DataRate::bps(*estimated_bitrate_bps);
    }
  }
  ...
  NetworkControlUpdate update;
  update.probe_cluster_configs = TriggerConstraintsUpdate(msg.at_time);
...
NetworkControlUpdate GoogCcNetworkController::OnProcessInterval(
  ...
  NetworkControlUpdate update;
  if (initial_config_) {
    SetConstraints(initial_config_->constraints);
    update.probe_cluster_configs = TriggerConstraintsUpdate(msg.at_time);
...
NetworkControlUpdate GoogCcNetworkController::OnTargetRateConstraints(
  ...
  NetworkControlUpdate update;
  update.probe_cluster_configs = TriggerConstraintsUpdate(constraints.at_time);
  MaybeTriggerOnNetworkChanged(&update, constraints.at_time);
  return update;
}

void GoogCcNetworkController::SetConstraints(
    TargetRateConstraints new_constraints) {
  // TODO(holmer): We should make sure the default bitrates are set to 10 kbps,
  // and that we don't try to set the min bitrate to 0 from any applications.
  // The congestion controller should allow a min bitrate of 0.
  min_data_rate_ = congestion_controller::GetMinBitrate();
  if (new_constraints.min_data_rate) {
    min_data_rate_ = std::max(min_data_rate_, *new_constraints.min_data_rate);
  }
  ...
  starting_rate_ =
      new_constraints.starting_rate.value_or(DataRate::PlusInfinity());
  if (starting_rate_ < min_data_rate_) {
...
GoogCcNetworkController::TriggerConstraintsUpdate(Timestamp at_time) {
  bandwidth_estimation_->SetBitrates(starting_rate_, min_data_rate_,
                                     max_data_rate_, at_time);

  if (starting_rate_.IsFinite())
  ...
  return probe_controller_->SetBitrates(
      min_data_rate_.bps(), starting_rate_.bps_or(-1),
