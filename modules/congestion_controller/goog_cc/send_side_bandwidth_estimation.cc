...
void SendSideBandwidthEstimation::OnRouteChange() {
  ...
  current_target_bitrate_ = DataRate::Zero();
  min_bitrate_configured_ =
      DataRate::bps(congestion_controller::GetMinBitrateBps());
  max_bitrate_configured_ = kDefaultMaxBitrate;
...
DataRate SendSideBandwidthEstimation::target_rate() const {
  RTC_DCHECK_GE(current_target_bitrate_, min_bitrate_configured_);
...
void SendSideBandwidthEstimation::OnProcessInterval(Timestamp at_time) {
  if (rtt_backoff_.CorrectedRtt(at_time) > rtt_backoff_.rtt_limit_) {
    if (at_time - time_last_decrease_ >= rtt_backoff_.drop_interval_ &&
        current_target_bitrate_ > rtt_backoff_.bandwidth_floor_) {
      time_last_decrease_ = at_time;
      DataRate new_bitrate =
          std::max(current_target_bitrate_ * rtt_backoff_.drop_fraction_,
...
void SendSideBandwidthEstimation::UpdateReceiverEstimate(
  ...
  UpdateTargetBitrate(at_time, current_target_bitrate_);
}
...
void SendSideBandwidthEstimation::UpdateUmaStatsPacketsLost(Timestamp at_time,
  ...
  DataRate bitrate_kbps =
      DataRate::kbps((current_target_bitrate_.bps() + 500) / 1000);
...
void SendSideBandwidthEstimation::UpdateEstimate(Timestamp at_time) {
  ...
  if (last_fraction_loss_ == 0 && IsInStartPhase(at_time)) {
    DataRate new_bitrate = GetUpperLimit();
    MaybeLogLowBitrateWarning(new_bitrate, at_time);
    new_bitrate = std::max(new_bitrate, min_bitrate_configured_);
    if (new_bitrate.IsInfinite() && new_bitrate != current_target_bitrate_) {
      min_bitrate_history_.clear();
      if (loss_based_bandwidth_estimation_.Enabled()) {
        ...
        min_bitrate_history_.push_back(
            std::make_pair(at_time, current_target_bitrate_));
      }
      current_target_bitrate_ = new_bitrate;
  ...
  if (loss_based_bandwidth_estimation_.Enabled()) {
    ...
    DataRate new_bitrate =
        MaybeRampupOrBackoff(current_target_bitrate_, at_time);
    UpdateTargetBitrate(at_time, new_bitrate);
    return;
  }
  ...
  // to congestion.
  if (current_target_bitrate_ < bitrate_threshold_ ||
    ...
    UpdateTargetBitrate(at_time, new_bitrate);
    return;
  } else if (current_target_bitrate_ > bitrate_threshold_) {
    if (loss <= high_loss_threshold_) {
      ...
      if (!has_decreased_since_last_fraction_loss_ &&
        ...
        DataRate new_bitrate =
            DataRate::bps((current_target_bitrate_.bps() *
...
void SendSideBandwidthEstimation::UpdateMinHistory(Timestamp at_time) {
  ...
  while (!min_bitrate_history_.empty() &&
         current_target_bitrate_ <= min_bitrate_history_.back().second) {
...
void SendSideBandwidthEstimation::MaybeLogLossBasedEvent(Timestamp at_time) {
  if (current_target_bitrate_ != last_logged_bitrate_ ||
      last_fraction_loss_ != last_logged_fraction_loss_ ||
      at_time - last_rtc_event_log_ > kRtcEventLogPeriod) {
    event_log_->Log(std::make_unique<RtcEventBweUpdateLossBased>(
        current_target_bitrate_.bps(), last_fraction_loss_,
        expected_packets_since_last_loss_update_));
    last_logged_fraction_loss_ = last_fraction_loss_;
    last_logged_bitrate_ = current_target_bitrate_;
...
void SendSideBandwidthEstimation::UpdateTargetBitrate(Timestamp at_time,
  ...
  current_target_bitrate_ = bitrate;
  MaybeLogLossBasedEvent(at_time);
  link_capacity_.OnRateUpdate(acknowledged_rate_, current_target_bitrate_,
                              at_time);
}
}  // namespace webrtc
