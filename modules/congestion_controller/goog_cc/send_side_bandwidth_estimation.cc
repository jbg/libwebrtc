...
SendSideBandwidthEstimation::SendSideBandwidthEstimation(
    ...
    : rtt_backoff_(key_value_config),
      ...
      loss_based_bwe_v1_(key_value_config),
      loss_based_bwe_v2_(LossBasedBweV2::Create(key_value_config)),
...
void SendSideBandwidthEstimation::IncomingPacketFeedbackVector(
  ...
  if (LossBasedBweV1Enabled()) {
    loss_based_bwe_v1_.UpdateLossStatistics(report.packet_feedbacks,
                                            report.feedback_time);
  }
  if (LossBasedBweV2Enabled()) {
    loss_based_bwe_v2_.UpdateBwe(report.packet_feedbacks);
  }
}
...
void SendSideBandwidthEstimation::UpdateEstimate(Timestamp at_time) {
  ...
  if (last_fraction_loss_ == 0 && IsInStartPhase(at_time)) {
    ...
    if (LossBasedBweV1Enabled()) {
      loss_based_bwe_v1_.Initialize(new_bitrate);
    }
    if (LossBasedBweV2Enabled()) {
      loss_based_bwe_v2_.SetInitialBwPrediction(new_bitrate);
    }
  ...
  if (LossBasedBweV1ReadyForUse()) {
    DataRate new_bitrate =
        loss_based_bwe_v1_.Update(at_time, min_bitrate_history_.front().second,
                                  delay_based_limit_, last_round_trip_time_);
    UpdateTargetBitrate(new_bitrate, at_time);
    return;
  }

  if (LossBasedBweV2ReadyForUse()) {
    DataRate new_bitrate = loss_based_bwe_v2_.GetBandwidthEstimate();
...
bool SendSideBandwidthEstimation::LossBasedBweV1Enabled() const {
  return loss_based_bwe_v1_.Enabled() && !LossBasedBweV2Enabled();
}

bool SendSideBandwidthEstimation::LossBasedBweV1ReadyForUse() const {
  return LossBasedBweV1Enabled() && loss_based_bwe_v1_.InUse();
}

bool SendSideBandwidthEstimation::LossBasedBweV2Enabled() const {
  return loss_based_bwe_v2_.IsEnabled();
}

bool SendSideBandwidthEstimation::LossBasedBweV2ReadyForUse() const {
  return LossBasedBweV2Enabled() && loss_based_bwe_v2_.IsReady();
}
