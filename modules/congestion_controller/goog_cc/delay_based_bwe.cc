...
DelayBasedBwe::DelayBasedBwe(const WebRtcKeyValueConfig* key_value_config,
                             ...
                             NetworkPredictor* network_predictor)
    : event_log_(event_log),
      ...
      alr_limited_backoff_enabled_(
          key_value_config->Lookup("WebRTC-Bwe-AlrLimitedBackoff")
              .find("Enabled") == 0),
      network_predictor_(network_predictor),
      last_packet_send_time_ms_(0),
      last_packet_arrival_time_ms_(0),
  ...
  delay_detector_.reset(new TrendlineEstimator(
      ...
      trendline_threshold_gain_, network_predictor_ != nullptr));
}
...
DelayBasedBwe::Result DelayBasedBwe::IncomingPacketFeedbackVector(
  ...
  BandwidthUsage prev_detector_state = delay_detector_->StatePredicted();
  for (const auto& packet_feedback : packet_feedback_vector) {
    ...
    if (prev_detector_state == BandwidthUsage::kBwUnderusing &&
        delay_detector_->StatePredicted() == BandwidthUsage::kBwNormal) {
      recovered_from_overuse = true;
    }
    prev_detector_state = delay_detector_->StatePredicted();
  }
...
void DelayBasedBwe::IncomingPacketFeedback(
  ...
  if (network_predictor_ != nullptr) {
    if (last_packet_send_time_ms_ == 0) {
      last_packet_send_time_ms_ = packet_feedback.send_time_ms;
      last_packet_arrival_time_ms_ = packet_feedback.arrival_time_ms;
      delay_detector_->SetStatePredicted(BandwidthUsage::kBwNormal);
      return;
    }
    int64_t send_time_delta_ms =
        packet_feedback.send_time_ms - last_packet_send_time_ms_;
    int64_t arrival_time_delta_ms =
        packet_feedback.arrival_time_ms - last_packet_arrival_time_ms_;
    int64_t delay_time_ms =
        packet_feedback.arrival_time_ms - packet_feedback.send_time_ms;
    int64_t delay_time_delta_ms = arrival_time_delta_ms - send_time_delta_ms;
    int64_t rtt_ms =
        clock_->TimeInMilliseconds() - packet_feedback.send_time_ms;
    int64_t state = network_predictor_->RunInference(
        delay_time_ms / 1000.0, delay_time_delta_ms / 1000.0, rtt_ms / 1000.0,
        static_cast<int>(delay_detector_->State()));
    last_packet_send_time_ms_ = packet_feedback.send_time_ms;
    last_packet_arrival_time_ms_ = packet_feedback.arrival_time_ms;
    delay_detector_->SetStatePredicted(static_cast<BandwidthUsage>(state));
    RTC_LOG(LS_ERROR)
        << "Using ML Network Predictor, the Network State Predicted is "
        << state << "it was: " << static_cast<int>(delay_detector_->State());
  }
...
DelayBasedBwe::Result DelayBasedBwe::MaybeUpdateEstimate(
  ...
  if (delay_detector_->StatePredicted() == BandwidthUsage::kBwOverusing) {
  ...
  BandwidthUsage detector_state = delay_detector_->StatePredicted();
...
bool DelayBasedBwe::UpdateEstimate(Timestamp at_time,
  ...
  const RateControlInput input(delay_detector_->StatePredicted(),
                               acked_bitrate);
