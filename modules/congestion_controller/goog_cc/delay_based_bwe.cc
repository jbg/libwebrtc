...
bool PacketDelayGroup::BelongsToGroup(Timestamp send_time) const {
...
void DelayBasedBwe::IncomingPacketFeedback(
  ...
  } else if (send_time >= packet_groups_.back().first_send_time) {
    if (packet_groups_.back().BelongsToGroup(send_time) ||
  ...
  if (packet_groups_.size() == 3) {
    ...
    if (receive_delta - system_delta >= kReceiveTimeOffsetThreshold) {
      RTC_LOG(LS_WARNING)
          << "The receive time to system clock offset has changed (diff = "
          << receive_delta.ms() - system_delta.ms() << " ms), resetting.";
      packet_groups_.pop_front();
    } else if (receive_delta < TimeDelta::Zero()) {
