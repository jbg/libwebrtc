...
DelayBasedBwe::DelayBasedBwe(const WebRtcKeyValueConfig* key_value_config,
  ...
  delay_detector_.reset(new TrendlineEstimator(
      ...
      trendline_threshold_gain_, network_state_predictor_ != nullptr));
}
...
void DelayBasedBwe::IncomingPacketFeedback(
  ...
  if (network_state_predictor_ != nullptr) {
    BandwidthUsage state = network_state_predictor_->Update(
        packet_feedback.send_time_ms, packet_feedback.arrival_time_ms,
        delay_detector_->State());
    delay_detector_->SetStatePredicted(state);
  }
