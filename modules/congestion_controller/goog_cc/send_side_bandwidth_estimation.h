...
class SendSideBandwidthEstimation {
 public:
  ...
  void OnProcessInterval(Timestamp at_time);

  void OnSentPacket(const SentPacket& sent_packet);
  void UpdatePropagationRtt(Timestamp at_time, TimeDelta propagation_rtt);

  // Call when we receive a RTCP message with TMMBR or REMB.
  void UpdateReceiverEstimate(Timestamp at_time,
 ...
 private:
  ...
  void UpdateTargetBitrate(Timestamp at_time, DataRate bitrate);
  ...
  DataRate current_target_bitrate_ = DataRate::Zero();
  DataRate last_logged_bitrate_ = DataRate::Zero();
  DataRate min_bitrate_configured_;
  DataRate max_bitrate_configured_;
  Timestamp last_low_bitrate_log_ = Timestamp::MinusInfinity();

  bool has_decreased_since_last_fraction_loss_ = false;
  Timestamp last_loss_packet_report_ = Timestamp::MinusInfinity();
  Timestamp last_timeout_ = Timestamp::MinusInfinity();
  uint8_t last_fraction_loss_ = 0;
  uint8_t last_logged_fraction_loss_ = 0;
  TimeDelta last_round_trip_time_ = TimeDelta::Zero();

  DataRate remote_based_limit_ = DataRate::PlusInfinity();
  DataRate delay_based_limit_ = DataRate::PlusInfinity();
  Timestamp time_last_decrease_ = Timestamp::MinusInfinity();
  Timestamp first_report_time_ = Timestamp::MinusInfinity();
  int initially_lost_packets_ = 0;
  DataRate bitrate_at_2_seconds_ = DataRate::Zero();
  UmaState uma_update_state_ = kNoUpdate;
  UmaState uma_rtt_state_ = kNoUpdate;
  std::vector<bool> rampup_uma_stats_updated_;
  RtcEventLog* event_log_;
  Timestamp last_rtc_event_log_ = Timestamp::MinusInfinity();
