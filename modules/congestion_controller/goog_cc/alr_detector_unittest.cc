...
TEST(AlrDetectorTest, ParseActiveFieldTrial) {
  ...
  FieldTrialBasedConfig field_trials;
  AlrDetector alr_detector(&field_trials);
  EXPECT_EQ(alr_detector.bandwidth_usage_percent(), 85);
  EXPECT_EQ(alr_detector.start_budget_level_percent(), 20);
  EXPECT_EQ(alr_detector.stop_budget_level_percent(), -20);
}

TEST(AlrDetectorTest, ParseAlrSpecificFieldTrial) {
  webrtc::test::ScopedFieldTrials scoped_field_trial(
      "WebRTC-AlrDetectorParameters/"
      "bw_usage_percent:1,start_budget_level_percent:2,stop_budget_level_"
      "percent:3/");
  FieldTrialBasedConfig field_trials;
  AlrDetector alr_detector(&field_trials);
  EXPECT_EQ(alr_detector.bandwidth_usage_percent(), 1);
  EXPECT_EQ(alr_detector.start_budget_level_percent(), 2);
  EXPECT_EQ(alr_detector.stop_budget_level_percent(), 3);
}
