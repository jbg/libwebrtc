...
class GenericNetworkControllerInterface {
 public:
  virtual ~GenericNetworkControllerInterface() = default;
...
// Interface implemented by network controllers utilizing per packet feedback
// information.
class FeedbackBasedNetworkControllerInterface
    : public virtual GenericNetworkControllerInterface {
 public:
  // Called with per packet feedback regarding receive time.
  virtual void OnTransportPacketsFeedback(TransportPacketsFeedback) = 0;
};

// Interface for creating a network controller that is entirely based on per
// packet feedback.
class FeedbackBasedNetworkControllerFactoryInterface {
 public:
  // Used to create a new network controller, requires an observer to be
  // provided to handle callbacks.
  virtual std::unique_ptr<FeedbackBasedNetworkControllerInterface>
  CreateFeedbackBased(NetworkControllerObserver* observer,
                      NetworkControllerConfig config) = 0;
  // Returns the interval by which the network controller expects
  // OnProcessInterval calls.
  virtual TimeDelta GetProcessInterval() const = 0;
  virtual ~FeedbackBasedNetworkControllerFactoryInterface() = default;
};

// Interface implemented by network controllers utilizing summarized
// information.
class SimplifiedNetworkControllerInterface
    : public virtual GenericNetworkControllerInterface {
 public:
...
// Interface implemented by network controllers combining packet based
// information with summarized information.
class CombinedNetworkControllerInterface
    : public FeedbackBasedNetworkControllerInterface,
      public SimplifiedNetworkControllerInterface {};

// Interface for creating a network controller that handles both simple feedback
// and combined feedback.
class CombinedNetworkControllerFactoryInterface
    : public FeedbackBasedNetworkControllerFactoryInterface {
 public:
  // Used to create a new network controller, requires an observer to be
  // provided to handle callbacks.
  virtual std::unique_ptr<CombinedNetworkControllerInterface> CreateCombined(
