...
class DebugStatePrinter {
 public:
  ...
  virtual void PrintHeaders(LogWriter* out) = 0;
  virtual void PrintValues(LogWriter* out) = 0;
...
class ControlStatePrinter {
 public:
  ControlStatePrinter(std::unique_ptr<LogWriter> output,
 ...
 private:
  std::unique_ptr<LogWriter> output_;
