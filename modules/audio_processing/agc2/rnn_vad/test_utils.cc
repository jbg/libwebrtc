...
ChunksFileReader CreatePcmSamplesReader(const int frame_length) {
  auto reader = std::make_unique<FloatFileReader<int16_t>>(
      /*filename=*/test::ResourcePath("audio_processing/agc2/rnn_vad/samples",
                                      "pcm"));
  const int num_chunks = reader->size() / frame_length;
  return {/*chunk_size=*/frame_length, num_chunks, std::move(reader)};
}
...
ChunksFileReader CreateVadProbsReader() {
  auto reader = std::make_unique<FloatFileReader<float>>(
      /*filename=*/test::ResourcePath("audio_processing/agc2/rnn_vad/vad_prob",
                                      "dat"));
  const int num_chunks = reader->size();
  return {/*chunk_size=*/1, num_chunks, std::move(reader)};
}
