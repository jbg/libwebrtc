...
void ComputeSlidingFrameSquareEnergies(
  ...
  for (int i = 1; rtc::SafeLt(i, yy_values.size()); ++i) {
    ...
    const float old_coefficient =
        pitch_buffer[kMaxPitch24kHz + kFrameSize20ms24kHz - i];
    const float new_coefficient = pitch_buffer[kMaxPitch24kHz - i];
    yy -= old_coefficient * old_coefficient;
    yy += new_coefficient * new_coefficient;
    yy = std::max(0.f, yy);
    yy_values[i] = yy;
  }
}
...
CandidatePitchPeriods FindBestPitchPeriods(
  ...
  for (int inverted_lag = 0;
       inverted_lag < rtc::dchecked_cast<int>(auto_correlation.size());
       ++inverted_lag) {
    // A pitch candidate must have positive correlation.
    if (auto_correlation[inverted_lag] > 0) {
      candidate.period_inverted_lag = inverted_lag;
    ...
    const float old_coeff = pitch_buffer[inverted_lag];
    const float new_coeff = pitch_buffer[inverted_lag + frame_size];
