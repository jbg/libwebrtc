...
rtc::ArrayView<float, kNumLowerBands> GedSpectralCrossCorrelation(
    std::array<float, kTestFeatureVectorSize>* feature_vector) {
  return {feature_vector->data() + kNumBands + 2 * kNumLowerBands,
          kNumLowerBands};
}

float* GetSpectralVariability(
    std::array<float, kTestFeatureVectorSize>* feature_vector) {
...
TEST(RnnVadTest, SpectralFeaturesWithAndWithoutSilence) {
  ...
  is_silence = sfe.CheckSilenceComputeFeatures(
      ...
      GedSpectralCrossCorrelation(&feature_vector),
      GetSpectralVariability(&feature_vector));
...
TEST(RnnVadTest, SpectralFeaturesConstantAverageZeroDerivative) {
  ...
  is_silence = sfe.CheckSilenceComputeFeatures(
      ...
      GedSpectralCrossCorrelation(&feature_vector_last),
      GetSpectralVariability(&feature_vector_last));
