...
void CheckPitch(float pitch_hz) {
  const size_t pitch_period =
      static_cast<size_t>(static_cast<float>(kSampleRate24kHz) / pitch_hz);
  ASSERT_LE(kInitialMinPitch24kHz, pitch_period);
  ASSERT_LE(pitch_period, kMaxPitch24kHz);
}
...
TEST(RnnVadTest, FeatureExtractionLowHighPitch) {
  ...
  CheckPitch(low_pitch_hz);
  CheckPitch(high_pitch_hz);
  ...
  const auto pitch_feature = [&](rtc::ArrayView<float> v) -> float {
    return v[kFeatureVectorSize - 2];
  };

  CreatePureTone(amplitude, low_pitch_hz, samples);
  ASSERT_FALSE(FeedTestData(&features_extractor, samples, feature_vector_view));
  float low_pitch_feature = pitch_feature(feature_vector_view);

  features_extractor.Reset();
  CreatePureTone(amplitude, high_pitch_hz, samples);
  ASSERT_FALSE(FeedTestData(&features_extractor, samples, feature_vector_view));
  float high_pitch_feature = pitch_feature(feature_vector_view);

  EXPECT_LT(low_pitch_feature, high_pitch_feature);
}
