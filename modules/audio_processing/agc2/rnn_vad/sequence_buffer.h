...
class SequenceBuffer {
 ...
 public:
  ...
  // Set the sequence buffer values to zero.
  void Reset() { buffer_.fill(0); }
  // Return a view on the whole buffer.
  rtc::ArrayView<const T> GetBufferView() const { return {buffer_.data(), S}; }
  // Return a view on part of the buffer; the first element starts at the given
  ...
  // Shift left the buffer by N items and add new N items at the end.
