...
TEST_P(RnnVadProbabilityParametrization, RnnVadProbabilityWithinTolerance) {
  ...
  ChunksFileReader samples_reader = CreatePcmSamplesReader(kFrameSize10ms48kHz);
  ChunksFileReader expected_vad_prob_reader = CreateVadProbsReader();

  // Input length.
  const int num_frames = samples_reader.num_chunks;
  ASSERT_GE(expected_vad_prob_reader.num_chunks, num_frames);
  ...
  ASSERT_TRUE(expected_vad_prob_reader.reader->ReadChunk(expected_vad_prob));

  // Compute VAD probabilities on the downsampled input.
  float cumulative_error = 0.f;
  for (int i = 0; i < num_frames; ++i) {
    samples_reader.reader->ReadChunk(samples_48k);
...
TEST_P(RnnVadProbabilityParametrization, DISABLED_RnnVadPerformance) {
  ...
  for (int i = 0; i < num_frames; ++i) {
    samples_reader.reader->ReadChunk(samples);
