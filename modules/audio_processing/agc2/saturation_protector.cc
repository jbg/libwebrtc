...
SaturationProtector::RingBuffer::RingBuffer() : next_(0), size_(0) {}
...
void SaturationProtector::RingBuffer::PushBack(float v) {
  ...
  buffer_[next_++] = v;
  if (next_ == static_cast<int>(buffer_.size())) {
    next_ = 0;
  }
  if (size_ < static_cast<int>(buffer_.size())) {
    size_++;
  }
}
...
absl::optional<float> SaturationProtector::RingBuffer::Front() const {
  ...
  RTC_DCHECK_LT(next_, buffer_.size());
  return buffer_[size_ == static_cast<int>(buffer_.size()) ? next_ : 0];
}

SaturationProtector::PeakEnveloper::PeakEnveloper()
    : speech_time_in_estimate_ms_(0), current_superframe_peak_dbfs_(-90.f) {}

void SaturationProtector::PeakEnveloper::Reset() {
  speech_time_in_estimate_ms_ = 0;
  current_superframe_peak_dbfs_ = 0;
