...
TEST(AutomaticGainController2InterpolatedGainCurve, CreateUse) {
  ...
  const auto levels = LinSpace(
      kLevelEpsilon, DbfsToLinear(limiter.max_input_level_db() + 1), 500);
...
TEST(AutomaticGainController2InterpolatedGainCurve, CheckValidOutput) {
  ...
  const auto levels =
      LinSpace(kLevelEpsilon, limiter.max_input_level_linear() * 2.0, 500);
  for (const auto level : levels) {
...
TEST(AutomaticGainController2InterpolatedGainCurve, CheckMonotonicity) {
  ...
  const auto levels =
      LinSpace(kLevelEpsilon,
               limiter.max_input_level_linear() + kLevelEpsilon + 0.5, 500);
...
TEST(AutomaticGainController2InterpolatedGainCurve, CheckIdentityRegion) {
  ...
  const auto levels =
      LinSpace(kLevelEpsilon, limiter.knee_start_linear(), kNumSteps);
  for (const auto level : levels) {
    SCOPED_TRACE(std::to_string(level));
...
TEST(AutomaticGainController2InterpolatedGainCurve,
  ...
  const auto levels = LinSpace(limiter.knee_start_linear() + kLevelEpsilon,
...
TEST(AutomaticGainController2InterpolatedGainCurve,
  ...
  const auto levels =
      LinSpace(limiter.limiter_start_linear() + kLevelEpsilon,
...
TEST(AutomaticGainController2InterpolatedGainCurve, CheckApproximationParams) {
  ComputeInterpolatedGainCurveCoefficients computed_coefficients;
  InterpolatedGainCurve igc(&apm_data_dumper);

  for (size_t i = 0; i < kInterpolatedGainCurveTotalPoints; ++i) {
    EXPECT_EQ(igc.approximation_params_x_[i],
              computed_coefficients.approx_params_x()[i]);
    EXPECT_EQ(igc.approximation_params_m_[i],
              computed_coefficients.approx_params_m()[i]);
    EXPECT_EQ(igc.approximation_params_q_[i],
              computed_coefficients.approx_params_q()[i]);
  }
}
