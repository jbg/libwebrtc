...
TEST(SpeechProbabilityBufferTest,
     CheckSegmentIsActiveAfterUpdatesWithLowAndHighProbabilities) {
  SpeechProbabilityBuffer buffer(kLowProbabilityThreshold);

  // Add low probabilities until the the buffer is full. That's not enough
  // to make `IsActiveSegmentI()` to return true.
...
TEST(SpeechProbabilityBufferTest, CheckSegmentIsActiveActiveAfterBufferIsFull) {
  SpeechProbabilityBuffer buffer(kLowProbabilityThreshold);
