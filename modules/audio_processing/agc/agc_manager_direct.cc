...
AgcManagerDirect::AgcManagerDirect(int num_capture_channels,
  ...
  if (clipping_cfg.enabled) {
    RTC_LOG(LS_INFO) << "[agc] Clipping prediction enabled.";
    switch (clipping_cfg.mode) {
      case ClippingPredictorConfig::kClippingEventPrediction:
        clipping_predictor_ =
            CreateClippingEventPredictor(num_capture_channels, clipping_cfg);
        break;
      case ClippingPredictorConfig::kAdaptiveStepClippingPeakPrediction:
        clipping_predictor_ = CreateAdaptiveStepClippingPeakPredictor(
            num_capture_channels, clipping_cfg);
        break;
      case ClippingPredictorConfig::kFixedStepClippingPeakPrediction:
        clipping_predictor_ = CreateFixedStepClippingPeakPredictor(
            num_capture_channels, clipping_cfg);
        break;
    }
    RTC_DCHECK(!!clipping_predictor_);
  }
  ...
  if (!!clipping_predictor_) {
    for (int channel = 0; channel < num_capture_channels_; ++channel) {
      ...
      if (step.has_value()) {
        ...
        clipping_predicted = clipping_predicted || true;
      }
    }
  }
