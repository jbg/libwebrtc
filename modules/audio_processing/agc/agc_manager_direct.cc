...
void LogClippintPredictorMetrics(const ClippingPredictorEvaluator& evaluator) {
  ...
  if (precision_denominator > 0 && recall_denominator > 0) {
    ...
    if (f1_score_denominator > 0.0f) {
      // TODO(bugs.webrtc.org/12774): Log F1 score as histogram.
...
AgcManagerDirect::AgcManagerDirect(
    ...
    : data_dumper_(
      ...
      clipping_predictor_evaluator_(
          /*history_size=*/clipping_config.window_length),
...
void AgcManagerDirect::AnalyzePreProcess(const float* const* audio,
  ...
  if (!!clipping_predictor_) {
    ...
    if (clipping_predictor_log_counter_ == kNumFramesIn30Seconds) {
      LogClippintPredictorMetrics(clipping_predictor_evaluator_);
      clipping_predictor_log_counter_ = 0;
    }
  }
