...
TEST(ErlEstimator, Estimates) {
  for (size_t num_render_channels : {1, 2, 8}) {
    for (size_t num_capture_channels : {1, 2, 8}) {
      ...
      std::vector<bool> converged_filters(num_capture_channels);
      std::fill(converged_filters.begin(), converged_filters.end(), false);
      size_t converged_idx = num_capture_channels - 1;
      converged_filters[converged_idx] = true;

      size_t active_idx = std::min(1ul, num_render_channels - 1);
      ...
      Y2[converged_idx].fill(10 * X2[active_idx][0]);
      ...
      // data increases.
      Y2[converged_idx].fill(10000 * X2[active_idx][0]);
