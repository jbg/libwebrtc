...
void ResidualEchoEstimator::Estimate(
  ...
  if (aec_state.UsableLinearEstimate()) {
    ...
    const float echo_path_gain = GetEchoPathGain(aec_state, true);
    ...
    if (!aec_state.TransparentMode() && model_reverb_in_nonlinear_mode_) {
...
void ResidualEchoEstimator::AddReverb(
  ...
  if (reverb_type == ReverbType::kLinear) {
    ...
    const float echo_path_gain = GetEchoPathGain(aec_state, false);
