...
class AdaptiveFirFilterTest : public Aec3MultiChannelTest {};
...
TEST_P(AdaptiveFirFilterTest, FilterAdaptationNeonOptimizations) {
...
TEST_P(AdaptiveFirFilterTest, ComputeFrequencyResponseNeonOptimization) {
  const size_t num_render_channels = std::get<0>(GetParam());
  for (size_t num_partitions : {2, 5, 12, 30, 50}) {
...
// their reference counterparts.
TEST_P(AdaptiveFirFilterTest, FilterAdaptationSse2Optimizations) {
  const size_t num_render_channels = std::get<0>(GetParam());
  constexpr int kSampleRateHz = 48000;
  constexpr size_t kNumBands = NumBandsForRate(kSampleRateHz);
...
// Verifies that the optimized method for frequency response computation is
// bitexact to the reference counterpart.
TEST_P(AdaptiveFirFilterTest, ComputeFrequencyResponseSse2Optimization) {
  const size_t num_render_channels = std::get<0>(GetParam());
  bool use_sse2 = (WebRtc_GetCPUInfo(kSSE2) != 0);
  if (use_sse2) {
    for (size_t num_partitions : {2, 5, 12, 30, 50}) {
...
TEST(AdaptiveFirFilterTest, FilterAndAdapt) {
  constexpr int kSampleRateHz = 48000;
  constexpr size_t kNumBands = NumBandsForRate(kSampleRateHz);
