...
void ErleCorrectionFactor::Dump(
  ...
  for (auto& att : correction_factor_) {
    data_dumper->DumpRaw("aec3_all_att", att);
  }
}
...
void ErleCorrectionFactor::UpdateBands(
  ...
  for (size_t subband = subband_start; subband < subband_stop; ++subband) {
    if (X2_subband[subband] > kX2BandEnergyThreshold &&
      ...
      erle_to_update[subband] = ErleBandUpdate(
          erle_to_update[subband], new_erle, false, kSmthConstantIncreases,
          kSmthConstantDecreases, min_erle, max_erle);
      erle_ref_[subband] = ErleBandUpdate(
          erle_ref_[subband], new_erle, false, kSmthConstantIncreases,
          kSmthConstantDecreases, min_erle, max_erle);
