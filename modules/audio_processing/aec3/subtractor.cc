...
// Number of blocks before a reset coarse filter is allowed to influence the
// adaptation speed of the refined filter.
constexpr int kCoarseResetHangoverBlocks = 25;
  ...
  for (size_t ch = 0; ch < num_capture_channels_; ++ch) {
    ...
    if (poor_coarse_filter_counters_[ch] < 5) {
      ...
      coarse_gains_[ch]->Compute(X2_coarse, render_signal_analyzer, E_refined,
                                 coarse_filter_[ch]->SizePartitions(),
                                 aec_state.SaturatedCapture(), &G);
      coarse_filter_reset_hangover_[ch] = kCoarseResetHangoverBlocks;
    }
