...
class TransparentModeImpl : public TransparentMode {
 public:
  ...
  void Update(int filter_delay_blocks,
    ...
    constexpr float kConvergedNormal = 0.05f;
    ...
    // Observed output.
    const int o = any_filter_converged;

    // Joint probabilites of the observed output and respective states.
    const float prob_joint_normal = prob_transition_normal * kB[0][o];
    const float prob_joint_transparent = prob_transition_transparent * kB[1][o];
...
std::unique_ptr<TransparentMode> TransparentMode::Create(
  ...
  if (DeactivateTransparentMode()) {
    return nullptr;
  } else if (DeactivateTransparentModeHmm()) {
