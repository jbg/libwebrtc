...
class RenderDelayBufferImpl2 final : public RenderDelayBuffer {
 ...
 private:
  ...
  int min_latency_blocks_ = 0;
  size_t lost_capture_counter_ = 0;
  ...
  bool DetectLostCaptureFrames();

  RTC_DISALLOW_IMPLICIT_CONSTRUCTORS(RenderDelayBufferImpl2);
};
...
RenderDelayBufferImpl2::PrepareCaptureProcessing() {
  ...
  bool lost_capture_frames = DetectLostCaptureFrames();
...
bool RenderDelayBufferImpl2::DetectLostCaptureFrames() {
  constexpr int kMaxLostFrames = 5;
  bool lost_capture = false;
  const int latency_blocks = BufferLatency(low_rate_) / sub_block_size_;
  min_latency_blocks_ = std::min(min_latency_blocks_, latency_blocks);
  if (++lost_capture_counter_ > config_.delay.api_call_jitter_blocks) {
    lost_capture = min_latency_blocks_ > kMaxLostFrames;
