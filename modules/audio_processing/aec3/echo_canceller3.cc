...
EchoCanceller3Config AdjustConfig(const EchoCanceller3Config& config) {
  ...
  constexpr char kInitialStateDurationFieldTrial[] =
      "WebRTC-Aec3InitialStateDurationExperiment";
  if (webrtc::field_trial::IsEnabled(kInitialStateDurationFieldTrial)) {
    const auto field_trial_string =
        webrtc::field_trial::FindFullName(kInitialStateDurationFieldTrial);
    float duration_seconds = 1.f;
    sscanf(field_trial_string.c_str(), "Enabled-%f", &duration_seconds);
    if (duration_seconds >= 0.f && duration_seconds <= 10.f) {
      adjusted_cfg.filter.initial_state_seconds = duration_seconds;
    }
  }
  ...
  constexpr char kReverbDefaultLengthFieldTrial[] =
      "WebRTC-Aec3ReverbDefaultLengthExperiment";
  if (webrtc::field_trial::IsEnabled(kReverbDefaultLengthFieldTrial)) {
    const auto field_trial_string =
        webrtc::field_trial::FindFullName(kReverbDefaultLengthFieldTrial);
    float reverb_default_length = 1.f;
    sscanf(field_trial_string.c_str(), "Enabled-%f", &reverb_default_length);
    if (reverb_default_length >= -1.f && reverb_default_length <= 1.f) {
      adjusted_cfg.ep_strength.default_len = reverb_default_length;
    }
  }
  ...
  constexpr char kDominantNearendActivationSensitivityFieldTrial[] =
      "WebRTC-Aec3DominantNearendActivationSensitivityExperiment";
  if (webrtc::field_trial::IsEnabled(
          kDominantNearendActivationSensitivityFieldTrial)) {
    const auto field_trial_string = webrtc::field_trial::FindFullName(
        kDominantNearendActivationSensitivityFieldTrial);
    float threshold = 1.f;
    sscanf(field_trial_string.c_str(), "Enabled-%f", &threshold);
    if (threshold >= 0.f && threshold <= 10.f) {
