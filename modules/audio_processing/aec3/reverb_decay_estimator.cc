...
ReverbDecayEstimator::ReverbDecayEstimator(const EchoCanceller3Config& config)
    : filter_length_blocks_(config.filter.main.length_blocks),
      ...
      erly_reverb_estimator_(config.filter.main.length_blocks -
...
void ReverbDecayEstimator::EstimateDecay(rtc::ArrayView<const float> filter) {
  ...
  const int size_early_reverb =
      std::pow(2.0f, erly_reverb_estimator_.Estimate());
  ...
  if (size_late_reverb >= 5) {
    if (valid_filter && late_reverb_decay_estimator_.EstimateAvailable()) {
      float decay = late_reverb_decay_estimator_.Estimate();
  ...
  if (block_to_analyze_ <= late_reverb_end_) {
    if (block_to_analyze_ >= late_reverb_start_) {
      for (float h2_k : h2) {
        ...
        erly_reverb_estimator_.Accumulate(h2_log2, smoothing_constant_);
      }
    } else {
      for (float h2_k : h2) {
        float h2_log2 = FastApproxLog2f(h2_k + 1e-10);
...
void ReverbDecayEstimator::Dump(ApmDataDumper* data_dumper) const {
  ...
  data_dumper->DumpRaw("aec3_blocks_after_early_reflections",
                       late_reverb_start_);
  erly_reverb_estimator_.Dump(data_dumper);
}

void ReverbDecayEstimator::LinearRegressor::Reset(int num_data_points) {
  RTC_DCHECK_LT(0, num_data_points);
...
void ReverbDecayEstimator::LinearRegressor::Accumulate(float z) {
  nz_ += count_ * z;
  ++count_;
  ++n_;
}

float ReverbDecayEstimator::LinearRegressor::Estimate() {
  RTC_DCHECK(EstimateAvailable());
  if (nn_ == 0.f) {
    RTC_NOTREACHED();
    return 0.f;
  }
  return nz_ / nn_ * kFftLengthBy2;
}
