...
float BlockAverage(rtc::ArrayView<const float> v, size_t block_index) {
  ...
  int i = block_index * kFftLengthBy2;
...
ReverbDecayEstimator::ReverbDecayEstimator(const EchoCanceller3Config& config)
    : filter_length_blocks_(config.filter.main.length_blocks),
      ...
      er_size_estimator_(config.filter.main.length_blocks -
...
void ReverbDecayEstimator::Update(rtc::ArrayView<const float> filter,
  ...
  bool estimation_feasible = filter_delay_blocks <= filter_length_blocks_ - 4;
  estimation_feasible =
  ...
  float new_smoothing_constant = filter_quality ? *filter_quality * 0.2f : 0.f;
  smoothing_constant_ = std::max(new_smoothing_constant, smoothing_constant_);
...
void ReverbDecayEstimator::UpdateDecay(rtc::ArrayView<const float> filter) {
  auto& h = filter;
  ...
  // Reset the block analyzis counter.
  block_to_analyze_ =
  ...
  const int size_early_reverb = er_size_estimator_.Estimate();
  ...
  if (size_late_reverb >= 5) {
    if (valid_filter && lr_decay_.EstimateAvailable()) {
      float decay = lr_decay_.Estimate();
    ...
    lr_decay_.Reset(size_late_reverb * kFftLengthBy2);
    late_reverb_start_ =
  ...
  if (block_to_analyze_ <= late_reverb_end_) {
    if (block_to_analyze_ >= late_reverb_start_) {
      for (float h2_k : h2) {
        ...
        lr_decay_.Accumulate(h2_log2);
        er_size_estimator_.Accumulate(h2_log2, smoothing_constant_);
      }
    } else {
      for (float h2_k : h2) {
        float h2_log2 = FastApproxLog2f(h2_k + 1e-10);
...
void ReverbDecayEstimator::Dump(ApmDataDumper* data_dumper) const {
  ...
  data_dumper->DumpRaw("aec3_blocks_after_early_reflections",
                       late_reverb_start_);
  er_size_estimator_.Dump(data_dumper);
}

void ReverbDecayEstimator::LateReverbDecay::Reset(int num_data_points) {
  const int N = num_data_points;
  nz_ = 0.f;
  // Arithmetic sum of $2 \sum_{i=0.5}^{(N-1)/2}i^2$ calculated directly.
  nn_ = N * (N * N - 1.0f) * (1.f / 12.f);
...
void ReverbDecayEstimator::LateReverbDecay::Accumulate(float z) {
  nz_ += count_ * z;
  ++count_;
  ++n_;
}

float ReverbDecayEstimator::LateReverbDecay::Estimate() {
  RTC_DCHECK(n_ == N_ && N_ != 0);
  if (nn_ == 0.f) {
    RTC_NOTREACHED();
    return 0.f;
  }
  return std::pow(2.0f, nz_ / nn_ * kFftLengthBy2);
}

ReverbDecayEstimator::EarlyReverbSizeEstimator::EarlyReverbSizeEstimator(
    int max_blocks)
...
void ReverbDecayEstimator::EarlyReverbSizeEstimator::Reset() {
  // Linear regression approach assumes symmetric index around 0.
  constexpr float kCount = -kBlocksPerSection * kFftLengthBy2 * 0.5f + 0.5f;
...
void ReverbDecayEstimator::EarlyReverbSizeEstimator::Accumulate(
    float value,
    float smoothing) {
...
int ReverbDecayEstimator::EarlyReverbSizeEstimator::Estimate() {
  constexpr float N = kBlocksPerSection * kFftLengthBy2;
  constexpr float nn = N * (N * N - 1.0f) * (1.f / 12.f);
  ...
  // below.
  float min_num_stable_region = *std::min_element(
...
void ReverbDecayEstimator::EarlyReverbSizeEstimator::Dump(
    ApmDataDumper* data_dumper) const {
  data_dumper->DumpRaw("aec3_lr_acum_numerator", numerators_);
}
