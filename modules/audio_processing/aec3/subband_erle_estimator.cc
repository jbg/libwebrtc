    ...
    if (accum_spectra.EnoughPoints(k) && accum_spectra.E2_[k]) {
      ...
      if (onset_detection && !accum_spectra_.low_render_energy_[k]) {
        if (coming_onset_[k]) {
          ...
          erle_onsets_[k] = aec3::ErleBandUpdate(
              erle_onsets_[k], new_erle, accum_spectra_.low_render_energy_[k],
              0.15f, 0.3f, min_erle_, max_erle);
        }
        hold_counters_[k] = kBlocksForOnsetDetection;
      }
      erle_[k] = aec3::ErleBandUpdate(
          erle_[k], new_erle, accum_spectra_.low_render_energy_[k],
          aec3::kSmthConstantIncreases, aec3::kSmthConstantDecreases, min_erle_,
          max_erle);
    }
  }
}
...
void SubbandErleEstimator::AccumulativeSpectra::Update(
  ...
  for (size_t k = 0; k < X2.size(); ++k) {
    if (update_on_low_render || X2[k] > aec3::kX2BandEnergyThreshold) {
      ...
      low_render_energy_[k] =
          low_render_energy_[k] || X2[k] < aec3::kX2BandEnergyThreshold;
      Y2_[k] += Y2[k];
      E2_[k] += E2[k];
      num_points_[k]++;
    }
