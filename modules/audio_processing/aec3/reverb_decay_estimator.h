...
// Class for estimating the decay if the late reverb.
class ReverbDecayEstimator {
 ...
 private:
  void UpdateDecay(rtc::ArrayView<const float> filter);
  ...
  // Class for identifying the size of the early reverb from the linear filter.
  class EarlyReverbSizeEstimator {
   public:
    explicit EarlyReverbSizeEstimator(int max_blocks);
    ~EarlyReverbSizeEstimator();
  ...
  LateReverbDecay lr_decay_;
  EarlyReverbSizeEstimator er_size_estimator_;
