...
void IdentifySmallNarrowBandRegions(
    ...
    std::vector<std::array<size_t, kFftLengthBy2 - 1>>*
        narrow_band_channel_counters,
    std::array<size_t, kFftLengthBy2 - 1>* narrow_band_counters) {
  RTC_DCHECK(narrow_band_channel_counters);
  RTC_DCHECK(narrow_band_counters);
  if (!delay_partitions) {
    for (auto& counters : *narrow_band_channel_counters) {
      counters.fill(0);
    }
  ...
  RTC_DCHECK_EQ(narrow_band_channel_counters->size(),
                render_buffer.Block(0)[0].size());
  for (size_t channel = 0; channel < narrow_band_channel_counters->size();
       ++channel) {
    rtc::ArrayView<const float> X2 =
        render_buffer.Spectrum(*delay_partitions, channel);
    rtc::ArrayView<size_t> counters = (*narrow_band_channel_counters)[channel];
    RTC_DCHECK_EQ(counters.size() + 2, X2.size());
...
RenderSignalAnalyzer::RenderSignalAnalyzer(const EchoCanceller3Config& config,
    ...
    : strong_peak_freeze_duration_(config.filter.main.length_blocks),
      narrow_band_channel_counters_(num_channels),
...
void RenderSignalAnalyzer::Update(
  ...
  IdentifySmallNarrowBandRegions(render_buffer, delay_partitions,
                                 ...
                                 &narrow_band_counters_);
  for (const auto& channel_counters : narrow_band_channel_counters_) {
    for (size_t k = 0; k < channel_counters.size(); ++k) {
      narrow_band_counters_[k] =
          std::max(channel_counters[k], narrow_band_counters_[k]);
    }
  }
