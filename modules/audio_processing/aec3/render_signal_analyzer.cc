...
namespace {
constexpr size_t kCounterThreshold = 7;
...
void IdentifyStrongNarrowBandComponent(const RenderBuffer& render_buffer,
                                       size_t strong_peak_freeze_duration,
  ...
  if (peak_bin > 0 && max_abs > 100 &&
    ...
    *narrow_peak_counter = 0;
    ...
    if (*narrow_peak_band &&
        ++(*narrow_peak_counter) > strong_peak_freeze_duration) {
...
RenderSignalAnalyzer::RenderSignalAnalyzer(EchoCanceller3Config config)
