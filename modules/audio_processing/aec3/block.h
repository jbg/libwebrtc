...
class Block {
 public:
  ...
  void SetNumChannels(int num_channels) {
    num_channels_ = num_channels;
    data_.resize(num_bands_ * num_channels_ * kBlockSize, 0.0f);
  }
  ...
  void Swap(Block& b) {
    RTC_DCHECK_EQ(num_bands_, b.num_bands_);
    std::swap(num_channels_, b.num_channels_);
    data_.swap(b.data_);
  }

 private:
  // Returns the index of the data for the requested |band| and |channel|.
