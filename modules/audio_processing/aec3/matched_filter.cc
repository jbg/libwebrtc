...
size_t ComputePreEchoLag(
  ...
  if (pre_echo_configuration.mode == 0) {
    // Mode 0: Pre echo lag is defined as the coefficient where the is lower
    // than a threshold with a certain decrease slope.
  ...
  } else if (pre_echo_configuration.mode == 1) {
  ...
  } else if (pre_echo_configuration.mode == 2) {
    // Mode 2: Pre echo lag is defined as the closest coefficient to that lag
