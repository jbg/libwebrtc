...
void UpdateAccumulatedError(
    ...
    float one_over_best_error_sum_anchor) {
  for (size_t k = 0; k < instantaneous_accumulated_error.size(); ++k) {
    float error_norm =
        instantaneous_accumulated_error[k] * one_over_best_error_sum_anchor;
...
size_t ComputePreEchoLag(const rtc::ArrayView<float> accumulated_error,
  ...
  for (size_t k = 2; k < maximum_pre_echo_lag; ++k) {
    ...
    : data_dumper_(data_dumper),
      ...
      accumulated_error_(
          ...
          std::vector<float>(window_size_sub_blocks * sub_block_size_ /
                             ...
                             1.0f)),
      instantaneous_accumulated_error_(window_size_sub_blocks *
                                           sub_block_size_ /
                                           kAccumulatedErrorSubSampleRate,
                                       0.0f),
      scratch_memory_(window_size_sub_blocks * sub_block_size_),
...
void MatchedFilter::Update(const DownsampledRenderBuffer& render_buffer,
  ...
  float winner_accuracy = 0.f;
  winner_lag_ = absl::nullopt;
  reported_lag_estimate_ = absl::nullopt;
  size_t alignment_shift = 0;
  absl::optional<size_t> previous_lag_estimate;
  int num_filters = static_cast<int>(filters_.size());
  int winner_index = -1;
  float one_over_best_error_sum_anchor = 1.0f;
  for (int n = 0; n < num_filters; ++n) {
    ...
    if (n == last_detected_best_lag_filter_) {
      one_over_best_error_sum_anchor = 1.0f / error_sum_anchor;
    }
    ...
    const float accuracy = error_sum_anchor - error_sum;
    const size_t lag = lag_estimate + alignment_shift;
    if (filters_updated && reliable && accuracy > winner_accuracy) {
  ...
  if (winner_index != -1) {
    ...
    if (detect_pre_echo_ && last_detected_best_lag_filter_ == winner_index) {
      if (one_over_best_error_sum_anchor < 1.0f / (30.0f * 30.0f * y.size())) {
        UpdateAccumulatedError(instantaneous_accumulated_error_,
                               accumulated_error_[winner_index],
                               one_over_best_error_sum_anchor);
      }
