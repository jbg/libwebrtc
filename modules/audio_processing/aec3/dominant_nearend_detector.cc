...
namespace {
// State transition probability echo to nearend.
constexpr float kPEchoToNearend = 0.001f;
// State transition probability nearend to echo.
constexpr float kPNearendToEcho = 0.001f;
// Probability of observing low ENR and high SNR in echo state.
constexpr float kOutputEcho = 0.1f;
...
constexpr float kTransitionProbability[2][2] = {
    {1.f - kPEchoToNearend, kPEchoToNearend},
    {kPNearendToEcho, 1.f - kPNearendToEcho}};

constexpr float kOutputProbability[2][2] = {
    {1 - kOutputEcho, kOutputEcho},
...
void DominantNearendDetectorHmm::Update(
  ...
  float p_output_echo = (1.f - p_nearend_) * kOutputProbability[0][output];
  float p_output_nearend = p_nearend_ * kOutputProbability[1][output];

  // Conditional probability of nearend state and the output.
  p_nearend_ = p_output_nearend / (p_output_echo + p_output_nearend);
}
