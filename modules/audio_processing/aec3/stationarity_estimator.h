...
class StationarityEstimator {
 public:
  ...
  void UpdateStationarityFlags(const VectorBuffer* const spectrum_buffer,
                               int idx_current,
                               int num_lookahead);
 ...
 private:
  static constexpr int kLongWindowSize = 13;
  ...
  bool EstimateBandStationarity(const VectorBuffer* const spectrum_buffer,
                                const std::array<int, kLongWindowSize>& indexes,
                                size_t band) const;
