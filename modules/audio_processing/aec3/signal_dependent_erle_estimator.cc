...
constexpr size_t kBandBoundaries[SignalDependentErleEstimator::kSubbands + 1] =
    {1, 8, 16, 24, 32, 48, kFftLengthBy2Plus1};

std::array<size_t, kFftLengthBy2Plus1> BandToSubband() {
  std::array<size_t, kFftLengthBy2Plus1> map_band_to_subband;
  map_band_to_subband.fill(0.f);
  size_t subband = 1;
  for (size_t k = kBandBoundaries[0]; k < map_band_to_subband.size(); ++k) {
    RTC_DCHECK_LT(subband, SignalDependentErleEstimator::kSubbands + 1);
...
SignalDependentErleEstimator::SignalDependentErleEstimator(
    ...
    : min_erle_(config.erle.min),
      ...
      band_to_subband_(BandToSubband()),
...
void SignalDependentErleEstimator::UpdateCorrectionFactors(
  ...
  auto compute_subbbands = [](rtc::ArrayView<const float> power_spectrum,
  ...
  compute_subbbands(X2, X2_subbands);
  compute_subbbands(E2, E2_subbands);
  compute_subbbands(Y2, Y2_subbands);
