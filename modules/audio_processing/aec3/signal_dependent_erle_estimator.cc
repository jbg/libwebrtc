...
constexpr size_t
    bands_boundaries_[SignalDependentErleEstimator::kSubbands + 1] = {
        1, 8, 16, 24, 32, 48, kFftLengthBy2Plus1 - 1};
...
// linear manner so lower sections that typically represents the direct path
...
SignalDependentErleEstimator::SignalDependentErleEstimator(
  ...
  // Defines the sections used for dividing the linear filter. Those sections
  // are used for analysing the echo estimates and investigating which
  // linear filter sections contributes most to the echo estimate energy. The
  // division of the linear filter in sections is not linear giving more
  // resolution to the parts of the filter that represents the direct
  // path Vs the regions of the filter that represents more reverberant paths.
...
// The function GetNumberOfActiveFilterSections estimates which region of the
...
float SignalDependentErleEstimator::GetCorrectionFactor(
    size_t band,
    size_t n_active_groups) {
  RTC_DCHECK_LT(n_active_groups, erle_estimators_.size());
  size_t subband = BandToSubband(band);
  return correction_factors_[n_active_groups][subband];
}
...
void SignalDependentErleEstimator::ComputeEchoEstimatePerFilterSection(
  ...
  for (size_t section = 0; section < section_boundaries_blocks_.size() - 1;
       ++section) {
  ...
  for (size_t section = 1; section < section_boundaries_blocks_.size() - 1;
       ++section) {
