...
void GainToNoAudibleEcho(
    ...
    const std::array<float, kFftLengthBy2Plus1>& enr_transparent,
    const std::array<float, kFftLengthBy2Plus1>& enr_suppress,
    const std::array<float, kFftLengthBy2Plus1>& emr_transparent,
  ...
  for (size_t k = 0; k < gain->size(); ++k) {
    ...
    float g = 1.0f;
    if (enr > enr_transparent[k] && emr > emr_transparent[k]) {
      g = (enr_suppress[k] - enr) / (enr_suppress[k] - enr_transparent[k]);
      g = std::max(g, emr_transparent[k] / emr);
    }
...
void SuppressionGain::LowerBandGain(
  ...
  if (enable_new_suppression_) {
    GainToNoAudibleEcho(config_, nearend, weighted_echo, comfort_noise,
                        enr_transparent_, enr_suppress_, emr_transparent_,
...
SuppressionGain::SuppressionGain(const EchoCanceller3Config& config,
  ...
  const size_t last_lf_band = 6;
  const size_t first_hf_band = 9;
