...
void GainToNoAudibleEchoFallback(
  ...
  for (size_t k = 0; k < gain->size(); ++k) {
    ...
    if (weighted_echo[k] <= nearend_masking_margin * unity_gain_masker ||
      ...
      (*gain)[k] = std::max(masker_margin * masker[k] * one_by_weighted_echo,
                            (*gain)[k]);
    }
...
void SuppressionGain::GetMinGain(
    ...
    const std::array<float, kFftLengthBy2Plus1>& weighted_residual_echo,
    bool low_noise_render,
    bool saturated_echo,
    float max_dec_factor_lf,
    std::array<float, kFftLengthBy2Plus1>* min_gain) const {
  ...
  if (!saturated_echo) {
    ...
    for (size_t k = 0; k < 6; ++k) {
      ...
      if (last_nearend_[k] > last_echo_[k]) {
        (*min_gain)[k] =
            std::max((*min_gain)[k], last_gain_[k] * max_dec_factor_lf);
        (*min_gain)[k] = std::min((*min_gain)[k], 1.f);
      }
    }
  } else {
...
void SuppressionGain::GetMaxGain(float max_inc_factor,
                                 rtc::ArrayView<float> max_gain) const {
  // Compute the maximum gain by limiting the gain increase from the previous
  // gain.
  for (size_t k = 0; k < max_gain.size(); ++k) {
    max_gain[k] = std::min(std::max(last_gain_[k] * max_inc_factor,
...
void SuppressionGain::LowerBandGain(
  ...
  GetMinGain(suppressor_input, weighted_residual_echo, low_noise_render,
             saturated_echo, params.max_dec_factor_lf, &min_gain);

  std::array<float, kFftLengthBy2Plus1> max_gain;
  GetMaxGain(params.max_inc_factor, max_gain);
  ...
  if (enable_new_suppression_) {
    ...
    for (int k = 0; k < 2; ++k) {
      ...
      GainToNoAudibleEchoFallback(
          ...
          nearend, weighted_residual_echo, masker, min_gain, max_gain, gain);
      AdjustForExternalFilters(gain);
    }
  }
...
void SuppressionGain::GetGain(
  ...
  LowerBandGain(low_noise_render, aec_state, nearend_average,
                suppressor_input_spectrum, residual_echo_spectrum,
