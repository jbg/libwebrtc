...
void UpdateMaxGainIncrease(
  ...
  max_increasing = 0.1f;
  max_decreasing = 0.1f;
  rate_increasing = 0.1f;
  rate_decreasing = 0.1f;
  min_increasing = 0.1f;
  min_decreasing = 0.1f;
...
void SuppressionGain::LowerBandGain(
  ...
  for (int k = 0; k < 2; ++k) {
    ...
    // GainToNoAudibleEcho(config_, low_noise_render, no_saturation_counter_ >
    // 10,
  ...
  //     std::max(0.001f, *std::min_element(gain->begin(), gain->end()) *
  //     10000.f);
