    ...
    for (; p < limit; ++p, ++X_partition) {
      for (size_t ch = 0; ch < num_render_channels; ++ch) {
        ...
        for (size_t k = 0, n = 0; n < kNumEightBinBands; ++n, k += 8) {
          ...
          // __m256 a = _mm256_mul_ps(X_re, G_re);
          // __m256 b = _mm256_mul_ps(X_im, G_re);
          // a = _mm256_fmadd_ps(X_im, G_im, a);
          // b = _mm256_fmsub_ps(X_re, G_im, b);
          // const __m256 c = _mm256_add_ps(H_re, a);
          // const __m256 d = _mm256_add_ps(H_im, b);
          // _mm256_storeu_ps(&H_p_ch.re[k], c);
          // _mm256_storeu_ps(&H_p_ch.im[k], d);
          const __m256 G_re = _mm256_loadu_ps(&G.re[k]);
          const __m256 G_im = _mm256_loadu_ps(&G.im[k]);
          const __m256 X_re = _mm256_loadu_ps(&X.re[k]);
          const __m256 X_im = _mm256_loadu_ps(&X.im[k]);
...
void ApplyFilter_Avx2(const RenderBuffer& render_buffer,
  ...
  const size_t lim1 = std::min(
      render_buffer_data.size() - render_buffer.Position(), num_partitions);
  const size_t lim2 = num_partitions;
  constexpr size_t kNumEightBinBands = kFftLengthBy2 / 4;
    ...
    for (; p < limit; ++p, ++X_partition) {
      for (size_t ch = 0; ch < num_render_channels; ++ch) {
        ...
        for (size_t k = 0, n = 0; n < kNumEightBinBands; ++n, k += 8) {
          ...
          // __m256 a = _mm256_mul_ps(X_im, H_im);
          // __m256 b = _mm256_mul_ps(X_im, H_re);
          // a = _mm256_fmsub_ps(X_re, H_re, a);
          // b = _mm256_fmadd_ps(X_re, H_im, b);
          // const __m256 c = _mm256_add_ps(S_re, a);
          // const __m256 d = _mm256_add_ps(S_im, b);
          // _mm256_storeu_ps(&S->re[k], c);
          // _mm256_storeu_ps(&S->im[k], d);
          const __m256 X_re = _mm256_loadu_ps(&X.re[k]);
          const __m256 X_im = _mm256_loadu_ps(&X.im[k]);
          const __m256 H_re = _mm256_loadu_ps(&H_p_ch.re[k]);
          const __m256 H_im = _mm256_loadu_ps(&H_p_ch.im[k]);
