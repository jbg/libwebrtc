...
class FilterAnalyzer {
 ...
 private:
  ...
  void ResetPeakDetector(size_t filter_length);

  void SetRegionToAnalyze(rtc::ArrayView<const float> filter_time_domain);

  void UpdateSignificantPeakDetector(
      rtc::ArrayView<const float> filter_to_analyze);

  bool SignificantPeakInFilter(rtc::ArrayView<const float> filter_to_analyze);
  ...
  class PeakDetector {
   public:
    void Reset();
    void Update(rtc::ArrayView<const float> filter_to_analyze,
                const FilterRegion& r,
                size_t peak_index);
    bool SignificantPeak() { return significant_peak_; }
