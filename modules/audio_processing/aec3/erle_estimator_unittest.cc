...
void FormFarendFrame(rtc::ArrayView<float> x,
                     std::array<float, kFftLengthBy2Plus1>* X2,
                     std::array<float, kFftLengthBy2Plus1>* E2,
                     std::array<float, kFftLengthBy2Plus1>* Y2,
                     float erle) {
  x[0] = 22.f * 1000.f;
  X2->fill(500.f * 1000.f * 1000.f);
...
void GetFilterFreq(std::vector<std::array<float, kFftLengthBy2Plus1>>&
  ...
  for (size_t k = 0; k < kFftLengthBy2Plus1; ++k) {
    filter_frequency_response[delay_headroom_blocks][k] = 3.f;
  }
}
...
TEST(ErleEstimator, VerifyErleIncreaseAndHold) {
  ...
  FormFarendFrame(x[0], &X2, &E2, &Y2, kTrueErle);
...
TEST(ErleEstimator, VerifyErleTrackingOnOnsets) {
  ...
  for (size_t burst = 0; burst < 20; ++burst) {
    FormFarendFrame(x[0], &X2, &E2, &Y2, kTrueErleOnsets);
