...
class ShadowFilterUpdateGainTest : public Aec3MultiChannelTest {};
...
TEST(ShadowFilterUpdateGainTest, NullDataOutputGain) {
  ApmDataDumper data_dumper(42);
...
TEST_P(ShadowFilterUpdateGainTest, GainCausesFilterToConverge) {
...
TEST_P(ShadowFilterUpdateGainTest, DecreasingGain) {
  const size_t num_render_channels = std::get<0>(GetParam());
  for (size_t filter_length_blocks : {12, 20, 30}) {
...
TEST_P(ShadowFilterUpdateGainTest, SaturationBehavior) {
  const size_t num_render_channels = std::get<0>(GetParam());
  std::vector<int> blocks_with_echo_path_changes;
  std::vector<int> blocks_with_saturation;
