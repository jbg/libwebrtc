...
std::unique_ptr<InputVolumeController> CreateInputVolumeController(
    bool enabled,
    const absl::optional<InputVolumeControllerConfig>& config,
    int num_channels) {
  if (enabled) {
    return std::make_unique<InputVolumeController>(
        num_channels, config.value_or(InputVolumeController::Config()));
  }
  return nullptr;
}
...
GainController2::GainController2(
    ...
    const absl::optional<InputVolumeControllerConfig>&
