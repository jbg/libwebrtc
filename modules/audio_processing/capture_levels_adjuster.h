...
// value `pre_gain` and the level `emulated_analog_mic_gain_level` to form a
// combined gain `pre_gain`*`emulated_analog_mic_gain_level`/255 which is
// multiplied to each sample. The intention of the
// `emulated_analog_mic_gain_level` is to be controlled by the analog AGC
// functionality. The post level adjustment is achieved by multiplying each
// sample with the value of `post_gain`. Any changes in the gains take are done
// smoothly over one frame and the scaled samples are clamped to fit into the
// allowed S16 sample range.
class CaptureLevelsAdjuster {
 public:
  ...
  // Enables or disables the emulated analog gain.
  void SetAnalogMicGainEnabled(bool enable);
 ...
 private:
  // Updates the value of pre_adjustment_gain_ based on the supplied values for
  // `pre_gain` and `emulated_analog_mic_gain_level_`.
  void UpdatePreAdjustmentGain();

  bool emulated_analog_mic_gain_enabled_;
