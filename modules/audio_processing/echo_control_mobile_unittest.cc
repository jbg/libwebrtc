...
TEST(EchoControlMobileTest, InterfaceConfiguration) {
  ...
  std::array<EchoControlMobileImpl::RoutingMode, 5> mode = {
  ...
  for (size_t i = 0; i < mode.size(); i++) {
    EXPECT_EQ(0, aecm.set_routing_mode(mode[i]));
    EXPECT_EQ(mode[i], aecm.routing_mode());
  }
  ...
  std::vector<char> echo_path_in(echo_path_size);
  std::vector<char> echo_path_out(echo_path_size);
  EXPECT_EQ(AudioProcessing::kNullPointerError,
            aecm.SetEchoPath(NULL, echo_path_size));
  EXPECT_EQ(AudioProcessing::kNullPointerError,
            aecm.GetEchoPath(NULL, echo_path_size));
