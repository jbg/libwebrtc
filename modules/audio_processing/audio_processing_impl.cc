...
AudioProcessingImpl::ApmPrivateCaptureSubmodules::
    ApmPrivateCaptureSubmodules() = default;
AudioProcessingImpl::ApmPrivateCaptureSubmodules::
    ~ApmPrivateCaptureSubmodules() = default;

AudioProcessingImpl::ApmPrivateRenderSubmodules::ApmPrivateRenderSubmodules() =
    default;
AudioProcessingImpl::ApmPrivateRenderSubmodules::~ApmPrivateRenderSubmodules() =
    default;
...
AudioProcessingImpl::AudioProcessingImpl(
      ...
      capture_nonlocked_() {
  {
    ...
    private_capture_submodules_.capture_post_processor =
        std::move(capture_post_processor);
    private_render_submodules_.render_pre_processor =
        std::move(render_pre_processor);
    private_capture_submodules_.capture_analyzer = std::move(capture_analyzer);
    if (echo_detector) {
      private_capture_submodules_.echo_detector = echo_detector;
    } else {
      private_capture_submodules_.echo_detector =
          new rtc::RefCountedObject<ResidualEchoDetector>();
    }
    // TODO(alessiob): Move the injected gain controller once injection is
    // implemented.
    private_capture_submodules_.gain_controller2.reset(new GainController2());

    RTC_LOG(LS_INFO) << "Capture analyzer activated: "
                     << !!private_capture_submodules_.capture_analyzer
...
AudioProcessingImpl::~AudioProcessingImpl() {
  ...
  private_capture_submodules_.agc_manager.reset();
...
int AudioProcessingImpl::InitializeLocked() {
  ...
  if (constants_.use_experimental_agc) {
    if (!private_capture_submodules_.agc_manager.get()) {
      private_capture_submodules_.agc_manager.reset(new AgcManagerDirect(
    ...
    private_capture_submodules_.agc_manager->Initialize();
    private_capture_submodules_.agc_manager->SetCaptureMuted(
...
void AudioProcessingImpl::ApplyConfig(const AudioProcessing::Config& config) {
  ...
  private_capture_submodules_.gain_controller2->ApplyConfig(
    ...
    switch (setting.type()) {
      case RuntimeSetting::Type::kCapturePreGain:
        if (config_.pre_amplifier.enabled) {
          ...
          private_capture_submodules_.pre_amplifier->SetGainFactor(value);
        }
...
void AudioProcessingImpl::HandleRenderRuntimeSettings() {
  ...
  while (render_runtime_settings_.Remove(&setting)) {
    ...
    switch (setting.type()) {
      case RuntimeSetting::Type::kCustomRenderProcessingRuntimeSetting:
        if (private_render_submodules_.render_pre_processor) {
          private_render_submodules_.render_pre_processor->SetRuntimeSetting(
              setting);
        }
        break;
      case RuntimeSetting::Type::kCapturePreGain:
...
void AudioProcessingImpl::EmptyQueuedRenderAudio() {
  ...
  while (red_render_signal_queue_->Remove(&red_capture_queue_buffer_)) {
    RTC_DCHECK(private_capture_submodules_.echo_detector);
    private_capture_submodules_.echo_detector->AnalyzeRenderAudio(
...
int AudioProcessingImpl::ProcessCaptureStreamLocked() {
  ...
  if (private_capture_submodules_.pre_amplifier) {
    private_capture_submodules_.pre_amplifier->ApplyGain(AudioFrameView<float>(
  ...
  if (private_capture_submodules_.echo_controller) {
    ...
    if (private_capture_submodules_.pre_amplifier) {
      float pre_amp_gain =
          private_capture_submodules_.pre_amplifier->GetGainFactor();
      capture_.echo_path_gain_change =
          capture_.echo_path_gain_change ||
    ...
    private_capture_submodules_.echo_controller->AnalyzeCapture(capture_buffer);
  }

  if (constants_.use_experimental_agc &&
      public_submodules_->gain_control->is_enabled()) {
    private_capture_submodules_.agc_manager->AnalyzePreProcess(
        capture_buffer->channels()[0], capture_buffer->num_channels(),
        capture_nonlocked_.capture_processing_format.num_frames());

    if (constants_.use_experimental_agc_process_before_aec) {
      private_capture_submodules_.agc_manager->Process(
  ...
  if (private_capture_submodules_.low_cut_filter &&
      !private_capture_submodules_.echo_controller) {
    private_capture_submodules_.low_cut_filter->Process(capture_buffer);
  }
  RETURN_ON_ERR(
  ...
  if (public_submodules_->echo_cancellation->is_enabled() &&
      !private_capture_submodules_.echo_controller && !was_stream_delay_set()) {
  ...
  if (private_capture_submodules_.echo_controller) {
    ...
    if (was_stream_delay_set()) {
      private_capture_submodules_.echo_controller->SetAudioBufferDelay(
          stream_delay_ms());
    }

    private_capture_submodules_.echo_controller->ProcessCapture(
  ...
  if (config_.residual_echo_detector.enabled) {
    RTC_DCHECK(private_capture_submodules_.echo_detector);
    private_capture_submodules_.echo_detector->AnalyzeCaptureAudio(
  ...
  if (capture_.transient_suppressor_enabled) {
    float voice_probability =
        private_capture_submodules_.agc_manager.get()
            ? private_capture_submodules_.agc_manager->voice_probability()
            : 1.f;
  ...
  if (private_capture_submodules_.capture_analyzer) {
    private_capture_submodules_.capture_analyzer->Analyze(capture_buffer);
  }

  if (config_.gain_controller2.enabled) {
    private_capture_submodules_.gain_controller2->NotifyAnalogLevel(
        gain_control()->stream_analog_level());
    private_capture_submodules_.gain_controller2->Process(capture_buffer);
  }

  if (private_capture_submodules_.capture_post_processor) {
    private_capture_submodules_.capture_post_processor->Process(capture_buffer);
  }
...
void AudioProcessingImpl::
    AnalyzeRenderStreamWithEchoControllerUnderCritRenderLock(
        AudioBuffer* render_buffer) {
  if (private_capture_submodules_.echo_controller) {
    private_capture_submodules_.echo_controller->AnalyzeRender(render_buffer);
  }
}
...
int AudioProcessingImpl::ProcessRenderStreamLocked() {
  ...
  if (private_render_submodules_.render_pre_processor) {
    private_render_submodules_.render_pre_processor->Process(render_buffer);
  }
  ...
  AnalyzeRenderStreamWithEchoControllerUnderCritRenderLock(render_buffer);
...
AudioProcessing::AudioProcessingStatistics AudioProcessingImpl::GetStatistics()
  ...
  rtc::CritScope cs_capture(&crit_capture_);
  if (private_capture_submodules_.echo_controller) {
    auto ec_metrics = private_capture_submodules_.echo_controller->GetMetrics();
  ...
  RTC_DCHECK(private_capture_submodules_.echo_detector);
  auto ed_metrics = private_capture_submodules_.echo_detector->GetMetrics();
...
AudioProcessingStats AudioProcessingImpl::GetStatistics(
  ...
  if (has_remote_tracks) {
    ...
    if (private_capture_submodules_.echo_controller) {
      auto ec_metrics =
          private_capture_submodules_.echo_controller->GetMetrics();
...
void AudioProcessingImpl::InitializeLowCutFilter() {
  if (submodule_states_.LowCutFilteringRequired()) {
    private_capture_submodules_.low_cut_filter.reset(
...
void AudioProcessingImpl::InitializeEchoController() {
  if (echo_control_factory_) {
    ...
    private_capture_submodules_.echo_controller.reset();
  }
}

void AudioProcessingImpl::InitializeGainController2() {
  if (config_.gain_controller2.enabled) {
    private_capture_submodules_.gain_controller2->Initialize(
        proc_sample_rate_hz());
  }
}
...
void AudioProcessingImpl::InitializePreAmplifier() {
  if (config_.pre_amplifier.enabled) {
    private_capture_submodules_.pre_amplifier.reset(
...
void AudioProcessingImpl::InitializeResidualEchoDetector() {
  RTC_DCHECK(private_capture_submodules_.echo_detector);
  private_capture_submodules_.echo_detector->Initialize(
...
void AudioProcessingImpl::InitializeAnalyzer() {
  if (private_capture_submodules_.capture_analyzer) {
    private_capture_submodules_.capture_analyzer->Initialize(
...
void AudioProcessingImpl::InitializePostProcessor() {
  if (private_capture_submodules_.capture_post_processor) {
    private_capture_submodules_.capture_post_processor->Initialize(
        proc_sample_rate_hz(), num_proc_channels());
  }
}
...
void AudioProcessingImpl::InitializePreProcessor() {
  if (private_render_submodules_.render_pre_processor) {
    private_render_submodules_.render_pre_processor->Initialize(
        formats_.render_processing_format.sample_rate_hz(),
        formats_.render_processing_format.num_channels());
  }
}
