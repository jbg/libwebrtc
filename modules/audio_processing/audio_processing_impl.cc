...
void AudioProcessingImpl::ApplyConfig(const AudioProcessing::Config& config) {
  ...
  // TODO(webrtc:7494): Remove the copy workaround once PreAmplifier is removed.
  // Copy deprecated PreAmplifier params to GainController2 only if PreAmplifier
  // is enabled. This does not break downstream projects while waiting until
  // those projects switch to GainController2::pre_fixed_digital.
  RTC_CHECK(!(config_.pre_amplifier.enabled &&
              config_.gain_controller2.enabled &&
              config_.gain_controller2.pre_fixed_digital.enabled))
      << "The AGC2 pre-processing fixed digital gain has been enabled using "
      << "both PreAmplifier (deprecated) and GainController2 parameters. This "
      << "is error-prone. Only use GainController2::pre_fixed_digital.";
