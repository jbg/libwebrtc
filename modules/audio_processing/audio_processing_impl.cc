...
void AudioProcessingImpl::InitializeEchoController() {
  ...
  size_t element_max_size_ =
      std::max(static_cast<size_t>(1),
               kMaxAllowedValuesOfSamplesPerBand *
                   EchoCancellationImpl::NumCancellersRequired(
                       num_output_channels(), num_reverse_channels()));

  std::vector<float> template_queue_element(element_max_size_);

  aec_render_signal_queue_.reset(
      new SwapQueue<std::vector<float>, RenderQueueItemVerifier<float>>(
          kMaxNumFramesToBuffer, template_queue_element,
          RenderQueueItemVerifier<float>(element_max_size_)));

  aec_render_queue_buffer_.resize(element_max_size_);
  aec_capture_queue_buffer_.resize(element_max_size_);
