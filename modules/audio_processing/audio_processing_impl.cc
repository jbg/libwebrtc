...
// Replaces analog AGC1 with AGC2 input volume controller if
// `input_volume_controller_config_override_` is not empty. Replacing is
// not done if neither or both of AGC1 analog and hybrid AGC is enabled
// or input volume controller is already enabled.
const AudioProcessing::Config AdjustConfig(
  ...
  if (hybrid_agc_config_detected == full_agc1_config_detected ||
    ...
    RTC_LOG(LS_ERROR) << "Input volume controller config override not used.";
    return config;
  }
