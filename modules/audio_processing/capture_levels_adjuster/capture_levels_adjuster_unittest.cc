...
INSTANTIATE_TEST_SUITE_P(
    ...
    [](const ::testing::TestParamInfo<
        std::tuple<int, int, bool, int, float, float>>& info) {
      rtc::StringBuilder builder;
      builder << "sample_rate_hz: " << std::get<0>(info.param);
      builder << "num_channels: " << std::get<1>(info.param);
      builder << "emulated_analog_mic_gain_enabled: "
              << std::get<2>(info.param);
      builder << "emulated_analog_mic_gain_level: " << std::get<3>(info.param);
      builder << "pre_gain: " << std::get<4>(info.param);
      builder << "post_gain: " << std::get<5>(info.param);
      return builder.str();
    });
