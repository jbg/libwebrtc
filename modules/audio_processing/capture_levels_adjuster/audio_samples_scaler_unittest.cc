...
INSTANTIATE_TEST_SUITE_P(
    ...
    [](const ::testing::TestParamInfo<std::tuple<int, int, float>>& info) {
      rtc::StringBuilder builder;
      builder << "sample_rate_hz: " << std::get<0>(info.param);
      builder << "num_channels: " << std::get<1>(info.param);
      builder << "initial_gain: " << std::get<2>(info.param);
      return builder.str();
    });
...
TEST(AudioSamplesScaler, UpwardsClamping) {
  ...
  static_assert(kGain * kMaxClampedSampleValue > kMaxClampedSampleValue, "");
...
TEST(AudioSamplesScaler, DownwardsClamping) {
  ...
  static_assert(kGain * kMinClampedSampleValue < kMinClampedSampleValue, "");
