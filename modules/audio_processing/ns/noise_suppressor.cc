...
constexpr std::array<float, 95> kBlocks160w256SecondHalf = {
    0.99986614f, 0.99946459f, 0.99879546f, 0.99785892f, 0.99665524f,
    0.99518473f, 0.99344778f, 0.99144486f, 0.98917651f, 0.98664333f,
    0.98384601f, 0.98078528f, 0.97746197f, 0.97387698f, 0.97003125f,
    0.96592583f, 0.96156180f, 0.95694034f, 0.95206268f, 0.94693013f,
    0.94154407f, 0.93590593f, 0.93001722f, 0.92387953f, 0.91749450f,
    0.91086382f, 0.90398929f, 0.89687274f, 0.88951608f, 0.88192126f,
    0.87409034f, 0.86602540f, 0.85772861f, 0.84920218f, 0.84044840f,
    0.83146961f, 0.82226822f, 0.81284668f, 0.80320753f, 0.79335334f,
    0.78328675f, 0.77301045f, 0.76252720f, 0.75183981f, 0.74095113f,
    0.72986407f, 0.71858162f, 0.70710678f, 0.69544264f, 0.68359230f,
    0.67155895f, 0.65934582f, 0.64695615f, 0.63439328f, 0.62166057f,
    0.60876143f, 0.59569930f, 0.58247770f, 0.56910015f, 0.55557023f,
    0.54189158f, 0.52806785f, 0.51410274f, 0.50000000f, 0.48576339f,
    0.47139674f, 0.45690388f, 0.44228869f, 0.42755509f, 0.41270703f,
    0.39774847f, 0.38268343f, 0.36751594f, 0.35225005f, 0.33688985f,
    0.32143947f, 0.30590302f, 0.29028468f, 0.27458862f, 0.25881905f,
    0.24298018f, 0.22707626f, 0.21111155f, 0.19509032f, 0.17901686f,
    0.16289547f, 0.14673047f, 0.13052619f, 0.11428696f, 0.09801714f,
    0.08172107f, 0.06540313f, 0.04906767f, 0.03271908f, 0.01636173f};
...
void ApplyFilterBankWindow(rtc::ArrayView<float, kFftSize> x) {
  ...
  for (size_t i = 161, k = 0; i < kFftSize; ++i, ++k) {
    x[i] = kBlocks160w256SecondHalf[k] * x[i];
  }
}

// Extends a frame with previous data.
void FormExtendedFrame(rtc::ArrayView<const float, kNrFrameSize> frame,
                       rtc::ArrayView<float, kFftSize - kNrFrameSize> old_data,
...
void OverlapAndAdd(rtc::ArrayView<const float, kFftSize> extended_frame,
                   ...
                   rtc::ArrayView<float, kNrFrameSize> output_frame) {
  ...
  std::copy(extended_frame.begin() + kOverlapSize,
            extended_frame.begin() + kNrFrameSize,
            output_frame.begin() + kOverlapSize);
  std::copy(extended_frame.begin() + kNrFrameSize, extended_frame.end(),
            overlap_memory.begin());
}

// Produces a delayed frame.
void DelaySignal(rtc::ArrayView<const float, kNrFrameSize> frame,
                 rtc::ArrayView<float, kFftSize - kNrFrameSize> delay_buffer,
                 rtc::ArrayView<float, kNrFrameSize> delayed_frame) {
  constexpr size_t kSamplesFromFrame = kNrFrameSize - (kFftSize - kNrFrameSize);
...
float ComputeEnergyOfExtendedFrame(
    rtc::ArrayView<const float, kNrFrameSize> frame,
    rtc::ArrayView<float, kFftSize - kNrFrameSize> old_data) {
  float energy = 0.f;
...
float ComputeUpperBandsGain(
  ...
  // positive.
  RTC_DCHECK_GT(sum_analysis_spectrum, 0);
...
NoiseSuppressor::ChannelState::ChannelState(
  ...
  prev_analysis_signal_spectrum.fill(0);
...
void NoiseSuppressor::Analyze(const AudioBuffer& audio) {
  ...
  bool zero_frame = false;
  for (size_t ch = 0; ch < num_channels_; ++ch) {
    rtc::ArrayView<const float, kNrFrameSize> y_band0(
        &audio.split_bands_const(ch)[0][0], kNrFrameSize);
    float energy = ComputeEnergyOfExtendedFrame(
        y_band0, channels_[ch]->analyze_analysis_memory);
    zero_frame = zero_frame && energy == 0.f;
  }
...
void NoiseSuppressor::Process(AudioBuffer* audio) {
  ...
  for (size_t ch = 0; ch < num_channels_; ++ch) {
    // Form an extended frame and apply analysis filter bank windowing.
    rtc::ArrayView<float, kNrFrameSize> y_band0(&audio->split_bands(ch)[0][0],
                                                kNrFrameSize);
  ...
  if (num_bands_ > 1) {
    ...
    for (size_t ch = 0; ch < num_channels_; ++ch) {
      for (size_t b = 1; b < num_bands_; ++b) {
        ...
        // the lowest band.
        rtc::ArrayView<float, kNrFrameSize> y_band(
            &audio->split_bands(ch)[b][0], kNrFrameSize);
        std::array<float, kNrFrameSize> delayed_frame;
        DelaySignal(y_band, channels_[ch]->process_delay_memory[b - 1],
                    delayed_frame);

        // Apply the time-domain noise-attenuating gain.
        for (size_t j = 0; j < kNrFrameSize; j++) {
  ...
  for (size_t ch = 0; ch < num_channels_; ++ch) {
    for (size_t b = 0; b < num_bands_; ++b) {
      rtc::ArrayView<float, kNrFrameSize> y_band(&audio->split_bands(ch)[b][0],
                                                 kNrFrameSize);
      for (size_t j = 0; j < kNrFrameSize; j++) {
