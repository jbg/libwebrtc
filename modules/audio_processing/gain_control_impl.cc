...
int GainControlImpl::AnalyzeCaptureAudio(AudioBuffer* audio) {
  ...
  if (mode_ == kAdaptiveAnalog) {
    ...
    for (auto& gain_controller : gain_controllers_) {
      // Why *reported*? When is it queried?
      gain_controller->set_capture_level(reported_analog_capture_level_);
...
int GainControlImpl::set_stream_analog_level(int level) {
  ...
  reported_analog_capture_level_ = level;
...
void GainControlImpl::Initialize(size_t num_proc_channels, int sample_rate_hz) {
  ...
  for (auto& gain_controller : gain_controllers_) {
    ...
    gain_controller->Initialize(minimum_capture_level_, maximum_capture_level_,
                                ...
                                reported_analog_capture_level_);
  }
