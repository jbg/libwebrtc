...
// via the |argv| argument. Pass |processed_capture_samples| to write the
// samples processed on the capture side; if |processed_capture_samples|  is not
// passed, the output file can optinally be specified via the |argv| argument.
int AudioprocFloatImpl(std::unique_ptr<AudioProcessingBuilder> ap_builder,
                       int argc,
                       char* argv[],
                       absl::string_view input_aecdump = nullptr,
