...
EchoCanceller3Config ParseAec3Parameters(const std::string& filename) {
  ...
  while (std::getline(f, s)) {
    ...
    if (name == "filter_length_blocks") {
      cfg.filter.length_blocks = std::stoi(value);
      std::cout << "filter_length_blocks:" << cfg.filter.length_blocks
                << std::endl;
      continue;
    }

    if (name == "filter_shadow_rate") {
      cfg.filter.shadow_rate = std::stof(value);
      std::cout << "filter_shadow_rate:" << cfg.filter.shadow_rate << std::endl;
      continue;
    }

    if (name == "filter_leakage_converged") {
      cfg.filter.leakage_converged = std::stof(value);
      std::cout << "filter_leakage_converged:" << cfg.filter.leakage_converged
                << std::endl;
      continue;
    }

    if (name == "filter_leakage_diverged") {
      cfg.filter.leakage_diverged = std::stof(value);
      std::cout << "filter_leakage_diverged:" << cfg.filter.leakage_diverged
                << std::endl;
      continue;
    }

    if (name == "filter_error_floor") {
      cfg.filter.error_floor = std::stof(value);
      std::cout << "filter_error_floor:" << cfg.filter.error_floor << std::endl;
      continue;
    }

    if (name == "filter_main_noise_gate") {
      cfg.filter.main_noise_gate = std::stof(value);
      std::cout << "filter_main_noise_gate:" << cfg.filter.main_noise_gate
                << std::endl;
      continue;
    }

    if (name == "filter_shadow_noise_gate") {
      cfg.filter.shadow_noise_gate = std::stof(value);
      std::cout << "filter_shadow_noise_gate:" << cfg.filter.shadow_noise_gate
                << std::endl;
      continue;
    }
