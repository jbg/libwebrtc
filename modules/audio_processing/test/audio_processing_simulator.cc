...
EchoCanceller3Config ParseAec3Parameters(const std::string& filename) {
  ...
  while (std::getline(f, s)) {
    ...
    if (name == "delay_delay_headroom_blocks") {
      cfg.delay.delay_headroom_blocks = std::stoi(value);
      std::cout << "delay_delay_headroom_blocks:"
                << cfg.delay.delay_headroom_blocks << std::endl;
      continue;
    }

    if (name == "delay_hysteresis_limit_1_blocks") {
      cfg.delay.hysteresis_limit_1_blocks = std::stoi(value);
      std::cout << "delay_hysteresis_limit_1_blocks:"
                << cfg.delay.hysteresis_limit_1_blocks << std::endl;
      continue;
    }

    if (name == "delay_hysteresis_limit_2_blocks") {
      cfg.delay.hysteresis_limit_2_blocks = std::stoi(value);
      std::cout << "delay_hysteresis_limit_2_blocks:"
                << cfg.delay.hysteresis_limit_2_blocks << std::endl;
      continue;
    }

    if (name == "filter_main" || name == "filter_main_initial") {
      EchoCanceller3Config::Filter::MainConfiguration c;
      size_t comma_index = value.find(',');
      if (comma_index == std::string::npos) {
        std::cout << "Error: wrong format for main filter configuration (1)"
                  << std::endl;
        RTC_NOTREACHED();
      }
      c.length_blocks = std::stoi(value.substr(0, comma_index));

      value = value.substr(comma_index + 1);
      comma_index = value.find(',');
      if (comma_index == std::string::npos) {
        std::cout << "Error: wrong format for main filter configuration (2)"
                  << std::endl;
        RTC_NOTREACHED();
      }
      c.leakage_converged = std::stof(value.substr(0, comma_index));

      value = value.substr(comma_index + 1);
      comma_index = value.find(',');
      if (comma_index == std::string::npos) {
        std::cout << "Error: wrong format for main filter configuration (3)"
                  << std::endl;
        RTC_NOTREACHED();
      }
      c.leakage_diverged = std::stof(value.substr(0, comma_index));

      value = value.substr(comma_index + 1);
      comma_index = value.find(',');
      if (comma_index == std::string::npos) {
        std::cout << "Error: wrong format for main filter configuration (4)"
                  << std::endl;
        RTC_NOTREACHED();
      }
      c.error_floor = std::stof(value.substr(0, comma_index));

      value = value.substr(comma_index + 1);
      c.noise_gate = std::stof(value);

      std::cout << name << ":{" << c.length_blocks << ", "
                << c.leakage_converged << ", " << c.leakage_diverged << ", "
                << c.error_floor << ", " << c.noise_gate << "}" << std::endl;

      if (name == "filter_main") {
        cfg.filter.main = c;
      } else if (name == "filter_main_initial") {
        cfg.filter.main_initial = c;
      } else {
        RTC_NOTREACHED();
      }
    ...
    if (name == "filter_shadow" || name == "filter_shadow_initial") {
      EchoCanceller3Config::Filter::ShadowConfiguration c;
      size_t comma_index = value.find(',');
      if (comma_index == std::string::npos) {
        std::cout << "Error: wrong format for shadow filter configuration (1)"
                  << std::endl;
        RTC_NOTREACHED();
      }
      c.length_blocks = std::stoi(value.substr(0, comma_index));
      ...
      comma_index = value.find(',');
      if (comma_index == std::string::npos) {
        std::cout << "Error: wrong format for shadow filter configuration (2)"
                  << std::endl;
        RTC_NOTREACHED();
      }
      c.rate = std::stof(value.substr(0, comma_index));

      value = value.substr(comma_index + 1);
      c.noise_gate = std::stof(value);

      std::cout << name << ":{" << c.length_blocks << ", " << c.rate << ", "
                << c.noise_gate << "}" << std::endl;

      if (name == "filter_shadow") {
        cfg.filter.shadow = c;
      } else if (name == "filter_shadow_initial") {
        cfg.filter.shadow_initial = c;
      } else {
        RTC_NOTREACHED();
      }
    ...
    if (name == "gain_updates_low_noise" || name == "gain_updates_initial" ||
        name == "gain_updates_normal" || name == "gain_updates_saturation" ||
        name == "gain_updates_nonlinear") {
      EchoCanceller3Config::GainUpdates::GainChanges gc;
      size_t comma_index = value.find(',');
      if (comma_index == std::string::npos) {
        std::cout << "Error: wrong format for gain_updates (1)" << std::endl;
        RTC_NOTREACHED();
      }
      ...
      comma_index = value.find(',');
      if (comma_index == std::string::npos) {
        std::cout << "Error: wrong format for gain_updates (2)" << std::endl;
        RTC_NOTREACHED();
      }
      ...
      comma_index = value.find(',');
      if (comma_index == std::string::npos) {
        std::cout << "Error: wrong format for gain_updates (3)" << std::endl;
        RTC_NOTREACHED();
      }
      ...
      comma_index = value.find(',');
      if (comma_index == std::string::npos) {
        std::cout << "Error: wrong format for gain_updates (4)" << std::endl;
        RTC_NOTREACHED();
      }
      ...
      comma_index = value.find(',');
      if (comma_index == std::string::npos) {
        std::cout << "Error: wrong format for gain_updates (5)" << std::endl;
        RTC_NOTREACHED();
      }
      gc.min_inc = std::stof(value.substr(0, comma_index));

      value = value.substr(comma_index + 1);
      gc.min_dec = std::stof(value);
      ...
      } else if (name == "gain_updates_initial") {
        cfg.gain_updates.initial = gc;
