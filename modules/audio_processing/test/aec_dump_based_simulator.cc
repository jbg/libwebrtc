...
void AecDumpBasedSimulator::HandleMessage(
  ...
  if (!settings_.discard_all_settings_in_aecdump) {
    ...
    if (msg.has_agc2_enabled() || settings_.use_agc2) {
      bool enable =
          settings_.use_agc2 ? *settings_.use_agc2 : msg.agc2_enabled();
...
void AecDumpBasedSimulator::HandleMessage(
  ...
  } else if (msg.has_capture_fixed_digital_gain_db()) {
    // Handle capture fixed-digital-gain runtime setting only if not overridden.
    if ((!settings_.use_agc2 || !(*settings_.use_agc2)) &&
        !settings_.agc2_fixed_gain_db) {
      ap_->SetRuntimeSetting(
          AudioProcessing::RuntimeSetting::CreateFixedDigitalGainDb(
              msg.capture_fixed_digital_gain_db()));
    }
  }
}
