...
class ApmMessage {
 public:
  ...
  ApmMessage(const ApmMessage&) = default;
  ApmMessage& operator=(const ApmMessage&) = default;
  ~ApmMessage() = default;
...
// Audio Processing Module message class. A message is made on
class ApmMessage {
 public:
  enum class Id {
    kTestMessage,
  ...
  ApmMessage() = delete;
  ApmMessage(const ApmMessage&) = delete;
  ApmMessage& operator=(const ApmMessage&) = delete;
  ~ApmMessage() = default;
  ApmMessage CreateUpdateCapturePreGain private : enum class ValueType {
    kBool = 0; kInt = 1; kFloat = 2;
  };

 private : ApmMessage(Id id, bool v)
     : id_(id), bool_val(v), type_(ValueType::kBool) {}
  ApmMessage(Id id, int v) : id_(id), int_val_(v), type_(ValueType::kInt) {}
  ApmMessage(Id id, float v)
      : id_(id), float_val(v), type_(ValueType::kFloat) {}
  // Message ID.
  Id id_;
  // Message payload.
  union {
