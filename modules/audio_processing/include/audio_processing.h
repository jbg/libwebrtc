...
class RTC_EXPORT AudioProcessing : public rtc::RefCountInterface {
 public:
  ...
  struct RTC_EXPORT Config {
    ...
    // TODO(peah): Deprecate and use the pre-gain functionality in
    ...
    struct CaptureLevelAdjustment {
      ...
      // The `legacy_pre_gain_factor` handles the legacy pre_amplifier gain
      // until that has been deprecated. This gain is applied in cascade with
      // the `pre_gain_factor`.
      float legacy_pre_gain_factor = 1.f;
