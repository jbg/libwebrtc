...
void JitterEstimator::UpdateEstimate(TimeDelta frame_delay,
  ...
  double frame_delay_ms = ToFractionalMillis(frame_delay);
...
double JitterEstimator::ToFractionalMillis(const TimeDelta& time_delta) const {
...
TimeDelta JitterEstimator::FromFractionalMillis(double millis) const {
  if (config_.microsecond_granularity) {
...
TimeDelta JitterEstimator::CalculateEstimate() {
  ...
  double ret_ms = kalman_filter_.GetFrameDelayVariationEstimateSizeBased(
                  ...
                  NoiseThreshold();
  TimeDelta ret = FromFractionalMillis(ret_ms);
