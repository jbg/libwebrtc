...
TEST_F(FrameSchedulerTest, SameFrameNotScheduledTwice) {
  // A frame could be scheduled twice if frames arrive out-of-order but the
  // older frame is old enough to be fast forwarded.
  //
  ...
  for (int i = 1; i <= 30; ++i) {
    frame_buffer_.InsertFrame(
        Builder().Id(i).Time(i * kFps30Rtp).AsLast().Build());
    time_controller_.AdvanceTime(kFps30Delay);
    timing_.IncomingTimestamp(
        i * kFps30Rtp, time_controller_.GetClock()->TimeInMilliseconds());
    OnQueue([&] { scheduler_->OnFrameBufferUpdated(); });
    OnQueue([&] { scheduler_->OnReadyForNextFrame(); });
  }
  ...
  frame_buffer_.InsertFrame(
      Builder().Id(32).Time(32 * kFps30Rtp).AsLast().Build());
  timing_.IncomingTimestamp(32 * kFps30Rtp,
                            time_controller_.GetClock()->TimeInMilliseconds());
  OnQueue([&] { scheduler_->OnFrameBufferUpdated(); });

  // F3 arrives before F2 is extracted.
  time_controller_.AdvanceTime(kFps30Delay / 4);
  frame_buffer_.InsertFrame(
      Builder().Id(33).Time(33 * kFps30Rtp).AsLast().Build());
  timing_.IncomingTimestamp(33 * kFps30Rtp,
                            time_controller_.GetClock()->TimeInMilliseconds());
  OnQueue([&] { scheduler_->OnFrameBufferUpdated(); });
  ...
  time_controller_.AdvanceTime(kFps30Delay / 2);
  frame_buffer_.InsertFrame(
      Builder().Id(31).Time(31 * kFps30Rtp).AsLast().Build());
  timing_.IncomingTimestamp(31 * kFps30Rtp,
                            time_controller_.GetClock()->TimeInMilliseconds());
  OnQueue([&] { scheduler_->OnFrameBufferUpdated(); });

  time_controller_.AdvanceTime(kFps30Delay);
  EXPECT_THAT(frames().back(), ElementsAre(FrameWithId(32)));
  OnQueue([&] { scheduler_->OnReadyForNextFrame(); });

  time_controller_.AdvanceTime(kFps30Delay);
  OnQueue([&] { scheduler_->OnReadyForNextFrame(); });
  EXPECT_THAT(frames().back(), ElementsAre(FrameWithId(33)));
  EXPECT_THAT(frames(), SizeIs(33));
}
