...
FrameScheduler::SelectNextDecodableFrame() {
  ...
  while (frame_buffer_->NextDecodableTemporalUnitRtpTimestamp().has_value()) {
    ...
    const bool too_many_frames_queued =
        frame_buffer_->CurrentSize() >
        zero_playout_delay_max_decode_queue_size_;
    ...
    if (max_wait > -kMaxAllowedFrameDelay ||
      ...
      RTC_DLOG(LS_VERBOSE) << __FUNCTION__ << " selected frame with rtp "
                           << next_rtp << " render time " << render_time
                           << " with a max wait of " << max_wait.ms() << "ms";
      return std::make_pair(next_rtp, max_wait);
    }
    RTC_DLOG(LS_VERBOSE) << __FUNCTION__ << " fast-forwarded frame " << next_rtp
                         << " render time " << render_time << " with delay "
                         << max_wait.ms() << "ms";
    frame_buffer_->DropNextDecodableTemporalUnit();
  }

  RTC_DLOG(LS_VERBOSE) << __FUNCTION__ << " selected no frame frame to decode.";
  return absl::nullopt;
}
