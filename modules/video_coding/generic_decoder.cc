...
VCMDecodedFrameCallback::VCMDecodedFrameCallback(VCMTiming* timing,
  ...
  int extra_decode_time_ms = 0;
  if (!group_name.empty() &&
      sscanf(group_name.c_str(), "%d", &extra_decode_time_ms) == 1 &&
      extra_decode_time_ms >= 0) {
    extra_decode_time_ms_ = extra_decode_time_ms;
  }
}
...
void VCMDecodedFrameCallback::Decoded(VideoFrame& decodedImage,
  ...
  if (extra_decode_time_ms_) {
    rtc::Thread::SleepMs(*extra_decode_time_ms_);
  }
