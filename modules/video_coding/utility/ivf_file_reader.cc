...
bool IvfFileReader::Reset() {
  ...
  if (memcmp(&ivf_header[0], kFileHeaderStart, 4) != 0) {
    RTC_LOG(LS_ERROR) << "File is not in IVF format: "
                      << std::string(reinterpret_cast<char const*>(ivf_header),
                                     kIvfHeaderSize);
    return false;
  }
...
absl::optional<EncodedImage> IvfFileReader::NextFrame() {
  ...
  while (next_frame_header_ &&
    ...
    size_t current_layer_start_pos = current_layer_start_pos = payload->size();
