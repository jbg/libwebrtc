...
class IvfFileReader {
 public:
  ...
  std::unique_ptr<IvfFileReader> Create(FileWrapper file) {
    auto reader =
 ...
 private:
  ...
  // contains 4 bytes and caller has to ensure that buffer won't overflow.
  absl::optional<VideoCodecType> ParseCodecType(uint8_t* buffer,
                                                size_t start_pos);
  absl::optional<FrameHeader> NextFrameHeader();
