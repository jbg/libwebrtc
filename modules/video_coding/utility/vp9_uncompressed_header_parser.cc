...
void Vp9ReadFrameSizeFromRefs(size_t total_buffer_size_bits,
                              BitstreamReader* br,
                              Vp9UncompressedHeader* frame_info) {
  auto found_ref = br->ReadBits(kVp9NumRefsPerFrame);
  if (found_ref != 0) {
    frame_info->infer_size_from_reference =
        frame_info->reference_buffers[kVp9NumRefsPerFrame -
                                      absl::bit_width(found_ref)];
  } else {
...
bool Parse(rtc::ArrayView<const uint8_t> buf,
  ...
  if (frame_info->is_keyframe) {
    ...
    if (is_intra_only) {
      ...
      Vp9ReadFrameSizeFromRefs(total_buffer_size_bits, &br, frame_info);
  ...
  // Trailing bits.
  br.ConsumeBits(br.RemainingBitCount() % 8);
