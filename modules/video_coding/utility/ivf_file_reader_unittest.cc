...
class IvfFileReaderTest : public ::testing::Test {
 protected:
  ...
  void ValidateFrame(absl::optional<EncodedImage> frame,
                     ...
                     int spatial_layers_count) {
    ASSERT_TRUE(frame);
    ASSERT_EQ(frame->SpatialIndex(), spatial_layers_count);
    ASSERT_EQ(frame->size(), sizeof(kDummyPayload) * spatial_layers_count);
    for (int i = 0; i < spatial_layers_count; ++i) {
      ASSERT_EQ(memcmp(&frame->data()[i * sizeof(kDummyPayload)], kDummyPayload,
                       sizeof(kDummyPayload)),
                0)
    ...
    if (use_capture_tims_ms) {
      ASSERT_EQ(frame->capture_time_ms_, static_cast<int64_t>(frame_index));
      ASSERT_EQ(frame->Timestamp(), static_cast<int64_t>(90 * frame_index));
    } else {
      ASSERT_EQ(frame->Timestamp(), static_cast<int64_t>(frame_index));
    }
  }
  ...
  void ValidateContent(VideoCodecType codec_type,
    ...
    ASSERT_EQ(reader->GetVideoCodecType(), codec_type);
    ASSERT_EQ(reader->GetFramesCount(),
    ...
    ASSERT_FALSE(reader->HasMoreFrames());
    ASSERT_FALSE(reader->NextFrame());
    ASSERT_FALSE(reader->HasError());
