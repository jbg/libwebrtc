...
H264SpsPpsTracker::PacketAction H264SpsPpsTracker::CopyAndFixBitstream(
  ...
  if (append_sps_pps) {
    ...
    for (size_t i = 0; i < codec_header->nalus_length; ++i) {
      if (codec_header->nalus[i].type == H264::NaluType::kIdr) {
        codec_header->nalus[i].sps_id = sps->first;
        codec_header->nalus[i].pps_id = pps->first;
        break;
      }
    }
    RTC_CHECK_LE(codec_header->nalus_length + 2,
                 sizeof(codec_header->nalus) / sizeof(NaluInfo));
    NaluInfo sps_info;
