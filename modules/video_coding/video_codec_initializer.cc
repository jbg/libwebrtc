...
VideoCodec VideoCodecInitializer::CullInactiveTopSpatialLayers(
  ...
  switch (config.codecType) {
    case kVideoCodecAV1: {
      std::string scalability_mode_string(
          ScalabilityModeToString(*config.GetScalabilityMode()));
      scalability_mode_string[1] = '1' + highest_active_spatial_layer_id;
      if (highest_active_spatial_layer_id == 0) {
        // L1Tx - remove any _KEY or _KEY_SHIFT suffixes.
        scalability_mode_string = scalability_mode_string.substr(0, 4);
      }
      absl::optional<ScalabilityMode> scalability_mode =
          ScalabilityModeStringToEnum(scalability_mode_string);
      RTC_CHECK(scalability_mode);
      culled_config.SetScalabilityMode(*scalability_mode);
    } break;
    case kVideoCodecVP9:
      culled_config.VP9()->numberOfSpatialLayers =
          highest_active_spatial_layer_id + 1;
      break;
    default:
