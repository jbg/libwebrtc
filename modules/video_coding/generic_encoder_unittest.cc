...
TEST(TestVCMEncodedFrameCallback, NotifiesAboutDroppedFrames) {
  ...
  callback.OnTargetBitrateChanged(500, 0);
  callback.OnEncodeStarted(static_cast<uint32_t>(kTimestampMs1 * 90),
                           kTimestampMs1, 0);
  EXPECT_EQ(0u, sink.GetNumFramesDropped());
  image.capture_time_ms_ = kTimestampMs1;
  image._timeStamp = static_cast<uint32_t>(kTimestampMs1 * 90);
  callback.OnEncodedImage(image, &codec_specific, nullptr);
  callback.OnEncodeStarted(static_cast<uint32_t>(kTimestampMs2 * 90),
                           kTimestampMs2, 0);
  // No OnEncodedImageCall for timestamp2. Yet, at this moment it's not known
  // that frame with timestamp2 was dropped.
  EXPECT_EQ(0u, sink.GetNumFramesDropped());
  callback.OnEncodeStarted(static_cast<uint32_t>(kTimestampMs3 * 90),
                           kTimestampMs3, 0);
  image.capture_time_ms_ = kTimestampMs3;
  image._timeStamp = static_cast<uint32_t>(kTimestampMs3 * 90);
  callback.OnEncodedImage(image, &codec_specific, nullptr);
  EXPECT_EQ(1u, sink.GetNumFramesDropped());
  callback.OnEncodeStarted(static_cast<uint32_t>(kTimestampMs4 * 90),
                           kTimestampMs4, 0);
  image.capture_time_ms_ = kTimestampMs4;
  image._timeStamp = static_cast<uint32_t>(kTimestampMs4 * 90);
  callback.OnEncodedImage(image, &codec_specific, nullptr);
  EXPECT_EQ(1u, sink.GetNumFramesDropped());
}
...
TEST(TestVCMEncodedFrameCallback, RestoresCaptureTimestamps) {
  EncodedImage image;
  CodecSpecificInfo codec_specific;
  const int64_t kTimestampMs = 0xFFFFFFFE;
  codec_specific.codecType = kVideoCodecGeneric;
  codec_specific.codecSpecific.generic.simulcast_idx = 0;
  FakeEncodedImageCallback sink;
  VCMEncodedFrameCallback callback(&sink, nullptr);
  // Any non-zero bitrate needed to be set before the first frame.
  callback.OnTargetBitrateChanged(500, 0);
  callback.OnEncodeStarted(static_cast<uint32_t>(kTimestampMs * 90),
                           kTimestampMs, 0);
  EXPECT_EQ(0u, sink.GetNumFramesDropped());
  image.capture_time_ms_ = 0;  // Incorrect timesetamp.
  image._timeStamp = static_cast<uint32_t>(kTimestampMs * 90);
  callback.OnEncodedImage(image, &codec_specific, nullptr);
