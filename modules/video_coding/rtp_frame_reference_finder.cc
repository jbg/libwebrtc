...
RtpFrameReferenceFinder::FrameDecision RtpFrameReferenceFinder::ManageFrameH264(
  ...
  int64_t tl0id = tl0_unwrapper_.Unwrap(rtp_frame_marking->tl0_pic_idx);

  // Clean up info for base layers that are too old.
  int64_t old_tl0_pic_idx = tl0id - kMaxLayerInfo;
  auto clean_layer_info_to = layer_info_.lower_bound(old_tl0_pic_idx);
  layer_info_.erase(layer_info_.begin(), clean_layer_info_to);
  ...
  if (tid == 0) {
    layer_info_it =
        layer_info_.insert(std::make_pair(tl0id, layer_info_it->second)).first;
    frame->num_references = 1;
    frame->references[0] = layer_info_it->second[0];
    UpdateDataH264(frame, tid, tl0id);
    return kHandOff;
...
void RtpFrameReferenceFinder::UpdateLayerInfoH264(
    RtpFrameObject* frame,
    uint8_t temporal_index, int64_t tl0_pic_index) {
  int64_t tl0id = tl0_pic_index;
