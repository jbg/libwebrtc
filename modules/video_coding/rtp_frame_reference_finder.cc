...
void RtpFrameReferenceFinder::ManageFrame(
  ...
  if (cleared_to_timestamp_ &&
      AheadOrAt<uint32_t>(*cleared_to_timestamp_, frame->Timestamp())) {
    return;
  }
...
void RtpFrameReferenceFinder::ClearTo(uint32_t timestamp) {
  rtc::CritScope lock(&crit_);
  cleared_to_timestamp_ = timestamp;

  auto it = stashed_frames_.begin();
  while (it != stashed_frames_.end()) {
    if (AheadOrAt<uint32_t>(timestamp, (*it)->Timestamp())) {
