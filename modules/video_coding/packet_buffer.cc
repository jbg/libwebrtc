  ...
  for (size_t i = 0; i < size_ && PotentialNewFrame(seq_num); ++i) {
    ...
    if (sequence_buffer_[index].frame_end) {
      ...
      if (is_h264) {
        ...
        // Let the RtpFrameObject to be created know what type of frame this is.
        if (is_h264_keyframe) {
          data_buffer_[start_seq_num % size_].frameType = kVideoFrameKey;
        } else {
          data_buffer_[start_seq_num % size_].frameType = kVideoFrameDelta;
        }
