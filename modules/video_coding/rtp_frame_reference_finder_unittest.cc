...
class TestRtpFrameReferenceFinder : public ::testing::Test {
 protected:
  TestRtpFrameReferenceFinder()
      : rand_(0x8739211),
        reference_finder_(new RtpFrameReferenceFinder()),
...
TEST_F(TestRtpFrameReferenceFinder, PaddingPackets) {
  ...
  OnCompleteFrames(reference_finder_->PaddingReceived(sn + 1));
...
TEST_F(TestRtpFrameReferenceFinder, PaddingPacketsReordered) {
  ...
  InsertGeneric(sn, sn, true);
  OnCompleteFrames(reference_finder_->PaddingReceived(sn + 1));
  OnCompleteFrames(reference_finder_->PaddingReceived(sn + 4));
...
TEST_F(TestRtpFrameReferenceFinder, PaddingPacketsReorderedMultipleKeyframes) {
  ...
  OnCompleteFrames(reference_finder_->PaddingReceived(sn + 6));
  OnCompleteFrames(reference_finder_->PaddingReceived(sn + 9));
