...
TEST_F(TestBasicJitterBuffer, SpsAndPpsHandling) {
  ...
  packet_->video_header.video_type_header = RTPVideoHeaderH264();
  auto* h264 =
      &absl::get<RTPVideoHeaderH264>(packet_->video_header.video_type_header);
  packet_->timestamp = timestamp_;
  packet_->frameType = kVideoFrameKey;
  packet_->is_first_packet_in_frame = true;
  packet_->markerBit = true;
  ...
  h264->nalu_type = H264::NaluType::kIdr;
  h264->nalus[0].type = H264::NaluType::kIdr;
  h264->nalus[0].sps_id = -1;
  h264->nalus[0].pps_id = 0;
  h264->nalus_length = 1;
  bool retransmitted = false;
  EXPECT_EQ(kCompleteSession,
            jitter_buffer_->InsertPacket(*packet_, &retransmitted));
  ...
  packet_->codec = kVideoCodecH264;
  packet_->video_header.codec = kVideoCodecH264;
  h264->nalu_type = H264::NaluType::kStapA;
  h264->nalus[0].type = H264::NaluType::kSps;
  h264->nalus[0].sps_id = 0;
  h264->nalus[0].pps_id = -1;
  h264->nalus[1].type = H264::NaluType::kPps;
  h264->nalus[1].sps_id = 0;
  h264->nalus[1].pps_id = 0;
  h264->nalus_length = 2;
  ...
  packet_->codec = kVideoCodecH264;
  packet_->video_header.codec = kVideoCodecH264;
  h264->nalu_type = H264::NaluType::kSlice;
  h264->nalus[0].type = H264::NaluType::kSlice;
  h264->nalus[0].sps_id = -1;
  h264->nalus[0].pps_id = 0;
  h264->nalus_length = 1;
