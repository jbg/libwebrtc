...
class VCMPacketTest : public VCMPacket {
 public:
  VCMPacketTest() {
    codec = kVideoCodecH264;
    video_header.is_first_packet_in_frame = false;
    RTPVideoHeaderH264 type_header = {};
...
class TestH264SpsPpsTracker : public ::testing::Test {
 public:
  void AddSps(VCMPacketTest* packet,
              uint8_t sps_id,
  ...
  void AddPps(VCMPacketTest* packet,
              uint8_t sps_id,
              uint8_t pps_id,
              std::vector<uint8_t>* data) {
    NaluInfo info;
  ...
  void AddIdr(VCMPacketTest* packet, int pps_id) {
    NaluInfo info;
