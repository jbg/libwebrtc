...
int StereoEncoderAdapter::InitEncode(const VideoCodec* inst,
  ...
  VideoCodec associated_codec(*inst);
  associated_codec.codecType = kVideoCodecVP9;

  for (size_t i = 0; i < kAlphaCodecStreams; ++i) {
    std::unique_ptr<VideoEncoder> encoder =
        factory_->CreateVideoEncoder(format);
    const int rv = encoder->InitEncode(&associated_codec, number_of_cores,
                                       max_payload_size);
    if (rv) {
...
EncodedImageCallback::Result StereoEncoderAdapter::OnEncodedImage(
  ...
  codec_info->codecSpecific.stereo.associated_codec_type =
      associated_coded_type;
  codec_info->codecSpecific.stereo.frame_index = stream_idx;
  codec_info->codecSpecific.stereo.frame_count = frame_count;
  codec_info->codecSpecific.stereo.picture_index = picture_index;
