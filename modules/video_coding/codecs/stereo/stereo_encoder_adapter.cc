...
int StereoEncoderAdapter::InitEncode(const VideoCodec* inst,
  ...
  for (size_t i = 0; i < kStereoCodecStreams; ++i) {
    std::unique_ptr<VideoEncoder> encoder =
        CreateScopedVideoEncoder(factory_, cricket::VideoCodec("VP9"));
...
int StereoEncoderAdapter::Encode(const VideoFrame& input_image,
  ...
  VideoFrame yuv_image(input_image);
  ...
  // Encode YUV
  int rv = encoders_[kYUVStream]->Encode(yuv_image, codec_specific_info,
                                         frame_types);
  return rv;
}
