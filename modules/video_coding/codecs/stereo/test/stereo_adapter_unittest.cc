...
TEST_F(TestStereoAdapter, EncodeDecodeI420AFrame) {
  ...
  EXPECT_EQ(kVideoCodecVP9, yuv_info.codecSpecific.stereo.associatedCodecType);
  EXPECT_EQ(kYUVStream, yuv_info.codecSpecific.stereo.frameIndex);
  EXPECT_EQ(kAlphaCodecStreams, yuv_info.codecSpecific.stereo.frameCount);
  EXPECT_EQ(0ull, yuv_info.codecSpecific.stereo.pictureIndex);

  const CodecSpecificInfo& axx_info = codec_specific_infos[kAXXStream];
  EXPECT_EQ(kVideoCodecStereo, axx_info.codecType);
  EXPECT_EQ(kVideoCodecVP9, axx_info.codecSpecific.stereo.associatedCodecType);
  EXPECT_EQ(kAXXStream, axx_info.codecSpecific.stereo.frameIndex);
  EXPECT_EQ(kAlphaCodecStreams, axx_info.codecSpecific.stereo.frameCount);
  EXPECT_EQ(0ull, axx_info.codecSpecific.stereo.pictureIndex);
