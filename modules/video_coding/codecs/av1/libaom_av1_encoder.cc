...
int32_t LibaomAv1Encoder::Encode(
  ...
  ScalableVideoController::TemporalUnitConfig next =
      svc_controller_->NextFrameConfig(keyframe_required_);
  if (next.stream_config) {
    // TODO(danilchap): Configure SVC.
  }

  if (next.layer_frames.empty()) {
  ...
  RTC_DCHECK_EQ(next.layer_frames.size(), 1);
  for (ScalableVideoController::LayerFrameConfig& layer_frame :
       next.layer_frames) {
    ...
    if (encoded_image.size() > 0) {
      ...
      if (layer_frame.is_keyframe) {
        codec_specific_info.template_structure =
            svc_controller_->DependencyStructure();
      }
      codec_specific_info.generic_frame_info = layer_frame.rtp_frame_info;
