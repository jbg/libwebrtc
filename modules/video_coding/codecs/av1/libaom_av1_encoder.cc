...
class LibaomAv1Encoder final : public VideoEncoder {
 public:
  explicit LibaomAv1Encoder(
      std::unique_ptr<FrameDependenciesController> layering_controller);
  ~LibaomAv1Encoder();
 ...
 private:
  const std::unique_ptr<FrameDependenciesController> layering_controller_;
  bool inited_;
...
LibaomAv1Encoder::LibaomAv1Encoder(
    std::unique_ptr<FrameDependenciesController> layering_controller)
    : layering_controller_(std::move(layering_controller)),
      inited_(false),
      keyframe_required_(true),
      frame_for_encode_(nullptr),
      encoded_image_callback_(nullptr) {
  RTC_DCHECK(layering_controller_);
}
...
int32_t LibaomAv1Encoder::Encode(
  ...
  std::vector<GenericFrameInfo> frame_infos =
      layering_controller_->NextFrameConfig(keyframe_required_);
  ...
  // and layering_controller_->DependencyStructure().num_decode_targets > 0.
  ...
  for (GenericFrameInfo& frame_info : frame_infos) {
    ...
    while (const aom_codec_cx_pkt_t* pkt =
      ...
      if (pkt->kind == AOM_CODEC_CX_FRAME_PKT && pkt->data.frame.sz > 0) {
        ...
        bool is_keyframe = ((pkt->data.frame.flags & AOM_EFLAG_FORCE_KF) != 0);
    ...
    if (encoded_image.size() > 0) {
      ...
      // TODO(danilchap): Set frame dependencies structures on key frame when
      // layering is enabled.
      codec_specific_info.generic_frame_info = frame_info;
...
std::unique_ptr<VideoEncoder> CreateLibaomAv1Encoder(
    std::unique_ptr<FrameDependenciesController> controller) {
  return std::make_unique<LibaomAv1Encoder>(std::move(controller));
}
