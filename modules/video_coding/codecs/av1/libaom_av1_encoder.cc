...
int32_t LibaomAv1Encoder::Encode(
  ...
  if (absl::optional<ScalableVideoController::StreamConfiguration> settings =
          svc_controller_->EncoderSettings(keyframe_required_)) {
    // TODO(danilchap): Configure SVC.
  }

  std::vector<GenericFrameInfo> frame_infos =
      svc_controller_->NextFrameConfig();
