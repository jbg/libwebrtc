...
MATCHER(SameLayerIdAndBitrateIsNear, "") {
  // Expect that result bitrate (almost) doesn't exceed requested,
  // but have relaxed expectation on the lower bound: check order of the bitrate
  // is correct: stream might be too simple to produce enough bits.

  // First check if layer id is the same.
  return std::get<0>(arg).first == std::get<1>(arg).first &&
         // check measured bitrate is not 10x lower than requested.
         std::get<0>(arg).second >= std::get<1>(arg).second * 0.1 &&
         // check measured bitrate is no more than 10% larger than requested.
...
TEST_P(LibaomAv1SvcTest, SetRatesMatchMeasuredBitrate) {
  ...
  constexpr TimeDelta kDuration = TimeDelta::Seconds(2);
