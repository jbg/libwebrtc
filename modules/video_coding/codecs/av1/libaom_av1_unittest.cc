...
TEST(LibaomAv1Test, EncodeDecode) {
  ...
  std::vector<TestVideoEncoder::Encoded> encoded_frames =
      TestVideoEncoder(*encoder).SetNumInputFrames(4).Encode();
...
TEST_P(LibaomAv1SvcTest, EncodeAndDecodeAllDecodeTargets) {
  ...
  ASSERT_EQ(encoder->InitEncode(&codec_settings, DefaultEncoderSettings()),
            WEBRTC_VIDEO_CODEC_OK);
  std::vector<TestVideoEncoder::Encoded> encoded_frames =
      TestVideoEncoder(*encoder)
          .SetNumInputFrames(GetParam().num_frames_to_generate)
          .SetResolution({kWidth, kHeight})
          .Encode();

  ASSERT_THAT(
      encoded_frames, Each(Truly([&](const TestVideoEncoder::Encoded& frame) {
  ...
  for (size_t dt = 0; dt < num_decode_targets; ++dt) {
    ...
    for (int64_t frame_id = 0;
      ...
      const TestVideoEncoder::Encoded& frame = encoded_frames[frame_id];
