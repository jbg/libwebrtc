...
VideoCodec DefaultCodecSettings() {
  ...
  codec_settings.maxFramerate = 15;
...
TEST(LibaomAv1EncoderTest, DropFramesWhenRequestedByController) {
  class DropAllFrames : public ScalableVideoController {
   public:
    FrameDependencyStructure DependencyStructure() const override { return {}; }
    TemporalUnitConfig NextFrameConfig(bool /*restart*/) override {
      TemporalUnitConfig no_layer_frames;
      // Explicitly set layer frames to an empty array, since that is used as
      // signal frame should be dropped.
      no_layer_frames.layer_frames = {};
