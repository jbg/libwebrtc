...
std::unique_ptr<VideoDecoder> VP8Decoder::Create() {
  return std::make_unique<LibvpxVp8Decoder>(FieldTrialBasedConfig());
}

std::unique_ptr<VideoDecoder> CreateVp8Decoder(const Environment& env) {
  // TODO: bugs.webrtc.org/15791 - Pass full environment when VP8Decoder::Create
  // is removed so that LibvpxVp8Decoder can use both injected field trials and
  // clock.
  return std::make_unique<LibvpxVp8Decoder>(env.field_trials());
}
