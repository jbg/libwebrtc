...
LibvpxVp8Encoder::PrepareBuffers(rtc::scoped_refptr<VideoFrameBuffer> buffer) {
  ...
  if (!mapped_buffer ||
    ...
    // Because not all buffer implementations overide CropAndScale(), and the
    // default implementations converts to I420 in the process, it is not safe
    // to invoke |buffer->Scale()| and expect zero-conversion unless |buffer|
    // is one of the known and mappable formats. Because we want all simulcast
    // layers to have the same pixel format, we set |buffer| to the newly
    // created I420 buffer which we know can safely be scaled down. This avoids
    // crashes on Android.
    // TODO(hbos): When CropAndScale() is pure virtual and all implementations
    // can scale without also converting, delete this line:
    buffer = mapped_buffer;
    if (!mapped_buffer ||
        absl::c_find(supported_formats, mapped_buffer->type()) ==
            supported_formats.end()) {
      mapped_buffer = buffer->ToI420();  // hack
    }
  }
