...
LibvpxVp8Encoder::PrepareBuffers(rtc::scoped_refptr<VideoFrameBuffer> buffer) {
  ...
  if (!mapped_buffer ||
    ...
    // to invoke |buffer->Scale()| if |buffer| was not a known/mappable format.
    // For this reason, we set |buffer| to the newly created I420 buffer which
    // we know can safely be scaled below. This avoids crashes on Android.
