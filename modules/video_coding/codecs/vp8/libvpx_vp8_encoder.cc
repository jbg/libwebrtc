...
LibvpxVp8Encoder::PrepareBuffers(rtc::scoped_refptr<VideoFrameBuffer> buffer) {
  ...
  if (!mapped_buffer ||
    ...
    // Because |buffer| had do be converted, use |converted_buffer| instead.
  ...
  for (size_t i = 1; i < encoders_.size(); ++i) {
    ...
    if (scaled_buffer->type() != mapped_buffer->type()) {
      RTC_LOG(LS_ERROR) << "When scaling "
                        ...
                        << ", the image was unexpecetdly converted to "
                        << VideoFrameBufferTypeToString(scaled_buffer->type())
                        << ". Can't encode frame.";
      return {};
    }
...
void LibvpxVp8Encoder::SetRawImagePlanes(vpx_image_t* raw_image,
