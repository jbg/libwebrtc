...
LibvpxVp8Encoder::PrepareBuffers(rtc::scoped_refptr<VideoFrameBuffer> buffer) {
  ...
  if (!mapped_buffer ||
    ...
    // The buffer should now be a mapped I420 format, but some buffer
    // implementations incorrectly return another kNative buffer here. As a
    // workaround to this, we re-run PrepareBuffers() with the native I420
    // buffer as argument.
    // TODO(https://crbug.com/webrtc/12602): When Android buffers have a correct
    // ToI420() implementaion, remove his workaround.
    if (buffer->type() == VideoFrameBuffer::Type::kNative) {
      return PrepareBuffers(buffer);
    }
    mapped_buffer = buffer;
  }
  ...
  for (size_t i = 1; i < encoders_.size(); ++i) {
    ...
    RTC_CHECK_EQ(scaled_buffer->type(), mapped_buffer->type())
