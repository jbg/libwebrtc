...
LibvpxVp8Encoder::PrepareBuffers(rtc::scoped_refptr<VideoFrameBuffer> buffer) {
  ...
  for (size_t i = 1; i < encoders_.size(); ++i) {
    // Some implementations of Scale() returns a different pixel format than the
    // original buffer. For this reason, we only use Scale() if we have a native
    // buffer. Otherwise, we fall back on libyuv scaling of the previous layer.
    if (buffer->type() == VideoFrameBuffer::Type::kNative) {
      auto scaled_buffer =
          buffer->Scale(raw_images_[i].d_w, raw_images_[i].d_h);
      if (scaled_buffer->type() == VideoFrameBuffer::Type::kNative) {
        scaled_buffer = scaled_buffer->GetMappedFrameBuffer(mapped_type);
        RTC_DCHECK(scaled_buffer);
      }
      ...
      // TODO(hbos): When CropAndScale() is pure virtual and all implementations
      // support scaling without conversion, delete this code path in favor of
      // always invoking Scale().
      switch (mapped_buffer->type()) {
        case VideoFrameBuffer::Type::kI420:
        case VideoFrameBuffer::Type::kI420A:
          libyuv::I420Scale(
              ...
              raw_images_[i].stride[VPX_PLANE_U],
