...
LibvpxVp8Encoder::PrepareBuffers(rtc::scoped_refptr<VideoFrameBuffer> buffer) {
  ...
  if (!mapped_buffer ||
    ...
    // If mapping to a supported format did not work then we cannot expect
    // Scale()+GetMappedFrameBuffer() to work either (for simulcast below). So
    // we start over with the I420 buffer as the input buffer.
    // TODO(hbos): Adding ToI420() code below might fix the problem?
    return PrepareBuffers(i420_buffer);
  }
