...
void ScreenshareLayers::SetQpLimits(size_t stream_index,
  ...
  RTC_DCHECK_GT(min_qp, 0);
  RTC_DCHECK_GE(max_qp, min_qp);

  RTC_DCHECK(min_qp_.has_value() == max_qp_.has_value());
  if (!min_qp_.has_value()) {
    min_qp_ = min_qp;
    max_qp_ = max_qp;
  } else {
    RTC_DCHECK_EQ(min_qp, *min_qp_);
    RTC_DCHECK_EQ(max_qp, *max_qp_);
  }
}
...
Vp8EncoderConfig ScreenshareLayers::UpdateConfiguration(size_t stream_index) {
  ...
  // TODO: !!! Check this function again. And again.
