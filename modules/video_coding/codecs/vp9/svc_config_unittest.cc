...
TEST(SvcConfig, AlwaysSendsAtLeastOneLayer) {
  ...
  std::vector<SpatialLayer> spatial_layers2 = GetSvcConfig(
      ...
      first_active_layer, max_num_spatial_layers, 1, false);
  EXPECT_EQ(spatial_layers2.size(), 1u);
  EXPECT_EQ(spatial_layers2.back().width, kMinVp9SpatialLayerShortSideLength);
}
