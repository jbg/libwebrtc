...
VP9EncoderImpl::VP9EncoderImpl()
    : encoded_image_(),
      ...
      inter_layer_pred_(InterLayerPredMode::kInterLayerPredOn),
...
int VP9EncoderImpl::InitAndSetControlSettings(const VideoCodec* inst) {
  ...
  if (num_spatial_layers_ > 1) {
    switch (inter_layer_pred_) {
      case InterLayerPredMode::kInterLayerPredOn:
        vpx_codec_control(encoder_, VP9E_SET_SVC_INTER_LAYER_PRED, 0);
        break;
      case InterLayerPredMode::kInterLayerPredOff:
        vpx_codec_control(encoder_, VP9E_SET_SVC_INTER_LAYER_PRED, 1);
        break;
      case InterLayerPredMode::kInterLayerPredOnKeyPic:
...
void VP9EncoderImpl::PopulateCodecSpecific(CodecSpecificInfo* codec_specific,
  ...
  const bool is_inter_layer_pred_allowed =
      (inter_layer_pred_ == InterLayerPredMode::kInterLayerPredOn ||
       (inter_layer_pred_ == InterLayerPredMode::kInterLayerPredOnKeyPic &&
        is_key_pic));
