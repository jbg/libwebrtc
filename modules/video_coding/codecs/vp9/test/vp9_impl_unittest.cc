...
using FramerateFractions =
    webrtc::VideoEncoder::EncoderInfo::FramerateFractions;
...
TEST_F(TestVp9Impl, EncoderInfoFpsAllocation) {
  ...
  expected_fps_allocation[0].push_back(EncoderInfo::kFullFramerate / 4);
  expected_fps_allocation[0].push_back(EncoderInfo::kFullFramerate / 2);
  expected_fps_allocation[0].push_back(EncoderInfo::kFullFramerate);
...
TEST_F(TestVp9Impl, EncoderInfoFpsAllocationFlexibleMode) {
  ...
  FramerateFractions expected_fps_allocation[kMaxSpatialLayers];
  expected_fps_allocation[0].push_back(EncoderInfo::kFullFramerate / 3);
  expected_fps_allocation[1].push_back(EncoderInfo::kFullFramerate / 2);
  expected_fps_allocation[2].push_back(EncoderInfo::kFullFramerate);
  EXPECT_THAT(encoder_->GetEncoderInfo().fps_allocation,
              ::testing::ElementsAreArray(expected_fps_allocation));
}
