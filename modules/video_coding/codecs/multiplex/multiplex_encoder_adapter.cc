...
MultiplexEncoderAdapter::MultiplexEncoderAdapter(
    ...
    : factory_(factory),
      ...
      has_whitelisted_rate_controllers_(false) {}
...
int MultiplexEncoderAdapter::InitEncode(const VideoCodec* inst,
  ...
  has_whitelisted_rate_controllers_ = true;
  for (size_t i = 0; i < kAlphaCodecStreams; ++i) {
    ...
    has_whitelisted_rate_controllers_ &=
        encoder->HasWhitelistedRateController();
...
bool MultiplexEncoderAdapter::HasWhitelistedRateController() const {
  return has_whitelisted_rate_controllers_;
}
