...
std::string VideoStatistic::ToString(std::string prefix) const {
  std::stringstream ss;
...
std::vector<VideoStatistic> Stats::SliceAndCalcLayerVideoStatistic(
    size_t first_frame_num,
    size_t last_frame_num) {
  std::vector<VideoStatistic> layer_stats;
  ...
  for (size_t spatial_layer_idx = 0; spatial_layer_idx < num_spatial_layers;
    ...
    for (size_t temporal_layer_idx = 0;
      ...
      VideoStatistic layer_stat = SliceAndCalcVideoStatistic(
...
VideoStatistic Stats::SliceAndCalcAggregatedVideoStatistic(
    size_t first_frame_num,
    size_t last_frame_num) {
  size_t num_spatial_layers = 0;
  size_t num_temporal_layers = 0;
...
size_t Stats::CalcLayerTargetBitrateKbps(size_t first_frame_num,
                                         ...
                                         bool aggregate) {
  std::vector<size_t> target_kbps(temporal_layer_idx + 1, 0);
  ...
  for (size_t frame_num = first_frame_num; frame_num <= last_frame_num;
       ++frame_num) {
    FrameStatistic superframe =
        aggregate ? AggregateFrameStatistic(frame_num, spatial_layer_idx)
                  : *GetFrame(frame_num, spatial_layer_idx);

    if (superframe.temporal_layer_idx <= temporal_layer_idx) {
      target_kbps[superframe.temporal_layer_idx] =
          std::max(target_kbps[superframe.temporal_layer_idx],
                   superframe.target_bitrate_kbps);
    }
  }

  return std::accumulate(target_kbps.begin(), target_kbps.end(), 0);
}

VideoStatistic Stats::SliceAndCalcVideoStatistic(size_t first_frame_num,
                                                 size_t last_frame_num,
                                                 size_t spatial_layer_idx,
                                                 size_t temporal_layer_idx,
                                                 bool aggregate) {
