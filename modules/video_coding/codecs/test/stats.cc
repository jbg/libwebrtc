...
FrameStatistics* Stats::AddFrame(size_t timestamp, size_t layer_idx) {
  RTC_DCHECK(frame_rtp_timestamp_[layer_idx].find(timestamp) ==
             frame_rtp_timestamp_[layer_idx].end());
  const size_t frame_num = layer_stats_[layer_idx].size();
  frame_rtp_timestamp_[layer_idx][timestamp] = frame_num;
...
FrameStatistics* Stats::GetFrameWithTimestamp(size_t timestamp,
  ...
  return GetFrame(frame_rtp_timestamp_[layer_idx][timestamp], layer_idx);
}
