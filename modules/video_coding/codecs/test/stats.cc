...
std::string VideoStatistic::ToString(std::string prefix) const {
  ...
  ss << "\n " << prefix << "encoding_speed_fps " << encoding_speed_fps;
  ss << "\n " << prefix << "decoding_speed_fps " << decoding_speed_fps;
  ss << "\n " << prefix << "avg_delay_sec " << avg_delay_sec;
  ss << "\n " << prefix << "max_key_frame_delay_sec "
     << max_key_frame_delay_sec;
  ss << "\n " << prefix << "max_delta_frame_delay_sec "
...
VideoStatistic Stats::SliceAndCalcVideoStatistic(
  ...
  for (size_t frame_num = first_frame_num; frame_num <= last_frame_num;
    ...
    if (frame_stat.encoding_successful > 0) {
