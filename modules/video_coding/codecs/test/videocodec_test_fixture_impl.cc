...
void ConfigureSimulcast(VideoCodec* codec_settings) {
  ...
  for (size_t i = 0; i < streams.size(); ++i) {
    ...
    if (streams[i].scalability_mode.has_value()) {
      ss->scalability_mode = *streams[i].scalability_mode;
    } else if (streams[i].num_temporal_layers.has_value()) {
      static const ScalabilityMode scalability_modes[3] = {
          ScalabilityMode::kL1T1, ScalabilityMode::kL1T2,
          ScalabilityMode::kL1T3};
      RTC_CHECK_GT(*streams[i].num_temporal_layers, 0);
      RTC_CHECK_LE(*streams[i].num_temporal_layers, 3);
      ss->scalability_mode =
          scalability_modes[*streams[i].num_temporal_layers - 1];
    } else {
      ss->scalability_mode = ScalabilityMode::kL1T1;
    }
...
void VideoCodecTestFixtureImpl::Config::SetCodecSettings(
    ...
    size_t width,
    size_t height) {
  // Must use scalability_mode to configure spatial layers.
  RTC_CHECK_EQ(num_spatial_layers, 1);
  ...
  SetCodecSettings(codec_name, num_simulcast_streams,
                   scalability_modes[num_temporal_layers - 1], denoising_on,
                   frame_dropper_on, spatial_resize_on, width, height);
}
...
size_t VideoCodecTestFixtureImpl::Config::NumberOfTemporalLayers() const {
  ...
  if (codec_settings.codecType == kVideoCodecVP9) {
    return std::max<int>(1, codec_settings.VP9().numberOfTemporalLayers);
  }
  return 1;
}
