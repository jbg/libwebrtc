...
TEST(TestConfig, TemporalLayersIndexForFrame_OneLayer) {
  const int kNumLayers = 1;
  TestConfig config;
  webrtc::test::CodecSettings(kVideoCodecVP8, &config.codec_settings);
  config.codec_settings.VP8()->numberOfTemporalLayers = kNumLayers;
  EXPECT_EQ(kNumLayers, config.NumberOfTemporalLayers());
...
TEST(TestConfig, TemporalLayersIndexForFrame_TwoLayers) {
...
TEST(TestConfig, TemporalLayersIndexForFrame_ThreeLayers) {
