...
TEST(VideoCodecTestLibvpx, HighBitrateVP9) {
  ...
  config.SetCodecSettings(cricket::kVp9CodecName, 1, ScalabilityMode::kL1T1,
...
TEST(VideoCodecTestLibvpx, DenoiserOnVP9) {
  auto config = CreateConfig();
  config.SetCodecSettings(cricket::kVp9CodecName, 1, ScalabilityMode::kL1T1,
                          true, true, false, kCifWidth, kCifHeight);
  config.num_frames = kNumFramesShort;
  const auto frame_checker = std::make_unique<QpFrameChecker>();
  config.encoded_frame_checker = frame_checker.get();
  auto fixture = CreateVideoCodecTestFixture(config);
...
TEST(VideoCodecTestLibvpx, VeryLowBitrateVP9) {
  auto config = CreateConfig();
  config.SetCodecSettings(cricket::kVp9CodecName, 1, ScalabilityMode::kL1T1,
                          false, true, true, kCifWidth, kCifHeight);
  const auto frame_checker = std::make_unique<QpFrameChecker>();
  config.encoded_frame_checker = frame_checker.get();
  auto fixture = CreateVideoCodecTestFixture(config);
...
TEST(VideoCodecTestLibvpx, MAYBE_TemporalLayersVP8) {
  auto config = CreateConfig();
  config.SetCodecSettings(cricket::kVp8CodecName, 1, ScalabilityMode::kL1T3,
                          true, true, false, kCifWidth, kCifHeight);
  const auto frame_checker = std::make_unique<QpFrameChecker>();
  config.encoded_frame_checker = frame_checker.get();
  auto fixture = CreateVideoCodecTestFixture(config);
...
TEST(VideoCodecTestLibvpx, MAYBE_MultiresVP8) {
  ...
  config.SetCodecSettings(cricket::kVp8CodecName, 3, ScalabilityMode::kL1T3,
                          true, true, false, 1280, 720);
  const auto frame_checker = std::make_unique<QpFrameChecker>();
  config.encoded_frame_checker = frame_checker.get();
  auto fixture = CreateVideoCodecTestFixture(config);
...
TEST(VideoCodecTestLibvpx, MAYBE_SvcVP9) {
  ...
  config.filepath = ResourcePath(config.filename, "yuv");
  config.num_frames = 100;
  config.SetCodecSettings(cricket::kVp9CodecName, 1, ScalabilityMode::kL3T3,
                          true, true, false, 1280, 720);
  const auto frame_checker = std::make_unique<QpFrameChecker>();
  config.encoded_frame_checker = frame_checker.get();
  auto fixture = CreateVideoCodecTestFixture(config);
