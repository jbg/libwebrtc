...
struct FrameStatistic {
  FrameStatistic(size_t frame_number, uint32_t rtp_timestamp)
...
class Stats {
 public:
  ...
  FrameStatistic* AddFrame(uint32_t timestamp);

  // Returns the FrameStatistic corresponding to |frame_number| or |timestamp|.
  FrameStatistic* GetFrame(size_t frame_number);
  FrameStatistic* GetFrameWithTimestamp(uint32_t timestamp);

  size_t size() const;

 private:
  std::vector<FrameStatistic> stats_;
  std::map<uint32_t, size_t> rtp_timestamp_to_frame_num_;
};
