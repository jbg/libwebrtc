...
class VideoProcessorIntegrationTestLibvpx
 ...
 protected:
  ...
  void PrintRdPerf(std::map<size_t, std::vector<VideoStatistic>> rd_stats) {
...
TEST_F(VideoProcessorIntegrationTestLibvpx, DISABLED_SimulcastVP8RdPerf) {
  ...
  std::map<size_t, std::vector<VideoStatistic>> rd_stats;
  for (const auto bitrate_kbps : kBitrateRdPerfKbps) {
    std::vector<RateProfile> rate_profiles = {
