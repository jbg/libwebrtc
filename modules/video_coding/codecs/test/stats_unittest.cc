...
TEST(StatsTest, AddFrame) {
  Stats stats;
  FrameStatistic* frame_stat = stats.AddFrame(kTimestamp, 0);
...
TEST(StatsTest, GetFrame) {
  ...
  FrameStatistic* frame_stat = stats.GetFrame(0u, 0);
...
TEST(StatsTest, AddFrames) {
  ...
  for (size_t i = 0; i < kNumFrames; ++i) {
    FrameStatistic* frame_stat = stats.AddFrame(kTimestamp + i, 0);
  ...
  // Get frame.
  size_t i = 22;
  FrameStatistic* frame_stat = stats.GetFrameWithTimestamp(kTimestamp + i, 0);
  EXPECT_EQ(i, frame_stat->frame_number);
  EXPECT_EQ(kTimestamp + i, frame_stat->rtp_timestamp);
}
...
TEST(StatsTest, AddFrameLayering) {
  ...
  for (size_t i = 0; i < 3; ++i) {
    ...
    FrameStatistic* frame_stat = stats.GetFrame(0u, i);
    EXPECT_EQ(0u, frame_stat->frame_number);
