...
VideoCodecTest::FakeEncodeCompleteCallback::OnEncodedImage(
  ...
  if (test_->encoded_frames_.size() ==
    ...
    test_->wait_for_encoded_frames_threshold_.reset();
    test_->encoded_frame_event_.Set();
  }
  return Result(Result::OK);
}
...
void VideoCodecTest::SetWaitForEncodedFramesThreshold(int num_frames) {
  rtc::CritScope lock(&encoded_frame_section_);
  wait_for_encoded_frames_threshold_ = rtc::Optional<int>(num_frames);
}
...
bool VideoCodecTest::WaitForEncodedFrames(
  ...
  EXPECT_TRUE(ret) << "Timed out while waiting for encoded frame.";
