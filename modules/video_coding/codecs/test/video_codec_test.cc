...
ABSL_FLAG(std::string, test_name, "VideoCodecTest", "Test name.");
...
TEST(VideoCodecTest, DISABLED_EncodeDecode) {
  ...
  for (int sidx = 0; sidx < num_spatial_layers; ++sidx) {
    for (int tidx = 0; tidx < num_temporal_layers; ++tidx) {
      ...
      stream.LogMetrics(
          GetGlobalMetricsLogger(), TestName(), metric_name_prefix,
          /*metadata=*/{{"test_name", absl::GetFlag(FLAGS_test_name)}});
    }
  }

  if (absl::GetFlag(FLAGS_write_csv)) {
    if (stats != nullptr) {
      rtc::StringBuilder csv_path;
      csv_path << TestOutputPath() << ".csv";
      stats->LogMetrics(csv_path.str(),
                        stats->Slice(Filter{}, /*merge=*/false), /*metadata=*/
                        {{"test_name", absl::GetFlag(FLAGS_test_name)}});
    }
  }
}
