...
void VideoProcessorIntegrationTest::ProcessAllFrames(
  ...
  while (frame_number < config_.num_frames) {
    if (frame_number ==
    ...
    if (frame_number + 1 < config_.num_frames) {
...
void VideoProcessorIntegrationTest::AnalyzeAllFrames(
    ...
    const std::vector<QualityThresholds>* quality_thresholds,
    const BitstreamThresholds* bs_thresholds) {
  const bool show_frame_by_frame_stats = false;
  const bool is_svc = config_.NumberOfSpatialLayers() > 1;
  const int number_of_simulcast_or_spatial_layers = std::max(
      1, std::max(config_.NumberOfSpatialLayers(),
                  static_cast<int>(
                      config_.codec_settings.numberOfSimulcastStreams)));
  const int number_of_temporal_layers = config_.NumberOfTemporalLayers();
  ...
  for (size_t rate_update_index = 0; rate_update_index < rate_profiles.size();
    ...
    const int number_of_frames = last_frame_number - first_frame_number + 1;
    ...
    if (show_frame_by_frame_stats) {
      PrintFrameByFrameStats(overall_stats);
    }

    for (int spatial_layer_number = 0;
         spatial_layer_number < number_of_simulcast_or_spatial_layers;
         ++spatial_layer_number) {
      for (int temporal_layer_number = 0;
        ...
        const float target_bitrate_kbps = layer_stats[0].target_bitrate_kbps;
        ...
        if (show_frame_by_frame_stats) {
          PrintFrameByFrameStats(layer_stats);
        }
      }
    }
  }
...
std::vector<FrameStatistic> VideoProcessorIntegrationTest::ExtractLayerStats(
    ...
    const int first_frame_number,
    const int last_frame_number,
    const bool combine_layers_stats) {
  size_t target_bitrate_kbps = 0;
  std::vector<FrameStatistic> layer_stats;
  for (int frame_number = first_frame_number; frame_number <= last_frame_number;
       ++frame_number) {
...
void VideoProcessorIntegrationTest::AnalyzeAndPrintStats(
    ...
    const BitstreamThresholds* bs_thresholds) {
  size_t num_input_frames = stats.size();
  ...
  for (size_t frame_idx = 0; frame_idx < stats.size(); ++frame_idx) {
    ...
    const float time_since_first_input_sec =
        frame_idx == 0
            ? 0.0
            : (frame_stat.rtp_timestamp - stats[0].rtp_timestamp) / 90000.0;
    const float time_since_last_input_sec =
        frame_idx == 0
            ? 0.0
            : (frame_stat.rtp_timestamp - stats[frame_idx - 1].rtp_timestamp) /
                  90000.0;
    ...
    total_encoded_bytes += frame_stat.encoded_frame_size_bytes;
    ...
    if (time_to_reach_target_bitrate_sec < 0) {
      const float curr_bitrate_kbps =
          (8.0 * total_encoded_bytes / 1000) / time_since_first_input_sec;
  ...
  const float encoded_bitrate_kbps =
      8 * total_encoded_bytes / input_duration_sec / 1000;
  const float bitrate_mismatch_percent =
...
void VideoProcessorIntegrationTest::PrintFrameByFrameStats(
    const std::vector<FrameStatistic>& stats) {
  for (const auto& frame_stat : stats) {
    printf(
        "frame %d %dx%d sl %zu tl %zu type %d length %zu qp %d psnr %f ssim "
        "%f enc_time_us %d dec_time_us %d rtp_ts %d bitrate_kbps %zu\n",
        frame_stat.frame_number, frame_stat.decoded_width,
        frame_stat.decoded_height, frame_stat.simulcast_svc_idx,
        frame_stat.temporal_layer_idx, frame_stat.frame_type,
        frame_stat.encoded_frame_size_bytes, frame_stat.qp, frame_stat.psnr,
        frame_stat.ssim, frame_stat.encode_time_us, frame_stat.decode_time_us,
        frame_stat.rtp_timestamp, frame_stat.target_bitrate_kbps);
  }
}
