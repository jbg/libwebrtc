...
void VideoProcessorIntegrationTest::AnalyzeAllFrames(
  ...
  for (size_t rate_update_idx = 0; rate_update_idx < rate_profiles.size();
    ...
    size_t highest_encoded_spatial_layer_idx = 0;
    size_t highest_encoded_temporal_layer_idx = 0;

    printf("\nRate update #%zu:", rate_update_idx);
    ...
    for (size_t spatial_layer_idx = 0;
      ...
      for (size_t temporal_layer_idx = 0;
        ...
        const VideoStatistic video_stat = stats_.SliceAndCalcVideoStatistic(
            first_frame_num, last_frame_num, spatial_layer_idx,
            temporal_layer_idx, rate_profiles[rate_update_idx].target_kbps,
            rate_profiles[rate_update_idx].input_fps, false);

        if (video_stat.length_bytes > 0) {
          printf("%s\n", video_stat.ToString("recv_").c_str());

          if (config_.print_frame_level_stats) {
            for (size_t frame_num = first_frame_num;
                 frame_num <= last_frame_num; ++frame_num) {
              FrameStatistic* frame_stat =
                  stats_.GetFrame(frame_num, spatial_layer_idx);
          ...
          highest_encoded_spatial_layer_idx =
              std::max(highest_encoded_spatial_layer_idx, spatial_layer_idx);
          highest_encoded_temporal_layer_idx =
              std::max(highest_encoded_temporal_layer_idx, temporal_layer_idx);
        }
      }
    }

    const VideoStatistic video_stat = stats_.SliceAndCalcVideoStatistic(
        first_frame_num, last_frame_num, highest_encoded_spatial_layer_idx,
        highest_encoded_temporal_layer_idx,
        rate_profiles[rate_update_idx].target_kbps,
        rate_profiles[rate_update_idx].input_fps, true);

    printf("%s\n", video_stat.ToString("send_").c_str());
    ...
    VerifyVideoStatistic(video_stat, rc_threshold, quality_threshold,
                         bs_thresholds,
                         rate_profiles[rate_update_idx].target_kbps,
...
void VideoProcessorIntegrationTest::VerifyVideoStatistic(
    const VideoStatistic& video_stat,
  ...
  if (rc_thresholds) {
    const float bitrate_mismatch_percent =
        100 * std::fabs(1.0f * video_stat.bitrate_kbps - target_kbps) /
        target_kbps;
    const float framerate_mismatch_percent =
        100 * std::fabs(video_stat.framerate_fps - input_fps) / input_fps;
