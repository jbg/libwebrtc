...
void VideoProcessorIntegrationTest::AnalyzeAllFrames(
  ...
  for (size_t rate_update_idx = 0; rate_update_idx < rate_profiles.size();
    ...
    const VideoStatistic video_stat = stats_.SliceAndCalcVideoStatistic(
        first_frame_num, last_frame_num, num_simulcast_or_spatial_layers - 1,
        num_temporal_layers - 1, rate_profiles[rate_update_idx].target_kbps,
        rate_profiles[rate_update_idx].input_fps, true);

    printf("Rate update #%zu:\n", rate_update_idx);
    printf("%s\n", video_stat.ToString("").c_str());
    ...
    for (size_t spatial_layer_idx = 0;
      ...
      for (size_t temporal_layer_idx = 0;
        ...
        if (video_stat.length_bytes > 0) {
          ...
          prefix << "sl" << spatial_layer_idx << "_tl" << temporal_layer_idx
                 << "_";
          printf("%s\n", video_stat.ToString(prefix.str()).c_str());
...
void VideoProcessorIntegrationTest::PrintSettings(
  ...
  printf(" Total # of frames    : %d", source_frame_reader_->NumberOfFrames());
