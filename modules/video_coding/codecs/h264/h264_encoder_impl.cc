...
int32_t H264EncoderImpl::InitEncode(const VideoCodec* inst,
  ...
  for (int i = 0, idx = number_of_streams - 1; i < number_of_streams;
       ++i, --idx) {
    // Temporal layers with simulcast still not supported.
    ...
    configurations_[i].sending_ = false;
    configurations_[i].width_ = inst->simulcastStream[idx].width;
    configurations_[i].height_ = inst->simulcastStream[idx].height;
    configurations_[i].max_frame_rate_ = static_cast<float>(inst->maxFramerate);
    configurations_[i].frame_dropping_on_ = inst->H264().frameDroppingOn;
    configurations_[i].key_frame_interval_ = inst->H264().keyFrameInterval;

    // Create downscaled image buffers.
    if (i) {
      downscaled_buffers_[i - 1] = I420Buffer::Create(
          configurations_[i].width_, configurations_[i].height_,
          configurations_[i].width_, configurations_[i].width_ / 2,
          configurations_[i].width_ / 2);
    }

    // Codec_settings uses kbits/second; encoder uses bits/second.
    configurations_[i].max_bps_ = inst->maxBitrate * 1000;
    if (inst->targetBitrate == 0)
      configurations_[i].target_bps_ = inst->startBitrate * 1000;
    else
      configurations_[i].target_bps_ = inst->targetBitrate * 1000;
    ...
    encoded_images_[i]._size =
        CalcBufferSize(VideoType::kI420, inst->simulcastStream[idx].width,
                       inst->simulcastStream[idx].height);
    encoded_images_[i]._buffer = new uint8_t[encoded_images_[i]._size];
    encoded_image_buffers_[i].reset(encoded_images_[i]._buffer);
    encoded_images_[i]._completeFrame = true;
    encoded_images_[i]._encodedWidth = inst->simulcastStream[idx].width;
    encoded_images_[i]._encodedHeight = inst->simulcastStream[idx].height;
  ...
  return SetRateAllocation(allocation, inst->maxFramerate);
}
...
int32_t H264EncoderImpl::SetRateAllocation(const BitrateAllocation& bitrate,
  ...
  for (size_t i = 0; i < encoders_.size(); ++i, --stream_idx) {
    // Update layer config.
    configurations_[i].target_bps_ = bitrate.GetSpatialLayerSum(stream_idx);
    configurations_[i].max_frame_rate_ = static_cast<float>(new_framerate);

    if (configurations_[i].target_bps_) {
...
int32_t H264EncoderImpl::Encode(const VideoFrame& input_frame,
  ...
  for (size_t i = 0; i < configurations_.size(); ++i) {
    if (configurations_[i].key_frame_request_ && configurations_[i].sending_) {
      send_key_frame = true;
      break;
    }
  }
  if (!send_key_frame && frame_types) {
    for (size_t i = 0; i < frame_types->size() && i < configurations_.size();
         ++i) {
      if ((*frame_types)[i] == kVideoFrameKey && configurations_[i].sending_) {
        send_key_frame = true;
        break;
      }
    }
  ...
  RTC_DCHECK_EQ(configurations_[0].width_, frame_buffer->width());
  RTC_DCHECK_EQ(configurations_[0].height_, frame_buffer->height());

  // Encode image for each layer.
  for (size_t i = 0; i < encoders_.size(); ++i) {
    // EncodeFrame input.
    pictures_[i] = {0};
    pictures_[i].iPicWidth = configurations_[i].width_;
    pictures_[i].iPicHeight = configurations_[i].height_;
    ...
    if (i == 0) {
      ...
      libyuv::I420Scale(pictures_[i - 1].pData[0], pictures_[i - 1].iStride[0],
                        ...
                        configurations_[i - 1].width_,
                        configurations_[i - 1].height_, pictures_[i].pData[0],
                        pictures_[i].iStride[0], pictures_[i].pData[1],
                        pictures_[i].iStride[1], pictures_[i].pData[2],
                        pictures_[i].iStride[2], configurations_[i].width_,
                        configurations_[i].height_, libyuv::kFilterBilinear);
    }

    if (!configurations_[i].sending_) {
      continue;
    }
    ...
    if (send_key_frame) {
      ...
      configurations_[i].key_frame_request_ = false;
    }
    // EncodeFrame output.
    SFrameBSInfo info;
    ...
    encoded_images_[i]._encodedWidth = configurations_[i].width_;
    encoded_images_[i]._encodedHeight = configurations_[i].height_;
    encoded_images_[i]._timeStamp = input_frame.timestamp();
    encoded_images_[i].ntp_time_ms_ = input_frame.ntp_time_ms();
    encoded_images_[i].capture_time_ms_ = input_frame.render_time_ms();
    encoded_images_[i].rotation_ = input_frame.rotation();
    encoded_images_[i].content_type_ = (codec_.mode == kScreensharing)
                                           ? VideoContentType::SCREENSHARE
                                           : VideoContentType::UNSPECIFIED;
    encoded_images_[i].timing_.flags = TimingFrameFlags::kInvalid;
...
SEncParamExt H264EncoderImpl::CreateEncoderParams(size_t i) const {
  ...
  encoder_params.iPicWidth = configurations_[i].width_;
  encoder_params.iPicHeight = configurations_[i].height_;
  encoder_params.iTargetBitrate = configurations_[i].target_bps_;
  encoder_params.iMaxBitrate = configurations_[i].max_bps_;
  // Rate Control mode
  encoder_params.iRCMode = RC_BITRATE_MODE;
  encoder_params.fMaxFrameRate = configurations_[i].max_frame_rate_;

  // The following parameters are extension parameters (they're in SEncParamExt,
  // not in SEncParamBase).
  encoder_params.bEnableFrameSkip = configurations_[i].frame_dropping_on_;
  // |uiIntraPeriod|    - multiple of GOP size
  // |keyFrameInterval| - number of frames
  encoder_params.uiIntraPeriod = configurations_[i].key_frame_interval_;
...
void H264EncoderImpl::LayerConfig::SetStreamState(bool send_stream) {
  if (send_stream && !sending_) {
    // Need a key frame if we have not sent this stream before.
    key_frame_request_ = true;
  }
  sending_ = send_stream;
}
