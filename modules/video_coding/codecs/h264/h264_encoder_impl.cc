...
int32_t H264EncoderImpl::InitEncode(const VideoCodec* inst,
  ...
  for (int i = 0, idx = number_of_streams - 1; i < number_of_streams;
    ...
    configurations_[i].width_ = codec_.simulcastStream[idx].width;
    configurations_[i].height_ = codec_.simulcastStream[idx].height;
    configurations_[i].max_frame_rate_ =
        static_cast<float>(codec_.maxFramerate);
    configurations_[i].frame_dropping_on_ = codec_.H264()->frameDroppingOn;
    configurations_[i].key_frame_interval_ = codec_.H264()->keyFrameInterval;
    ...
    configurations_[i].max_bps_ = codec_.maxBitrate * 1000;
    if (codec_.targetBitrate == 0)
      configurations_[i].target_bps_ = codec_.startBitrate * 1000;
    else
      configurations_[i].target_bps_ = codec_.targetBitrate * 1000;
