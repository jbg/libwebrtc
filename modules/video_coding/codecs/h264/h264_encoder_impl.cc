...
// Copies the encoded bytes from |info| to |encoded_image|.
// The |encoded_image->_buffer| may be deleted and reallocated if a bigger
// buffer is required.
//
...
static void RtpFragmentize(EncodedImage* encoded_image, SFrameBSInfo* info) {
  ...
  // Iterate layers and NAL units and copy the data to |encoded_image->_buffer|.
  const uint8_t start_code[4] = {0, 0, 0, 1};
  encoded_image->set_size(0);
  for (int layer = 0; layer < info->iLayerNum; ++layer) {
    const SLayerBSInfo& layerInfo = info->sLayerInfo[layer];
    // Iterate NAL units making up this layer.
    size_t layer_len = 0;
