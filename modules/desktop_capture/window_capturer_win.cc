...
void OwnedWindowCapturerCallback::OnCaptureResult(
  ...
  if (result != DesktopCapturer::Result::SUCCESS) {
    ...
    RTC_LOG(LS_INFO) << "Capturing owned window failed (previous error/warning "
                        "pertained to that)";
    return;
  }
...
class WindowCapturerWin : public DesktopCapturer {
 public:
  ...
  void StartOrRestart(Callback* callback);
...
void WindowCapturerWin::Start(Callback* callback) {
  assert(!callback_);
  StartOrRestart(callback);
}

void WindowCapturerWin::StartOrRestart(Callback* callback) {
  assert(callback);
      ...
      if (!owned_windows_.empty()) {
        ...
        for (auto it = owned_windows_.rbegin(); it != owned_windows_.rend();
          ...
          if (owned_window_capturer_->SelectSource(
            ...
            owned_window_capturer_->StartOrRestart(&callback);
