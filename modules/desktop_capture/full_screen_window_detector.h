...
class FullScreenWindowDetector : public rtc::RefCountedBase {
 ...
 protected:
  ...
  // Will be called no often than twice per second, so no need to have
  // throttling inside implementation.
  // Returns true in case if window list has been updated sucessfully.
  ...
  // - to return FullScreenApplicationHandler specific for application (Google
