...
bool DxgiDuplicatorController::EnsureFrameCaptured(Context* context,
  ...
  if (GetNumFramesCaptured() == 0 && !IsConsoleSession()) {
    // When capturing a local session, waiting for a single frame is sufficient
    // to ensure that DXGI output duplication is working. When the session is
    // not attached to the console, it has been observed that DXGI may produce
    // up to 4 frames (typically 1-2 though) before stopping. When this
    // condition occurs, no errors are returned from the output duplication API,
    // it simply appears that nothing is changing on the screen. Thus for
