...
std::unique_ptr<DesktopFrame> WgcCaptureSession::GetFrame(bool* result) {
  ...
  if (allow_zero_hertz()) {
    // Update the updated region of the current frame depending on if the
    // current frame is different (contains modified pixel values) or not.
    ...
    if (previous_frame) {
      ...
      if (current_frame_size == previous_frame_size) {
        frames_are_equal = !memcmp(current_frame->data(),
                                   previous_frame->data(), current_frame_size);
      }
      ...
      if (frames_are_equal) {
        ...
        // Mark the complete screen/window as a damaged region. The frame will
        // be forwareded to the client since it contains new content.
