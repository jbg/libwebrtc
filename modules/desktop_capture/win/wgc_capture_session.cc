...
HRESULT WgcCaptureSession::GetFrame(
  ...
  int smaller_height = std::min(previous_size_.Height, new_size.Height);
  int smaller_width = std::min(previous_size_.Width, new_size.Width);
  int row_data_length = smaller_width * DesktopFrame::kBytesPerPixel;
  ...
  std::unique_ptr<uint8_t[]> image_data =
      std::make_unique<uint8_t[]>(smaller_height * row_data_length);
  uint8_t* image_data_ptr = image_data.get();
  for (auto i = 0; i < smaller_height; i++) {
  ...
  DesktopSize size(smaller_width, smaller_height);
