...
const int kWindowXCoord = 50;
const int kWindowYCoord = 75;
...
TEST_F(WgcCaptureSourceTest, WindowPosition) {
  ...
  MoveTestWindow(window_info_.hwnd, kWindowXCoord, kWindowYCoord);
  DesktopVector source_vector = source_->GetTopLeft();
  EXPECT_EQ(source_vector.x(), kWindowXCoord);
  EXPECT_EQ(source_vector.y(), kWindowYCoord);
}
