...
std::wstring GetPathByWindowId(HWND window_id) {
  ...
  if (status >= 0 && pbi.PebBaseAddress) {
    ...
    if (process_reader.Read(pbi.PebBaseAddress, &peb, sizeof(peb)) &&
      ...
      std::vector<wchar_t> buffer(peb_upp.CommandLine.Length + 1, 0);
      if (process_reader.Read(peb_upp.CommandLine.Buffer, buffer.data(),
                              peb_upp.CommandLine.Length)) {
        int num_args = 0;
        LPWSTR* argv_arr = ::CommandLineToArgvW(buffer.data(), &num_args);
