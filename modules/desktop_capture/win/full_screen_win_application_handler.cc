...
std::wstring wstring_trim(const std::wstring& s) {
  ...
  const std::wstring::size_type last = s.find_first_not_of(kWhitespace);
...
DesktopCapturer::SourceList GetProcessWindows(
  ...
  std::copy_if(sources.begin(), sources.end(), std::back_inserter(result),
               [process](DesktopCapturer::Source source) {
                 return WindowProcess(reinterpret_cast<HWND>(source.id)) ==
                        process;
               });
  return result;
}

class FullScreenWinApplicationHandler : public FullScreenApplicationHandler {
 public:
...
class FullScreenPowerPointHandler : public FullScreenWinApplicationHandler {
 public:
  DesktopCapturer::SourceId FindFullScreenWindow(
      const DesktopCapturer::SourceList& previous,
    ...
    DesktopCapturer::SourceList pp_windows =
        GetProcessWindows(current, process);
