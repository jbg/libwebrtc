...
void LogDesktopCapturerUsage(DesktopCapturerUsage usage) {
  constexpr int bucket_count =
      static_cast<int>(DesktopCapturerUsage::kMaxValue) + 1;
  RTC_HISTOGRAM_ENUMERATION("WebRTC.Screenshare.DesktopCapturerUsage",
                            static_cast<int>(usage), bucket_count);
}
