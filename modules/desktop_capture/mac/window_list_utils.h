...
// The bounds of the window with |id| is in DIP coordinates and |rect| is the
// CGImage size of the window with |id| in physical coordinates. Comparing them
// can give the current scale factor.
// If |rect| overlaps multiple monitors, OS will decide on which monitor the
// window is displayed and use its scale factor to the |rect|. So this method
// still works.
float GetCurrentScaleFactorOfWindow(CGWindowID id,
                                    DesktopRect rect);
