...
class FullScreenMacApplicationHandler : public FullScreenApplicationHandler {
 public:
  ...
  WindowId FindFullScreenWindowWithSamePid(const DesktopCapturer::SourceList& source_list) const {
    const WindowId original_window = GetWindowId();
  ...
  DesktopCapturer::SourceId FindFullScreenWindow(
    ...
    return IsWindowOnScreen(GetWindowId()) ? 0 : FindFullScreenWindowWithSamePid(source_list);
  }
...
std::unique_ptr<FullScreenApplicationHandler> CreateFullScreenMacApplicationHandler(
  ...
  if (path_length > 0) {
    ...
    if (name.find("Google Chrome") == 0 || name == "Chromium") {
      ...
      result.reset(new FullScreenMacApplicationHandler(slide_show_title_predicate));
    } else if (name == "Keynote") {
      result.reset(new FullScreenMacApplicationHandler(equal_title_predicate));
    }
  }

  if (result) result->SetSourceId(sourceId);
