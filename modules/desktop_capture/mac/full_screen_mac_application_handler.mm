...
class FullScreenMacApplicationHandler : public FullScreenApplicationHandler {
 public:
  ...
  virtual bool TitlePredicate(const std::string& original_title, const std::string& title) const {
  ...
  WindowId FindFullScreenWindowWithSamePid() const {
    ...
    for (CFIndex i = 0; i < count; ++i) {
      ...
      if (!TitlePredicate(title, window_title)) continue;
...
class PowerPointFullScreenApplicationHandler : public FullScreenMacApplicationHandler {
 public:
  bool TitlePredicate(const std::string& original_title, const std::string& title) const override {
...
std::unique_ptr<FullScreenApplicationHandler> CreateFullScreenMacApplicationHandler(
  ...
  if (path_length > 0) {
    ...
    if (name.find("Google Chrome") == 0 || name == "Chromium") {
      result.reset(new FullScreenMacApplicationHandler());
    } else if (name == "Microsoft PowerPoint") {
      result.reset(new PowerPointFullScreenApplicationHandler());
