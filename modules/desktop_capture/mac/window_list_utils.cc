...
float GetCurrentScaleFactorOfWindow(CGWindowID id,
  ...
  if (!window_bounds.is_empty() && !rect.is_empty()) {
    float scale_x = rect.width() / window_bounds.width();
    float scale_y = rect.height() / window_bounds.height();
    // Currently the scale in X and Y directions must be same.
    if (scale_x == scale_y && scale_x > 0) {
      scale = scale_x;
    }
  }
