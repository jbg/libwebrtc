...
DesktopFrameProvider::~DesktopFrameProvider() {
  ...
  InvalidateAllIOSurfaces();
}

std::unique_ptr<SharedDesktopFrame> DesktopFrameProvider::LatestFrameForDisplay(
    CGDirectDisplayID display_id) {
  if (!allow_iosurface_) {
    // Regenerate a snapshot.
    return SharedDesktopFrame::Wrap(DesktopFrameCGImage::CreateForDisplay(display_id));
  }
...
void DesktopFrameProvider::InvalidateIOSurface(CGDirectDisplayID display_id,
  ...
  WriteLockScoped scoped_io_surfaces_lock(*io_surfaces_lock_);
  io_surfaces_[display_id] = SharedDesktopFrame::Wrap(std::move(desktop_frame_iosurface));
}

void DesktopFrameProvider::InvalidateAllIOSurfaces() {
  if (!allow_iosurface_) {
    return;
  }
