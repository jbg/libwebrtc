...
void ScreenCapturerMacBase::SetExcludedWindow(WindowId window) {
...
void ScreenCapturerMacBase::CopyDisplayRegionToFrame(DesktopRect display_bounds,
  ...
  RTC_CHECK(display_base_address);
...
bool ScreenCapturerMacBase::Blit(const DesktopFrame& frame, const DesktopRegion& region) {
...
bool ScreenCapturerMacBase::RegisterRefreshAndMoveHandlers() {
  ...
  for (const auto& config : desktop_config_.displays) {
    ...
    CGDisplayStreamFrameAvailableHandler handler = ^(CGDisplayStreamFrameStatus status,
      ...
      DisplayRefresh(display_id, display_time, frame_surface);
    };
