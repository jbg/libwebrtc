...
class OpenOfficeApplicationHandler : public FullScreenMacApplicationHandler {
 public:
  ...
  DesktopCapturer::SourceId FindFullScreenWindow(
    ...
    if (it == cache_sources_.end()) {
      return kCGNullWindowID;
    }

    // If slide show window is followed by another window
    // then it could be either editor window or presenter window
    // related to the same document, both of them should have not
    // empty title equal to original window title.
    auto next = it + 1;
    if (next != cache_sources_.end() &&
        (next->id == original_window || next->title == original_title)) {
      return it->id;
    } else {
      return kCGNullWindowID;
    }
