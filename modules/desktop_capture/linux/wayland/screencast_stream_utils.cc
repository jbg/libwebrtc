...
PipeWireVersion PipeWireVersion::Parse(const std::string& version) {
...
bool PipeWireVersion::operator>=(const PipeWireVersion& required_pw_version) {
  if (!major && !minor && micro) {
    return false;
  }

  return std::tie(major, minor, micro) >= std::tie(required_pw_version.major,
                                                   required_pw_version.minor,
