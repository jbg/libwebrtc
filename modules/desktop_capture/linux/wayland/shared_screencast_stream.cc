...
bool SharedScreenCastStreamPrivate::ProcessMemFDBuffer(
  ...
  // DesktopRect frame_rect = DesktopRect::MakeWH(frame.size().width(),
  // frame.size().height());
  ...
  if (effectively_new_frame || !use_damage_region_ ||
    ...
    for (DesktopRegion::Iterator it(damage_region_); !it.IsAtEnd();
      ...
      // const int rect_right = UpToMultiple(rect.right(), grid_size,
      // grid_size_mask); const int rect_top = DownToMultiple(rect.top(),
      // grid_size_mask); const int rect_bottom = UpToMultiple(rect.bottom(),
      // grid_size, grid_size_mask); DesktopRect new_rect =
      ...
      const int rect_width =
          std::min(rect.width() + 2 * offset, frame.size().width() - rect_left);
      const int rect_height = std::min(rect.height() + 2 * offset,
                                       frame.size().height() - rect_top);
      frame.CopyPixelsFrom(
          src + rect_top * stride + rect_left * DesktopFrame::kBytesPerPixel,
          stride,
          DesktopRect::MakeXYWH(rect_left, rect_top, rect_width, rect_height));
