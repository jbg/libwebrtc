...
void SharedScreenCastStreamPrivate::ProcessBuffer(pw_buffer* buffer) {
  TRACE_EVENT0("WEBRTC_STREAM", "ProcessBuffer");
...
bool SharedScreenCastStreamPrivate::ProcessMemFDBuffer(
  ...
  if (effectively_new_frame || !use_damage_region_ ||
    ...
    for (DesktopRegion::Iterator it(damage_region_); !it.IsAtEnd();
      ...
      const int rect_right =
          UpToMultiple(rect.right(), grid_size, grid_size_mask);
      const int rect_top = DownToMultiple(rect.top(), grid_size_mask);
      const int rect_bottom =
          UpToMultiple(rect.bottom(), grid_size, grid_size_mask);
      DesktopRect new_rect =
          DesktopRect::MakeLTRB(rect_left, rect_top, rect_right, rect_bottom);
      new_rect.IntersectWith(frame_rect);
      frame.CopyPixelsFrom(
          src + rect_top * stride + rect_left * DesktopFrame::kBytesPerPixel,
          stride, new_rect);

      // Rather than copying the exact regions reported by mutter, this may copy
      // a bigger chunk in an attempt to eliminate the artifacts on the screen.
      // constexpr int offset = 0;
      // const int rect_top = std::max(rect.top() - offset, 0);
      // const int rect_left = std::max(rect.left() - offset, 0);
      // const int rect_width =
      //     std::min(rect.width() + 2 * offset, frame.size().width() -
      //     rect_left);
      // const int rect_height = std::min(rect.height() + 2 * offset,
      //                                  frame.size().height() - rect_top);
      // frame.CopyPixelsFrom(
      //     src + rect_top * stride + rect_left * DesktopFrame::kBytesPerPixel,
      //     stride,
      //     DesktopRect::MakeXYWH(rect_left, rect_top, rect_width,
      //     rect_height));

      // frame.CopyPixelsFrom(
      //   src + rect.top() * stride + rect.left() *
      //   DesktopFrame::kBytesPerPixel, stride,
      //   DesktopRect::MakeXYWH(rect.left(), rect.top(), rect.width(),
      //                                 rect.height()));
    }
  }
