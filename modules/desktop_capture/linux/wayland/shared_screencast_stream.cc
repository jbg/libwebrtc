...
const pw_version kDmaBufMinVersion = {0, 3, 24};
const pw_version kDmaBufModifierMinVersion = {0, 3, 33};
...
bool PipeWireVersionIsAtLeast(pw_version current_version,
                              pw_version required_version) {
...
class SharedScreenCastStreamPrivate {
 ...
 private:
  ...
  webrtc::Mutex proccess_frame_lock_;
  ScreenCaptureFrameQueue<SharedDesktopFrame> queue_;
  std::unique_ptr<MouseCursor> mouse_cursor_;
  DesktopVector mouse_cursor_position_;
...
std::unique_ptr<DesktopFrame> SharedScreenCastStreamPrivate::CaptureFrame() {
  webrtc::MutexLock lock(&proccess_frame_lock_);
...
void SharedScreenCastStreamPrivate::ProcessBuffer(pw_buffer* buffer) {
  ...
  if (spa_buffer->datas[0].chunk->size == 0) {
    RTC_LOG(LS_INFO) << "Got empty buffer. The buffer possibly carried only "
...
DesktopVector SharedScreenCastStream::CaptureCursorPosition() {
  return d->CaptureCursorPosition();
}
