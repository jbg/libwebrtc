...
void SharedScreenCastStreamPrivate::ProcessBuffer(pw_buffer* buffer) {
  ...
  // handle screen/window sharing differently: 1) KDE (KWin) KWin doesn't use
  ...
  // DesktopFrame using the size of the stream itself. 2) GNOME (Mutter) Mutter
  ...
  // Use SPA_META_VideoCrop metadata to get the DesktopFrame size only when
  // the size is smaller then the size of the stream. GNOME (Mutter) always
  // sets these metadata, even when sharing a screen, and in this case the
  // size will be equal to the size of the stream, therefore there is no need
  // to use these metadata in this case.
