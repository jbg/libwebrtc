...
void BaseCapturerPipeWire::OnScreenCastRequestResult(RequestResponse result,
  ...
  } else if (is_screencast_portal_) {
    ScreenCastPortal* screencast_portal =
        static_cast<ScreenCastPortal*>(portal_.get());
    if (screencast_portal && !screencast_portal->RestoreToken().empty()) {
...
void BaseCapturerPipeWire::Start(Callback* callback) {
  ...
  if (is_screencast_portal_) {
    ScreenCastPortal* screencast_portal =
        static_cast<ScreenCastPortal*>(portal_.get());
    if (screencast_portal) {
      if (selected_source_id_) {
        ...
        RestoreTokenManager::GetInstance().RemoveToken(selected_source_id_);
      } else {
