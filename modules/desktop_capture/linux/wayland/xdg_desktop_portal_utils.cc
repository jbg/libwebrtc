...
void SetupSessionRequestHandlers(
  ...
  variant_string =
      g_strdup_printf("%s_session%d", std::string(portal_prefix).c_str(),
                      g_random_int_range(0, G_MAXINT));
  g_variant_builder_add(&builder, "{sv}", "session_handle_token",
                        g_variant_new_string(variant_string.get()));

  variant_string = g_strdup_printf("%s_%d", std::string(portal_prefix).c_str(),
                                   g_random_int_range(0, G_MAXINT));
...
void StartSessionRequest(
  ...
  g_variant_builder_init(&builder, G_VARIANT_TYPE_VARDICT);
  variant_string = g_strdup_printf("%s%d", std::string(prefix).c_str(),
                                   g_random_int_range(0, G_MAXINT));
  g_variant_builder_add(&builder, "{sv}", "handle_token",
                        g_variant_new_string(variant_string.get()));
