...
static const char* getRenderNode() {
  ...
  for (int i = 0; i < ret; i++) {
    ...
    render_node = strndup(device->nodes[DRM_NODE_RENDER], 256);
    break;
  }
...
void BaseCapturerPipeWire::InitEGL() {
  ...
  const char* render_node = getRenderNode();
  if (!render_node) {
    return;
  }

  drm_fd_ = open(render_node, O_RDWR);
...
void BaseCapturerPipeWire::HandleBuffer(pw_buffer* buffer) {
  ...
  if (spaBuffer->datas[0].type == pw_core_type_->data.MemFd ||
    ...
    gbm_bo* imported =
        gbm_bo_import(gbm_device_, GBM_BO_IMPORT_FD, &importInfo, 0);
    if (!imported) {
      RTC_LOG(LS_ERROR)
