...
class LogWriter {
 public:
  LogWriter(LogWriterImplManagerInterface* manager, std::string filename)
      : manager_(manager), impl_(manager_->Create(filename)) {}
  LogWriter(const LogWriter&) = delete;
  LogWriter& operator=(const LogWriter&) = delete;
  ~LogWriter() { manager_->Destroy(impl_); }
 ...
 private:
  LogWriterImplManagerInterface* const manager_;
  LogWriterImplInterface* const impl_;
};
...
class LogWriterFactory {
 ...
 private:
  const std::unique_ptr<LogWriterImplManagerInterface> impl_;
};
}  // namespace webrtc
