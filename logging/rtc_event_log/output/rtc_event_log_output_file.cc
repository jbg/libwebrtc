...
FileWrapper FileWrapperFromPlatformFile(rtc::PlatformFile platform_file) {
  FILE* f = (platform_file == rtc::kInvalidPlatformFileValue)
                ? nullptr
                : rtc::FdopenPlatformFileForWriting(platform_file);
  if (f) {
...
RtcEventLogOutputFile::RtcEventLogOutputFile(FILE* file)
    : RtcEventLogOutputFile(FileWrapper(file), RtcEventLog::kUnlimitedOutput) {}
...
RtcEventLogOutputFile::~RtcEventLogOutputFile() = default;
