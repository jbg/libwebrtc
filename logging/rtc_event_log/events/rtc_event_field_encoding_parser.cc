...
uint64_t EventParser::ReadVarInt() {
  uint64_t output = 0;
  bool success;
  std::tie(success, s_) = DecodeVarInt(s_, &output);
  if (!success) {
    SetError();
  }
  return output;
...
uint64_t EventParser::ReadOptionalValuePositions(std::vector<bool>* positions) {
  ...
  if (s_.size() * 8 < bits_to_read) {
    SetError();
    return 0;
  }

  BitstreamReader reader(s_);
...
void EventParser::ReadDeltasAndPopulateValues(
  ...
  if (s_.size() * 8 < num_deltas * params.delta_width_bits()) {
    SetError();
    return;
  }
  ...
  for (uint64_t i = 0; i < num_deltas; ++i) {
    uint64_t delta = reader.ReadBits(params.delta_width_bits());
  ...
  s_ = s_.substr((num_deltas * params.delta_width_bits() + 7) / 8);
}
