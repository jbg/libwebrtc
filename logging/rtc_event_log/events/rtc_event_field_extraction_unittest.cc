...
TEST(UnsignedBitWidthTest, WidthOfZeroValue) {
  EXPECT_EQ(webrtc_event_logging::UnsignedBitWidth(0, true), 0u);
  EXPECT_EQ(webrtc_event_logging::UnsignedBitWidth(0, false), 1u);
  EXPECT_EQ(webrtc_event_logging::UnsignedBitWidth(1, true), 1u);
  EXPECT_EQ(webrtc_event_logging::UnsignedBitWidth(1, false), 1u);
}
...
TEST(ConvertToUnsignedTest, NegativeValues) {
  // Negative values are converted as if cast to unsigned type of
  // the same bitsize using 2-complement representation.
  int16_t x = -1;
  EXPECT_EQ(ConvertToUnsignedIfSigned(x), static_cast<uint64_t>(0xFFFF));
  int64_t y = -1;
  EXPECT_EQ(ConvertToUnsignedIfSigned(y),
            static_cast<uint64_t>(0xFFFFFFFFFFFFFFFFull));
}

TEST(ConvertToUnsignedTest, PositiveValues) {
  // Postive values are unchanged.
  int16_t x = 42;
  EXPECT_EQ(ConvertToUnsignedIfSigned(x), static_cast<uint64_t>(42));
  int64_t y = 42;
  EXPECT_EQ(ConvertToUnsignedIfSigned(y), static_cast<uint64_t>(42));
}
