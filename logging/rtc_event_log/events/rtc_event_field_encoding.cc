...
std::string EncodeSingleValue(uint64_t value, FieldType field_type) {
  switch (field_type) {
    case FieldType::kFixed8:
      return webrtc_event_logging::SerializeLittleEndian(value, /*bytes=*/1);
    case FieldType::kFixed32:
      return webrtc_event_logging::SerializeLittleEndian(value, /*bytes=*/4);
    case FieldType::kFixed64:
      return webrtc_event_logging::SerializeLittleEndian(value, /*bytes=*/8);
...
FixedLengthEncodingParametersV3::CalculateParameters(
  ...
  const uint8_t delta_width_bits_unsigned =
      webrtc_event_logging::UnsignedBitWidth(max_unsigned_delta);
  const uint8_t delta_width_bits_signed = webrtc_event_logging::SignedBitWidth(
  ...
  const uint8_t delta_width_bits =
...
FixedLengthEncodingParametersV3::ParseDeltaHeader(uint64_t header,
                                                  uint8_t value_width_bits) {
  uint8_t delta_width_bits = (header & ((1u << 6) - 1)) + 1;
...
void EventEncoder::EncodeField(const FieldParameters& params,
  ...
  return EncodeField(params, values.values, &values.positions);
}
