...
// TODO(terelius): Compared to generic 'Status' classes, this
...
class EventParser {
 public:
  ...
  // Returns RtcEventLogParseStatus::Success and clears `values` (and
  // `positions`) if the field doesn't exist. Returns a ParseStatus::Error if
  // the log is incomplete, malformed or otherwise can't be parsed. `values` and
  // `positions` are pure out-parameters that allow the caller to reuse the same
  // temporary storage for all fields. Any previous content in the out
  // parameter(s) is cleared.
