...
bool FixedLengthDeltaEncoder::EncodeHeader(BitWriter* writer) {
  ...
  // TODO: !!! Encoding type...?
  return writer->WriteBits(static_cast<uint64_t>(
                               EncodingType::kFixedSizeWithAllOptionalFields),
                           kBitsInHeaderForEncodingType) &&
         writer->WriteBits(original_width_bits_ - 1,
                           kBitsInHeaderForOriginalWidthBits) &&
         writer->WriteBits(delta_width_bits_ - 1,
...
bool FixedLengthDeltaDecoder::IsSuitableDecoderFor(const std::string& input) {
  ...
  return static_cast<EncodingType>(encoding_type_bits) ==
         EncodingType::kFixedSizeWithOnlyMandatoryFields;
}
...
bool FixedLengthDeltaDecoder::GetDelta(uint64_t* delta) {
  ...
  const size_t lower_bit_count = std::max<uint64_t>(delta_width_bits_, 32u);
