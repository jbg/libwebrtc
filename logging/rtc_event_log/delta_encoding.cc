...
// TODO(eladalon): Make the decoder build for tools and unit tests,
// but not for "standard" WebRTC.
...
void FixedLengthDeltaEncoder::EncodeHeader() {
  RTC_DCHECK(writer_);
  if (original_width_bits_ == kDefaultOriginalWidthBits &&
      signed_deltas_ == kDefaultSignedDeltas &&
      values_optional_ == kDefaultValuesOptional) {
...
void FixedLengthDeltaEncoder::EncodeHeaderWithOnlyMandatoryFields() {
  ...
  RTC_DCHECK_EQ(original_width_bits_, kDefaultOriginalWidthBits);
  RTC_DCHECK_EQ(signed_deltas_, kDefaultSignedDeltas);
  RTC_DCHECK_EQ(values_optional_, kDefaultValuesOptional);
}
...
class FixedLengthDeltaDecoder final {
 ...
 private:
  ...
  // Number of bits necessary to hold the largest value in the sequence of
  // values that |this| will be used to decode.
  const uint64_t original_width_bits_;
...
bool FixedLengthDeltaDecoder::GetDelta(uint64_t* delta) {
  RTC_DCHECK(reader_);
  RTC_DCHECK(!signed_deltas_) << "Not implemented.";    // Reminder.
  RTC_DCHECK(!values_optional_) << "Not implemented.";  // Reminder.
...
uint64_t FixedLengthDeltaDecoder::ApplyDelta(uint64_t base,
  ...
  if (original_width_bits_ < 64) {  // Naturally wraps around otherwise.
    result %= (static_cast<uint64_t>(1) << original_width_bits_);
  }
  return result;
}
