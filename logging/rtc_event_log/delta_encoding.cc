...
enum class EncodingType {
  ...
  kReserved1 = 2,
  kReserved2 = 3,
  kNumberOfEncodingTypes  // Kep last
};
...
std::string FixedLengthDeltaEncoder::EncodeDeltas(
  ...
  printf("%d delta_width_bits_signed = %lu, delta_width_bits_unsigned = %lu\n",
         __LINE__, delta_width_bits_signed, delta_width_bits_unsigned);
...
FixedLengthDeltaEncoder::FixedLengthDeltaEncoder(uint64_t original_width_bits,
  ...
  printf("delta_width_bits_ = %lu, original_width_bits_ = %lu\n",
         delta_width_bits_, original_width_bits_);
...
bool FixedLengthDeltaDecoder::IsSuitableDecoderFor(const std::string& input) {
  ...
  printf("%d, encoding_type_bits = %u\n", __LINE__, encoding_type_bits);
...
std::unique_ptr<FixedLengthDeltaDecoder> FixedLengthDeltaDecoder::Create(
  ...
  printf("... %d\n", __LINE__);
  printf("%d original_width_bits = %lu, delta_width_bits = %lu\n", __LINE__, original_width_bits, delta_width_bits);
...
FixedLengthDeltaDecoder::FixedLengthDeltaDecoder(
  ...
  printf("delta_width_bits = %lu\n", delta_width_bits);
  RTC_DCHECK(reader_);
  printf("... %d\n", __LINE__);
  printf("original_width_bits = %lu\n", original_width_bits);
  printf("delta_width_bits = %lu\n", delta_width_bits);
}
...
bool FixedLengthDeltaDecoder::GetDelta(uint64_t* delta) {
  ...
  printf("GetDelta - %d (lower_bit_count = %lu)\n", __LINE__, lower_bit_count);
...
uint64_t FixedLengthDeltaDecoder::ApplyDelta(uint64_t base,
  ...
  if (original_width_bits_ < 64) {
    result %= (1 << (original_width_bits_ - 1));
  }
  return result;
}
