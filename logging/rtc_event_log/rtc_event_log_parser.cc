...
ParsedRtcEventLog::ParseStatus ParsedRtcEventLog::StoreNetEqSetMinimumDelay(
  ...
  std::vector<absl::optional<uint64_t>> minimum_delay_ms_values =
      DecodeDeltas(proto.minimum_delay_ms_deltas(), proto.minimum_delay_ms(),
                   number_of_deltas);
  ...
  for (size_t i = 0; i < number_of_deltas; ++i) {
    ...
    RTC_PARSE_CHECK_OR_RETURN_LE(minimum_delay_ms_values[i].value(),
    ...
    const int minimum_delay_ms =
        static_cast<int>(remote_ssrc_values[i].value());
