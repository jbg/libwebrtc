...
RtcEventLogDependencyDescriptorEncoderDecoder::Encode(
  ...
  // Start and end bit.
  {
    ...
    if (!delta_dds.empty()) {
      ...
      if (encoded_deltas.empty()) {
        RTC_LOG(LS_WARNING) << "Failed to encode start_end_bit_deltas.";
        return {};
      }
  ...
  // Template IDs.
  {
    ...
    if (!delta_dds.empty()) {
      ...
      res.set_template_id_deltas(EncodeDeltas(template_id, values));
    }
  }
  ...
  // Frame IDs.
  {
    ...
    if (!delta_dds.empty()) {
      ...
      res.set_frame_id_deltas(EncodeDeltas(frame_id, values));
    }
  }
  ...
  // Extended info
  {
    ...
    bool expect_encoded_data = false;
    for (size_t i = 0; i < raw_dd_data.size(); ++i) {
      if (raw_dd_data[i].size() > 3) {
        ...
        values[i] = {reinterpret_cast<const char*>(extended_info.data()),
                     extended_info.size()};
        expect_encoded_data = true;
      }
    }

    std::string encoded_blobs = EncodeOptionalBlobs(values);
    if (expect_encoded_data && encoded_blobs.empty()) {
      RTC_LOG(LS_WARNING) << "Failed to encode extended_infos.";
      return {};
    }
...
RtcEventLogDependencyDescriptorEncoderDecoder::Decode(
  ...
  auto get_extended = [&](int i) -> std::string* {
    if (!extended_infos.empty() && extended_infos[i].has_value()) {
      return &*extended_infos[i];
    }
    return nullptr;
  };

  auto recreate_raw_dd =
      [&](int i, const absl::optional<uint64_t>& be,
          const absl::optional<uint64_t>& tid,
          const absl::optional<uint64_t>& fid, std::string* ext) {
        if (be.has_value() && tid.has_value() && fid.has_value()) {
          res[i].reserve(3 + (ext ? ext->size() : 0));
          ...
          RTC_LOG(LS_WARNING) << "Not all required fields present.";
          return false;
        } else if (ext) {
          RTC_LOG(LS_WARNING)
  ...
  if (!recreate_raw_dd(0, start_end_bit_base, template_id_base, frame_id_base,
                       get_extended(0))) {
    return {};
  }
