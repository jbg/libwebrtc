...
std::string EncodeBlobs(const std::vector<std::string>& blobs) {
  ...
  for (auto it = blobs.begin(); it != blobs.end(); ++it) {
    ...
    RTC_DCHECK_GE(result_length_bound + it->length(), result_length_bound);
    result_length_bound += it->length();
  }
  ...
  for (absl::string_view blob : blobs) {
    result.insert(result.length(), blob.data(), blob.length());
  }
...
std::vector<absl::string_view> DecodeBlobs(absl::string_view encoded_blobs,
  ...
  std::vector<size_t> lengths(num_of_blobs);
