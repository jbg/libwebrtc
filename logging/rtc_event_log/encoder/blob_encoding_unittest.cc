...
TEST(BlobEncoding, DecodeBlobsHandlesErrorsGracefullyDefectiveVarInt) {
  ...
  for (size_t i = 0; i < kMaxVarIntLengthBytes; ++i) {
    ...
    defective_varint.insert(defective_varint.end(),
                            static_cast<CharT>(static_cast<size_t>(0x80u) | i));
  }
  defective_varint.insert(defective_varint.end(), static_cast<CharT>(0x01u));
...
TEST(BlobEncoding, DecodeBlobsHandlesErrorsGracefullyLengthSumWrapAround) {
  ...
  for (size_t i = 0; i < kMaxVarIntLengthBytes - 1; ++i) {
    max_size_varint.insert(max_size_varint.end(), static_cast<CharT>(0xffu));
  }
  max_size_varint.insert(max_size_varint.end(), static_cast<CharT>(0x7fu));
