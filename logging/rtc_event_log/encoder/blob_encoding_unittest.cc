...
TEST(BlobEncoding, DecodeBlobsHandlesErrorsGracefullyDefectiveVarInt) {
  ...
  defective_varint.insert(defective_varint.end(), static_cast<char>(0x01));
...
TEST(BlobEncoding, DecodeBlobsHandlesErrorsGracefullyLengthSumWrapAround) {
  ...
  for (size_t i = 0; i < kMaxVarIntLengthBytes - 1; ++i) {
    max_size_varint.insert(max_size_varint.end(), static_cast<char>(0xff));
  }
  max_size_varint.insert(max_size_varint.end(), static_cast<char>(0x7f));
