...
std::string RtcEventLogEncoderV3::EncodeBatch(
  ...
  for (auto it = begin; it != end; ++it) {
    ...
    // SSRC.
    uint64_t type = static_cast<uint64_t>((*it)->GetType());
    uint64_t group_events_by = (*it)->GetGroupKey();
    uint64_t key = (group_events_by << 32) + type;
    event_groups[key].push_back(it->get());
  }

  for (auto& kv : event_groups) {
    // The event type is stored in the lower 32 bits of the key.
    uint32_t event_type = kv.first & 0xFFFFFFFF;
    auto it = encoders_.find(event_type);
