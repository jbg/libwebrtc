...
void RtcEventLogImpl::LogEventsFromMemoryToOutput() {
  ...
  RTC_DCHECK_GE(num_config_events_written_, 0);
  RTC_DCHECK_LE(num_config_events_written_, config_history_.size());
  if (num_config_events_written_ < config_history_.size()) {
    auto begin = config_history_.begin() + num_config_events_written_;
    auto end = config_history_.end();
    WriteToOutput(event_encoder_->EncodeBatch(begin, end));
    num_config_events_written_ = config_history_.size();
  }

  // Serialize the events in the event queue.
  WriteToOutput(event_encoder_->EncodeBatch(history_.begin(), history_.end()));
  history_.clear();
}
