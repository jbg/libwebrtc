...
TEST_F(FileUtilsTest, ResourcePathReturnsCorrectPath) {
  std::string result = webrtc::test::ResourcePath("far16_stereo", "pcm");
#if defined(WEBRTC_IOS)
  // iOS bundles resources straight into the bundle root.
  std::string expected_end = Path("/far16_stereo.pcm");
#else
  // Other platforms: it's a separate dir.
  std::string expected_end = Path("resources/far16_stereo.pcm");
#endif
