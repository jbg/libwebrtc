...
// Here we assume for simplicity that no one will try to use utf-8 filenames
// on Windows for WebRTC test resources.
...
std::string ProjectRootPath() {
  ...
  if (GetModuleFileName(NULL, buf, MAX_PATH) == 0)
    return kCannotFindProjectRootDir;
  std::string exe_path = WideStringToString(std::wstring(buf));
