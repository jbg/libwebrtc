...
template <typename T>
using RefPtr = rtc::scoped_refptr<rtc::RefCountedObject<T>>;

template <typename T, typename... Args>
RefPtr<T> MakeRefCounted(Args... args) {
  return RefPtr<T>(new rtc::RefCountedObject<T>(args...));
}

template <typename T>
RefPtr<T> MakeRefCounted(T&& obj) {
  return RefPtr<T>(new rtc::RefCountedObject<T>(std::move(obj)));
}
...
class VideoFrameMatcher {
 ...
 private:
  ...
  struct CapturedFrame {
    ...
    rtc::scoped_refptr<VideoFrameBuffer> thumb;
    double best_score;
    RefPtr<DecodedFrame> best_decode;
    bool matched;
  };
  struct VideoLayer {
    int layer_id;
    std::deque<CapturedFrame> captured_frames;
    RefPtr<DecodedFrame> last_decode;
