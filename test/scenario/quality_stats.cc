...
void VideoFrameMatcher::OnCapturedFrame(const VideoFrame& frame,
  ...
  CapturedFrame captured{
      ...
      ScaleVideoFrameBuffer(*frame.video_frame_buffer()->ToI420(), kThumbWidth,
                            kThumbHeight),
      INFINITY,
      MakeNullRefCountPtr<DecodedFrame>()};
  task_queue_.PostTask([this, captured]() mutable {
...
void VideoFrameMatcher::OnDecodedFrame(const VideoFrame& frame,
  ...
  DecodedFrame decoded = {
      layers_[layer_id].next_decoded_id++, render_time,
      frame.video_frame_buffer(),
      ScaleVideoFrameBuffer(*frame.video_frame_buffer()->ToI420(), kThumbWidth,
                            kThumbHeight),
      0};
  task_queue_.PostTask([this, decoded, layer_id] {
    ...
    auto decoded_ref = MakeRefCountPtr(DecodedFrame(decoded));
    layer.last_decode = decoded_ref;
    while (!layer.captured_frames.empty()) {
      auto& captured = layer.captured_frames.front();
      double score =
          I420SSE(*captured.thumb->GetI420(), *decoded.thumb->GetI420());
      if (score < captured.best_score) {
        captured.best_score = score;
        captured.best_decode = decoded_ref;
        break;
      } else {
        HandleMatch(captured, layer_id);
        layer.captured_frames.pop_front();
      }
    }
    for (size_t i = 1; i < layer.captured_frames.size(); ++i) {
      auto& captured = layer.captured_frames[i];
      double score =
          I420SSE(*captured.thumb->GetI420(), *decoded.thumb->GetI420());
      if (score < captured.best_score) {
        captured.best_score = score;
