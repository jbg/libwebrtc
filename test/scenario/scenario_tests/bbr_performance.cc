...
TEST_P(BbrScenarioTest, ReceivesVideo) {
  ...
  std::string folder_path = OutputPath() + "output_data/bbr_test_gen";
  CreateDir(folder_path);
  std::string base_name = folder_path + "/bbr__" + conf_.Name();
  ...
  CallClient* alice = s.CreateClient("send", [&](CallClientConfig* c) {
    if (conf_.tuning.use_bbr)
      c->cc.type = CallClientConfig::CongestionControl::kBbr;
    c->cc.log_interval = TimeDelta::ms(100);
    c->rates.min_rate = DataRate::kbps(30);
    c->rates.max_rate = DataRate::kbps(1800);
  });
  ...
  ColumnsPrinter send_stats_printer(
      base_name + ".send.stats.txt",
      {s.TimePrinter(), alice->StatsPrinter(),
       alice_video->send()->StatsPrinter(), cross_traffic->StatsPrinter(),
       ColumnPrinter::Lambda(
           "propagation_delay capacity", [&](rtc::SimpleStringBuilder& sb) {
             sb.AppendFormat("%.3lf %.0lf",
                             net_conf.simulation.delay.seconds<double>(),
                             net_conf.simulation.bandwidth.bps() / 8.0);
           })});
  ...
  std::vector<ColumnPrinter> return_printers{
      s.TimePrinter(), bob->StatsPrinter(),
      ColumnPrinter::Fixed("cross_traffic_rate", "0"),
      ColumnPrinter::Lambda(
          "propagation_delay capacity", [&](rtc::SimpleStringBuilder& sb) {
            sb.AppendFormat("%.3lf %.0lf",
                            net_conf.simulation.delay.seconds<double>(),
  ...
  ColumnsPrinter return_stats_printer(base_name + ".return.stats.txt",
                                      return_printers);
  return_stats_printer.PrintHeaders();
  s.Every(TimeDelta::ms(100), [&]() { return_stats_printer.PrintRow(); });
