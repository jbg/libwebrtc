...
std::unique_ptr<FileLogWriterManager> GetScenarioLogManager(
  ...
  if (FLAG_scenario_logs && !file_name.empty()) {
    ...
    return absl::make_unique<FileLogWriterManager>(base_filename);
  }
  return nullptr;
}
}
...
Scenario::Scenario()
    : Scenario(std::unique_ptr<LogWriterImplFactoryInterface>(), true) {}
...
Scenario::Scenario(
    std::unique_ptr<LogWriterImplFactoryInterface> log_writer_manager,
    bool real_time)
  ...
  if (log_writer_manager) {
    log_writer_factory_ =
        absl::make_unique<LogWriterFactory>(std::move(log_writer_manager));
  }
...
CallClient* Scenario::CreateClient(std::string name, CallClientConfig config) {
  ...
  CallClient* client =
      new CallClient(clock_, name, log_writer_factory_.get(), config);
...
SimulatedTimeClient* Scenario::CreateSimulatedTimeClient(
  ...
  SimulatedTimeClient* client = new SimulatedTimeClient(
      name, log_writer_factory_.get(), config, stream_configs, send_link,
      return_link, send_id, return_id, Now());
...
VideoStreamPair* Scenario::CreateVideoStream(
    std::pair<CallClient*, CallClient*> clients,
    VideoStreamConfig config) {
  std::unique_ptr<LogWriter> quality_logger;
  if (config.analyzer.log_to_file)
    quality_logger = GetLogWriter(clients.first->name_ + ".video_quality.txt");
