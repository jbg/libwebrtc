...
Scenario::Scenario(
    ...
    : log_writer_factory_(std::move(log_writer_factory)),
      time_simulation_(real_time ? nullptr
                                 : absl::make_unique<TimeSimulation>(
                                       Timestamp::seconds(100000),
                                       log_writer_factory_.get() != nullptr)),
      real_time_runner_factory_(
          real_time ? absl::make_unique<DefaultTaskRunnerFactory>() : nullptr),
      task_runner_factory_(
          real_time
              ? static_cast<TaskRunnerFactory*>(real_time_runner_factory_.get())
              : static_cast<TaskRunnerFactory*>(time_simulation_.get())),
      clock_(real_time ? Clock::GetRealTimeClock()
                       : time_simulation_->GetClock()),
      audio_decoder_factory_(CreateBuiltinAudioDecoderFactory()),
      audio_encoder_factory_(CreateBuiltinAudioEncoderFactory()),
      network_task_runner_(task_runner_factory_, "Network") {}
...
CallClient* Scenario::CreateClient(std::string name, CallClientConfig config) {
  RTC_DCHECK(real_time_runner_factory_);
  CallClient* client = new CallClient(clock_, task_runner_factory_,
...
void Scenario::Every(TimeDelta interval,
  ...
  network_task_runner_.DelayedStart(interval, [interval, function] {
    function(interval);
    return interval;
  });
}
...
void Scenario::At(TimeDelta offset, std::function<void()> function) {
  network_task_runner_.PostDelayed(offset - Duration(), function);
}
...
void Scenario::RunFor(TimeDelta duration) {
  if (start_time_.IsInfinite())
    Start();
  if (time_simulation_) {
    time_simulation_->RunFor(duration);
  } else {
    rtc::Event done;
...
void Scenario::RunUntil(TimeDelta max_duration,
  ...
  if (time_simulation_) {
    while (Duration() < max_duration) {
      time_simulation_->RunFor(poll_interval);
      if (exit_function())
        break;
    }
  } else {
    rtc::Event done;
    TaskHandle handle =
        network_task_runner_.Start([poll_interval, exit_function, &done] {
          if (exit_function())
            done.Set();
          return poll_interval;
        });
    auto duration = max_duration - Duration();
    done.Wait(duration.ms<int>());
    handle.PostStop();
  }
}
