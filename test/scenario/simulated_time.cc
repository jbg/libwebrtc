...
SimulatedTimeClient::SimulatedTimeClient(
    ...
    uint64_t return_receiver_id,
    Timestamp at_time)
    : network_controller_factory_(log_filename,
                                  log_writer_factory,
                                  config.transport),
  ...
  if (!log_filename.empty() && log_writer_factory) {
    packet_log_ = log_writer_factory->Create(log_filename + ".packets.txt");
    packet_log_->Write(
...
void SimulatedTimeClient::OnPacketReceived(EmulatedIpPacket packet) {
  ...
  for (PacketResult& feedback : report.packet_feedbacks) {
    if (packet_log_)
      Format(packet_log_.get(), "%" PRId64 " %" PRId64 " %.3lf %.3lf %.3lf\n",
