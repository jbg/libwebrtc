...
DirectTransport::DirectTransport(SingleThreadedTaskQueueForTesting* task_queue,
    ...
    : send_call_(send_call),
      ...
      fake_network_ptr_(std::unique_ptr<FakeNetworkPipe>(
          new FakeNetworkPipe(clock_, config, std::move(demuxer)))) {
  Start();
}
...
DirectTransport::DirectTransport(SingleThreadedTaskQueueForTesting* task_queue,
    ...
    : send_call_(send_call),
      clock_(Clock::GetRealTimeClock()),
      task_queue_(task_queue),
      fake_network_ptr_(std::move(pipe)) {
  Start();
}
...
void DirectTransport::SetConfig(const FakeNetworkPipe::Config& config) {
  fake_network_ptr_->SetConfig(config);
}
...
void DirectTransport::SetReceiver(PacketReceiver* receiver) {
  RTC_DCHECK_CALLED_SEQUENTIALLY(&sequence_checker_);
  fake_network_ptr_->SetReceiver(receiver);
}
...
bool DirectTransport::SendRtp(const uint8_t* data,
  ...
  fake_network_ptr_->SendPacket(data, length);
  return true;
}
...
int DirectTransport::GetAverageDelayMs() {
  return fake_network_ptr_->AverageDelay();
}
...
void DirectTransport::SendPackets() {
  ...
  fake_network_ptr_->Process();

  int64_t delay_ms = fake_network_ptr_->TimeUntilNextProcess();
