...
TEST_P(UnsignaledStreamTest, ReplacesUnsignaledStreamOnCompletedSignaling) {
  ...
  // - MID is neegotiated which means PT-based demuxing is enabled, and the
  //   packets have the MID. This means packets either get forwarded or dropped
  //   depending on if the MID is known.

  uint32_t first_ssrc = 0;
  uint32_t second_ssrc = 0;
  absl::optional<int> mid_header_extension_id = absl::nullopt;
  RTC_LOG(LS_ERROR) << "kMidTestConfiguration: " << kMidTestConfiguration;

  signaling.NegotiateSdp(
      /* munge_sdp = */
      [&](SessionDescriptionInterface* offer) {
        // Obtain the MID header extension ID and if we want he
        // kMidNotNegotiated setup then we remove the MID header extension.
            ...
            if (ByteReader<uint32_t>::ReadBigEndian(&(packet.cdata()[8])) ==
              ...
              switch (kMidTestConfiguration) {
                ...
                case MidTestConfiguration::kMidNegotiatedAndPresentInPackets:
                  ...
                  // conflicts if we later add that second m= section.
