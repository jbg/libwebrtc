...
PartialFrameCompressor::PartialFrameCompressor() = default;
PartialFrameCompressor::~PartialFrameCompressor() = default;

void PartialFrameCompressor::SetSource(
...
void PartialFrameCompressor::OnFrame(const VideoFrame& frame) {
  ...
  part_desc.full_width = frame.width();
  part_desc.full_height = frame.height();
...
void PartialFrameCompressor::AddOrUpdateSink(
...
void PartialFrameCompressor::RemoveSink(
    rtc::VideoSinkInterface<VideoFrame>* sink) {
  {
    rtc::CritScope lock(&source_crit_);
    if (source_) {
