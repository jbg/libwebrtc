...
void EmulatedNetworkOutgoingStatsBuilder::OnPacketSent(
  ...
  if (mode == EmulatedEndpointConfig::StatsGatheringMode::kDebug) {
    sent_packets_size_counter_.AddSample(
        SamplesStatsCounter::StatsSample(packet_size.bytes(), sent_time));
  }
}
...
void EmulatedNetworkIncomingStatsBuilder::OnPacketDropped(
  ...
  if (mode == EmulatedEndpointConfig::StatsGatheringMode::kDebug) {
    dropped_packets_size_counter_.AddSample(
        SamplesStatsCounter::StatsSample(packet_size.bytes(), dropped_time));
  }
}
...
void EmulatedNetworkIncomingStatsBuilder::OnPacketReceived(
  ...
  if (mode == EmulatedEndpointConfig::StatsGatheringMode::kDebug) {
    received_packets_size_counter_.AddSample(
        SamplesStatsCounter::StatsSample(packet_size.bytes(), received_time));
  }
}
...
void EmulatedNetworkStatsBuilder::OnPacketSent(
  ...
  RTC_DCHECK_RUN_ON(&sequence_checker_);
  if (mode == EmulatedEndpointConfig::StatsGatheringMode::kDebug) {
    sent_packets_queue_wait_time_us_.AddSample(SamplesStatsCounter::StatsSample(
        (sent_time - queued_time).us(), sent_time));
  }
...
void EmulatedNetworkStatsBuilder::OnPacketDropped(
    ...
    EmulatedEndpointConfig::StatsGatheringMode mode) {
  RTC_DCHECK_RUN_ON(&sequence_checker_);
  incoming_stats_per_source_[source_ip].OnPacketDropped(dropped_time,
                                                        packet_size, mode);
}
...
void EmulatedEndpointImpl::OnPacketReceived(EmulatedIpPacket packet) {
  ...
  rtc::CritScope crit(&receiver_lock_);
  Timestamp current_time = clock_->CurrentTime();
  stats_builder_.OnPacketReceived(current_time, packet.from.ipaddr(),
  ...
  if (it == port_to_receiver_.end()) {
    ...
    stats_builder_.OnPacketDropped(current_time, packet.from.ipaddr(),
                                   DataSize::Bytes(packet.ip_packet_size()),
                                   stats_gathering_mode_);
    return;
  }
