...
class AsyncPacketSocketWrapper : public rtc::AsyncPacketSocket {
 public:
  AsyncPacketSocketWrapper(webrtc::test::EmulatedTURNServer* turn_server,
                           ...
                           uint16_t port)
      : turn_server_(turn_server), endpoint_(endpoint), port_(port) {}
  ~AsyncPacketSocketWrapper() { turn_server_->Unbind(GetLocalAddress()); }
...
void EmulatedTURNServer::Stop() {
  thread_->Invoke<void>(RTC_FROM_HERE, [=]() { sockets_.clear(); });
}
...
rtc::AsyncPacketSocket* EmulatedTURNServer::Wrap(EmulatedEndpoint* endpoint) {
  ...
  thread_->Invoke<void>(RTC_FROM_HERE, [=]() {
    sockets_[rtc::SocketAddress(endpoint->GetPeerLocalAddress(), port)] =
        socket;
  });
  return socket;
}
