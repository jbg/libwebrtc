...
EncodedImageCallback::Result FakeH264Encoder::OnEncodedImage(
  ...
  uint8_t* const data = const_cast<uint8_t*>(encoded_image.data());
  if (current_idr_counter % kIdrFrequency == 0 &&
    ...
    data[fragmentation.fragmentationOffset[0]] = kSpsNalHeader;
    data[fragmentation.fragmentationOffset[1]] = kPpsNalHeader;
    data[fragmentation.fragmentationOffset[2]] = kIdrNalHeader;
  } else {
    ...
    data[fragmentation.fragmentationOffset[0]] = kNalHeader;
  }
  uint8_t value = 0;
