...
void GenerateFloatFrame(test::FuzzDataHelper* fuzz_data,
  ...
  std::fill(float_frames[0], float_frames[0] + 480, 0);
  std::fill(float_frames[1], float_frames[1] + 480, 0);
  ...
  for (size_t i = 0; i < num_channels; ++i) {
    for (size_t j = 0; j < samples_per_input_channel; ++j) {
      ...
      float_frames[i][j] =
          ...
          static_cast<float>(limits_int16::max());
    }
  }
}
    ...
    if (is_float) {
      ...
      for (size_t i = 0; i < num_channels; ++i) {
        for (size_t j = 0; j < 480; ++j) {
          RTC_CHECK(!std::isnan(ptr_to_float_frames[i][j]));
        }
      }
      if (is_capture) {
