...
void RtpReplayer::ReplayPackets(
  ...
  while (true) {
    ...
    ++num_packets;
    RtpPacketReceived received_packet(&extensions, Timestamp::Millis(now_ms));
    ...
    call->Receiver()->DeliverRtpPacket(
        webrtc::MediaType::VIDEO, std::move(received_packet),
        [&](const RtpPacketReceived& parsed_packet) -> bool {
