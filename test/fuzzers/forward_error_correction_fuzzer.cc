...
void FuzzOneInput(const uint8_t* data, size_t size) {
  ...
  char* packet_buffer = reinterpret_cast<char*>(received_packet.pkt->data);
  uint8_t reordering;
  uint16_t seq_num_diff;
  uint8_t packet_type;
  uint8_t packet_loss;
  while (true) {
    if (!fuzz_buffer.ReadBytes(packet_buffer, kPacketSize))
      return;
    ...
    if (packet_type % 2 == 0) {
      ...
      ByteWriter<uint8_t>::WriteBigEndian(
          reinterpret_cast<uint8_t*>(&packet_buffer[8]), 1);  // SSRCCount.
      ByteWriter<uint32_t>::WriteBigEndian(
          reinterpret_cast<uint8_t*>(&packet_buffer[12]), kMediaSsrc);  // SSRC.
      ByteWriter<uint16_t>::WriteBigEndian(
          reinterpret_cast<uint8_t*>(&packet_buffer[16]),
          original_media_seqnum);  // SN base.
    } else {
