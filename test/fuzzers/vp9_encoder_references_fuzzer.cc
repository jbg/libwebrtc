...
bool IsSupported(int num_spatial_layers,
  ...
  // Vp9 encoder doesn't support certain configuration.
  // BitrateAllocator shouldn't produce them.
  if (parameters.bitrate.get_sum_bps() == 0) {
    // Ignore allocation that turns off all the layers. In such a case
  ...
  for (int tid = 0; tid < num_temporal_layers; ++tid) {
    ...
    if (num_enabled_spatial_layers == 0) {
      // Each temporal layers should be enanbled because skipping full frames is
