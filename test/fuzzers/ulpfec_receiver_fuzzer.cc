...
void FuzzOneInput(const uint8_t* data, size_t size) {
  ...
  uint32_t ulpfec_ssrc;
  memcpy(&ulpfec_ssrc, data + 0, 4);
  uint16_t ulpfec_seq_num;
  memcpy(&ulpfec_seq_num, data + 4, 2);
  uint32_t media_ssrc;
  ...
  UlpfecReceiver* receiver = UlpfecReceiver::Create(ulpfec_ssrc, &callback);
