...
TEST(TimeSimulationTaskRunnerTest, TaskIsStoppedOnStop) {
  ...
  TaskRunner task_handler(&time_simulation, "TestQueue");
  ...
  time_simulation.RunFor(kShortInterval * (kShortIntervalCount + kMargin));
  EXPECT_EQ(counter.load(), kShortIntervalCount);

  handle.PostStop();
  // Wait long enough that the task would run at least once more if not
  // stopped.
  time_simulation.RunFor(kLongInterval * 2);
  EXPECT_EQ(counter.load(), kShortIntervalCount);
}
...
TEST(TimeSimulationTaskRunnerTest, TaskCanStopItself) {
  ...
  time_simulation.RunFor(TimeDelta::ms(10));
