...
class PendingTaskInterface {
 public:
  ...
  virtual void RunClosure() = 0;
};
class RepeatingTaskHandleInterface {
 public:
  virtual void Stop() = 0;
  virtual void PostStop() = 0;

 protected:
  ~RepeatingTaskHandleInterface() = default;
};
class RepeatingTaskInterface {
 public:
  virtual ~RepeatingTaskInterface() = default;
  virtual TimeDelta RunClosure(Timestamp at_time) = 0;
};
...
class TaskRunnerImplInterface {
 protected:
  ...
  virtual RepeatingTaskHandleInterface* Start(
      TimeDelta delay,
