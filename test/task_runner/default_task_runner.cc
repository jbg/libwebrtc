...
class QueuedRepeatingTaskWrapper : public QueuedTask,
                                   public RepeatingTaskHandleInterface {
 public:
  ...
  bool Run() final {
    ...
    if (next_run_time_.IsPlusInfinity())
      return true;
    TimeDelta delay = task_->RunClosure(next_run_time_);
...
RepeatingTaskHandleInterface* DefaultTaskRunner::Start(
    TimeDelta delay,
    std::unique_ptr<RepeatingTaskInterface> task) {
...
TaskRunnerImplInterface* DefaultTaskRunnerFactory::Create(
