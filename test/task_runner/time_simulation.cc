...
RepeatingTaskHandleInterface* SimulatedTimeTaskRunner::Start(
  ...
  auto task_wrapper = absl::make_unique<RepeatingTaskImpl>();
  repeating_tasks_.push_back(RepeatingTaskImpl(target_time, std::move(task)));
  auto* task_ptr = &repeating_tasks_.back();
  if (delay <= TimeDelta::Zero()) {
    next_task_time_ = Timestamp::MinusInfinity();
  } else {
