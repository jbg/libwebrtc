 ...
 */
#include "test/task_runner/time_simulation.h"
...
class SimulatedTimeTaskRunner : public TaskRunnerImplInterface {
 public:
 ...
 private:
  ...
  std::list<std::unique_ptr<RepeatingTaskImpl>> repeating_tasks_;
...
RepeatingTaskHandleInterface* SimulatedTimeTaskRunner::Start(
    TimeDelta delay,
    std::unique_ptr<RepeatingTaskInterface> task) {
  auto target_time =
      handler_->GetCurrentTime() + std::max(delay, TimeDelta::Zero());
  auto task_wrapper =
      absl::make_unique<RepeatingTaskImpl>(target_time, std::move(task));
  auto* task_ptr = task_wrapper.get();
  repeating_tasks_.push_back(std::move(task_wrapper));
  RTC_DCHECK(repeating_tasks_.back());

  if (delay <= TimeDelta::Zero()) {
    next_task_time_ = Timestamp::MinusInfinity();
  } else {
    next_task_time_ = std::min(next_task_time_, task_ptr->next_time_);
  }
  return task_ptr;
}
...
void SimulatedTimeTaskRunner::RunTasks() {
  while (!pending_tasks_.empty()) {
    pending_tasks_.front()->RunClosure();
    pending_tasks_.pop_front();
  }
  while (!pending_repeating_tasks_.empty()) {
    auto* task_wrapper = pending_repeating_tasks_.front();
    task_wrapper->next_time_ +=
        task_wrapper->task_->RunClosure(task_wrapper->next_time_);
...
void SimulatedTimeTaskRunner::UpdateTimedTasks(Timestamp at_time) {
  ...
  for (auto& repeating : repeating_tasks_) {
    RTC_DCHECK(repeating);
    if (repeating->next_time_ <= at_time) {
      pending_repeating_tasks_.push_back(repeating.get());
    }
  }
  repeating_tasks_.remove_if(
      [](const std::unique_ptr<RepeatingTaskImpl>& repeating) {
...
void TimeSimulation::RunFor(TimeDelta duration) {
...
TaskRunnerImplInterface* TimeSimulation::Create(absl::string_view queue_name,
                                                TaskQueuePriority priority) {
