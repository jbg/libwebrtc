...
class LogWriterFactory {
 public:
  ...
  virtual ~LogWriterFactory() = default;
};

template <class... Args>
inline void Format(RtcEventLogOutput* out_, const char* fmt, Args&&... args) {
  rtc::StringBuilder sb;
  sb.AppendFormat(fmt, std::forward<Args>(args)...);
  // TODO(srte): Add formatted output override to RtcEventLogOutput.
  out_->Write(sb.str());
}
}  // namespace webrtc
