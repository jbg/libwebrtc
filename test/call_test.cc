...
void CallTest::RunBaseTest(BaseTest* test) {
  SendTask(task_queue(), [this, test]() {
    ...
    CallConfig send_config(
        EnvironmentBuilder().With(send_event_log_.get()).Build());
    ...
    if (test->ShouldCreateReceivers()) {
      CallConfig recv_config(
          EnvironmentBuilder().With(recv_event_log_.get()).Build());
...
void CallTest::CreateCalls() {
  CreateCalls(
      ...
      CallConfig(EnvironmentBuilder().With(recv_event_log_.get()).Build()));
}
...
void CallTest::CreateSenderCall(const CallConfig& config) {
  ...
  sender_config.env = EnvironmentBuilder(config.env)
...
void CallTest::CreateReceiverCall(const CallConfig& config) {
  ...
  receiver_config.env = EnvironmentBuilder(config.env)
                            .With(task_queue_factory_.get())
                            .With(&field_trials_)
                            .Build();
