...
void CallTest::CreateCalls() {
  CreateCalls(
      Call::Config(ContextBuilder().With(send_event_log_.get()).Build()),
      Call::Config(ContextBuilder().With(recv_event_log_.get()).Build()));
}
...
void CallTest::CreateReceiverCall(const Call::Config& config) {
  Call::Config receiver_config = config;
