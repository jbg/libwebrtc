...
void CallTest::CreateCalls() {
  CreateCalls(CallConfig(ContextBuilder().With(send_event_log_.get()).Build()),
              CallConfig(ContextBuilder().With(recv_event_log_.get()).Build()));
}
...
void CallTest::CreateSenderCall() {
  CreateSenderCall(
      CallConfig(ContextBuilder().With(send_event_log_.get()).Build()));
}

void CallTest::CreateSenderCall(const CallConfig& config) {
  auto sender_config = config;
  sender_config.context = ContextBuilder(config.context)
...
void CallTest::CreateReceiverCall(const CallConfig& config) {
  ...
  receiver_config.context = ContextBuilder(config.context)
                                .With(task_queue_factory_.get())
                                .With(&field_trials_)
                                .Build();
