...
constexpr int kStereoNumChannels = 2;
 ...
 public:
  WavFileReader(std::string filename, int sampling_frequency_in_hz)
      : sampling_frequency_in_hz_(sampling_frequency_in_hz),
        ...
        num_channels_(wav_reader_.num_channels()) {
...
class WavFileWriter final : public test::FakeAudioDevice::Renderer {
 public:
  ...
  bool NumChannels() const override {
    return num_channels_;
  }
...
class DiscardRenderer final : public test::FakeAudioDevice::Renderer {
 public:
  explicit DiscardRenderer(int sampling_frequency_in_hz, int num_channels = 1)
      : sampling_frequency_in_hz_(sampling_frequency_in_hz),
        num_channels_(num_channels) {}
...
FakeAudioDevice::CreateBoundedWavFileWriter(std::string filename,
                                            int sampling_frequency_in_hz,
                                            int num_channels) {
  return std::unique_ptr<FakeAudioDevice::Renderer>(new BoundedWavFileWriter(
...
int32_t FakeAudioDevice::SetStereoPlayout(bool enable) {
  bool stereo_enabled = (renderer_->NumChannels() == kStereoNumChannels);
  if (stereo_enabled == enabled) {
    return 0;
  }
  return -1;
}

int32_t FakeAudioDevice::StereoPlayoutIsAvailable(bool* available) const {
  *available = (renderer_->NumChannels() == kStereoNumChannels);
  return 0;
}

int32_t FakeAudioDevice::StereoPlayout(bool* enabled) const {
  *enabled = (renderer_->NumChannels() == kStereoNumChannels);
  return 0;
}

int32_t FakeAudioDevice::SetStereoRecording(bool enable) {
  bool stereo_enabled = (capturer_->NumChannels() == kStereoNumChannels);
  if (stereo_enabled == enabled) {
    return 0;
  }
  return -1;
...
int32_t FakeAudioDevice::StereoRecordingIsAvailable(bool* available) const {
  *available = (capturer_->NumChannels() == kStereoNumChannels);
  return 0;
}

int32_t FakeAudioDevice::StereoRecording(bool* enabled) const {
  *enable = (capturer_->NumChannels() == kStereoNumChannels);
  return 0;
}
