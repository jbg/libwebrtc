...
class TransparentNetworkNode : public webrtc::NetworkNode {
 public:
  explicit TransparentNetworkNode(std::string id);
  ~TransparentNetworkNode() override;

  void Process(Timestamp cur_time) override;
  void SetReceiver(std::string endpoint_id,
                   webrtc::NetworkReceiverInterface* receiver) override;
 ...
 private:
  ...
  std::map<std::string, webrtc::NetworkReceiverInterface*> routing_
      RTC_GUARDED_BY(lock_);
  std::deque<std::unique_ptr<EmulatedIpPacket>> packets_ RTC_GUARDED_BY(lock_);
};
...
class EndpointNodeImpl : public webrtc::EndpointNode {
 public:
  EndpointNodeImpl(std::string endpoint_id,
  ...
  void SetPeerLocalAddress(absl::optional<rtc::SocketAddress> address) override;

 protected:
  void DeliverPacketInternal(std::unique_ptr<EmulatedIpPacket> packet) override;

  webrtc::NetworkNode* GetSendNode() override;
  webrtc::NetworkNode* GetReceiveNode() override;
 ...
 private:
  ...
  webrtc::NetworkNode* const send_node_;
  webrtc::NetworkNode* const receive_node_;
