...
template <typename T>
absl::optional<T> MaybeGetValue(const std::map<int, T>& map, int key) {
...
uint16_t DefaultVideoQualityAnalyzer::OnFrameCaptured(
  ...
  }
  {
    ...
    std::map<int, FrameCounters>& receiver_counters =
        stream_frame_counters_[stream_label];
    for (auto& peer_entry : peer_by_index_) {
      receiver_counters[peer_entry.first].captured++;
    }
    ...
    if (it != captured_frames_in_flight_.end()) {
      ...
      for (auto& peer_entry : peer_to_index_) {
        ...
        receiver_counters[receiver_index].dropped++;
...
void DefaultVideoQualityAnalyzer::OnFramePreEncode(
  ...
  std::map<int, FrameCounters>& receiver_counters =
      stream_frame_counters_[it->second.stream_label()];
  for (auto& peer_entry : peer_by_index_) {
    receiver_counters[peer_entry.first].pre_encoded++;
  }
...
void DefaultVideoQualityAnalyzer::OnFrameEncoded(
  ...
  if (!it->second.HasEncodedTime()) {
    ...
    std::map<int, FrameCounters>& receiver_counters =
        stream_frame_counters_[it->second.stream_label()];
    for (auto& peer_entry : peer_by_index_) {
      receiver_counters[peer_entry.first].encoded++;
    }
  }
