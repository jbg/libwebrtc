...
void DefaultVideoQualityAnalyzer::Start(
  ...
  peers_ = std::vector<std::string>(peer_names.begin(), peer_names.end());
  for (size_t i = 0; i < peers_.size(); ++i) {
    peer_to_index_[peers_[i]] = i;
  }
...
uint16_t DefaultVideoQualityAnalyzer::OnFrameCaptured(
  ...
  size_t peer_index = peer_to_index_.at(std::string(peer_name));
  {
    rtc::CritScope crit(&lock_);
  ...
  }
  {
    ...
    for (size_t i = 0; i < peers_.size(); ++i) {
      ...
      StatsKey stats_key(stream_label, std::string(peer_name), peers_[i]);
  ...
  }
  {
    ...
    for (size_t i = 0; i < peers_.size(); ++i) {
      if (i != peer_index) {
        StatsKey key(stream_label, std::string(peer_name), peers_[i]);
    ...
    if (state_it == stream_states_.end()) {
      stream_states_.insert(
          {stream_label, StreamState(peer_index, peers_.size())});
    }
    ...
    if (it != captured_frames_in_flight_.end()) {
      ...
      for (size_t i = 0; i < peers_.size(); ++i) {
        ...
        AddComparison(StatsKey(stream_label, std::string(peer_name), peers_[i]),
...
void DefaultVideoQualityAnalyzer::OnFramePreEncode(
  ...
  for (size_t i = 0; i < peers_.size(); ++i) {
    if (i != peer_index) {
      StatsKey key(it->second.stream_label(), std::string(peer_name),
                   peers_[i]);
...
void DefaultVideoQualityAnalyzer::OnFramePreDecode(
  ...
  size_t peer_index = peer_to_index_.at(std::string(peer_name));
  RTC_DCHECK(!it->second.HasReceivedTime(peer_index))
      << "Received multiple spatial layers for stream_label="
      << it->second.stream_label();
  frame_counters_.received++;
  StatsKey key(it->second.stream_label(),
               stream_to_sender_.at(it->second.stream_label()),
...
void DefaultVideoQualityAnalyzer::OnFrameRendered(
  ...
  StreamState* state = &stream_states_.at(stream_label);
  const StatsKey stats_key(stream_label, peers_[state->owner()],
                           std::string(peer_name));
  ...
  while (!state->IsEmpty(peer_index) &&
    ...
    if (dropped_frame_it->second.DidAllPeersReceived()) {
  ...
  if (frame_it->second.DidAllPeersReceived()) {
  ...
  // holds any locks.
  {
    ...
    for (auto& state_entry : stream_states_) {
      ...
      for (size_t i = 0; i < peers_.size(); ++i) {
        ...
        StatsKey stats_key(stream_label, peers_[stream_state.owner()],
                           peers_[i]);
...
DefaultVideoQualityAnalyzer::FrameInFlight::GetPeersWhichDidntReceive() {
  ...
  for (size_t i = 0; i < receivers_count_; ++i) {
...
void DefaultVideoQualityAnalyzer::FrameInFlight::OnFramePreDecode(
  ...
  received_time_.insert({peer, received_time});
  decode_start_time_.insert({peer, decode_start_time});
}
...
DefaultVideoQualityAnalyzer::FrameInFlight::GetStatsForPeer(size_t peer) {
  ...
  stats.received_time = MaybeGetValue<Timestamp>(received_time_, peer)
                            .value_or(Timestamp::MinusInfinity());
  stats.decode_start_time = MaybeGetValue<Timestamp>(decode_start_time_, peer)
                                .value_or(Timestamp::MinusInfinity());
  stats.decode_end_time = MaybeGetValue<Timestamp>(decode_end_time_, peer)
                              .value_or(Timestamp::MinusInfinity());
  stats.rendered_time = MaybeGetValue<Timestamp>(rendered_time_, peer)
                            .value_or(Timestamp::MinusInfinity());
  stats.prev_frame_rendered_time =
      MaybeGetValue<Timestamp>(prev_frame_rendered_time_, peer)
          .value_or(Timestamp::MinusInfinity());
  stats.rendered_frame_width = MaybeGetValue<int>(rendered_frame_width_, peer);
  stats.rendered_frame_height =
      MaybeGetValue<int>(rendered_frame_height_, peer);
  return stats;
}
