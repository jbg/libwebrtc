...
void AnalyzingVideoSinksSupport::AddConfig(absl::string_view sender_peer_name,
                                           const VideoConfig& config) {
  MutexLock lock(&mutex_);
  auto insert_result = video_configs_.emplace(
      *config.stream_label, std::pair{std::string(sender_peer_name), config});
  if (!insert_result.second) {
    insert_result.first->second = {std::string(sender_peer_name), config};
  }
}

absl::optional<std::pair<std::string, VideoConfig>>
AnalyzingVideoSinksSupport::GetPeerAndConfig(absl::string_view stream_label) {
  MutexLock lock(&mutex_);
...
void AnalyzingVideoSinksSupport::RemoveConfig(absl::string_view stream_label) {
  MutexLock lock(&mutex_);
  video_configs_.erase(std::string(stream_label));
}

test::VideoFrameWriter* AnalyzingVideoSinksSupport::AddVideoWriter(
...
void AnalyzingVideoSinksSupport::CloseAndRemoveVideoWriters(
...
void AnalyzingVideoSinksSupport::Clear() {
  MutexLock lock(&mutex_);
