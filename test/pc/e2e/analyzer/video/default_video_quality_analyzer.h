 ...
 private:
  ...
  class StreamState {
   public:
    ...
    bool IsEmpty(int peer) const { return frame_ids_.IsEmpty(peer); }
    // Crash if state is empty.
    uint16_t Front(int peer) const { return frame_ids_.Front(peer).value(); }

    int GetAliveFramesCount() { return frame_ids_.size() - dead_frames_count_; }
    uint16_t MarkNextAliveFrameAsDead();

    void SetLastRenderedFrameTime(int peer, Timestamp time);
   ...
   private:
    ...
    // Count of dead frames in the beginning of the |frame_ids_|.
  ...
  class FrameInFlight {
   public:
    ...
    std::vector<int> GetPeersWhichDidntReceive();
    bool DoesAllPeersReceived() {
    ...
    void OnFramePreDecode(int peer,
                          webrtc::Timestamp received_time,
                          webrtc::Timestamp decode_start_time);
    bool HasReceivedTime(int peer) const {
