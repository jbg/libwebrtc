...
AnalyzingVideoSink::AnalyzingVideoSink(
    absl::string_view peer_name,
    Clock* clock,
    VideoQualityAnalyzerInterface& analyzer,
    AnalyzingVideoSinksSupport& sinks_support,
    const VideoSubscription& subscription)
    : peer_name_(peer_name),
      clock_(clock),
      analyzer_(&analyzer),
      sinks_support_(&sinks_support),
...
void AnalyzingVideoSink::UpdateSubscription(
  ...
  sinks_support_->CloseAndRemoveVideoWriters(writers_to_close);
}
...
void AnalyzingVideoSink::OnFrame(const VideoFrame& frame) {
  ...
  if (frame.id() != VideoFrame::kNotSetId) {
    ...
    Sinks* sinks = PopulateSinks(stream_label);
    if (sinks == nullptr) {
      return;
    }
    for (auto& sink : sinks->sinks) {
      sink->OnFrame(frame);
    }
  }
}

AnalyzingVideoSink::Sinks* AnalyzingVideoSink::PopulateSinks(
  ...
  absl::optional<std::pair<std::string, VideoConfig>> peer_and_config =
      sinks_support_->GetPeerAndConfig(stream_label);
  ...
  Sinks sinks(sender_peer_name, *resolution);
  if (config.output_dump_options.has_value()) {
    ...
    sinks.sinks.push_back(std::make_unique<VideoWriter>(
        writer.get(), config.output_dump_options->sampling_modulo()));
    sinks.video_frame_writer =
        sinks_support_->AddVideoWriter(std::move(writer));
  }
  if (config.show_on_screen) {
    sinks.sinks.push_back(absl::WrapUnique(test::VideoRenderer::Create(
        (*config.stream_label + "-render").c_str(), resolution->width(),
        resolution->height())));
  }
  return &stream_sinks_.emplace(stream_label, std::move(sinks)).first->second;
}
