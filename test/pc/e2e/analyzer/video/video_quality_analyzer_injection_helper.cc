...
class VideoRenderedsink final : public rtc::VideoSinkInterface<VideoFrame> {
 public:
  VideoRenderedsink(std::string title, size_t width, size_t height)
      : title_(std::move(title)) {
    renderer_ = absl::WrapUnique(
        test::VideoRenderer::Create(title_.c_str(), width, height));
  }

  void OnFrame(const VideoFrame& frame) override { renderer_->OnFrame(frame); }

 private:
  const std::string title_;
  std::unique_ptr<test::VideoRenderer> renderer_;
};
...
VideoQualityAnalyzerInjectionHelper::WrapFrameGenerator(
  ...
  if (config.show_on_screen) {
    sinks.push_back(absl::make_unique<VideoRenderedsink>(
        *config.stream_label + "-capture", config.width, config.height));
  }
...
VideoQualityAnalyzerInjectionHelper::CreateVideoSink(
  ...
  if (config.show_on_screen) {
    sinks.push_back(absl::make_unique<VideoRenderedsink>(
        *config.stream_label + "-render", config.width, config.height));
  }
