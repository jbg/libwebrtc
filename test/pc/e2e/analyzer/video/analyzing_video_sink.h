...
class AnalyzingVideoSink : public rtc::VideoSinkInterface<VideoFrame> {
 public:
  struct SinkStats {
    ...
    // Collected only for frames with ID set.
  ...
  AnalyzingVideoSink(
      absl::string_view peer_name, Clock* clock,
 ...
 private:
  ...
  const bool collect_infra_stats_;
  Clock* const clock_;
  ...
  SinkStats sink_stats_ RTC_GUARDED_BY(mutex_);
};
