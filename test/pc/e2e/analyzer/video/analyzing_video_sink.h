...
class AnalyzingVideoSink : public rtc::VideoSinkInterface<VideoFrame> {
 ...
 private:
  ...
  Sinks* PopulateSinks(absl::string_view stream_label);

  const std::string peer_name_;
  Clock* const clock_;
  VideoQualityAnalyzerInterface* const analyzer_;
  AnalyzingVideoSinksSupport* const sinks_support_;

  Mutex mutex_;
  PeerConnectionE2EQualityTestFixture::VideoSubscription subscription_
      RTC_GUARDED_BY(mutex_);
  std::map<std::string, Sinks> stream_sinks_ RTC_GUARDED_BY(mutex_);
};
