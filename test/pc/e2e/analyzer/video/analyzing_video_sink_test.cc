...
class AnalyzingVideoSinkTest : public Test {
 protected:
  ...
  void SetUp() override {
    ...
    test_directory_ = test::JoinFilename(test::OutputPath(),
                                         "AnalyzingVideoSinkTest_TestDir");
...
TEST_F(AnalyzingVideoSinkTest, VideoFramesAreDumpedCorrectly) {
  ...
  {
    // `support` and `sink` has to be destroyed so all frames will be written
    // to the disk.
    AnalyzingVideoSinksSupport support;
    support.AddConfig("alice", video_config);
    AnalyzingVideoSink sink("bob", Clock::GetRealTimeClock(), analyzer, support,
                            subscription);
    sink.OnFrame(frame);
  }
...
TEST_F(AnalyzingVideoSinkTest,
...

  {
    ...
    AnalyzingVideoSinksSupport support;
    support.AddConfig("alice", video_config);
    AnalyzingVideoSink sink("bob", simulated_time.GetClock(), analyzer, support,
                            subscription);
    sink.OnFrame(frame1);
