...
void DefaultAudioQualityAnalyzer::OnStatsReports(
  ...
  for (auto& stat : stats) {
    ...
    audio_stream_stats.speech_expand_rate_new.AddSample((speech_concealed_samples - prev_speech_concealed_samples) / total_samples_diff);
...
void DefaultAudioQualityAnalyzer::Stop() {
  ...
  for (auto& item : streams_stats_) {
    ...
    //             "unitless", ImproveDirection::kSmallerIsBetter);
    ReportResult("accelerate_rate", item.first, item.second.accelerate_rate_new,
                 "unitless", ImproveDirection::kSmallerIsBetter);
    // ReportResult("preemptive_rate", item.first, item.second.preemptive_rate,
    //             "unitless", ImproveDirection::kSmallerIsBetter);
    ReportResult("preemptive_rate", item.first, item.second.preemptive_rate_new,
                 "unitless", ImproveDirection::kSmallerIsBetter);
