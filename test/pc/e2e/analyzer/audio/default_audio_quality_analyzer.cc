...
void DefaultAudioQualityAnalyzer::OnStatsReports(
  ...
  for (const StatsReport* stats_report : stats_reports) {
    ...
    std::printf("Stream label map (%s) -> (%s): \n",
                report_track_id->string_val().c_str(), stream_label.c_str());
...
void DefaultAudioQualityAnalyzer::OnStatsReports(
  ...
  for (auto& stat : stats) {
    ...
    audio_stream_stats.expand_rate_new.AddSample(
        ...
        total_samples_diff / time_between_samples_sec);
    ...
    if (sample.concealed_samples - prev_sample.concealed_samples > 0) {
...
void DefaultAudioQualityAnalyzer::Stop() {
  ...
  for (auto& item : streams_stats_) {
    ...
    //ReportResult("preferred_buffer_size_ms_new", item.first,
