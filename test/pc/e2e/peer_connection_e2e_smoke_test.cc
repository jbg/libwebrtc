...
ParameterizedTestParams() {
  return {{},
          {.enable_experiment_send_on_worker_thread = true},
          {.enable_experiment_send_on_worker_thread = true,
 ...
 public:
  ...
  RunParams CreateParameterizedRunParams(TimeDelta run_time) {
    ...
    run_params.use_network_thread_as_worker_thread =
        GetParam().use_network_thread_as_worker_thread;
    return run_params;
  };
...
INSTANTIATE_TEST_SUITE_P(
    ...
    [](const testing::TestParamInfo<
      ...
      if (info.param.use_network_thread_as_worker_thread) {
        description = "NetworkThreadIsWorkerThread";
      }
      if (info.param.enable_experiment_send_on_worker_thread) {
        description += "SendOntWorkerThreadEnabled";
      } else {
        description += "SendOntWorkerThreadDisabled";
      }
      return description;
    });
