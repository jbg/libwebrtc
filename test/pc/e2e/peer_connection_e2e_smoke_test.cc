...
std::unique_ptr<rtc::NetworkManager> CreateFakeNetworkManager(
  ...
  for (auto* endpoint : endpoints) {
    network_manager->AddInterface(
        rtc::SocketAddress(endpoint->GetPeerLocalAddress(), 0));
  }
...
TEST(PeerConnectionE2EQualityTestSmokeTest, RunWithEmulatedNetwork) {
  ...
  alice_video_config.width = 1024;
  alice_video_config.height = 768;
  ...
  // 150 = 30fps * 5s
  EXPECT_NEAR(video_analyzer->frames_captured(), 150, 5);
  EXPECT_NEAR(video_analyzer->frames_sent(), 150, 5);
  EXPECT_NEAR(video_analyzer->frames_received(), 150, 5);
  EXPECT_NEAR(video_analyzer->frames_rendered(), 150, 5);
}
