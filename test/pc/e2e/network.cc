...
TransparentNetworkNode::TransparentNetworkNode(std::string id)
    : webrtc::NetworkNode(id) {}

TransparentNetworkNode::~TransparentNetworkNode() = default;

void TransparentNetworkNode::DeliverPacketInternal(
...
void TransparentNetworkNode::Process(Timestamp cur_time) {
  ...
  for (auto& packet : to_send) {
    webrtc::NetworkReceiverInterface* receiver = nullptr;
    {
      rtc::CritScope crit(&lock_);
    ...
    receiver->DeliverPacket(std::move(packet));
  }
}

void TransparentNetworkNode::SetReceiver(
    std::string endpoint_id,
    webrtc::NetworkReceiverInterface* receiver) {
  rtc::CritScope crit(&lock_);
  routing_[endpoint_id] = receiver;
}
...
void EmulatedNetworkNode::DeliverPacketInternal(
    std::unique_ptr<EmulatedIpPacket> packet) {
...
EndpointNodeImpl::EndpointNodeImpl(std::string endpoint_id,
                                   webrtc::NetworkNode* send_node,
                                   webrtc::NetworkNode* receive_node)
    : webrtc::EndpointNode(endpoint_id),
      connected_endpoint_(nullptr),
      send_node_(send_node),
      receive_node_(receive_node),
      router_(absl::make_unique<Router>()),
      network_thread_(nullptr) {
  receive_node_->SetReceiver(endpoint_id, this);
}
...
void EndpointNodeImpl::SendPacket(const rtc::SocketAddress& from,
  ...
  send_node_->DeliverPacket(absl::make_unique<EmulatedIpPacket>(
...
void EndpointNodeImpl::SetPeerLocalAddress(
    absl::optional<rtc::SocketAddress> address) {
  peer_local_addr_ = address;
}

void EndpointNodeImpl::DeliverPacketInternal(
    std::unique_ptr<EmulatedIpPacket> packet) {
  ...
  webrtc::FakeNetworkSocket* socket = router_->GetSocket(packet->to.port());
  if (!socket) {
...
webrtc::NetworkNode* EndpointNodeImpl::GetSendNode() {
  return send_node_;
}
webrtc::NetworkNode* EndpointNodeImpl::GetReceiveNode() {
  return receive_node_;
}
