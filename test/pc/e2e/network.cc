...
void TransparentNetworkNode::Process(Timestamp cur_time) {
  std::vector<std::unique_ptr<EmulatedNetworkPacket>> to_send;
  {
    rtc::CritScope crit(&lock_);
...
EmulatedNetworkNode::StoredPacket::StoredPacket(
    uint64_t id,
    std::unique_ptr<webrtc::EmulatedNetworkPacket> packet,
    bool removed)
...
void EndpointNodeImpl::SendPacket(const rtc::SocketAddress& from,
  ...
  entry_node_->SendPacket(absl::make_unique<EmulatedNetworkPacket>(
