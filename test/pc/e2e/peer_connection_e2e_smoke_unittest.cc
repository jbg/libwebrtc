...
TEST(PeerConnectionE2EQualityTestSmokeTest, RunWithEmulatedNetwork) {
  ...
  alice_params->audio_config = AudioConfig{
      AudioConfig::Mode::kFile,
      webrtc::test::ResourcePath(
          "audio_processing/conversational_speech/EN_script2_M_sp1_A3", "wav"),
  ...
  ASSERT_GT(video_analyzer->frames_captured(), 0ul);
  ASSERT_GT(video_analyzer->frames_rendered(), 0ul);
}
