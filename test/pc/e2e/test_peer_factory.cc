...
std::unique_ptr<TestPeer> TestPeerFactory::CreateTestPeer(
  ...
  rtc::Thread* worker_thread = components->worker_thread;
  std::unique_ptr<rtc::Thread> owned_worker_thread =
      worker_thread ? nullptr : time_controller_.CreateThread("worker_thread");
  if (!worker_thread) {
    worker_thread = owned_worker_thread.get();
  }
