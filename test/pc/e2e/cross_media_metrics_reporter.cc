...
void CrossMediaMetricsReporter::Start(absl::string_view test_case_name,
...
void CrossMediaMetricsReporter::OnStatsReports(
  ...
  std::map<AVSyncGroupKey, std::vector<const RTCInboundRTPStreamStats*>>
  ...
  for (const auto& stat : inbound_stats) {
    ...
    if (stat->estimated_playout_timestamp.ValueOrDefault(0.) > 0 &&
      ...
      AVSyncGroupKey key{Timestamp::Micros(stat->timestamp_us()),
                         analyzer_helper_->GetStreamLabelFromTrackId(
                             *media_source_stat->track_identifier)};
      statsByTimestampAndTrackId[std::move(key)].push_back(stat);
    }
  }
  ...
  for (const auto& pair : statsByTimestampAndTrackId) {
    ...
    if (audio_video_playout_diff > 0) {
      stats_[pair.first.stream_label].audio_ahead_ms.AddSample(
          audio_video_playout_diff);
      stats_[pair.first.stream_label].video_ahead_ms.AddSample(0);
    } else {
      stats_[pair.first.stream_label].audio_ahead_ms.AddSample(0);
      stats_[pair.first.stream_label].video_ahead_ms.AddSample(
          std::abs(audio_video_playout_diff));
    }
  }
}

void CrossMediaMetricsReporter::Stop() {
  rtc::CritScope cs(&lock_);
  for (const auto& pair : stats_) {
    ReportResult("audio_ahead_ms", GetTestCaseName(pair.first),
                 pair.second.audio_ahead_ms, "ms",
                 webrtc::test::ImproveDirection::kSmallerIsBetter);
    ReportResult("video_ahead_ms", GetTestCaseName(pair.first),
...
std::string CrossMediaMetricsReporter::GetTestCaseName(
  ...
  return std::string(test_case_name_) + "/" + std::string(stream_label);
}
