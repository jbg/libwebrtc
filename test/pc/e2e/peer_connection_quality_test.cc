...
void PeerConnectionE2EQualityTest::WaitUntilIceCandidatesGathered() {
  ...
  ASSERT_TRUE(time_controller_.Wait(
      [&]() {
        return alice_->signaling_state() ==
               webrtc::PeerConnectionInterface::kStable;
      },
      kDefaultTimeout));
  ASSERT_TRUE(time_controller_.Wait(
      [&]() {
        return bob_->signaling_state() ==
               webrtc::PeerConnectionInterface::kStable;
      },
      kDefaultTimeout));
  ASSERT_TRUE(time_controller_.Wait(
      [&]() { return alice_->IsIceGatheringDone(); }, kDefaultTimeout));
  ASSERT_TRUE(time_controller_.Wait(
      [&]() { return bob_->IsIceGatheringDone(); }, kDefaultTimeout));
}

void PeerConnectionE2EQualityTest::WaitUntilPeersAreConnected() {
  // This means that ICE and DTLS are connected.
  time_controller_.Wait([&]() { return alice_->IsIceConnected(); },
                        kDefaultTimeout);
  alice_connected_ = alice_->IsIceConnected();
  time_controller_.Wait([&]() { return bob_->IsIceConnected(); },
                        kDefaultTimeout);
