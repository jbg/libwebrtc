...
class FrameGeneratorTest : public ::testing::Test {
 ...
 protected:
  ...
  void WriteNV12File(FILE* file, uint8_t y, uint8_t u, uint8_t v) {
    RTC_DCHECK(file);
    constexpr size_t size_y = static_cast<size_t>(kFrameWidth) * kFrameHeight;
    constexpr size_t size_uv =
        static_cast<size_t>(kFrameWidth + kFrameWidth % 2) *
        ((kFrameHeight + 1) / 2);
    uint8_t plane_buffer[size_y];

    memset(&plane_buffer, y, size_y);
    fwrite(&plane_buffer, 1, size_y, file);
    for (size_t i = 0; i < size_uv / 2; ++i) {
      plane_buffer[2 * i] = u;
      plane_buffer[2 * i + 1] = v;
    }
    fwrite(&plane_buffer, 1, size_uv, file);
  }
