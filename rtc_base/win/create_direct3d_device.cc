...
HRESULT CreateDirect3DDeviceFromDXGIDevice(
  ...
  HRESULT hr = create_d3d11_device_func(dxgi_device.Get(), &inspectableSurface);
  if (FAILED(hr))
    return hr;
  ...
  Microsoft::WRL::ComPtr<
      ABI::Windows::Graphics::DirectX::Direct3D11::IDirect3DDevice>
      temp_d3d11_device;
  hr = inspectableSurface->QueryInterface(IID_PPV_ARGS(&temp_d3d11_device));
  if (FAILED(hr))
    return hr;

  *out_d3d11_device = std::move(temp_d3d11_device);
  return hr;
}
