...
class OperationTracker {
 public:
  ...
  std::function<void(scoped_refptr<OperationsChainCallback>)>
  BindSynchronousOperation(Event* operation_complete_event) {
    return [this, operation_complete_event](
  ...
  std::function<void(scoped_refptr<OperationsChainCallback>)>
  BindAsynchronousOperation(Event* unblock_operation_event,
                            Event* operation_complete_event) {
    return [this, unblock_operation_event, operation_complete_event](
               scoped_refptr<OperationsChainCallback> callback) {
 ...
 private:
  ...
  void StartSynchronousOperation(
      Event* operation_complete_event,
      scoped_refptr<OperationsChainCallback> callback) {
...
TEST(OperationsChainTest, OperationNotInvokingCallbackShouldCrash) {
  scoped_refptr<OperationsChain> operations_chain = OperationsChain::Create();
  EXPECT_DEATH(operations_chain->ChainOperation(
                   [](scoped_refptr<OperationsChainCallback> callback) {}),
               "");
}
