...
void RunOnThread(Thread& thread, absl::AnyInvocable<void() &&> task) {
  rtc::Event event;
  thread.PostTask([&] {
    std::move(task)();
    event.Set();
  });
  ASSERT_TRUE(event.Wait(webrtc::TimeDelta::Seconds(5)));
}
...
void TestSend(SocketServer* internal,
  ...
  TestClient* in;
  RunOnThread(th_int, [&] { in = CreateTestClient(natsf, internal_addr); });
  ...
  RunOnThread(th_int, [&] { in->SendTo(buf, len, out[0]->address()); });
  ...
  for (int i = 1; i < 4; i++) {
    RunOnThread(th_int, [&] { in->SendTo(buf, len, out[i]->address()); });
...
void TestRecv(SocketServer* internal,
  ...
  RunOnThread(th_ext, [&] { out[1]->SendTo(buf, len, trans_addr); });
  RunOnThread(th_int,
              [&] { EXPECT_TRUE(CheckReceive(in, !filter_ip, buf, len)); });
  RunOnThread(th_ext, [&] { out[2]->SendTo(buf, len, trans_addr); });  // fails?

  RunOnThread(th_int,
              [&] { EXPECT_TRUE(CheckReceive(in, !filter_port, buf, len)); });

  RunOnThread(th_ext, [&] { out[3]->SendTo(buf, len, trans_addr); });
