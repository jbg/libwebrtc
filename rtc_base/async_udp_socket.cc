...
AsyncUDPSocket::AsyncUDPSocket(Socket* socket)
    : socket_(socket), size_(BUF_SIZE), buf_(new char[size_]) {
...
void AsyncUDPSocket::OnReadEvent(Socket* socket) {
  ...
  int64_t timestamp = -1;
  int len = socket_->RecvFrom(buf_.get(), size_, &remote_addr, &timestamp);
  ...
  SignalReadPacket(this, buf_.get(), static_cast<size_t>(len), remote_addr,
                   timestamp);
}
