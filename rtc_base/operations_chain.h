...
class OperationsChain : public rtc::RefCountInterface {
 public:
  ...
  ~OperationsChain() {
    ...
    RTC_DCHECK(pending_operation_ids_.empty());
  }
    ...
    chained_operations_.push_back(
        std::unique_ptr<rtc_operations_chain_internal::Operation>(
            new rtc_operations_chain_internal::OperationWithFunctor<FunctorT>(
                std::forward<FunctorT>(functor), CreateOperationCallback())));
 ...
 private:
  std::function<void()> CreateOperationCallback() {
    RTC_DCHECK_RUN_ON(&sequence_checker_);
    size_t operation_id = next_operation_id_++;
    pending_operation_ids_.insert(operation_id);
    // The callback keeps |this| alive until the operation has completed.
    return
        [this_ref = rtc::scoped_refptr<OperationsChain>(this), operation_id]() {
          RTC_DCHECK_RUN_ON(&this_ref->sequence_checker_);
          auto it = this_ref->pending_operation_ids_.find(operation_id);
          RTC_DCHECK(it != this_ref->pending_operation_ids_.end())
              << "The operation executing on the OperationsChain has already "
                 "invoked its callback.";
          this_ref->pending_operation_ids_.erase(it);
  ...
  // Used to ensure that an operation does not invoke its completion callback
  // multiple times.
  size_t next_operation_id_ RTC_GUARDED_BY(sequence_checker_) = 0;
  std::set<size_t> pending_operation_ids_ RTC_GUARDED_BY(sequence_checker_);
