...
class OperationsChain final : public RefCountedObject<RefCountInterface> {
 public:
  ...
  // - FunctorT is movable.
  // - FunctorT implements
  //   "T operator()(scoped_refptr<OperationsChainCallback> callback)" or
  //   "T operator()(scoped_refptr<OperationsChainCallback> callback) const" for
  //   some T (if T is not void, the return value is discarded in the invoking
  //   sequence). The operator starts the operation; when the operation is
  //   complete, "callback" MUST be invoked, and it MUST be so on the sequence
  //   that ChainOperation() was invoked on.
  //
