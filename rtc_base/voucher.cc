...
Voucher::Annex::Id Voucher::Annex::GetNextId() {
  static std::atomic<Voucher::Annex::Id> current_id = 0;
  auto id = current_id.fetch_add(1);
  RTC_CHECK(id < Voucher::kAnnexCapacity);
  return id;
}
...
Voucher::Voucher() : annex_(Voucher::kAnnexCapacity) {}
...
void Voucher::SetAnnex(Annex::Id id, std::unique_ptr<Annex> annex) {
