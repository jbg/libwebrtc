...
template <typename U, int64_t MaxVal = int64_t{1} << 32>
...
class UnwrapperConformanceFixture : public Test {
 public:
  ...
  static constexpr int64_t kMaxIncrease = (kMaxVal - 1) / 2;
  static constexpr int64_t kMaxBackwardsIncrease = kMaxVal - kMaxIncrease;
...
TYPED_TEST_P(UnwrapperConformanceFixture, PositiveWrapAround) {
  ...
  EXPECT_EQ(TestFixture::kMaxVal - 2,
            this->ref_unwrapper_.Unwrap(2 * TestFixture::kMaxIncrease));
  // Wrapped 0 is an increase of kMaxVal - 2 to kMaxVal.
  EXPECT_EQ(TestFixture::kMaxVal, this->ref_unwrapper_.Unwrap(0));
  EXPECT_EQ(TestFixture::kMaxVal + TestFixture::kMaxIncrease,
            this->ref_unwrapper_.Unwrap(TestFixture::kMaxIncrease));
}
...
TYPED_TEST_P(UnwrapperConformanceFixture, NegativeUnwrap) {
  ...
  EXPECT_EQ(-2, this->ref_unwrapper_.Unwrap(TestFixture::kMaxVal - 2));
...
TYPED_TEST_P(UnwrapperConformanceFixture, MultiplePositiveWrapArounds) {
  ...
  for (int i = 0; i < 16; ++i) {
    ...
    wrapped_val =
        (wrapped_val + TestFixture::kMaxIncrease) % TestFixture::kMaxVal;
  }
}
...
TYPED_TEST_P(UnwrapperConformanceFixture, MultipleNegativeWrapArounds) {
  ...
  for (int i = 0; i < 16; ++i) {
    ...
    val -= TestFixture::kMaxIncrease;
    wrapped_val =
        (wrapped_val + this->kMaxBackwardsIncrease) % TestFixture::kMaxVal;
  }
}
...
constexpr int64_t k15BitMax = int64_t{1} << 15;
using UnwrapperTypes = ::testing::Types<
    ...
    FixtureParams<webrtc::SeqNumUnwrapper<uint32_t, k15BitMax>, k15BitMax>>;
 ...
 public:
  ...
  static std::string GetName(int) {
    if constexpr (std::is_same<typename T::Unwrapper,
      ...
      return "rtc::TimestampWrapAroundHandler";
    if constexpr (std::is_same<typename T::Unwrapper,
                               webrtc::TimestampUnwrapper>())
      return "webrtc::TimestampUnwrapper";
    if constexpr (std::is_same<typename T::Unwrapper,
                               webrtc::SeqNumUnwrapper<uint32_t>>())
      return "webrtc::SeqNumUnwrapper";
    if constexpr (std::is_same<typename T::Unwrapper,
                               webrtc::SeqNumUnwrapper<uint32_t, k15BitMax>>())
      return "webrtc::SeqNumUnwrapper(15-bit)";
    if constexpr (std::is_same<typename T::Unwrapper,
                               UnwrapperHelper<TestSequence::Unwrapper>>())
      return "UnwrappedSequenceNumber::Unwrapper";
  }
};
