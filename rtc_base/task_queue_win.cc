...
class DelayedTaskInfo {
 public:
  ...
  DelayedTaskInfo() {}
  DelayedTaskInfo(Timestamp due_time, absl::AnyInvocable<void() &&> task)
      : due_time_(due_time), task_(std::move(task)) {}
 ...
 private:
  Timestamp due_time_ = Timestamp::MinusInfinity();
...
void TaskQueueWin::PostDelayedTask(absl::AnyInvocable<void() &&> task,
  ...
  auto* task_info = new DelayedTaskInfo(CurrentTime() + delay, std::move(task));
  RTC_CHECK(thread_.GetHandle() != absl::nullopt);
