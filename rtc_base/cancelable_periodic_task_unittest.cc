...
class MockClosure {
 public:
  MOCK_METHOD0(Call, absl::optional<int>());
...
class MoveOnlyClosure {
 public:
  ...
  absl::optional<int> operator()() { return mock_->Call(); }
...
TEST(CancelablePeriodicTaskTest, CancelTaskAfterItRuns) {
  rtc::Event done(false, false);
  MockClosure mock;
  EXPECT_CALL(mock, Call).WillOnce(Return(absl::nullopt));
  EXPECT_CALL(mock, Delete).WillOnce(Invoke([&done] { done.Set(); }));
...
TEST(CancelablePeriodicTaskTest, StartPeriodicTask) {
  MockFunction<absl::optional<int>()> closure;
  rtc::Event done(false, false);
...
TEST(CancelablePeriodicTaskTest, DeletingHandleDoesnStopTheTask) {
  rtc::Event run(false, false);
  rtc::TaskQueue task_queue("queue");
  auto task = rtc::CreateCancelablePeriodicTask(([&]() -> absl::optional<int> {
    run.Set();
    return absl::nullopt;
  }));
  rtc::CancelableTaskHandle handle = task->GetCancellationHandle();
...
TEST(CancelablePeriodicTaskTest, Example) {
  class ObjectOnTaskQueue {
   public:
    void DoPeriodicTask() {}
    absl::optional<int> TimeUntilNextRunMs() { return 100; }
