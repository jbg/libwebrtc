...
class RefCounter {
 public:
  ...
  void IncRef() {
    // No barrier: if this is the first reference current thread should be the
    // only thread that has access to the object protected by the reference
    ...
    // in synchronization that makes the ref_count_ one or zero.
