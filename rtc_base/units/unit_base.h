...
// what scale it represents. It's used like: class MyUnit: public
// UnitBase<MyUnit> {/*Add accessors.*/}; Unit_T represents the specific
...
class UnitBase {
 ...
 protected:
  ...
  static Unit_T FromFraction(T value) {
    RTC_DCHECK_GT(value,
                  Unit_T::one_sided ? 0 : MinusInfinityVal() / Denominator);
...
// that have a meaningful relation between values such that a/b or a += b is a
