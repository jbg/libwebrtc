...
class RTC_EXPORT PhysicalSocketServer : public SocketServer {
 ...
 private:
  ...
  int epoll_fd_ = INVALID_SOCKET;  // Assigned in constructor only
  ...
  std::unique_ptr<Signaler> signal_wakeup_;  // Assigned in constructor only
  CriticalSection crit_;
  bool fWait_;  // Unclear thread usage.
#if defined(WEBRTC_WIN)
  WSAEVENT socket_ev_;  // Assigned in constructor only
#endif
};
