...
class RepeatingTaskBase : public rtc::QueuedTask {
 ...
 private:
  ...
  void PostStop();
  void PostOrDelay(TimeDelta delay);
...
class RepeatingTask {
 public:
  ...
  // RepeatingTask.
  template <class Closure>
  void Start(rtc::TaskQueue* task_queue,
             TimeDelta first_delay,
             IntervalMode interval_mode,
             Closure&& closure) {
    ...
    repeating_task_ =
        new webrtc_repeating_task_impl::RepeatingTaskImpl<Closure>(
            task_queue, first_delay, interval_mode,
            std::forward<Closure>(closure));
    // Transfers ownership to the task queue.
    repeating_task_->PostOrDelay(first_delay);
  }
  template <class Closure>
  void Start(TimeDelta first_delay,
             IntervalMode interval_mode,
             Closure&& closure) {
    Start(rtc::TaskQueue::Current(), first_delay, interval_mode,
          std::forward<Closure>(closure));
  }
  template <class Closure>
  void Start(rtc::TaskQueue* task_queue,
             TimeDelta first_delay,
             Closure&& closure) {
    Start(task_queue, first_delay, IntervalMode::kIncludingExecution,
          std::forward<Closure>(closure));
  }
  template <class Closure>
  void Start(TimeDelta first_delay, Closure&& closure) {
    Start(rtc::TaskQueue::Current(), first_delay,
          IntervalMode::kIncludingExecution, std::forward<Closure>(closure));
  }
  template <class Closure>
  void Start(Closure&& closure) {
    Start(rtc::TaskQueue::Current(), TimeDelta::Zero(),
          IntervalMode::kIncludingExecution, std::forward<Closure>(closure));
  }
