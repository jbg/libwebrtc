...
static void PrintCert(BoringSSLCertificate* cert) {
  // Since we're using CRYPTO_BUFFER, we can't use X509_print_ex, so will just
...
bool AddTime(CBB* cbb, time_t time) {
  ...
  if (!ASN1_TIME_set(asn1_time, time)) {
    goto error;
  }
...
// fields), primarily for the purpose of extracting signature_algorithm and
// expiration_time.
...
std::unique_ptr<BoringSSLCertificate> BoringSSLCertificate::FromPEMString(
  ...
  std::string der;
  if (!SSLIdentity::PemToDer(kPemTypeCertificate, pem_string, nullptr)) {
    return nullptr;
  }
  bssl::UniquePtr<CRYPTO_BUFFER> cert_buffer(
...
#define OID_MATCHES(oid, oid_other)   \
  (oid.size() == sizeof(oid_other) && \
   0 == memcmp(&oid[0], oid_other, sizeof(oid_other)))
...
bool BoringSSLCertificate::GetSignatureDigestAlgorithm(
  ...
  if (!ParseCertificate(cert_buffer_.get(), &oid, nullptr)) {
...
bool BoringSSLCertificate::ComputeDigest(const CRYPTO_BUFFER* cert_buffer,
  ...
  if (!EVP_Digest(CBS_data(&cbs), CBS_len(&cbs), digest, &n, md, nullptr)) {
    return false;
  }
  *length = n;
  return true;
...
std::string BoringSSLCertificate::ToPEMString() const {
  CBS cbs;
  CRYPTO_BUFFER_init_CBS(cert_buffer_.get(), &cbs);
  return OpenSSLIdentity::DerToPem(kPemTypeCertificate, CBS_data(&cbs),
                                   CBS_len(&cbs));
}
...
void BoringSSLCertificate::ToDER(Buffer* der_buffer) const {
  CBS cbs;
  CRYPTO_BUFFER_init_CBS(cert_buffer_.get(), &cbs);
  der_buffer->SetData(CBS_data(&cbs), CBS_len(&cbs));
}

bool BoringSSLCertificate::operator==(const BoringSSLCertificate& other) const {
  CBS cbs, cbs_other;
  CRYPTO_BUFFER_init_CBS(cert_buffer_.get(), &cbs);
  CRYPTO_BUFFER_init_CBS(other.cert_buffer_.get(), &cbs_other);
  return CBS_len(&cbs) == CBS_len(&cbs_other) &&
         0 == memcmp(CBS_data(&cbs), CBS_data(&cbs_other), CBS_len(&cbs));
}
...
int64_t BoringSSLCertificate::CertificateExpirationTime() const {
  int64_t ret;
  if (!ParseCertificate(cert_buffer_.get(), nullptr, &ret)) {
    RTC_LOG(LS_ERROR) << "Failed to parse certificate.";
    return -1;
  }
  return ret;
