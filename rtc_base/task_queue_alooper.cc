...
class TaskQueue::Impl : public RefCountInterface {
 ...
 private:
  ...
  static int OnWakeup(int socket, int events, void* context);  // NOLINT
  static void RunTask(int fd, int events, void* context);      // NOLINT
...
void TaskQueue::Impl::PostDelayedTask(std::unique_ptr<QueuedTask> task,
  ...
  // This is true if the task will be the first to be executed, in this case we
  // will need to wake the poll loop.
  bool needs_wakeup_ = false;
  {
    CritScope lock(&pending_lock_);
...
void TaskQueue::Impl::ThreadMain(void* context) {
  ...
  while (queue_context.is_active) {
    ALooper_pollOnce(next_timeout_ms, nullptr /* outFd */,
                     nullptr /* outEvents */, nullptr /* outData */);

    if (!queue_context.is_active)
      break;
    ...
    while (true) {
      ...
      std::unique_ptr<QueuedTask> task_to_run;
      {
        ...
        auto it = me->pending_timer_events_.begin();

        if (it == me->pending_timer_events_.end())
          break;

        if (it->when <= current_time) {
          task_to_run = std::move(it->task);
          me->pending_timer_events_.pop_front();
        } else {
          next_timeout_ms = it->when - current_time;
          break;
        }
      }
...
// static
int TaskQueue::Impl::OnWakeup(int socket,
                              int events,
                              void* context) {  // NOLINT
  QueueContext* ctx =
      static_cast<QueueContext*>(pthread_getspecific(GetQueuePtrTls()));
  ...
  switch (buf) {
    case kQuit:
      ctx->is_active = false;
      ALooper_wake(ctx->queue->alooper_);
      break;
    case kRunTask: {
      std::unique_ptr<QueuedTask> task;
      {
...
// static
void TaskQueue::Impl::RunTask(int fd, int events, void* context) {  // NOLINT
