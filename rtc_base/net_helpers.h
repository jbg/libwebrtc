...
class RTC_EXPORT AsyncResolver : public AsyncResolverInterface {
 ...
 private:
  enum State { IDLE, SIGNALLING_COMPLETION, PLEASE_DESTRUCT };

  void ResolveDone(std::vector<IPAddress> addresses, int error)
      RTC_EXCLUSIVE_LOCKS_REQUIRED(sequence_checker_);
  void MaybeSelfDestruct() RTC_EXCLUSIVE_LOCKS_REQUIRED(sequence_checker_);
  ...
  bool recursion_check_ RTC_GUARDED_BY(sequence_checker_) =
