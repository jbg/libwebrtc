...
class LifeCycleFunctor {
 public:
  ...
  LifeCycleFunctor(const LifeCycleFunctor& other)
      : stats_(nullptr), event_(nullptr) {
    *this = other;
  }
  LifeCycleFunctor(LifeCycleFunctor&& other)
      : stats_(nullptr), event_(nullptr) {
    *this = std::move(other);
  }
 ...
 public:
  DestructionFunctor(Thread* thread, bool* thread_was_current, Event* event)
      : thread_(thread),
        ...
        event_(event) {}
  DestructionFunctor(const DestructionFunctor& other)
      : thread_(nullptr), thread_was_current_(nullptr), event_(nullptr) {
    *this = other;
  }
  DestructionFunctor(DestructionFunctor&& other)
      : thread_(nullptr), thread_was_current_(nullptr), event_(nullptr) {
    *this = std::move(other);
  }
  ~DestructionFunctor() {
  ...
  DestructionFunctor& operator=(const DestructionFunctor& other) {
    thread_ = other.thread_;
    thread_was_current_ = other.thread_was_current_;
    event_ = other.event_;
    return *this;
  }

  DestructionFunctor& operator=(DestructionFunctor&& other) {
    thread_ = other.thread_;
    thread_was_current_ = other.thread_was_current_;
    event_ = other.event_;
    return *this;
