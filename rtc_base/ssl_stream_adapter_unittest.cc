...
class SSLStreamAdapterTestBase : public ::testing::Test,
 ...
 public:
  ...
  void TestHandshakeWithDelayedIdentity(bool valid_identity) {
    ...
    uint8_t packet[1];
    size_t sent;
    EXPECT_EQ(rtc::SR_BLOCK, client_ssl_->Write(packet, &sent, 0));
    EXPECT_EQ(rtc::SR_BLOCK, server_ssl_->Write(packet, &sent, 0));
    ...
    if (valid_identity) {
      ...
      EXPECT_EQ(rtc::SR_SUCCESS, client_ssl_->Write(packet, &sent, 0));
      EXPECT_EQ(rtc::SR_BLOCK, server_ssl_->Read(packet, 0, 0));
    } else {
