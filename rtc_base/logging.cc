...
ABSL_CONST_INIT std::atomic<webrtc::Mutex*> g_log_mutex(nullptr);
static_assert(std::is_trivially_destructible<decltype(g_log_mutex)>::value,
...
webrtc::Mutex& GetLoggingLock() {
  webrtc::Mutex* mutex = g_log_mutex.load(std::memory_order_acquire);
  if (mutex != nullptr) {
    std::unique_ptr<webrtc::Mutex> nex_mutex(new webrtc::Mutex());
    webrtc::Mutex* expected = nullptr;
    if (g_log_mutex.compare_exchange_strong(expected, nex_mutex.get())) {
      // The ownership of nex_mutex is passed to g_log_mutex. Return it.
      mutex = nex_mutex.release();
    } else {
      // g_log_mutex is now holding non-null mutex.
      mutex = g_log_mutex.load();
    }
  }
  return *mutex;
}
