...
void TestClient::OnPacket(AsyncPacketSocket* socket,
  ...
  webrtc::MutexLock lock(&mutex_);
  packets_.push_back(std::make_unique<Packet>(packet));
}
...
TestClient::Packet::Packet(const rtc::ReceivedPacket& packet)
    : addr(packet.source_address()),
      buf(packet.payload().data(), packet.payload().size()),
      packet_time(packet.arrival_time()) {}
