...
TEST(RateControlSettingsTest, PacingFactor) {
  ...
  EXPECT_EQ(RateControlSettings::ParseFromFieldTrials().GetPacingFactor(), 1.2);
}
...
TEST(RateControlSettingsTest, GetSimulcastHysteresisFactor) {
  const RateControlSettings settings_before =
      RateControlSettings::ParseFromFieldTrials();
  EXPECT_EQ(settings_before.GetSimulcastHysteresisFactor(
  ...
  EXPECT_EQ(settings_after.GetSimulcastHysteresisFactor(
                VideoCodecMode::kRealtimeVideo),
            1.2);
  EXPECT_EQ(settings_after.GetSimulcastHysteresisFactor(
                VideoEncoderConfig::ContentType::kRealtimeVideo),
