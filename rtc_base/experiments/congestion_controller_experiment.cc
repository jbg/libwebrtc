...
CongestionControllerExperiment::GetBbrExperimentConfig() {
  ...
  if (sscanf(trial_string.c_str(),
             "Enabled,BBR,"
             "%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,"
             "%lf,%lf,%lf,%lf,%lf,%lf",
             &config.exit_startup_on_loss, &config.exit_startup_on_loss,
             &config.exit_startup_rtt_threshold_ms, &config.fully_drain_queue,
             &config.initial_conservation_in_startup, &config.num_startup_rtts,
             &config.probe_rtt_based_on_bdp,
             &config.probe_rtt_disabled_if_app_limited,
             &config.probe_rtt_skipped_if_similar_rtt,
             &config.rate_based_recovery, &config.rate_based_startup,
             &config.slower_startup, &config.encoder_rate_gain,
             ...
             &config.probe_rtt_congestion_window_gain) == 18) {
    return config;
  } else {
    return rtc::nullopt;
  }
