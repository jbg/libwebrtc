  ...
  for (; now <= deadline; now = Now()) {
    ...
    Message message;
    {
      ...
      if (!messages_.empty()) {
        ...
        messages_.pop_front();
      } else if (!delayed_messages_.empty()) {
        const DelayedMessage& next = delayed_messages_.top();
        if (now < next.run_at) {
          delay_until = std::min(deadline, next.run_at);
        } else {
          message = next.msg;
    ...
    // Quit if got to deadline when there are no ready tasks just now.
    // Tests may call ProcessMessages to run tasks that ready now and
    // advance time manually. This checks avoid infinite busy loop in such case.
    if (now == deadline)
      break;

    // Wait and multiplex in the meantime
    RTC_DCHECK_GT(delay_until, now);
    if (!ss_->Wait(delay_until - now, /*process_io=*/true))
      return !IsQuitting();
  }
  return true;
}
