...
// This file is derived from Chromium's base/scoped_generic.h.

#ifndef RTC_BASE_SCOPED_GENERIC_H_
#define RTC_BASE_SCOPED_GENERIC_H_
...
class ScopedGeneric {
 ...
 public:
  ...
  virtual ~ScopedGeneric() {
    RTC_CHECK(!receiving_) << "ScopedGeneric destroyed with active receiver";
    FreeIfNecessary();
  }
  ...
  class Receiver {
   public:
    explicit Receiver(ScopedGeneric& parent) : scoped_generic_(&parent) {
      RTC_CHECK(!scoped_generic_->receiving_)
    ...
    Receiver(Receiver&& move) {
      RTC_CHECK(!used_) << "moving into already-used Receiver";
      RTC_CHECK(!move.used_) << "moving from already-used Receiver";
...
#endif  // RTC_BASE_SCOPED_GENERIC_H_
