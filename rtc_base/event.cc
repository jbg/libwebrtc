...
#if defined(WEBRTC_WIN)
class EventImpl : public EventInterface {
  EventImpl(bool manual_reset, bool initially_signaled) {
    event_handle_ = ::CreateEvent(nullptr,  // Security attributes.
                                  manual_reset, initially_signaled,
                                  nullptr);  // Name.
    RTC_CHECK(event_handle_);
  }
  ~EventImpl() override { CloseHandle(event_handle_); }
  void Set() override { SetEvent(event_handle_); }

  void Reset() override { ResetEvent(event_handle_); }

  bool Wait(const int give_up_after_ms, int /*warn_after_ms*/) override {
...
bool EventImpl::Wait(const int give_up_after_ms, const int warn_after_ms) {
  ...
  const absl::optional<timespec> warn_ts =
      warn_after_ms == Event::kForever ||
              (give_up_after_ms != Event::kForever &&
               warn_after_ms > give_up_after_ms)
          ? absl::nullopt
...
CreateNativeEventImpl(bool manual_reset, bool initially_signaled) {
  return absl::make_unique<EventImpl>(manual_reset, initially_signaled);
}
...
Event::Event(bool manual_reset, bool initially_signaled)
    : impl_(
          ScopedYieldPolicy::Active()
              ? ScopedYieldPolicy::CreateEvent(manual_reset, initially_signaled)
              : CreateNativeEventImpl(manual_reset, initially_signaled)) {}
...
bool Event::Wait(int give_up_after_ms, int warn_after_ms) {
  return impl_->Wait(give_up_after_ms, warn_after_ms);
}
