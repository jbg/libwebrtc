...
timespec GetTimespec(webrtc::TimeDelta duration_from_now) {
  timespec ts;
  ...
  ts.tv_nsec +=
      (microsecs_from_now % kNumMicrosecsPerSec) * rtc::kNumNanosecsPerMicrosec;
...
bool Event::Wait(webrtc::TimeDelta give_up_after,
  ...
  const absl::optional<timespec> warn_ts =
      warn_after.IsPlusInfinity() ||
              (!give_up_after.IsPlusInfinity() && warn_after > give_up_after)
          ? absl::nullopt
