...
#if defined(WEBRTC_MAC)
  // This allows us to use SequencedChecker to check calls that comes from
  // system callbacks such as those from a capture device even if the callbacks
  // might come on different underlying threads. Note that on other platforms,
  // the equivalent calls will have to come from a consistent thread as we will
  // rely on the thread id rather than the queue.
