...
// Sets the current thread-local yield handler while it's in scope and reverts
// to the previous handler when it leaves the scope.
class ScopedYieldPolicy final {
 public:
  explicit ScopedYieldPolicy(YieldInterface* handler);
  ~ScopedYieldPolicy();
