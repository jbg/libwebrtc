...
class MdnsResponderInterface {
 public:
  ...
  // the given address is returned and its reference count is incremented by
  // one.
  ...
  // association between the address and its mapped name, and returns true via
  // |callback| if the decremented reference count reaches zero. Otherwise no
  // operation is done and false is returned.
