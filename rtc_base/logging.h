...
template <
    class T,
    class T1 = absl::decay_t<T>,
    absl::enable_if_t<
        std::is_class<T1>::value &&
        !absl::disjunction<std::is_same<T1, std::string>,
                           std::is_same<T1, LogMetadata>,
                           has_to_log_string<T1>,
#ifdef WEBRTC_ANDROID
                           std::is_same<T1, LogMetadataTag>,
#endif
...
template <class T, absl::enable_if_t<has_to_log_string<T>::value>* = nullptr>
ToStringVal MakeVal(T x) {
