...
class ReadableStreamBase : protected SourceController {
 protected:
  ...
  // Called by ReadableStreamController<T>::Enqueue().
  void OnEnqueue();

  // Called by ReadableStreamController<T>::IsReady() if false.
...
class ReadableStreamPushSource final : public ReadableStream<T> {
 public:
  ...
  void Enqueue(T chunk) override { controller_->Enqueue(std::move(chunk)); }
