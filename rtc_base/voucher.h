...
class RTC_EXPORT Voucher {
 public:
  ...
  // For Attachments: stores an annex into a voucher. If one is already present,
  // it gets replaced.
  void SetAttachment(Attachment::Id id, std::unique_ptr<Attachment> annex);
 ...
 private:
  ...
  Mutex mu_;
  absl::InlinedVector<std::unique_ptr<Attachment>, kAttachmentCapacity> annex_
      RTC_GUARDED_BY(&mu_);
};
