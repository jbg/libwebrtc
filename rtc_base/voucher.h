...
// are posted, the current voucher is inherited and set as current to the new
...
class Voucher {
 public:
  ...
  // Vouchers aggregate annexes, which are application-specific attachments that
  // perform logic unrelated to the mechanics of Voucher progression.
  class Annex {
   public:
    using Id = size_t;

    // Annexes should call this function one to get an ID to use with SetAnnex.
    static Annex::Id GetNextId();

    virtual ~Annex() = default;
  };

  // Scoped setter that saves the current voucher on stack an instates a new
  ...
  // For Annexes: stores an annex into a voucher. If one is already present,
  // it gets replaced.
