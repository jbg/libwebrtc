...
class RTC_LOCKABLE Thread : public MessageQueue {
 ...
 private:
  ...
  // Used to check access to unguarded thread control state variables.
  // This might represent a 'parent' thread from which Start()/Stop() are called
  // or it could represent the worker thread itself in case the thread is
  // wrapped since the functions that modified the control state will in this
  // case be called from that thread.
  ...
  // this instance ore not. (i.e. owned_ == !wrapped).
