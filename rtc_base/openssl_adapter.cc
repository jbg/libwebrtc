...
void OpenSSLAdapter::SSLInfoCallback(const SSL* s, int where, int value) {
  ...
  if (info_log) {
    char buf[256];
    SimpleStringBuilder log(buf);
    log << type << " " << SSL_state_string_long(s);
    RTC_LOG(LS_INFO) << log.str();
  }
  if (alert_log) {
    char buf[512];
    SimpleStringBuilder log(buf);
    log << type << " " << SSL_alert_type_string_long(value) << " "
        << SSL_alert_desc_string_long(value) << " " << SSL_state_string_long(s);
    RTC_LOG(LS_WARNING) << log.str();
  }
}
