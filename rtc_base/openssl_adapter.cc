...
#define USE_CRYPTO_BUFFER_CALLBACK
#endif
...
#endif
#ifndef USE_CRYPTO_BUFFER_CALLBACK
...
OpenSSLAdapter::OpenSSLAdapter(AsyncSocket* socket,
  ...
  // Otherwistene, we'll create our own later.
...
#ifdef USE_CRYPTO_BUFFER_CALLBACK
...
#else
  bool is_valid_cert_name =
      openssl::VerifyPeerCertMatchesHost(ssl, host) && (custom_cert_verifier_status_ || ignore_bad_cert_);
#endif
...
enum ssl_verify_result_t OpenSSLAdapter::SSLVerifyCallback(SSL* ssl,
                                                           uint8_t* out_alert) {
  ssl_verify_result_t ret = ssl_verify_invalid;
  ...
  if (stream->ssl_cert_verifier_ != nullptr) {
    ...
    if (sk_CRYPTO_BUFFER_num(chain) == 0) {
      ...
      if (stream->ssl_cert_verifier_->Verify(cert)) {
        ...
        RTC_LOG(LS_WARNING)
            << "Failed to verify certificate using custom callback";
      }
    }
  } else {
...
#else  // USE_CRYPTO_BUFFER_CALLBACK
int OpenSSLAdapter::SSLVerifyCallback(int ok, X509_STORE_CTX* store) {
  ...
  if (!ok && stream->ssl_cert_verifier_ != nullptr) {
    RTC_LOG(LS_INFO) << "Invoking SSL Verify Callback.";
    // Note that it's possible here that we create a OpenSSLCertificate even
    // through we're building with BoringSSL and using BoringSSLCertificate
    // everywhere else. This should be safe as long as this certificate is only
    // used in the verification callback.
...
#endif  // USE_CRYPTO_BUFFER_CALLBACK
...
#ifdef USE_CRYPTO_BUFFER_CALLBACK
  // If X509 certificates aren't used, we can use these methods to avoid
