...
int OpenSSLAdapter::SSLVerifyInternal(int ok, SSL* ssl, X509_STORE_CTX* store) {
  ...
  if (!bio) {
    RTC_LOG(LS_ERROR) << "Failed to allocate BIO.";
    return ok;
  }
  if (!i2d_X509_bio(bio.get(), X509_STORE_CTX_get_current_cert(store))) {
    RTC_LOG(LS_ERROR) << "Failed to write X509 to BIO.";
    return ok;
  }
  char* data = nullptr;
  size_t length = BIO_get_mem_data(bio.get(), &data);
  bssl::UniquePtr<CRYPTO_BUFFER> crypto_buffer(CRYPTO_BUFFER_new(
      reinterpret_cast<uint8_t*>(data), length, openssl::GetBufferPool()));
  if (!crypto_buffer) {
