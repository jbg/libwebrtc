...
bool PasteIntoBuffer(PlanarYuvBuffer* canvas,
                     const PlanarYuvBuffer& picture,
                     int offset_col,
                     int offset_row) {
  if (canvas->type() == VideoFrameBuffer::Type::kI420) {
    I420Buffer* buf = static_cast<I420Buffer*>(canvas);
    return buf->PasteIntoBuffer(*picture.GetI420(), offset_col, offset_row);
  } else {
    I010Buffer* buf = static_cast<I010Buffer*>(canvas);
    return buf->PasteIntoBuffer(*picture.GetI010(), offset_col, offset_row);
  }
}
...
TEST_P(TestPlanarYuvBuffer, PastesIntoBuffer) {
  ...
  EXPECT_FALSE(PasteIntoBuffer(buf.get(), *picture, kWidth - 1, kOffsety));

  EXPECT_TRUE(PasteIntoBuffer(buf.get(), *picture, kOffsetx, kOffsety));
