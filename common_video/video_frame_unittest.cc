...
TEST(TestUpdateRectScale, NoScale) {
  ...
  a.ScaleWithFrame(width, height, width, height, width, height);
  EXPECT_EQ(a, VideoFrame::UpdateRect({100, 50, 100, 200}));
}
...
TEST(TestUpdateRectScale, CropOnly) {
  ...
  const int height = 480;
  VideoFrame::UpdateRect a = {100, 50, 100, 200};
  a.ScaleWithFrame(width, height, width - 20, height - 20, width - 20,
                   height - 20);
  EXPECT_EQ(a, VideoFrame::UpdateRect({90, 40, 100, 200}));
}
...
TEST(TestUpdateRectScale, CropOnlyToOddOffset) {
  ...
  const int height = 480;
  VideoFrame::UpdateRect a = {100, 50, 100, 200};
  a.ScaleWithFrame(width, height, width - 10, height - 10, width - 10,
                   height - 10);
  EXPECT_EQ(a, VideoFrame::UpdateRect({94, 44, 102, 202}));
}
...
TEST(TestUpdateRectScale, ScaleByHalf) {
  ...
  a.ScaleWithFrame(width, height, width, height, width / 2, height / 2);
  // Scaled by half and +2 pixels in all directions.
  EXPECT_EQ(a, VideoFrame::UpdateRect({48, 28, 54, 104}));
}

TEST(TestUpdateRectScale, CropUpdateOut) {
  const int width = 640;
  const int height = 480;
  VideoFrame::UpdateRect a = {100, 60, 100, 200};
  a.ScaleWithFrame(width, height, 10, 10, 10, 10);
  // Scaled by half and +2 pixels in all directions.
  EXPECT_EQ(a, VideoFrame::UpdateRect({0, 0, 0, 0}));
}
...
TEST(TestUpdateRectScale, CropInsideUpdate) {
  ...
  a.ScaleWithFrame(width, height, 10, 10, 10, 10);
  // Scaled by half and +2 pixels in all directions.
  EXPECT_EQ(a, VideoFrame::UpdateRect({0, 0, 10, 10}));
}
...
TEST(TestUpdateRectScale, CropAndScaleByHalf) {
  ...
  const int height = 480;
  VideoFrame::UpdateRect a = {100, 60, 100, 200};
  a.ScaleWithFrame(width, height, width - 20, height - 20, (width - 20) / 2,
                   (height - 20) / 2);
  // Scaled by half and +3 pixels in all directions, because of odd offset after
  // crop and scale.
  EXPECT_EQ(a, VideoFrame::UpdateRect({42, 22, 56, 106}));
}
