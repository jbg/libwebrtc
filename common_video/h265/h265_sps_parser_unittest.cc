...
// Example SPS can be generated with ffmpeg. Here's an example command,
// runnable on Linux:
...
// cropped.h265
//
// 3) Open camera.h265 and find the SPS, generally everything between the second
...
TEST_F(H265SpsParserTest, TestSampleSPSVerticalCropLandscape) {
  // SPS for a 640x260 camera capture from ffmpeg on Linux,. Contains emulation
...
TEST_F(H265SpsParserTest, TestSyntheticSPSQvgaLandscape) {
  rtc::Buffer buffer;
  GenerateFakeSps(320u, 180u, 1, 0, 0, 0, &buffer);
  EXPECT_TRUE(static_cast<bool>(
      sps_ = H265SpsParser::ParseSps(buffer.data(), buffer.size())));
...
TEST_F(H265SpsParserTest, TestSyntheticSPSWeirdResolution) {
  rtc::Buffer buffer;
  GenerateFakeSps(156u, 122u, 2, 0, 0, 0, &buffer);
  EXPECT_TRUE(static_cast<bool>(
      sps_ = H265SpsParser::ParseSps(buffer.data(), buffer.size())));
...
TEST_F(H265SpsParserTest, TestLog2MaxSubLayersMinus1) {
  ...
  EXPECT_EQ(180u, sps_->height);
  EXPECT_EQ(1u, sps_->id);
  EXPECT_EQ(0u, sps_->sps_max_sub_layers_minus1);

  GenerateFakeSps(320u, 180u, 1, 6, 0, 0, &buffer);
  EXPECT_TRUE(static_cast<bool>(
      sps_ = H265SpsParser::ParseSps(buffer.data(), buffer.size())));
  EXPECT_EQ(320u, sps_->width);
  EXPECT_EQ(180u, sps_->height);
  EXPECT_EQ(1u, sps_->id);
  EXPECT_EQ(6u, sps_->sps_max_sub_layers_minus1);

  GenerateFakeSps(320u, 180u, 1, 7, 0, 0, &buffer);
  EXPECT_FALSE(H265SpsParser::ParseSps(buffer.data(), buffer.size()));
}
...
TEST_F(H265SpsParserTest, TestSubLayerOrderingInfoPresentFlag) {
  ...
  GenerateFakeSps(320u, 180u, 1, 6, 1, 0, &buffer);
  EXPECT_TRUE(static_cast<bool>(
      sps_ = H265SpsParser::ParseSps(buffer.data(), buffer.size())));
  EXPECT_EQ(320u, sps_->width);
  EXPECT_EQ(180u, sps_->height);
...
TEST_F(H265SpsParserTest, TestLongTermRefPicsPresentFlag) {
  ...
  EXPECT_EQ(180u, sps_->height);
  EXPECT_EQ(1u, sps_->id);
  EXPECT_EQ(0u, sps_->long_term_ref_pics_present_flag);

  GenerateFakeSps(320u, 180u, 1, 6, 0, 1, &buffer);
  EXPECT_TRUE(static_cast<bool>(
      sps_ = H265SpsParser::ParseSps(buffer.data(), buffer.size())));
  EXPECT_EQ(320u, sps_->width);
  EXPECT_EQ(180u, sps_->height);
  EXPECT_EQ(1u, sps_->id);
  EXPECT_EQ(1u, sps_->long_term_ref_pics_present_flag);
}
