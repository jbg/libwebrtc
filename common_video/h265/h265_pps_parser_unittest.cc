 ...
 public:
  ...
  void VerifyParsing(const H265PpsParser::PpsState& pps,
    ...
    WritePps(pps, slice_group_map_type, num_slice_groups, pic_size_in_map_units,
             &buffer_);
    parsed_pps_ = H265PpsParser::ParsePps(buffer_.data(), buffer_.size());
...
TEST_F(PpsParserTest, PpsIdFromSlice) {
  absl::optional<uint32_t> pps_id = H265PpsParser::ParsePpsIdFromSlice(
      kH265BitstreamChunk, sizeof(kH265BitstreamChunk));
  ASSERT_TRUE(pps_id);
  EXPECT_EQ(2u, *pps_id);
}
