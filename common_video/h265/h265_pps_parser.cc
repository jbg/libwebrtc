...
bool H265PpsParser::ParsePpsIds(const uint8_t* data,
  ...
  const std::vector<uint8_t> unpacked_buffer = H265::ParseRbsp(data, length);
  BitstreamReader reader(unpacked_buffer);
  return ParsePpsIdsInternal(reader, pps_id, sps_id);
}
...
absl::optional<H265PpsParser::PpsState> H265PpsParser::ParseInternal(
  ...
  pps.dependent_slice_segments_enabled_flag = reader.ReadBits(1);
  // output_flag_present_flag: u(1)
  pps.output_flag_present_flag = reader.ReadBits(1);
  // num_extra_slice_header_bits: u(3)
  pps.num_extra_slice_header_bits = reader.ReadBits(3);
  // sign_data_hiding_enabled_flag: u(1)
  reader.ConsumeBits(1);
  // cabac_init_present_flag: u(1)
  pps.cabac_init_present_flag = reader.ReadBits(1);
  ...
  bool cu_qp_delta_enabled_flag = reader.ReadBits(1);
  ...
  pps.weighted_pred_flag = reader.ReadBits(1);
  // weighted_bipred_flag: u(1)
  pps.weighted_bipred_flag = reader.ReadBits(1);
  // transquant_bypass_enabled_flag: u(1)
  reader.ConsumeBits(1);
  // tiles_enabled_flag: u(1)
  bool tiles_enabled_flag = reader.ReadBits(1);
  ...
  if (tiles_enabled_flag) {
    ...
    bool uniform_spacing_flag = reader.ReadBits(1);
  ...
  bool deblocking_filter_control_present_flag = reader.ReadBits(1);
  if (deblocking_filter_control_present_flag) {
    // deblocking_filter_override_enabled_flag: u(1)
    reader.ConsumeBits(1);
    // pps_deblocking_filter_disabled_flag: u(1)
    bool pps_deblocking_filter_disabled_flag = reader.ReadBits(1);
  ...
  bool pps_scaling_list_data_present_flag = reader.ReadBits(1);
  ...
  pps.lists_modification_present_flag = reader.ReadBits(1);
