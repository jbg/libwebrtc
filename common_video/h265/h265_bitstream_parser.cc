...
H265BitstreamParser::Result H265BitstreamParser::ParseNonParameterSetNalu(
  ...
  if (first_slice_segment_in_pic_flag == 0) {
    if (pps_->dependent_slice_segments_enabled_flag) {
      ...
      RETURN_INV_ON_FAIL(
          slice_reader.ReadBits(&dependent_slice_segment_flag, 1));
    }
    ...
    RETURN_INV_ON_FAIL(
        slice_reader.ReadBits(&bits_tmp, slice_segment_address_bits));
  }
  ...
  if (dependent_slice_segment_flag == 0) {
    ...
    if (nalu_type != H265::NaluType::kIdrWRadl &&
      ...
      RETURN_INV_ON_FAIL(
          slice_reader.ReadBits(&short_term_ref_pic_set_sps_flag, 1));
      if (!short_term_ref_pic_set_sps_flag) {
        ...
        if (short_term_ref_pic_set_idx_bits > 0) {
          RETURN_INV_ON_FAIL(slice_reader.ReadBits(
              &short_term_ref_pic_set_idx, short_term_ref_pic_set_idx_bits));
        }
      }
      if (sps_->long_term_ref_pics_present_flag) {
        ...
        // num_long_term_sps: ue(v)
        RETURN_INV_ON_FAIL(
            slice_reader.ReadExponentialGolomb(&num_long_term_pics));
        ...
        for (uint32_t i = 0; i < num_long_term_sps + num_long_term_pics; i++) {
          if (i < num_long_term_sps) {
            if (sps_->num_long_term_ref_pics_sps > 1) {
              ...
              RETURN_INV_ON_FAIL(
                  slice_reader.ReadBits(&lt_idx_sps[i], lt_idx_sps_bits));
            }
          } else {
            // poc_lsb_lt: u(v)
            uint32_t poc_lsb_lt_bits =
                sps_->log2_max_pic_order_cnt_lsb_minus4 + 4;
            RETURN_INV_ON_FAIL(
                slice_reader.ReadBits(&bits_tmp, poc_lsb_lt_bits));
            // used_by_curr_pic_lt_flag: u(1)
            RETURN_INV_ON_FAIL(
                slice_reader.ReadBits(&used_by_curr_pic_lt_flag[i], 1));
          }
          // delta_poc_msb_present_flag: u(1)
          uint32_t delta_poc_msb_present_flag = 0;
          RETURN_INV_ON_FAIL(
              slice_reader.ReadBits(&delta_poc_msb_present_flag, 1));
      ...
      if (sps_->sps_temporal_mvp_enabled_flag) {
        ...
        RETURN_INV_ON_FAIL(
            slice_reader.ReadBits(&slice_temporal_mvp_enabled_flag, 1));
      }
    }
    ...
    if (slice_type == H265::SliceType::kP ||
      ...
      if (num_ref_idx_active_override_flag) {
        ...
        RETURN_INV_ON_FAIL(
            slice_reader.ReadExponentialGolomb(&num_ref_idx_l0_active_minus1));
      ...
      if (pps_->lists_modification_present_flag && num_pic_total_curr > 1) {
        ...
        if (ref_pic_list_modification_flag_l0) {
          for (uint32_t i = 0; i < num_ref_idx_l0_active_minus1; i++) {
            // list_entry_l0: u(v)
            RETURN_INV_ON_FAIL(
                slice_reader.ReadBits(&bits_tmp, list_entry_bits));
          }
        }
        if (slice_type == H265::SliceType::kB) {
      ...
      if (slice_temporal_mvp_enabled_flag) {
        ...
        if (slice_type == H265::SliceType::kB) {
          ...
          RETURN_INV_ON_FAIL(
              slice_reader.ReadBits(&collocated_from_l0_flag, 1));
        }
