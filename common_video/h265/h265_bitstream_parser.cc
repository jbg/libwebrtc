...
H265BitstreamParser::Result H265BitstreamParser::ParseNonParameterSetNalu(
  ...
  bool first_slice_segment_in_pic_flag = slice_reader.ReadBits(1);
  ...
  if (!first_slice_segment_in_pic_flag) {
    if (pps_->dependent_slice_segments_enabled_flag) {
      ...
      dependent_slice_segment_flag = slice_reader.ReadBits(1);
    }
  ...
  if (!dependent_slice_segment_flag) {
    if (pps_->num_extra_slice_header_bits > 0) {
      slice_reader.ConsumeBits(pps_->num_extra_slice_header_bits);
    }
    // slice_type: ue(v)
    ...
    if (nalu_type != H265::NaluType::kIdrWRadl &&
      ...
      short_term_ref_pic_set_sps_flag = slice_reader.ReadBits(1);
      ...
      if (sps_->long_term_ref_pics_present_flag) {
        ...
        for (uint32_t i = 0; i < num_long_term_sps + num_long_term_pics; i++) {
          if (i < num_long_term_sps) {
            ...
            used_by_curr_pic_lt_flag[i] = slice_reader.ReadBits(1);
          }
          // delta_poc_msb_present_flag: u(1)
          uint32_t delta_poc_msb_present_flag = 0;
          delta_poc_msb_present_flag = slice_reader.ReadBits(1);
      ...
      if (sps_->sps_temporal_mvp_enabled_flag) {
        ...
        slice_temporal_mvp_enabled_flag = slice_reader.ReadBits(1);
      }
    }
    ...
    if (slice_type == H265::SliceType::kP ||
      ...
      uint32_t num_ref_idx_active_override_flag = slice_reader.ReadBits(1);
      ...
      if (pps_->lists_modification_present_flag && num_pic_total_curr > 1) {
        ...
        bool ref_pic_list_modification_flag_l0 = slice_reader.ReadBits(1);
        ...
        if (slice_type == H265::SliceType::kB) {
          ...
          uint32_t ref_pic_list_modification_flag_l1 = slice_reader.ReadBits(1);
      ...
      if (slice_temporal_mvp_enabled_flag) {
        ...
        if (slice_type == H265::SliceType::kB) {
          ...
          collocated_from_l0_flag = slice_reader.ReadBits(1);
        }
