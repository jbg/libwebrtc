...
H265BitstreamParser::Result H265BitstreamParser::ParseNonParameterSetNalu(
    ...
    uint8_t nalu_type) {
  if (!vps_ || !sps_ || !pps_ || pps_->sps_id != sps_->sps_id ||
      sps_->vps_id != vps_->id)
    return kInvalidStream;
  ...
  if (dependent_slice_segment_flag == 0) {
    ...
    if (nalu_type != H265::NaluType::kIdrWRadl &&
      ...
      if (!short_term_ref_pic_set_sps_flag) {
        ...
        uint32_t short_term_ref_pic_set_idx_bits =
            H265::Log2Ceiling(sps_->num_short_term_ref_pic_sets);
      ...
      if (sps_->long_term_ref_pics_present_flag) {
        if (sps_->num_long_term_ref_pics_sps > 0) {
          ...
          IN_RANGE_OR_RETURN(slice_type, 0, sps_->num_long_term_ref_pics_sps);
        }
        ...
        for (uint32_t i = 0; i < num_long_term_sps + num_long_term_pics; i++) {
          if (i < num_long_term_sps) {
            if (sps_->num_long_term_ref_pics_sps > 1) {
              ...
              uint32_t lt_idx_sps_bits =
                  H265::Log2Ceiling(sps_->num_long_term_ref_pics_sps);
          ...
          if (delta_poc_msb_present_flag) {
            ...
            IN_RANGE_OR_RETURN(
                ...
                std::pow(2, 32 - sps_->log2_max_pic_order_cnt_lsb_minus4 - 4));
          }
        }
      }
    ...
    if (slice_type == H265::SliceType::kP ||
      ...
      if (!slice_reader.Ok() ||
          ((pps_->weighted_pred_flag && slice_type == H265::SliceType::kP) ||
  ...
  IN_RANGE_OR_RETURN(26 + pps_->init_qp_minus26 + last_slice_qp_delta,
                     -pps_->qp_bd_offset_y, 51);
  ...
  switch (nalu_type) {
    case H265::NaluType::kVps: {
      if (length >= H265::kNaluHeaderSize) {
        vps_ = H265VpsParser::ParseVps(slice + H265::kNaluHeaderSize,
    ...
    case H265::NaluType::kSps: {
      if (length >= H265::kNaluHeaderSize) {
        sps_ = H265SpsParser::ParseSps(slice + H265::kNaluHeaderSize,
                                       length - H265::kNaluHeaderSize);
      }
      if (!sps_) {
    ...
    case H265::NaluType::kPps: {
      if (length >= H265::kNaluHeaderSize) {
        pps_ = H265PpsParser::ParsePps(slice + H265::kNaluHeaderSize,
                                       length - H265::kNaluHeaderSize, sps_);
      }
      if (!pps_) {
...
bool H265BitstreamParser::GetLastSliceQp(int* qp) const {
  ...
  const int parsed_qp = 26 + pps_->init_qp_minus26 + *last_slice_qp_delta_;
