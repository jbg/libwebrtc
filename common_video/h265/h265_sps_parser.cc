...
absl::optional<H265SpsParser::SpsState> H265SpsParser::ParseSps(
    const uint8_t* data,
    size_t length) {
  const std::vector<uint8_t> unpacked_buffer = H265::ParseRbsp(data, length);
  BitstreamReader reader(unpacked_buffer);
  return ParseSpsInternal(reader);
}
...
bool H265SpsParser::ParseScalingListData(BitstreamReader& reader) {
  ...
  for (int size_id = 0; size_id < 4; size_id++) {
    for (int matrix_id = 0; matrix_id < 6;
      ...
      scaling_list_pred_mode_flag[size_id][matrix_id] = reader.ReadBits(1);
      if (!scaling_list_pred_mode_flag[size_id][matrix_id]) {
        ...
        for (uint32_t i = 0; i < coef_num; i++) {
          ...
          int32_t scaling_list_delta_coef = reader.ReadExponentialGolomb();
...
H265SpsParser::ParseShortTermRefPicSet(
  ...
  if (st_rps_idx != 0) {
    ...
    inter_ref_pic_set_prediction_flag = reader.ReadBits(1);
  }
  if (inter_ref_pic_set_prediction_flag) {
    ...
    for (uint32_t j = 0; j <= num_delta_pocs; j++) {
      ...
      ref_pic_set.used_by_curr_pic_flag[j] = reader.ReadBits(1);
      if (!ref_pic_set.used_by_curr_pic_flag[j]) {
        // use_delta_flag: u(1)
        ref_pic_set.use_delta_flag[j] = reader.ReadBits(1);
      }
    }
  } else {
    ...
    for (uint32_t i = 0; i < ref_pic_set.num_negative_pics; i++) {
      ...
      ref_pic_set.used_by_curr_pic_s0_flag[i] = reader.ReadBits(1);
    }
    ...
    for (uint32_t i = 0; i < ref_pic_set.num_positive_pics; i++) {
      ...
      ref_pic_set.used_by_curr_pic_s1_flag[i] = reader.ReadBits(1);
    }
  }
...
absl::optional<H265SpsParser::SpsState> H265SpsParser::ParseSpsInternal(
  ...
  uint32_t sps_video_parameter_set_id = reader.ReadBits(4);
  ...
  for (uint32_t i = 0; i < sps_max_sub_layers_minus1; i++) {
    ...
    sub_layer_profile_present = reader.ReadBits(1);
    sub_layer_level_present = reader.ReadBits(1);
  ...
  if (sps.chroma_format_idc == 3) {
    ...
    sps.separate_colour_plane_flag = reader.ReadBits(1);
  }
  // pic_width_in_luma_samples: ue(v)
  uint32_t pic_width_in_luma_samples = reader.ReadExponentialGolomb();
  // pic_height_in_luma_samples: ue(v)
  uint32_t pic_height_in_luma_samples = reader.ReadExponentialGolomb();
  ...
  uint32_t sps_sub_layer_ordering_info_present_flag = reader.ReadBits(1);
  ...
  bool scaling_list_enabled_flag = reader.ReadBits(1);
  if (scaling_list_enabled_flag) {
    // sps_scaling_list_data_present_flag: u(1)
    bool sps_scaling_list_data_present_flag = reader.ReadBits(1);
  ...
  sps.sample_adaptive_offset_enabled_flag = reader.ReadBits(1);
  // pcm_enabled_flag: u(1)
  bool pcm_enabled_flag = reader.ReadBits(1);
  if (pcm_enabled_flag) {
  ...
  sps.long_term_ref_pics_present_flag = reader.ReadBits(1);
  if (sps.long_term_ref_pics_present_flag) {
    ...
    for (uint32_t i = 0; i < sps.num_long_term_ref_pics_sps; i++) {
      ...
      sps.used_by_curr_pic_lt_sps_flag[i] = reader.ReadBits(1);
    }
  }

  // sps_temporal_mvp_enabled_flag: u(1)
  sps.sps_temporal_mvp_enabled_flag = reader.ReadBits(1);
