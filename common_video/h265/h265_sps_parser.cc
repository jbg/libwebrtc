...
absl::optional<H265SpsParser::SpsState> H265SpsParser::ParseSpsInternal(
  ...
  if (!reader.Ok() || sps_max_sub_layers_minus1 > (kMaxSubLayers - 1)) {
    return absl::nullopt;
  }
  ...
  if (!reader.Ok() ||
      sps.num_short_term_ref_pic_sets > kMaxShortTermRefPicSets) {
    return absl::nullopt;
  }
  ...
  if (sps.long_term_ref_pics_present_flag) {
    ...
    if (!reader.Ok() ||
        sps.num_long_term_ref_pics_sps > kMaxLongTermRefPicSets) {
      return absl::nullopt;
    }
