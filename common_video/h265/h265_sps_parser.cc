...
bool H265SpsParser::ParseScalingListData(rtc::BitBuffer* buffer) {
  ...
  for (int size_id = 0; size_id < 4; size_id++) {
    for (int matrix_id = 0; matrix_id < 6;
      ...
      RETURN_FALSE_ON_FAIL(buffer->ReadBits(
          &scaling_list_pred_mode_flag[size_id][matrix_id], 1));
      if (!scaling_list_pred_mode_flag[size_id][matrix_id]) {
        // scaling_list_pred_matrix_id_delta: ue(v)
        RETURN_FALSE_ON_FAIL(buffer->ReadExponentialGolomb(
        ...
        if (size_id > 1) {
          ...
          RETURN_FALSE_ON_FAIL(buffer->ReadSignedExponentialGolomb(
        ...
        for (uint32_t i = 0; i < coef_num; i++) {
          ...
          RETURN_FALSE_ON_FAIL(
              buffer->ReadSignedExponentialGolomb(&scaling_list_delta_coef));
...
H265SpsParser::ParseShortTermRefPicSet(
  ...
  if (st_rps_idx != 0) {
    ...
    RETURN_EMPTY2_ON_FAIL(
        buffer->ReadBits(&inter_ref_pic_set_prediction_flag, 1));
  }
  if (inter_ref_pic_set_prediction_flag) {
    ...
    for (uint32_t j = 0; j <= num_delta_pocs; j++) {
      ...
      RETURN_EMPTY2_ON_FAIL(
          buffer->ReadBits(&ref_pic_set.used_by_curr_pic_flag[j], 1));
      if (!ref_pic_set.used_by_curr_pic_flag[j]) {
        // use_delta_flag: u(1)
        RETURN_EMPTY2_ON_FAIL(
            buffer->ReadBits(&ref_pic_set.use_delta_flag[j], 1));
      }
    }
  } else {
    // num_negative_pics: ue(v)
    RETURN_EMPTY2_ON_FAIL(
        buffer->ReadExponentialGolomb(&ref_pic_set.num_negative_pics));
    // num_positive_pics: ue(v)
    RETURN_EMPTY2_ON_FAIL(
        buffer->ReadExponentialGolomb(&ref_pic_set.num_positive_pics));
    ...
    for (uint32_t i = 0; i < ref_pic_set.num_negative_pics; i++) {
      ...
      RETURN_EMPTY2_ON_FAIL(
          buffer->ReadExponentialGolomb(&ref_pic_set.delta_poc_s0_minus1[i]));
      // used_by_curr_pic_s0_flag: u(1)
      RETURN_EMPTY2_ON_FAIL(
          buffer->ReadBits(&ref_pic_set.used_by_curr_pic_s0_flag[i], 1));
    }
    ...
    for (uint32_t i = 0; i < ref_pic_set.num_positive_pics; i++) {
      ...
      RETURN_EMPTY2_ON_FAIL(
          buffer->ReadExponentialGolomb(&ref_pic_set.delta_poc_s1_minus1[i]));
      // used_by_curr_pic_s1_flag: u(1)
      RETURN_EMPTY2_ON_FAIL(
          buffer->ReadBits(&ref_pic_set.used_by_curr_pic_s1_flag[i], 1));
    }
  }
...
absl::optional<H265SpsParser::SpsState> H265SpsParser::ParseSpsInternal(
  ...
  RETURN_EMPTY_ON_FAIL(
      buffer->ReadBits(&sps_sub_layer_ordering_info_present_flag, 1));
  for (uint32_t i = (sps_sub_layer_ordering_info_present_flag != 0)
                        ? 0
                        : sps_max_sub_layers_minus1;
       i <= sps_max_sub_layers_minus1; i++) {
    // sps_max_dec_pic_buffering_minus1: ue(v)
    RETURN_EMPTY_ON_FAIL(buffer->ReadExponentialGolomb(
  ...
  if (scaling_list_enabled_flag) {
    ...
    RETURN_EMPTY_ON_FAIL(
        buffer->ReadBits(&sps_scaling_list_data_present_flag, 1));
  ...
  RETURN_EMPTY_ON_FAIL(
      buffer->ReadBits(&sps.sample_adaptive_offset_enabled_flag, 1));
  ...
  RETURN_EMPTY_ON_FAIL(
      buffer->ReadExponentialGolomb(&sps.num_short_term_ref_pic_sets));
  ...
  for (uint32_t st_rps_idx = 0; st_rps_idx < sps.num_short_term_ref_pic_sets;
    ...
    OptionalShortTermRefPicSet ref_pic_set =
        ParseShortTermRefPicSet(st_rps_idx, sps.num_short_term_ref_pic_sets,
                                sps.short_term_ref_pic_set, sps, buffer);
    if (ref_pic_set) {
  ...
  RETURN_EMPTY_ON_FAIL(
      buffer->ReadBits(&sps.long_term_ref_pics_present_flag, 1));
  if (sps.long_term_ref_pics_present_flag) {
    // num_long_term_ref_pics_sps: ue(v)
    RETURN_EMPTY_ON_FAIL(
        buffer->ReadExponentialGolomb(&sps.num_long_term_ref_pics_sps));
    ...
    for (uint32_t i = 0; i < sps.num_long_term_ref_pics_sps; i++) {
      ...
      RETURN_EMPTY_ON_FAIL(
          buffer->ReadBits(&sps.used_by_curr_pic_lt_sps_flag[i], 1));
    }
  }
