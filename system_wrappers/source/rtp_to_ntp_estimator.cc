...
int64_t RtpToNtpEstimator::Unwrap(uint32_t rtp_timestamp) const {
  ...
  lastWrapTimestamp_.emplace(rtp_timestamp);
  return rtp_timestamp + (static_cast<int64_t>(wrap_arounds_) << 32);
}

int CheckForWrapArounds(uint32_t new_timestamp, uint32_t old_timestamp) {
  if (new_timestamp < old_timestamp) {
    // This difference should be more than 2^31 if we have had a wrap around
    // (e.g. |new_timestamp| = 1, |old_timestamp| = 2^32 - 1).
    if (old_timestamp - new_timestamp >= 1u << 31) {
      // Forward wrap around.
      return 1;
    }
  } else if (new_timestamp - old_timestamp >= 1u << 31) {
    // This difference should be more than 2^31 if we have had a backward wrap
    // around.
    return -1;
  }
  return 0;
