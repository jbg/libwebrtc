...
bool LinearRegression(const rtc::ArrayView<const double> x,
                      const rtc::ArrayView<const double> y,
                      double* k,
                      double* b) {
  size_t n = x.size();
  if (n == 0)
  ...
  for (size_t i = 0; i < n; ++i) {
    // Use normalized values to avoid overflow.
    double normalized_x = x[i] - x[0];
    double normalized_y = y[i] - y[0];
    sum_x += normalized_x;
    sum_y += normalized_y;
    sum_xx += static_cast<long double>(normalized_x) * normalized_x;
    sum_xy += static_cast<long double>(normalized_x) * normalized_y;
  }
  long double variance_x = sum_xx * n - sum_x * sum_x;
  // Can't construct linear regression if all samples are on a vertical line.
  if (std::fabs(variance_x) < 1e-8)
    return false;
  long double covariance_xy = sum_xy * n - sum_x * sum_y;

  // Slope is the same for normalized and non-normalized samples.
  *k = static_cast<double>(covariance_xy / variance_x);
  // Use non-normalized values to compute the offset.
  long double avg_x = sum_x / n + x[0];
  long double avg_y = sum_y / n + y[0];
...
void RtpToNtpEstimator::UpdateParameters() {
  ...
  if (!LinearRegression(rtc::ArrayView<const double>(&x[0], x.size()),
                        rtc::ArrayView<const double>(&y[0], y.size()), &slope,
                        &offset)) {
    return;
  }

  params_ = Parameters{1 / slope, offset};
}
