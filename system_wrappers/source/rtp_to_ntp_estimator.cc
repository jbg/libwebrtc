...
bool LinearRegression(rtc::ArrayView<const double> x,
  ...
  for (size_t i = 0; i < n; ++i) {
    ...
    sum_x += normalized_x;
    sum_y += normalized_y;
    sum_xx += normalized_x * normalized_x;
    sum_xy += normalized_x * normalized_y;
  }
  long double variance_x = static_cast<long double>(sum_xx) * n - sum_x * sum_x;
  ...
  long double covariance_xy =
      static_cast<long double>(sum_xy) * n - sum_x * sum_y;
  ...
  *k = static_cast<double>(covariance_xy / variance_x);
  ...
  double avg_x = sum_x / n + x[0];
  double avg_y = sum_y / n + y[0];
