...
class VideoStreamEncoder : public VideoStreamEncoderInterface,
 ...
 private:
  ...
  absl::optional<int64_t> ExtractEncodeStartTime(size_t simulcast_svc_idx,
      ...
      RTC_EXCLUSIVE_LOCKS_REQUIRED(timing_frames_lock_);
  ...
  rtc::CriticalSection encoder_info_lock_;
  VideoEncoder::EncoderInfo encoder_info_ RTC_GUARDED_BY(&encoder_info_lock_);
  VideoEncoderFactory::CodecInfo codec_info_
      RTC_GUARDED_BY(&encoder_info_lock_);
  VideoBitrateAllocation last_bitrate_allocation_
      RTC_GUARDED_BY(&encoder_info_lock_);
  uint32_t last_framerate_fps_ RTC_GUARDED_BY(&encoder_info_lock_);
  ...
  rtc::CriticalSection timing_frames_lock_;
  // Separate instance for each simulcast stream or spatial layer.
  std::vector<TimingFramesLayerInfo> timing_frames_info_
      RTC_GUARDED_BY(&timing_frames_lock_);
  int64_t last_timing_frame_time_ms_ RTC_GUARDED_BY(&timing_frames_lock_);
  size_t incorrect_capture_time_logged_messages_
      RTC_GUARDED_BY(&timing_frames_lock_);
  size_t reordered_frames_logged_messages_ RTC_GUARDED_BY(&timing_frames_lock_);
  size_t stalled_encoder_logged_messages_ RTC_GUARDED_BY(&timing_frames_lock_);
  VideoCodec::TimingFrameTriggerThresholds timing_frame_thresholds_
      RTC_GUARDED_BY(&timing_frames_lock_);
