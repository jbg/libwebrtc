...
void VideoReceiveStream::OnFrame(const VideoFrame& video_frame) {
  ...
  if (config_.rtp.playback_timing && playback_timing_callback_ &&
    ...
    int64_t first_packet_received_ms =
        std::min_element(
            ...
            ->receive_time_ms();
    int64_t last_packet_received_ms =
        std::max_element(
            video_frame.packet_infos().cbegin(),
            video_frame.packet_infos().cend(),
            [](const webrtc::RtpPacketInfo& a, const webrtc::RtpPacketInfo& b) {
    ...
    playback_timing_callback_->UpdatePacketAndDecodeTiming(
        config_.rtp.local_ssrc, video_frame.timestamp(),
        first_packet_received_ms, last_packet_received_ms,
        video_frame.processing_time()->start.ms(),
        video_frame.processing_time()->finish.ms());
  }
