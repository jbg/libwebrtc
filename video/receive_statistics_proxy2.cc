...
void ReceiveStatisticsProxy::OnDecodedFrame(const VideoFrameMetaData& frame,
  ...
  rtc::CritScope lock(&crit_);
  video_quality_observer_->OnDecodedFrame(frame.rtp_timestamp, qp,
                                          last_codec_type_);
  ...
  decode_fps_estimator_.Update(1, frame.decode_timestamp.ms());

  if (last_decoded_frame_time_ms_) {
    int64_t interframe_delay_ms =
        frame.decode_timestamp.ms() - *last_decoded_frame_time_ms_;
  ...
  if (stats_.frames_decoded == 1) {
    first_decoded_frame_time_ms_.emplace(frame.decode_timestamp.ms());
  }
  last_decoded_frame_time_ms_.emplace(frame.decode_timestamp.ms());
}
