...
VideoReceiveStream::Stats ReceiveStatisticsProxy::GetStats() const {
  ...
  // As we might have pending data waiting to be added to
  // |interframe_delay_max_moving_|, we need to reset the array and base time.
  interframe_delay_max_moving_.Reset();
...
void ReceiveStatisticsProxy::OnDecodedFrame(const VideoFrame& frame,
  ...
  // TODO(webrtc:11489): On iOS this gets called on
...
void ReceiveStatisticsProxy::OnDecodedFrame(const VideoFrameMetaData& frame,
  ...
  if (last_decoded_frame_time_ms_) {
    RTC_DCHECK_GE(frame.now_ms, *last_decoded_frame_time_ms_);
...
void ReceiveStatisticsProxy::OnStreamInactive() {
  ...
  // TODO(tommi): Should we rather do this in DecoderThreadStopped?
  //last_decoded_frame_time_ms_.reset();
