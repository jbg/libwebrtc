  ...
  class TestEncoder : public test::FakeEncoder {
   public:
    ...
    VideoEncoder::EncoderInfo GetEncoderInfo() const override {
      ...
      info.supported_pixel_formats = supported_pixel_formats_;
      return info;
    }
    ...
    void SetSupportedPixelFormats(
      ...
      MutexLock lock(&local_mutex_);
      supported_pixel_formats_ = std::move(supported_pixel_formats);
    }
   ...
   private:
    ...
    absl::optional<VideoFrameBuffer::Type> last_input_pixel_format_
        RTC_GUARDED_BY(local_mutex_);
    std::vector<VideoFrameBuffer::Type> supported_pixel_formats_
        RTC_GUARDED_BY(local_mutex_);
  };
...
TEST_F(VideoStreamEncoderTest,
       NonI420FrameShouldConvertToI420IfNotEncoderSupported) {
  video_stream_encoder_->OnBitrateUpdatedAndWaitForManagedResources(
      DataRate::BitsPerSec(kTargetBitrateBps),
      DataRate::BitsPerSec(kTargetBitrateBps),
      DataRate::BitsPerSec(kTargetBitrateBps), 0, 0, 0);
  fake_encoder_.SetSupportedPixelFormats({});
...
TEST_F(VideoStreamEncoderTest,
       NV12FrameIsEncodedWithoutConversionIfEncodedSupported) {
  video_stream_encoder_->OnBitrateUpdatedAndWaitForManagedResources(
      DataRate::BitsPerSec(kTargetBitrateBps),
      DataRate::BitsPerSec(kTargetBitrateBps),
      DataRate::BitsPerSec(kTargetBitrateBps), 0, 0, 0);
  fake_encoder_.SetSupportedPixelFormats({VideoFrameBuffer::Type::kNV12});
