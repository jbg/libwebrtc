...
class SimpleVideoStreamEncoderFactory {
 public:
  static constexpr int kSteadyStateQP = 15;
  ...
  class MockFakeEncoder : public test::FakeEncoder {
   public:
    ...
    EncoderInfo GetEncoderInfo() const {
      auto info = test::FakeEncoder::EncoderInfo();
      info.steady_state_qp = kSteadyStateQP;
      return info;
    }
  };
...
TEST(VideoStreamEncoderFrameCadenceTest, UsesFrameCadenceAdapterForFrameRate) {
  ...
  PassAFrame(encoder_queue, video_stream_encoder_callback);
  factory.DepleteTaskQueues();
}
...
TEST(VideoStreamEncoderFrameCadenceTest, UpdatesQualityConvergence) {
  ...
  EXPECT_CALL(factory.GetMockFakeEncoder(), EncodeHook)
      .WillRepeatedly(Invoke([](EncodedImage& encoded_image,
                                rtc::scoped_refptr<EncodedImageBuffer> buffer) {
        encoded_image.qp_ = SimpleVideoStreamEncoderFactory::kSteadyStateQP + 1;
        CodecSpecificInfo codec_specific;
  ...
  EXPECT_CALL(factory.GetMockFakeEncoder(), EncodeHook)
      .WillRepeatedly(Invoke([](EncodedImage& encoded_image,
                                rtc::scoped_refptr<EncodedImageBuffer> buffer) {
        encoded_image.qp_ = SimpleVideoStreamEncoderFactory::kSteadyStateQP +
