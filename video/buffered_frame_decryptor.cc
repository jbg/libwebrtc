...
BufferedFrameDecryptor::BufferedFrameDecryptor(
    ...
    : crypto_options_(crypto_options),
      ...
      decrypted_frame_callback_(std::move(decrypted_frame_callback)) {}
...
void BufferedFrameDecryptor::ManageEncryptedFrame(
  ...
  switch (DecryptFrame(encrypted_frame.get())) {
    ...
    case FrameDecision::kDecrypted:
      RetryStashedFrames();
      decrypted_frame_callback_(std::move(encrypted_frame));
      break;
  ...
  for (auto frame_it = stashed_frames_.begin();
    ...
    switch (DecryptFrame(frame_it->get())) {
      case FrameDecision::kStash:
        ...
        break;
      case FrameDecision::kDecrypted:
        decrypted_frame_callback_(std::move(*frame_it));
        RTC_FALLTHROUGH();
      case FrameDecision::kDrop:
