...
// Validates that frames can successfully be encrypted and decrypted on the
// other end specifically with the VP8 protocol.
TEST_F(FrameEncryptionEndToEndTest, ReceivesDecryptedFrame) {
  class DecryptedFrameObserver : public test::EndToEndTest,
   ...
   private:
    ...
    // Generate the frame with a non standard rotation.
    ...
    VideoRotation expected_rotation_ = kVideoRotation_90;
  } test;
