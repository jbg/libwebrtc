...
class FrameObserver : public test::RtpRtcpObserver,
 ...
 private:
  ...
  Action OnSendRtp(const uint8_t* packet, size_t length) override {
    ...
    if (!last_timestamp_ || header.timestamp != *last_timestamp_) {
      ...
      if (last_payload_type_) {
        const bool new_payload_type = header.payloadType != *last_payload_type_;
    ...
    sent_timestamps_.insert(header.timestamp);
    return SEND_PACKET;
  }

  // Verify that all sent frames are received and decoded.
  void OnFrame(const VideoFrame& decoded_frame) override {
    rtc::CritScope lock(&crit_);
    ++num_received_frames_;
    ASSERT_TRUE(sent_timestamps_.find(decoded_frame.timestamp()) !=
                sent_timestamps_.end());
    sent_timestamps_.erase(decoded_frame.timestamp());
    if (num_received_frames_ >= kFramesToObserve) {
  ...
  int num_received_frames_ RTC_GUARDED_BY(crit_) = 0;
  std::set<uint32_t> sent_timestamps_ RTC_GUARDED_BY(crit_);
};

class MultiCodecReceiveTest : public test::CallTest {
 public:
  MultiCodecReceiveTest() : observer_(new FrameObserver()) {
    task_queue_.SendTask([this]() {
      ...
      receive_transport_.reset(new test::PacketTransport(
          &task_queue_, receiver_call_.get(), observer_.get(),
 ...
 private:
  const std::unique_ptr<FrameObserver> observer_;
...
void MultiCodecReceiveTest::RunTestWithCodecs(
  ...
  task_queue_.SendTask([this, encoder_factories, payload_names]() {
    ...
    video_receive_configs_[0].renderer = observer_.get();
...
TEST_F(MultiCodecReceiveTest, SingleStreamReceivesVp8Vp9H264) {
  ...
  RunTestWithCodecs(
      {"VP8", "VP9", "H264"},
      {&vp8_encoder_factory, &vp9_encoder_factory, &h264_encoder_factory});
}
