...
size_t ExpectedNumTemporalLayers(TestExpectation test_expectation,
  ...
  switch (test_expectation) {
    case TestExpectation::kNoReport:
      return 0u;
    case TestExpectation::kBitrateNotDistributed:
      return 1u;
    case TestExpectation::kBitrateDistributed:
...
class RtcpObserver : public test::EndToEndTest {
 public:
  RtcpObserver(VideoEncoderFactory* encoder_factory,
      ...
      : EndToEndTest(test::CallTest::kDefaultTimeoutMs),
        ...
        expected_num_ssrcs_(
            (expectation == TestExpectation::kNoReport) ? 0u : num_ssrcs) {}
 ...
 private:
  ...
  int observed_rtcp_sr_ RTC_GUARDED_BY(&crit_) = 0;
...
TEST_P(ExtendedTargetBitrateTest, SendsXrVp8) {
  ...
  test::FunctionVideoEncoderFactory encoder_factory(
      []() { return VP8Encoder::Create(); });
  RtcpObserver test(&encoder_factory, "VP8", /*num_ssrcs_*/ 1u,
                    TestExpectation::kBitrateDistributed);
  RunBaseTest(&test);
}
