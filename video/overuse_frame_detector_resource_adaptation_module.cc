...
OveruseFrameDetectorResourceAdaptationModule::GetAdaptDownTarget(
  ...
  switch (EffectiveDegradationPreference()) {
    ...
    case DegradationPreference::MAINTAIN_FRAMERATE: {
      ...
      // PerformAdaptationTo() or elsewhere - simply checking which adaptation
...
void OveruseFrameDetectorResourceAdaptationModule::PerformAdaptationTo(
...
void OveruseFrameDetectorResourceAdaptationModule::OnResourceUnderuse(
  ...
  if (!target.has_value())
    return;
  // Perform adaptation.
  PerformAdaptationTo(target.value(), min_pixels_per_frame, reason);
