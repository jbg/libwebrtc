...
// happens.
//
// It also ensures thread safety between SetSourceDegradationPreference() calls
// on the worker thread and adaptation logic on the encoder task queue.
class OveruseFrameDetectorResourceAdaptationModule::VideoSourceRestrictor {
 public:
  explicit VideoSourceRestrictor(VideoSourceController* video_source_controller)
  ...
  // degradation preference moves in our out of "balanced" more or when clearing
  // all counters. When moving the remaining degradation preference logic inside
  // the VideoSourceController to here, stop explicitly setting the controller's
  // restrictions and instead inform the VideoStreamEncoder of updated
  // restrictions using OnVideoSourceRestrictionsUpdated().
  void ResetPixelFpsCount() {
    rtc::CritScope lock(&crit_);
    source_restrictions_ =
        VideoSourceRestrictions(absl::nullopt, absl::nullopt, absl::nullopt);
    video_source_controller_->SetRestrictions(source_restrictions_);
  ...
  bool RequestResolutionLowerThan(int pixel_count,
    ...
    rtc::CritScope lock(&crit_);
    if (!has_source_ || !IsResolutionScalingEnabled(degradation_preference_)) {
    ...
    if (pixels_wanted >=
        OptionalSizeTToInt(source_restrictions_.max_pixels_per_frame())) {
      return false;
    }
    ...
    source_restrictions_ = VideoSourceRestrictions(
        pixels_wanted != std::numeric_limits<int>::max()
            ...
            : absl::nullopt,
        absl::nullopt, source_restrictions_.max_frame_rate());
    return true;
  }
  ...
  bool RestrictFramerate(int fps) {
    // Called on the encoder task queue.
    rtc::CritScope lock(&crit_);
    if (!has_source_ || !IsFramerateScalingEnabled(degradation_preference_))
      return false;

    const int fps_wanted = std::max(kMinFramerateFps, fps);
    if (fps_wanted >= OptionalSizeTToInt(source_restrictions_.max_frame_rate()))
      return false;

    RTC_LOG(LS_INFO) << "Scaling down framerate: " << fps_wanted;
    source_restrictions_ =
        VideoSourceRestrictions(source_restrictions_.max_pixels_per_frame(),
                                source_restrictions_.target_pixels_per_frame(),
 ...
 private:
  ...
  bool has_source_ RTC_GUARDED_BY(&crit_);
...
OveruseFrameDetectorResourceAdaptationModule::
    ...
    : encoder_queue_(nullptr),
      ...
      source_restrictor_(
          std::make_unique<VideoSourceRestrictor>(video_source_controller)),
...
void OveruseFrameDetectorResourceAdaptationModule::
    SetSourceDegradationPreference(
        bool has_source,
        DegradationPreference degradation_preference) {
  source_restrictor_->SetSourceDegradationPreference(has_source,
...
void OveruseFrameDetectorResourceAdaptationModule::RefreshTargetFramerate(
  ...
  // scenarios.
  auto sink = video_source_controller->CurrentSettingsToSinkWants();
  int target_framerate = std::min(codec_max_framerate_, sink.max_framerate_fps);
