...
//
// Thread safety is ensured between SetSourceDegradationPreference() calls on
// the worker thread and adaptation logic on the encoder task queue using a
// lock.
class OveruseFrameDetectorResourceAdaptationModule::VideoSourceRestrictor {
 public:
  ...
  // preference logic inside the VideoSourceController to here, stop explicitly
  // setting the controller's restrictions and instead inform the
...
void OveruseFrameDetectorResourceAdaptationModule::RefreshTargetFramerate() {
  RTC_DCHECK(encoder_queue_);
  RTC_DCHECK_RUN_ON(encoder_queue_);
  // We need the "sink wants" from the |video_source_controller_| because the
  // controller filters its current settings as "sink wants" differently
  ...
  auto sink_wants = video_source_controller_->CurrentSettingsToSinkWants();
