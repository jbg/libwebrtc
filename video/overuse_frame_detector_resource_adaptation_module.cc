...
class OveruseFrameDetectorResourceAdaptationModule::VideoSourceRestrictor {
 ...
 private:
  static int GetIncreasedMaxPixelsWanted(int target_pixels) {
    ...
    return GetLowerResolutionThan(target_pixels) * 4;
  }
...
bool OveruseFrameDetectorResourceAdaptationModule::
    WaitingForPreviousAdaptationToGoUp(int input_pixel_count) {
  if (!last_adaptation_request_ ||
      last_adaptation_request_->mode_ != AdaptationRequest::Mode::kAdaptUp)
    return false;
  // TODO(hbos): What about other degradation preferences?
  return degradation_preference_ == DegradationPreference::MAINTAIN_FRAMERATE &&
         input_pixel_count <= last_adaptation_request_->input_pixel_count_;
}

bool OveruseFrameDetectorResourceAdaptationModule::
    BalancedSettingsAllowsAdaptingUp(
        AdaptationObserverInterface::AdaptReason reason) {
...
void OveruseFrameDetectorResourceAdaptationModule::OnResourceUnderuse(
  ...
  if (WaitingForPreviousAdaptationToGoUp(
          adaptation_request.input_pixel_count_)) {
    return;
  }
  if (!BalancedSettingsAllowsAdaptingUp(reason))
    return;
...
bool OveruseFrameDetectorResourceAdaptationModule::ExistsLowerSetting(
    int input_frame_rate) {
  if (EffectiveDegradataionPreference() ==
      DegradationPreference::MAINTAIN_RESOLUTION) {
    // TODO(hbos): The usage of |last_adaptation_was_down| looks like a mistake.
    // Delete this.
    bool last_adaptation_was_down =
        last_adaptation_request_ &&
        last_adaptation_request_->mode_ == AdaptationRequest::Mode::kAdaptDown;
    if (input_frame_rate <= 0 ||
        (last_adaptation_was_down && input_frame_rate < kMinFramerateFps)) {
...
bool OveruseFrameDetectorResourceAdaptationModule::
    WaitingForPreviousAdaptationToGoDown(int input_pixel_count) {
  if (!last_adaptation_request_ ||
      last_adaptation_request_->mode_ != AdaptationRequest::Mode::kAdaptDown) {
    return false;
  }
...
OveruseFrameDetectorResourceAdaptationModule::OnResourceOveruse(
  ...
  // disabled, not just CPU. This is not a use case we want to support. Remove
  // the enum value.
  ...
  if (!ExistsLowerSetting(adaptation_request.framerate_fps_))
    return ResourceListenerResponse::kNothing;
  if (WaitingForPreviousAdaptationToGoDown(
          adaptation_request.input_pixel_count_)) {
    return ResourceListenerResponse::kNothing;
  }
