 ...
 private:
  ...
  static std::unique_ptr<StreamState> ConfigureFromFile(
    ...
    for (const auto& json : json_configs) {
      ...
      std::string config_str = writer.write(json);
      printf("%s\n", config_str.c_str());

      // Create the configuration and parse the JSON into the config.
      VideoReceiveStream::Config receive_config =
          VideoReceiveStream::Config(&(stream_state->transport));
      ...
      for (const auto& ext_json : json["extensions"]) {
        receive_config.rtp.extensions.emplace_back(
            ext_json["uri"].asString(), ext_json["id"].asInt64(),
