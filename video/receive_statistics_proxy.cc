...
// A workaround for how some tests wrap the main thread and use it for multiple
// purposes (e.g. PeerConnectionBundleBaseTest).
webrtc::TaskQueueBase* GetCurrentTaskQueue() {
  webrtc::TaskQueueBase* ret = webrtc::TaskQueueBase::Current();
  if (!ret)
...
ReceiveStatisticsProxy::ReceiveStatisticsProxy(
  ...
  RTC_CHECK(worker_thread_) << "Should be running on the worker thread.";
...
void ReceiveStatisticsProxy::RtcpPacketTypesCounterUpdated(
  ...
  // This is pretty hacky. It turns out that RtpRtcp::Configuration has a single
  // RtcpPacketTypeCounterObserver and that the same configuration may be used
  // for both receiver and sender (see ModuleRtpRtcpImpl::ModuleRtpRtcpImpl).
  // The RTCPSender implementation makes callbacks on a process thread whereas
  // the RTCPReceiver implementation calls back on the worker thread.
  // The ReceiveStatisticsProxy is interested in receive statistics only, so
  // ideally there should be a way to specify separate callbacks, but we rely
  // on the thread check for now (previously just relied on the ssrc).
