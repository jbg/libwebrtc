...
class ReceiveStatisticsProxy2Test : public ::testing::Test {
 ...
 protected:
  ...
  VideoFrame CreateFrameWithRenderTimeMs(Timestamp render_time) {
    return CreateVideoFrame(kWidth, kHeight, render_time.ms());
  }
  ...
  Timestamp Now() {
    return Timestamp::Millis(fake_clock_.TimeInMilliseconds());
  }
...
TEST_F(ReceiveStatisticsProxy2Test, ZeroDelayReportedIfFrameNotDelayed) {
  ...
  const Timestamp kNowMs = Now();
  statistics_proxy_->OnRenderedFrame(
      MetaData(CreateFrameWithRenderTimeMs(kNowMs), kNowMs));
...
TEST_F(ReceiveStatisticsProxy2Test, DelayReportedIfFrameIsDelayed) {
  ...
  const Timestamp kNowMs = Now();
  statistics_proxy_->OnRenderedFrame(MetaData(
      CreateFrameWithRenderTimeMs(kNowMs - TimeDelta::Millis(1)), kNowMs));
...
TEST_F(ReceiveStatisticsProxy2Test, AverageDelayOfDelayedFramesIsReported) {
  ...
  const Timestamp kNowMs = Now();
  statistics_proxy_->OnRenderedFrame(MetaData(
      CreateFrameWithRenderTimeMs(kNowMs - TimeDelta::Millis(10)), kNowMs));
  statistics_proxy_->OnRenderedFrame(MetaData(
      CreateFrameWithRenderTimeMs(kNowMs - TimeDelta::Millis(6)), kNowMs));
  statistics_proxy_->OnRenderedFrame(
      MetaData(CreateFrameWithRenderTimeMs(kNowMs), kNowMs));
  statistics_proxy_->OnRenderedFrame(MetaData(
      CreateFrameWithRenderTimeMs(kNowMs + TimeDelta::Millis(1)), kNowMs));
