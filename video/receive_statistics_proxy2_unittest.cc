...
TEST_F(ReceiveStatisticsProxy2Test, ReportsFreezeMetrics) {
  ...
  for (size_t i = 0; i < VideoQualityObserver::kMinFrameSamplesToDetectFreeze;
    ...
    statistics_proxy_->OnRenderedFrame(
        VideoFrameMetaData(frame, fake_clock_.TimeInMilliseconds()));
  }
...
TEST_F(ReceiveStatisticsProxy2Test, GetStatsReportsReceivedFrameStats) {
  ...
  statistics_proxy_->OnRenderedFrame(VideoFrameMetaData(
      CreateFrame(kWidth, kHeight), fake_clock_.TimeInMilliseconds()));
...
TEST_F(ReceiveStatisticsProxy2Test, ZeroDelayReportedIfFrameNotDelayed) {
  ...
  statistics_proxy_->OnRenderedFrame(
      VideoFrameMetaData(CreateFrameWithRenderTimeMs(kNowMs), kNowMs));
...
TEST_F(ReceiveStatisticsProxy2Test, DelayReportedIfFrameIsDelayed) {
  ...
  const int64_t kNowMs = fake_clock_.TimeInMilliseconds();
  statistics_proxy_->OnRenderedFrame(
      VideoFrameMetaData(CreateFrameWithRenderTimeMs(kNowMs - 1), kNowMs));
...
TEST_F(ReceiveStatisticsProxy2Test, AverageDelayOfDelayedFramesIsReported) {
  ...
  const int64_t kNowMs = fake_clock_.TimeInMilliseconds();
  statistics_proxy_->OnRenderedFrame(
      VideoFrameMetaData(CreateFrameWithRenderTimeMs(kNowMs - 10), kNowMs));
  statistics_proxy_->OnRenderedFrame(
      VideoFrameMetaData(CreateFrameWithRenderTimeMs(kNowMs - 6), kNowMs));
  statistics_proxy_->OnRenderedFrame(
      VideoFrameMetaData(CreateFrameWithRenderTimeMs(kNowMs), kNowMs));
  statistics_proxy_->OnRenderedFrame(
      VideoFrameMetaData(CreateFrameWithRenderTimeMs(kNowMs + 1), kNowMs));
...
TEST_P(ReceiveStatisticsProxy2TestWithContent, FreezesAreReported) {
  ...
  for (int i = 0; i < kMinRequiredSamples; ++i) {
    VideoFrameMetaData meta(frame, fake_clock_.TimeInMilliseconds());
...
TEST_P(ReceiveStatisticsProxy2TestWithContent, TimeInHdReported) {
  ...
  // HD frames.
  for (int i = 0; i < kMinRequiredSamples; ++i) {
    VideoFrameMetaData meta(frame_hd, fake_clock_.TimeInMilliseconds());
    statistics_proxy_->OnDecodedFrame(meta, absl::nullopt, 0, content_type_);
    statistics_proxy_->OnRenderedFrame(meta);
    fake_clock_.AdvanceTimeMilliseconds(kInterFrameDelayMs);
  }
  ...
  for (int i = 0; i < 2 * kMinRequiredSamples; ++i) {
    VideoFrameMetaData meta(frame_sd, fake_clock_.TimeInMilliseconds());
    statistics_proxy_->OnDecodedFrame(meta, absl::nullopt, 0, content_type_);
    statistics_proxy_->OnRenderedFrame(meta);
    fake_clock_.AdvanceTimeMilliseconds(kInterFrameDelayMs);
  }
  // Extra last frame.
  statistics_proxy_->OnRenderedFrame(
      VideoFrameMetaData(frame_sd, fake_clock_.TimeInMilliseconds()));
...
TEST_P(ReceiveStatisticsProxy2TestWithContent, DownscalesReported) {
  ...
  FlushWorker();
  statistics_proxy_->OnRenderedFrame(
      VideoFrameMetaData(frame_hd, fake_clock_.TimeInMilliseconds()));
  fake_clock_.AdvanceTimeMilliseconds(kInterFrameDelayMs);
  ...
  // Downscale.
  statistics_proxy_->OnRenderedFrame(
      VideoFrameMetaData(frame_ld, fake_clock_.TimeInMilliseconds()));
  fake_clock_.AdvanceTimeMilliseconds(kInterFrameDelayMs);
