...
TEST_P(FrameBufferProxyTest, RtpTimestampRollover) {
  // Test inserting 31 frames and roll over on frame 32.
  constexpr uint32_t kBaseRtp =
      std::numeric_limits<uint32_t>::max() - kFps30Rtp * 31 + 1;
  ...
  constexpr uint32_t kRolloverRtp = kBaseRtp + 31 * kFps30Rtp;
  proxy_->InsertFrame(Builder()
                          .Id(31)
                          .Time(kRolloverRtp)
                          .ReceivedTime(clock_->CurrentTime())
                          .AsLast()
                          .Build());
  EXPECT_THAT(WaitForFrameOrTimeout(kFps30Delay), Frame(WithId(31)));
}

TEST_P(FrameBufferProxyTest, PausedStream) {
  ...
  // by over half.
  constexpr uint32_t kLastRtp = kBaseRtp + 30 * kFps30Rtp;
