...
class FrameCadenceAdapterImpl : public FrameCadenceAdapterInterface {
 public:
  FrameCadenceAdapterImpl(const WebRtcKeyValueConfig& field_trials,
                          Clock* clock,
                          TaskQueueBase* queue);
...
FrameCadenceAdapterInterface::Create(const WebRtcKeyValueConfig& field_trials,
                                     Clock* clock,
                                     TaskQueueBase* queue) {
  return std::make_unique<FrameCadenceAdapterImpl>(field_trials, clock, queue);
}
