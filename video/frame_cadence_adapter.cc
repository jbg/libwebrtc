...
class PassthroughAdapterMode : public AdapterMode {
 public:
  ...
  void OnFrame(Timestamp post_time,
    ...
    RTC_DLOG(LS_INFO) << "Pass::OnFrame(post_time=" << post_time
...
class ZeroHertzAdapterMode : public AdapterMode {
 public:
  ...
  // Always called during construction using `max_frame_rate` from the
  // initializer list which allows "inheritance" of previous frame rate
...
void ZeroHertzAdapterMode::ScheduleRepeat(int frame_id, bool idle_repeat) {
  ...
  RTC_DLOG(LS_INFO) << __func__ << "(frame_id=" << frame_id
                    << ", idle_repeat=" << idle_repeat << ") ["
  ...
  if (is_zero_hertz_enabled) {
    ...
    if (!was_zero_hertz_enabled || max_fps_has_changed) {
      ...
      zero_hertz_adapter_.emplace(queue_, clock_, callback_,
                                  source_constraints_->max_fps.value());
      // Maintain the current frame-rate restriction but only if the new max
      // rate is higher.
      if (restricted_max_frame_rate_ < source_constraints_->max_fps.value()) {
