...
void VideoStreamDecoderImpl::DecodeThreadFunction(void* ptr) {
  ...
  // supported
  //                 by the |frame_buffer_|.
  static constexpr int kForever = 100000000;

  int wait_time_ms = kForever;
  bool keyframe_required = true;
  auto* vs_decoder = static_cast<VideoStreamDecoderImpl*>(ptr);
  while (true) {
    DecodeResult decode_result =
        vs_decoder->Decode(wait_time_ms, keyframe_required);

    if (decode_result == kShutdown)
      return;

    if (decode_result == kOk) {
      wait_time_ms = kForever;
      keyframe_required = false;
      continue;
    }

    if (decode_result == kDecodeFailure) {
      wait_time_ms = 0;
      keyframe_required = true;
      continue;
    }

    if (decode_result == kNoFrame) {
...
VideoStreamDecoderImpl::DecodeResult VideoStreamDecoderImpl::Decode(
    int wait_time_ms,
    bool keyframe_required) {
  std::unique_ptr<video_coding::EncodedFrame> frame;
  video_coding::FrameBuffer::ReturnReason res =
      frame_buffer_.NextFrame(wait_time_ms, &frame, keyframe_required);
