...
OveruseFrameDetector::~OveruseFrameDetector() {
  RTC_DCHECK(!check_overuse_task_.Running())
      << "StopCheckForOverUse must be called.";
}
...
void OveruseFrameDetector::StartCheckForOveruse(
  ...
  SetOptions(options);
  check_overuse_task_.Start(TimeDelta::ms(kTimeToFirstCheckForOveruseMs),
                            RepeatingTask::IntervalMode::kExcludingExecution,
...
void OveruseFrameDetector::StopCheckForOveruse() {
  RTC_DCHECK_CALLED_SEQUENTIALLY(&task_checker_);
  if (check_overuse_task_.Running()) {
