...
class OveruseFrameDetector::SendProcessingUsage {
 public:
  ...
  void Reset() {
    ...
    load_estimate_ = (options_.low_encode_usage_threshold_percent +
                      options_.high_encode_usage_threshold_percent) / 200.0;
  }
 ...
 private:
  ...
  // TODO(nisse): Move to CpuOveruseOptions, and delete unused options.
  const double kTau_ = 5.0;  // Five second average.
...
void OveruseFrameDetector::FrameEncoded(int64_t capture_time_us,
  ...
  if (FrameSizeChanged(width * height) ||
      FrameTimeoutDetected(capture_time_us)) {
  ...
  if (encoder_timing_) {
    ...
    encoder_timing_->OnEncodeTiming(capture_time_us /
