 ...
 protected:
  VideoSendStreamImplTest()
      : clock_(1000 * 1000 * 1000),
        ...
        test_queue_(field_trials_,
                    "test_queue",
                    CreateDefaultTaskQueueFactory().get()),
        stats_proxy_(&clock_,
                     config_,
    ...
    ON_CALL(transport_controller_, GetWorkerQueue())
        .WillByDefault(Return(&test_queue_));
  }
  ~VideoSendStreamImplTest() {}
  ...
  std::unique_ptr<VideoSendStreamImpl> CreateVideoSendStreamImpl(
    ...
    auto ret = std::make_unique<VideoSendStreamImpl>(
        &clock_, &stats_proxy_, &transport_controller_, &bitrate_allocator_,
    ...
    // By executing a task asynchrounouse, we ensure no tasks are pending.
    test_queue_.RunSynchronous([] {});
...
TEST_F(VideoSendStreamImplTest, DisablesPaddingOnPausedEncoder) {
  ...
  rtc::Event done;
  test_queue_.TaskQueueForDelayedTasks()->PostDelayedTask(
      [&] {
