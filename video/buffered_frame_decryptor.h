...
class BufferedFrameDecryptor final {
 public:
  using OnDecryptedFrameCallback =
      std::function<void(std::unique_ptr<video_coding::RtpFrameObject>)>;
 ...
 private:
  ...
  rtc::scoped_refptr<FrameDecryptorInterface> frame_decryptor_
      RTC_GUARDED_BY(network_tc_);
  rtc::ThreadChecker network_tc_;
  OnDecryptedFrameCallback decrypted_frame_callback_;
