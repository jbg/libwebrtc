...
void VideoStreamBufferController::UpdateJitterBufferDelay(
    ...
    Timestamp now) {
  // The stats spec [1] mandates that this
  // "video frame takes from the time the first packet is received by the jitter
  // buffer (ingest timestamp) to the time it exits the jitter buffer (emit
  // timestamp)" Since the "video jitter buffer" is not a monolith in this code
  // base, we define "ingest timestamp" == first packet received by xyz "emit
  // timestamp" == the decodable frame is released for decoding. Note that this
  // does NOT include any thread hops caused after this method. Those thread
  // hops are, on the other hand, measured by totalProcessingDelay.
  //
  // [1]:
  // https://w3c.github.io/webrtc-stats/#dom-rtcinboundrtpstreamstats-jitterbufferemittedcount
  TimeDelta jitter_buffer_delay = now - receive_time;
