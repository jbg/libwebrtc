...
void VideoStreamBufferController::FrameReadyForDecode(uint32_t rtp_timestamp,
  ...
  if (!decodable_tu_info) {
    RTC_LOG(LS_ERROR)
        ...
        << rtp_timestamp
        << ". Cancelling the decode of this frame, and decoding "
           "will resume again when the frame buffer becomes decodable again.";
    return;
  }
  RTC_DCHECK(decodable_tu_info->next_rtp_timestamp == rtp_timestamp)
      << "Frame buffer's next decodable frame was not the one sent for "
         "extraction. Expected frame with rtp-timestamp "
      << rtp_timestamp << " but next decodable rtp timestamp is "
