...
TEST_F(ReceiveStatisticsProxyTest, GetStatsReportsDecodeTimingStats) {
  ...
  EXPECT_EQ(kJitterBufferDelayMs, stats.jitter_buffer_delay_seconds * 1000);
  EXPECT_EQ(kTargetDelayMs, stats.jitter_buffer_target_delay_seconds * 1000);
  EXPECT_EQ(1u, stats.jitter_buffer_emitted_count);
  EXPECT_EQ(kMinimumDelayMs, stats.jitter_buffer_minimum_delay_seconds * 1000);
}
...
TEST_F(ReceiveStatisticsProxyTest, CumulativeDecodeGetStatsAccumulate) {
  ...
  statistics_proxy_->OnDecodableFrame(kJitterBufferDelayMs, kTargetDelayMs,
                                      kMinimumDelayMs);
  VideoReceiveStreamInterface::Stats stats = FlushAndGetStats();
  EXPECT_EQ(2 * kJitterBufferDelayMs, stats.jitter_buffer_delay_seconds * 1000);
  EXPECT_EQ(2 * kTargetDelayMs,
            stats.jitter_buffer_target_delay_seconds * 1000);
  EXPECT_EQ(2u, stats.jitter_buffer_emitted_count);
  EXPECT_EQ(2 * kMinimumDelayMs,
            stats.jitter_buffer_minimum_delay_seconds * 1000);
}
