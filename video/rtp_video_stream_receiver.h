 ...
 private:
  ...
  class RtcpFeedbackBuffer : public KeyFrameRequestSender,
   ...
   private:
    ...
    rtc::CriticalSection nack_cs_;

    // Key-frame-request-related state.
    bool request_key_frame_ RTC_GUARDED_BY(nack_cs_);

    // NACK-related state.
    std::vector<uint16_t> nack_sequence_numbers_ RTC_GUARDED_BY(nack_cs_);
    ...
    absl::optional<LossNotificationState> lntf_state_ RTC_GUARDED_BY(nack_cs_);
  };
