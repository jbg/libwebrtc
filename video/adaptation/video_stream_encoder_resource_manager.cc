 ...
 public:
  ...
  void OnEncoderSettingsUpdated(
    ...
    const bool source_resolution_changed =
        ...
        adaptation_counters == last_adaptation_counters_;
...
void VideoStreamEncoderResourceManager::OnVideoSourceRestrictionsUpdated(
  ...
  RTC_DCHECK_RUN_ON(encoder_queue_);
  if (video_source_restrictions_.max_frame_rate() ==
      restrictions.max_frame_rate()) {
    // Ignore adaptation in framerate. |current_adaptation_counters_| is used to
    // detect changes in source resolution not caused by adapatation.
