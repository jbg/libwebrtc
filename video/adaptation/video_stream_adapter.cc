...
Adaptation::Adaptation(Step step)
    : status_(Status::kIsValid),
      step_(std::move(step)),
      min_pixel_limit_reached_(false) {}

Adaptation::Adaptation(Step step, bool min_pixel_limit_reached)
    : status_(Status::kIsValid),
      step_(std::move(step)),
  ...
  switch (EffectiveDegradationPreference()) {
    case DegradationPreference::BALANCED: {
      ...
      if (source_restrictor_->CanIncreaseFrameRateTo(target_fps)) {
        return Adaptation(Adaptation::Step(
            Adaptation::StepType::kIncreaseFrameRate, target_fps));
      }
    ...
    case DegradationPreference::MAINTAIN_FRAMERATE: {
      ...
      return Adaptation(Adaptation::Step(
          Adaptation::StepType::kIncreaseResolution, target_pixels));
    }
  ...
  switch (EffectiveDegradationPreference()) {
    case DegradationPreference::BALANCED: {
      ...
      if (source_restrictor_->CanDecreaseFrameRateTo(target_fps)) {
        return Adaptation(Adaptation::Step(
            Adaptation::StepType::kDecreaseFrameRate, target_fps));
      }
...
VideoSourceRestrictions VideoStreamAdapter::PeekNextRestrictions(
  ...
  restrictor_copy.ApplyAdaptationSteps(adaptation.step(),
...
ResourceListenerResponse VideoStreamAdapter::ApplyAdaptation(
  ...
  // Adapt!
  source_restrictor_->ApplyAdaptationSteps(adaptation.step(),
                                           EffectiveDegradationPreference());
