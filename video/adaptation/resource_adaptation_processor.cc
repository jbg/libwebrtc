...
void ResourceAdaptationProcessor::OnResourceUnderuse(
  ...
  if (adaptation.status() != Adaptation::Status::kIsValid)
    return;
  // Apply target.
  stream_adapter_->ApplyAdaptation(adaptation, encoder_settings_, input_mode,
...
ResourceListenerResponse ResourceAdaptationProcessor::OnResourceOveruse(
  ...
  if (adaptation.status() != Adaptation::Status::kIsValid)
    return ResourceListenerResponse::kNothing;
  // Apply target.
  ResourceListenerResponse response = stream_adapter_->ApplyAdaptation(
