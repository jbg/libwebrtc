...
VideoStreamEncoder::ParseEncoderSwitchFieldTrial() const {
  ...
  // "|" character.
  webrtc::FieldTrialOptional<std::string> codec_thresholds{"codec_thresholds"};
  ...
  webrtc::ParseFieldTrial(
      {&codec_thresholds, &to_codec, &to_param, &to_value, &window},
      webrtc::field_trial::FindFullName(
          "WebRTC-NetworkCondition-EncoderSwitch"));

  if (!codec_thresholds || !to_codec || !window) {
    return {};
  }
  ...
  if (rtc::split(*codec_thresholds, '|', &from_split) == 0) {
    return {};
  }

  for (const std::string& from_setting : from_split) {
    std::vector<std::string> from_setting_split;
    if (rtc::split(from_setting, ';', &from_setting_split) != 3) {
      return {};
    }

    VideoCodecType codec = PayloadStringToCodecType(from_setting_split[0]);
    int bitrate_kbps;
    rtc::FromString(from_setting_split[1], &bitrate_kbps);
    int pixel_count;
    rtc::FromString(from_setting_split[2], &pixel_count);
