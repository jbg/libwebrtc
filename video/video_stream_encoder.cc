...
void ApplyVideoCodecBitrateLimits(const VideoEncoder::EncoderInfo& encoder_info,
  ...
  if (!(codec->codecType == VideoCodecType::kVideoCodecVP9 ||
    ...
    // ApplyVideoCodecBitrateLimits() supports VP9 and AV1 with scalability mode
    // set only.
    return;
  }
...
void VideoStreamEncoder::ReconfigureEncoder() {
  ...
  } else if (encoder_config_.codec_type == kVideoCodecAV1) {
    log_stream << "AV1 Scalability Mode is not set.\n";
  }
