...
#include <stdio.h>
FILE* g_yuv_dump = nullptr;

void VideoStreamEncoder::OnFrame(const VideoFrame& video_frame) {
  RTC_DCHECK_RUNS_SERIALIZED(&incoming_frame_race_checker_);
  VideoFrame incoming_frame = video_frame;

  if (g_yuv_dump == nullptr) {
    g_yuv_dump = fopen("/Users/ssilkin/tmp/input.yuv", "wb");
  }

  auto yuv_buffer = video_frame.video_frame_buffer()->GetI420();
  fwrite(yuv_buffer->DataY(), video_frame.height() * yuv_buffer->StrideY(), 1,
         g_yuv_dump);
  fwrite(yuv_buffer->DataU(), video_frame.height() / 2 * yuv_buffer->StrideU(),
         1, g_yuv_dump);
  fwrite(yuv_buffer->DataV(), video_frame.height() / 2 * yuv_buffer->StrideV(),
         1, g_yuv_dump);
