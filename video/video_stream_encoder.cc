...
void VideoStreamEncoder::ReconfigureEncoder() {
  ...
  std::unique_ptr<VideoBitrateAllocator> new_rate_allocator =
      settings_.bitrate_allocator_factory->CreateVideoBitrateAllocator(codec);
  const bool created_rate_allocator =
      rate_allocator_ == nullptr && new_rate_allocator != nullptr;
  rate_allocator_ = std::move(new_rate_allocator);
  ...
  if (created_rate_allocator && last_observed_bitrate_bps_ > 0) {
    // A rate allocator has been created after a target bitrate has been
    // set. Create an allocation, potentially signaling it to observers,
    // and configure the encoder.
    // TODO(sprang): Only updating on the first creation of an allocator
    //               preserves existing behavior. Consider if we should
    //               update tests to always expect a rate allocation here.
...
EncodedImageCallback::Result VideoStreamEncoder::OnEncodedImage(
  ...
  if (result.error == Result::OK) {
    ...
    // atomic flag. Thi is because we can't easily wait for the worker thread
    ...
    bool kTestValue = true;
    const bool kConsumedValue = false;
    result.drop_next_frame = drop_next_frame_.compare_exchange_strong(
        kTestValue, kConsumedValue, std::memory_order_release,
