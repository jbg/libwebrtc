...
bool VideoStreamEncoder::DropDueToSize(uint32_t pixel_count) const {
  ...
  } else if (encoder_start_bitrate_bps_ < 300000 /* qvga */) {
...
bool VideoStreamEncoder::CanAdaptUpResolution(int pixels,
  ...
  printf("\n%d %d", bitrate_bps, bitrate_limits->min_bitrate_bps);
  fflush(stdout);
  RTC_DCHECK_GE(bitrate_limits->frame_size_pixels, pixels);
  return bitrate_bps >= static_cast<uint32_t>(bitrate_limits->min_bitrate_bps);
}
