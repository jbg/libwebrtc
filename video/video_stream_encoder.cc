...
void VideoStreamEncoder::ReconfigureEncoder() {
  ...
  if (!rate_allocator_) {
    rate_allocator_ = absl::make_unique<DefaultVideoBitrateAllocator>(codec);
  }
  ...
  if (pending_encoder_creation_) {
    ...
    {
      rtc::CritScope cs(&frame_control_lock_);
      encoder_has_internal_source_ = info.has_internal_source;
    }
  }
  ...
  uint32_t framerate_fps;
  {
    ...
    framerate_fps = input_framerate_.Rate(clock_->TimeInMilliseconds())
                        .value_or(max_framerate_);
  ...
  if (last_observed_bitrate_bps_ > 0) {
    bitrate_allocation = rate_allocator_->GetAllocation(
...
VideoStreamEncoder::GetBitrateAllocationAndNotifyObserver(
  ...
  if (!rate_allocator_) {
    // WHY?!
    bitrate_allocation.SetBitrate(0, 0, target_bitrate_bps * 1000);
  } else if (target_bitrate_bps > 0) {
    bitrate_allocation =
...
EncodedImageCallback::Result VideoStreamEncoder::OnEncodedImage(
  ...
  if (result.error == Result::OK) {
    ...
    if (encoded_image.size() > 0) {
      frame_dropper_.Fill(encoded_image.size(),
                          encoded_image._frameType != kVideoFrameKey);
    }

    if (encoder_has_internal_source_) {
      // Update frame dropper after the fact for internal sources. Default to
      // 30fps so we don't need to cach another max_farmerate_. This should not
      // matter since it will only affect the first two frames.
      const uint32_t kDefaultFps = 30;
      frame_dropper_.Leak(input_framerate_.Rate(clock_->TimeInMilliseconds())
                              .value_or(kDefaultFps));
      // Signal to encoder to drop next frame.
      result.drop_next_frame = frame_dropper_.DropFrame();
    }
  }
