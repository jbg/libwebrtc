...
bool VideoStreamEncoder::DropDueToSize(uint32_t pixel_count) const {
  ...
  uint32_t bitrate_bps = encoder_target_bitrate_bps_.value();
  if (stream_resource_manager_.UseBandwidthAllocationBps()) {
    bitrate_bps = stream_resource_manager_.UseBandwidthAllocationBps().value();
  }
