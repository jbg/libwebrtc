...
bool PartialFrameDecompressor::ApplyPartialUpdate(
  ...
  if (input_buffer->height() == partial_desc->full_height &&
    ...
    if ((input_buffer->width() % 2 != 0 &&
         input_buffer->width() + partial_desc->offset_x <
             partial_desc->full_width) ||
        (input_buffer->height() % 2 != 0 &&
         input_buffer->height() + partial_desc->offset_y <
             partial_desc->full_height)) {
    ...
    if (partial_desc->offset_x + input_buffer->width() >
            partial_desc->full_width ||
        partial_desc->offset_y + input_buffer->height() >
            partial_desc->full_height) {
    ...
    if (input_buffer->width() > 0 && input_buffer->height() > 0) {
      cached_frame_buffer_->PasteFrom(*input_buffer->ToI420().get(),
                                      partial_desc->offset_x,
                                      partial_desc->offset_y);
      uncompressed_frame->set_unchanged(false);
    } else {
      uncompressed_frame->set_unchanged(true);
    }
