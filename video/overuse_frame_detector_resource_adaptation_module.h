...
class OveruseFrameDetectorResourceAdaptationModule
 ...
 private:
  ...
  struct AdaptationTarget {
    ...
    // Allow this struct to be instantiated as as an optional, even though it's
  ...
  absl::optional<AdaptationTarget> GetAdaptDownTarget(
      ...
      AdaptationObserverInterface::AdaptReason reason) const;
  void PerformAdaptationTo(const AdaptationTarget& target,
                           int min_pixels_per_frame,
                           AdaptationObserverInterface::AdaptReason reason);

  // Applies the adaptation up/down target if there is one.
