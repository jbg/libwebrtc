...
bool VideoAnalyzer::IsInSelectedSpatialAndTemporalLayer(
  ...
  if (rtp_packet.PayloadType() != test::CallTest::kPayloadTypeVP9 &&
      rtp_packet.PayloadType() != test::CallTest::kPayloadTypeVP8) {
    return true;
  } else {
    ...
    std::unique_ptr<VideoRtpDepacketizer> depacketizer =
        CreateVideoRtpDepacketizer(is_vp8 ? kVideoCodecVP8 : kVideoCodecVP9);
    auto parsed_payload = depacketizer->Parse(rtp_packet.RtpPayload());
