...
TimeDelta DetermineMaxWaitForFrame(const TimeDelta rtp_history,
                                   bool is_keyframe) {
...
void VideoReceiveStream2::SetNackHistory(int history_ms) {
  ...
  decode_queue_.PostTask([this, history_ms = TimeDelta::Millis(history_ms)]() {
    RTC_DCHECK_RUN_ON(&decode_queue_);
    max_wait_for_keyframe_ = DetermineMaxWaitForFrame(history_ms, true);
    max_wait_for_frame_ = DetermineMaxWaitForFrame(history_ms, false);
  });

  // TODO(tommi): Update frame buffer since it's based on max values:
  /*
  frame_buffer_ = FrameBufferProxy::CreateFromFieldTrial(
      clock_, call_->worker_thread(), timing_.get(), &stats_proxy_,
      decode_queue_.Get(), this, max_wait_for_keyframe_, max_wait_for_frame_,
