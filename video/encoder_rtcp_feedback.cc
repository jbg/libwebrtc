...
void EncoderRtcpFeedback::OnReceivedLossNotification(
  ...
  const std::vector<uint16_t> seq_nums = std::vector<uint16_t>(
      seq_num_of_last_decodable, seq_num_of_last_received);
  ...
  printf("OnReceivedLossNotification(%u, %u, %u, %d)\n", ssrc,
         last_decodable.timestamp, last_received.timestamp, decodability_flag);
