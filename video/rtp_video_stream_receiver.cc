...
void RtpVideoStreamReceiver::NotifyReceiverOfEmptyPacket(uint16_t seqno) {
  reference_finder_->PaddingReceived(seqno);
  packet_buffer_->PaddingReceived(seqno);
}

void RtpVideoStreamReceiver::NotifyReceiverOfFecPacket(
    const RTPHeader& header) {
  ntp_estimator_.Estimate(header.timestamp);
  if (nack_module_) {
