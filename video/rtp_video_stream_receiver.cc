...
RtpVideoStreamReceiver::RtpVideoStreamReceiver(
  ...
  if (webrtc::field_trial::IsEnabled("WebRTC-RtcpLossNotification")) {
    ...
    RTC_NOTREACHED();  // TODO: !!!
    // TODO: !!! See why we're sending so many key frames when this is
    // not defined.
...
int32_t RtpVideoStreamReceiver::OnReceivedPayloadData(
  ...
  if (loss_notification_controller_) {
    printf("is_recovered = %d\n", is_recovered);
    if (!is_recovered) {  // TODO: !!!
...
void RtpVideoStreamReceiver::SendLossNotification(
  ...
  // TODO: !!! Why is decodability_flag always off?
  printf("RtpVideoStreamReceiver::SendLossNotification(%u, %u, %d)\n",
...
void RtpVideoStreamReceiver::OnAssembledFrame(
  ...
  RTC_DCHECK(bool(descriptor));  // TODO: !!! Remove.

  if (loss_notification_controller_ && descriptor) {
    // TODO(eladalon): Pass |*frame| in.
...
void RtpVideoStreamReceiver::NotifyReceiverOfEmptyPacket(uint16_t seq_num) {
  ...
  if (loss_notification_controller_) {
    RTC_NOTREACHED();  // TODO: !!! Remove.
    RTC_LOG(LS_WARNING) << "Cannot handle empty packet.";
  }
}
