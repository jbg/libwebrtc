...
void RtpVideoStreamReceiver::AddReceiveCodec(
    ...
    bool raw_payload) {
  std::unique_ptr<RtpVideoDepacketizer> video_type;
  if (raw_payload) {
    video_type = std::make_unique<RtpVideoDepacketizerRaw>();
  } else {
    video_type = RtpVideoDepacketizerCreate(video_codec.codecType);
  }
  depacketizers_.emplace(video_codec.plType, std::move(video_type));
...
void RtpVideoStreamReceiver::ReceivePacket(const RtpPacketReceived& packet) {
  ...
  absl::optional<RtpVideoDepacketizer::Parsed> parsed =
