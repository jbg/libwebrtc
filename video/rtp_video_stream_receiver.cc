...
void RtpVideoStreamReceiver::OnReceivedPayloadData(
  ...
  OnAssembledFrames(assembled_frames);
}
...
RtpVideoStreamReceiver::OnInsertedPacket(
  ...
  for (auto& packet : result.packets) {
    ...
    if (packet->is_first_packet_in_frame()) {
      ...
      min_recv_time = packet_info.receive_time_ms();
      max_recv_time = packet_info.receive_time_ms();
      payloads.clear();
      packet_infos.clear();
    } else {
      max_nack_count = std::max(max_nack_count, packet->times_nacked);
      min_recv_time = std::min(min_recv_time, packet_info.receive_time_ms());
      max_recv_time = std::max(max_recv_time, packet_info.receive_time_ms());
    }
...
void RtpVideoStreamReceiver::OnAssembledFrames(
...
void RtpVideoStreamReceiver::NotifyReceiverOfEmptyPacket(uint16_t seq_num) {
  ...
  {
    MutexLock lock(&packet_buffer_lock_);
    assembled_frames = OnInsertedPacket(packet_buffer_.InsertPadding(seq_num));
  }
  OnAssembledFrames(assembled_frames);
  if (nack_module_) {
