...
void RtpVideoStreamReceiver::OnReceivedFrame(
  ...
  if (frame_decryptor_ != nullptr) {
    ...
    // Retrieve the header that is authenticated additional data.
    rtc::ArrayView<const uint8_t> descriptor_bitstream =
        descriptor->GetByteRepresentation();
    ...
    if (frame_decryptor_->Decrypt(
            ...
            /*additional_data=*/descriptor_bitstream, encrypted_frame_bitstream,
