...
VideoSendStream::VideoSendStream(
    ...
    : rtp_transport_queue_(transport->GetWorkerQueue()),
      ...
      send_stream_(
          std::make_unique<VideoSendStreamImpl>(clock,
                                                &stats_proxy_,
...
VideoSendStream::~VideoSendStream() {
  ...
  // TODO(tommi): What if StopPermanently hasn't been called? (just stopped)
  RTC_DCHECK(!running_);
}
...
void VideoSendStream::Stop() {
  ...
  running_ = false;
  send_stream_->Stop();  // Stop() will proceed asyncrhonously.
}
  ...
  if (running_) {
    ...
    rtp_transport_queue_->PostTask([this, rtp_state_map, payload_state_map]() {
      ...
      *payload_state_map = send_stream_->GetRtpPayloadStates();
      // TODO(tommi): Delete rtp_video_sender_ here by calling
