...
class VideoSendStreamImpl : public webrtc::BitrateAllocatorObserver,
 ...
 private:
  ...
  class PacketAddedTask;
  class PacketFeedBackVectorTask;
...
class VideoSendStreamImpl::PacketAddedTask : public rtc::QueuedTask {
 public:
  PacketAddedTask(const rtc::WeakPtr<VideoSendStreamImpl>& send_stream,
                  uint32_t ssrc,
                  uint16_t seq_num)
      : send_stream_(std::move(send_stream)), ssrc_(ssrc), seq_num_(seq_num) {}

 private:
  bool Run() override {
    if (send_stream_)
      send_stream_->OnPacketAdded(ssrc_, seq_num_);
    return true;
  }
...
class VideoSendStreamImpl::PacketFeedBackVectorTask : public rtc::QueuedTask {
 public:
  PacketFeedBackVectorTask(
      const rtc::WeakPtr<VideoSendStreamImpl>& send_stream,
 ...
 private:
  bool Run() override {
    if (send_stream_)
      send_stream_->OnPacketFeedbackVector(packet_feedback_vector_);
    return true;
  }
...
void VideoSendStreamImpl::OnPacketAdded(uint32_t ssrc, uint16_t seq_num) {
  if (!worker_queue_->IsCurrent()) {
    worker_queue_->PostTask(std::unique_ptr<rtc::QueuedTask>(
        new PacketAddedTask(weak_ptr_, ssrc, seq_num)));
    return;
  }
...
void VideoSendStreamImpl::OnPacketFeedbackVector(
  ...
  if (!worker_queue_->IsCurrent()) {
    worker_queue_->PostTask(std::unique_ptr<rtc::QueuedTask>(
        new PacketFeedBackVectorTask(weak_ptr_, packet_feedback_vector)));
    return;
  }
