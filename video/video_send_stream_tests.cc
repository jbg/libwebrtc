...
class MaxPaddingSetTest : public test::SendTest {
 public:
  ...
  MaxPaddingSetTest(bool test_switch_content_type,
      ...
      : SendTest(test::CallTest::kDefaultTimeoutMs),
        initial_running_without_padding_(test_switch_content_type),
        current_running_without_padding_(test_switch_content_type),
  ...
  void ModifyVideoConfigs(
    ...
    RTC_DCHECK_RUN_ON(&task_queue_thread_);
    RTC_DCHECK_EQ(1, encoder_config->number_of_streams);
    if (current_running_without_padding_) {
  ...
  Action OnSendRtp(const uint8_t* packet, size_t length) override {
    ...
    task_queue_->PostTask([this]() {
      ...
      if (current_running_without_padding_) {
        ...
        current_running_without_padding_ = false;
  ...
  void PerformTest() override {
    if (initial_running_without_padding_) {
      ASSERT_TRUE(
 ...
 private:
  ...
  const bool initial_running_without_padding_;
  bool current_running_without_padding_ RTC_GUARDED_BY(task_queue_thread_);
