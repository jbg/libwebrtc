 ...
 public:
  ...
  ContentSwitchTest()
      : SendTest(test::CallTest::kDefaultTimeoutMs),
        ...
        send_stream_config_(nullptr),
        packets_sent_(0),
        stream_resetter_(nullptr) {}

  void SetResetterFunction(T* stream_reset_fun) {
    stream_resetter_ = stream_reset_fun;
    RTC_DCHECK(stream_resetter_);
  }
...
TEST_F(VideoSendStreamTest, SwitchesToScreenshareAndBack) {
  typedef std::function<void(const VideoSendStream::Config&,
                             const VideoEncoderConfig&)>
      ResetterFn;
  ContentSwitchTest<ResetterFn> test;
  ResetterFn reset_fun = [this, &test](
                             const VideoSendStream::Config& send_stream_config,
                             const VideoEncoderConfig& encoder_config) {
  ...
  test.SetResetterFunction(&reset_fun);
  RunBaseTest(&test);
}
