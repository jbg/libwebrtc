...
void VideoSendStreamTest::TestReconfigureEncoder(
    ...
    int expected_num_init_encode) {
  class ConfigurationObserver : public test::SendTest,
                                public test::FakeEncoder,
                                public VideoBitrateAllocatorFactory {
   public:
    ConfigurationObserver(const std::vector<VideoStream>& configs,
                          int expected_num_init_encode,
                          TaskQueueBase* task_queue)
        : SendTest(test::CallTest::kDefaultTimeoutMs),
          FakeEncoder(Clock::GetRealTimeClock()),
    ...
    void VerifyConfig(const VideoCodec& codec) const {
    ...
    void PerformTest() override {
      ...
      EXPECT_EQ(3, num_bitrate_allocators_);
