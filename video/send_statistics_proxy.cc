...
rtc::Optional<int> GetFallbackMaxPixels(const std::string& group) {
  ...
  int max_pixels;
  int min_bps;
  if (sscanf(group.c_str(), "abled-%d,%d,%d", &min_pixels, &max_pixels,
             &min_bps) != 3) {
    return rtc::Optional<int>();
  }
...
rtc::Optional<int> GetFallbackMaxPixelsIfFieldTrialEnabled() {
  ...
  return (group.find("Enabled") == 0) ? GetFallbackMaxPixels(group.substr(2))
                                      : rtc::Optional<int>();
}
...
rtc::Optional<int> GetFallbackMaxPixelsIfFieldTrialDisabled() {
  std::string group =
      webrtc::field_trial::FindFullName(kVp8ForcedFallbackEncoderFieldTrial);
  return (group.find("Disabled") == 0) ? GetFallbackMaxPixels(group.substr(3))
                                       : rtc::Optional<int>();
}
}  // namespace
