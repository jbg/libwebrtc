...
function rename_in_build_file {
  ...
  new_file_name=$(echo "$new_file_path" |
      sed "s#$(dirname "$old_file_path")/##")
...
function rename_in_deps {
  ...
  while read -r referencer_path && [[ -n "$referencer_path" ]]; do
    echo referencer
    echo "$referencer_path"
    sed -i "s#$(deps_reference "$old_path")#$(deps_reference "$new_path")#g" \
        "$referencer_path"
    let count=count+1
  ...
  deps_path="$(dirname "$old_path")/DEPS"
  if [ -f "$deps_path" ]; then
    function deps_entry {
      echo "\"$(basename "$1" .h)\\\\\\.h\":"
    }
    sed -i "s#$(deps_entry "$old_path")#$(deps_entry "$new_path")#g" \
        "$deps_path"
  fi
}
...
function update_all_includes {
  ...
  new_header_path="$2"
  count=0
  for read -r includer_path && [[ -n "$includer_path" ]]; do
