...
from check_third_party_changes import GetChromiumOwnedAddedDeps, \
  CheckNoNotOwned3ppDeps, CheckNoBothOwned3ppDeps, \
  CheckNoChangesInAutoImportedDeps
...
class CheckThirdPartyChangesTest(unittest.TestCase):
  ...
  def testGetChromiumOwnedAddedDeps(self):
    ...
    added_deps = GetChromiumOwnedAddedDeps(self._input_api)
  ...
  def testCheckNoNotOwned3ppDepsFire(self):
    self._input_api.presubmit_local_path = os.path.join(TEST_DATA_DIR,
                                                        'not_owned_dep')
    errors = CheckNoNotOwned3ppDeps(self._input_api, self._output_api,
                                    ['webrtc'], ['chromium'])
  ...
  def testCheckNoNotOwned3ppDepsNotFire(self):
    self._input_api.presubmit_local_path = os.path.join(TEST_DATA_DIR,
                                                        'not_owned_dep')
    errors = CheckNoNotOwned3ppDeps(self._input_api, self._output_api,
                                    ['webrtc', 'not_owned'], ['chromium'])
    self.assertEqual(len(errors), 0, errors)

  def testCheckNoBothOwned3ppDepsFire(self):
    errors = CheckNoBothOwned3ppDeps(self._output_api, ['foo', 'bar'],
                                     ['buzz', 'bar'])
    self.assertEqual(len(errors), 1)
  ...
  def testCheckNoChangesInAutoImportedDepsFire(self):
    ...
    errors = CheckNoChangesInAutoImportedDeps(self._input_api, self._output_api,
                                              ['webrtc'], ['chromium'], [],
                                              None)
    self.assertEqual(len(errors), 1)
