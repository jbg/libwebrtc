...
def BelongsToTestTarget(file_path, out_dir):
  """Returns True is file_path is part of a testonly build target.

  This function relies on 'gn refs'. It actually runs:
  $ gn refs <out_dir> file_path --testonly=true`

  Which returns a list of build targets that contain file_path (well, only
  one build target should own a file, but that is not a strict rule).

  It the list is empty, it means the file is owned by a non-testonly build
