...
GRADLEW_BIN = os.path.join(CHECKOUT_ROOT, 'examples/androidtests/third_party/gradle/gradlew')
...
ARR_PROJECT_DEPENDENCY = "implementation 'org.webrtc:google-webrtc:1.0.+'"
AAR_PROJECT_VERSION_DEPDENDENCY = "implementation 'org.webrtc:google-webrtc:%s'"
...
def _TestAAR(tmp_dir, username, password, version):
  ...
  try:
    ...
    if ARR_PROJECT_DEPENDENCY not in gradle_app:
      raise Exception('Dependency not found in the build file.')
    # Set version to the version to be tested.
    target_dependency = AAR_PROJECT_VERSION_DEPDENDENCY % version
    gradle_app = gradle_app.replace(ARR_PROJECT_DEPENDENCY, target_dependency)
...
def _PublishAAR(user, password, version, additional_args):
  args = {
    'publish_wait_for_secs': -1 # Publish asynchronously.
  }
