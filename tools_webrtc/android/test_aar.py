...
ARCHS = ['arm64-v8a', 'x86_64']  # TODO(daniel.l): rollback
...
def _TestAAR(build_dir):
    ...
    try:
        ...
        # Then run the tests.
        subprocess.check_call([GRADLEW_BIN, 'connectedDebugAndroidTest', '-PaarDir=' + os.path.abspath(build_dir)],
                              cwd=AAR_PROJECT_DIR)
...
def BuildAndTestAar(use_goma, skip_tests, build_dir):
    ...
    logging.info('Building and Testing AAR version %s with hash %s', version, commit)
    ...
    try:
        ...
        BuildAar(ARCHS,
                 ...
                 extra_gn_args=["cc_wrapper=\"ccache\""]) # TODO(daniel.l): remove
