...
class MetaBuildWrapper(object):
  ...
  def GetSwarmingCommand(self, target, vals):
    ...
    if test_type == 'script':
      ...
      cmdline += [
          'src/ios/build/bots/scripts/run.py --app',
          'src/out/Release-iphonesimulator/%s.app' % target, '--host-app',
          'NO_PATH', '--out-dir', '${ISOLATED_OUTDIR}', '--xcode-build-version',
          '12d4e'
          '--mac-toolchain-cmd', './mac_toolchain', '--xcode-path', 'Xcode.app',
          '--wpr-tools-path', 'NO_PATH', '--replay-path', 'NO_PATH',
          #'--args-json',
