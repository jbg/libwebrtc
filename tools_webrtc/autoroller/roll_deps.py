...
def GenerateCommitMessage(rev_update, current_commit_pos,
  ...
  commit_msg = ['Roll chromium_revision %s (%s)\n' % (rev_interval,
                ...
                'Full diff: %s\n' % (CHROMIUM_COMMIT_TEMPLATE %
                                     rev_interval),
                'Roll chrome third_party %s' % rev_3p_interval,
                'Change log: %s' % (CHROMIUM_3P_LOG_TEMPLATE % rev_3p_interval)]
  tbr_authors = ''
...
def BuildRevUpdate(opts, webrtc_deps):
  ...
  current_cr_3p_rev = webrtc_deps['vars']['chromium_third_party_revision']
