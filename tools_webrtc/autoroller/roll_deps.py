...
def _GenerateAppearingDeps(old, new):
  """
  Gather dependencies only in |new|.

  Yields: path of each appearing dependency.
  """
  old_entries = BuildDepsentryDict(old)
  new_entries = BuildDepsentryDict(new)
  for path in new_entries:
    if path not in DONT_AUTOROLL_THESE and \
       path not in old_entries:
      yield path


def GenerateAddedDeps(webrtc_deps, new_cr_deps):
  """
  ...
  deps sitting in |ANDROID_DEPS_PATH|. Other deps won't be tracked.

  Caveat: doesn't detect a new package in existing dep.

  Yields: path of each added dependency.
  """
  for path in _GenerateAppearingDeps(webrtc_deps, new_cr_deps):
    if path.startswith(ANDROID_DEPS_PATH):
      yield path
    # Otherwise, assumes that's a Chromium-only dependency.


def GenerateRemovedDeps(webrtc_deps, new_cr_deps):
  """
  ...
  Yields: path of each disappearing dependency.
  """
  for path in _GenerateAppearingDeps(new_cr_deps, webrtc_deps):
    if path.startswith(ANDROID_DEPS_PATH):
      yield path
    elif isinstance(webrtc_deps['deps'][path], DepsEntry):
      # Handled in CalculateChangedDeps
      continue
    else:
  ...
  for path, webrtc_deps_entry in webrtc_entries.iteritems():
    ...
    if cr_deps_entry:
      ...
      if isinstance(webrtc_deps_entry, DepsEntry):
        ...
        # Handled in GenerateRemovedDeps
        continue
...
def GenerateCommitMessage(rev_update, current_commit_pos, new_commit_pos,
                          ...
                          added_deps_paths=None, removed_deps_paths=None):
...
def UpdateDepsFile(deps_filename, rev_update, changed_deps, added_paths=None,
                   removed_paths=None, new_cr_content=None):
  ...
  if added_paths or removed_paths:
    # Workaround, since gclient cannot add or remove deps. Rely on the fact that
    # all concerned deps are located in one place we can # copy/paste.
    assert all(path.startswith(ANDROID_DEPS_PATH)
               for path in added_paths + removed_paths)
    assert new_cr_content
...
def main():
  ...
  added_deps = list(GenerateAddedDeps(webrtc_deps, new_cr_deps))
  removed_deps = list(GenerateRemovedDeps(webrtc_deps, new_cr_deps))
  clang_change = CalculateChangedClang(rev_update.new_chromium_rev)
  commit_msg = GenerateCommitMessage(rev_update,
                                     current_commit_pos, new_commit_pos,
                                     changed_deps, added_deps, removed_deps,
                                     clang_change)
  ...
  if not opts.dry_run:
    UpdateDepsFile(deps_filename, rev_update, changed_deps,
                   added_deps, removed_deps, new_cr_content)
  if _IsTreeClean():
