...
class ChromeRevisionUpdate(object):

  def __init__(self, current_chrome_rev, new_chrome_rev,
      current_third_party_rev,
      new_third_party_rev):
    self.current_chrome_rev = current_chrome_rev
    self.new_chrome_rev = new_chrome_rev
    self.current_third_party_rev = current_third_party_rev
    self.new_third_party_rev = new_third_party_rev
...
def GenerateCommitMessage(rev_update, current_commit_pos,
  ...
  current_cr_rev = rev_update.current_chrome_rev[0:10]
  new_cr_rev = rev_update.new_chrome_rev[0:10]
...
def UpdateDepsFile(deps_filename, rev_update, changed_deps):
  ...
  deps_content = deps_content.replace(rev_update.current_chrome_rev,
                                      rev_update.new_chrome_rev)
...
def GetRollRevisionRanges(opts, webrtc_deps):
  ...
  return ChromeRevisionUpdate(current_cr_rev, new_cr_rev,
                              current_third_party_rev,
                              new_third_party_rev)
...
def main():
  ...
  current_commit_pos = ParseCommitPosition(
      ReadRemoteCrCommit(rev_update.current_chrome_rev))
  new_commit_pos = ParseCommitPosition(
      ReadRemoteCrCommit(rev_update.new_chrome_rev))

  new_cr_deps = ParseRemoteCrDepsFile(rev_update.new_chrome_rev)
  changed_deps = CalculateChangedDeps(webrtc_deps, new_cr_deps)
  clang_change = CalculateChangedClang(rev_update.new_chrome_rev)
