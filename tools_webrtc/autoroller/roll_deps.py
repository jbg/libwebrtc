...
class RevUpdate(object):

  def __init__(self, current_cr_rev, new_cr_rev, current_cr_3p_rev,
      new_cr_3p_rev):
    self.current_cr_rev = current_cr_rev
    self.new_cr_rev = new_cr_rev
    self.current_cr_3p_rev = current_cr_3p_rev
    self.new_cr_3p_rev = new_cr_3p_rev
...
def GenerateCommitMessage(rev_update, current_commit_pos,
  ...
  current_cr_rev = rev_update.current_cr_rev[0:10]
  new_cr_rev = rev_update.new_cr_rev[0:10]
  rev_interval = '%s..%s' % (current_cr_rev, new_cr_rev)
  rev_3p_interval = '%s..%s' % (rev_update.current_cr_3p_rev[0:10],
                                rev_update.new_cr_3p_rev[0:10])
  ...
  commit_msg = ['Roll chromium_revision %s (%s)\n' % (rev_interval,
                ...
                'Full diff: %s\n' % (CHROMIUM_COMMIT_TEMPLATE %
                                     rev_interval),
                '\n',
                'Roll chrome third_party %s\n' % rev_3p_interval,
                'Change log: %s' % (CHROMIUM_3P_LOG_TEMPLATE % rev_interval)]
  tbr_authors = ''
...
def UpdateDepsFile(deps_filename, rev_update, changed_deps):
  ...
  deps_content = deps_content.replace(rev_update.current_cr_rev,
                                      rev_update.new_cr_rev)
  deps_content = deps_content.replace(rev_update.current_cr_3p_rev,
                                      rev_update.new_cr_3p_rev)
...
def _LoadThirdPartyDepsAndFiles(filename):
  ...
  deps_list = []
  deps_list.extend(third_party_deps.get('COMMON_DEPS', []))
  for platform, deps in third_party_deps.get('PLATFORM_DEPS', {}).iteritems():
    deps_list.extend(deps)
  return set(deps_list), third_party_deps.get('FILES', [])


def UpdateThirdPatryDeps(cur_rev, new_rev, dest_dir, source_dir,
...
def BuildRevUpdate(opts, webrtc_deps):
  current_cr_rev = webrtc_deps['vars']['chromium_revision']
  current_cr_3p_rev = webrtc_deps['vars']['chrome_third_party_revision']
  ...
  new_cr_3p_rev = opts.third_party_revision
  if not new_cr_3p_rev:
    stdout, _ = _RunCommand(
        ['git', 'ls-remote', CHROMIUM_THIRD_PARTY_URL, 'HEAD'])
    new_cr_3p_rev = stdout.strip().split('\t')[0]
    logging.info(
        'No third_party revision specified. Using HEAD: %s' % new_cr_3p_rev)

  return RevUpdate(current_cr_rev, new_cr_rev, current_cr_3p_rev,
                   new_cr_3p_rev)
...
def main():
  ...
  p.add_argument('--third-party-revision',
                 help=('Chromium third_party Git revision to roll to. Default '
                       'to the Chromium third_party HEAD revision if ommited.'))
  ...
  if not os.path.exists(cr_3p_repo):
    raise RollError('third_party_chromium not found. Please add custom vars'
  ...
  rev_update = BuildRevUpdate(opts, webrtc_deps)

  current_commit_pos = ParseCommitPosition(
      ReadRemoteCrCommit(rev_update.current_cr_rev))
  new_commit_pos = ParseCommitPosition(
      ReadRemoteCrCommit(rev_update.new_cr_rev))

  new_cr_deps = ParseRemoteCrDepsFile(rev_update.new_cr_rev)
  changed_deps = CalculateChangedDeps(webrtc_deps, new_cr_deps)
  clang_change = CalculateChangedClang(rev_update.new_cr_rev)
  ...
  UpdateThirdPatryDeps(rev_update.current_cr_3p_rev, rev_update.new_cr_3p_rev,
