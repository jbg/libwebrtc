...
def CheckAbseilDependencies(input_api, gn_files, output_api):
  ...
  exceptions = [
    re.compile(r'third_party/abseil-cpp/absl/flags'),
    re.compile(r'third_party/abseil-cpp/absl/debugging')
  ]
  def _AllowedDep(dep):
    for exception_re in exceptions:
      if re.search(exception_re, dep):
        return True
    return False
    ...
    for target_match in TARGET_RE.finditer(gn_file_content):
      ...
      if target_type in target_types_to_check:
        for deps_match in DEPS_RE.finditer(target_contents):
          ...
          for dep in deps:
            if re.search(absl_re, dep) and not _AllowedDep(dep):
              errors.append(
