...
std::vector<const std::string*> GetStatsReferencedIds(const RTCStats& stats) {
  ...
  } else if (type == RTCLegacyMediaStreamStats::kType) {
    const auto& stream = static_cast<const RTCLegacyMediaStreamStats&>(stats);
    AddIdsIfDefined(stream.track_ids, &neighbor_ids);
  } else if (type == RTCLegacyMediaStreamTrackStats::kType) {
    const auto& track =
        static_cast<const RTCLegacyMediaStreamTrackStats&>(stats);
