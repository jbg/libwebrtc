...
bool SrtpTransport::SetRtpParams(int send_cs,
  ...
  RTC_DCHECK(IsActive());
  // If there is a selected network route, udpate the SRTP transport overhead.
  if (HasSelectedNetworkRoute()) {
    // Fire the signal to BaseChannel to update the transport overhead.
    int srtp_overhead = 0;
    GetSrtpOverhead(&srtp_overhead);
    network_route_.srtp_overhead_per_packet = srtp_overhead;
