...
class DtmfSenderTest : public ::testing::Test {
 protected:
  ...
  void GetDtmfInfoFromString(
      ...
      int comma_tone_gap = webrtc::DtmfSender::kDtmfDefaultCommaDelayMs) {
...
TEST_F(DtmfSenderTest, InsertDtmfWithNonDefaultCommaDelay) {
  ...
  int duration = 100;
  int inter_tone_gap = 50;
  int comma_tone_gap = 500;
  EXPECT_TRUE(
      dtmf_->InsertDtmf(tones, duration, inter_tone_gap, comma_tone_gap));
  EXPECT_TRUE_SIMULATED_WAIT(observer_->completed(), kMaxWaitMs, fake_clock_);

  VerifyOnProvider(tones, duration, inter_tone_gap, comma_tone_gap);
  VerifyOnObserver(tones);
}
