...
TEST_F(PeerConnectionAdaptationIntegrationTest, HelloWorld) {
  ...
  caller_->pc()->AddAdaptationResource(fake_resource);
  fake_resource->SetUsageState(ResourceUsageState::kOveruse);

  // Wait until adaptation is applied.
  // TODO(hbos): The successfully applied restrictions/sinkwants do not have an
  // effect in the testing environment. Fix the test!
  EXPECT_TRUE_WAIT(callee_->renderer()->width() < kMaxSenderWidth,
                   kDefaultTimeoutMs);
}
