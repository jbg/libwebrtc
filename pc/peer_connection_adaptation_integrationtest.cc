...
TEST_F(PeerConnectionAdaptationIntegrationTest, HelloWorld) {
  ...
  rtc::Event event;
  // Why does the pc not agree with the test about worker thread?
  static_cast<PeerConnection*>(caller_->pc())
      ->worker_thread()
      ->PostTask(RTC_FROM_HERE, [this, fake_resource, &event] {
        printf("AddResource... begin\n");
        static_cast<PeerConnection*>(caller_->pc())
            ->call()
            ->AddResource(fake_resource);
        printf("AddResource... done\n");
        event.Set();
      });
