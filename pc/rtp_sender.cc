...
RtpSenderBase::RtpSenderBase(
    ...
    const std::string& id,
    std::unique_ptr<SetStreamIDsCallback> set_stream_ids_callback)
    : worker_thread_(worker_thread),
      id_(id),
      set_stream_ids_callback_(std::move(set_stream_ids_callback)) {
...
void RtpSenderBase::SetStreamIDs(const std::vector<std::string>& stream_ids) {
  ...
  if (set_stream_ids_callback_)
    set_stream_ids_callback_->OnSetStreamIDs();
}
...
rtc::scoped_refptr<AudioRtpSender> AudioRtpSender::Create(
  ...
  return rtc::scoped_refptr<AudioRtpSender>(
      new rtc::RefCountedObject<AudioRtpSender>(
          worker_thread, id, stats, std::move(set_stream_ids_callback)));
}
...
AudioRtpSender::AudioRtpSender(
    ...
    StatsCollector* stats,
    std::unique_ptr<SetStreamIDsCallback> set_stream_ids_callback)
    : RtpSenderBase(worker_thread, id, std::move(set_stream_ids_callback)),
      stats_(stats),
...
rtc::scoped_refptr<VideoRtpSender> VideoRtpSender::Create(
  ...
  return rtc::scoped_refptr<VideoRtpSender>(
      new rtc::RefCountedObject<VideoRtpSender>(
          worker_thread, id, std::move(set_stream_ids_callback)));
}
