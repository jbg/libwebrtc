...
RtpSenderInternal::RtpSenderInternal(rtc::Thread* worker_thread,
                                     const std::string& id)
...
void RtpSenderInternal::SetFrameEncryptor(
...
void RtpSenderInternal::SetMediaChannel(cricket::MediaChannel* media_channel) {
  RTC_DCHECK(media_channel == nullptr ||
             media_channel->media_type() == this->media_type());
  media_channel_ = media_channel;
}

RtpParameters RtpSenderInternal::GetParameters() const {
  if (stopped_) {
...
RTCError RtpSenderInternal::SetParameters(const RtpParameters& parameters) {
  TRACE_EVENT0("webrtc", "RtpSenderInternal::SetParameters");
  if (stopped_) {
...
bool RtpSenderInternal::SetTrack(MediaStreamTrackInterface* track) {
  TRACE_EVENT0("webrtc", "RtpSenderInternal::SetTrack");
  if (stopped_) {
    RTC_LOG(LS_ERROR) << "SetTrack can't be called on a stopped RtpSender.";
    return false;
  }
  if (track && track->kind() != this->track_kind()) {
    RTC_LOG(LS_ERROR) << "SetTrack with " << track->kind()
                      << " called on RtpSender with " << this->track_kind()
                      << " track.";
    return false;
  }
...
void RtpSenderInternal::SetSsrc(uint32_t ssrc) {
  TRACE_EVENT0("webrtc", "RtpSenderInternal::SetSsrc");
  ...
  if (frame_encryptor_) {
    this->SetFrameEncryptor(frame_encryptor_);
  }
}

void RtpSenderInternal::Stop() {
  TRACE_EVENT0("webrtc", "RtpSenderInternal::Stop");
...
RTCError RtpSenderInternal::DisableEncodingLayers(
  ...
  RtpParameters parameters = this->GetParameters();
  ...
  RTCError result = this->SetParameters(parameters);
  if (result.ok()) {
...
AudioRtpSender::AudioRtpSender(rtc::Thread* worker_thread,
                               const std::string& id,
                               StatsCollector* stats)
    : RtpSenderInternal(worker_thread, id),
      stats_(stats),
