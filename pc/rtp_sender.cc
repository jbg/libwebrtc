...
RTCError RtpSenderBase::CheckSetParameters(
    const RtpParameters& parameters) {
  RTC_DCHECK_RUN_ON(signaling_thread_);
...
RTCError RtpSenderBase::SetParameters(const RtpParameters& parameters) {
  ...
  // Some tests rely on working in single thread mode without a run loop and a blocking call is required to keep them working.
  // The encoder configuration also involves another thread with an asynchronous task, thus we still do need to wait for the callback to be resolved this way.
