...
class JsepTransportCollection {
 public:
  ...
  // Returns all live transports, assuming those not currently mapped to any
  // MID because they're being kept alive in case of rollback.
 ...
 private:
  ...
  // Transports that would otherwise have been deleted, but are being kept
  // alive in case they are needed for rollback.
  std::map<std::string, std::unique_ptr<cricket::JsepTransport>>
      rollback_jsep_transports_by_name_ RTC_GUARDED_BY(sequence_checker_);
