...
bool BaseChannel::SendPacket(bool rtcp,
                             rtc::CopyOnWriteBuffer* packet,
                             const rtc::PacketOptions& options) {
  // Until all the code is migrated to use PacketType instead of bool.
  PacketType packet_type = rtcp ? PacketType::kRtcp : PacketType::kRtp;
  ...
  if (!IsValidPacketSize(packet_type, packet->size())) {
    RTC_LOG(LS_ERROR) << "Dropping outgoing " << content_name_ << " "
                      << StringLiteral(packet_type)
...
void BaseChannel::OnPacketReceived(bool rtcp,
  ...
  if (!srtp_active() && srtp_required_) {
    ...
    RTC_LOG(LS_WARNING)
        ...
        << StringLiteral(rtcp ? PacketType::kRtcp : PacketType::kRtp)
