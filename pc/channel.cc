...
void BaseChannel::SetTransports_n(
  ...
  if (rtp_dtls_transport) {
    ...
    debug_name = rtp_packet_transport->transport_id();
  }
...
void BaseChannel::OnNetworkRouteChanged(
  ...
  invoker_.AsyncInvoke<void>(RTC_FROM_HERE, worker_thread_, [=] {
    media_channel_->OnNetworkRouteChanged(transport_id, new_route);
  });
}
      ...
      if (ret && rtcp_mux_filter_.IsActive()) {
        ...
        std::string debug_name =
            transport_name_.empty()
                ? rtp_transport_->rtp_packet_transport()->transport_id()
                : transport_name_;
