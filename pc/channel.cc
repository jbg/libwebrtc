...
void BaseChannel::UpdateTransport(
    bool update_demuxer,
    absl::optional<RtpHeaderExtensions> extensions) {
  if (!update_demuxer && !extensions)
    return;

  // TODO(tommi): See if we can do this asynchronously.
  ...
  if (webrtc::RtpTransceiverDirectionHasRecv(content->direction())) {
    ...
    /*
        ...
        if (criteria_modified && !RegisterRtpDemuxerSink_w()) {
          error_desc = StringFormat("Failed to set up audio demuxing for
       mid='%s'.", content_name().c_str()); return false;
        }
