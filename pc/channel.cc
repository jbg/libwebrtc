...
bool BaseChannel::SetRtpTransport(webrtc::RtpTransportInternal* rtp_transport) {
  ...
  if (rtp_transport_) {
    ...
    RTC_DCHECK(!media_channel()->HasNetworkInterface());
    media_channel()->SetInterface(this);

    media_channel()->OnReadyToSend(rtp_transport_->IsReadyToSend());
...
void BaseChannel::OnNetworkRouteChanged(
  ...
  media_channel()->OnNetworkRouteChanged(transport_name(), new_route);
}
...
void BaseChannel::OnTransportReadyToSend(bool ready) {
  RTC_DCHECK_RUN_ON(network_thread());
  RTC_DCHECK(network_initialized());
  media_channel()->OnReadyToSend(ready);
}
...
void BaseChannel::OnRtpPacket(const webrtc::RtpPacketReceived& parsed_packet) {
  ...
  media_channel()->OnPacketReceived(
