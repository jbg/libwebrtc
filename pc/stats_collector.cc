...
void StatsCollector::UpdateStats(
  ...
  // TODO(bugs.webrtc.org/11687): If we're in the middle of an async update:
  ...
  if (stats_gathering_started_ != 0 &&
    ...
    // TODO(bugs.webrtc.org/11687): All of these hop over to the worker thread
    // to fetch information.  We could use an AsyncInvoker to run all of these
    // and post the information back to the signaling thread where we can create
    // and update stats reports.  That would also clean up the threading story a
    // bit since we'd be creating/updating the stats report objects consistently
    // on the same thread (this class has no locks right now).
    ExtractSessionInfo();
    ExtractBweInfo();
    ExtractMediaInfo();
    ExtractSenderInfo();
  ...
  // TODO(bugs.webrtc.org/11687): If the caller provided an on_done callback:
...
void StatsCollector::ExtractSessionInfo() {
  ...
  // TODO(bugs.webrtc.org/11687): GetPooledCandidateStats hops to the network
  // thread.
  ...
  // TODO(bugs.webrtc.org/11687): GetTransportStatsByNames hops to the network
  // thread.
  ...
  for (const auto& entry : transport_stats_by_name) {
    ...
    // TODO(bugs.webrtc.org/11687): GetLocalCertificate will hop over to the
    // network thread.
...
void StatsCollector::ExtractBweInfo() {
  ...
  // TODO(bugs.webrtc.org/11687): GetCallStats hops to the worker thread.
  ...
  // TODO(bugs.webrtc.org/11687): GetTransceiversInternal() implicitly expects
  // the signaling thread (which we're on right now).
