...
void StatsCollector::UpdateStats(
  ...
  SessionStats session_stats = ExtractSessionInfo();
  ...
  ExtractBweInfo();
  ExtractMediaInfo(session_stats.transport_names_by_mid);
  ExtractSenderInfo();
  ExtractDataInfo();
  UpdateTrackReports();
}
...
StatsCollector::SessionStats StatsCollector::ExtractSessionInfo() {
  RTC_DCHECK_RUN_ON(pc_->signaling_thread());
...
StatsCollector::SessionStats StatsCollector::ExtractSessionInfo_n() {
  ...
  for (auto& entry : transport_stats_by_name) {
    stats.transport_stats.emplace_back(std::move(entry.first),
...
void StatsCollector::ExtractMediaInfo(
...

  {
    ...
    for (const auto& transceiver : pc_->GetTransceiversInternal()) {
      ...
      auto found = transport_names_by_mid.find(gatherer->mid);
      RTC_DCHECK(found != transport_names_by_mid.end());
      gatherer->transport_name = found->second;
