...
TEST_F(RtpSenderReceiverTest, AudioSenderCantSetUnimplementedRtpParameters) {
  ...
  RtpParameters params = audio_rtp_sender_->GetParameters();
  EXPECT_EQ(1u, params.encodings.size());
  ASSERT_TRUE(audio_rtp_sender_->SetParameters(params).ok());

  // Unimplemented RtpParameters: transaction_id, mid, header_extensions,
  // degredation_preference.
  params.transaction_id = "dummy_transaction_id";
  EXPECT_FALSE(audio_rtp_sender_->SetParameters(params).ok());
  params.transaction_id.clear();
  ASSERT_TRUE(audio_rtp_sender_->SetParameters(params).ok());
  ...
  EXPECT_FALSE(audio_rtp_sender_->SetParameters(params).ok());
  params.header_extensions.clear();
  ASSERT_TRUE(audio_rtp_sender_->SetParameters(params).ok());

  EXPECT_EQ(DegradationPreference::BALANCED, params.degradation_preference);
...
TEST_F(RtpSenderReceiverTest,
  ...
  EXPECT_FALSE(audio_rtp_sender_->SetParameters(params).ok());
  params.encodings[0].codec_payload_type = rtc::nullopt;
  ASSERT_TRUE(audio_rtp_sender_->SetParameters(params).ok());

  params.encodings[0].fec = RtpFecParameters();
  EXPECT_FALSE(audio_rtp_sender_->SetParameters(params).ok());
  params.encodings[0].fec = rtc::nullopt;
  ASSERT_TRUE(audio_rtp_sender_->SetParameters(params).ok());

  params.encodings[0].rtx = RtpRtxParameters();
  EXPECT_FALSE(audio_rtp_sender_->SetParameters(params).ok());
  params.encodings[0].rtx = rtc::nullopt;
  ASSERT_TRUE(audio_rtp_sender_->SetParameters(params).ok());

  params.encodings[0].dtx = DtxStatus::ENABLED;
  EXPECT_FALSE(audio_rtp_sender_->SetParameters(params).ok());
  params.encodings[0].dtx = rtc::nullopt;
  ASSERT_TRUE(audio_rtp_sender_->SetParameters(params).ok());

  params.encodings[0].ptime = 1;
  EXPECT_FALSE(audio_rtp_sender_->SetParameters(params).ok());
  params.encodings[0].ptime = rtc::nullopt;
  ASSERT_TRUE(audio_rtp_sender_->SetParameters(params).ok());

  params.encodings[0].max_framerate = 1;
  EXPECT_FALSE(audio_rtp_sender_->SetParameters(params).ok());
  params.encodings[0].max_framerate = rtc::nullopt;
  ASSERT_TRUE(audio_rtp_sender_->SetParameters(params).ok());

  params.encodings[0].scale_resolution_down_by = 2.0;
  EXPECT_FALSE(audio_rtp_sender_->SetParameters(params).ok());
  params.encodings[0].scale_resolution_down_by = rtc::nullopt;
  ASSERT_TRUE(audio_rtp_sender_->SetParameters(params).ok());

  params.encodings[0].rid = "dummy_rid";
  EXPECT_FALSE(audio_rtp_sender_->SetParameters(params).ok());
  params.encodings[0].rid.clear();
  ASSERT_TRUE(audio_rtp_sender_->SetParameters(params).ok());

  params.encodings[0].dependency_rids.push_back("dummy_rid");
  EXPECT_FALSE(audio_rtp_sender_->SetParameters(params).ok());
  params.encodings[0].dependency_rids.clear();
...
TEST_F(RtpSenderReceiverTest, VideoSenderCantSetUnimplementedRtpParameters) {
  ...
  // degredation_preference.
  params.transaction_id = "dummy_transaction_id";
  EXPECT_FALSE(video_rtp_sender_->SetParameters(params).ok());
  params.transaction_id.clear();
  ASSERT_TRUE(video_rtp_sender_->SetParameters(params).ok());
...
TEST_F(RtpSenderReceiverTest, VideoSenderCantSetPerSenderEncodingParameters) {
  ...
  EXPECT_FALSE(video_rtp_sender_->SetParameters(params).ok());
  params.encodings[1].bitrate_priority = kDefaultBitratePriority;
  ASSERT_TRUE(video_rtp_sender_->SetParameters(params).ok());
