...
rtc::scoped_refptr<PeerConnectionFactory> PeerConnectionFactory::Create(
  ...
  auto context = ConnectionContext::Create(
      EnvironmentBuilder()
          ...
          .With(std::move(std::move(dependencies.trials)))
          .Build(),
      &dependencies);
  if (!context) {
    return nullptr;
...
PeerConnectionFactory::CreatePeerConnectionOrError(
  ...
  EnvironmentBuilder builder(context_->env());
  builder.With(std::move(dependencies.trials));

  if (event_log_factory_) {
    worker_thread()->BlockingCall([&] {
      RTC_DCHECK_RUN_ON(worker_thread());
      Environment env_for_rtc_event_log = builder.Build();
      builder.With(event_log_factory_->Create(env_for_rtc_event_log));
    });
  }
