...
TEST_F(PeerConnectionEncodingsIntegrationTest,
       PreferToReceiveVp8DoesNotPreventVp9EncodingParameters) {
  ...
  // Apply the local description, restricting the receive codec to VP8.
  std::unique_ptr<SessionDescriptionInterface> offer =
      CreateOffer(local_pc_wrapper);
  rtc::scoped_refptr<MockSetSessionDescriptionObserver> p1 =
      SetLocalDescription(local_pc_wrapper, offer.get());
  EXPECT_TRUE(Await({p1}));

  // Because the send codecs are still unknown, setting "L3T3_KEY" should work.
  // This allows `remote_pc_wrapper` to prefer to receive VP9 if it wanted to.
