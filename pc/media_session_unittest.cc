...
TEST_F(MediaSessionDescriptionFactoryTest,
       AllowsChangingPacketizationByMunging) {
  MediaSessionOptions opts;
  AddMediaDescriptionOptions(MEDIA_TYPE_VIDEO, "video1",
                             RtpTransceiverDirection::kSendRecv, kActive,
                             &opts);

  std::vector codecs = {CreateVideoCodec(96, "VP8"),
                        CreateVideoCodec(120, "VP9")};
  f1_.set_video_codecs(codecs, codecs);
  f2_.set_video_codecs(codecs, codecs);

  std::unique_ptr<SessionDescription> offer =
      f1_.CreateOfferOrError(opts, nullptr).MoveValue();
  ASSERT_THAT(offer->contents(), SizeIs(1));
  ASSERT_THAT(offer->contents()[0].media_description()->codecs(), SizeIs(2));

  // Change the packetization for video codes, which simulates munging.
  Codec codec1 = offer->contents()[0].media_description()->codecs()[0];
  ASSERT_EQ(codec1.packetization, std::nullopt);
  codec1.packetization = "raw";
  offer->contents()[0].media_description()->AddOrReplaceCodec(codec1);
  Codec codec2 = offer->contents()[0].media_description()->codecs()[1];
  ASSERT_EQ(codec2.packetization, std::nullopt);
  codec2.packetization = "raw";
  offer->contents()[0].media_description()->AddOrReplaceCodec(codec2);

  std::unique_ptr<SessionDescription> answer =
      f2_.CreateAnswerOrError(offer.get(), opts, nullptr).MoveValue();
  ASSERT_THAT(answer, NotNull());
  ASSERT_THAT(answer->contents(), SizeIs(1));
  // Expect no codec is negotiated away.
  ASSERT_THAT(answer->contents()[0].media_description()->codecs(), SizeIs(2));
  EXPECT_THAT(
      (std::set{answer->contents()[0].media_description()->codecs()[0].id,
                answer->contents()[0].media_description()->codecs()[1].id}),
      UnorderedElementsAre(96, 120));
}
