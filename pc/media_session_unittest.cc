...
TEST_F(MediaSessionDescriptionFactoryTest,
  ...
  auto codec1 = offer->contents()[0].media_description()->codecs()[0];
  EXPECT_THAT(codec1.packetization, Eq(std::nullopt));
  codec1.packetization = "raw";
  offer->contents()[0].media_description()->AddOrReplaceCodec(codec1);
  auto codec2 = offer->contents()[0].media_description()->codecs()[1];
  EXPECT_THAT(codec2.packetization, Eq(std::nullopt));
