...
bool DtlsSrtpTransport::IsDtlsActive() {
  auto rtcp_transport = rtcp_mux_enabled() ? nullptr : rtcp_dtls_transport_;

  return (rtp_dtls_transport_ && rtp_dtls_transport_->IsDtlsActive() &&
          (!rtcp_transport || rtcp_transport->IsDtlsActive()));
}
...
bool DtlsSrtpTransport::IsDtlsConnected() {
  ...
  return (rtp_dtls_transport_ &&
          ...
          (!rtcp_transport ||
           rtcp_transport->dtls_state() == cricket::DTLS_TRANSPORT_CONNECTED));
}
...
bool DtlsSrtpTransport::SetRtpParams(
    int send_cs,
    ...
    int recv_key_len,
    const std::vector<int>& recv_extension_ids) {
  return srtp_transport_->SetRtpParams(send_cs, send_key, send_key_len,
                                       send_extension_ids, recv_cs, recv_key,
                                       recv_key_len, recv_extension_ids);
}

bool DtlsSrtpTransport::SetRtcpParams(
    int send_cs,
    const uint8_t* send_key,
    int send_key_len,
    const std::vector<int>& send_extension_ids,
    ...
    int recv_key_len,
    const std::vector<int>& recv_extension_ids) {
  // RTCP doesn't need to call SetRtcpParam again when the transport is already
  // active. It is only used to update the header extension IDs for RTP.
  if (IsActive()) {
    return true;
  }

  return srtp_transport_->SetRtcpParams(send_cs, send_key, send_key_len,
                                        send_extension_ids, recv_cs, recv_key,
                                        recv_key_len, recv_extension_ids);
}
