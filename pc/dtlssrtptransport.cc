...
void DtlsSrtpTransport::SetDtlsTransports(
  ...
  LOG(LS_INFO) << "Setting RTP Transport on "
               ...
               << rtp_dtls_transport;
  SetDtlsTransport(/*rtcp=*/false, rtp_dtls_transport);
...
bool DtlsSrtpTransport::CanSetupDtlsSrtp() {
  bool can_setup_rtp =
      rtp_dtls_transport_ && rtp_dtls_transport_->IsDtlsActive();
  bool can_setup_rtcp =
      rtcp_mux_enabled()
          ? can_setup_rtp
          : rtcp_dtls_transport_ && rtcp_dtls_transport_->IsDtlsActive();
  return can_setup_rtp && can_setup_rtcp;
}
...
bool DtlsSrtpTransport::DtlsHandshakeCompleted() {
  ...
  bool rtp_handshake_completed =
      rtp_dtls_transport_->dtls_state() == cricket::DTLS_TRANSPORT_CONNECTED;
  bool rtcp_handshake_completed =
      rtcp_mux_enabled()
          ? rtp_handshake_completed
          : !rtcp_dtls_transport_ || rtcp_dtls_transport_->dtls_state() ==
                                         cricket::DTLS_TRANSPORT_CONNECTED;
  return rtp_handshake_completed && rtcp_handshake_completed;
}
...
void DtlsSrtpTransport::UpdateWritableState() {
  ...
  if (rtp_packet_transport && rtp_packet_transport->writable() &&
    ...
    // Only fire the signal when the writable state changes.
    if (writable_) {
      writable_ = false;
      SignalWritableState(writable_);
    }
