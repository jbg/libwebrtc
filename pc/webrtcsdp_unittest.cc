...
TEST_F(WebRtcSdpTest, DeserializeSessionDescriptionSpecialMsid) {
  // Create both msid lines for Plan B & Unified Plan support.
...
// Unified Plan and Plan B style SDPs. It tests the case for no stream ids and
// multiple stream ids. This SDP should include both a=msid lines and a=ssrc
// lines that signal the media stream id.
...
// Tests that a Unified Plan style SDP (does not contain a=ssrc lines that
// signal stream IDs) is deserialized appropriately. It tests the case for no
...
// Tests that a Unified Plan style SDP (does not contain a=ssrc lines that
// signal stream IDs) is serialized appropriately. It tests the case for no
// stream ids and multiple stream ids.
