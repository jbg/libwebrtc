...
bool AudioRtpReceiver::SetOutputVolume(double volume) {
  ...
  return worker_thread_->Invoke<bool>(RTC_FROM_HERE, [&] {
    return media_channel_->SetDefaultOutputVolume(volume);
  });
}
...
void AudioRtpReceiver::SetupMediaChannel(absl::optional<uint32_t> ssrc) {
  ...
  if (ssrc) {
    ...
    source_->Start(media_channel_);
    // TODO(dinosaurav, crbug/webrtc/8694): Stop using 0 to mean unsignalled
    // SSRC value.
