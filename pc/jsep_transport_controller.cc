...
JsepTransportController::MaybeCreateDatagramTransport(
  ...
  if (transport_description->opaque_parameters->transport_name !=
      config_.media_transport_factory->GetTransportName()) {
    RTC_LOG(LS_INFO) << "Opaque transport parameters for transport_name="
                     << transport_description->opaque_parameters->transport_name
                     << ", which does not match supported transport_name="
                     << config_.media_transport_factory->GetTransportName();
    return nullptr;
  }
...
JsepTransportController::GetTransportParameters(const std::string& mid) {
  ...
  params.transport_name = config_.media_transport_factory->GetTransportName();
  return params;
}
