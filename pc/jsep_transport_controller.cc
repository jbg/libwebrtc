...
MediaTransportConfig JsepTransportController::GetMediaTransportConfig(
  ...
  absl::optional<size_t> max_datagram_size;

  if (datagram_transport != nullptr) {
    max_datagram_size = datagram_transport->GetLargestDatagramSize();
  }

  return MediaTransportConfig(media_transport,
                              /*rtp_max_packet_size=*/max_datagram_size);
}
...
JsepTransportController::MaybeCreateDatagramTransport(
  ...
  if (local) {
    if (offer_datagram_transport_) {
      ...
      RTC_LOG(LS_INFO)
          << "Not returning quic transport. Either SDES wasn't enabled, or "
             "datagram transport didn't return an offer earlier.";
      // Offer wasn't generated. Either because datagram transport didn't want
      // it, r because SDES wasn't enabled.
      return nullptr;
    }
  }
