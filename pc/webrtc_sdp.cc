...
// Determines whether a RTCP feedback parameter is applicable to a codec.
// For example, it makes no sense to add PLI to ULPFEC which has no concept
// of frames or to nack on RTX.
bool IsRtcpFeedbackApplicable(const cricket::Codec* codec,
  ...
  // See bugs.webrtc.org/4543 for the disabled-by-default audio NACK.
  ...
  if (codec->name == cricket::kOpusCodecName &&
      !(feedback_param.id() == cricket::kRtcpFbParamTransportCc ||
        feedback_param.id() == cricket::kRtcpFbParamNack)) {
    return false;
  }
  return true;
}
...
void AddFeedbackParameter(const cricket::FeedbackParam& feedback_param,
                          cricket::Codec* codec) {
  if (IsRtcpFeedbackApplicable(codec, feedback_param)) {
...
void AddFeedbackParameters(const cricket::FeedbackParams& feedback_params,
  ...
  for (const cricket::FeedbackParam& feedback_param :
    ...
    AddFeedbackParameter(feedback_param, codec);
  }
}
