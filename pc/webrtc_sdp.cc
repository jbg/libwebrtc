  ...
  *session_version = fields[2];
  ...
  while (GetLineWithType(message, pos, &line, kLineTypeAttributes)) {
    if (HasAttribute(line, kAttributeGroup)) {
      ...
      std::string media_transport_settings;
      if (!rtc::tokenize_first(value, kSdpDelimiterColonChar,
                               &media_transport_type,
                               &media_transport_settings)) {
        return ParseFailedGetValue(message, kMediaTransportSettingLine, error);
      }
      std::string decoded_value;
      if (!rtc::Base64::Decode(media_transport_settings, rtc::Base64::DO_STRICT,
                               &decoded_value, nullptr)) {
        return ParseFailedGetValue(message, kMediaTransportSettingLine, error);
      }
      for (const auto& setting : desc->MediaTransportSettings()) {
        if (setting.transport_name == media_transport_type) {
          // Reviewer(mellem): the only thing I wonder if we shouldn't rather
          // skip the line, instead of failing the parser if the line is
          // repeated. Maybe in the future we will want to allow multiple "rtp"
          // transports.
          return ParseFailedGetValue(message, kMediaTransportSettingLine,
                                     error);
        }
      }
      desc->AddMediaTransportSetting(media_transport_type, decoded_value);
    }
  }
