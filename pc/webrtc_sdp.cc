...
static const char kAttributeRtcpGoogRemoteEstimate[] = "remote-net-estimate";
...
void BuildRtpContentAttributes(const MediaContentDescription* media_desc,
  ...
  if (media_desc->remote_estimate()) {
    InitAttrLine(kAttributeRtcpGoogRemoteEstimate, &os);
    AddLine(os.str(), message);
  }
  ...
  while (!IsLineType(message, kLineTypeMedia, *pos)) {
    ...
    if (HasAttribute(line, kAttributeMid)) {
      ...
      *bundle_only = true;
      ...
      } else if (HasAttribute(line, kAttributeRtcpGoogRemoteEstimate)) {
