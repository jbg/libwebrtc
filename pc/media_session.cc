...
static RTCError CreateMediaContentOffer(
  ...
  if (!AddStreamParams(media_description_options.sender_options,
    ...
    RTCError(RTCErrorType::INTERNAL_ERROR, "Failed to add stream parameters");
  }
...
std::unique_ptr<SessionDescription> MediaSessionDescriptionFactory::CreateOffer(
  ...
  auto result = CreateOfferOrError(session_options, current_description);
...
MediaSessionDescriptionFactory::CreateAnswer(
    ...
    const SessionDescription* current_description) const {
  auto result =
      CreateAnswerOrError(offer, session_options, current_description);
  return result.ok() ? std::move(result.value()) : nullptr;
}
...
RTCError MediaSessionDescriptionFactory::AddAudioContentForAnswer(
  ...
  if (!audio_transport) {
    LOG_AND_RETURN_ERROR(RTCErrorType::INTERNAL_ERROR,
                         "Failed to create transport answer");
  }
