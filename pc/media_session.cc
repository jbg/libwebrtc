...
std::unique_ptr<SessionDescription> MediaSessionDescriptionFactory::CreateOffer(
  ...
  RtpDataCodecs offer_data_codecs;
...
MediaSessionDescriptionFactory::CreateAnswer(
  ...
  RtpDataCodecs answer_data_codecs;
  ...
  for (const ContentInfo* content : current_active_contents) {
    if (IsMediaContentOfType(content, MEDIA_TYPE_AUDIO)) {
      ...
      if (data) {
        ...
        MergeCodecs<RtpDataCodec>(data->codecs(), data_codecs, used_pltypes);
      }
    }
  }
}
...
void MediaSessionDescriptionFactory::GetCodecsForOffer(
    ...
    AudioCodecs* audio_codecs,
    VideoCodecs* video_codecs,
    RtpDataCodecs* data_codecs) const {
...
void MediaSessionDescriptionFactory::GetCodecsForAnswer(
  ...
  RtpDataCodecs filtered_offered_data_codecs;
  for (const ContentInfo& content : remote_offer.contents()) {
    if (IsMediaContentOfType(&content, MEDIA_TYPE_AUDIO)) {
      ...
      if (data) {
        ...
        for (const RtpDataCodec& offered_data_codec : data->codecs()) {
          if (!FindMatchingCodec<RtpDataCodec>(data->codecs(),
                                               filtered_offered_data_codecs,
                                               offered_data_codec, nullptr) &&
              FindMatchingCodec<RtpDataCodec>(data->codecs(), data_codecs_,
                                              offered_data_codec, nullptr)) {
...
bool MediaSessionDescriptionFactory::AddRtpDataContentForOffer(
    ...
    const ContentInfo* current_content,
    const SessionDescription* current_description,
    const RtpDataCodecs& data_codecs,
    StreamParamsVec* current_streams,
    SessionDescription* desc,
    IceCredentialsIterator* ice_credentials) const {
