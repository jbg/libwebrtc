...
bool MediaSessionDescriptionFactory::AddDataContentForOffer(
  ...
  if (is_sctp) {
    return AddSctpDataContentForOffer(media_description_options,
                                      session_options, current_content,
                                      current_description, data_codecs,
                                      current_streams, desc, ice_credentials);
  } else {
...
SctpDataContentDescription* GetFirstSctpDataContentDescription(
    SessionDescription* sdesc) {
  return GetFirstMediaContentDescription(sdesc, MEDIA_TYPE_DATA)->as_sctp();
}
