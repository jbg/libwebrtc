...
void PeerConnection::OnStateChanged(DataChannelState state) {
  RTC_DCHECK_RUN_ON(network_thread());
  if (state != DataChannelState::kWritable) {
    return;
  }
  data_channel_transport_invoker_->AsyncInvoke<void>(
      RTC_FROM_HERE, signaling_thread(), [this] {
        ...
        if (data_channel_transport_negotiated_) {
          SignalDataChannelTransportWritable_s(true);
        }
      });
}
...
bool PeerConnection::CreateDataChannel(const std::string& mid) {
  switch (data_channel_type_) {
    case cricket::DCT_SCTP:
      ...
      // Using datagram implementation of data channel transport.
      // This initializes both SCTP and data channel transport, so fallthrough
      // to set up data channel transport.
      RTC_FALLTHROUGH();
    case cricket::DCT_MEDIA_TRANSPORT:
      if (!use_datagram_transport_for_data_channels_) {
        // Using media transport mode, which means media tranpsort is the only
...
void PeerConnection::OnDataChannelTransportNegotiated(
    cricket::JsepTransport* transport,
    cricket::JsepTransport::DataChannelTransportVariant data_channel) {
  RTC_DCHECK_RUN_ON(network_thread());
  if (data_channel.data_channel_transport != nullptr) {
    RTC_DCHECK(data_channel.dtls_transport == nullptr);
    data_channel_transport_invoker_->AsyncInvoke<void>(
        RTC_FROM_HERE, signaling_thread(), [this] {
          RTC_DCHECK_RUN_ON(signaling_thread());
          data_channel_transport_negotiated_ = true;
  ...
  } else if (data_channel.dtls_transport != nullptr) {
    RTC_DCHECK(data_channel.data_channel_transport == nullptr);
    sctp_invoker_->AsyncInvoke<void>(RTC_FROM_HERE, signaling_thread(), [this] {
      RTC_DCHECK_RUN_ON(signaling_thread());
      sctp_negotiated_ = true;
...
void PeerConnection::OnDataChannelTransportRemoved(
    cricket::JsepTransport* transport,
    cricket::JsepTransport::DataChannelTransportVariant data_channel) {
  RTC_DCHECK_RUN_ON(network_thread());
  if (data_channel.data_channel_transport != nullptr) {
