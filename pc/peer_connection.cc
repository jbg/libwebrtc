...
bool PeerConnection::GetLocalCertificate(
  ...
  // TODO(bugs.webrtc.org/11687): This call will hop over to the network thread
  // if we're not already on the network thread. To avoid this, we should
  // require GetLocalCertificate to be called on the network thread.
...
std::unique_ptr<rtc::SSLCertChain> PeerConnection::GetRemoteSSLCertChain(
  ...
  // TODO(bugs.webrtc.org/11687): This call will hop over to the network thread
  // if we're not already on the network thread. To avoid this, we should
  // require GetRemoteSSLCertChain to be called on the network thread.
