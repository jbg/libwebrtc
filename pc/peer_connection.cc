...
class PeerConnection::ImplicitCreateSessionDescriptionObserver
 ...
 public:
  ...
  void SetOperationCompleteCallback(
      std::function<void()> operation_complete_callback) {
    printf("--- SetOperationCompleteCallback ---\n");
    operation_complete_callback_ = std::move(operation_complete_callback);
  }
  ...
  void OnFailure(RTCError error) override {
    ...
    // // Abort early if |pc_| is no longer valid.
    // if (!pc_) {
    //   printf("--- HBOS --- ImplicitCreateSessionDescription OnFailure\n");
    //   operation_complete_callback_();
    //   return;
    // }
...
class PeerConnection::SetLocalDescriptionObserverAdapter
 ...
 public:
  ...
  void OnSetLocalDescriptionComplete(RTCError error) override {
    if (!pc_) {
      // The peer connection no longer exists. For consistency with resolving
      // |wrapper| in a posted message - which gets cancelled upon destruction
      // - we leave |wrapper| in an unresolved state here.
      return;
    }
    if (error.ok())
      ...
      [this_weak_ptr = weak_ptr_factory_.GetWeakPtr(),
       create_sdp_observer](std::function<void()> operations_chain_callback) {
        ...
        static_cast<ImplicitCreateSessionDescriptionObserver*>(
            create_sdp_observer.get())  // <--- why static cast?
