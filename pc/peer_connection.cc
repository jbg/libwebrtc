...
PeerConnection::CreateDataChannelOrError(const std::string& label,
  ...
  if (sctp_mid()) {
    // Note: Using the `fallback_ssl_role` works fine if we are the offerer, but
    // can be a mistake if we are the answerer and the remote offer is ACTIVE.
    // In that case, we will guess the role wrong.
    // TODO(bugs.webrtc.org/13668): Check if this actually happens.
    internal_config.fallback_ssl_role =
