...
RTCError PeerConnection::UpdateTransceiverChannel(
  ...
  // TODO(tommi): This function always returns RTCError::OK(). Some of the
  // below methods, specifically Create & Destroy, need to be called on the
  // worker thread. Consider if there should be a split here where we do things
  // asynchronously in two steps and change the return type of the function to
  // be void. Note that in the case of 'create', that would/could mean that
  // SetChannel might get called at a much later stage than it happens now.
  cricket::ChannelInterface* channel = transceiver->internal()->channel();
  if (content.rejected) {
    if (channel) {
...
cricket::VoiceChannel* PeerConnection::CreateVoiceChannel(
  ...
  // TODO(tommi): CreateVoiceChannel internally switches to the worker thread.
  // We shouldn't be using the |call_ptr_| hack here but simply be on the worker
  // thread and use |call_| (update upstream code).
...
cricket::VideoChannel* PeerConnection::CreateVideoChannel(
  ...
  // TODO(tommi): CreateVideoChannel internally switches to the worker thread.
  // We shouldn't be using the |call_ptr_| hack here but simply be on the worker
  // thread and use |call_| (update upstream code).
...
void PeerConnection::DestroyChannelInterface(
  ...
  // TODO(tommi): All the below methods should be called on the worker thread.
  // (they switch internally anyway). Change DestroyChannelInterface to either
  // be called on the worker thread, or do this asynchronously on the worker.
  RTC_DCHECK(channel);
...
PeerConnection::InitializeRtcpCallback() {
  ...
  return [this, flag = std::move(flag)](const rtc::CopyOnWriteBuffer& packet,
                                        int64_t packet_time_us) {
    RTC_DCHECK_RUN_ON(network_thread());
    // TODO(tommi): We should actually be delivering this call directly to
    // the Call class somehow directly on the network thread and not incur
    // this hop here. The DeliverPacket() method will eventually just have to
    // hop back over to the network thread.
