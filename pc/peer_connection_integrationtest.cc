...
TEST_P(PeerConnectionIntegrationTest, NewGetStatsManyAudioAndManyVideoStreams) {
  ...
  for (const auto& stat : outbound_stream_stats) {
    ...
    const auto* track_stat =
        caller_report->GetAs<webrtc::RTCLegacyMediaStreamTrackStats>(
            *stat->track_id);
  ...
  for (const auto& stat : inbound_stream_stats) {
    ...
    ASSERT_TRUE(stat->track_id.is_defined());
    const auto* track_stat =
        callee_report->GetAs<webrtc::RTCLegacyMediaStreamTrackStats>(
            *stat->track_id);
    ASSERT_TRUE(track_stat);
...
TEST_P(PeerConnectionIntegrationTest,
  ...
  auto media_stats =
      report->GetStatsOfType<webrtc::RTCLegacyMediaStreamTrackStats>();
...
// Test that the "RTCLegacyMediaStreamTrackStats"  object is updated correctly
// when SSRCs are unsignaled, and the SSRC of the received (audio) stream
// changes. This should result in two "RTCInboundRTPStreamStats", but only one
// "RTCLegacyMediaStreamTrackStats", whose counters go up continuously rather
// than being reset to 0 once the SSRC change occurs.
//
...
double GetAudioEnergyStat(PeerConnectionIntegrationWrapper* pc) {
  ...
  auto track_stats_list =
      report->GetStatsOfType<webrtc::RTCLegacyMediaStreamTrackStats>();
  const webrtc::RTCLegacyMediaStreamTrackStats* remote_track_stats = nullptr;
