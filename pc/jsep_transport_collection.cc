...
std::vector<cricket::JsepTransport*> JsepTransportCollection::Transports() {
  ...
  for (auto& kv : rollback_jsep_transports_by_name_) {
    result.push_back(kv.second.get());
  }
  return result;
}
...
void JsepTransportCollection::DestroyAllTransports() {
  ...
  rollback_jsep_transports_by_name_.clear();
  RTC_DCHECK(IsConsistent());
}
...
const cricket::JsepTransport* JsepTransportCollection::GetTransportByName(
  ...
  if (it == jsep_transports_by_name_.end()) {
    it = rollback_jsep_transports_by_name_.find(transport_name);
  }
  return (it == rollback_jsep_transports_by_name_.end()) ? nullptr
                                                         : it->second.get();
}
...
bool JsepTransportCollection::RollbackTransports() {
  RTC_DCHECK_RUN_ON(&sequence_checker_);
  bool ret = true;
  // Move transports that were being kept alive specifically because they would
  // be needed for rollback back to the primary map.
  for (auto it = rollback_jsep_transports_by_name_.begin();
       it != rollback_jsep_transports_by_name_.end();) {
    jsep_transports_by_name_[it->first] = std::move(it->second);
    it = rollback_jsep_transports_by_name_.erase(it);
  }
  // Remove any new mid->transport mappings.
  ...
  // Moving a transport from rollback_jsep_transports_by_name_ to
  // jsep_transports_by_name_ means it's once again factored into the
  // connection state, so it may need to be updated.
...
void JsepTransportCollection::MaybeDestroyJsepTransport(
  ...
  bool needed_for_rollback = TransportNeededForRollback(transport);
  for (auto it = jsep_transports_by_name_.begin();
       it != jsep_transports_by_name_.end(); ++it) {
    if (it->second.get() == transport) {
      if (needed_for_rollback) {
        rollback_jsep_transports_by_name_[it->first] = std::move(it->second);
      }
      jsep_transports_by_name_.erase(it);
      state_change_callback_();
      break;
    }
  }
