...
class RtpSenderInternal : public RtpSenderInterface {
 public:
  ...
  virtual void SetStreamIDs(const std::vector<std::string>& stream_ids) = 0;
...
class RtpSenderBase : public RtpSenderInternal, public ObserverInterface {
 public:
  class SetStreamIDsCallback {
   public:
    virtual ~SetStreamIDsCallback() = default;
 ...
 protected:
  ...
  std::unique_ptr<SetStreamIDsCallback> set_stream_ids_callback_;
};
...
class VideoRtpSender : public RtpSenderBase {
 public:
  ...
  // The sender is initialized with no track to send and no associated streams.
  // |set_stream_ids_callback| is invoked when SetStremIDs is called.
