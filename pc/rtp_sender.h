...
class RtpSenderInternal : public RtpSenderInterface, public ObserverInterface {
 public:
  ...
  virtual void SetMediaChannel(cricket::MediaChannel* media_channel);
  ...
  virtual void set_stream_ids(const std::vector<std::string>& stream_ids) {
  ...
  virtual int AttachmentId() const { return attachment_id_; }

  // Disables the layers identified by the specified RIDs.
  // If the specified list is empty, this is a no-op.
  virtual RTCError DisableEncodingLayers(const std::vector<std::string>& rid);

  // ObserverInterface implementation.
  void OnChanged() override {}

 protected:
  RtpSenderInternal(rtc::Thread* worker_thread, const std::string& id);
  // TODO(nisse): Since SSRC == 0 is technically valid, figure out
  // some other way to test if we have a valid SSRC.
  bool can_send_track() const { return track_ && ssrc_; }

  rtc::Thread* worker_thread_;
  virtual std::string track_kind() const { return ""; }
...
class AudioRtpSender : public DtmfProviderInterface, public RtpSenderInternal {
 public:
...
class VideoRtpSender : public RtpSenderInternal {
 public:
