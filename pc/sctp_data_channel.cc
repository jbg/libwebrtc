...
void SctpDataChannel::OnClosingProcedureComplete() {
  ...
  // to `SetState(kClosed)`. To avoid that (and knowing that kClosed is the last
  // and final state), we free our reference to the controller here.
  controller_.reset();
...
void SctpDataChannel::OnDataReceived(webrtc::DataMessageType type,
  ...
  bool binary = (type == webrtc::DataMessageType::kBinary);
