...
void DataChannelController::OnDataReceived(
  ...
  if (HandleOpenMessage_n(type, channel_id, buffer))
    return;
  ...
  signaling_thread()->PostTask(
      SafeTask(signaling_safety_.flag(), [this, channel_id] {
        ...
        if (it != sctp_data_channels_.end()) {
          ...
          // OnSctpDataChannelClosed() steps have been taken and we don't need
          // to free the channel asynchronously. However, we need to notify
          // `pc_` of the state change.
          pc_->OnSctpDataChannelStateChanged(channel.get(), channel->state());
        }
      }));
}
