...
class FakeVideoMediaChannelForStats : public cricket::FakeVideoMediaChannel {
 public:
  ...
  void SetStats(const cricket::VideoMediaInfo& video_info) {
    switch (role()) {
      case MediaChannel::Role::kSend:
        RTC_LOG(LS_ERROR) << "DEBUG: Setting send stats, this = " << this;
      ...
      case MediaChannel::Role::kReceive:
        RTC_LOG(LS_ERROR) << "DEBUG: SetStats (video receive) called, this="
                          << this;
        ...
        if (receive_stats_->receivers.size() > 0 &&
            receive_stats_->receivers[0]
                .last_packet_received_timestamp_ms.has_value()) {
          RTC_LOG(LS_ERROR)
              << "DEBUG: "
                 "receive_stats.receivers[0].last_packet_received_timestamp_ms="
              << receive_stats_->receivers[0]
                     .last_packet_received_timestamp_ms.value();
        }
        break;
      default:
  ...
  bool GetReceiveStats(cricket::VideoMediaReceiveInfo* info) override {
    RTC_LOG(LS_ERROR) << "DEBUG: Getting Video receive stats, this=" << this;
    RTC_DCHECK(role() == MediaChannel::Role::kReceive);
    if (receive_stats_) {
      if (receive_stats_->receivers.size() > 0) {
        if (receive_stats_->receivers[0]
                .last_packet_received_timestamp_ms.has_value()) {
          RTC_LOG(LS_ERROR)
              << "DEBUG: "
                 "receive_stats.receivers[0].last_packet_received_timestamp_ms="
          ...
          RTC_LOG(LS_ERROR)
              << "DEBUG: last_packet_received_timestamp has no value";
        }
      } else {
        RTC_LOG(LS_ERROR) << "DEBUG: No receivers";
      }
      *info = *receive_stats_;
      return true;
    }
