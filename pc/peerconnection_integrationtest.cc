...
class PeerConnectionWrapper : public webrtc::PeerConnectionObserver,
 ...
 private:
  ...
  // transceivers that are no longer receiving.
  void RemoveUnusedVideoRenderers() {
    auto transceivers = pc()->GetTransceivers();
    for (auto& transceiver : transceivers) {
      if (transceiver->stopped() &&
          transceiver->receiver()->media_type() == cricket::MEDIA_TYPE_VIDEO) {
        auto it =
...
// are expected to be received for a caller/callee and checking to see if those
// frames counts have been increased by their expected values. It provides
// helper functions to specify these expectations. The object initially starts
// in a state of no expectations. When expectations are specified on the
// object it sets its expectations based on the current frame count. For video,
// uses min_video_frames_received_per_track for the case of multiple video
// tracks being received. If it is expected to not receive any audio/video it
// will check that the frame count has not increased from the time the
// expectation is set to the time of checking. For not expecting any video, uses
// total_video_frames_received to see that no video tracks have received.
// Note: you can't reuse expectations - they must be set every time to the
// current frame count.
class MediaExpectations {
 public:
  ...
  MediaExpectations(PeerConnectionWrapper* caller,
                    PeerConnectionWrapper* callee)
      : caller_(caller), callee_(callee) {}

  bool HasExpectedFrames() const {
    return CallerHasExpectedAudioFrames() && CallerHasExpectedVideoFrames() &&
           CalleeHasExpectedAudioFrames() && CalleeHasExpectedVideoFrames();
  }
  ...
  void CallerExpectsAllMedia() {
    CallerExpectsSomeAudio();
    CallerExpectsSomeVideo();
  }

  void CalleeExpectsAllMedia() {
    CalleeExpectsSomeAudio();
    CalleeExpectsSomeVideo();
  }
  ...
  void CallerExpectsSomeAudio(
    ...
    caller_audio_frames_expected_ =
        caller_->audio_frames_received() + expected_audio_frames;
  }

  void CallerExpectsNoAudio() {
    caller_audio_expectation_ = kExpectNoFrames;
    caller_audio_frames_expected_ = caller_->audio_frames_received();
  }

  bool CallerHasExpectedAudioFrames() const {
    if (caller_audio_expectation_ == kExpectSomeFrames) {
      return caller_->audio_frames_received() >= caller_audio_frames_expected_;
    }
    return true;
  }

  bool CallerHasNoUnexpectedAudioFrames() const {
    if (caller_audio_expectation_ == kExpectNoFrames) {
      return caller_->audio_frames_received() == caller_audio_frames_expected_;
    }
    return true;
  }
  ...
  void CallerExpectsSomeVideo(
    ...
    caller_video_frames_expected_ =
        caller_->min_video_frames_received_per_track() + expected_video_frames;
  }

  void CallerExpectsNoVideo() {
    caller_video_expectation_ = kExpectNoFrames;
    caller_video_frames_expected_ = caller_->total_video_frames_received();
  }

  bool CallerHasExpectedVideoFrames() const {
    if (caller_video_expectation_ == kExpectSomeFrames) {
      return caller_->min_video_frames_received_per_track() >=
             caller_video_frames_expected_;
    }
    return true;
  }

  bool CallerHasNoUnexpectedVideoFrames() const {
    if (caller_video_expectation_ == kExpectNoFrames) {
      return caller_->total_video_frames_received() ==
             caller_video_frames_expected_;
    }
    return true;
  }
  ...
  void CalleeExpectsSomeAudio(
    ...
    callee_audio_frames_expected_ =
        callee_->audio_frames_received() + expected_audio_frames;
  }

  void CalleeExpectsNoAudio() {
    callee_audio_expectation_ = kExpectNoFrames;
    callee_audio_frames_expected_ = callee_->audio_frames_received();
  }

  bool CalleeHasExpectedAudioFrames() const {
    if (callee_audio_expectation_ == kExpectSomeFrames) {
      return callee_->audio_frames_received() >= callee_audio_frames_expected_;
    }
    return true;
  }

  bool CalleeHasNoUnexpectedAudioFrames() const {
    if (callee_audio_expectation_ == kExpectNoFrames) {
      return callee_->audio_frames_received() == callee_audio_frames_expected_;
    }
    return true;
  }
  ...
  void CalleeExpectsSomeVideo(
    ...
    callee_video_frames_expected_ =
        callee_->min_video_frames_received_per_track() + expected_video_frames;
  }

  void CalleeExpectsNoVideo() {
    callee_video_expectation_ = kExpectNoFrames;
    callee_video_frames_expected_ = callee_->total_video_frames_received();
  }

  bool CalleeHasExpectedVideoFrames() const {
    if (callee_video_expectation_ == kExpectSomeFrames) {
      return callee_->min_video_frames_received_per_track() >=
             callee_video_frames_expected_;
    }
    return true;
  }

  bool CalleeHasNoUnexpectedVideoFrames() const {
    if (callee_video_expectation_ == kExpectNoFrames) {
      return callee_->total_video_frames_received() ==
             callee_video_frames_expected_;
    }
    return true;
  }
 ...
 private:
  PeerConnectionWrapper* caller_;
  PeerConnectionWrapper* callee_;
  ...
  int caller_audio_frames_expected_;
  int caller_video_frames_expected_;
  int callee_audio_frames_expected_;
  int callee_video_frames_expected_;
};
...
class PeerConnectionIntegrationBaseTest : public testing::Test {
 public:
  ...
  // |media_expectations|, and we will wait for the frame counts to increase
  // to these expected values.
  void ExpectNewFrames(const MediaExpectations& media_expectations) {
    // First wait for the expected frames.
    EXPECT_TRUE_WAIT(media_expectations.HasExpectedFrames(),
                     kMaxWaitForFramesMs);

    // After the combined wait, print out a more detailed message upon
    // failure.
    EXPECT_TRUE(media_expectations.CallerHasExpectedAudioFrames());
    EXPECT_TRUE(media_expectations.CallerHasExpectedVideoFrames());
    EXPECT_TRUE(media_expectations.CalleeHasExpectedAudioFrames());
    EXPECT_TRUE(media_expectations.CalleeHasExpectedVideoFrames());

    // We want to make sure nothing unexpected was received.
    EXPECT_TRUE(media_expectations.CallerHasNoUnexpectedAudioFrames());
    EXPECT_TRUE(media_expectations.CallerHasNoUnexpectedVideoFrames());
    EXPECT_TRUE(media_expectations.CalleeHasNoUnexpectedAudioFrames());
    EXPECT_TRUE(media_expectations.CalleeHasNoUnexpectedVideoFrames());
  }
...
TEST_P(PeerConnectionIntegrationTest, EndToEndCallWithDtls) {
  ...
  caller()->CreateAndSetAndSignalOffer();
  ASSERT_TRUE_WAIT(SignalingStateStable(), kDefaultTimeout);
  MediaExpectations media_expectations(caller(), callee());
...
TEST_P(PeerConnectionIntegrationTest, OneWayMediaCall) {
  ...
  caller()->CreateAndSetAndSignalOffer();
  MediaExpectations media_expectations(caller(), callee());
  media_expectations.CalleeExpectsAllMedia();
  ExpectNewFrames(media_expectations);
}
...
TEST_P(PeerConnectionIntegrationTest, BundlingEnabledWhileIceRestartOccurs) {
  ...
  // Expect additional frames to be received after the ICE restart, by resetting
  // the expectations.
  media_expectations.ExpectBidirectionalAudio();
  media_expectations.ExpectBidirectionalVideo();
  ExpectNewFrames(media_expectations);
...
TEST_P(PeerConnectionIntegrationTest, VideoRejectedInSubsequentOffer) {
  ...
  MediaExpectations audio_expectations(caller(), callee());
  audio_expectations.ExpectBidirectionalAudio();
  audio_expectations.ExpectNoVideo();
  ExpectNewFrames(audio_expectations);
}
