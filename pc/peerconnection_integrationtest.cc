...
struct ExpectedReceivedFrames {
  int caller_audio_frames;
  int caller_video_frames;
  int callee_audio_frames;
  int callee_video_frames;
};
...
class PeerConnectionWrapper : public webrtc::PeerConnectionObserver,
 ...
 private:
  ...
  std::unique_ptr<SessionDescriptionInterface> CreateOffer() {
    ...
    auto offer = WaitForDescriptionFromObserver(observer);
    return offer;
  }
  ...
  std::unique_ptr<SessionDescriptionInterface> CreateAnswer() {
    ...
    auto answer = WaitForDescriptionFromObserver(observer);
    return answer;
  }
...
class PeerConnectionIntegrationBaseTest : public testing::Test {
 public:
  ...
  std::unique_ptr<PeerConnectionWrapper> CreatePeerConnectionWrapper(
      ...
      std::unique_ptr<rtc::RTCCertificateGeneratorInterface> cert_generator,
      bool use_test_sdp_semantics = true) {
    RTCConfiguration modified_config;
    if (config) {
      modified_config = *config;
    }
    if (use_test_sdp_semantics) {
  ...
  bool CreatePeerConnectionWrappersWithConfig(
    ...
    caller_ =
        CreatePeerConnectionWrapper("Caller", nullptr, nullptr, &caller_config,
                                    nullptr, use_test_sdp_semantics);
    callee_ =
  ...
  void ExpectNewFramesReceivedWithWait(
      const ExpectedReceivedFrames& expected_frames,
      int wait_ms,
      bool any_video_receiving = false) {
    // Add current frame counts to the provided values, in order to wait for
    // the frame count to increase.
    int expected_caller_audio_frames_received =
        expected_frames.caller_audio_frames + caller()->audio_frames_received();
    int expected_callee_audio_frames_received =
        expected_frames.callee_audio_frames + callee()->audio_frames_received();
    int total_caller_received_video_frames =
        caller()->total_video_frames_received();
    int total_callee_received_video_frames =
        callee()->total_video_frames_received();

    if (any_video_receiving) {
      // We just want to know if any video is receiving the expected amount of
      // frames.
      int expected_caller_video_frames_received =
          total_caller_received_video_frames +
          expected_frames.caller_video_frames;
      int expected_callee_video_frames_received =
          total_callee_received_video_frames +
          expected_frames.callee_video_frames;
      EXPECT_TRUE_WAIT(caller()->audio_frames_received() >=
                               expected_caller_audio_frames_received &&
                           caller()->total_video_frames_received() >=
                               expected_caller_video_frames_received &&
                           callee()->audio_frames_received() >=
                               expected_callee_audio_frames_received &&
                           callee()->total_video_frames_received() >=
                               expected_callee_video_frames_received,
                       wait_ms);
      // After the combined wait, print out a more detailed message upon
      // failure.
      EXPECT_GE(caller()->total_video_frames_received(),
                expected_caller_video_frames_received);
      EXPECT_GE(callee()->total_video_frames_received(),
                expected_callee_video_frames_received);
    } else {
      // We want to know if all videos are receiving the expected minimum amount
      // of frames.
      int expected_caller_video_frames_received =
          expected_frames.caller_video_frames +
    ...
    if (expected_frames.caller_audio_frames == 0) {
      EXPECT_EQ(caller()->audio_frames_received(),
                expected_caller_audio_frames_received);
    }
    if (expected_frames.caller_video_frames == 0) {
      EXPECT_EQ(caller()->total_video_frames_received(),
                total_caller_received_video_frames);
    }
    if (expected_frames.callee_audio_frames == 0) {
      EXPECT_EQ(callee()->audio_frames_received(),
                expected_callee_audio_frames_received);
    }
    if (expected_frames.callee_video_frames == 0) {
      EXPECT_EQ(callee()->total_video_frames_received(),
                total_callee_received_video_frames);
    }
  }
...
TEST_P(PeerConnectionIntegrationTest, EndToEndCallWithDtls) {
  ...
  caller()->CreateAndSetAndSignalOffer();
  ASSERT_TRUE_WAIT(SignalingStateStable(), kDefaultTimeout);
  ExpectedReceivedFrames expected_frames;
  expected_frames.caller_audio_frames = kDefaultExpectedAudioFrameCount;
  expected_frames.caller_video_frames = kDefaultExpectedVideoFrameCount;
  expected_frames.callee_audio_frames = kDefaultExpectedAudioFrameCount;
  expected_frames.callee_video_frames = kDefaultExpectedVideoFrameCount;
  ExpectNewFramesReceivedWithWait(expected_frames, kMaxWaitForFramesMs);
  EXPECT_LE(
...
TEST_P(PeerConnectionIntegrationTest, OneWayMediaCall) {
  ...
  caller()->CreateAndSetAndSignalOffer();
  ExpectedReceivedFrames expected_frames;
  expected_frames.caller_audio_frames = 0;
  expected_frames.caller_video_frames = 0;
  expected_frames.callee_audio_frames = kDefaultExpectedAudioFrameCount;
  expected_frames.callee_video_frames = kDefaultExpectedVideoFrameCount;
  ExpectNewFramesReceivedWithWait(expected_frames, kMaxWaitForFramesMs);
}
...
TEST_P(PeerConnectionIntegrationTest, AudioToVideoUpgrade) {
  ...
  expected_frames.caller_video_frames = 0;
  expected_frames.callee_audio_frames = kDefaultExpectedAudioFrameCount;
  expected_frames.callee_video_frames = 0;
  ExpectNewFramesReceivedWithWait(expected_frames, kMaxWaitForFramesMs);
  ...
  expected_frames.callee_audio_frames = kDefaultExpectedAudioFrameCount;
  expected_frames.callee_video_frames = kDefaultExpectedVideoFrameCount;
  ExpectNewFramesReceivedWithWait(expected_frames, kMaxWaitForFramesMs, true);
}
...
TEST_P(PeerConnectionIntegrationTest, AnswererRejectsAudioSection) {
  ...
  expected_frames.caller_audio_frames = 0;
  expected_frames.caller_video_frames = kDefaultExpectedVideoFrameCount;
  expected_frames.callee_audio_frames = 0;
  expected_frames.callee_video_frames = kDefaultExpectedVideoFrameCount;
  ExpectNewFramesReceivedWithWait(expected_frames, kMaxWaitForFramesMs);
...
TEST_P(PeerConnectionIntegrationTest,
  ...
  expected_frames.caller_audio_frames = 0;
  expected_frames.caller_video_frames = 0;
  expected_frames.callee_audio_frames = 1;
  expected_frames.callee_video_frames = 0;
  ExpectNewFramesReceivedWithWait(expected_frames, kMaxWaitForFramesMs);
...
TEST_P(PeerConnectionIntegrationTest,
  ...
  expected_frames.caller_video_frames = 0;
  expected_frames.callee_audio_frames = 1;
  expected_frames.callee_video_frames = 1;
  ExpectNewFramesReceivedWithWait(expected_frames, kMaxWaitForFramesMs);
...
TEST_P(PeerConnectionIntegrationTest,
  ...
  expected_frames.caller_audio_frames = 0;
  expected_frames.caller_video_frames = 0;
  expected_frames.callee_audio_frames = 25;
  expected_frames.callee_video_frames = 0;
  ExpectNewFramesReceivedWithWait(expected_frames, kMaxWaitForFramesMs);
