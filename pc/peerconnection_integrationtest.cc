...
class PeerConnectionWrapper : public webrtc::PeerConnectionObserver,
 ...
 private:
  ...
  void RelayIceMessageIfReceiverExists(const std::string& sdp_mid,
                                       int sdp_mline_index,
                                       const std::string& msg) {
    if (signaling_message_receiver_ && signal_ice_candidates_) {
...
TEST_F(PeerConnectionIntegrationTest, SctpDataChannelConfigSentToOtherSide) {
  ...
  EXPECT_EQ(kDataChannelLabel, callee()->data_channel()->label());
...
// With a max bundle policy and RTCP muxing, adding a new media to the
// connection should not affect ICE at all because the new media will use the
// existing connection.
TEST_F(PeerConnectionIntegrationTest,
