 ...
 private:
  ...
  bool SetRemoteDescription(std::unique_ptr<SessionDescriptionInterface> desc) {
    ...
    if (sdp_semantics_ == SdpSemantics::kUnifiedPlan) {
      RemoveUnusedVideoRenderers(desc.get());
    }
    pc()->SetRemoteDescription(observer, desc.release());
    if (sdp_semantics_ == SdpSemantics::kUnifiedPlan) {
      RemoveVideoRendererFromStoppedTransceivers();
    }
    EXPECT_TRUE_WAIT(observer->called(), kDefaultTimeout);
  ...
  // transceivers that are currently receiving but the new remote description
  // sets them to be no longer receiving.
  void RemoveUnusedVideoRenderers(SessionDescriptionInterface* remote_desc) {
    ...
    for (auto& transceiver : transceivers) {
      ...
      const ContentInfo* content =
          remote_desc->description()->GetContentByName(*transceiver->mid());
      if (!content) {
        continue;
      }
      RtpTransceiverDirection new_direction =
          webrtc::RtpTransceiverDirectionReversed(
              content->media_description()->direction());
      if (!webrtc::RtpTransceiverDirectionHasRecv(new_direction) &&
  ...
  // There is a case where we create a transceiver with a receive direction and
  // fake video renderer, then dissociate this transceiver because it is
  // stopped. This is a work around to remove the fake video renderer.
  void RemoveVideoRendererFromStoppedTransceivers() {
    auto transceivers = pc()->GetTransceivers();
    for (auto& transceiver : transceivers) {
      if (transceiver->receiver()->media_type() != cricket::MEDIA_TYPE_VIDEO) {
        continue;
...
class MediaExpectations {
 public:
  ...
  int caller_audio_frames_expected() const {
    return caller_audio_frames_expected_;
  }

  int caller_video_frames_expected() const {
    return caller_video_frames_expected_;
  }

  int callee_audio_frames_expected() const {
    return callee_audio_frames_expected_;
  }

  int callee_video_frames_expected() const {
    return callee_video_frames_expected_;
  }

  ExpectFrames caller_audio_expectation() const {
    return caller_audio_expectation_;
  }

  ExpectFrames caller_video_expectation() const {
    return caller_video_expectation_;
  }

  ExpectFrames callee_audio_expectation() const {
    return callee_audio_expectation_;
  }

  ExpectFrames callee_video_expectation() const {
    return callee_video_expectation_;
  }
