...
void RtpTransport::OnReadPacket(rtc::PacketTransportInternal* transport,
  ...
  cricket::PacketType packet_type = cricket::GetPacketType(data, len);
  // Filter out the packet that is neither RTP nor RTCP.
  if (packet_type == cricket::PacketType::kUnknown) {
    return;
  }

  // Protect ourselves against crazy data.
  if (!cricket::IsValidPacketSize(packet_type, len)) {
    RTC_LOG(LS_ERROR) << "Dropping incoming "
                      << cricket::StringLiteral(packet_type)
                      << " packet: wrong size=" << len;
    return;
  }

  rtc::CopyOnWriteBuffer packet(data, len);
  if (packet_type == cricket::PacketType::kRtcp) {
