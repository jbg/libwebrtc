...
class MockSetStreamIDsCallback
    : public webrtc::RtpSenderBase::SetStreamIDsCallback {
 public:
...
class RtpSenderReceiverTest
 ...
 public:
  ...
  void CreateAudioRtpSender(
    ...
    MockSetStreamIDsCallback* set_stream_ids_callback =
        new MockSetStreamIDsCallback();
    audio_rtp_sender_ =
        AudioRtpSender::Create(worker_thread_, audio_track_->id(), nullptr,
                               absl::WrapUnique(set_stream_ids_callback));
    ASSERT_TRUE(audio_rtp_sender_->SetTrack(audio_track_));
    EXPECT_CALL(*set_stream_ids_callback, OnSetStreamIDs());
