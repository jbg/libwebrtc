 ...
 public:
  ...
  bool HasOutboundRtpExpectedResolutions(
    ...
    for (const RidAndResolution& resolution : resolutions) {
      ...
      RTC_CHECK_LE(*outbound_rtp->frame_width, resolution.width);
      RTC_CHECK_LE(*outbound_rtp->frame_height, resolution.height);
...
TEST_F(PeerConnectionSimulcastWithMediaFlowTests,
  ...
  EXPECT_TRUE_WAIT(HasOutboundRtpBytesSent(local_pc_wrapper, 1u),
                   kLongTimeoutForRampingUp.ms());
  // Legacy SVC fallback uses L3T3_KEY but the API is lying to us.
  rtc::scoped_refptr<const RTCStatsReport> report = GetStats(local_pc_wrapper);
  std::vector<const RTCOutboundRTPStreamStats*> outbound_rtps =
      report->GetStatsOfType<RTCOutboundRTPStreamStats>();
  ...
  EXPECT_TRUE(absl::EqualsIgnoreCase(
      GetCurrentCodecMimeType(report, *outbound_rtps[0]), "video/VP9"));
  // SVC is "L3T3_KEY" but stats is lying to us.
  EXPECT_EQ(*outbound_rtps[0]->scalability_mode, "L1T3");
  // GetParameters() is also lying to us.
  parameters = sender->GetParameters();
  ASSERT_EQ(parameters.encodings.size(), 3u);
  EXPECT_TRUE(parameters.encodings[0].scalability_mode.has_value());
  EXPECT_EQ(parameters.encodings[0].scalability_mode.value(), "L1T3");
  EXPECT_TRUE(parameters.encodings[1].scalability_mode.has_value());
  EXPECT_EQ(parameters.encodings[1].scalability_mode.value(), "L1T3");
  EXPECT_TRUE(parameters.encodings[2].scalability_mode.has_value());
  EXPECT_EQ(parameters.encodings[2].scalability_mode.value(), "L1T3");
}
