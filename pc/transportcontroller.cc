...
RTCError TransportController::SetRtpTransportDescription(
  ...
  if (!network_thread_->IsCurrent()) {
    network_thread_->Invoke<RTCError>(RTC_FROM_HERE, [&] {
      return SetRtpTransportDescription(sdesc, type, source, bundle_enabled,
                                        rtcp_mux_required);
    });
  }
  ...
  if (!ExtractAndSetSrtpType(sdesc)) {
    LOG_AND_RETURN_ERROR(RTCErrorType::INVALID_PARAMETER,
                         "Fail to set the SRTP type.");
  }
  ...
  // Set the content description for each RTP level transport when the bundle is
  // not enabled.
  RTCError error;
...
bool TransportController::ExtractAndSetSrtpType(
    const webrtc::SessionDescriptionInterface* sdesc) {
  if (!sdesc) {
    return false;
  }
  ...
  // Changing from SDES to DTLS-SRTP or vice versa is not supported.
...
void TransportController::MergeEncryptedHeaderExtensionIdsForBundle(
  ...
  // Union the encrypted header IDs when bundle is enabled.
  for (auto content_info : sdesc->description()->contents()) {
