...
ConnectionContext::ConnectionContext(
  ...
  channel_manager_ = std::make_unique<cricket::ChannelManager>(
      ...
      std::make_unique<cricket::RtpDataEngine>(), worker_thread(),
      network_thread());
  // TODO(tommi): Call as part of initialization on worker.
...
ConnectionContext::~ConnectionContext() {
  RTC_DCHECK_RUN_ON(signaling_thread_);
  // TODO(tommi): Consider deleting channel manager on worker.
