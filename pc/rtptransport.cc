...
void RtpTransport::SetRtpPacketTransport(
  ...
  if (rtp_packet_transport_) {
    ...
    SignalNetworkRouteChanged(rtp_packet_transport_->transport_id(),
  ...
  if (new_packet_transport) {
    ...
    SignalNetworkRouteChanged(new_packet_transport->transport_id(),
...
void RtpTransport::SetRtcpPacketTransport(
  ...
  if (rtcp_packet_transport_) {
    ...
    // Reset the network route of the old transport.
    SignalNetworkRouteChanged(rtcp_packet_transport_->transport_id(),
                              rtc::Optional<rtc::NetworkRoute>());
  }
  if (new_packet_transport) {
    new_packet_transport->SignalReadyToSend.connect(
...
void RtpTransport::OnNetworkRouteChange(
  ...
  RTC_DCHECK(transport_id == rtp_packet_transport_->transport_id() ||
             transport_id == rtcp_packet_transport_->transport_id());
  SignalNetworkRouteChanged(transport_id, network_route);
}
