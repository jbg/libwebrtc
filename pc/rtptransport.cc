...
void RtpTransport::SetRtpPacketTransport(
  ...
  if (rtp_packet_transport_) {
    ...
    SignalNetworkRouteChanged(rtp_packet_transport_->transport_name(),
  ...
  if (new_packet_transport) {
    ...
    SignalNetworkRouteChanged(new_packet_transport->transport_name(),
                              new_packet_transport->network_route());
  }
...
void RtpTransport::SetRtcpPacketTransport(
  ...
  if (rtcp_packet_transport_) {
    ...
    // Reset the network route of the old transport.
    SignalNetworkRouteChanged(rtcp_packet_transport_->transport_name(),
                              rtc::Optional<rtc::NetworkRoute>());
  }
  if (new_packet_transport) {
    new_packet_transport->SignalReadyToSend.connect(
...
void RtpTransport::OnNetworkRouteChange(
  ...
  RTC_DCHECK(transport_name == rtp_packet_transport_->transport_name() ||
             transport_name == rtcp_packet_transport_->transport_name());
  SignalNetworkRouteChanged(transport_name, network_route);
}
