...
static bool GetLine(const std::string& message,
  ...
  *line = message.substr(line_begin, (line_end - line_begin));
  ...
  if (line->length() < 3 || !islower(cline[0]) ||
      cline[1] != kSdpDelimiterEqual || cline[2] == kSdpDelimiterSpace) {
    *pos = line_begin;
    return false;
  }
  return true;
...
std::string SdpSerialize(const JsepSessionDescription& jdesc,
  ...
  for (std::set<std::string>::const_iterator it = media_stream_labels.begin();
       it != media_stream_labels.end(); ++it) {
    os << " " << *it;
  }
  AddLine(os.str(), &message);
...
bool AddSctpDataCodec(DataContentDescription* media_desc, int sctp_port) {
  ...
  LOG(INFO) << "AddSctpDataCodec: Got SCTP Port Number " << sctp_port;
