...
class PeerConnectionEndToEndBaseTest : public sigslot::has_slots<>,
 ...
 public:
  ...
  void CreatePcs(
    ...
    EXPECT_TRUE(caller_->CreatePc(config_,
                                  audio_encoder_factory1,
                                  audio_decoder_factory1));
    EXPECT_TRUE(callee_->CreatePc(config_,
  ...
  void GetAndAddUserMedia() {
    ...
    GetAndAddUserMedia(true, true, video_constraints);
  }

  void GetAndAddUserMedia(bool audio,
                          bool video,
                          const FakeConstraints& video_constraints) {
    caller_->GetAndAddUserMedia(audio,
                                video, video_constraints);
    callee_->GetAndAddUserMedia(audio,
                                video, video_constraints);
  }
