...
std::unique_ptr<MediaEngineInterface> CreateFakeMediaEngine() {
  FakeMediaEngine* fme = new cricket::FakeMediaEngine();
  fme->SetAudioCodecs(MAKE_VECTOR(kAudioCodecs));
  fme->SetVideoCodecs(MAKE_VECTOR(kVideoCodecs));
  return std::unique_ptr<MediaEngineInterface>(fme);
}

std::unique_ptr<DataEngineInterface> CreateFakeDataEngine() {
  return std::unique_ptr<DataEngineInterface>(new cricket::FakeDataEngine());
}
}  // namespace
...
TEST_F(ChannelManagerTest, SetVideoRtxEnabled) {
  ...
  // Enable and check.
  cm_ = cricket::ChannelManager::Create(CreateFakeMediaEngine(),
                                        CreateFakeDataEngine(), true, worker_,
                                        network_.get());
  cm_->GetSupportedVideoSendCodecs(&send_codecs);
  ...
  // Disable and check.
  cm_ = cricket::ChannelManager::Create(CreateFakeMediaEngine(),
                                        CreateFakeDataEngine(), false, worker_,
                                        network_.get());
  cm_->GetSupportedVideoSendCodecs(&send_codecs);
  EXPECT_FALSE(ContainsMatchingCodec(send_codecs, rtx_codec));
  cm_->GetSupportedVideoSendCodecs(&recv_codecs);
