...
RTCError SdpOfferAnswerHandler::ApplyLocalDescription(
  ...
  error = pc_->UpdateSessionState(type, cricket::CS_LOCAL,
                                  local_description()->description());
  if (!error.ok()) {
    return error;
  }
  ...
  if (IsSctpLike(pc_->data_channel_type()) && pc_->GetSctpSslRole(&role)) {
    pc_->data_channel_controller_.AllocateSctpSids(role);
  }
  ...
  if (data_content) {
    ...
    if (rtp_data_desc) {
      pc_->data_channel_controller_.UpdateLocalRtpDataChannels(
...
RTCError SdpOfferAnswerHandler::ApplyRemoteDescription(
  ...
  error = pc_->UpdateSessionState(type, cricket::CS_REMOTE,
                                  remote_description()->description());
  if (!error.ok()) {
    return error;
  }
  ...
  if (IsUnifiedPlan()) {
    ...
    for (const auto& transceiver : now_receiving_transceivers) {
      RTC_DCHECK_RUN_ON(pc_->signaling_thread());
      pc_->stats_->AddTrack(transceiver->receiver()->track());
  ...
  if (!IsUnifiedPlan()) {
    ...
    for (size_t i = 0; i < new_streams->count(); ++i) {
      ...
      pc_->stats_->AddStream(new_stream);
...
void SdpOfferAnswerHandler::DoCreateOffer(
  ...
  {
    RTC_DCHECK_RUN_ON(pc_->signaling_thread());
    pc_->webrtc_session_desc_factory_->CreateOffer(observer, options,
                                                   session_options);
  }
}
...
void SdpOfferAnswerHandler::DoCreateAnswer(
  ...
  {
    RTC_DCHECK_RUN_ON(pc_->signaling_thread());
    pc_->webrtc_session_desc_factory_->CreateAnswer(observer, session_options);
  }
}
...
void SdpOfferAnswerHandler::DoSetRemoteDescription(
  ...
  if (IsUnifiedPlan()) {
    RTC_DCHECK_RUN_ON(pc_->signaling_thread());
    if (pc_->configuration_.enable_implicit_rollback) {
...
void SdpOfferAnswerHandler::SetAssociatedRemoteStreams(
  ...
  for (const std::string& stream_id : stream_ids) {
    ...
    rtc::scoped_refptr<MediaStreamInterface> stream =
        pc_->remote_streams_->find(stream_id);
    if (!stream) {
      stream = MediaStreamProxy::Create(rtc::Thread::Current(),
                                        MediaStream::Create(stream_id));
      pc_->remote_streams_->AddStream(stream);
...
void SdpOfferAnswerHandler::ChangeSignalingState(
  ...
  if (signaling_state == PeerConnectionInterface::kClosed) {
    RTC_DCHECK_RUN_ON(pc_->signaling_thread());
    pc_->ice_connection_state_ = PeerConnectionInterface::kIceConnectionClosed;
    pc_->Observer()->OnIceConnectionChange(pc_->ice_connection_state_);
    pc_->standardized_ice_connection_state_ =
        PeerConnectionInterface::IceConnectionState::kIceConnectionClosed;
    pc_->connection_state_ =
        PeerConnectionInterface::PeerConnectionState::kClosed;
    pc_->Observer()->OnConnectionChange(pc_->connection_state_);
  }
...
bool SdpOfferAnswerHandler::CheckIfNegotiationIsNeeded() {
  ...
  if (pc_->data_channel_controller_.HasSctpDataChannels()) {
...
RTCError SdpOfferAnswerHandler::ValidateSessionDescription(
  ...
  {
    RTC_DCHECK_RUN_ON(pc_->signaling_thread());
    if (pc_->webrtc_session_desc_factory_->SdesPolicy() ==
            cricket::SEC_REQUIRED ||
        pc_->dtls_enabled_) {
      RTCError crypto_error =
          VerifyCrypto(sdesc->description(), pc_->dtls_enabled_);
      if (!crypto_error.ok()) {
        return crypto_error;
      }
    }
