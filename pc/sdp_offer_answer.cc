...
bool SdpOfferAnswerHandler::UpdatePayloadTypeDemuxingState(
  ...
  bool has_mid_header_extension_audio = false;
  bool has_mid_header_extension_video = false;
  for (auto& content_info : sdesc->description()->contents()) {
    ...
    switch (content_info.media_description()->type()) {
      case cricket::MediaType::MEDIA_TYPE_AUDIO: {
        if (!has_mid_header_extension_audio) {
          has_mid_header_extension_audio =
      ...
      case cricket::MediaType::MEDIA_TYPE_VIDEO: {
        if (!has_mid_header_extension_video) {
          has_mid_header_extension_video =
              ContentHasHeaderExtension(content_info, RtpExtension::kMidUri);
        }
        const cricket::VideoContentDescription* video_desc =
