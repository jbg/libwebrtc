...
bool SdpOfferAnswerHandler::ReadyToUseRemoteCandidate(
  ...
  *valid = true;
  ...
  std::string transport_name = pc_->GetTransportName(result.value()->name);
...
RTCError SdpOfferAnswerHandler::CreateChannels(const SessionDescription& desc) {
  ...
  if (pc_->data_channel_type() != cricket::DCT_NONE && data &&
      !data->rejected && !pc_->data_channel_controller_.rtp_data_channel() &&
...
bool SdpOfferAnswerHandler::CreateDataChannel(const std::string& mid) {
  ...
  switch (pc_->data_channel_type()) {
    ...
    default:
      ...
      {
        RTC_DCHECK_RUN_ON(pc_->signaling_thread());
        pc_->data_channel_controller_.set_rtp_data_channel(
      ...
      pc_->data_channel_controller_.rtp_data_channel()
          ->SignalSentPacket()
          .connect(pc_, &PeerConnection::OnSentPacket_w);
      pc_->data_channel_controller_.rtp_data_channel()->SetRtpTransport(
          rtp_transport);
...
void SdpOfferAnswerHandler::DestroyDataChannelTransport() {
  RTC_DCHECK_RUN_ON(signaling_thread());
  if (pc_->data_channel_controller_.rtp_data_channel()) {
    pc_->data_channel_controller_.OnTransportChannelClosed();
    DestroyChannelInterface(pc_->data_channel_controller_.rtp_data_channel());
    pc_->data_channel_controller_.set_rtp_data_channel(nullptr);
  }
