...
void PeerConnection::ReportSdpFormatReceived(
  ...
  size_t num_audio_mlines = 0;
  size_t num_video_mlines = 0;
  size_t num_audio_tracks = 0;
  size_t num_video_tracks = 0;
  for (const ContentInfo& content : remote_offer.description()->contents()) {
    cricket::MediaType media_type = content.media_description()->type();
    size_t num_tracks =
        std::max(1ul, content.media_description()->streams().size());
    if (media_type == cricket::MEDIA_TYPE_AUDIO) {
      num_audio_mlines += 1;
  ...
  SdpFormatReceived format = kSdpFormatReceivedSimple;
