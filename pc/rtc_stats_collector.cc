...
void RTCStatsCollector::ProduceCodecStats_n(
  ...
  // (https://crbug.com/webrtc/14414).
  typedef std::pair<int, cricket::CodecParameterMap>
      PayloadTypeAndCodecParametesMap;
  std::map<std::string, std::set<PayloadTypeAndCodecParametesMap>>
      send_codecs_by_transport;
  std::map<std::string, std::set<PayloadTypeAndCodecParametesMap>>
      receive_codecs_by_transport;

  /*typedef std::pair<std::string, int> PayloadTypeAndMid;
  std::map<PayloadTypeAndMid, */
  ...
  for (const auto& stats : transceiver_stats_infos) {
    ...
    std::set<PayloadTypeAndCodecParametesMap>& send_codecs =
        send_codecs_by_transport[transport_id];
    std::set<PayloadTypeAndCodecParametesMap>& receive_codecs =
    ...
    if (stats.track_media_info_map.voice_media_info().has_value()) {
      ...
      for (const auto& pair :
        ...
        if (receive_codecs
                .insert(PayloadTypeAndCodecParametesMap(codec.payload_type,
                                                        codec.parameters))
                .second != true) {
