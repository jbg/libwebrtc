...
std::string RTCRemoteInboundRtpStreamStatsIdFromSsrcs(bool audio,
...
ProduceRemoteInboundRtpStreamStatsFromReportBlockData(
  ...
  auto remote_inbound = absl::make_unique<RTCRemoteInboundRtpStreamStats>(
      ...
      report_block_data.report_block_timestamp_us);
  ...
  std::string local_id =
      RTCOutboundRTPStreamStatsIDFromSSRC(is_audio, report_block.source_ssrc);
  ...
  if (local_id_stat) {
    ...
    const auto* transport_from_id =
        outbound_audio.transport_id.is_defined()
            ? report.Get(*outbound_audio.transport_id)
            : nullptr;
    ...
    const auto* codec_from_id = outbound_audio.codec_id.is_defined()
                                    ? report.Get(*outbound_audio.codec_id)
                                    : nullptr;
    if (codec_from_id) {
  ...
  for (const cricket::VoiceSenderInfo& voice_sender_info :
    ...
    for (const auto& report_block_data : voice_sender_info.report_block_datas) {
      report->AddStats(ProduceRemoteInboundRtpStreamStatsFromReportBlockData(
          report_block_data, true /* is_audio */, *report));
    }
  }
}
...
void RTCStatsCollector::ProduceVideoRTPStreamStats_n(
  ...
  for (const cricket::VideoSenderInfo& video_sender_info :
    ...
    for (const auto& report_block_data : video_sender_info.report_block_datas) {
      report->AddStats(ProduceRemoteInboundRtpStreamStatsFromReportBlockData(
          report_block_data, false /* is_audio */, *report));
    }
  }
}
