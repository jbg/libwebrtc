...
std::string RTCRemoteInboundRtpStreamStatsIdFromSsrcs(
    bool audio, uint32_t sender_ssrc, uint32_t source_ssrc) {
  char buf[1024];
  rtc::SimpleStringBuilder sb(buf);
  sb << "RTCRemoteInboundRtp" << (audio ? "Audio" : "Video") << "Stream_"
     << sender_ssrc << "_" << source_ssrc;
  return sb.str();
}
...
ProduceRemoteInboundRtpStreamStatsFromReportBlockWithRtt(
    const ReportBlockWithRtt& report_block_with_rtt,
    bool is_audio,
    const RTCStatsReport& report) {
  const auto& report_block = report_block_with_rtt.report_block;
  auto remote_inbound =
      std::make_unique<RTCRemoteInboundRtpStreamStats>(
          RTCRemoteInboundRtpStreamStatsIdFromSsrcs(
              is_audio, report_block.sender_ssrc, report_block.source_ssrc),
          report_block_with_rtt.report_received_timestamp_ms);  // is us
  remote_inbound->ssrc = report_block.sender_ssrc;
  remote_inbound->kind = is_audio ? "audio" : "video";
  // TODO: Requires expected sequence number!
  ...
  if (local_id_stat) {
    ...
    const auto* transport_from_id =
        outbound_audio.transport_id.is_defined()
            ? report.Get(*outbound_audio.transport_id) : nullptr;
    if (transport_from_id) {
      const auto& transport =
          transport_from_id->cast_to<RTCTransportStats>();
    ...
    // based on the previous codec, but there is no way to tell which point
    // in time the codec changed for the remote end.
    const auto* codec_from_id =
        outbound_audio.codec_id.is_defined()
            ? report.Get(*outbound_audio.codec_id) : nullptr;
    if (codec_from_id) {
      remote_inbound->codec_id = *outbound_audio.codec_id;
      ...
      if (codec.clock_rate.is_defined()) {
        ...
        // (https://tools.ietf.org/html/rfc3550#section-6.4.1), to convert
        // this to seconds we divide by the codec's clock rate.
...
void RTCStatsCollector::ProduceAudioRTPStreamStats_n(
  ...
  for (const cricket::VoiceSenderInfo& voice_sender_info :
       track_media_info_map.voice_media_info()->senders) {
    for (const auto& report_block_with_rtt : voice_sender_info.report_blocks) {
      report->AddStats(ProduceRemoteInboundRtpStreamStatsFromReportBlockWithRtt(
          report_block_with_rtt, true /* is_audio */, *report));
    }
  }
}
...
void RTCStatsCollector::ProduceVideoRTPStreamStats_n(
  ...
  for (const cricket::VideoSenderInfo& video_sender_info :
       track_media_info_map.video_media_info()->senders) {
    for (const auto& report_block_with_rtt : video_sender_info.report_blocks) {
      report->AddStats(ProduceRemoteInboundRtpStreamStatsFromReportBlockWithRtt(
          report_block_with_rtt, false /* is_audio */, *report));
    }
  }
}
