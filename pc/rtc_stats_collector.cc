...
std::string RTCMediaSourceStatsIDFromKindAndAttachment(bool audio,
                                                       int attachment_id) {
  char buf[1024];
  rtc::SimpleStringBuilder sb(buf);
  sb << "RTC" << (audio ? "Audio" : "Video") << "Source_" << attachment_id;
  return sb.str();
}
...
ProduceMediaStreamTrackStatsFromVoiceSenderInfo(
  ...
  audio_track_stats->media_source_id =
      RTCMediaSourceStatsIDFromKindAndAttachment(/*audio=*/true, attachment_id);
...
ProduceMediaStreamTrackStatsFromVideoSenderInfo(
  ...
  video_track_stats->media_source_id =
      RTCMediaSourceStatsIDFromKindAndAttachment(/*audio=*/false,
                                                 attachment_id);
      ...
      if (track->kind() == MediaStreamTrackInterface::kAudioKind) {
        media_source_stats = absl::make_unique<RTCAudioSourceStats>(
            RTCMediaSourceStatsIDFromKindAndAttachment(
                /*audio=*/true, sender_internal->AttachmentId()),
            timestamp_us);
      } else {
        RTC_DCHECK_EQ(MediaStreamTrackInterface::kVideoKind, track->kind());
        auto video_source_stats = absl::make_unique<RTCVideoSourceStats>(
            RTCMediaSourceStatsIDFromKindAndAttachment(
                /*audio=*/false, sender_internal->AttachmentId()),
            timestamp_us);
  ...
  for (const cricket::VoiceSenderInfo& voice_sender_info :
    ...
    if (audio_track) {
      ...
      outbound_audio->media_source_id =
          RTCMediaSourceStatsIDFromKindAndAttachment(/*audio=*/true,
                                                     attachment_id);
    }
