...
// TODO(steveanton): Remove this duplicate from PeerConnection once the classes
// are merged together.
...
bool WebRtcSession::CreateDataChannel(const cricket::ContentInfo* content,
  ...
  if (sctp) {
    ...
    rtp_data_channel_.reset(pc_->channel_manager()->CreateRtpDataChannel(
...
void WebRtcSession::DestroyDataChannel() {
  ...
  pc_->channel_manager()->DestroyRtpDataChannel(rtp_data_channel_.release());
  transport_controller_->DestroyDtlsTransport(
      transport_name, cricket::ICE_CANDIDATE_COMPONENT_RTP);
  if (need_to_delete_rtcp) {
    transport_controller_->DestroyDtlsTransport(
