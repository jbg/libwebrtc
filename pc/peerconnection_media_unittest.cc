...
// Test that stopping the local transceivers causes the media channels on the
// callee to be destroyed after calling SetRemoteDescription on the generated
// offer.
TEST_P(PeerConnectionMediaTest, StoppedLocalTransceiversRemovesMediaChannels) {
  if (sdp_semantics_ != SdpSemantics::kUnifiedPlan) {
    // See below test for equivalent behavior with Plan B semantics.
    return;
  }
...
TEST_P(PeerConnectionMediaTest, EmptyRemoteOfferRemovesRecvStreams) {
  if (sdp_semantics_ != SdpSemantics::kPlanB) {
    // See above test for equivalent behavior with Unified Plan semantics.
    return;
  }
...
// Test that stopping the remote transceivers causes the media channels to be
// destroyed after calling SetLocalDescription on the local answer.
TEST_P(PeerConnectionMediaTest, StoppedRemoteTransceiversRemovesMediaChannels) {
  if (sdp_semantics_ != SdpSemantics::kUnifiedPlan) {
    // See below test for equivalent behavior with Plan B semantics.
    return;
  }
