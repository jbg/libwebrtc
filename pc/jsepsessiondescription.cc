...
SessionDescriptionInterface* CreateSessionDescription(const std::string& type,
                                                      ...
                                                      SdpParseError* error) {
  SdpType sdp_type;
  if (!SdpTypeFromString(type, &sdp_type)) {
    return nullptr;
  }

  return CreateSessionDescription(sdp_type, sdp, error).release();
}
...
JsepSessionDescription::JsepSessionDescription(const std::string& type) {
  if (!SdpTypeFromString(type, &type_)) {
