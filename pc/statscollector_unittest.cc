...
class FakeAudioProcessor : public webrtc::AudioProcessorInterface {
 ...
 private:
  ...
  AudioProcessorInterface::AudioProcessorStatistics GetStats(
    ...
    stats.typing_noise_detected = true;
    if (has_recv_streams) {
      stats.apm_statistics.echo_return_loss = rtc::Optional<double>(2.0);
      stats.apm_statistics.echo_return_loss_enhancement =
          rtc::Optional<double>(3.0);
      stats.apm_statistics.delay_median_ms = rtc::Optional<int32_t>(4);
      stats.apm_statistics.delay_standard_deviation_ms =
          rtc::Optional<int32_t>(5);
    }
    return stats;
  }
};
