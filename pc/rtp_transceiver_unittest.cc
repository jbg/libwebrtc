...
TEST_F(RtpTransceiverTestForHeaderExtensions,
  ...
  std::vector<RtpHeaderExtensionCapability> extensions = {
      RtpHeaderExtensionCapability(RtpExtension::kDependencyDescriptorUri, 1,
                                   RtpTransceiverDirection::kStopped),
      RtpHeaderExtensionCapability(RtpExtension::kVideoLayersAllocationUri, 2,
                                   RtpTransceiverDirection::kStopped),
  };
  ...
  simulcast_parameters.encodings = {{}, {}};
  auto simulcast_sender = rtc::make_ref_counted<MockRtpSenderInternal>();
  EXPECT_CALL(*simulcast_sender.get(), GetParametersInternal())
  ...
  auto svc_transceiver = rtc::make_ref_counted<RtpTransceiver>(
      ...
      context(), extensions,
      /* on_negotiation_needed= */ [] {});
  auto svc_extensions = svc_transceiver->GetHeaderExtensionsToNegotiate();
