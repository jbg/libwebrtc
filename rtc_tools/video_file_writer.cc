...
void WriteVideoToFile(const rtc::scoped_refptr<Video>& video,
  ...
  if (isY4m) {
    // Non-integer frame rates should be expressed as fractions, eg 30000:1001.
    int denominator = 1;
    while (std::fmod(fps * denominator, 1.0f) != 0.0 && denominator < 100000) {
      denominator *= 10;
    }
    fprintf(output_file, "YUV4MPEG2 W%d H%d F%g:%d C420\n", video->width(),
            video->height(), fps * denominator, denominator);
  }
