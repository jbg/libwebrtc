...
std::vector<LoggedPacketPointer> SortedPackets(
  ...
  for (const auto& stream : rtp_in_streams) {
    all_iters.emplace_back(new LogIterImpl<LoggedRtpPacketIncoming>(
        PacketType::kIncomingPacket, stream.incoming_packets));
  ...
  all_iters.emplace_back(new LogIterImpl<LoggedRtcpPacketTransportFeedback>(
      PacketType::kIncomingFeedback, in_feedbacks));
...
struct LoggedPacket {
  LoggedPacket(bool incoming, const LoggedRtpPacket& rtp)
      : incoming(incoming),
...
void PrintHeader() {
  printf(
      "incoming ssrc stream_seq transport_seq packet_size capt_time log_time "
      "recv_time feed_time\n");
}
void FormatOut(const LoggedPacket& packet) {
  const char* incoming_str = packet.incoming ? "true" : "false";
  printf("%s %u %u %.0f %ld %.6f %.6f %.6f %.6f\n", incoming_str, packet.ssrc,
...
void ParseLog(const ParsedRtcEventLogNew& parsed_log_) {
  ...
  for (LoggedPacketPointer& packet_ptr : sorted_packets) {
    switch (packet_ptr.type) {
      case PacketType::kIncomingPacket: {
      ...
      case PacketType::kIncomingFeedback: {
