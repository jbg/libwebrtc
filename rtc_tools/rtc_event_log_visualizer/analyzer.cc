...
EventLogAnalyzer::EventLogAnalyzer(const ParsedRtcEventLog& log,
                                   bool normalize_time)
    : parsed_log_(log) {
  config_.window_duration_ms_ = 250;
  config_.step_ms_ = 10;
...
void EventLogAnalyzer::CreateTotalIncomingBitrateGraph(Plot* plot) {
  ...
  if (!packets_in_order.empty()) {
    ...
    for (Timestamp time = config_.begin_time_;
         time < config_.end_time_ + TimeDelta::Millis(config_.step_ms_);
         time += TimeDelta::Millis(config_.step_ms_)) {
      ...
      while (window_begin != packets_in_order.end() &&
             window_begin->first <
                 time - TimeDelta::Millis(config_.window_duration_ms_)) {
        RTC_DCHECK_LE(window_begin->second, bytes_in_window);
        bytes_in_window -= window_begin->second;
        ++window_begin;
      }
      float window_duration_in_seconds =
          static_cast<float>(config_.window_duration_ms_) / kNumMillisecsPerSec;
