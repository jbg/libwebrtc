...
class AnalyzerConfig {
 public:
  ...
  // The generated data points will be `step_ms_` milliseconds apart.
  // Only events occurring at most `window_duration_ms_` milliseconds before the
...
void MovingAverage(
  ...
  for (Timestamp t = config.begin_time_;
       t < config.end_time_ + TimeDelta::Millis(config.step_ms_);
       t += TimeDelta::Millis(config.step_ms_)) {
    ...
    while (window_index_begin < data_view.size() &&
           data_view[window_index_begin].log_time() <
               t - TimeDelta::Millis(config.window_duration_ms_)) {
    ...
    float window_duration_s =
        static_cast<float>(config.window_duration_ms_) / kNumMillisecsPerSec;
