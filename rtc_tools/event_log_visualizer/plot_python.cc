...
void PythonPlot::Draw() {
  ...
  if (!series_list_.empty()) {
    ...
    // Other color maps that work well: jet, brg, nipy_spectral, Paired, rainbow
    // printf("cmap = plt.cm.get_cmap(\'rainbow\', color_count)\n");
    // printf("if color_count == 1:\n");
    // printf("  colors = [cmap(0.0)]\n");
    // printf("else:\n");
    // printf("  colors = [cmap(float(i)/(color_count-1)) for i in "
    //        "range(num_series)]\n");
