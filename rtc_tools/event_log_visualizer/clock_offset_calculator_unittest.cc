...
class OffsetTransactionBuilder final {
 public:
  ...
  void Build() {
    ...
    uint64_t time_ms = 0 + delay_until_start_;
    initiator_events_->emplace_back(time_ms * 1000,
                                    IceCandidatePairEventType::kCheckSent,
                                    candidate_pair_id_, transaction_id_);
    time_ms += network_delay_initiator_to_responder_;
    RTC_DCHECK(static_cast<int64_t>(time_ms) + offset_ >= 0);
    responder_events_->emplace_back((time_ms + offset_) * 1000,
                                    IceCandidatePairEventType::kCheckReceived,
                                    candidate_pair_id_, transaction_id_);
    time_ms += processing_delay_;
    responder_events_->emplace_back(
        (time_ms + offset_) * 1000,
        IceCandidatePairEventType::kCheckResponseSent, candidate_pair_id_,
        transaction_id_);
    time_ms += network_delay_responder_to_initiator_;
    initiator_events_->emplace_back(
        time_ms * 1000, IceCandidatePairEventType::kCheckResponseReceived,
        candidate_pair_id_, transaction_id_);
  }
 ...
 private:
  ...
  int64_t offset_ = 0;  // responder offset from initiator's clock
...
class ClockOffsetCalculatorTest : public ::testing::Test {
 public:
  ...
  void PrintEventsToLog() {
    RTC_LOG(LS_INFO) << "log1_events_ = " << ToString(log1_events_);
    RTC_LOG(LS_INFO) << "log2_events_ = " << ToString(log2_events_);
  }

  void ProcessTransactions() {
    IceTransactions transactions =
        webrtc::IceTransactions::BuildIceTransactions(log1_events_,
                                                      log2_events_);
    c_.ProcessTransactions(transactions);
  }

  void ExpectStats(int64_t expected_count,
                   int64_t expected_mean,
                   int64_t expected_median) {
    auto count = c_.FullSequenceCount();
    EXPECT_EQ(expected_count, count);
    auto mean = c_.CalculateMeanOffsetMs();
    EXPECT_EQ(expected_mean, mean);
    auto median = c_.CalculateMedianOffsetMs();
...
TEST_F(ClockOffsetCalculatorTest, TestMedianWithTwoEvents) {
  builder().set_offset(1).Build();
  builder().NextTransactionId().set_offset(3).Build();

  PrintEventsToLog();
  ProcessTransactions();
  // Median averages two middle values when count is even.
  ExpectStats(2, 2, 2);
}
...
TEST_F(ClockOffsetCalculatorTest, TestSecondLogInitiator) {
  // The second log is the initiator, so the output offsets will have the
  // opposite sign.
  builder().SwapOutputEvents();
  builder().set_delay_until_start(100).set_offset(-1).Build();
