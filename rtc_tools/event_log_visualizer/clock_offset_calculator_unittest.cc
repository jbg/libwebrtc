...
class ClockOffsetCalculatorTest : public ::testing::Test {
 public:
  ...
  void ExpectLimits(int64_t expect_first, int64_t expect_second) {
    auto limits = c_.CalculateOffsetLimits();
    EXPECT_EQ(expect_first, limits.first);
    EXPECT_EQ(expect_second, limits.second);
  }
...
TEST_F(ClockOffsetCalculatorTest, TestMedianWithTwoEvents) {
  ...
  PrintEventsToLog();
  ProcessLogs();
  // Median averages two middle values when count is event.
  ExpectStats(2, 2, 2);
}
...
TEST_F(ClockOffsetCalculatorTest, TestInconsistentLimits) {
  builder().set_delay_until_start(100).set_offset(10).Build();
  builder()
      .NextTransactionId()
      .set_delay_until_start(200)
      .set_offset(-10)
  ...
  // TODO(zstein): This is not useful as is.
  // ExpectLimits(-9, 9);
}
