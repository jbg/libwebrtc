...
// TODO(nisse): This is a copy of the class in
// test/function_audio_decoder_factory.h, which is in a test-only target.
// Rearrange build targets so we can include it here.
...
std::unique_ptr<test::NetEqStatsGetter> CreateNetEqTestAndRun(
  ...
  rtc::scoped_refptr<AudioDecoderFactory> decoder_factory =
      new rtc::RefCountedObject<FunctionAudioDecoderFactory>(
  ...
  test::NetEqTest test(config, decoder_factory, codecs, ext_codecs,
