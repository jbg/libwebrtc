...
void EventLogAnalyzer::CreateSendSideBweSimulationGraph(Plot* plot) {
  ...
  // We use BWE_TEST_LOGGING_COMPILE_TIME_ENABLE to print the internal state
  // of the AcknowledgedBitrateEstimator that is used inside the
  // SendSideCongestionController. If we instantiate it here too, we would get
  // duplicated outputs which might cause confusing and/or misleading results.
