...
std::pair<int64_t, int64_t> ClockOffsetCalculator::CalculateOffsetLimits()
    const {
  std::pair<int64_t, int64_t> result{0, 0};
  for (const auto& limits : offset_limits_) {
    result.first = std::max(result.first, limits.first);
    result.second = std::max(result.second, limits.second);
  }
  return result;
}
...
absl::optional<int64_t> ClockOffsetCalculator::CalculateOffset(
  ...
  offset_limits_.emplace_back(AbsDiff(*timestamps[1], *timestamps[0]),
                              AbsDiff(*timestamps[3], *timestamps[2]));
