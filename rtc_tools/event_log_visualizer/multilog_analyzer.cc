...
IceEventWithLogId::IceEventWithLogId(LoggedIceCandidatePairEvent event,
                                     size_t log_id)
...
MultiEventLogAnalyzer::MultiEventLogAnalyzer(
  ...
  // TODO(zstein): Use x axis from the config (optionally?).
...
void MultiEventLogAnalyzer::CreateIceSequenceDiagrams(
  ...
  for (const auto& transaction_events : events_by_transaction_id_) {
    ...
    const IceEventWithLogIdVec& ice_events_with_log_ids =
        transaction_events.second;
    ...
    for (const auto& ice_event_with_log_id : ice_events_with_log_ids) {
      candidate_pair_ids.insert(ice_event_with_log_id.event.candidate_pair_id);
      float x = ToCallTimeSec(ice_event_with_log_id.event.log_time_us(),
                              ice_event_with_log_id.log_id);
      float y = ice_event_with_log_id.log_id;
      has_non_check_msg |= ice_event_with_log_id.event.type !=
...
void MultiEventLogAnalyzer::CreateIceTransactionGraphs(
  ...
  for (const auto& transaction_events : events_by_transaction_id_) {
    ...
    for (const auto& ice_event_with_log_id : ice_events_with_log_ids) {
      candidate_pair_ids.insert(ice_event_with_log_id.event.candidate_pair_id);
      float x = ToCallTimeSec(ice_event_with_log_id.event.timestamp_us,
                              ice_event_with_log_id.log_id);
      float y = static_cast<float>(ice_event_with_log_id.event.type);
      has_non_check_msg |= ice_event_with_log_id.event.type !=
                           IceCandidatePairEventType::kCheckSent;
      time_series.points.emplace_back(x, y);
    }
...
void MultiEventLogAnalyzer::CreateIceTransactionRttGraphs(
  ...
  for (const auto& connection : connections) {
    ...
    if (y_limits.max() != 0 &&
      ...
      plot->SetSuggestedXAxis(x_limits.min(), x_limits.max(),
                              "Unnormalized Time (s)", 0.01, 0.01);
      plot->SetSuggestedYAxis(0, y_limits.max(), "RTT (us)", config_.y_margin,
                              config_.y_margin);
      plot->AppendTimeSeries(std::move(series));
    }
  }
