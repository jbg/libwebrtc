...
int main(int argc, char* argv[]) {
  ...
  absl::optional<webrtc::MultiEventLogAnalyzer> multi_analyzer;
  if (argc == 3) {
    ...
    webrtc::MultiEventLogAnalyzerConfig config(parsed_log, second_log);

    multi_analyzer.emplace(config, parsed_log.ice_candidate_pair_events(),
                           parsed_log.first_timestamp(),
                           second_log.ice_candidate_pair_events(),
                           second_log.first_timestamp());
  }

  if (FLAG_plot_ice_sequence_diagrams) {
    if (multi_analyzer) {
      multi_analyzer->CreateIceSequenceDiagrams(collection.get());
    } else {
  ...
  if (FLAG_plot_ice_transaction_graphs) {
    if (multi_analyzer) {
      multi_analyzer->CreateIceTransactionGraphs(collection.get());
    } else {
  ...
  if (FLAG_plot_ice_transaction_state_graph) {
    if (multi_analyzer) {
      multi_analyzer->CreateIceTransactionStateGraph(collection.get());
    } else {
  ...
  if (FLAG_plot_ice_transaction_rtt_graphs) {
    if (multi_analyzer) {
      multi_analyzer->CreateIceTransactionRttGraphs(collection.get());
    } else {
