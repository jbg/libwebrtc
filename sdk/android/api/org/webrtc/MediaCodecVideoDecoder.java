...
public class MediaCodecVideoDecoder {
  ...
  private boolean initDecode(int video_codec_type, int width, int height) {
    ...
    if (video_codec_type == VideoCodecTypeInt.VP8) {
      mime = VP8_MIME_TYPE;
      supportedCodecPrefixes = supportedVp8HwCodecPrefixes();
    } else if (video_codec_type == VideoCodecTypeInt.VP9) {
      mime = VP9_MIME_TYPE;
      supportedCodecPrefixes = supportedVp9HwCodecPrefixes;
    } else if (video_codec_type == VideoCodecTypeInt.H264) {
      mime = H264_MIME_TYPE;
      supportedCodecPrefixes = supportedH264HwCodecPrefixes();
    } else {
      throw new RuntimeException("initDecode: Non-supported codec " + video_codec_type);
    }
    DecoderProperties properties = findDecoder(mime, supportedCodecPrefixes);
    if (properties == null) {
      throw new RuntimeException("Cannot find HW decoder for " + video_codec_type);
    }

    Logging.d(TAG,
        "Java initDecode: " + video_codec_type + " : " + width + " x " + height + ". Color: 0x"
