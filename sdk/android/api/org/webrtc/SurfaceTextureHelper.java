...
public class SurfaceTextureHelper {
  ...
  private YuvConverter yuvConverterOES;
  private YuvConverter yuvConverterRGB;
  ...
  @Deprecated
  @SuppressWarnings("deprecation") // yuvConverterOES.convert is deprecated
  void textureToYUV(final ByteBuffer buf, final int width, final int height, final int stride,
    ...
    ThreadUtils.invokeAtFrontUninterruptibly(handler, new Runnable() {
      @Override
      public void run() {
        if (yuvConverterOES == null) {
          yuvConverterOES = new YuvConverter();
        }
        yuvConverterOES.convert(buf, width, height, stride, textureId, transformMatrix);
      }
    });
  }
  ...
  public VideoFrame.I420Buffer textureToYuv(final TextureBuffer textureBuffer) {
    ...
    ThreadUtils.invokeAtFrontUninterruptibly(handler, () -> {
      if (textureBuffer.getType() == TextureBuffer.Type.RGB) {
        // This is a generated mask texture
        if (yuvConverterRGB == null) {
          yuvConverterRGB = new YuvConverter();
        }
        result[0] = yuvConverterRGB.convertRedToY(textureBuffer);
      } else {
        if (yuvConverterOES == null) {
          yuvConverterOES = new YuvConverter();
        }
        result[0] = yuvConverterOES.convert(textureBuffer,
                textureBuffer.getMaskTextureId() != -1);

        if (textureBuffer.getMaskTextureId() == -1) return;

        if (yuvConverterRGB == null) {
          yuvConverterRGB = new YuvConverter();
        }

        int stride = (textureBuffer.getWidth() + 7) / 8 * 8;
        yuvConverterRGB.convert(result[0].getDataA(), textureBuffer.getWidth(), textureBuffer.getHeight(),
                stride, textureBuffer.getMaskTextureId(),
  ...
  // encoded.
  // |mask_texture| is an array of size 3 stores the texture id, width and height in order.
  public TextureBuffer createTextureBufferWithAlpha(
      int width, int height, Matrix transformMatrix, int[] mask_texture, Matrix maskTransform) {
    return new TextureBufferImpl(
            width, height, TextureBuffer.Type.OES, oesTextureId, transformMatrix, this,
            mask_texture[0], mask_texture[1], mask_texture[2], maskTransform,
            new Runnable() {
              @Override
              public void run() {
                returnTextureFrame();
