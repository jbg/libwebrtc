...
template <typename T, typename Java_T, typename Convert>
std::vector<T> JavaListToNativeVector(JNIEnv* env,
                                      const JavaRef<jobject>& j_list,
                                      Convert convert) {
  std::vector<T> native_list;
  for (const ScopedJavaLocalRef<jobject>& j_item : Iterable(env, j_list)) {
    native_list.emplace_back(convert(
        env,
        ScopedJavaLocalRef<Java_T>(env, static_cast<Java_T>(j_item.obj()))));
  }
