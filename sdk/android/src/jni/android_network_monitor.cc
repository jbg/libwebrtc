...
static rtc::IPAddress JavaToNativeIpAddress(
  ...
  JavaByteArrayReadableRef j_addresses(
      jni, Java_IPAddress_getAddress(jni, j_ip_address));
  size_t address_length = j_addresses.size();
  if (address_length == 4) {
    // IP4
    struct in_addr ip4_addr;
    memcpy(&ip4_addr.s_addr, j_addresses.data(), 4);
    return rtc::IPAddress(ip4_addr);
  }
  // IP6
  RTC_CHECK(address_length == 16);
  struct in6_addr ip6_addr;
  memcpy(ip6_addr.s6_addr, j_addresses.data(), address_length);
