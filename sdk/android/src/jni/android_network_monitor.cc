...
void AndroidNetworkMonitor::Start() {
  ...
  started_ = true;
  find_network_handle_without_ipv6_temporary_address_ =
      webrtc::field_trial::IsEnabled(
          "WebRTC-FindNetworkHandleWithoutIpv6TemporaryAddress");
...
rtc::NetworkBindingResult AndroidNetworkMonitor::BindSocketToNetwork(
  ...
  // as ERR_NETWORK_CHANGED, rather than MapSystemError(ENONET) which gives
  // back the less descriptive ERR_FAILED.
  if (rv == 0) {
...
AndroidNetworkMonitor::FindNetworkHandleFromAddress(
  ...
  if (find_network_handle_without_ipv6_temporary_address_) {
    for (auto const& iter : network_info_by_handle_) {
      ...
      auto address_it =
          std::find_if(addresses.begin(), addresses.end(),
                       [this, ip_address](rtc::IPAddress address) {
...
bool AndroidNetworkMonitor::AddressMatch(const rtc::IPAddress& ip1,
                                         const rtc::IPAddress& ip2) const {
