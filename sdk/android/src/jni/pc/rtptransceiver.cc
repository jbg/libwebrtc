...
RtpTransceiverInit JavaToNativeRtpTransceiverInit(
  ...
  ScopedJavaLocalRef<jobject> j_stream_labels =
      Java_RtpTransceiverInit_getStreamLabels(jni, j_init);
  for (const JavaRef<jobject>& j_stream_label :
       Iterable(jni, j_stream_labels)) {
    std::string stream_label = JavaToNativeString(jni, JavaParamRef<jstring>(
        static_cast<jstring>(j_stream_label.obj())));
    init.stream_labels.push_back(stream_label);
  }
  return init;
}
