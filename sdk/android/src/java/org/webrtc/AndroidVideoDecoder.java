...
class AndroidVideoDecoder implements VideoDecoder, VideoSink {
  ...
  protected void deliverDecodedFrame() {
    ...
    try {
      ...
      // is idle.
      int outputBufferId = codec.dequeueOutputBuffer(info, DEQUEUE_OUTPUT_BUFFER_TIMEOUT_US);
      ...
      if (outputBufferId < 0) {
        Logging.v(TAG, "dequeueOutputBuffer returned " + outputBufferId);
        return;
      }
      ...
      if (surfaceTextureHelper != null) {
        deliverTextureFrame(outputBufferId, info, rotation, decodeTimeMs);
      } else {
        deliverByteFrame(outputBufferId, info, rotation, decodeTimeMs);
      }
  ...
  private void deliverByteFrame(
      int outputBufferId, MediaCodec.BufferInfo info, int rotation, Integer decodeTimeMs) {
    // Load dimensions from shared memory under the dimension lock.
    int width;
    int height;
    int stride;
