...
typedef void (^VideoSinkCallback)(WebRTCVideoFrame *);
...
- (void)testOnCapturedFrameAdaptsFrame {
  ...
  WebRTCVideoFrame *frame =
      [[WebRTCVideoFrame alloc] initWithBuffer:buffer rotation:RTCVideoRotation_0 timeStampNs:0];
...
- (void)testOnCapturedFrameAdaptationResultsInCommonResolutions {
  ...
  WebRTCVideoFrame *frame = [[WebRTCVideoFrame alloc] initWithBuffer:buffer
...
- (void)testOnCapturedFrameWithoutAdaptation {
  ...
  ObjCCallbackVideoSink callback_video_sink(^void(WebRTCVideoFrame *outputFrame) {
    XCTAssertEqual(frame.width, outputFrame.width);
    XCTAssertEqual(frame.height, outputFrame.height);

    WebRTCCVPixelBuffer *outputBuffer = outputFrame.buffer;
...
- (void)testOnCapturedFrameI420BufferNeedsAdaptation {
  ...
  WebRTCI420Buffer *buffer = [[WebRTCI420Buffer alloc] initWithFrameBuffer:i420Buffer];
  WebRTCVideoFrame *frame =
      [[WebRTCVideoFrame alloc] initWithBuffer:buffer rotation:RTCVideoRotation_0 timeStampNs:0];
  ...
  ObjCCallbackVideoSink callback_video_sink(^void(WebRTCVideoFrame *outputFrame) {
    ...
    WebRTCI420Buffer *outputBuffer = (WebRTCI420Buffer *)outputFrame.buffer;
