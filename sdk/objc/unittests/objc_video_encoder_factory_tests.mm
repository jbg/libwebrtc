...
id<WebRTCVideoEncoderFactory> CreateEncoderFactoryReturning(int return_code) {
  id encoderMock = OCMProtocolMock(@protocol(WebRTCVideoEncoder));
  ...
  id encoderFactoryMock = OCMProtocolMock(@protocol(WebRTCVideoEncoderFactory));
...
id<WebRTCVideoEncoderFactory> CreateOKEncoderFactory() {
  return CreateEncoderFactoryReturning(WEBRTC_VIDEO_CODEC_OK);
}

id<WebRTCVideoEncoderFactory> CreateErrorEncoderFactory() {
  return CreateEncoderFactoryReturning(WEBRTC_VIDEO_CODEC_ERROR);
}

std::unique_ptr<webrtc::VideoEncoder> GetObjCEncoder(id<WebRTCVideoEncoderFactory> factory) {
...
TEST(ObjCVideoEncoderFactoryTest, EncodeReturnsOKOnSuccess) {
  ...
  rtc::scoped_refptr<webrtc::VideoFrameBuffer> buffer =
      new rtc::RefCountedObject<webrtc::ObjCFrameBuffer>(
          [[WebRTCCVPixelBuffer alloc] initWithPixelBuffer:pixel_buffer]);
