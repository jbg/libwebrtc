...
// A simple wrapper around EncodedImageBufferInterface to make it usable with
// associated objects.
@interface RTCWrappedEncodedImageBuffer : NSObject
@property(nonatomic) rtc::scoped_refptr<EncodedImageBufferInterface> buffer;
- (instancetype)initWithEncodedImageBuffer:(rtc::scoped_refptr<EncodedImageBufferInterface>)buffer;
@end
...
- (rtc::scoped_refptr<EncodedImageBufferInterface>)encodedData {
  RTCWrappedEncodedImageBuffer *wrappedBuffer =
      objc_getAssociatedObject(self, @selector(encodedData));
  return wrappedBuffer.buffer;
}

- (void)setEncodedData:(rtc::scoped_refptr<EncodedImageBufferInterface>)buffer {
