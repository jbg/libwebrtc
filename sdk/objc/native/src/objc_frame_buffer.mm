...
/** ObjCFrameBuffer that conforms to I420BufferInterface by wrapping WebRTCI420Buffer */
class ObjCI420FrameBuffer : public I420BufferInterface {
 public:
  explicit ObjCI420FrameBuffer(id<WebRTCI420Buffer> frame_buffer)
 ...
 private:
  id<WebRTCI420Buffer> frame_buffer_;
  int width_;
  int height_;
};
...
ObjCFrameBuffer::ObjCFrameBuffer(id<WebRTCVideoFrameBuffer> frame_buffer)
    : frame_buffer_(frame_buffer), width_(frame_buffer.width), height_(frame_buffer.height) {}
...
id<WebRTCVideoFrameBuffer> ObjCFrameBuffer::wrapped_frame_buffer() const {
  return frame_buffer_;
}

id<WebRTCVideoFrameBuffer> ToObjCVideoFrameBuffer(const rtc::scoped_refptr<VideoFrameBuffer>& buffer) {
  if (buffer->type() == VideoFrameBuffer::Type::kNative) {
    return static_cast<ObjCFrameBuffer*>(buffer.get())->wrapped_frame_buffer();
  } else {
    return [[WebRTCI420Buffer alloc] initWithFrameBuffer:buffer->ToI420()];
  }
}
