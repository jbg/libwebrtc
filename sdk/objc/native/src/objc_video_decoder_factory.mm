...
class ObjCVideoDecoder : public VideoDecoder {
 public:
  ObjCVideoDecoder(id<WebRTCVideoDecoder> decoder)
  ...
  int32_t Decode(const EncodedImage &input_image,
    ...
    WebRTCEncodedImage *encodedImage =
        [[WebRTCEncodedImage alloc] initWithNativeEncodedImage:input_image];
  ...
  int32_t RegisterDecodeCompleteCallback(DecodedImageCallback *callback) override {
    [decoder_ setCallback:^(WebRTCVideoFrame *frame) {
 ...
 private:
  id<WebRTCVideoDecoder> decoder_;
  const std::string implementation_name_;
};
}  // namespace

ObjCVideoDecoderFactory::ObjCVideoDecoderFactory(id<WebRTCVideoDecoderFactory> decoder_factory)
    : decoder_factory_(decoder_factory) {}

ObjCVideoDecoderFactory::~ObjCVideoDecoderFactory() {}

id<WebRTCVideoDecoderFactory> ObjCVideoDecoderFactory::wrapped_decoder_factory() const {
...
std::unique_ptr<VideoDecoder> ObjCVideoDecoderFactory::CreateVideoDecoder(
  ...
  for (WebRTCVideoCodecInfo *codecInfo in decoder_factory_.supportedCodecs) {
    if ([codecName isEqualToString:codecInfo.name]) {
      id<WebRTCVideoDecoder> decoder = [decoder_factory_ createDecoder:codecInfo];
...
std::vector<SdpVideoFormat> ObjCVideoDecoderFactory::GetSupportedFormats() const {
  ...
  for (WebRTCVideoCodecInfo *supportedCodec in decoder_factory_.supportedCodecs) {
