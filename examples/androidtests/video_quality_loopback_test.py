...
def SetUpTools(android_device, processes):
  # Ensure apprtc directory exists and is empty
  output_dir = os.path.join(RTC_TOOLS_DIR, 'apprtc')
  ...
  utils.UnpackArchiveTo(apprtc_archive, output_dir)
  utils.UnpackArchiveTo(golang_archive, output_dir)

  # Build AppRTC
  build_apprtc_script = os.path.join(testing_tools_dir, 'build_apprtc.py')
  ...
  _RunCommand([sys.executable, build_apprtc_path, apprtc_src_dir, go_dir,
              collider_dir])

  # Start AppRTC Server
  dev_appserver = os.path.join(output_dir, 'apprtc', 'temp', 'google-cloud-sdk',
                              'bin', 'dev_appserver.py')
  appengine_dir = os.path.join(output_dir, 'apprtc', 'out', 'app_engine')
  ...
  # Start Collider
  collider_path = os.path.join(output_dir, 'collider', 'collidermain')
  processes.append(_RunBackgroundCommand([
...
def main():
  ...
  try:
    ...
    SetUpTools(android_device, adb_path, processes)
    RunTest(android_device, temp_dir)
  finally:
