...
//
// It can be created via PeerConnectionFactoryDependencies::Create, then filled
// with any objects the caller wants to inject (including audio/video
// encoder/decoder factories), and passed into CreatePeerConnection (using
// std::move, since the struct is only moveable).
//
// All new dependencies should be added here instead of adding a new
// CreatePeerConnectionFactory overload. If possible please allow the
// PeerConnectionFactory to take ownership of the dependency by unique_ptr, as
...
// 3. Add a new Create method ("CreateWithoutBuiltInFoo"?) that *doesn't*
//    depend on "builtin_foo_factory", and leaves the "foo_factory" member as
//    null.
