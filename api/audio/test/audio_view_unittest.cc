...
TEST(AudioViewTest, InterleavedView) {
  ...
  InterleavedView<int16_t> interleaved(arr, 1, kArraySize);
  ...
  interleaved = InterleavedView<int16_t>(arr, 2, kArraySize / 2);
...
TEST(AudioViewTest, DeinterleavedView) {
  ...
  di = DeinterleavedView<int16_t>(arr, 2, 50);
  EXPECT_EQ(SamplesPerChannel(mono_ch), di.stride());
  const_di = di;
  ...
  EXPECT_EQ(&mono_ch[0], &arr[di.stride()]);
}
