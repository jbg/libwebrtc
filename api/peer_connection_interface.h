...
struct RTC_EXPORT PeerConnectionFactoryDependencies final {
  ...
  // `media_factory` allows webrtc to be optionally built without media support
  // (i.e., if only being used for data channels). By default media is disabled.
  // To enable media support call `EnableMedia(PeerConnectionFactoryDeps&)`.
  // Definition of the `MediaFactory` interface is webrtc implementation detail.
