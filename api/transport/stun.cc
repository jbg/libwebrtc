...
StunMessage::IntegrityStatus StunMessage::ValidateMessageIntegrity(
  ...
  if (IsStunRequestType(type_)) {
    RTC_HISTOGRAM_ENUMERATION("WebRTC.StunIntegrity.Request", integrity,
                              boundary);
  } else if (IsStunSuccessResponseType(type_)) {
    RTC_HISTOGRAM_ENUMERATION("WebRTC.StunIntegrity.Response", integrity,
                              boundary);
  } else if (IsStunIndicationType(type_)) {
    RTC_HISTOGRAM_ENUMERATION("WebRTC.StunIntegrity.Indication", integrity,
                              boundary);
  } else {
    ...
    if (!error_attribute) {
      RTC_HISTOGRAM_ENUMERATION(
          "WebRTC.StunIntegrity.ErrorResponse.NoErrorAttribute", integrity,
          boundary);
    } else {
      switch (error_attribute->code()) {
        case STUN_ERROR_TRY_ALTERNATE:
          RTC_HISTOGRAM_ENUMERATION(
              "WebRTC.StunIntegrity.ErrorResponse.TryAlternate", integrity,
              boundary);
          break;
        case STUN_ERROR_BAD_REQUEST:
          RTC_HISTOGRAM_ENUMERATION(
              "WebRTC.StunIntegrity.ErrorResponse.BadRequest", integrity,
              boundary);
          break;
        case STUN_ERROR_UNAUTHORIZED:
          RTC_HISTOGRAM_ENUMERATION(
              "WebRTC.StunIntegrity.ErrorResponse.Unauthorized", integrity,
              boundary);
          break;
        case STUN_ERROR_UNKNOWN_ATTRIBUTE:
          RTC_HISTOGRAM_ENUMERATION(
              "WebRTC.StunIntegrity.ErrorResponse.UnknownAttribute", integrity,
              boundary);
          break;
        case STUN_ERROR_STALE_NONCE:
          RTC_HISTOGRAM_ENUMERATION(
              "WebRTC.StunIntegrity.ErrorResponse.StaleNonce", integrity,
              boundary);
          break;
        case STUN_ERROR_SERVER_ERROR:
          RTC_HISTOGRAM_ENUMERATION(
              "WebRTC.StunIntegrity.ErrorResponse.ServerError", integrity,
              boundary);
          break;
        case STUN_ERROR_GLOBAL_FAILURE:
          RTC_HISTOGRAM_ENUMERATION(
              "WebRTC.StunIntegrity.ErrorResponse.GlobalFailure", integrity,
              boundary);
          break;
        default:
          RTC_HISTOGRAM_ENUMERATION(
              "WebRTC.StunIntegrity.ErrorResponse.ErrorOther", integrity,
              boundary);
          break;
      }
    }
