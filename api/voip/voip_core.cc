...
int VoipCore::CreateChannel() {
  int channel = -1;
  auto audio_channel = std::make_shared<AudioChannel>(
      Clock::GetRealTimeClock(), task_queue_factory_.get(),
...
bool VoipCore::UpdateAudioTransportWithSenders() {
...

  {
    ...
    for (auto channel : channels_) {
      auto* egress = channel->GetAudioEgress();
      if (egress->Sending()) {
        audio_senders.push_back(egress);
        max_sample_rate_hz =
            std::max(max_sample_rate_hz, egress->EncoderSampleRate());
        max_num_channels =
            std::max(max_num_channels, egress->EncoderNumChannel());
      }
    }
...
bool VoipCore::StopPlayout(int channel) {
  ...
  bool stop_device = true;
  {
    rtc::CritScope scoped(&lock_);
    for (auto channel : channels_) {
      if (channel->GetAudioIngress()->Playing()) {
        stop_device = false;
        break;
      }
    }
...
bool VoipCore::RegisterTransport(int channel, Transport* transport) {
  if (auto audio_channel = GetChannel(channel)) {
    return audio_channel->RegisterTransport(transport);
  }
  return false;
}

bool VoipCore::DeRegisterTransport(int channel) {
  if (auto audio_channel = GetChannel(channel)) {
    return audio_channel->DeRegisterTransport();
  }
  return false;
}
...
bool VoipCore::SetSendCodec(int channel,
  ...
  auto* egress = audio_channel->GetAudioEgress();
  auto encoder = encoder_factory_->MakeAudioEncoder(
      payload_type, encoder_format, absl::nullopt);
  egress->SetEncoder(payload_type, encoder_format, std::move(encoder));
  return true;
}
...
bool VoipCore::SetReceiveCodecs(
  ...
  auto* ingress = audio_channel->GetAudioIngress();
  ingress->SetReceiveCodecs(decoder_specs);
  return true;
}
