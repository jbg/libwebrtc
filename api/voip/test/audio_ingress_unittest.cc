...
class AudioIngressTest : public ::testing::Test {
 public:
  ...
  void SetUp() override {
    ingress_ = std::make_unique<AudioIngress>(decoder_factory_);
...
TEST_F(AudioIngressTest, SetAndGetRemoteSsrc) {
  ingress_->SetRemoteSSRC(kRemoteSsrc);
  EXPECT_EQ(ingress_->Ssrc(), static_cast<int>(kRemoteSsrc));
}
...
TEST_F(AudioIngressTest, NtpTimeEstimatorCallback) {
  constexpr int64_t kNtpTimestamp = 0xdeadc0dedeadbeef;
  auto ntp_estimator = [&](uint32_t) { return kNtpTimestamp; };
  ingress_->SetNtpEstimator(ntp_estimator);
  ...
  EXPECT_EQ(
      ingress_->GetAudioFrameWithInfo(kPcmuFormat.clockrate_hz, &audio_frame),
      AudioMixer::Source::AudioFrameInfo::kNormal);
  EXPECT_EQ(audio_frame.ntp_time_ms_, kNtpTimestamp);
}
