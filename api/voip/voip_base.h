...
//
// Underneath the interface, a channel handle is mapped into media session
// object that is capble of sending and receiving single RTP stream with
...
// session has RTP streams with multple remote endpoints.
//
...
class VoipBase {
 public:
  ...
  // engine. VoipEngine framework expect applications to handle network I/O
  ...
  // during media session construction.
  // Each channel handle maps into one audio media session where each has its
