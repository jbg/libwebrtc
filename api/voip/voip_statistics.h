...
struct NetEqStatistics {
  ...
  // Below statistics persist over the lifetime.
  ...
  // A delayed packet outage event is defined as an expand period caused not by
  // an actual packet loss, but by a delayed packet.
...
class VoipStatistics {
 public:
  // Gets the statistics on ACM (Audio Coding Module) decoding performance.
  virtual void GetDecodingStatistics(ChannelId channel_id,
                                     DecodingStatistics& decoding_stats) = 0;

  // Gets the statistics from NetEq. The statistics are reset after each query.
  virtual void GetNetEqStatistics(ChannelId channel_id,
                                  NetEqStatistics& neteq_stats) = 0;
