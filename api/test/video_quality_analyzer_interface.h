...
class VideoQualityAnalyzerInterface
 ...
 public:
  ...
  // Informs analyzer that peer `peer_name` is expected to receive all streams
  // from `sender_peer_name`.
  virtual void OnPeerPauseSenderStreams(absl::string_view peer_name,
                                        absl::string_view sender_peer_name) {}
  // Informs analyzer that peer `peer_name` shouldn't receive all streams from
  // sender `sender_peer_name`.
  virtual void OnPeerResumeSenderStreams(absl::string_view peer_name,
                                         absl::string_view sender_peer_name) {}
