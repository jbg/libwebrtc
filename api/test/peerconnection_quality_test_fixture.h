...
class PeerConnectionE2EQualityTestFixture {
 public:
  ...
  class VideoDumpOptions {
   public:
    // output_directory - output directory to which requried stream has to be
    // copied. The output files' names will be constructed as
    ...
    // export_frame_ids - specified if frame ids should be exported together
    // with content of the stream. If true, the file with name
    // `file_name`.frame_ids.txt will be created and it will contain assigned
    // frame ids in the same order as original frames in the output file with
    // stream content. (Default: false)
  ...
  struct VideoConfig {
    ...
    // this stream. The produced files contains what was rendered for this video
