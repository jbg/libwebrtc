...
MediaTransportPair::MediaTransportPair(rtc::Thread* thread)
    : first_(thread, &second_),
      ...
      first_datagram_transport_(thread, &second_datagram_transport_),
      second_datagram_transport_(thread, &first_datagram_transport_),
...
MediaTransportPair::LoopbackMediaTransport::LoopbackMediaTransport(
    ...
    : dc_transport_(thread, &other->dc_transport_),
      thread_(thread),
      other_(other) {
...
void MediaTransportPair::LoopbackMediaTransport::OnStateChanged() {
  ...
  dc_transport_.OnStateChanged(state_);
}

void MediaTransportPair::LoopbackDataChannelTransport::OnStateChanged(
    MediaTransportState state) {
  invoker_.AsyncInvoke<void>(RTC_FROM_HERE, thread_, [this, state] {
    RTC_DCHECK_RUN_ON(thread_);
    state_ = state;
...
void MediaTransportPair::LoopbackDatagramTransport::SetState(
    MediaTransportState state) {
  dc_transport_.OnStateChanged(state);
}
