...
void FillNV12Buffer(rtc::scoped_refptr<NV12Buffer> buf) {
  ...
  for (int row = 0; row < buf->ChromaHeight(); row++) {
    for (int col = 0; col < buf->StrideUV(); col += 2) {
      int u_index = row * buf->StrideUV() + col;
      buf->MutableDataUV()[u_index] = U;
      buf->MutableDataUV()[u_index + 1] = V;
    }
  }
}
...
TEST(NV12BufferTest, InitialStrides) {
...
TEST(NV12BufferTest, ToI420) {
  ...
  libyuv::I420ToNV12(
      reference->DataY(), reference->StrideY(), reference->DataU(),
      reference->StrideU(), reference->DataV(), reference->StrideV(),
      nv12_buffer->MutableDataY(), nv12_buffer->StrideY(),
      nv12_buffer->MutableDataUV(), nv12_buffer->StrideUV(), width, height);
