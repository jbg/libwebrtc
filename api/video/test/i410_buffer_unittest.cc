...
TEST(I410BufferTest, ReadPixels) {
  ...
  for (int row = 0; row < height; row++) {
    for (int col = 0; col < width; col++) {
      EXPECT_EQ(4, GetY(i410_buffer, col, row));
      EXPECT_EQ(8, GetU(i410_buffer, col, row));
      EXPECT_EQ(16, GetV(i410_buffer, col, row));
    }
  }
}
...
TEST(I410BufferTest, ToI420) {
  ...
  memset(reference->MutableDataY(), 1, size_y);
  memset(reference->MutableDataU(), 2, size_u);
  memset(reference->MutableDataV(), 4, size_v);
