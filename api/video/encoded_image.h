...
// releaseOutputBuffer.
class EncodedImageBufferInterface : public rtc::RefCountedBase {
 public:
  virtual const uint8_t* data() const = 0;
  // TODO(nisse): Make interface essentially read-only, delete this non-const
  // data method.
...
class EncodedImageBuffer : public EncodedImageBufferInterface {
 public:
  static rtc::scoped_refptr<EncodedImageBuffer> Create(size_t size) {
    return new EncodedImageBuffer(size);
  }
  static rtc::scoped_refptr<EncodedImageBuffer> Create(const uint8_t* data,
                                                       size_t size) {
    return new EncodedImageBuffer(data, size);
  }
...
class RTC_EXPORT EncodedImage {
 public:
  ...
  size_t capacity() const {
    return buffer_ ? capacity_ : encoded_data_->size();
  }
  ...
  void SetBuffer(rtc::scoped_refptr<EncodedImageBufferInterface> buffer) {
    encoded_data_ = buffer;
