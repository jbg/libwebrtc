...
class RTC_EXPORT VideoFrame {
 public:
  ...
  bool unchanged() { return unchanged_; }
  void set_unchanged(bool unchanged) { unchanged_ = unchanged; }
 ...
 private:
  ...
  absl::optional<ColorSpace> color_space_;
  absl::optional<PartialFrameDescription> partial_frame_description_;
  // Set by WebRTC if the current input frame is the same as the previous one.
  bool unchanged_;
};
