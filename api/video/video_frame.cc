...
VideoFrame::UpdateRect VideoFrame::UpdateRect::ScaleWithFrame(
  ...
  RTC_DCHECK_LE(crop_width, frame_width);
  RTC_DCHECK_LE(crop_height, frame_height);

  RTC_DCHECK_GT(scaled_width, 0);
  RTC_DCHECK_GT(scaled_height, 0);

  int x = offset_x - (frame_width - crop_width) / 2;
  int w = width;
  if (x < 0) {
    w += x;
    x = 0;
  }
  int y = offset_y - (frame_height - crop_height) / 2;
  int h = height;
  if (y < 0) {
    h += y;
    y = 0;
