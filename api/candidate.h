...
class Candidate {
 public:
  ...
  //
  // Each network in our implementation is identified by its network interface
  // name and network prefix (see rtc::Network::key() in network.h), so the same
  // network interface with multiple IPv4 and/or IPv6 addresses corresponds to
  // multiple networks and network ids.
  uint16_t network_id() const { return network_id_; }
  void set_network_id(uint16_t network_id) { network_id_ = network_id; }

  // An ID assigned to the network interface hosting the candidate.
  //
  // This id is uniquely assigned to each interface name.
