...
class IceTransportFactory {
 public:
  ...
  // construct into which it is injected, e.g. a peer connection. As a result,
  // the returned ICE transport should not hold anything that it does not own
  // and is limited to the lifetime of the host construct. Also, assumptions on
  // the threading safety of the returned transport should be clarified by
  // implementations. For example, a peer connection requires the returned
  // transport to be constructed and destroyed on the network thread and an ICE
  // transport factory that intends to work with a peer connection should offer
