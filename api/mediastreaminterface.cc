...
AudioProcessorInterface::GetStats(bool /*has_remote_tracks*/) {
  ...
  new_stats.apm_statistics.delay_median_ms =
      rtc::Optional<int32_t>(stats.echo_delay_median_ms);
  new_stats.apm_statistics.delay_standard_deviation_ms =
      rtc::Optional<int32_t>(stats.echo_delay_std_ms);
  new_stats.apm_statistics.echo_return_loss =
      rtc::Optional<double>(stats.echo_return_loss);
