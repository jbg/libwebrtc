...
struct Vp8EncoderConfig {
  struct TemporalLayerConfig {
    bool operator!=(const TemporalLayerConfig& rhs) const {
      return ts_number_layers != rhs.ts_number_layers ||
             ts_target_bitrate != rhs.ts_target_bitrate ||
             ts_rate_decimator != rhs.ts_rate_decimator ||
             ts_periodicity != rhs.ts_periodicity ||
             ts_layer_id != rhs.ts_layer_id;
    }
  ...
  absl::optional<uint32_t> rc_min_quantizer;
...
class Vp8FrameBufferController {
 public:
  ...
  // Called by the encoder before encoding a frame. Returns a set of possible
  // overrides the controller wishes to enact in the encoder's configuration.
