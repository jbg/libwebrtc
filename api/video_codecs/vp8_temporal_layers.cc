...
bool Vp8TemporalLayers::SupportsEncoderFrameDropping(
  ...
  return controllers_[stream_index]->SupportsEncoderFrameDropping(stream_index);
}
...
void Vp8TemporalLayers::OnRatesUpdated(
    ...
    int framerate_fps) {
  RTC_DCHECK_LT(stream_index, controllers_.size());
  return controllers_[stream_index]->OnRatesUpdated(stream_index, bitrates_bps,
                                                    framerate_fps);
}
...
bool Vp8TemporalLayers::UpdateConfiguration(size_t stream_index,
  ...
  RTC_DCHECK_LT(stream_index, controllers_.size());
  return controllers_[stream_index]->UpdateConfiguration(stream_index, cfg);
}

Vp8FrameConfig Vp8TemporalLayers::UpdateLayerConfig(size_t stream_index,
                                                    uint32_t rtp_timestamp) {
  RTC_DCHECK_LT(stream_index, controllers_.size());
  return controllers_[stream_index]->UpdateLayerConfig(stream_index,
                                                       rtp_timestamp);
}
...
void Vp8TemporalLayers::OnEncodeDone(size_t stream_index,
  ...
  RTC_DCHECK_LT(stream_index, controllers_.size());
  return controllers_[stream_index]->OnEncodeDone(
      stream_index, rtp_timestamp, size_bytes, is_keyframe, qp, info);
}
