...
namespace webrtc {
namespace video_encoder_factory_template_impl {

template <typename... Ts>
class VideoEncoderFactoryT : public VideoEncoderFactory {
 public:
  // TODO(philipel): What to do about duplicates?
 ...
 private:
  ...
  void GetSupportedFormatsInternal(std::vector<SdpVideoFormat>& formats) const {
    ...
    formats.insert(formats.begin(), supported_formats.begin(),
  ...
  std::unique_ptr<VideoEncoder> CreateVideoEncoderInternal(
      const SdpVideoFormat& format) {
    if (absl::c_count(V::SupportedFormats(), format) > 0) {
  ...
  CodecSupport QueryCodecSupportInternal(
    ...
    return {.is_supported = absl::c_count(GetSupportedFormats(), format) > 0};
  }
};
...
std::unique_ptr<VideoEncoderFactory> CreateVideoEncoderFactory() {
  ...
  static_assert(sizeof...(Ts) >= 1,
                "At least one implementation must be specified.");

  return std::make_unique<
      video_encoder_factory_template_impl::VideoEncoderFactoryT<Ts...>>();
}
