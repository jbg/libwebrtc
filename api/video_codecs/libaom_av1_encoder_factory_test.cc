...
class FrameEncoderSettingsBuilder {
 public:
  FrameEncoderSettingsBuilder() {
    frame_encode_settings_.output_functions =
        std::make_unique<OutFuncs>(ignored_output_);
  }
  ...
  FrameEncoderSettingsBuilder& Out(EncOut& out) {
    frame_encode_settings_.output_functions = std::make_unique<OutFuncs>(out);
    return *this;
  }
 ...
 private:
  struct OutFuncs : public VideoEncoderInterface::OutputFunctions {
    explicit OutFuncs(EncOut& e) : eo(e) {}
    rtc::ArrayView<uint8_t> GetBitstreamOutputBuffer(
        DataSize min_size) override {
      eo.bitstream.resize(min_size.bytes());
