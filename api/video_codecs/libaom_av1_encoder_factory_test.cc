...
class FrameEncoderSettingsBuilder {
 public:
  FrameEncoderSettingsBuilder() {
    frame_encode_settings_.result_callback = [](const EncodeResult& result) {};
    frame_encode_settings_.output_buffer_provider = [this](int min_size) {
      ignored_output_.bitstream.resize(min_size);
      return rtc::ArrayView<uint8_t>(ignored_output_.bitstream);
    };
  }
  ...
  FrameEncoderSettingsBuilder& Out(EncOut& out) {
    frame_encode_settings_.output_buffer_provider = [&out](int min_size) {
      out.bitstream.resize(min_size);
      return rtc::ArrayView<uint8_t>(out.bitstream);
    };
    frame_encode_settings_.result_callback =
        [&out](const EncodeResult& result) { out.res = result; };
    return *this;
  }
