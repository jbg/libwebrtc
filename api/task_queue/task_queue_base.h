...
class RTC_LOCKABLE RTC_EXPORT TaskQueueBase {
 public:
  ...
  // TODO(bugs.webrtc.org/xxx): Delete
  virtual void PostTask(std::unique_ptr<QueuedTask> task);
  // TODO(bugs.webrtc.org/xxx): Make pure virtual.
  virtual void Add(absl::AnyInvocable<void() &&> functor);
  ...
  // TODO(bugs.webrtc.org/xxx): Make pure virtual.
  virtual void AddDelayed(absl::AnyInvocable<void() &&> functor,
                          TimeDelta delay);
  ...
  // TODO(bugs.webrtc.org/xxx): Make pure virtual.
  virtual void AddWithHighPrecisionDelay(absl::AnyInvocable<void() &&> functor,
                                         TimeDelta delay);
  ...
  void AddDelayedWithPrecision(DelayPrecision precision,
                               absl::AnyInvocable<void() &&> functor,
                               TimeDelta delay) {
    switch (precision) {
      case DelayPrecision::kLow:
        AddDelayed(std::move(functor), delay);
        break;
      case DelayPrecision::kHigh:
        AddWithHighPrecisionDelay(std::move(functor), delay);
        break;
    }
  }
