...
TEST(RtpPacketInfosTest, BasicFunctionality) {
  auto p0 = RtpPacketInfo::Create(123, {1, 2}, 42, 89, 5, 7);
  auto p1 = RtpPacketInfo::Create(456, {3, 4}, 37, 89, 4, 1);
  auto p2 = RtpPacketInfo::Create(789, {5, 6}, 91, 88, 1, 7);

  RtpPacketInfos x({p0, p1, p2});

  ASSERT_THAT(x.size(), 3);

  EXPECT_THAT(x[0].get(), p0);
  EXPECT_THAT(x[1].get(), p1);
  EXPECT_THAT(x[2].get(), p2);
