...
Clock* ContextBuilder::DefaultClock() {
  return Clock::GetRealTimeClock();
}

TaskQueueFactory* ContextBuilder::DefaultTaskQueueFactory() {
  static TaskQueueFactory* tqf = CreateDefaultTaskQueueFactory().release();
  return tqf;
}

const FieldTrialsView* ContextBuilder::DefaultExperiments() {
  static FieldTrialsView* expriments = new FieldTrialBasedConfig;
  return expriments;
}

RtcEventLog* ContextBuilder::DefaultRtcEventLog() {
