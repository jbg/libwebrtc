...
class MediaTransportEncodedVideoFrame {
 public:
  enum class FrameType {
    // Key frame -- flushes the state of the decoder (equivalent to
    // webrtc::kVideoFrameKey).
    kFrameKey,

    // Delta frame (equivalent to webrtc::kVideoFrameDelta).
    kFrameDelta,
  };
  ...
  FrameType frame_type() const { return frame_type; }
 ...
 private:
  ...
  // Whether the given frame is a keyframe or not.
  FrameType frame_type_;
