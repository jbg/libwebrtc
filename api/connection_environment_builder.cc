...
ConnectionEnvironment ConnectionEnvironmentBuilder::Build() {
  ...
  if (!deleters_.empty()) {
    class DeleterStorage : public rtc::RefCountedBase {
     public:
      DeleterStorage(rtc::scoped_refptr<rtc::RefCountedBase> parent,
                     std::vector<absl::AnyInvocable<void() &&>> deleters)
          : parent_(std::move(parent)), deleters_(std::move(deleters)) {}

     private:
      rtc::scoped_refptr<rtc::RefCountedBase> parent_;
      std::vector<absl::AnyInvocable<void() &&>> deleters_;
    };
    storage_ = rtc::make_ref_counted<DeleterStorage>(std::move(storage_),
                                                     std::move(deleters_));
  }
  ...
  return ConnectionEnvironment(storage_, clock_, task_queue_factory_,
                               experiments_, event_log_);
}
