  ...
  bool IsOk() const {
    ...
    for (size_t i = 0; i < single_stream_config.num_channels; ++i) {
      ...
      if (coded_channels_to_input_channels[coded_channel] != kNotSet) {
        // Can't use 'log' from api/ ?
        // RTC_LOG(LS_INFO) << "Multistream coded channel #" << coded_channel
        //                  << " cannot come from both input channels "
        //                  << coded_channels_to_input_channels[coded_channel]
        //                  << " and " << i;
        return false;
      }
    ...
    for (int i = 0; i < max_coded_channel; ++i) {
      if (coded_channels_to_input_channels[i] == kNotSet) {
        // Can't use LOG in api/
        // RTC_LOG(LS_INFO) << "Multistream coded channel #" << i
        //                  << " has unspecified input channel.";
        return false;
      }
    }
