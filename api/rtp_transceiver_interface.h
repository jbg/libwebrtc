...
struct RtpHeaderExtensionCapabilityWithOptionalDirection
  ...
  RtpHeaderExtensionCapabilityWithOptionalDirection(
      const std::string& uri,
      absl::optional<int> preferred_id,
      absl::optional<RtpTransceiverDirection> direction)
      : RtpHeaderExtensionCapability(uri), direction(direction) {
    this->preferred_id = preferred_id;
  }
  // A nullopt value means an by default unexposed header extension.
  absl::optional<RtpTransceiverDirection> direction;
};
...
class RTC_EXPORT RtpTransceiverInterface : public rtc::RefCountInterface {
 public:
  ...
  // ???? FIXME: ref
  // https://w3c.github.io/webrtc-pc/#dom-rtcrtptransceiver-SetOfferedRtpHeaderExtensions
  virtual std::vector<RtpHeaderExtensionCapabilityWithOptionalDirection>
