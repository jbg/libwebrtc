...
class EmulatedNonMonotoneousClock : public EmulatedClock {
 public:
  ...
  int64_t Query(int64_t time_us) {
    size_t ixStart = last_reset_query_time_us_ / kResolutionUs + 1;
    size_t ixEnd = time_us / kResolutionUs;
...
TEST(ClockRepair, NoClockDrift) {
  ...
  for (int seed = kFirstSeed; seed < kSeeds + kFirstSeed; ++seed) {
    std::vector<std::tuple<int64_t, int64_t, int64_t, int64_t, int64_t>> clocks;
    ...
    for (int64_t time_us = 0; time_us < kRuntimeUs;
      ...
      if (monotone_clock.Stalled() && non_monotone_clock.WasReset()) {
        ...
        if (initial_clock_stall) {
          // Resets during an initial clock stall cannot be repaired and error
          // will propagate through rest of trace.
      ...
      clocks.emplace_back(std::make_tuple(time_us, socket_time_us,
                                          system_time_us, monotone_us,
                                          corrected_clock_us));
