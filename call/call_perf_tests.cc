...
void CallPerfTest::TestEncodeFramerate(VideoEncoderFactory* encoder_factory,
                                       const std::string& payload_name,
                                       const std::vector<int>& max_framerates) {
  static constexpr int64_t kMinGetStatsIntervalMs = 400;
  static constexpr int64_t kMinRunTimeMs = 15000;
   ...
   public:
    ...
    void ModifyVideoConfigs(
      ...
      for (size_t i = 0; i < max_framerates_.size(); ++i) {
        ...
        stream.max_framerate = max_framerates_[i];
      }
    ...
    Action OnSendRtp(const uint8_t* packet, size_t length) override {
      ...
      if (now_ms - last_getstats_ms_ > kMinGetStatsIntervalMs) {
        ...
        task_queue_->PostTask(ToQueuedTask([this]() {
          ...
          if (clock_->TimeInMilliseconds() - start_ms_ > kMinRunTimeMs) {
            VerifyStats();
            observation_complete_.Set();
          }
        }));
