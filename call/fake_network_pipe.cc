...
uint64_t NetworkPacketStorage::EmplaceBack(NetworkPacket&& packet) {
...
NetworkPacket NetworkPacketStorage::Remove(uint64_t packet_id) {
  ...
  for (auto it = packets_.begin(); it != packets_.end() && it->removed;
       ++packet_it) {
    packets_.pop_front();
  }
...
bool FakeNetworkPipe::EnqueuePacket(rtc::CopyOnWriteBuffer packet,
  ...
  int64_t packet_id = packets_in_flight_.EmplaceBack(std::move(net_packet));
