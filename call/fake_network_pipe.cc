...
NetworkPacket NetworkPacketStorage::Pop(NetworkPacket* packet_ptr) {
  ...
  // We canät pop a packet twice, ensure that it is not in the deque and not in
  // the pending removal set.
  ...
  // Cleanup: If it is the first packet in the deque, remove the enrty and then
