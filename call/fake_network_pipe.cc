...
FakeNetworkPipe::FakeNetworkPipe(
    ...
    : FakeNetworkPipe(clock, std::move(network_simulation), nullptr, nullptr) {}

FakeNetworkPipe::FakeNetworkPipe(
    Clock* clock,
    std::unique_ptr<NetworkSimulationInterface>&& network_simulation,
    PacketReceiver* receiver)
    : FakeNetworkPipe(clock, std::move(network_simulation), receiver, nullptr) {
}
...
uint64_t NetworkPacketStorage::Enqueue(NetworkPacket&& packet) {
  packets_.emplace_back(StoredPacket{std::move(packet), false});
  return (uint64_t)&packets_.back();
}

void NetworkPacketStorage::PopBack() {
  packets_.pop_back();
}

NetworkPacket NetworkPacketStorage::Remove(uint64_t packet_id) {
  auto packet_it = std::find_if(packets_.begin(), packets_.end(),
                                [&packet_id](StoredPacket& packet_ref) {
                                  return (uint64_t)&packet_ref == packet_id;
                                });
  // Check that packet is in the deque of packets in flight.
  RTC_CHECK(packet_it != packets_.end());
  // Check that packet is not already removed.
  RTC_DCHECK(!packet_it->removed);

  NetworkPacket packet_to_return = std::move(packet_it->packet);
...
void FakeNetworkPipe::Process() {
  ...
  std::queue<NetworkPacket> packets_to_deliver;
  {
    ...
    for (auto& delivery_info : delivery_infos) {
      NetworkPacket packet = packets_in_flight_.Remove(delivery_info.packet_id);
