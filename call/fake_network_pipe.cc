...
std::vector<DelayedPacketInfo> SimulatedNetwork::PopDeliverablePackets(
  ...
  }
  {
    ...
    while (!delay_link_.empty() &&
      ...
      packets_to_deliver.emplace_back(
          DelayedPacketInfo(packet_info.packet, packet_info.arrival_time_us));
...
void FakeNetworkPipe::Process() {
  ...
  std::queue<NetworkPacket> packets_to_deliver;
  {
    ...
    std::vector<DelayedPacketInfo> delivery_infos =
        fake_network_->PopDeliverablePackets(time_now_us);
    for (auto& delivery_info : delivery_infos) {
      ...
      if (delivery_info.receive_time_us != DelayedPacketInfo::kNotReceived) {
  ...
  rtc::Optional<int64_t> delivery_us = fake_network_->NextDeliveryTimeUs();
