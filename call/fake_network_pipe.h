...
// Class storing network packets. It allows adding packets to the end and
// getting packets as specified by an identifier.
class NetworkPacketStorage {
 public:
  // Adds a packet to the end and returns a unique id to it.
  uint64_t EmplaceBack(NetworkPacket&& packet);
  // Removes the packet with packet_id and returns it.
  NetworkPacket Remove(uint64_t packet_id);
  void PopBack();
  bool Empty() const;
