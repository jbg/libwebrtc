...
// to handle removing stored packets out of order without having to sort the
// packets left. When a packet is removed, the address is added to a set of
...
class NetworkPacketStorage {
 public:
  ...
  uint64_t Enqueue(NetworkPacket&& packet);
  // Removes the packet as indicated by packet_ptr and returns it.
  NetworkPacket Remove(uint64_t);
  // Removes the last packet.
  void PopBack();
