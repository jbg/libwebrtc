...
void RtpPayloadParams::Vp9ToGeneric(const CodecSpecificInfoVP9& vp9_info,
  ...
  const int last_active_spatial_id_plus_1 = vp9_header.num_spatial_layers;
  ...
  if (!(temporal_index < num_temporal_layers &&
        ...
        spatial_index < last_active_spatial_id_plus_1 &&
        last_active_spatial_id_plus_1 <= num_spatial_layers)) {
    ...
    RTC_LOG(LS_ERROR) << "Inconsistent layer id sid=" << spatial_index
                      ...
                      << (last_active_spatial_id_plus_1 - 1) << "]";
    return;
  }
  ...
  result.active_decode_targets =
      ((uint32_t{1} << num_temporal_layers * last_active_spatial_id_plus_1) -
       1) ^
  ...
  if (!vp9_header.inter_pic_predicted && !vp9_header.inter_layer_predicted) {
    ...
    for (int sid = spatial_index; sid < last_active_spatial_id_plus_1; ++sid) {
      chain_last_frame_id_[sid] = -1;
    }
  }
  result.chain_diffs.resize(num_spatial_layers, 0);
  for (int sid = first_active_spatial_id; sid < last_active_spatial_id_plus_1;
       ++sid) {
  ...
  if (temporal_index == 0) {
    ...
    if (!vp9_header.non_ref_for_inter_layer_pred) {
      for (int sid = spatial_index + 1; sid < last_active_spatial_id_plus_1;
           ++sid) {
