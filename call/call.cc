...
class SendDegradationProxy : public Transport {
 public:
  SendDegradationProxy(
      const webrtc::FakeNetworkPipe::Config& config,
      Clock* clock,
  ...
  void SetTransport(Transport* transport, ProcessThread* process_thread) {
    ...
    if (transport_) {
      ...
      fake_pipe_ =
          rtc::MakeUnique<FakeNetworkPipe>(clock_, config_, transport_);
  ...
  bool SendRtp(const uint8_t* packet,
    ...
    if (!fake_pipe_)
      return false;
    fake_pipe_->SendRtp(packet, length, options);

    rtc::SentPacket packet_info;
    packet_info.packet_id = options.packet_id;
    packet_info.send_time_ms = clock_->TimeInMilliseconds();
    send_delay_stats_->OnSentPacket(packet_info.packet_id,
                                    packet_info.send_time_ms);
    transport_controller_->OnSentPacket(packet_info);

    // Always return true, indicating the packet was sent. If we let the pacer
    // know the fake network pipe dropped it, it will be retried automatically.
    return true;
  }
 ...
 private:
  ...
  RtpTransportControllerSendInterface* const transport_controller_;
...
PacketReceiver::DeliveryStatus InternalPacketReceiver::DeliverPacket(
  ...
  if (RtpHeaderParser::IsRtcp(packet.cdata(), packet.size()))
    return call_->DeliverRtcp(media_type, packet.cdata(), packet.size());

  return call_->DeliverRtp(media_type, std::move(packet), packet_time);
}
...
void Call::OnSentPacket(const rtc::SentPacket& sent_packet) {
  if (degraded_send_transport_) {
    // We use the time the packet was put into the fake network as send time,
    // so ignore when the packet was actually sent (otherwise we would hide
    // the fake network delay from the congestion controller).
    return;
  }
