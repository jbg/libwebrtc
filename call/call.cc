...
class SharedModuleThread::Impl {
 public:
  Impl(const char* name, std::function<void()> on_one_ref_remaining)
      : module_thread_(ProcessThread::Create(name)),
...
SharedModuleThread::SharedModuleThread(
    ...
    : impl_(std::make_unique<Impl>(name, std::move(on_one_ref_remaining))) {}
...
rtc::scoped_refptr<SharedModuleThread> SharedModuleThread::Create(
    const char* name,
    std::function<void()> on_one_ref_remaining) {
  return new SharedModuleThread(name, std::move(on_one_ref_remaining));
}
