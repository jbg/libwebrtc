...
class Call final : public webrtc::Call,
 ...
 private:
  ...
  rtc::SentPacket last_sent_packet_ RTC_GUARDED_BY(mutex_);
...
void Call::OnSentPacket(const rtc::SentPacket& sent_packet) {
  // Every SentPacket is multiply notified by different channels, as
  // WebRtcVoiceMediaChannel and WebRtcVideoChannel share the same RtpTransport.
  // Filter SentPacket to avoid redundant notifications to downstream objects.
  {
    MutexLock lock(&mutex_);
    if (last_sent_packet_.packet_id == sent_packet.packet_id &&
