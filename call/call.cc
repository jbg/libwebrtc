...
class Call final : public webrtc::Call,
 ...
 public:
  ...
  //  media transport.
  void MaybeRegisterMediaTransportRateObserver(
      MediaTransportInterface* media_transport)
      RTC_LOCKS_EXCLUDED(target_rate_observer_subscription_crit_);
 ...
 private:
  ...
  bool registered_target_rate_observer_
...
void Call::MaybeRegisterMediaTransportRateObserver(
    MediaTransportInterface* media_transport) {
  if (!media_transport) {
    return;
  }
  ...
  if (!registered_target_rate_observer_) {
    media_transport_ = media_transport;
    registered_target_rate_observer_ = true;
...
void Call::MediaTransportChange(MediaTransportInterface* media_transport) {
  ...
  MaybeRegisterMediaTransportRateObserver(media_transport);

  rtc::CritScope lock(&target_rate_observer_subscription_crit_);
  if (!media_transport && media_transport_ &&
      registered_target_rate_observer_) {
    media_transport_->RemoveTargetTransferRateObserver(this);
    media_transport_ = nullptr;
    registered_target_rate_observer_ = false;
  }
}
...
webrtc::AudioSendStream* Call::CreateAudioSendStream(
  ...
  {
    MaybeRegisterMediaTransportRateObserver(config.media_transport);
