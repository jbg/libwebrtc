...
class Call final : public webrtc::Call,
 ...
 private:
  ...
  Call::Config config_ RTC_GUARDED_BY(worker_thread_);
  const WebRtcKeyValueConfig& trials_;  // Maps to config_.trials.
  ...
  rtc::scoped_refptr<PendingTaskSafetyFlag> send_transport_safety_ =
...
Call::~Call() {
  ...
  if (is_started_) {
    // Stop callback handling while we're tearing things down.
    send_transport_queue_->PostTask(ToQueuedTask([this]() {
      RTC_DCHECK_RUN_ON(send_transport_queue_);
      send_transport_safety_->SetNotAlive();
    }));
  }
...
void Call::EnsureStarted() {
  ...
  // Open callback functions on the transport queue for receiving callbacks.
  // This callback will run just before `RegisterTargetTransferRateObserver`
  // sets `this` as the observer callback.
  send_transport_queue_->PostTask(ToQueuedTask([this]() {
    RTC_DCHECK_RUN_ON(send_transport_queue_);
    send_transport_safety_->SetAlive();
  }));
...
void Call::OnStartRateUpdate(DataRate start_rate) {
  RTC_DCHECK_RUN_ON(send_transport_queue_);
  RTC_DCHECK(send_transport_safety_->alive());
