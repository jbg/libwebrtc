...
std::unique_ptr<Call> Call::Create(const Call::Config& config) {
  return Create(config, RtpTransportControllerSendFactory().Create(
...
std::unique_ptr<Call> Call::Create(
  ...
  return std::make_unique<internal::Call>(config,
...
Call::Call(const Call::Config& config,
    ...
    : c_(config.context),
      ...
      decode_sync_(config.metronome
                       ? std::make_unique<DecodeSynchronizer>(&c_.clock(),
                                                              config.metronome,
                                                              worker_thread_)
                       : nullptr),
      num_cpu_cores_(CpuInfo::DetectNumberOfCores()),
      call_stats_(new CallStats(&c_.clock(), worker_thread_)),
      bitrate_allocator_(new BitrateAllocator(this)),
      config_(config),
      audio_network_state_(kNetworkDown),
      video_network_state_(kNetworkDown),
      aggregate_network_up_(false),
      receive_stats_(&c_.clock()),
      send_stats_(&c_.clock()),
      receive_side_cc_(&c_.clock(),
      ...
      receive_time_calculator_(
          ReceiveTimeCalculator::CreateFromFieldTrial(c_.experiments())),
      video_send_delay_stats_(new SendDelayStats(&c_.clock())),
      start_of_call_(c_.clock().CurrentTime()),
  ...
  receive_side_cc_periodic_task_ = RepeatingTaskHandle::Start(
      ...
      TaskQueueBase::DelayPrecision::kLow, &c_.clock());
}
...
Call::~Call() {
  ...
  RTC_HISTOGRAM_COUNTS_100000(
      ...
      (c_.clock().CurrentTime() - start_of_call_).seconds());
}
...
webrtc::AudioSendStream* Call::CreateAudioSendStream(
  ...
  AudioSendStream* send_stream = new AudioSendStream(
      &c_.clock(), config, config_.audio_state, &c_.task_queue_factory(),
      transport_send_.get(), bitrate_allocator_.get(), &c_.event_log(),
      call_stats_->AsRtcpRttStats(), suspended_rtp_state, c_.experiments());
...
webrtc::AudioReceiveStreamInterface* Call::CreateAudioReceiveStream(
  ...
  RTC_DCHECK_RUN_ON(worker_thread_);
  EnsureStarted();
  c_.event_log().Log(std::make_unique<RtcEventAudioReceiveStreamConfig>(
      CreateRtcLogStreamConfig(config)));

  AudioReceiveStreamImpl* receive_stream = new AudioReceiveStreamImpl(
      &c_.clock(), transport_send_->packet_router(), config_.neteq_factory,
      config, config_.audio_state, &c_.event_log());
...
webrtc::VideoSendStream* Call::CreateVideoSendStream(
  ...
  for (size_t ssrc_index = 0; ssrc_index < config.rtp.ssrcs.size();
       ++ssrc_index) {
    c_.event_log().Log(std::make_unique<RtcEventVideoSendStreamConfig>(
  ...
  VideoSendStream* send_stream = new VideoSendStream(
      &c_.clock(), num_cpu_cores_, &c_.task_queue_factory(), network_thread_,
      call_stats_->AsRtcpRttStats(), transport_send_.get(),
      bitrate_allocator_.get(), video_send_delay_stats_.get(), &c_.event_log(),
...
webrtc::VideoSendStream* Call::CreateVideoSendStream(
  ...
  std::unique_ptr<FecController> fec_controller =
      config_.fec_controller_factory
          ...
          : std::make_unique<FecControllerDefault>(&c_.clock());
...
webrtc::VideoReceiveStreamInterface* Call::CreateVideoReceiveStream(
  ...
  c_.event_log().Log(std::make_unique<RtcEventVideoReceiveStreamConfig>(
  ...
  VideoReceiveStream2* receive_stream = new VideoReceiveStream2(
      &c_.task_queue_factory(), this, num_cpu_cores_,
      transport_send_->packet_router(), std::move(configuration),
      call_stats_.get(), &c_.clock(),
      std::make_unique<VCMTiming>(&c_.clock(), c_.experiments()),
      &nack_periodic_processor_, decode_sync_.get(), &c_.event_log());
...
FlexfecReceiveStream* Call::CreateFlexfecReceiveStream(
  ...
  FlexfecReceiveStreamImpl* receive_stream = new FlexfecReceiveStreamImpl(
      &c_.clock(), std::move(config), &video_receiver_controller_,
...
void Call::DeliverRtcpPacket(rtc::CopyOnWriteBuffer packet) {
  ...
  if (rtcp_delivered) {
    c_.event_log().Log(std::make_unique<RtcEventRtcpPacketIncoming>(packet));
  }
}
...
void Call::DeliverRtpPacket(
  ...
  if (receive_time_calculator_) {
    ...
    packet_time_us = receive_time_calculator_->ReconcileReceiveTimes(
        packet_time_us, rtc::TimeUTCMicros(), c_.clock().TimeInMicroseconds());
    packet.set_arrival_time(Timestamp::Micros(packet_time_us));
  }

  NotifyBweOfReceivedPacket(packet, media_type);

  c_.event_log().Log(std::make_unique<RtcEventRtpPacketIncoming>(packet));
