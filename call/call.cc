...
class Call final : public webrtc::Call,
 ...
 private:
  ...
  Call::Config config_;
  // TODO(tommi): Replace with worker_thread_.
  ...
  std::map<uint32_t, VideoSendStream*> video_send_ssrcs_
      RTC_GUARDED_BY(configuration_sequence_checker_);
  std::set<VideoSendStream*> video_send_streams_ RTC_GUARDED_BY(configuration_sequence_checker_);
...
void Call::UpdateAggregateNetworkState() {
  ...
  bool have_audio = !audio_send_ssrcs_.empty() || !audio_receive_streams_.empty();
  bool have_video = !video_send_ssrcs_.empty() || !video_receive_streams_.empty();
...
void Call::OnTargetTransferRate(TargetTransferRate msg) {
  ...
  worker_thread_->PostTask(
      ToQueuedTask(task_safety_, [this, target_bitrate_bps]() {
        ...
        bool sending_video = !video_send_streams_.empty();
