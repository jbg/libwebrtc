...
void RampUpTester::EnsurePollTimeSet() {
  RTC_DCHECK_RUN_ON(task_queue_);
  if (!poll_time_base_)
    poll_time_base_ = rtc::TimeMillis();
}

int64_t RampUpTester::GetIntervalForNextPoll() {
  RTC_DCHECK_RUN_ON(task_queue_);
  RTC_DCHECK_NE(poll_time_base_, 0) << "No call to EnsurePollTimeSet()";
  auto now = rtc::TimeMillis();
  poll_time_base_ += kPollIntervalMs;
  auto interval = poll_time_base_ - now;
