...
TEST_F(BitrateAllocatorTest,
  ...
  // Simulate some packet loss and assume that the |bitrate_observer| uses
  // 80% of it's allocated total bitrate for media and the rest for protection.
  uint8_t fraction_loss = 32;  // Q8.
  ...
  allocator_->OnNetworkChanged(120001, 0, fraction_loss,
                               kDefaultProbingIntervalMs);
  EXPECT_EQ(120001u, bitrate_observer.last_bitrate_bps_);

  // Simulate a non-lossy network and assume that the |bitrate_observer| uses
  // 100% of it's allocated total bitrate for media.
  fraction_loss = 0;
  bitrate_observer.SetBitrateProtectionRatio(/*protection_ratio=*/0.0);
  // Total allocation limits are unaffected by the protection rate change.
  EXPECT_CALL(limit_observer_, OnAllocationLimitsChanged(_, _, _)).Times(0);
  allocator_->OnNetworkChanged(120002, 0, fraction_loss,
                               kDefaultProbingIntervalMs);
  EXPECT_EQ(120002u, bitrate_observer.last_bitrate_bps_);

  // Again simulate some packet loss and protection.
  fraction_loss = 32;
  bitrate_observer.SetBitrateProtectionRatio(/*protection_ratio=*/0.2);
  // Total allocation limits are unaffected by the protection rate change.
  EXPECT_CALL(limit_observer_, OnAllocationLimitsChanged(_, _, _)).Times(0);
  allocator_->OnNetworkChanged(120003, 0, fraction_loss,
                               kDefaultProbingIntervalMs);
