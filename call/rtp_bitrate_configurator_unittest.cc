...
class RtpBitrateConfiguratorTest : public testing::Test {
 public:
  RtpBitrateConfiguratorTest()
      : configurator_(new RtpBitrateConfigurator(BitrateConfig())) {}
  std::unique_ptr<RtpBitrateConfigurator> configurator_;
  void ConfigMatches(BitrateConfig bitrate_config, BitrateConfigMask match) {
    rtc::Optional<BitrateConfig> result =
        configurator_->UpdateBitrateConfig(bitrate_config);
    EXPECT_TRUE(result.has_value());
    if (match.start_bitrate_bps.has_value())
      EXPECT_EQ(result->start_bitrate_bps, match.start_bitrate_bps);
    if (match.min_bitrate_bps.has_value())
      EXPECT_EQ(result->min_bitrate_bps, match.min_bitrate_bps);
    if (match.max_bitrate_bps.has_value())
      EXPECT_EQ(result->max_bitrate_bps, match.max_bitrate_bps);
  }

  void MaskMatches(BitrateConfigMask bitrate_mask, BitrateConfigMask match) {
    rtc::Optional<BitrateConfig> result =
        configurator_->UpdateBitrateConfigMask(bitrate_mask);
    EXPECT_TRUE(result.has_value());
    if (match.start_bitrate_bps.has_value())
      EXPECT_EQ(result->start_bitrate_bps, match.start_bitrate_bps);
    if (match.min_bitrate_bps.has_value())
  ...
  BitrateConfigMask Match(rtc::Optional<int> min_bitrate_bps,
                          rtc::Optional<int> start_bitrate_bps,
                          rtc::Optional<int> max_bitrate_bps) {
    BitrateConfigMask mask;
    mask.min_bitrate_bps = min_bitrate_bps;
    mask.start_bitrate_bps = start_bitrate_bps;
    mask.max_bitrate_bps = max_bitrate_bps;
    return mask;
  }
};
...
TEST_F(RtpBitrateConfiguratorTest, NewConfigWithValidConfigReturnsNewConfig) {
  ...
  ConfigMatches(bitrate_config, Match(1, 2, 3));
}
...
TEST_F(RtpBitrateConfiguratorTest, NewConfigWithDifferentMinReturnsNewConfig) {
  ...
  ConfigMatches(bitrate_config, Match(11, -1, 30));
}
...
TEST_F(RtpBitrateConfiguratorTest,
  ...
  ConfigMatches(bitrate_config, Match(10, 21, 30));
}
...
TEST_F(RtpBitrateConfiguratorTest, NewConfigWithDifferentMaxReturnsNewConfig) {
  ...
  ConfigMatches(bitrate_config, Match(10, -1, 31));
}
...
TEST_F(RtpBitrateConfiguratorTest, NewConfigWithSameConfigElidesSecondCall) {
  ...
  ConfigMatches(bitrate_config, Match(1, 2, 3));
  EXPECT_FALSE(configurator_->UpdateBitrateConfig(bitrate_config).has_value());
}
...
TEST_F(RtpBitrateConfiguratorTest, BiggerMaskMinUsed) {
  ...
  MaskMatches(mask, Match(*mask.min_bitrate_bps, nullopt, nullopt));
}

TEST_F(RtpBitrateConfiguratorTest, BiggerConfigMinUsed) {
  BitrateConfigMask mask;
  mask.min_bitrate_bps = 1000;
  MaskMatches(mask, Match(1000, nullopt, nullopt));

  BitrateConfig config;
  config.min_bitrate_bps = 1234;
  ConfigMatches(config, Match(1234, nullopt, nullopt));
}
...
TEST_F(RtpBitrateConfiguratorTest, LatestStartMaskPreferred) {
  ...
  MaskMatches(mask, Match(nullopt, *mask.start_bitrate_bps, nullopt));

  BitrateConfig bitrate_config;
  bitrate_config.start_bitrate_bps = 1200;

  ConfigMatches(bitrate_config,
                Match(nullopt, bitrate_config.start_bitrate_bps, nullopt));
}
...
TEST_F(RtpBitrateConfiguratorTest, SmallerMaskMaxUsed) {
  ...
  MaskMatches(mask, Match(nullopt, nullopt, *mask.max_bitrate_bps));
}
...
TEST_F(RtpBitrateConfiguratorTest, SmallerConfigMaxUsed) {
  ...
  EXPECT_FALSE(configurator_->UpdateBitrateConfigMask(mask).has_value());
}
...
TEST_F(RtpBitrateConfiguratorTest, MaskStartLessThanConfigMinClamped) {
  ...
  MaskMatches(mask, Match(2000, 2000, nullopt));
}
...
TEST_F(RtpBitrateConfiguratorTest, MaskStartGreaterThanConfigMaxClamped) {
  ...
  MaskMatches(mask, Match(nullopt, -1, 1000));
}
...
TEST_F(RtpBitrateConfiguratorTest, MaskMinGreaterThanConfigMaxClamped) {
  ...
  MaskMatches(mask, Match(1000, nullopt, 1000));
}
...
TEST_F(RtpBitrateConfiguratorTest, NewConfigWithNoChangesDoesNotCallNewConfig) {
  ...
  // change any values.
  ConfigMatches(config1, Match(0, 1000, -1));
  EXPECT_FALSE(configurator_->UpdateBitrateConfig(config2).has_value());
}
...
TEST_F(RtpBitrateConfiguratorTest,
  ...
  ConfigMatches(config, Match(nullopt, nullopt, 2000));

  // Reduce effective max to 1000 with the mask.
  BitrateConfigMask mask;
  mask.max_bitrate_bps = 1000;
  MaskMatches(mask, Match(nullopt, nullopt, 1000));

  // This leaves the effective max unchanged, so new config shouldn't be
  // returned again.
  config.max_bitrate_bps = 1000;
  EXPECT_FALSE(configurator_->UpdateBitrateConfig(config).has_value());
}
...
TEST_F(RtpBitrateConfiguratorTest, NewConfigNotReturnedWhenStartMaskRemoved) {
  BitrateConfigMask mask;
  mask.start_bitrate_bps = 1000;
  MaskMatches(mask, Match(0, 1000, -1));

  mask.start_bitrate_bps.reset();
  EXPECT_FALSE(configurator_->UpdateBitrateConfigMask(mask).has_value());
}

// Test that if a new config is returned after BitrateConfigMask applies a
...
TEST_F(RtpBitrateConfiguratorTest, NewConfigAfterBitrateConfigMaskWithStart) {
  ...
  // -1.
  ConfigMatches(config, Match(0, -1, 5000));
}
...
TEST_F(RtpBitrateConfiguratorTest,
  ...
  MaskMatches(mask, Match(1000, -1, 1000));
