...
void ResourceAdaptationProcessor::AddResource(
  ...
  RTC_DCHECK_RUN_ON(&sequence_checker_);
  // TODO(hbos): Allow adding resources while |is_running_| by registering as a
  // listener of the resource on adding it.
  RTC_DCHECK(!is_running_);
...
void ResourceAdaptationProcessor::RemoveResource(
    rtc::scoped_refptr<Resource> resource) {
  RTC_DCHECK_RUN_ON(&sequence_checker_);
  // TODO(hbos): Allow removing resources while |is_running_| by unregistering
  // as a listener of the resource on removing it.
  RTC_DCHECK(!is_running_);
