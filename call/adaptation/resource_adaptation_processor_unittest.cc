...
class ResourceAdaptationProcessorListenerForTesting
 ...
 public:
  ...
  void OnVideoSourceRestrictionsUpdated(
    ...
    printf(
        "{max_pixels_per_frame: %lu\n"
        " target_pixels_per_frame: %lu\n"
        " max_frame_rate: %f}\n",
        restrictions.max_pixels_per_frame().value_or(13371337),
        restrictions.target_pixels_per_frame().value_or(13371337),
        restrictions.max_frame_rate().value_or(1337.1337));
...
TEST_F(ResourceAdaptationProcessorTest, InsufficientInput) {
  ...
  frame_rate_provider_.set_fps(kDefaultFrameRate);
  resource_.set_usage_state(ResourceUsageState::kOveruse);
  EXPECT_EQ(0u, processor_listener_.restrictions_updated_count());
  // When frame rate is missing.
  // 0 is treated as null by the input state provider.
  SetInputStates(true, 0, kDefaultFrameSize);
  resource_.set_usage_state(ResourceUsageState::kOveruse);
  EXPECT_EQ(0u, processor_listener_.restrictions_updated_count());
