...
NetworkLinkRtcpObserver* RtpTransportControllerSend::GetBandwidthObserver() {
  return this;
}
...
void RtpTransportControllerSend::OnReport(
  ...
  for (const ReportBlockData& report_block : report_blocks) {
    ...
    if (it != last_report_blocks_.end()) {
      int number_of_packets = report_block.extended_highest_sequence_number() -
                              it->second.extended_highest_sequence_number();
      total_packets_delta += number_of_packets;
      int lost_delta =
          report_block.cumulative_lost() - it->second.cumulative_lost();
