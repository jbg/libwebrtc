...
void RtpTransportControllerSend::StartProcessPeriodicTasks() {
  if (!pacer_queue_update_task_) {
    pacer_queue_update_task_ = RepeatingTask::Start(
        &task_queue_, kPacerQueueUpdateInterval,
        RepeatingTaskIntervalMode::kExcludingExecution, [this]() {
          RTC_DCHECK_RUN_ON(&task_queue_);
  ...
  if (controller_task_) {
    ...
    RepeatingTask::Stop(std::move(controller_task_));
  }
  if (process_interval_.IsFinite()) {
    ...
    // first.
    controller_task_ = RepeatingTask::Start(
