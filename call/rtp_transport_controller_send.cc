...
RtpTransportControllerSend::RtpTransportControllerSend(
  ...
  GetPacer()->SetPacingRates(DataRate::bps(bitrate_config.start_bitrate_bps),
                             DataRate::Zero());
...
RtpPacketPacer* RtpTransportControllerSend::GetPacer() {
  // TODO(bugs.webrtc.org/10809): Return reference to the correct
  // pacer implementation.
  return &pacer_;
}

const RtpPacketPacer* RtpTransportControllerSend::GetPacer() const {
  // TODO(bugs.webrtc.org/10809): Return reference to the correct
  // pacer implementation.
  return &pacer_;
}
...
void RtpTransportControllerSend::SetQueueTimeLimit(int limit_ms) {
  GetPacer()->SetQueueTimeLimit(TimeDelta::ms(limit_ms));
}
...
void RtpTransportControllerSend::OnNetworkRouteChanged(
  ...
  if (kv->second.connected != network_route.connected ||
    ...
    task_queue_.PostTask([this, msg] {
      ...
      GetPacer()->UpdateOutstandingData(DataSize::Zero());
    });
  }
}
...
int64_t RtpTransportControllerSend::GetPacerQueuingDelayMs() const {
  return GetPacer()->OldestPacketWaitTime().ms();
}
absl::optional<Timestamp> RtpTransportControllerSend::GetFirstPacketTime()
    const {
  return GetPacer()->FirstSentPacketTime();
}
...
void RtpTransportControllerSend::OnSentPacket(
  ...
  GetPacer()->UpdateOutstandingData(
...
void RtpTransportControllerSend::AccountForAudioPacketsInPacedSender(
  ...
  GetPacer()->SetAccountForAudioPackets(account_for_audio);
}
  ...
  if (!pacer_queue_update_task_.Running()) {
    pacer_queue_update_task_ = RepeatingTaskHandle::DelayedStart(
        task_queue_.Get(), kPacerQueueUpdateInterval, [this]() {
          RTC_DCHECK_RUN_ON(&task_queue_);
          TimeDelta expected_queue_time = GetPacer()->ExpectedQueueTime();
...
void RtpTransportControllerSend::UpdateControllerWithTimeInterval() {
  ...
  if (add_pacing_to_cwin_)
    msg.pacer_queue = GetPacer()->QueueSizeData();
...
void RtpTransportControllerSend::PostUpdates(NetworkControlUpdate update) {
  if (update.congestion_window) {
    if (update.congestion_window->IsFinite()) {
      GetPacer()->SetCongestionWindow(*update.congestion_window);
    } else {
      GetPacer()->SetCongestionWindow(absl::nullopt);
    }
  }
  if (update.pacer_config) {
    GetPacer()->SetPacingRates(update.pacer_config->data_rate(),
                               update.pacer_config->pad_rate());
  }
  for (const auto& probe : update.probe_cluster_configs) {
    GetPacer()->CreateProbeCluster(probe.target_data_rate, probe.id);
  }
