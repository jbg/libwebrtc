...
RtpTransportControllerSend::RtpTransportControllerSend(
    ...
    : c_(config.context),
      ...
      last_report_block_time_(c_.clock().CurrentTime()),
      reset_feedback_on_route_change_(
          !c_.experiments().IsEnabled("WebRTC-Bwe-NoFeedbackReset")),
      add_pacing_to_cwin_(c_.experiments().IsEnabled(
      ...
      is_congested_(false),
      retransmission_rate_limiter_(&c_.clock(), kRetransmitWindowSizeMs) {
  ParseFieldTrial(
      {&relay_bandwidth_cap_},
      c_.experiments().Lookup("WebRTC-Bwe-NetworkRouteConstraints"));
  initial_config_.constraints =
      ConvertConstraints(config.bitrate_config, c_.clock());
  initial_config_.event_log = &c_.event_log();
  initial_config_.key_value_config = &c_.experiments();
...
RtpVideoSenderInterface* RtpTransportControllerSend::CreateRtpVideoSender(
  ...
  video_rtp_senders_.push_back(std::make_unique<RtpVideoSender>(
      &c_.clock(), suspended_ssrcs, states, rtp_config, rtcp_report_interval_ms,
      ...
      c_.experiments(), &c_.task_queue_factory()));
...
void RtpTransportControllerSend::OnNetworkRouteChanged(
  ...
  if (IsRelevantRouteChange(old_route, network_route)) {
    ...
    c_.event_log().Log(std::make_unique<RtcEventRouteChange>(
        network_route.connected, network_route.packet_overhead));
    NetworkRouteChange msg;
    msg.at_time = Timestamp::Millis(c_.clock().TimeInMilliseconds());
    msg.constraints = ConvertConstraints(bitrate_config, c_.clock());
    transport_overhead_bytes_per_packet_ = network_route.packet_overhead;
...
void RtpTransportControllerSend::UpdateBitrateConstraints(
  ...
  RTC_DCHECK_RUN_ON(&sequence_checker_);
  TargetRateConstraints msg = ConvertConstraints(updated, c_.clock());
  if (controller_) {
...
void RtpTransportControllerSend::OnAddPacket(
  ...
  RTC_DCHECK_RUN_ON(&sequence_checker_);
  Timestamp creation_time = Timestamp::Millis(c_.clock().TimeInMilliseconds());
...
void RtpTransportControllerSend::OnRemoteNetworkEstimate(
  ...
  RTC_DCHECK_RUN_ON(&sequence_checker_);
  c_.event_log().Log(std::make_unique<RtcEventRemoteEstimate>(
      estimate.link_capacity_lower, estimate.link_capacity_upper));
  estimate.update_time = Timestamp::Millis(c_.clock().TimeInMilliseconds());
  if (controller_)
...
void RtpTransportControllerSend::MaybeCreateControllers() {
  ...
  initial_config_.constraints.at_time =
      Timestamp::Millis(c_.clock().TimeInMilliseconds());
...
void RtpTransportControllerSend::UpdateStreamsConfig() {
  streams_config_.at_time = Timestamp::Millis(c_.clock().TimeInMilliseconds());
  if (controller_)
