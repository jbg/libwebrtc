...
uint32_t RtpVideoSender::GetPostEncodeOverheadBps() const {
  uint32_t post_encode_overhead_bps = 0;
  for (size_t i = 0; i < rtp_streams_.size(); ++i) {
    if (rtp_streams_[i].rtp_rtcp->SendingMedia()) {
      post_encode_overhead_bps +=
          rtp_streams_[i].sender_video->PostEncodeOverheadBps();
    }
  }
  return post_encode_overhead_bps;
}
...
void RtpVideoSender::OnBitrateUpdated(BitrateAllocationUpdate update,
  ...
  uint32_t post_encode_overhead_bps =
      std::min(GetPostEncodeOverheadBps(), encoder_target_rate_bps_ / 2);
