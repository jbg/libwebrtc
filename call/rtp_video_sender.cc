  ...
  if (transformer != nullptr) {
    video_transformer =
        rtc::make_ref_counted<RTPSenderVideoFrameTransformerDelegate>(
            rtp_sender_video.get(), std::move(transformer), ssrc, tq_factory);
    video_transformer->Init();
    sender = video_transformer.get();
  } else {
...
RtpStreamSender::~RtpStreamSender() {
  if (video_transformer != nullptr) {
    video_transformer->Reset();
  }
}
