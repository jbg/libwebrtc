...
std::map<uint32_t, RtpState> RtpVideoSender::GetRtpStates() const {
  ...
  for (size_t i = 0; i < rtp_config_.ssrcs.size(); ++i) {
    ...
    // Happens during shutdown, when RTP module is already inactive,
  ...
  *sent_fec_rate_bps = 0;
  for (const RtpStreamSender& stream : rtp_streams_) {
    if (use_deferred_fec_) {
      ...
      auto send_bitrate = stream.rtp_rtcp->GetBitrateSent();
      *sent_video_rate_bps +=
          send_bitrate.find(RtpPacketMediaType::kVideo)->second.bps();
      *sent_fec_rate_bps +=
          send_bitrate.find(RtpPacketMediaType::kForwardErrorCorrection)
              ->second.bps();
      *sent_nack_rate_bps +=
          send_bitrate.find(RtpPacketMediaType::kRetransmission)->second.bps();
    } else {
