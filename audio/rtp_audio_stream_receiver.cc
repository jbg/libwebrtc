...
void RtpAudioStreamReceiver::SetNACKStatus(bool enable,
...
void RtpAudioStreamReceiver::OnRtpPacket(const RtpPacketReceived& packet) {
  ...
  receive_statistics_->IncomingPacket(header, packet.size());
...
RtpAudioStreamReceiver::Stats RtpAudioStreamReceiver::GetRtpStatistics() const {
  ...
  if (rtp_rtcp_->DataCountersRTP(&bytesSent, &packetsSent) != 0) {
    RTC_DLOG(LS_WARNING)
...
int64_t RtpAudioStreamReceiver::GetRTT(bool allow_associate_channel) const {
