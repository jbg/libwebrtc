...
int64_t ChannelSend::GetRTT() const {
  uint32_t local_ssrc = _rtpRtcpModule->SSRC();
  for (const ReportBlockData& report_block_data :
       _rtpRtcpModule->GetLatestReportBlockData()) {
    // We don't know in advance the remote ssrc used by the other end's receiver
    // reports, so use the first report block for our local_ssrc to get rtt.
    if (report_block_data.report_block().source_ssrc == local_ssrc) {
      return report_block_data.last_rtt_ms();
    }
  }
  return 0;
}
