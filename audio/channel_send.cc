...
class TransportFeedbackProxy : public TransportFeedbackObserver {
 public:
  ...
  void SetTransportFeedbackObserver(
    ...
    RTC_DCHECK(feedback_observer_ == nullptr ||
  ...
  void OnTransportFeedback(const rtcp::TransportFeedback& feedback) override {
    RTC_DCHECK_RUN_ON(&thread_checker_);
    RTC_DCHECK(feedback_observer_);
    // It is safe to access |feedback_observer_| here because when
    // SetTransportFeedbackObserver() is called, the Rtp module associated with
    // |this| is not registered with PacketRouter.
