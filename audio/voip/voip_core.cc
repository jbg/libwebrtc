...
bool VoipCore::EnsureInitialized() {
  // |audio_device_module_| internally owns a lock and current set of APIs
  // invoked here are ensured that there is no deadlock here. Further changes in
  // this method will need to make sure that no such risk is introduced in the
  // future.
...
bool VoipCore::UpdateAudioTransportWithSenders() {
  ...
  if (!audio_senders.empty()) {
    // Ensure audio device module is initialized.
