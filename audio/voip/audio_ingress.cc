...
void AudioIngress::ReceivedRTPPacket(const uint8_t* data, size_t length) {
  ...
  // If sampling rate info is not available in our received codec set, it would
  // mean that remote media endpoint is sending incorrect payload id which can't
  // be processed correctly especially on payload type id in dynamic range.
  if (sampling_rate == -1) {
    return;
  }
