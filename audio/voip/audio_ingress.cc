...
AudioCodingModule::Config GetAcmConfig(
...
AudioIngress::AudioIngress(
    ...
    : playing_(false),
      ...
      rtp_rtcp_(rtp_rtcp),
      acm_receiver_(GetAcmConfig(decoder_factory)),
...
void AudioIngress::SetReceiveCodecs(
  ...
  {
    rtc::CritScope lock(&payload_type_lock_);
...
void AudioIngress::ReceivedRTPPacket(const uint8_t* data, size_t length) {
  ...
  {
    rtc::CritScope lock(&payload_type_lock_);
    const auto& it = payload_type_sampling_rate_.find(rtp_packet.PayloadType());
...
bool AudioIngress::Playing() const {
  RTC_DCHECK(worker_thread_checker_.IsCurrent() ||
             network_thread_checker_.IsCurrent());
  return playing_;
}
