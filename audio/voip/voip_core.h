...
class VoipCore : public VoipEngine,
 ...
 private:
  ...
  // Returns false only when ADM fails to initialize which would presumably
  // render further processing useless. From Android N and onwards, the mobile
  // app may not be able to gain microphone access when in background mode.
  // Therefore it would be better to delay the logic as late as possible.
  ...
  // boolean flag to initialize once.
