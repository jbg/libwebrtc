...
//
// This class assumes single thread access on all APIs. Optionally
// caller can choose to use different thread on ReceivedRTPPacket and
// ReceivedRTCPPacket APIs only as network is often operated by a separate
// worker thread in application level. However, once initially a seperate thread
// is used to inject RTP/RTCP packet through these APIs, caller cannot change
// the thread thereafter. This restriction is enforced by SequenceChecker which
// checks the variable annotated by RTC_GUARDED_BY macro at compile time and
// actual thread id checking through run time in debug mode.
//
