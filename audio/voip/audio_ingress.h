...
class AudioIngress : public AudioMixer::Source {
 ...
 private:
  ...
  class NtpEstimator {
   public:
    explicit NtpEstimator(Clock* clock) : ntp_estimator_(clock) {}
  ...
  class ReceiveCodecInfo {
   public:
    ...
    // Returns -1 if codec has not yet been assigned.
  ...
  // Wrapper for thread safety on TimestampWrapAroundHandler.
  class TimestampWrapHandler {
   public:
    int64_t Unwrap(uint32_t timestamp) {
      rtc::CritScope lock(&lock_);
      return wraparound_handler_.Unwrap(timestamp);
    }

   private:
    rtc::CriticalSection lock_;
    rtc::TimestampWrapAroundHandler wraparound_handler_ RTC_GUARDED_BY(lock_);
  };
  ...
  TimestampWrapHandler timestamp_wrap_handler_;
};
