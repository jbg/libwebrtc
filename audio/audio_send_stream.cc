...
AudioSendStream::AudioSendStream(
  ...
  if (config.media_transport) {
    // TODO(sukhanov): Currently media transport audio overhead is constant and
    // we will not get overhead_observer calls when using media_transport.
    // However when we implement RTP Media Transport we may have to change media
    // transport interface to implement observer callback.
