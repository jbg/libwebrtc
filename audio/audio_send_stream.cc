...
uint32_t AudioSendStream::OnBitrateUpdated(BitrateAllocationUpdate update) {
  ...
  DataRate min_overhead = total_overhead_per_packet_ / max_frame_length;
  DataRate max_overhead = total_overhead_per_packet_ / min_frame_length;
  DataRate min_bitrate = DataRate::bps(config_.min_bitrate_bps) + min_overhead;
  DataRate max_bitrate = DataRate::bps(config_.max_bitrate_bps) + max_overhead;
  ...
  if (update.target_bitrate.IsZero()) {
    update.target_bitrate = min_bitrate;
  }
  RTC_DCHECK_GE(update.target_bitrate.bps(), min_bitrate.bps());
  // The bitrate allocator might allocate an higher than max configured bitrate
  // if there is room, to allow for, as example, extra FEC. Ignore that for now.
  update.target_bitrate.Clamp(min_bitrate, max_bitrate);
...
void AudioSendStream::ConfigureBitrateObserver(int min_bitrate_bps,
  ...
  worker_queue_->PostTask([&] {
    ...
    {
      rtc::CritScope cs(&overhead_per_packet_lock_);
      packet_overhead = DataSize::bytes(GetPerPacketOverheadBytes());
    }
    // TODO(srte): Get actual frame rate ranges from encoder.
    DataRate min_overhead =
        packet_overhead / allocation_settings_.MaxFrameLength();
    DataRate max_overhead =
        packet_overhead / allocation_settings_.MinFrameLength();
    //    auto max_overhead_bps =
    //    allocation_settings_.MaxOverheadBitrate().bps();
    // This either updates the current observer or adds a new observer.
    bitrate_allocator_->AddObserver(
        this, MediaStreamAllocationConfig{
                  static_cast<uint32_t>(min_bitrate_bps + min_overhead.bps()),
                  static_cast<uint32_t>(max_bitrate_bps + max_overhead.bps()),
                  0, true, config_.track_id, bitrate_priority});
