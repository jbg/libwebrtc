...
uint32_t AudioSendStream::OnBitrateUpdated(BitrateAllocationUpdate update) {
  DataRate min_bitrate = DataRate::bps(config_.min_bitrate_bps) +
                         allocation_settings_.MinOverheadBitrate();
  DataRate max_bitrate = DataRate::bps(config_.max_bitrate_bps) +
                         allocation_settings_.MaxOverheadBitrate();
...
void AudioSendStream::ConfigureBitrateObserver(int min_bitrate_bps,
  ...
  worker_queue_->PostTask([&] {
    ...
    auto min_overhead_bps = allocation_settings_.MinOverheadBitrate().bps();
    auto max_overhead_bps = allocation_settings_.MaxOverheadBitrate().bps();
    // This either updates the current observer or adds a new observer.
    bitrate_allocator_->AddObserver(
        this, MediaStreamAllocationConfig{
                  static_cast<uint32_t>(min_bitrate_bps + min_overhead_bps),
                  static_cast<uint32_t>(max_bitrate_bps + max_overhead_bps), 0,
