...
class ChannelReceive::ProcessAndEncodeAudioTask : public rtc::QueuedTask {
 public:
 ...
 private:
  ...
  ChannelReceive* const channel_;
};

int32_t ChannelReceive::SendData(FrameType frameType,
                                 uint8_t payloadType,
                                 uint32_t timeStamp,
  ...
  if (!_rtpRtcpModule->SendOutgoingData(
    ...
    RTC_DLOG(LS_ERROR)
        << "ChannelReceive::SendData() failed to send data to RTP/RTCP module";
    return -1;
  }
...
bool ChannelReceive::SendRtp(const uint8_t* data,
  ...
  if (_transportPtr == NULL) {
    RTC_DLOG(LS_ERROR)
        << "ChannelReceive::SendPacket() failed to send RTP packet due to"
  ...
  if (!_transportPtr->SendRtp(data, len, options)) {
    RTC_DLOG(LS_ERROR)
        << "ChannelReceive::SendPacket() RTP transmission failed";
    return false;
  }
  return true;
}
...
ChannelReceive::ChannelReceive(rtc::TaskQueue* encoder_queue,
                               ProcessThread* module_process_thread,
                               AudioDeviceModule* audio_device_module,
                               RtcpRttStats* rtcp_rtt_stats,
                               RtcEventLog* rtc_event_log)
    : ChannelReceive(module_process_thread,
                     audio_device_module,
                     rtcp_rtt_stats,
                     rtc_event_log,
                     0,
...
int32_t ChannelReceive::StartSend() {
...
bool ChannelReceive::SetEncoder(int payload_type,
...
void ChannelReceive::ModifyEncoder(
...
void ChannelReceive::SetBitRate(int bitrate_bps, int64_t probing_interval_ms) {
...
void ChannelReceive::OnTwccBasedUplinkPacketLossRate(float packet_loss_rate) {
...
void ChannelReceive::OnRecoverableUplinkPacketLossRate(
...
void ChannelReceive::OnUplinkPacketLossRate(float packet_loss_rate) {
  if (use_twcc_plr_for_ana_)
    return;
  audio_coding_->ModifyEncoder([&](std::unique_ptr<AudioEncoder>* encoder) {
    if (*encoder) {
...
bool ChannelReceive::EnableAudioNetworkAdaptor(
...
void ChannelReceive::DisableAudioNetworkAdaptor() {
  audio_coding_->ModifyEncoder([&](std::unique_ptr<AudioEncoder>* encoder) {
    if (*encoder)
...
void ChannelReceive::SetReceiverFrameLengthRange(int min_frame_length_ms,
...
void ChannelReceive::SetInputMute(bool enable) {
  rtc::CritScope cs(&volume_settings_critsect_);
  input_mute_ = enable;
}

bool ChannelReceive::InputMute() const {
  rtc::CritScope cs(&volume_settings_critsect_);
  return input_mute_;
}
...
int ChannelReceive::SendTelephoneEventOutband(int event, int duration_ms) {
...
int ChannelReceive::SetSendTelephoneEventPayloadType(int payload_type,
...
void ChannelReceive::SetMid(const std::string& mid, int extension_id) {
...
int ChannelReceive::SetSendAudioLevelIndicationStatus(bool enable,
                                                      unsigned char id) {
  _includeAudioLevelIndication = enable;
  return SetSendRtpHeaderExtension(enable, kRtpExtensionAudioLevel, id);
}

void ChannelReceive::EnableSendTransportSequenceNumber(int id) {
  int ret =
      SetSendRtpHeaderExtension(true, kRtpExtensionTransportSequenceNumber, id);
  RTC_DCHECK_EQ(0, ret);
}

void ChannelReceive::RegisterSenderCongestionControlObjects(
...
void ChannelReceive::ResetSenderCongestionControlObjects() {
  RTC_DCHECK(packet_router_);
...
void ChannelReceive::SetRTCPStatus(bool enable) {
  _rtpRtcpModule->SetRTCPStatus(enable ? RtcpMode::kCompound : RtcpMode::kOff);
}

int ChannelReceive::SetRTCP_CNAME(const char cName[256]) {
...
void ChannelReceive::ProcessAndEncodeAudio(
...
void ChannelReceive::ProcessAndEncodeAudioOnTaskQueue(AudioFrame* audio_input) {
  RTC_DCHECK_RUN_ON(encoder_queue_);
...
void ChannelReceive::UpdateOverheadForEncoder() {
...
void ChannelReceive::SetTransportOverhead(
...
void ChannelReceive::OnOverheadChanged(size_t overhead_bytes_per_packet) {
  rtc::CritScope cs(&overhead_per_packet_lock_);
...
ANAStats ChannelReceive::GetANAStatistics() const {
...
RtpRtcp* ChannelReceive::GetRtpRtcp() const {
...
int ChannelReceive::SetSendRtpHeaderExtension(bool enable,
...
int64_t ChannelReceive::GetRTT(bool allow_associate_channel) const {
