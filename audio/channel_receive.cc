  ...
  header.extension.absolute_capture_time =
      absolute_capture_time_receiver_.OnReceivePacket(
          AbsoluteCaptureTimeReceiver::GetSource(header.ssrc,
                                                 header.arrOfCSRCs),
          header.timestamp,
          // Note that GetRtpTimestampRateHz may produce incorrect results, as
          // the sample rate may not always equals RTP clock rate. For example,
          // the sample rate of G722 encoder is different from its RTP clock
          // rate. GetRtpTimestampRateHz must be fixed, otherwise
          // sender/receiver interpolation might be not in sync, and we will
          // suffer from interpolation errors. As Opus 48000 Hz is used, there
