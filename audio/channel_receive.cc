...
class ChannelReceive : public ChannelReceiveInterface,
 ...
 private:
  ...
  // TODO(nisse): Need lock? Or use on worker thread only?
...
bool ChannelReceive::GetRecCodec(CodecInst* codec) const {
  RTC_DCHECK(worker_thread_checker_.CalledOnValidThread());
  return (acm_receiver_.LastAudioCodec(codec) == 0);
}
...
void ChannelReceive::ReceivePacket(const uint8_t* packet,
  ...
  if (payload_data_length == 0) {
    ...
    OnReceivedPayloadData(rtc::ArrayView<const uint8_t>(), &webrtc_rtp_header);
    return;
  }
...
int ChannelReceive::GetRtpTimestampRateHz() const {
  const auto format = acm_receiver_.LastAudioFormat();
