...
namespace internal {
namespace {
std::unique_ptr<voe::ChannelReceive> CreateChannelReceive(
  ...
  return absl::make_unique<voe::ChannelReceive>(
...
AudioReceiveStream::AudioReceiveStream(
    ...
    const rtc::scoped_refptr<webrtc::AudioState>& audio_state,
    webrtc::RtcEventLog* event_log,
    std::unique_ptr<voe::ChannelReceive> channel_receive)
...
void AudioReceiveStream::ConfigureStream(AudioReceiveStream* stream,
  ...
  RTC_LOG(LS_INFO) << "AudioReceiveStream::ConfigureStream: "
                   << new_config.ToString();
  RTC_DCHECK(stream);
  const auto& channel_proxy = stream->channel_receive_;
