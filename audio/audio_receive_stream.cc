...
webrtc::AudioReceiveStream::Stats AudioReceiveStream::GetStats() const {
  ...
  if (!audio_coding_->ReceiveCodec(&codec_inst)) {
    return stats;
  }
...
#if 0
  // TODO(nisse): XXX From AudioLevel.
...
// own mixing/dynamic processing.
  ...
  output_audio_level.ComputeLevel(*audio_frame, kAudioSampleDurationSeconds);
...
#else
  // TODO(nisse): XXX playout_timestamp_rtp_. Related to
  // AudioDeviceModule::PlayoutDelay, but scaled to rtp time.
  return 0;
#endif
}
...
void AudioReceiveStream::ConfigureStream(AudioReceiveStream* stream,
  ...
  if (first_time || old_config.rtp.nack.rtp_history_ms !=
    ...
    bool enable = (new_config.rtp.nack.rtp_history_ms != 0);
    int max_number_of_packets = new_config.rtp.nack.rtp_history_ms / 20;
    stream->rtp_audio_stream_receiver_->SetNACKStatus(enable,
