...
TEST_P(WebRtcVoiceEngineTestFake, InitRecordingOnSend) {
  EXPECT_CALL(*adm_, RecordingIsInitialized()).WillRepeatedly(Return(false));
  EXPECT_CALL(*adm_, Recording()).WillRepeatedly(Return(false));
  EXPECT_CALL(*adm_, InitRecording()).Times(1);

  cricket::VoiceMediaChannel* channel = engine_->CreateMediaChannel(
...
TEST_P(WebRtcVoiceEngineTestFake, SkipInitRecordingOnSend) {
  ...
  cricket::VoiceMediaChannel* channel = engine_->CreateMediaChannel(
      &call_, cricket::MediaConfig(), options, webrtc::CryptoOptions());
