...
using ::webrtc::ConnectionEnvironmentBuilder;
...
TEST(WebRtcVoiceEngineTest, StartupShutdown) {
  ...
  for (bool use_null_apm : {false, true}) {
    ...
    engine.Init();
    CallConfig call_config(ConnectionEnvironmentBuilder()
                               .With(&field_trials)
                               .With(task_queue_factory.get())
                               .Build());
    auto call = Call::Create(call_config);
    std::unique_ptr<cricket::VoiceMediaSendChannelInterface> send_channel =
  ...
  for (bool use_null_apm : {false, true}) {
        ...
        ::testing::NiceMock<webrtc::test::MockAudioDeviceModule>>();
    {
      ...
      cricket::WebRtcVoiceEngine engine(
          ...
          nullptr, nullptr, field_trials);
      engine.Init();
      auto call = Call::Create(CallConfig(ConnectionEnvironmentBuilder()
                                              .With(&field_trials)
                                              .With(task_queue_factory.get())
                                              .Build()));
      std::unique_ptr<cricket::VoiceMediaSendChannelInterface> send_channel =
