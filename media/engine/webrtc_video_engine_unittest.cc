...
class WebRtcVideoChannelTest : public WebRtcVideoEngineTest {
 ...
 protected:
  ...
  FakeVideoSendStream* AddSendStream(const StreamParams& sp) {
    ...
    if (num_streams == 0 && receive_channel_) {
      // First channel.
      // Emulate the old behavior of setting the SSRC of the receive
      // channel when doing AddSendStream.
      if (receive_channel_) {
        receive_channel_->SetLocalSsrc(sp);
      }
    }
