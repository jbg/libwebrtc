...
TEST_F(WebRtcVideoChannelFlexfecRecvTest, SetDefaultRecvCodecsWithoutSsrc) {
  ...
  EXPECT_EQ(video_config.rtp.rtp_packet_sink_, nullptr);
}
...
TEST_F(WebRtcVideoChannelFlexfecRecvTest, SetDefaultRecvCodecsWithSsrc) {
  ...
  EXPECT_NE(video_config.rtp.rtp_packet_sink_, nullptr);
}
...
TEST_F(WebRtcVideoChannelFlexfecRecvTest,
  ...
  const std::vector<FakeVideoReceiveStream*>& video_streams =
      fake_call_->GetVideoReceiveStreams();
  ASSERT_EQ(1U, video_streams.size());
  // TODO(tommi): Here, we probably should expect rtp_packet_sink_ to be
  ...
  // TODO(tommi): Here we now get 3 receive streams.
  // (rtc_media_unittests)
  //EXPECT_EQ(2, fake_call_->GetNumCreatedReceiveStreams())
  ...
  // TODO(tommi): Here either rtp_packet_sink_ remains the same, or
