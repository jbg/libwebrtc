...
SimulcastEncoderAdapter::SimulcastEncoderAdapter(VideoEncoderFactory* factory,
    ...
    : inited_(0),
      ...
      whitelisted_rate_controller_(false) {
  RTC_DCHECK(factory_);
...
int SimulcastEncoderAdapter::InitEncode(const VideoCodec* inst,
  ...
  whitelisted_rate_controller_ = true;
  ...
  for (int i = 0; i < number_of_streams; ++i) {
    ...
    whitelisted_rate_controller_ &= encoder->HasWhitelistedRateController();
...
bool SimulcastEncoderAdapter::HasWhitelistedRateController() const {
  RTC_DCHECK_CALLED_SEQUENTIALLY(&encoder_queue_);
  return whitelisted_rate_controller_;
}
