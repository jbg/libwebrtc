...
RtpCapabilities WebRtcVoiceEngine::GetCapabilities() const {
  ...
  // We always support transport sequence numbers.
 ...
 private:
  ...
  void UpdateAllowedBitrateRange() {
    ...
    if (allocation_settings_.IncludeAudioInAllocation()) {
      RTC_DCHECK(is_opus);
      config_.min_bitrate_bps = allocation_settings_.DefaultMinBitrate().bps();
      config_.max_bitrate_bps =
          rtp_parameters_.encodings[0].max_bitrate_bps.value_or(
