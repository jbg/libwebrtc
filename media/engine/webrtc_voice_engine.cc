...
void WebRtcVoiceEngine::AddCapabilities(RtpCapabilities* capabilities) const {
  RTC_DCHECK(signal_thread_checker_.CalledOnValidThread());
  RTC_DCHECK(capabilities);
  bool success =
      capabilities->AddRtpExtension(webrtc::RtpExtension::kAudioLevelUri);
  if (allocation_settings_.EnableTransportSequenceNumberExtension()) {
    success &= capabilities->AddRtpExtension(
