...
void WebRtcVoiceEngine::AppendCapabilities(
    RtpCapabilities* capabilities) const {
  RTC_DCHECK(signal_thread_checker_.CalledOnValidThread());
  RTC_DCHECK(capabilities);
  capabilities->AppendRtpExtension(webrtc::RtpExtension::kAudioLevelUri);
  if (allocation_settings_.EnableTransportSequenceNumberExtension()) {
    capabilities->AppendRtpExtension(
