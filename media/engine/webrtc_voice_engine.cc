...
// Set and adjust gain control options.
  ...
  if (!ap) {
    RTC_LOG(LS_INFO) << "No audio processing module present. No effects (AEC, "
                        "NS, AGC, ...) are activated";
    return true;
  }
...
bool WebRtcVoiceEngine::StartAecDump(webrtc::FileWrapper file,
  ...
  if (!ap) {
    RTC_LOG(LS_WARNING)
        << "Attempting to get start aecdump when no audio processing module is "
...
void WebRtcVoiceEngine::StopAecDump() {
  ...
  if (ap) {
    ...
    RTC_LOG(LS_WARNING) << "Attempting to get stop aecdump when no audio "
