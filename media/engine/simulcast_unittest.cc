...
TEST(SimulcastTest, GetConfigForScreenshare) {
  const std::vector<VideoStream> kDefault = GetScreenshareBitrates();
...
TEST(SimulcastTest, GetConfigForScreenshareSimulcast) {
  ...
  const std::vector<VideoStream> kDefault = GetScreenshareSimulcastBitrates();

  const size_t kMaxLayers = kDefault.size();
  std::vector<VideoStream> streams = cricket::GetSimulcastConfig(
      kMaxLayers, 1280, 720, kMaxBitrateBps, kBitratePriority, kQpMax, kMaxFps,
      kScreenshare);
  ...
  EXPECT_EQ(kMaxLayers, streams.size());
  for (size_t i = 0; i < streams.size(); ++i) {
    EXPECT_EQ(1280u, streams[i].width) << "Screen content never scaled.";
    EXPECT_EQ(720u, streams[i].height) << "Screen content never scaled.";
    EXPECT_EQ(kQpMax, streams[i].max_qp);
    EXPECT_TRUE(streams[i].active);
    EXPECT_EQ(kDefault[i].min_bitrate_bps, streams[i].min_bitrate_bps);
    EXPECT_EQ(kDefault[i].target_bitrate_bps, streams[i].target_bitrate_bps);
