...
TEST(SimulcastTest, GetSimulcastConfig) {
  ...
  for (const auto& stream : streams) {
    EXPECT_EQ(kDefaultTemporalLayers, stream.num_temporal_layers);
    EXPECT_EQ(kMaxFps, stream.max_framerate);
    EXPECT_EQ(kQpMax, stream.max_qp);
...
TEST(SimulcastTest, GetSimulcastConfigBitrates) {
