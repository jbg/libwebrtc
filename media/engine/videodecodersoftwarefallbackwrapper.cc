...
int32_t VideoDecoderSoftwareFallbackWrapper::InitDecode(
  ...
  const int32_t ret = hw_decoder_->InitDecode(codec_settings, number_of_cores);
  hw_decoder_initialized_ = (ret == WEBRTC_VIDEO_CODEC_OK);
  use_hw_decoder_ = hw_decoder_initialized_;
