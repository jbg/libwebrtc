...
// the codec.
void UpdateScalabilityModeIfNotSupported(
    const VideoCodec& codec,
    const webrtc::VideoSendStream::Config& config,
  ...
  if (!absl::c_any_of(encodings,
                      [](const webrtc::RtpEncodingParameters& encoding) {
                        return encoding.scalability_mode &&
                               !encoding.scalability_mode.value().empty();
                      })) {
...
void WebRtcVideoChannel::WebRtcVideoSendStream::SetCodec(
  ...
  RTC_DCHECK_RUN_ON(&thread_checker_);
  UpdateScalabilityModeIfNotSupported(codec_settings.codec, parameters_.config,
