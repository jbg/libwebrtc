...
void WebRtcVideoChannel::ConfigureReceiverRtp(
  ...
  // rtx-time is a declarative attribute similar to rtcp-rsize and
  // determined by the send codec.
...
void WebRtcVideoChannel::WebRtcVideoSendStream::SetCodec(
  ...
  // TODO: this is not relevant since it is for the send configuration?
...
void WebRtcVideoChannel::WebRtcVideoReceiveStream::SetFeedbackParameters(
  ...
  // TODO:
  // This is called during renegotiation. How would the rtx-time setting be
  // recovered after disabling NACK and re-enabling it?
  // This is somewhat unlikely in practice but one could pass the rtx-time
  // from the changed_send_codecc or send_codec_.
  int nack_history_ms = nack_enabled ? config_.rtp.nack.rtp_history_ms
