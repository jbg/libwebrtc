...
// for new additions the [35, 65] range) to the input codecs, and also add ULPFEC,
// RED, FlexFEC, and associated RTX codecs for recognized codecs (VP8, VP9, H264,
// and RED). It will also add default feedback params to the codecs.
std::vector<VideoCodec> AssignPayloadTypesAndDefaultCodecs(
    std::vector<webrtc::SdpVideoFormat> input_formats,
    const webrtc::WebRtcKeyValueConfig& trials) {
  if (input_formats.empty())
    return std::vector<VideoCodec>();
  // Due to intraoperability issues with old Chrome/WebRTC versions only use
  ...
  for (const webrtc::SdpVideoFormat& format : input_formats) {
    ...
    if (payload_type_upper > kLastDynamicPayloadTypeUpperRange) {
      ...
      // TODO: how can we ensure that we are not adding a codec without its RTX
      // type?
      break;
    }
