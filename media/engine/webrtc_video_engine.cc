...
bool WebRtcVideoChannel::MaybeCreateDefaultReceiveStream(
  ...
  if (is_rtx_payload) {
    ...
    if (current_default_ssrc) {
      ...
      ReCreateDefaulReceivetStream(/*ssrc =*/*current_default_ssrc,
                                   packet.Ssrc());
    } else {
      ...
      ReCreateDefaulReceivetStream(/*ssrc =*/14795, /*rtx_ssrc=*/packet.Ssrc());
    }
    return true;
  } else {
  ...
  // negotiated.
  ReCreateDefaulReceivetStream(packet.Ssrc(), absl::nullopt);
  last_unsignalled_ssrc_creation_time_ms_ = rtc::TimeMillis();
  return true;
}

void WebRtcVideoChannel::ReCreateDefaulReceivetStream(
    uint32_t ssrc,
