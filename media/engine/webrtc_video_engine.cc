...
bool WebRtcVideoChannel::GetStats(VideoMediaInfo* info, bool legacy) {
  RTC_DCHECK_RUN_ON(&thread_checker_);
  ...
  info->Clear();
  FillSenderStats(info, log_stats, legacy);
  ...
  for (std::map<uint32_t, WebRtcVideoSendStream*>::iterator it =
    ...
    if (legacy) {
      video_media_info->senders.push_back(
          it->second->GetVideoSenderInfoLegacy(log_stats));
    } else {
      auto infos = it->second->GetVideoSenderInfos(log_stats);
      for (auto&& info : infos) {
...
WebRtcVideoChannel::WebRtcVideoSendStream::GetVideoSenderInfos(bool log_stats) {
  RTC_DCHECK_RUN_ON(&thread_checker_);
...
VideoSenderInfo
WebRtcVideoChannel::WebRtcVideoSendStream::GetVideoSenderInfoLegacy(
    bool log_stats) {
  VideoSenderInfo info;
  RTC_DCHECK_RUN_ON(&thread_checker_);
  for (uint32_t ssrc : parameters_.config.rtp.ssrcs)
