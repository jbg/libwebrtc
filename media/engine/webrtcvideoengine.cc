...
class EncoderFactoryAdapter {
 public:
  ...
  virtual AllocatedEncoder CreateVideoEncoder(const VideoCodec& codec,
...
std::vector<VideoCodec> AssignPayloadTypesAndAddAssociatedRtxCodecs(
  ...
  for (VideoCodec codec : input_codecs) {
    ...
    if (CodecNamesEq(codec.name, kVp9CodecName) ||
        CodecNamesEq(codec.name, kVp8CodecName) ||
        CodecNamesEq(codec.name, kH264CodecName)) {
      output_codecs.push_back(
...
WebRtcVideoChannel::SelectSendVideoCodec(
  ...
  for (const VideoCodecSettings& remote_mapped_codec : remote_mapped_codecs) {
    // HARDCODE TO ALPHA
    if (!cricket::VideoCodec::IsStereoCodec(remote_mapped_codec.codec)) continue;
    //if (remote_mapped_codec.codec.name.find("VP8") == std::string::npos) continue;
...
WebRtcVideoChannel::SelectStereoAssociatedVideoCodec(
  ...
  for (const VideoCodecSettings& remote_mapped_codec : remote_mapped_codecs) {
    ALOGE("WebRtcVideoChannel::SelectStereoAssociatedVideoCodec %s",remote_mapped_codec.codec.name.c_str());
  }

  // Select the first remote codec that is supported locally.
  for (const VideoCodecSettings& remote_mapped_codec : remote_mapped_codecs) {
    // HARDCODE TO VP8
    if (!CodecNamesEq(remote_mapped_codec.codec.name.c_str(), kVp8CodecName))
      continue;
...
CricketEncoderFactoryAdapter::CreateVideoEncoder(const VideoCodec& codec,
                                                 bool is_stereo_codec) const {
    ALOGE("Qiang Chen WebRTCVideoEngine Create Encoder: %s", codec.name.c_str());
  if (is_stereo_codec) {
    if (external_encoder_factory_ != nullptr &&
        FindMatchingCodec(external_encoder_factory_->supported_codecs(),
                          codec)) {
      ALOGE("Qiang Chen WebRTCVideoEngine Stereo Encoder Created: Hardware %s", codec.ToString().c_str());
      std::unique_ptr<webrtc::VideoEncoder> stereo_encoder(
          new webrtc::StereoEncoderAdapter(external_encoder_factory_.get(), internal_encoder_factory_.get()));
    ...
    if (FindMatchingCodec(internal_encoder_factory_->supported_codecs(),
                          codec)) {
      ALOGE("Qiang Chen WebRTCVideoEngine Stereo Encoder Created: Software");
      std::unique_ptr<webrtc::VideoEncoder> stereo_encoder(
  ...
  if (false &&external_encoder_factory_ != nullptr &&
      FindMatchingCodec(external_encoder_factory_->supported_codecs(), codec)) {
    std::unique_ptr<webrtc::VideoEncoder> external_encoder;
    ALOGE("Qiang Chen WebRTCVideoEngine Encoder Created: Hardware %s", codec.ToString().c_str());
  ...
  if (FindMatchingCodec(internal_encoder_factory_->supported_codecs(), codec)) {
    ALOGE("Qiang Chen WebRTCVideoEngine Encoder Created: Software %s", codec.name.c_str());
    if (CodecNamesEq(codec.name.c_str(), kVp8CodecName)) {
...
CricketDecoderFactoryAdapter::CreateVideoDecoder(
    ...
    bool is_stereo_codec) const {
  if (is_stereo_codec) {
    // We can't use an external decoder because we do not have a way of merging
    // two images currently.
