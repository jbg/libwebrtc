...
constexpr int kScreenshareSimulcastLayers = 2;
...
std::vector<webrtc::VideoStream> GetScreenshareLayers(
  ...
  size_t num_simulcast_layers =
      std::min<int>(max_layers, kScreenshareSimulcastLayers);
  ...
  if (num_simulcast_layers == kScreenshareSimulcastLayers) {
    ...
    if (!temporal_layers_supported) {
      ...
      max_bitrate_bps = static_cast<int>(
          ...
          webrtc::SimulcastRateAllocator::GetTemporalRateAllocation(
              kScreenshareSimulcastLayers, 0, base_heavy_tl3_rate_alloc));
    } else {
    ...
    layers[1].num_temporal_layers =
        temporal_layers_supported ? kScreenshareSimulcastLayers : 1;
