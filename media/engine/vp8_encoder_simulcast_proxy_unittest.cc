...
TEST(VP8EncoderSimulcastProxy, ForwardsWhitelistSetting) {
  NiceMock<MockEncoder>* mock_encoder = new NiceMock<MockEncoder>();
  NiceMock<MockVideoEncoderFactory> simulcast_factory;
  ...
  EXPECT_CALL(*mock_encoder, HasWhitelistedRateController())
      .WillRepeatedly(Return(true));

  EXPECT_TRUE(simulcast_enabled_proxy.HasWhitelistedRateController());
}
