...
TEST(CodecTest, TestVideoCodecIntersectPacketization) {
  ...
  EXPECT_EQ(IntersectPacketization(c1, c2), "raw");
  EXPECT_EQ(IntersectPacketization(c1, c3), absl::nullopt);
}
...
TEST(CodecTest, TestSetParamGetParamAndRemoveParam) {
  AudioCodec codec = cricket::CreateAudioCodec(0, "foo", 22222, 20000, 2);
  codec.SetParam("a", "1");
  codec.SetParam("b", "x");
