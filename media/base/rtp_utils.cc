...
bool IsRtpPacket(rtc::ArrayView<const char> packet) {
  ...
  return (static_cast<uint8_t>(packet[0]) >> 6) == kRtpVersion;
}
...
bool IsRtcpPacket(rtc::ArrayView<const char> packet) {
  ...
  if ((static_cast<uint8_t>(packet[0]) >> 6) != kRtpVersion) {
    return false;
  }
...
bool IsValidPacketSize(RtpPacketType packet_type, size_t size) {
  // TODO(webrtc:10418): uncomment this when supported.
