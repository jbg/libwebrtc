...
struct RtpCapabilities {
  RtpCapabilities();
  ~RtpCapabilities();
  bool AddRtpExtension(const std::string& uri);
  bool AddRtpExtension(const std::string& uri, bool encrypt);
...
class VoiceEngineInterface {
 public:
  ...
  virtual void AddCapabilities(RtpCapabilities* capabilities) const = 0;
