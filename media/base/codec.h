...
struct RTC_EXPORT Codec {
  ...
  // This is used in SDP offer/answer negotiation.
  bool Matches(const Codec& codec) const;

  // This is an exact match similar to what is described in
  // https://w3c.github.io/webrtc-pc/#dfn-codec-match
  // with two differences:
  // - rtx which is included in capabilities  without the apt parameter does not
  // check
  //   number of channels, clock rate or the equality of the parameters.
  // - parameters is compared element-wise, not as a string comparison.
  // It should only be used to compare input on our end to something we
