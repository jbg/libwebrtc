...
Codec::Codec(Type type, int id, const std::string& name, int clockrate)
    : Codec(type, id, name, clockrate, 0, 0) {}
...
Codec::Codec(Type type) : Codec(type, 0, 0, 0) {}
...
AudioCodec::AudioCodec(int id,
                       ...
                       int bitrate,
                       size_t channels)
    : Codec(Type::kAudio, id, name, clockrate, bitrate, channels) {}
...
absl::optional<std::string> IntersectPacketization(const Codec& local_codec,
...
AudioCodec CreateAudioCodec(int id,
                            ...
                            int bitrate,
                            size_t channels) {
  return AudioCodec(id, name, clockrate, bitrate, channels);
}
...
VideoCodec CreateVideoCodec(int id, std::string& name) {
