...
// Maps SCTP transport address to SctpTransport object, necessary in send
// threshold callback and send callback.
// TODO(deadbeef): Remove once sctp_get_ulpinfo is available, after a usrsctp
// update.
class SctpTransportMap {
 public:
  static SctpTransportMap* Instance() {
    static SctpTransportMap* instance = new SctpTransportMap();
    return instance;
  }
...
class SctpTransport::UsrSctpWrapper {
 public:
  ...
  static int OnSctpOutboundPacket(void* addr,
                                  ...
                                  uint8_t tos,
                                  uint8_t set_df) {
    SctpTransport* transport = SctpTransportMap::Instance()->Retrieve(
...
bool SctpTransport::OpenSctpSocket() {
  ...
  id_ = SctpTransportMap::Instance()->Register(this);
...
void SctpTransport::CloseSctpSocket() {
  ...
  if (sock_) {
    ...
    RTC_CHECK(SctpTransportMap::Instance()->Deregister(id_));
    UsrSctpWrapper::DecrementUsrSctpUsageCount();
    ready_to_send_data_ = false;
  }
}
