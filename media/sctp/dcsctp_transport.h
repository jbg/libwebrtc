...
class DcSctpTransport : public cricket::SctpTransportInternal,
 ...
 private:
  ...
  // is closed.
  struct ClosingState {
    // True when the peer has initiated the reset.
    // If the peer receives a reset, it needs to fire the signal
    // SignalClosingProcedureStartedRemotely.
    bool closure_initiated = false;
    // True when the peer received OnIncomingStreamsReset
    bool incoming_reset_done = false;
    // True when the peer received OnStreamsResetPerformed
    bool outgoing_reset_done = false;
  };

  flat_map<dcsctp::StreamID, ClosingState> closing_states_;
