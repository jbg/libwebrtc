...
TEST_F(DISABLED_UsrSctpReliabilityTest,
  ...
  constexpr uint32_t messages_count = 100;
  constexpr uint32_t parallel_ping_pongs = 8 * 4096;
  constexpr uint32_t total_ping_pong_tests = 8 * parallel_ping_pongs;
  ...
  for (uint32_t i = 0; i < total_ping_pong_tests; i++) {
    ...
    auto test = std::make_unique<SctpPingPong>(
        ...
        messages_count, 10, send_params);
    test->Start();
    tests.emplace(std::move(test));

    while (tests.size() >= parallel_ping_pongs) {
      auto& oldest_test = tests.front();
      oldest_test->WaitCompletion(std::max<uint32_t>(
          messages_count * tests.size() * parallel_ping_pongs * 100,
          kDefaultTimeout));
      tests.pop();
    }
  }
  ...
  while (!tests.empty()) {
    ...
    oldestTest->WaitCompletion(std::max<uint32_t>(
        messages_count * tests.size() * parallel_ping_pongs, kDefaultTimeout));
    tests.pop();
  }
}
