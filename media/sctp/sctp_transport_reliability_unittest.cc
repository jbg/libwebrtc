...
class LossyPacketTransport final : public rtc::PacketTransportInternal {
 public:
  LossyPacketTransport(std::string name,
  ...
  int SendPacket(const char* data,
    ...
    // Introduce random send delay, which will also work as random packet
    // reordering mechanism.
    auto actual_send_delay = avg_send_delay_millis_;
    auto reorder_delay = static_cast<int16_t>(
        avg_send_delay_millis_ *
        std::min(1.0, std::max(-1.0, random_.Gaussian(0, 0.5))));
    if (reorder_delay > 0) {
      actual_send_delay += static_cast<uint16_t>(reorder_delay);
    } else {
      actual_send_delay -= static_cast<uint16_t>(reorder_delay);
    }
 ...
 * stress conditions: multple sockets, concurrent access,
 * lossy network link. It was observed in the past that
 * usrsctp might misbehave under in concurrent environment
...
TEST_F(DISABLED_UsrSctpReliabilityTest,
  ...
  constexpr uint8_t packet_loss_percents = 3;
  constexpr uint16_t avg_send_delay_millis = 16;
