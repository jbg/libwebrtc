...
class UsrSctpWrapper : public rtc::RefCountInterface {
 public:
  ...
  // RefCountInterface implementation. We can't just use RefCountedObject
  // because Release may need to clear the global instance, which must be done
  // while holding a mutex, to prevent GetOrCreateInstance from handing out an
