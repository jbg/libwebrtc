...
void OutstandingData::RemoveAcked(UnwrappedTSN cumulative_tsn_ack,
  ...
  stream_reset_breakpoint_tsns_.erase(
      stream_reset_breakpoint_tsns_.begin(),
      stream_reset_breakpoint_tsns_.upper_bound(cumulative_tsn_ack.next_value()));
}
...
void OutstandingData::BeginResetStreams() {
  if (!outstanding_data_.empty()) {
    RTC_DLOG(LS_VERBOSE) << "Adding a stream-reset-breakpoint-tsn at "
                         << *next_tsn_.Wrap();
