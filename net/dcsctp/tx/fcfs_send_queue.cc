...
void FCFSSendQueue::Add(TimeMs now,
  ...
  if (send_options.lifetime.has_value()) {
    expires_at = TimeMs(*now + **send_options.lifetime);
  }
...
void FCFSSendQueue::Discard(IsUnordered unordered,
                            StreamID stream_id,
                            MID message_id) {
  // Can only discard the topmost item - it's the only partial.
  if (!items_.empty()) {
...
void FCFSSendQueue::PrepareResetStreams(
  ...
  // Will not discard partially sent messages - only whole messages.
