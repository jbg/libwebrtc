...
// schedule them to be retransmitted if necessary. Chunks are retransmitted when
// they have been lost for a number of consecutive SACKs, or when the
...
// in-flight (send, but not yet acknowledged).
class RetransmissionQueue {
 public:
  ...
  // the first TSN to use for sent fragments. It will poll data from `producer`
  // and call `on_chunk_producer_empty` when it is empty. When SACKs are
  // received, it will estimate the RTT, and call `on_new_rtt`. When an
