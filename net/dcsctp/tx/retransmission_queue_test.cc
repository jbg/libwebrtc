...
TEST_F(RetransmissionQueueTest, InitialAckedPrevTsn) {
  ...
  EXPECT_THAT(buf.GetChunkStates(), ElementsAre(Pair(TSN(9), State::kAcked)));
}
...
TEST_F(RetransmissionQueueTest, SendOneChunkAndAck) {
  ...
  EXPECT_THAT(buf.GetChunkStates(), ElementsAre(Pair(TSN(10), State::kAcked)));
}
...
TEST_F(RetransmissionQueueTest, ValidateCumTsnAtRest) {
  ...
  EXPECT_FALSE(buf.IsAcknowledgeValid(SackChunk(TSN(8), kArwnd, {}, {})));
  EXPECT_TRUE(buf.IsAcknowledgeValid(SackChunk(TSN(9), kArwnd, {}, {})));
  EXPECT_FALSE(buf.IsAcknowledgeValid(SackChunk(TSN(10), kArwnd, {}, {})));
}
...
TEST_F(RetransmissionQueueTest, ValidateCumTsnAckOnInflightData) {
  ...
  EXPECT_TRUE(buf.IsAcknowledgeValid(SackChunk(TSN(12), kArwnd, {}, {})));
  EXPECT_TRUE(buf.IsAcknowledgeValid(SackChunk(TSN(13), kArwnd, {}, {})));
  EXPECT_TRUE(buf.IsAcknowledgeValid(SackChunk(TSN(14), kArwnd, {}, {})));
  EXPECT_TRUE(buf.IsAcknowledgeValid(SackChunk(TSN(15), kArwnd, {}, {})));
  EXPECT_TRUE(buf.IsAcknowledgeValid(SackChunk(TSN(16), kArwnd, {}, {})));
  EXPECT_TRUE(buf.IsAcknowledgeValid(SackChunk(TSN(17), kArwnd, {}, {})));
  EXPECT_FALSE(buf.IsAcknowledgeValid(SackChunk(TSN(18), kArwnd, {}, {})));
}
...
TEST_F(RetransmissionQueueTest, StaysWithinAvailableSize) {
  ...
  RetransmissionQueue buf(
      "", TSN(10), kArwnd, &producer_, on_rtt_.AsStdFunction(),
      on_outgoing_message_buffer_empty_.AsStdFunction(),
      on_clear_retransmission_counter_.AsStdFunction(), timer_.get(), options);
  ...
  std::vector<std::pair<TSN, Data>> chunks_to_send =
      buf.GetChunksToSend(now_, 1188 - 12);
  EXPECT_THAT(chunks_to_send, ElementsAre(Pair(TSN(10), _), Pair(TSN(11), _)));
}
