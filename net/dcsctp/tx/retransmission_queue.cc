...
RetransmissionQueue::RetransmissionQueue(
    ...
    : options_(options),
      ...
      last_cumulative_tsn_ack_(tsn_unwrapper_.Unwrap(
          handover_state ? TSN(handover_state->tx.last_cumulative_tsn_ack)
...
bool RetransmissionQueue::NackItem(UnwrappedTSN cumulative_tsn_ack,
                                   TxData& item,
                                   bool retransmit_now) {
  ...
  switch (item.Nack(retransmit_now)) {
    ...
    case TxData::NackAction::kRetransmit:
      to_be_retransmitted_.insert(cumulative_tsn_ack);
      RTC_DLOG(LS_VERBOSE) << log_prefix_ << *cumulative_tsn_ack.Wrap()
...
void RetransmissionQueue::AddHandoverState(DcSctpSocketHandoverState& state) {
  ...
  state.tx.last_cumulative_tsn_ack = last_cumulative_tsn_ack_.Wrap().value();
}
}  // namespace dcsctp
