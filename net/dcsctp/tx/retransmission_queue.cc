...
RetransmissionQueue::RetransmissionQueue(
    ...
    : options_(options),
      ...
      on_chunk_producer_empty_(std::move(on_chunk_producer_empty)),
...
void RetransmissionQueue::RemoveAcked(AckInfo& ack_info,
...
void RetransmissionQueue::AckGapBlocks(
  ...
  for (auto& block : gap_ack_blocks) {
    auto start =
        outstanding_data_.lower_bound(cumulative_tsn_ack.AddTo(block.start));
    auto end =
        outstanding_data_.upper_bound(cumulative_tsn_ack.AddTo(block.end));
    for (auto iter = start; iter != end; ++iter) {
      if (iter->second.state != State::kAcked) {
        ack_info.bytes_acked_by_new_gap_ack_blocks += iter->second.data.size();
        iter->second.state = State::kAcked;
        ack_info.htna = std::max(ack_info.htna, iter->first);
...
void RetransmissionQueue::NackBetweenAckBlocks(
  ...
  UnwrappedTSN max_tsn_to_nack = ack_info.htna;
  if (is_in_fast_recovery() && cumulative_tsn_ack > last_cumulative_tsn_ack_) {
    ...
    max_tsn_to_nack = cumulative_tsn_ack.AddTo(
        gap_ack_blocks.empty() ? 0 : gap_ack_blocks.rbegin()->end);
  }

  UnwrappedTSN prev_block_last_acked = cumulative_tsn_ack;
  for (auto& block : gap_ack_blocks) {
    UnwrappedTSN cur_block_first_acked = cumulative_tsn_ack.AddTo(block.start);
    for (auto iter = outstanding_data_.upper_bound(prev_block_last_acked);
      ...
      if (iter->first <= max_tsn_to_nack) {
        ++iter->second.nack_count;
        if (iter->second.nack_count >= kNumberOfNacksForRetransmission) {
          iter->second.state = State::kToBeRetransmitted;
          ack_info.has_packet_loss = true;
          RTC_DLOG(LS_VERBOSE) << log_prefix_ << *iter->first.Wrap()
                               << " marked for retransmission";
        } else {
          iter->second.state = State::kNacked;
        }
      }
    }
    prev_block_last_acked = cumulative_tsn_ack.AddTo(block.end);
  }
  ...
  // up until the HTNA (see above) should be considered when NACKing.
}
...
void RetransmissionQueue::HandlePacketLoss(UnwrappedTSN htna) {
...
void RetransmissionQueue::StopT3RtxTimerOnIncreasedCumulativeTsnAck(
  ...
  if (cumulative_tsn_ack > last_cumulative_tsn_ack_) {
    ...
    // https://tools.ietf.org/html/rfc4960#section-7.2.4
    // "Restart the T3-rtx timer only if the last SACK
    // acknowledged the lowest outstanding TSN number sent to that address, or
    // the endpoint is retransmitting the first outstanding DATA chunk sent to
    // that address."
    t3_rtx_.Stop();
    // Note: It may be started again in `StartT3RtxTimerIfOutstandingData`
  }
}
...
bool RetransmissionQueue::IsAcknowledgeValid(const SackChunk& sack) const {
  ...
  //
  // Note: Important not to drop SACKs with identical TSN as previously, as the
  // gap ack blocks or dup tsn fields may have changed.
  UnwrappedTSN cumulative_tsn_ack =
  ...
  if (cumulative_tsn_ack < last_cumulative_tsn_ack_) {
    ...
    //
    // Note: Important not to drop SACKs with identical TSN as previously, as
    // the gap ack blocks or dup tsn fields may have changed.
    return false;
...
void RetransmissionQueue::HandleAcknowledge(TimeMs now, const SackChunk& sack) {
  ...
  RemoveAcked(ack_info, cumulative_tsn_ack);

  // ACK packets reported in the gap ack blocks
  AckGapBlocks(ack_info, cumulative_tsn_ack, sack.gap_ack_blocks());

  // NACK and possibly mark for retransmit chunks that weren't acked.
  NackBetweenAckBlocks(ack_info, cumulative_tsn_ack, sack.gap_ack_blocks());
  ...
  StopT3RtxTimerOnIncreasedCumulativeTsnAck(cumulative_tsn_ack);
  ...
  if (ack_info.has_packet_loss) {
    ...
    HandlePacketLoss(ack_info.htna);
  }
...
void RetransmissionQueue::UpdateRTT(TimeMs now,
  ...
  // Consider occasionally sending DATA chunks with I-bit set and use only those
  // packets for measurement.

  auto it = outstanding_data_.find(cumulative_tsn_ack);
  if (it != outstanding_data_.end()) {
    if (it->second.num_retransmissions == 0) {
      ...
      // later instance)"
      DurationMs rtt = DurationMs(*now - *it->second.time_sent);
      on_new_rtt_(rtt);
    }
  }
}
  ...
  outstanding_bytes_ = absl::c_accumulate(
      ...
      [&](size_t r, const std::pair<const UnwrappedTSN, TxData>& d) {
        ...
        return r + (d.second.state == State::kInFlight
                        ? d.second.data.size() + data_chunk_header_size_
                        : 0);
      });
}
...
void RetransmissionQueue::HandleT3RtxTimerExpiry() {
  ...
  for (auto& elem : outstanding_data_) {
    ...
    if (item.state == State::kInFlight || item.state == State::kNacked) {
      ...
      item.state = State::kToBeRetransmitted;
      ++count;
    }
  }
...
RetransmissionQueue::GetChunksToBeRetransmitted(size_t max_size) {
  ...
  for (auto& elem : outstanding_data_) {
    ...
    size_t serialized_size = item.data.size() + data_chunk_header_size_;
    if (item.state == State::kToBeRetransmitted &&
        serialized_size <= max_size) {
      item.state = State::kInFlight;
      item.nack_count = 0;
      ++item.num_retransmissions;
  ...
  if (is_in_fast_retransmit()) {
    ...
    size_t to_be_sent_bytes = absl::c_accumulate(
        to_be_sent, 0, [&](size_t r, const std::pair<TSN, Data>& d) {
          return r + d.second.size() + data_chunk_header_size_;
        });
    ...
    size_t max_bytes = std::min(std::min(bytes_remaining_in_packet, rwnd()),
    ...
    while (max_bytes > data_chunk_header_size_) {
      absl::optional<SendQueue::DataToSend> chunk_opt =
          producer_.Produce(now, max_bytes - data_chunk_header_size_);
      if (!chunk_opt.has_value()) {
        on_chunk_producer_empty_();
        break;
      }
      ...
      size_t chunk_size = chunk_opt->data.size() + data_chunk_header_size_;
...
RetransmissionQueue::GetChunkStates() const {
...
void RetransmissionQueue::ExpireChunks(TimeMs now) {
  for (auto& elem : outstanding_data_) {
    ...
    if (item.state != State::kAcked && item.state != State::kAbandoned) {
      if (item.max_retransmissions.has_value() &&
          item.num_retransmissions >= *item.max_retransmissions) {
        RTC_DLOG(LS_VERBOSE)
            << "Marking chunk " << *tsn.Wrap() << " and message "
            << *item.data.message_id << " as abandoned by max_retransmissions";
        ExpireAllFor(item);
      } else if (item.expires_at.has_value() && *item.expires_at <= now) {
        RTC_DLOG(LS_VERBOSE)
            << "Marking chunk " << *tsn.Wrap() << " and message "
            << *item.data.message_id << " as abandoned by lifetime expire";
        ExpireAllFor(item);
      }
    }
  }
...
void RetransmissionQueue::ExpireAllFor(
  ...
  producer_.Discard(item.data.is_unordered, item.data.stream_id,
...
void RetransmissionQueue::PrepareResetStreams(
  ...
  // TODO(boivie) These calls are now only affecting the chunk producer. The
  // packet buffer can also change behavior - for example draining the chunk
  // producer and eagerly assign TSNs so that a OutgoingSSNResetRequestParameter
  // can be sent quickly, with a known sender_last_assigned_tsn.
  producer_.PrepareResetStreams(streams);
}
bool RetransmissionQueue::CanResetStreams() const {
  return producer_.CanResetStreams();
}
void RetransmissionQueue::CommitResetStreams() {
  producer_.CommitResetStreams();
}
void RetransmissionQueue::RollbackResetStreams() {
  producer_.RollbackResetStreams();
}
