...
class RandomFuzzedChunk : public Chunk {
 public:
  RandomFuzzedChunk(FuzzState& state) : state_(state) {}
  void SerializeTo(std::vector<uint8_t>& out) const override {
    int bytes = state_.GetByte();
...
std::vector<uint8_t> GeneratePacket(FuzzState& state) {
  ...
  for (int i = 0; i < 5 && builder.bytes_remaining() > options.mtu * 0.1; ++i) {
  ...
  while (!state.empty()) {
    switch (state.GetByte()) {
      ...
      case 6: {
        ...
        size_t payload_exponent = (flags >> 2) % 0xF;
        size_t payload_size = 1 << payload_exponent;
