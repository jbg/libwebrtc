...
TEST_F(StreamResetHandlerTest, ChunkWithNoParametersReturnsError) {
  EXPECT_FALSE(handler_.Process(ReConfigChunk(Parameters())).has_value());
}
...
TEST_F(StreamResetHandlerTest, ChunkWithInvalidParametersReturnsError) {
  ...
  EXPECT_FALSE(handler_.Process(ReConfigChunk(builder.Build())).has_value());
}
...
TEST_F(StreamResetHandlerTest, ResetStreamsNotDeferred) {
  ...
  ASSERT_HAS_VALUE_AND_ASSIGN(
      ...
      handler_.Process(ReConfigChunk(builder.Build())));
...
TEST_F(StreamResetHandlerTest, SendOutgoingResettingOnPositiveResponse) {
  ...
  ASSERT_HAS_VALUE_AND_ASSIGN(
      std::vector<ReconfigurationResponseParameter> responses,
      handler_.Process(response_reconfig));
...
TEST_F(StreamResetHandlerTest, SendOutgoingResetRetransmitOnInProgress) {
  ...
  ReConfigChunk reconfig2 = handler_.MakeReconfigChunk();
  ASSERT_HAS_VALUE_AND_ASSIGN(
  ...
  EXPECT_THAT(req2.stream_ids(), UnorderedElementsAre(StreamID(42)));
}
...
TEST_F(StreamResetHandlerTest, SendSameRequestTwiceReturnsNothingToDo) {
  ...
  ASSERT_HAS_VALUE_AND_ASSIGN(
      ...
      handler_.Process(ReConfigChunk(builder1.Build())));
  ...
  ASSERT_HAS_VALUE_AND_ASSIGN(
      ...
      handler_.Process(ReConfigChunk(builder2.Build())));
