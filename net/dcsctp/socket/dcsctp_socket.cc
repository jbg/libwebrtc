...
bool DcSctpSocket::IsConsistent() const {
  ...
  inconsistent |= (tcb_ != nullptr) &&
                  (state_ == State::kClosed || state_ == State::kCookieWait);
  inconsistent |= (tcb_ == nullptr) &&
                  (state_ != State::kClosed && state_ != State::kCookieWait);

  inconsistent |= t1_init_->is_running() && (state_ != State::kCookieWait);
  inconsistent |= !t1_init_->is_running() && (state_ == State::kCookieWait);

  inconsistent |= t1_cookie_->is_running() && (state_ != State::kCookieEchoed);
  inconsistent |= !t1_cookie_->is_running() && (state_ == State::kCookieEchoed);

  inconsistent |=
      t2_shutdown_->is_running() &&
      (state_ != State::kShutdownSent && state_ != State::kShutdownAckSent);
  inconsistent |=
      !t2_shutdown_->is_running() &&
      (state_ == State::kShutdownSent || state_ == State::kShutdownAckSent);

  inconsistent |=
      !cookie_echo_chunk_.has_value() && (state_ == State::kCookieEchoed);
  inconsistent |= cookie_echo_initiate_tag_ == VerificationTag(0) &&
                  (state_ == State::kCookieEchoed);

  if (inconsistent) {
    RTC_DLOG(LS_ERROR)
        << log_prefix() << "tcb=" << ((tcb_ == nullptr) ? "unset" : "set")
        << ", t1_init=" << (t1_init_->is_running() ? "running" : "idle")
        << ", t1_cookie=" << (t1_cookie_->is_running() ? "running" : "idle")
        << ", t2_shutdown=" << (t2_shutdown_->is_running() ? "running" : "idle")
        << " - inconsistent";
  }
  return !inconsistent;
}
