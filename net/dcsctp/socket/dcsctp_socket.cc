...
TieTag MakeTieTag(DcSctpSocketCallbacks& cb) {
  uint32_t tie_tag_upper = cb.GetRandomInt(0, 4294967295);
  uint32_t tie_tag_lower = cb.GetRandomInt(1, 4294967295);
...
void DcSctpSocket::MakeConnectionParameters() {
  ...
  VerificationTag new_verification_tag(callbacks_.GetRandomInt(1, 4294967295));
  // https://tools.ietf.org/html/rfc4960#section-3.3.2
  TSN initial_tsn(callbacks_.GetRandomInt(0, 4294967295));
...
void DcSctpSocket::CloseConnectionByTooManyTransmissionErrors() {
...
bool DcSctpSocket::ValidateVerificationTag(const SctpPacket& packet) {
...
void DcSctpSocket::HandleTimeout(TimeoutID timeout_id) {
  ...
  if (tcb_ != nullptr && tcb_->HasTooManyTxErrors()) {
    ...
    CloseConnectionByTooManyTransmissionErrors();
  }
...
void DcSctpSocket::ReceivePacket(rtc::ArrayView<const uint8_t> data) {
  if (data.empty()) {
    // Silently discard empty packets. Quite common in tests.
    return;
  }
...
void DcSctpSocket::DebugPrintIncoming(const SctpPacket::ChunkDescriptor& desc) {
  RTC_DLOG(LS_VERBOSE) << log_prefix_ << "Received "
...
void DcSctpSocket::DebugPrintOutgoing(rtc::ArrayView<const uint8_t> payload) {
  ...
  for (const auto& desc : packet->descriptors()) {
    RTC_DLOG(LS_VERBOSE) << log_prefix_ << "Sent "
                         << DebugConvertChunkToString(desc.data);
  }
}
...
bool DcSctpSocket::HandleUnrecognizedChunk(
  ...
  RTC_DLOG(LS_VERBOSE) << log_prefix_ << "Received unknown chunk: "
  ...
  if (report_as_error) {
    ...
    RTC_DLOG(LS_VERBOSE) << log_prefix_
                         << "Chunk type has 'report_as_error' flag set";
  ...
  if (continue_processing) {
    ...
    RTC_DLOG(LS_VERBOSE) << log_prefix_
                         << "Chunk type has 'stop_processing' flag set";
  }
...
absl::optional<DurationMs> DcSctpSocket::OnInitTimerExpiry() {
  ...
  RTC_DLOG(LS_VERBOSE) << log_prefix_ << "Timer " << t1_init_->name()
...
absl::optional<DurationMs> DcSctpSocket::OnCookieTimerExpiry() {
  ...
  RTC_DLOG(LS_VERBOSE) << log_prefix_ << "Timer " << t1_cookie_->name()
...
void DcSctpSocket::HandleData(const CommonHeader& header,
  ...
  if (ValidateParseSuccess(chunk) && ValidateHasTCB()) {
    AnyDataChunk::ImmediateAckFlag immediate_ack =
        chunk->options().immediate_ack;
    TSN tsn = chunk->tsn();
    HandleData(tsn, std::move(*chunk).extract(), immediate_ack);
  }
}
...
void DcSctpSocket::HandleInit(const CommonHeader& header,
  ...
  if (state_ == State::kClosed) {
    ...
    for (int tries = 0; tries < 10; ++tries) {
      connect_params_.verification_tag =
          VerificationTag(callbacks_.GetRandomInt(1, 4294967295));
...
void DcSctpSocket::SendCookieEcho() {
  SctpPacket::Builder b(cookie_echo_initiate_tag_, options_);
...
void DcSctpSocket::HandleInitAck(
  ...
  if (state_ != State::kCookieWait) {
    ...
    RTC_DLOG(LS_VERBOSE) << log_prefix_
                         << "Received INIT_ACK in state=" << ToString(state_);
    return;
  }
  ...
  state_ = State::kCookieEchoed;
  cookie_echo_initiate_tag_ = chunk->initiate_tag();
...
void DcSctpSocket::HandleCookieEcho(
  ...
  if (tcb_ != nullptr) {
    ...
    if ((header.verification_tag != tcb_->my_verification_tag()) &&
      ...
      RTC_DLOG(LS_VERBOSE) << log_prefix_
                           << "Received CookieEcho indicating a restarted peer";
      ...
      RTC_DLOG(LS_VERBOSE)
          << log_prefix_
          << "Received CookieEcho indicating simultaneous connections";
      tcb_ = nullptr;
    } else if ((header.verification_tag != tcb_->my_verification_tag()) &&
               (tcb_->peer_verification_tag() == cookie->initiate_tag()) &&
      ...
      RTC_DLOG(LS_VERBOSE)
          << log_prefix_
          << "Received CookieEcho indicating a late CookieEcho. Discarding";
      ...
      // ACK."
      RTC_DLOG(LS_VERBOSE) << log_prefix_
                           << "Received CookieEcho indicating ???";
    }
  } else {
    if (header.verification_tag != connect_params_.verification_tag) {
      callbacks_.OnError(
...
void DcSctpSocket::HandleCookieAck(
  ...
  t1_cookie_->Stop();
  cookie_echo_initiate_tag_ = VerificationTag(0);
...
void DcSctpSocket::HandleData(TSN tsn,
                              Data data,
                              AnyDataChunk::ImmediateAckFlag immediate_ack) {
  ...
  RTC_DLOG(LS_VERBOSE) << log_prefix_ << "Handle DATA, queue_size="
  ...
  if (tcb_->reassembly_queue().is_above_watermark()) {
    RTC_DLOG(LS_VERBOSE) << log_prefix_ << "Is above high watermark";
...
void DcSctpSocket::HandleAbort(const CommonHeader& header,
  ...
  if (ValidateParseSuccess(chunk)) {
    ...
    if (tcb_ == nullptr) {
      ...
      RTC_DLOG(LS_VERBOSE) << log_prefix_ << "Received ABORT (" << error_string
                           << ") on a connection with no TCB. Ignoring";
      return;
    }

    RTC_DLOG(LS_WARNING) << log_prefix_ << "Received ABORT (" << error_string
                         << ") - closing connection.";
...
void DcSctpSocket::HandleError(const CommonHeader& header,
  ...
  if (ValidateParseSuccess(chunk)) {
    std::string error_string = ErrorCausesToString(chunk->error_causes());
    if (tcb_ == nullptr) {
      RTC_DLOG(LS_VERBOSE) << log_prefix_ << "Received ERROR (" << error_string
                           << ") on a connection with no TCB. Ignoring";
      return;
    }

    RTC_DLOG(LS_WARNING) << log_prefix_ << "Received ERROR: " << error_string;
...
void DcSctpSocket::HandleForwardTsn(const AnyForwardTsnChunk& chunk) {
...
void DcSctpSocket::MaybeSendShutdownOrAck() {
  if (state_ == State::kShutdownPending &&
      tcb_->retransmission_queue().outstanding_bytes() == 0) {
    // https://tools.ietf.org/html/rfc4960#section-9.2
