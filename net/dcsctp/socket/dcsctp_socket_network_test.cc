...
TEST_F(DcSctpSocketNetworkTest, DCSCTP_NDEBUG_TEST(CanSendPRMessagesWithLoss)) {
  DcSctpOptions options = MakeOptionsForTest();
  SctpActor sender("A", send_socket_, options);
  SctpActor receiver("Z", recv_socket_, options);
  ...
  SIMULATED_WAIT(false, 6000, clock_);
  ...
  EXPECT_THAT(bitrate, AllOf(Ge(20), Le(40)));
}

TEST_F(DcSctpSocketNetworkTest,
       DCSCTP_NDEBUG_TEST(CanSendMessagesReliablyWithLoss)) {
  DcSctpOptions options = MakeOptionsForTest();
  SctpActor sender("A", send_socket_, options);
  SctpActor receiver("Z", recv_socket_, options);
  ...
  receiver.SetActorMode(ActorMode::kThroughputReceiver);
  ss_.set_drop_probability(0.00001);
  ...
  // Verify that the bitrates are in the range of 120-160 Mbps.
  double bitrate = receiver.avg_received_bitrate_mbps();
  EXPECT_THAT(bitrate, AllOf(Ge(120), Le(160)));
}
