...
class SctpActor : public rtc::MessageHandlerAutoCleanup,
 ...
 public:
  ...
  void OnMessage(rtc::Message* pmsg) override {
    if (pmsg->message_id == static_cast<uint32_t>(MessageId::kPrintBandwidth)) {
      ...
      RTC_LOG(LS_INFO) << log_prefix() << "Received "
                       << rtc::StringFormat("%lu kB, %0.2f Mbps",
                                            (bytes / 1000), bitrate_mbps);
  ...
  const std::vector<double>& received_bitrate_mbps() const {
    return received_bitrate_mbps_;
  }
...
TEST_F(DcSctpSocketNetworkTest, DCSCTP_NDEBUG_TEST(CanSendMessagesWithLoss)) {
  ...
  ASSERT_THAT(bitrates, SizeIs(Ge(10u)));
  // Remove the first three seconds, that are needed for the congestion control
  // to ramp up. The last second isn't complete, so it will be removed as well.

  bitrates.erase(bitrates.begin(), bitrates.begin() + 3);
  bitrates.pop_back();
