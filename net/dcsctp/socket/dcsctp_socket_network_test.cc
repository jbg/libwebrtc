...
class SctpActor : public rtc::MessageHandlerAutoCleanup,
 ...
 public:
  ...
  TimeMs TimeMillis() override { return TimeMs(rtc::TimeNanos() / 1'000'000); }
 ...
 protected:
  explicit DcSctpSocketNetworkTest(bool enable_message_interleaving = false)
      : options_(MakeOptionsForTest()),
        ...
        sender_("A", rtc::Thread::Current(), send_socket_, options_),
        receiver_("Z", rtc::Thread::Current(), recv_socket_, options_) {
...
// These tests are too slow to run in debug mode unfortunately, due to extensive
