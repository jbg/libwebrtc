...
#if defined(NDEBUG) || defined(RUN_EXPENSIVE_TESTS_IN_DEBUG_MODE)
...
class ActorSocket : public webrtc::EmulatedNetworkReceiverInterface {
 public:
  ...
  void SetDestination(const ActorSocket& socket) {
    dest_address_ = socket.source_address_;
  }

  webrtc::EmulatedEndpoint* endpoint() { return endpoint_; }
  const rtc::SocketAddress& source_address() const { return source_address_; }
  const rtc::SocketAddress& dest_address() const { return source_address_; }
 ...
 protected:
  DcSctpSocketNetworkTest()
      : options_(MakeOptionsForTest()),
        emulation_(webrtc::TimeMode::kSimulated) {}
  ...
  void MakeNetwork(const webrtc::BuiltInNetworkBehaviorConfig& config) {
    ...
    auto* node1 = emulation_.CreateEmulatedNode(
        std::make_unique<webrtc::SimulatedNetwork>(config));

    auto* node2 = emulation_.CreateEmulatedNode(
        std::make_unique<webrtc::SimulatedNetwork>(config));
  ...
  webrtc::test::NetworkEmulationManagerImpl emulation_;
  ActorSocket emulated_socket_a_;
  ActorSocket emulated_socket_z_;
};
