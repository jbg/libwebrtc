...
struct DcSctpOptions {
  ...
  // For t1-init, t1-cookie, t2-shutdown, t3-rtx, re-config, this value -
  // if set - will be the upper bound on how large the exponentially backed off
  // timeout can become. The lower the duration, the faster the connection can
  // recover on transient network issues. Setting this value may require
  // changing `max_retransmissions` and `max_init_retransmits` to ensure that
  // the connection is not closed too quickly.
