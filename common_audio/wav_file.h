...
// Interface to provide scoped access WAV formats.
class WavFile {
 public:
  enum class SampleFormats { kInt16, kFloat };
};

// Simple C++ class for writing 16-bit PCM and 32 bit floating point WAV files.
// All error handling is by calls to RTC_CHECK(), making it unsuitable for
// anything but debug code.
class WavWriter final : public WavFile {
 public:
  ...
  size_t num_samples() const { return num_samples_written_; }
...
class WavReader final : public WavFile {
 public:
  ...
  int sample_rate() const { return sample_rate_; }
  size_t num_channels() const { return num_channels_; }
  size_t num_samples() const { return num_samples_in_file_; }
