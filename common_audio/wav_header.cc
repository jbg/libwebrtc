...
static_assert(rtc::sanitizer_impl::IsTriviallyCopyable<uint32_t>(), "");
...
static_assert(rtc::sanitizer_impl::IsTriviallyCopyable<ChunkHeader>(), "");

struct RiffHeader {
  RiffHeader(const RiffHeader&) = default;
  RiffHeader& operator=(const RiffHeader&) = default;
  ChunkHeader header;
  uint32_t Format;
};
static_assert(rtc::sanitizer_impl::IsTriviallyCopyable<RiffHeader>(), "");
...
struct FmtSubchunk {
  FmtSubchunk(const FmtSubchunk&) = default;
  FmtSubchunk& operator=(const FmtSubchunk&) = default;
  ChunkHeader header;
...
static_assert(rtc::sanitizer_impl::IsTriviallyCopyable<FmtSubchunk>(), "");

// We can't nest this definition in WavHeader because when using GCC
// rtc::sanitizer_impl::IsTriviallyCopyable fails.
struct DataHeader {
  DataHeader(const DataHeader&) = default;
  DataHeader& operator=(const DataHeader&) = default;
  ChunkHeader header;
};
static_assert(rtc::sanitizer_impl::IsTriviallyCopyable<DataHeader>(), "");
...
// TODO(alessiob): Remove once WavHeader trivially copyable issue is fixed.
...
static_assert(rtc::sanitizer_impl::IsTriviallyCopyable<WavHeader>(), "");
