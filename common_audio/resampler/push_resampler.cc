...
int PushResampler<T>::Resample(InterleavedView<const T> src,
  ...
  RTC_DCHECK_EQ(src.data().size(), src_size_10ms);
  RTC_DCHECK_GE(dst.size(), dst_size_10ms);

  if (src_sample_rate_hz_ == dst_sample_rate_hz_) {
    // The old resampler provides this memcpy facility in the case of matching
    // sample rates, so reproduce it here for the sinc resampler.
    // TODO: b/335805780 - Support a copy operation in the view that checks
    // the target view size as well.
    memcpy(dst.data(), src.data().data(), src.data().size() * sizeof(T));
  ...
  Deinterleave(src.data().data(), src.samples_per_channel(), src.num_channels(),
  ...
  for (auto& resampler : channel_resamplers_) {
    dst_length_mono = resampler.resampler->Resample(
        ...
        resampler.destination.data(), dst_capacity_mono);
  }
